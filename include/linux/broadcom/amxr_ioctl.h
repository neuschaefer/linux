...
struct amxr_ioctl_get_portlist
{
   AMXR_PORTS          *portlist;    /*<< (o) ptr to store cnx list */
   int                  maxlen;     /*<< (i) max length in bytes */
};
...
#define AMXR_IOCTL_GET_PORTLIST  _IOR( AMXR_MAGIC_TYPE, AMXR_CMD_GET_PORTLIST, struct amxr_ioctl_get_portlist )
/* ---- Variable Externs ------------------------------------------ */
