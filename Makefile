...
CONSTIFY_PLUGIN := -DCONSTIFY_PLUGIN
...
gcc-plugins-build:
# 	Kernel build as it is configured currently can't build constify_plugin with standard 
# 	command $(Q)$(MAKE) $(build)=tools/gcc, and that's why we use special command below.
	@(cd tools/gcc &&\
		$(HOSTCC) -export-dynamic -shared -m32 -fPIC -DSONAME=\"constify_plugin.so\" -o constify_plugin.so -fPIC -I$(CROSSTOOLS_PLUGIN_INC) constify_plugin.c \
	)
