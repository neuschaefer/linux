    ...
    for(index = 0; index < CHAL_CAPH_SRCM_MAX_FIFOS; index++)
    {
        ...
        if((1UL << index)&fifo)
        {
            ...
            else
            {
                ...
                BRCM_WRITE_REG_IDX( srcm_cb->base,  SRCMIXER_SRC_CHANNEL1_CTRL1,  (srcm_cb->ctrl_addr[index]/sizeof(cUInt32)), reg_val);
                /* Clear output FIFO */
                reg_val = BRCM_READ_REG_IDX( srcm_cb->base,  SRCMIXER_SRC_CHANNEL1_CTRL2,  (srcm_cb->ctrl_addr[index]/sizeof(cUInt32)));

                reg_val |= SRCMIXER_SRC_CHANNEL1_CTRL2_TAPSDOWNFIFO1_CLEAR_MASK;
                BRCM_WRITE_REG_IDX( srcm_cb->base,  SRCMIXER_SRC_CHANNEL1_CTRL2,  (srcm_cb->ctrl_addr[index]/sizeof(cUInt32)), reg_val);
