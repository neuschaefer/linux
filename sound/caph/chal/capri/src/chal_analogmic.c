...
#define KONA_PAD_CTRL_VA 			HW_IO_PHYS_TO_VIRT(PAD_CTRL_BASE_ADDR) //defn of KONA_PAD_CTRL in io_map.h mapped to CHIPCTRL_BASE 
#endif
 ...
 * ============================================================================
 *
 *  Function Name: void chal_audio_mic_input_select(CHAL_HANDLE handle, cUInt16 mic_input)
 *
...
void chal_audio_mic_input_select(CHAL_HANDLE handle, cUInt16 mic_input)
{
	...
	/* add the code from Rhea CHAL to be in complaint. Later check, if we can re-use the  chal_audio_enable_aci_auxmic() function.
	Before making use this of this function, chal_aci_auxmic_init needs to be called.*/
...
void chal_audio_mic_pga(CHAL_HANDLE handle, cUInt32 gain_regval)
{
	...
	if(gain_regval > 0x3f)	gain_regval = 0x3f;
	if(gain_regval < 0x00) gain_regval = 0;
 ...
 * ============================================================================
 *
 *  Function Name: void chal_audio_mic_pga_get_gain(CHAL_HANDLE handle, cUInt32 *gain_regval)
 *
 *  Description:   Set gain on analog microphone path
 * 
 *  Parameters:
...
void chal_audio_mic_pwrctrl(CHAL_HANDLE handle, _Bool pwronoff)
{
    ...
    if(pwronoff == TRUE)
    {
        ...
        reg_val &= ~(AUDIOH_AUDIORX_VMIC_AUDIORX_VAUXMIC_CTRL_MASK);
        reg_val |= (3 << AUDIOH_AUDIORX_VMIC_AUDIORX_VAUXMIC_CTRL_SHIFT);
        ...
        /* AUDIORX_VRX2/AUDIORX_VMIC bias to 2.1 volts
	    ...
	    3 => 2.40V
	    */
    	reg_val = BRCM_READ_REG(base, AUDIOH_AUDIORX_VRX2);
	    reg_val &= ~(AUDIOH_AUDIORX_VRX2_AUDIORX_VMIC_CTRL_MASK);
	    reg_val |= (0 << AUDIOH_AUDIORX_VRX2_AUDIORX_VMIC_CTRL_SHIFT);

        BRCM_WRITE_REG(base,  AUDIOH_AUDIORX_VRX2, reg_val);
    ...
    else
    {
        ...
        reg_val &= ~(AUDIOH_AUDIORX_VMIC_AUDIORX_MIC_EN_MASK);
        reg_val |= (AUDIOH_AUDIORX_VMIC_AUDIORX_VAUXMIC_CTRL_MASK);
        reg_val |= (0 << AUDIOH_AUDIORX_VMIC_AUDIORX_VAUXMIC_CTRL_SHIFT);
        BRCM_WRITE_REG(base,  AUDIOH_AUDIORX_VMIC, reg_val);
 ...
 * ============================================================================
 * 
 *  Function Name: void chal_audio_dmic1_pwrctrl(CHAL_HANDLE handle, _Bool pwronoff)
 * 
 ...
 * ============================================================================
 * 
 *  Function Name: void chal_audio_dmic2_pwrctrl(CHAL_HANDLE handle, _Bool pwronoff)
 * 
 *  Description:   power on/off digital microphone path
 * 
 *  Parameters:
 ...
 * ============================================================================
 * 
 *  Function Name: void chal_audio_hs_mic_pwrctrl(CHAL_HANDLE handle, _Bool pwronoff)
 * 
...
EXPORT_SYMBOL(chal_audio_mic_pga);
...
EXPORT_SYMBOL(chal_audio_dmic1_pwrctrl);
EXPORT_SYMBOL(chal_audio_dmic2_pwrctrl);
EXPORT_SYMBOL(chal_audio_hs_mic_pwrctrl);
EXPORT_SYMBOL(chal_audio_mic_mute);
#endif
