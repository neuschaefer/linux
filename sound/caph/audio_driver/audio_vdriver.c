...
/*      if ( FALSE==vopath_enabled && FALSE==DspVoiceIfActive_DL()
	...
	if (voicePlayOutpathEnabled == FALSE) {
		...
		audio_control_dsp(AUDDRV_DSPCMD_AUDIO_ENABLE, FALSE, 0, 0,
						  0, 0);
	...
	if (AUDIO_MODE_BLUETOOTH == AUDCTRL_GetAudioMode())
		audio_control_dsp(AUDDRV_DSPCMD_AUDIO_SET_PCM, FALSE, 0, 0,
						  0, 0);
...
/*For SS multicast case always load params from mode AUDIO_MODE_SPEAKERPHONE*/
	...
	if (app >= AUDIO_APP_NUMBER)
		p1 = &(MMAudParmP()[audio_mode + app * AUDIO_MODE_NUMBER]);
	else
#endif
...
/* By default the PCM port is occupied by trace port on development board */
	if (on_off) {
		audio_control_dsp(AUDDRV_DSPCMD_COMMAND_DIGITAL_SOUND,
				on_off, 0, 0, 0, 0);
	} else {
		audio_control_dsp(AUDDRV_DSPCMD_COMMAND_DIGITAL_SOUND,
				on_off, 0, 0, 0, 0);
