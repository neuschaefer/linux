...
static CAPH_SWITCH_TRIGGER_e csl_caph_get_dev_trigger(CSL_CAPH_DEVICE_e dev)
{
	...
	case CSL_CAPH_DEV_ANALOG_MIC:
	case CSL_CAPH_DEV_HS_MIC:
	/* LEFT channel associated with DMIC2 mapped to VIN FIFO L and RIGHT channel associated with
	DMIC1 mapped to VIN FIFO R. assign trigger accordingly */
	case CSL_CAPH_DEV_DIGI_MIC_R:
	case CSL_CAPH_DEV_DIGI_MIC:
	/* verify stereo DIGITAL MIC functionality - why ADC_VOICE_FIFOR assigned to EANC? */
	case CSL_CAPH_DEV_EANC_DIGI_MIC:
	...
	case CSL_CAPH_DEV_DIGI_MIC_L:
		...
		break;
	/* LEFT channel associated with DMIC4 mapped to NVIN FIFO L and RIGHT channel associated with
		...
		CSL_CAPH_DEVICE_e source, CSL_CAPH_DEVICE_e sink)
{
    ...
    if(ssp6)
	{
		clkIDSSP[2] = clk_get(NULL, "ssp6_audio_clk");
		if(enable && !clkIDSSP[2]->use_cnt)
            clk_enable(clkIDSSP[2]);
		else if(!enable && clkIDSSP[2]->use_cnt)
			clk_disable(clkIDSSP[2]);
	}
}
#endif
...
void csl_caph_hwctrl_init(void)
{
    ...
    pcmHandleSSP = (CSL_HANDLE)csl_pcm_init
    		(addr.ssp6_baseAddr, (UInt32)caph_intc_handle);
#else
