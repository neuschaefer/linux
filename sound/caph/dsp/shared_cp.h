...
#define	MAX_DSP_SHARED_BUF_LENGTH	4096			//	This should be always  2^ numbers (1024, 2048, 4096, 8192....)	
...
#define GET_ENTRY_BSIC(entry)           ( ( (entry).entry1 >>  2 ) & 0x3f )
...
#define SET_ENTRY_BSIC(entry, m)		{( (entry).entry1 &= 0xff03 );		 \
										( (entry).entry1 |= ( (m) << 2 ) );}
...
EXTERN UInt16 RESERVED_PART1[SIZE_OF_RESERVED_ZONE-19-DSP_SYNC_LIST_SIZE]			   SHARED_SEC_PARAMET_MODEM;	
...
EXTERN UInt16 shared_not_used_0					 			 SHARED_SEC_SMC;				// Log the current HW SMC page and index when the DSP generates the RFIC interrupt
...
EXTERN UInt16 shared_not_used_1				 				 SHARED_SEC_SMC;	// 
 ...
 */
EXTERN UInt16 shared_encoder_InputBuffer[AUDIO_SIZE_PER_PAGE-1307] SHARED_SEC_GEN_AUDIO;
