			   ...
			   int vol_left, int vol_right, unsigned int pathID)
{
	...
	if (app >= AUDIO_APP_NUMBER)
		p1 = &(MMAudParmP()[mode + app * AUDIO_MODE_NUMBER]);
	else
#endif
		p = &(AudParmP()[mode + app * AUDIO_MODE_NUMBER]);
...
#if defined(USE_NEW_AUDIO_MM_PARAM)
	if (app >= AUDIO_APP_NUMBER) {
		users_gain[AUDPATH_FM].L = p1->fm_radio_digital_vol[vol_left];
		/*users_gain[AUDPATH_FM].R =
		p1->fm_radio_digital_vol[vol_right];*/
		/*vol_right is always 0. need to fix it in caph_ctl.c*/
		users_gain[AUDPATH_FM].R = p1->fm_radio_digital_vol[vol_left];
	} else{
#endif
...
#if defined(USE_NEW_AUDIO_MM_PARAM)
	if (app >= AUDIO_APP_NUMBER)
		user_vol_setting[app][mode].L = (short)p1->srcmixer_output_fine_gain_l;
	else
#endif
...
#if defined(USE_NEW_AUDIO_MM_PARAM)
	if (app >= AUDIO_APP_NUMBER)
		user_vol_setting[app][mode].R = (short)p1->srcmixer_output_fine_gain_r;
	else
#endif
