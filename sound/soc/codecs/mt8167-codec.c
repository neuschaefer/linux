			...
			struct snd_soc_dai *codec_dai)
{
	...
	dev_err(codec_dai->codec->dev, "%s command = %d\n ",
			__func__, command);
	return 0;
}
...
static int mt8167_spdifin_tunner_set(struct mt8167_codec_priv *codec_data, int value)
{
	/*
		* on: M = 0x1f4000, for SPDIF In tuner, range: [47.5,48.5], [43.6,44.6]
		* off: M = 0x1, for normal playback, range
		*/
	if (value < 0 || value > 1) {
			dev_info(codec_data->codec->dev, "%s, invalied value:%d\n", __func__, value);
			return -1;
	}

	if (value){
		snd_soc_update_bits(codec_data->codec,
				APLL1_CON1, 0x7FFFFFFF, (0x37945ea6 - 0x19CFB)); /*180.6336 -M */   //0x1f4000
		snd_soc_update_bits(codec_data->codec,
				APLL1_CON_TUNER, 0x7FFFFFFF, (0x37945ea6 + 0x19CFB));  //0x1f4000
		snd_soc_update_bits(codec_data->codec,
				APLL2_CON1, 0x7FFFFFFF, (0x3c7ea932 - 0x19CFB)); /*196.608-M*/  //0x1f4000
		snd_soc_update_bits(codec_data->codec,
				APLL2_CON_TUNER, 0x7FFFFFFF, (0x3c7ea932 + 0x19CFB));  //0x1f4000
	}else{
		snd_soc_update_bits(codec_data->codec,
				APLL1_CON1, 0x7FFFFFFF, (0x37945ea6)); /*180.6336 -M */
		snd_soc_update_bits(codec_data->codec,
				APLL1_CON_TUNER, 0x7FFFFFFF, (0x37945ea6 + 0x1));
		snd_soc_update_bits(codec_data->codec,
				APLL2_CON1, 0x7FFFFFFF, (0x3c7ea932)); /*196.608-M*/
		snd_soc_update_bits(codec_data->codec,
				APLL2_CON_TUNER, 0x7FFFFFFF, (0x3c7ea932 + 0x1));
	}
...
static int spdif_rx_apll_info(struct snd_kcontrol *kcontrol,
...
static int spdif_rx_apll_put(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
    int ret = -1;
	struct snd_soc_component *component = snd_kcontrol_chip(kcontrol);
    ...
    ret = mt8167_spdifin_tunner_set(codec_data, value);

	return ret;
}

static int spdif_rx_apll_get(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}
...
static const struct snd_kcontrol_new mt8167_codec_controls[] = {
		...
		mt8167_codec_dmic_rate_mode_put),
	{
		...
		.name = "SPDIF In Apll",
		.info = spdif_rx_apll_info,
		.get = spdif_rx_apll_get,
		.put = spdif_rx_apll_put
	},
};
...
static const struct mt8167_codec_reg_attr mt8167_codec_dump_reg_list[] = {
	...
	DUMP_REG_ENTRY(APLL1_CON0),
	DUMP_REG_ENTRY(APLL1_CON1),
	DUMP_REG_ENTRY(APLL1_CON2),
	DUMP_REG_ENTRY(APLL1_CON3),
	DUMP_REG_ENTRY(APLL1_CON_TUNER),
	DUMP_REG_ENTRY(APLL2_CON0),
	DUMP_REG_ENTRY(APLL2_CON1),
	DUMP_REG_ENTRY(APLL2_CON2),
	DUMP_REG_ENTRY(APLL2_CON3),
	DUMP_REG_ENTRY(APLL2_CON_TUNER),
};
