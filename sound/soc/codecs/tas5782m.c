			    ...
			    struct snd_soc_dai *dai)
{
	...
	printk("db setting is :0x%x\n", tas5782m->volume_db);
	if (tas5782m->volume_db == TAS5782M_INVAID_VOL) {
		regmap_update_bits(tas5782m->regmap,TAS5782M_REG_3C,0xFF,0x01);
		regmap_update_bits(tas5782m->regmap,TAS5782M_REG_3D,0xFF,0x30);
	}
		/* Set the DAI format */
...
static void tas5782m_shutdown(struct snd_pcm_substream *substream,
                              struct snd_soc_dai *dai)
{
	struct snd_soc_codec *codec = dai->codec;
...
static int tas5782m_probe(struct device *dev, struct regmap *regmap)
{
	...
	tas5782m->volume_db = TAS5782M_INVAID_VOL;
