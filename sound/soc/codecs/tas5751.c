...
struct tas5751_private {
	...
	int master_volume;
	unsigned int			format;
//	bool                            muteflag;// for mute current sound
...
#if 1
static int tas5751_startup(struct snd_pcm_substream *substream,struct snd_soc_dai *dai)
{
	struct tas5751_private *priv = snd_soc_codec_get_drvdata(dai->codec);
	int hardmute = 0x0,ret;
	int db;
        unsigned char awData[2];

    	regmap_raw_write(priv->regmap, eSysCtl2, (void *)&hardmute, sizeof(hardmute));

    	if (TAS5751_INVAID_VOL == priv->master_volume) {
		awData[0] = 0x00;
		awData[1] = 0xC0;
		...
		db = 0xC0 + (100 - priv->master_volume )* 4;

		printk("priv->master_volume :0x%x\n", db);

		db = (db&0xFF00)>>8 | (db&0xFF)<<8;

		printk("volume setting is :0x%x db\n", db);

	ret = regmap_raw_write(priv->regmap, eMasterVol, (void *)&db, sizeof(db));
	if (ret < 0) {
...
static void tas5751_shutdown(struct snd_pcm_substream *substream,struct snd_soc_dai *dai)
{
	struct tas5751_private *priv = snd_soc_codec_get_drvdata(dai->codec);
	int hardmute = 0x40,readval,ret;

    	regmap_raw_write(priv->regmap, eSysCtl2, (void *)&hardmute, sizeof(hardmute));

	ret = regmap_raw_read(priv->regmap, eSysCtl2, (void *)&readval, sizeof(readval));
	if (ret < 0) {
		printk("%s failed(%d) to set eSysCtl2 control\n",
		       __func__, ret);
	}
	printk("readval is :%x\n", readval);
			   ...
			   struct snd_ctl_elem_value *ucontrol)
{
	...
	int value, ret;
	unsigned short db;
	//int hardmute = 0x40,hardunmute=0x0;

	value = ucontrol->value.integer.value[0];
	//printk("volume setting is :%d,muteflag:%d\n", value,tas5751->muteflag);

	if (value == 0) {
		//regmap_raw_write(tas5751->regmap, eSysCtl2, (void *)&hardmute, sizeof(hardmute));
		//tas5751->muteflag = 1;
		db = 0x3FF;
	}
	else {
		#if 0
		if(tas5751->muteflag) {
		regmap_raw_write(tas5751->regmap, eSysCtl2, (void *)&hardunmute, sizeof(hardunmute));
		tas5751->muteflag = 0;
		}
#endif
		db = 0xC0 + (100 - value) * 4;
...
static int tas5751_soc_resume(struct snd_soc_codec *codec)
{
	...
	if (gpio_is_valid(priv_resume->reset_gpio))
    	...
    	mdelay(14);/*delay13.5ms after reset accoding to 5751spec*/
...
{
	int ret;
	unsigned char trimPtr = 0x0, sysCtl2Ptr = 0x40; //0x80;
			     ...
			     const struct i2c_device_id *id)
{
	...
	priv->chip = of_id->data;
    	...
    	priv->master_volume = TAS5751_INVAID_VOL;
//	priv->muteflag = 0;
