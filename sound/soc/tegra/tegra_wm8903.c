...
#if defined(CONFIG_NBX_JACK)
static int tegra_wm8903_driver_suspend(struct platform_device *pdev, pm_message_t state)
{
	nbx_jack_suspend();

	return 0;
}
static int tegra_wm8903_driver_resume(struct platform_device *pdev)
{
	nbx_jack_resume();
...
#if defined(CONFIG_NBX_JACK)
	.resume = tegra_wm8903_driver_resume,
	.suspend = tegra_wm8903_driver_suspend,
#endif
};
