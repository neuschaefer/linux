...
static const struct afe_dump_reg_attr spdif_out_dump_regs[] = {
	DUMP_REG_ENTRY(AUDIO_TOP_CON0),
	DUMP_REG_ENTRY(AFE_DAC_CON0),
	DUMP_REG_ENTRY(AFE_IRQ_MCU_CON),
	DUMP_REG_ENTRY(AFE_MEMIF_PBUF_SIZE),
	DUMP_REG_ENTRY(AFE_SPDIF_OUT_BASE),
	DUMP_REG_ENTRY(AFE_SPDIF_OUT_END),
	DUMP_REG_ENTRY(AFE_SPDIF_OUT_CUR),
...
static const struct afe_dump_reg_attr spdif_in_dump_regs[] = {
	...
	DUMP_REG_ENTRY(AFE_SPDIFIN_CLK_CFG),
	DUMP_REG_ENTRY(SPDIFIN_APLL_TUNER_CFG),
	DUMP_REG_ENTRY(SPDIFIN_APLL_TUNER_CFG1),
	DUMP_REG_ENTRY(SPDIFIN_APLL2_TUNER_CFG),
	DUMP_REG_ENTRY(SPDIFIN_APLL2_TUNER_CFG1),
	DUMP_REG_ENTRY(SPDIFIN_TYPE_DET),
};
...
static ssize_t mt8167_afe_spdif_out_read_file(struct file *file, char __user *user_buf,
				size_t count, loff_t *pos)
{
	struct mtk_afe *afe = file->private_data;
	ssize_t ret, i;
	...
	for (i = 0; i < ARRAY_SIZE(spdif_out_dump_regs); i++) {
		if (regmap_read(afe->regmap, spdif_out_dump_regs[i].offset, &reg_value))
			n += scnprintf(buf + n, count - n, "%s = N/A\n",
				       spdif_out_dump_regs[i].name);
		else
			n += scnprintf(buf + n, count - n, "%s = 0x%x\n",
				       spdif_out_dump_regs[i].name, reg_value);
	}
...
static ssize_t mt8167_afe_spdif_in_read_file(struct file *file, char __user *user_buf,
				size_t count, loff_t *pos)
{
	struct mtk_afe *afe = file->private_data;
	ssize_t ret, i;
...
static const struct file_operations mt8167_afe_spdif_out_fops = {
	.open = simple_open,
	.read = mt8167_afe_spdif_out_read_file,
	.llseek = default_llseek,
};
...
static const struct mt8167_afe_debug_fs afe_debug_fs[MT8167_AFE_DEBUGFS_NUM] = {
	...
	{"mtksocspdifoutaudio", &mt8167_afe_spdif_out_fops},
