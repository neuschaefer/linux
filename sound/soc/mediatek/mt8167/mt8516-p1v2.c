...
static void mt8516_codec_ext_hp_amp_turn_on(struct snd_soc_card *card)
{
    ...
    //wait for voltage stability to avoid pop noise
    usleep_range(AMP_ON_DELAY,AMP_ON_DELAY+1);
...
static struct snd_soc_dai_link mt8516_p1v2_dais[] = {
	...
	},
	{
		...
		.cpu_dai_name = "TDM_IN",
		.codec_name = "tlv320aic3101.0-0018", //i2c0 addr 0x18
...
static int mt8516_p1_v2_dev_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < card->num_links; i++) {
		if (tdmin_adc_node && (strcmp(mt8516_p1v2_dais[i].cpu_dai_name, "TDM_IN") == 0)) {
			mt8516_p1v2_dais[i].codec_of_node = tdmin_adc_node;
			mt8516_p1v2_dais[i].codec_name = NULL;
			printk("mt8516_p1v2_dais[%d].codec_name: %s\n", i, mt8516_p1v2_dais[i].codec_name);
			continue;
		}
