...
static struct snd_soc_dai_link mt8167s_som_dais[] = {
	...
	},
	{
		...
		.cpu_dai_name = "TDM_IN",
		.codec_name = "tlv320aic3101.1-0018", //i2c1 addr 0x18
...
static int mt8167s_som_dev_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < card->num_links; i++) {
		if (tdmin_adc_node && (strcmp(mt8167s_som_dais[i].cpu_dai_name, "TDM_IN") == 0)) {
			mt8167s_som_dais[i].codec_of_node = tdmin_adc_node;
			mt8167s_som_dais[i].codec_name = NULL;
			printk("mt8167s_som_dais[%d].codec_name: %s\n", i, mt8167s_som_dais[i].codec_name);
			continue;
		}
