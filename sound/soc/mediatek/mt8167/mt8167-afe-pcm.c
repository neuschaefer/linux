...
#define MT8167_SPDIF_OUT_MCLK_MULTIPLIER 128
...
/* afe_iec_configurate */
struct mt8167_afe_iec_rate {
	unsigned int rate;
	unsigned int channel_status;
	unsigned int force_update_size;
};

static struct mt8167_afe_iec_rate mt8167_afe_iec_rates[] = {
	{ .rate = 32000, .channel_status = 0x03001900, .force_update_size = 2},
	{ .rate = 44100, .channel_status = 0x00001900, .force_update_size = 3},
	{ .rate = 48000, .channel_status = 0x02001900, .force_update_size = 3},
	{ .rate = 88200, .channel_status = 0x08001900, .force_update_size = 5},
	{ .rate = 96000, .channel_status = 0x0A001900, .force_update_size = 5},
	{ .rate = 176400, .channel_status = 0x0C001900, .force_update_size = 9},
	{ .rate = 192000, .channel_status = 0x0E001900, .force_update_size = 0xa},
};
...
static int mt8167_afe_enable_irq(struct mtk_afe *afe, struct mt8167_afe_memif *memif)
{
	...
	case MT8167_AFE_IRQ_5:
		...
		break;
	case MT8167_AFE_IRQ_6:
		regmap_update_bits(afe->regmap, AFE_IRQ_MCU_CON, 1 << 13, 1 << 13);
		break;
...
static int mt8167_afe_disable_irq(struct mtk_afe *afe, struct mt8167_afe_memif *memif)
{
	...
	case MT8167_AFE_IRQ_5:
		...
		break;
	case MT8167_AFE_IRQ_6:
		regmap_update_bits(afe->regmap, AFE_IRQ_MCU_CON, 1 << 13, 0 << 13);
	...
	case MT8167_AFE_IRQ_13:
		...
		regmap_write(afe->regmap, AFE_IRQ_CLR, 1 << 12);
		break;
	default:
		break;
	}
			       ...
			       struct snd_soc_dai *dai)
{
	...
	unsigned int stream = substream->stream;
        afe->stream = stream;
	afe->clk_mode = clk_mode;

	dev_err(afe->dev, "%s '%s',afe->stream[%d],afe->clk_mode[%d]\n",
		__func__, snd_pcm_stream_str(substream),afe->stream,afe->clk_mode);
			       ...
			       struct snd_soc_dai *dai)
{
	...
	const bool i2s_in_slave = (stream == SNDRV_PCM_STREAM_CAPTURE) &&
        ...
        afe->stream = stream;
        afe->clk_mode = clk_mode;
	dev_err(afe->dev, "%s '%s',afe->stream[%d]\n",
		__func__, snd_pcm_stream_str(substream),afe->stream);
				...
				struct snd_soc_dai *dai)
{
	...
	dev_err(afe->dev, "%s enable[%d]\n", __func__,be->startuped[stream]);
	return 0;
}
				...
				struct snd_soc_dai *dai)
{
	...
	if (be->prepared[stream]) {
		...
		if (be->cached_rate[stream] % 8000)
			mt8167_afe_disable_apll_hdmi_associated_cfg(afe, MT8167_AFE_APLL1);
		else
			mt8167_afe_disable_apll_hdmi_associated_cfg(afe, MT8167_AFE_APLL2);
	}

	if (rate % 8000) {
		mt8167_afe_enable_apll_hdmi_associated_cfg(afe, MT8167_AFE_APLL1);
		clk_set_parent(afe->clocks[MT8167_CLK_I2S4_M_SEL], afe->clocks[MT8167_CLK_AUD1]);
	} else {
		mt8167_afe_enable_apll_hdmi_associated_cfg(afe, MT8167_AFE_APLL2);
				...
				struct snd_soc_dai *dai)
{
	...
	bck = ((channels == 6 || channels == 2) ? 8 : channels) * rate * bit_width;
				  ...
				  struct snd_soc_dai *dai)
{
	...
	if (data->id == MT8167_AFE_MEMIF_SPDIF_OUT)
		return 0;
...
static int mt8167_afe_iec_startup(struct snd_pcm_substream* substream,
				struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mtk_afe *afe = snd_soc_platform_get_drvdata(rtd->platform);
	...
	mt8167_afe_enable_main_clk(afe);
	mt8167_afe_enable_afe_on(afe);
	mt8167_afe_dais_enable_clks(afe, afe->clocks[MT8167_CLK_APLL12_DIV6], NULL);

	return 0;
}

static struct mt8167_afe_iec_rate mt8167_afe_iec_fs(unsigned int sample_rate)
{
	int i;

	for (i = 0; i < sizeof(mt8167_afe_iec_rates)/sizeof(mt8167_afe_iec_rates[0]); i++)
		if (mt8167_afe_iec_rates[i].rate == sample_rate)
			return mt8167_afe_iec_rates[i];
	/* if no match return 48000 rate*/
	return mt8167_afe_iec_rates[2];
}

static int mt8167_afe_iec_prepare(struct snd_pcm_substream *substream,
				struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mtk_afe *afe = snd_soc_platform_get_drvdata(rtd->platform);
	...
	const unsigned int stream = substream->stream;
	const int bit_width = snd_pcm_format_width(runtime->format);
	/*	bitstream: pcm data(0) or raw data(1),
	 * add mixer ctl to dynamically control the value in the further
	 */
	unsigned int bitstream = 0;
	unsigned int burst_length = frames_to_bytes(runtime, runtime->period_size) * 8;
	struct mt8167_afe_iec_rate iec_rate = mt8167_afe_iec_fs(rate);
	unsigned int stat0 = iec_rate.channel_status | ((!!bitstream) << 1);
	unsigned int force_update_size = iec_rate.force_update_size;
	unsigned int word_length = bitstream ? 0 : ((bit_width <= 16) ? 0x2 /*16bit*/ : 0xB /*24bit*/);
	...
	if (be->prepared[stream]) {
		mt8167_afe_disable_top_cg(afe, MT8167_AFE_CG_SPDIF);
	...
	if (rate % 8000) {
		mt8167_afe_enable_apll_associated_cfg(afe, MT8167_AFE_APLL1);
		clk_set_parent(afe->clocks[MT8167_CLK_SPDIF_B_SEL], afe->clocks[MT8167_CLK_AUD1]);
		dev_info(afe->dev, "%s: rate:%u, apll:%s\n", __func__, rate, "apll11");
	} else {
		mt8167_afe_enable_apll_associated_cfg(afe, MT8167_AFE_APLL2);
		clk_set_parent(afe->clocks[MT8167_CLK_SPDIF_B_SEL], afe->clocks[MT8167_CLK_AUD2]);
		dev_info(afe->dev, "%s: rate:%u, apll:%s\n", __func__, rate, "apll22");
	}

	mt8167_afe_dais_set_clks(afe, afe->clocks[MT8167_CLK_APLL12_DIV6],
				 rate * MT8167_SPDIF_OUT_MCLK_MULTIPLIER, NULL, 0);

	mt8167_afe_enable_top_cg(afe, MT8167_AFE_CG_SPDIF);

	/* PDN signal for SPDIF Out memif and clock */
	...
	regmap_update_bits(afe->regmap, AFE_HDMI_CONN0, 0x1 << 30, 0x0 << 30);

	regmap_update_bits(afe->regmap, AFE_MEMIF_PBUF_SIZE, 0x3 << 6, 0x3 << 6);
	/* start */
	regmap_write(afe->regmap, AFE_SPDIF_OUT_BASE, runtime->dma_addr);
	/* end */
	regmap_write(afe->regmap, AFE_SPDIF_OUT_END, runtime->dma_addr + runtime->dma_bytes);

	regmap_update_bits(afe->regmap, AFE_IEC_NSNUM,
					   IEC_INT_SAM_NUM_MASK | IEC_NEXT_SAM_NUM_MASK,
					 ((runtime->period_size / 2) << 16)| runtime->period_size);
	regmap_update_bits(afe->regmap, AFE_IEC_BURST_LEN, IEC_BURST_LEN_MASK, burst_length);
	regmap_update_bits(afe->regmap, AFE_IEC_BURST_INFO, IEC_BURST_INFO_MASK, 0x0000);

	if(1) {//extend in the further
		/* chst bit31 ~ bit0 *//* chst bit35 ~ bit32 *//* chst bit47 ~ bit36 */
		regmap_write(afe->regmap, AFE_IEC_CHL_STAT0, stat0);
		regmap_update_bits(afe->regmap, AFE_IEC_CHL_STAT1,
				(0xfff << 4) | (0xf		<< 0),
				(0x000 << 4) | (word_length << 0));

		/* chst bit31 ~ bit0 *//* chst bit35 ~ bit32 *//* chst bit47 ~ bit36 */
		regmap_write(afe->regmap, AFE_IEC_CHR_STAT0, stat0);
		regmap_update_bits(afe->regmap, AFE_IEC_CHR_STAT1,
				(0xfff << 4) | (0xf		<< 0),
				(0x000 << 4) | (word_length << 0));
	}

	/*IEC_REG_LOCK_EN_MASK: whether or not to be determinted*/
	regmap_update_bits(afe->regmap, AFE_IEC_CFG,
				IEC_RAW_24BIT_SWITCH_MODE_MASK | IEC_RAW_24BIT_MODE_MASK |
				IEC_FORCE_UPDATE_SIZE_MASK	   | IEC_RAW_SEL_MASK		 |
				IEC_REG_LOCK_EN_MASK		   | IEC_FORCE_UPDATE_MASK	 |
				IEC_MUTE_DATA_MASK			   | IEC_VALID_DATA_MASK	 |
				IEC_SW_RST_MASK,
				IEC_RAW_24BIT_SWITCH_MODE_OFF  | IEC_RAW_24BIT_MODE_OFF	 |
				(force_update_size << 24)	   | IEC_RAW_SEL_RAW		 |
				IEC_REG_LOCK_EN_MASK			| IEC_FORCE_UPDATE		  |
				IEC_MUTE_DATA_NORMAL		   | IEC_VALID_DATA			 |
				IEC_SW_NORST);
	...
	/* afe_iec_burst_info_clear_ready */
	regmap_update_bits(afe->regmap, AFE_IEC_BURST_INFO, IEC_BURST_NOT_READY_MASK, IEC_BURST_NOT_READY);
	mdelay(100);/* delay for prefetch */
...
static void mt8167_afe_iec_shutdown(struct snd_pcm_substream *substream,
				struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mtk_afe *afe = snd_soc_platform_get_drvdata(rtd->platform);
	...
	mt8167_afe_dais_disable_clks(afe, afe->clocks[MT8167_CLK_APLL12_DIV6], NULL);

	mt8167_afe_disable_afe_on(afe);
	mt8167_afe_disable_main_clk(afe);
}

static int mt8167_afe_iec_trigger(struct snd_pcm_substream *substream, int cmd,
				struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mtk_afe *afe = snd_soc_platform_get_drvdata(rtd->platform);
	...
	case SNDRV_PCM_TRIGGER_START:
	case SNDRV_PCM_TRIGGER_RESUME:
		regmap_update_bits(afe->regmap, AFE_IEC_CFG, IEC_EN_MASK, IEC_ENABLE);
...
static void mt8167_afe_iec_isr(struct mtk_afe *afe)
{
	unsigned nsadr = 0;
	...
	regmap_read(afe->regmap, AFE_IEC_BURST_INFO, &reg_val);
	if((reg_val & IEC_BURST_NOT_READY_MASK) != IEC_BURST_READY) {
		pr_debug("%s() burst info is not ready\n", __func__);
		return;
	}

	regmap_read(afe->regmap, AFE_IEC_NSADR, &nsadr);
	regmap_read(afe->regmap, AFE_IEC_BURST_LEN, &burst_len);
	regmap_read(afe->regmap, AFE_SPDIF_OUT_END, &end_addr);
	regmap_read(afe->regmap, AFE_SPDIF_OUT_BASE, &base_addr);

	nsadr += (burst_len & 0x0007ffff) >> 3;
	if (nsadr >= end_addr)
		nsadr = base_addr;

	regmap_write(afe->regmap, AFE_IEC_NSADR, nsadr);
	/*afe_iec_burst_info_clear_ready*/
	regmap_update_bits(afe->regmap, AFE_IEC_BURST_INFO, IEC_BURST_NOT_READY_MASK, IEC_BURST_NOT_READY);
...
static const struct snd_soc_dai_ops mt8167_afe_spdif_out_ops = {
	.startup	= mt8167_afe_iec_startup,
	.shutdown	= mt8167_afe_iec_shutdown,
	.prepare	= mt8167_afe_iec_prepare,
	.trigger	= mt8167_afe_iec_trigger,
};
...
static int mt8167_afe_i2s_suspend(struct snd_soc_dai *dai)
{
	struct mtk_afe *afe = snd_soc_dai_get_drvdata(dai);
	struct mt8167_afe_be_dai_data *be = &afe->be_data[dai->id - MT8167_AFE_BACKEND_BASE];
        unsigned int stream = afe->stream;
	unsigned int clk_mode = afe->clk_mode;

	dev_info(afe->dev, "%s,stream[%d],prepared[%d],startupd[%d],clk_mode[%d]\n",__func__,
			stream,be->prepared[stream],be->startuped[stream],clk_mode);
	...
	if(be->startuped[stream]) {
		if (stream == SNDRV_PCM_STREAM_PLAYBACK || clk_mode == MT8167_AFE_I2S_SHARED_CLOCK)
			mt8167_afe_dais_disable_clks(afe, afe->clocks[MT8167_CLK_APLL12_DIV1], NULL);

		if (stream == SNDRV_PCM_STREAM_CAPTURE || clk_mode == MT8167_AFE_I2S_SHARED_CLOCK)
			mt8167_afe_dais_disable_clks(afe, afe->clocks[MT8167_CLK_APLL12_DIV2], NULL);
...
static int mt8167_afe_i2s_resume(struct snd_soc_dai *dai)
{
	struct mtk_afe *afe = snd_soc_dai_get_drvdata(dai);
	struct mt8167_afe_be_dai_data *be = &afe->be_data[dai->id - MT8167_AFE_BACKEND_BASE];
	unsigned int stream = afe->stream;
...
static int mt8167_afe_2ndi2s_suspend(struct snd_soc_dai *dai)
{
	struct mtk_afe *afe = snd_soc_dai_get_drvdata(dai);
	struct mt8167_afe_be_dai_data *be = &afe->be_data[dai->id - MT8167_AFE_BACKEND_BASE];
        unsigned int stream = afe->stream;
...
static int mt8167_afe_2ndi2s_resume(struct snd_soc_dai *dai)
{
	struct mtk_afe *afe = snd_soc_dai_get_drvdata(dai);
	struct mt8167_afe_be_dai_data *be = &afe->be_data[dai->id - MT8167_AFE_BACKEND_BASE];
	unsigned int stream = afe->stream;
...
static int mt8167_afe_hdmi_suspend(struct snd_soc_dai *dai)
{
	struct mtk_afe *afe = snd_soc_dai_get_drvdata(dai);
	struct mt8167_afe_be_dai_data *be = &afe->be_data[dai->id - MT8167_AFE_BACKEND_BASE];

	dev_info(afe->dev, "%s,prepared[%d],startupd[%d]\n",__func__,
			be->prepared[SNDRV_PCM_STREAM_PLAYBACK],be->startuped[SNDRV_PCM_STREAM_PLAYBACK]);

	if(be->prepared[SNDRV_PCM_STREAM_PLAYBACK]) {
		if(be->cached_rate[SNDRV_PCM_STREAM_PLAYBACK] % 8000)
			mt8167_afe_disable_apll_associated_cfg(afe,MT8167_AFE_APLL1);
		else
			mt8167_afe_disable_apll_associated_cfg(afe,MT8167_AFE_APLL2);
	}
	if(be->startuped[SNDRV_PCM_STREAM_PLAYBACK]) {
		mt8167_afe_dais_disable_clks(afe,afe->clocks[MT8167_CLK_APLL12_DIV4],afe->clocks[MT8167_CLK_APLL12_DIV4B]);
		mt8167_afe_disable_main_clk(afe);
	}
	return 0;
...
static int mt8167_afe_hdmi_resume(struct snd_soc_dai *dai)
{
	struct mtk_afe *afe = snd_soc_dai_get_drvdata(dai);
	struct mt8167_afe_be_dai_data *be = &afe->be_data[dai->id - MT8167_AFE_BACKEND_BASE];
	...
	if(be->startuped[SNDRV_PCM_STREAM_PLAYBACK]) {
			mt8167_afe_enable_main_clk(afe);
			mt8167_afe_dais_enable_clks(afe,afe->clocks[MT8167_CLK_APLL12_DIV4],afe->clocks[MT8167_CLK_APLL12_DIV4B]);
		}
...
static int mt8167_afe_tdmin_suspend(struct snd_soc_dai *dai)
{
	struct mtk_afe *afe = snd_soc_dai_get_drvdata(dai);
	struct mt8167_afe_be_dai_data *be = &afe->be_data[dai->id - MT8167_AFE_BACKEND_BASE];

	dev_info(afe->dev, "%s,prepared[%d],startupd[%d]\n",__func__,
			be->prepared[SNDRV_PCM_STREAM_CAPTURE],be->startuped[SNDRV_PCM_STREAM_CAPTURE]);

	if(be->prepared[SNDRV_PCM_STREAM_CAPTURE]) {
		if(be->cached_rate[SNDRV_PCM_STREAM_CAPTURE] % 8000)
			mt8167_afe_disable_apll_associated_cfg(afe,MT8167_AFE_APLL1);
		else
			mt8167_afe_disable_apll_associated_cfg(afe,MT8167_AFE_APLL2);
	}
	if(be->startuped[SNDRV_PCM_STREAM_CAPTURE]) {
		mt8167_afe_dais_disable_clks(afe,afe->clocks[MT8167_CLK_APLL12_DIV5],afe->clocks[MT8167_CLK_APLL12_DIV5B]);
		mt8167_afe_disable_main_clk(afe);
	}
	return 0;
...
static int mt8167_afe_tdmin_resume(struct snd_soc_dai *dai)
{
	struct mtk_afe *afe = snd_soc_dai_get_drvdata(dai);
	struct mt8167_afe_be_dai_data *be = &afe->be_data[dai->id - MT8167_AFE_BACKEND_BASE];
	...
	if(be->startuped[SNDRV_PCM_STREAM_CAPTURE]) {
			mt8167_afe_enable_main_clk(afe);
			mt8167_afe_dais_enable_clks(afe,afe->clocks[MT8167_CLK_APLL12_DIV5],afe->clocks[MT8167_CLK_APLL12_DIV5B]);
		}
...
static struct snd_soc_dai_driver mt8167_afe_pcm_dais[] = {
	...
	/* BE DAIs */
		...
		.suspend = mt8167_afe_i2s_suspend,
		.resume = mt8167_afe_i2s_resume,
		...
		.suspend = mt8167_afe_2ndi2s_suspend,
		.resume = mt8167_afe_2ndi2s_resume,
		.playback = {
	...
	/* BE DAIs */
		...
		.suspend = mt8167_afe_hdmi_suspend,
		.resume = mt8167_afe_hdmi_resume,
		.playback = {
	...
	/* BE DAIs */
		...
		.suspend = mt8167_afe_tdmin_suspend,
		.resume = mt8167_afe_tdmin_resume,
		.capture = {
		...
		.name = "SPDIF_OUT_IO",
		.id = MT8167_AFE_IO_SPDIF_OUT,
		.playback = {
			.stream_name = "SPDIF_OUT Playback",
			.channels_min = 2,
			.channels_max = 2,
			.rates = SNDRV_PCM_RATE_32000
				| SNDRV_PCM_RATE_44100
...
static const struct snd_soc_dapm_route mt8167_afe_pcm_routes[] = {
	...
	{"SPDIF_OUT Playback", NULL, "SPDIF_OUT"},
...
static struct mt8167_afe_memif_data memif_data[MT8167_AFE_MEMIF_NUM] = {
	{
		...
		.name = "SPDIF_OUT",
		.id = MT8167_AFE_MEMIF_SPDIF_OUT,
		.reg_ofs_base = AFE_SPDIF_OUT_BASE,
		.reg_ofs_end = AFE_SPDIF_OUT_END,
		.reg_ofs_cur = AFE_SPDIF_OUT_CUR,
		.fs_shift = -1,
		.mono_shift = -1,
		.enable_shift = -1,
		.irq_reg_cnt = -1,
		.irq_cnt_shift = -1,
		.irq_mode = MT8167_AFE_IRQ_6,
		.irq_fs_reg = -1,
		.irq_fs_shift = -1,
		.irq_clr_shift = 5,
		.max_sram_size = 36 * 1024,
