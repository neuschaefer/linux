...
static int roku_application_dumped = 0;
extern void ltcore_dump(long sig, struct pt_regs *regs);
int is_roku_application(struct task_struct *t)
{
...
void do_coredump(long signr, int exit_code, struct pt_regs *regs)
{
	 ...
	 * correctly on reboot.
	 */
	if (!roku_application_dumped) { // is_roku_application(current)) 
		/* We now do ltcore_dump for all processes. */
		if (is_roku_application(current)){
			roku_application_dumped = 1;
		}
		ltcore_dump(signr, regs);
	} else {
		printk("skipping ltcore_dump from %s\n", current->group_leader ?
   ...
   if (!little_core_dump)
	...
	return access_ok(VERIFY_READ, addr, nr) && file->f_op->write(file, (const char __force_user *)addr, nr, &file->f_pos) == nr;
	   ...
	   if (nr) {
		   ...
		   if (rv != Z_OK) {
			   pr_err("%s: zlib_deflate failed with %d %d\n", __func__, rv, ltcore_dump_zstr.avail_out);
			   return 0;
		   }
		   if (ltcore_dump_zstr.avail_out == 0) {
			   pr_err("%s: !!!LIMIT. cannot dump core.  Buffer full.", __func__);
			   return 0;
		}
	   }
	#else
	   while (nr) {
		   ...
		   if (nr < chnk) chnk = nr;
		   if (ltcore_dump_cnt >= LTCORE_MAX_PAGES*PAGE_SIZE)
			   return 0;
		   memcpy(ltcore_dump_file + page*PAGE_SIZE + offset, addr, chnk);
		   ltcore_dump_cnt += chnk;
		   //tracek("ltcore_dump_cnt: %d", ltcore_dump_cnt, 0);
		   nr -= chnk;
		   addr = (char*)addr + chnk;
	   }
	#endif
