...
#if (LINUX_VERSION_CODE < KERNEL_VERSION(3, 4, 0))
	retval = TIMED_CALL(mtd, mtd->erase(mtd, &ei), erase, (size_t*)&ei.len);
#else
...
#if (LINUX_VERSION_CODE < KERNEL_VERSION(3, 4, 0))
	retval = TIMED_CALL(mtd, mtd->write_oob(mtd, addr, &ops, 0), write, (data) ?  &ops.retlen : &ops.oobretlen);
#else
				   ...
				   enum yaffs_ecc_result *ecc_result)
{
	 ...
	 */
#if (LINUX_VERSION_CODE < KERNEL_VERSION(3, 4, 0))
	retval = TIMED_CALL(mtd, mtd->read_oob(mtd, addr, &ops, 0), read, (data) ? &ops.retlen : &ops.oobretlen);
#else
