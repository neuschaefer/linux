...
#define YAFFS_AUTO_CHECKPOINT_DELAY  5000 /*ms*/
...
unsigned int yaffs_uid = 0;
unsigned int yaffs_gid = 0;

unsigned int yaffs_last_active = 0;
...
static void inline yaffs_active(void)
{
	yaffs_last_active = jiffies;
	yaffs_trace(YAFFS_TRACE_SYNC, "yaffs_active %u", yaffs_last_active);
}
...
static inline int is_auto_checkpoint_time(void)
{
	unsigned long checkpt_time = yaffs_last_active + msecs_to_jiffies(YAFFS_AUTO_CHECKPOINT_DELAY);
	int i = time_after(jiffies, checkpt_time);
	return i;
}
...
static int yaffs_bg_thread_fn(void *data)
{
	...
	while (context->bg_running) {
		...
		if (!dev->is_checkpointed && is_auto_checkpoint_time()) {
			struct yaffs_linux_context *lc = yaffs_dev_to_lc(dev);
			struct super_block *sb = lc->super;
			yaffs_trace(YAFFS_TRACE_SYNC, "auto_checkpoint");
