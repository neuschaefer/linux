...
#define NAND_DUMMY_WRITE
#ifdef NAND_DUMMY_WRITE
char *toi_dummy_page;
#endif
	...
	if (!toi_cur_sig_page) {
		...
		if (!toi_cur_sig_page) {
			...
			return -ENOMEM;
                ...
                #ifdef NAND_DUMMY_WRITE
		toi_dummy_page = (char *) toi_get_zeroed_page(38, TOI_ATOMIC_GFP);
		if (!toi_dummy_page) {
			printk(KERN_ERR "Failed to allocate memory for the "
				"current image signature.\n");
			return -ENOMEM;
...
void forget_signature_page(void)
{
	if (toi_cur_sig_page) {
        ...
        #ifdef NAND_DUMMY_WRITE
	if (toi_dummy_page) {
		toi_free_page(38, (unsigned long) toi_dummy_page);
		toi_dummy_page = NULL;
	}
        #endif
...
int toi_bio_mark_have_image(void)
{
	...
	toi_bio_ops.bdev_page_io(READ, resume_block_device, resume_firstblock+256, virt_to_page(toi_dummy_page));
	toi_bio_ops.bdev_page_io(WRITE, resume_block_device, resume_firstblock+256, virt_to_page(toi_dummy_page));
