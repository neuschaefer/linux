...
#include "nfsb/nfsb_key_public.h"
...
static int __init dm_setup_linear(struct nfsb_header *hdr, dev_t *dev)
{
	...
	*dev = disk_devt(dm_disk(md));
	pr_info("dm: target linear on %s(%i:%i) size %lu is ready\n", dm_device_name(md), MAJOR(*dev), MINOR(*dev),dm_table_get_size(table));
	if (old_map)
...
static dev_t __init dm_setup_nfsb(struct nfsb_header *hdr, dev_t *dev)
{
	...
	*/
	...
	/* Success. */
	*dev = disk_devt(dm_disk(md));
	pr_info("dm: target verity on %s(%i:%i) size %lu is ready\n", dm_device_name(md), MAJOR(*dev), MINOR(*dev),dm_table_get_size(table));
	if (old_map)
		dm_table_destroy(old_map);
	kfree(target_params);
	dm_unlock_md_type(md);
