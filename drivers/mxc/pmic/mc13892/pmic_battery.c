...
static void chg_thread(struct work_struct *work)
{
	...
	if (charger != g_ntx_bat_di->charger_online) {
        ...
        if( gTerryBatteryDebug )
		    printk("[%s %d]queue mc13892_battery_update_status\n",__func__,__LINE__);
		    
		if ( gTerryBatteryDebug )
			    printk("[%d %s] send uevent\n",__LINE__,__func__);
    ...
    // Terry add 20130509 : check charger and battery level
    //                      if changed, send uevent to Android
...
static void mc13892_battery_update_status(struct mc13892_dev_info *di)
{
	...
	if (old_battery_status != POWER_SUPPLY_STATUS_UNKNOWN) {
		if (gTerryBatteryDebug)
		    printk("[%d %s] get battery value\n",__LINE__,__func__);

		int curVoltage = ntx_get_battery_vol();


        if (gTerryBatteryDebug)
		    printk("[%d %s] (di->voltage_raw, curVoltage) = (%d,%d) \n",__LINE__,__func__,di->voltage_raw ,curVoltage );

		if((di->battery_status != old_battery_status) || (di->voltage_raw != curVoltage) || curVoltage == 0) {
			di->voltage_raw = curVoltage;
			if ( gTerryBatteryDebug )
			    printk("[%d %s] send uevent\n",__LINE__,__func__);
...
//	mc13892_battery_update_status(g_ntx_bat_di);
	...
	if (chg_wq) {
		if ( gTerryBatteryDebug )
			printk("[%s %d]queue chg_thread\n",__func__,__LINE__);
		cancel_rearming_delayed_workqueue(chg_wq, &chg_work);
		queue_delayed_work(chg_wq, &chg_work, HZ/100);
	}
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
        ...
        if ( gTerryBatteryDebug )
            printk("[%s %d]\n",__func__,__LINE__);
		val->intval = ntx_get_battery_vol () * 7000 + 3400000;
#endif
		break;
...
#if 1
		//val->intval = di->voltage_raw = ntx_get_battery_vol ();
		if ( gTerryBatteryDebug )
			printk("[%s %d]\n",__func__,__LINE__);
		//val->intval = di->voltage_raw;
