...
static void enable_mmc0();
static void disable_mmc0();

static void sunximmc_suspend_pins(struct sunxi_mmc_host* smc_host, bool keep_power_state)
{
	int ret;
...
static void sunximmc_resume_pins(struct sunxi_mmc_host* smc_host, bool keep_power_state)
{
	int ret;
	u32 i;
...
static void sunximmc_cd_timer(unsigned long data)
{
	...
	present = sd_inserted;
	SMC_DBG("host present %d, cur present %d\n", smc_host->present, present);
	...
	switch (ios->power_mode)
	{
		...
		case MMC_POWER_UP:
			if (!smc_host->power_on)
			{
				...
				sunximmc_resume_pins(smc_host,false);
				/* enable mmc hclk */
				clk_enable(smc_host->hclk);
				/* enable mmc mclk */
				clk_enable(smc_host->mclk);
		...
		case MMC_POWER_OFF:
			if (smc_host->power_on)
			{
				...
				sunximmc_suspend_pins(smc_host,false);
				smc_host->power_on = 0;
				smc_host->ferror = 0;
			}
		default:
...
#ifdef CONFIG_PM
#ifdef CONFIG_MMC_SUNXI_KEEP_POWER_IN_SUSPEND
extern int g_suspend_state;
static inline int keep_power(struct platform_device *pdev)
{
	return (pdev->id == 2 || g_suspend_state == PM_SUSPEND_PARTIAL || g_suspend_state == PM_SUSPEND_STANDBY);
}
#else
#define keep_power(pdev) (0)
#endif

static int sunximmc_suspend(struct device *dev)
{
	struct platform_device *pdev = to_platform_device(dev);
	struct mmc_host *mmc = (struct mmc_host *)platform_get_drvdata(pdev);
	int ret = 0;

	printk("[%s] g_suspend_state = %i; keep_power = %i\n",__func__,g_suspend_state,keep_power(pdev));
	...
	if (mmc)
	{
		...
		if (mmc->card && (mmc->card->type!=MMC_TYPE_SDIO || mmc_pm_io_shd_suspend_host())) {
			if (keep_power(pdev)) mmc->pm_flags |= MMC_PM_KEEP_POWER;
		...
		if (smc_host->power_on) {
			...
			/* suspend pins to save power */
			sunximmc_suspend_pins(smc_host, keep_power(pdev));
		}
	}
...
printk("--- sunximmc_resume(%d) start\n", pdev->id);
	if (mmc)
	{
		struct sunxi_mmc_host *smc_host = mmc_priv(mmc);
		...
		if (smc_host->power_on) {
			/* resume pins to correct status */
			sunximmc_resume_pins(smc_host, keep_power(pdev));
...
printk("--- sunximmc_resume(%d) complete\n", pdev->id);
...
static unsigned int mmc0_gpio_handle = 0;
...
static void enable_mmc0()
{
	...
	if (! mmc0_gpio_handle) {
		mmc0_gpio_handle = gpio_request_ex("MMC0_enable", "mmc0_enable");

		if (! mmc0_gpio_handle)
			printk("<1> %s error\n", __func__);
	}

	gpio_write_one_pin_value(mmc0_gpio_handle, 0, "mmc0_enable");
}
...
static void disable_mmc0()
{
	...
	gpio_write_one_pin_value(mmc0_gpio_handle, 1, "mmc0_enable");
}

static u32 det_handle;

static irqreturn_t SD_EINT_Handler(int irq, void *dev_id)
{
	int reg_val = 0;
	struct device *dev = (struct device *)dev_id;

	reg_val = sw_gpio_eint_get_irqpd_sta(det_handle);
	if (reg_val != 1)
		return IRQ_NONE;

	if (sd_inserted == 0) {
		sw_gpio_set_trigger(det_handle, TRIG_LEVL_HIGH);
		sd_inserted = 1;
	}
	else {
		sw_gpio_set_trigger(det_handle, TRIG_LEVL_LOW);
		sd_inserted = 0;
	}

	printk("<1> sd_inserted %d dev %p %p\n", sd_inserted, dev, dev ? dev->power.wakeup : NULL);
	pm_wakeup_event(dev, 1000);

	sw_gpio_eint_clr_irqpd_sta(det_handle);

	return IRQ_HANDLED;
}

static int mmc0_interrupt_init(struct device *dev)
{
	int pending, err;

	printk("<1> %s\n", __func__);

	det_handle = sw_gpio_irq_request("MMC0_enable", "mmc0_det", TRIG_LEVL_LOW);
	if (!det_handle) {
	...
	err = request_irq(SW_INT_IRQNO_PIO, SD_EINT_Handler, IRQF_SHARED  ,"sd-in", dev);
	if (err < 0) {
	...
	sw_gpio_eint_set_enable(det_handle, 1);
	pending = sw_gpio_eint_get_irqpd_sta(det_handle);
	if (pending < 0)
...
static int __init sunximmc_init(void)
{
	...
	for (i=0; i<SUNXI_MMC_HOST_NUM; i++) {
		// enable interrupt for mmc0 on sd_insert
		if (i == 0) {
			enable_mmc0();
		}
		...
		if (used)
		{
			...
			if (i == 0) {
				mmc0_interrupt_init(&awmmc_device[i].dev);
			}
		}
