...
struct msdc_host {
	...
	struct clk *cg_clk; /* msdc source clock control gate */
...
static void msdc_gate_clock(struct msdc_host *host)
{
	...
	/*
	 * Chaotian: in my opinion, no need host->sdio_clk_cnt, as gate/ungate
	 * are always balance.
	 */
	host->sdio_clk_cnt--;
...
static void msdc_set_mclk(struct msdc_host *host, unsigned char timing, u32 hz)
{
	...
	if (!hz) {
		dev_info(host->dev, "set sdio mclk to 0\n");
		host->mclk = 0;
...
static void msdc_init_hw(struct msdc_host *host)
{
	...
	/* Configure to MMC/SD mode, clock not free running */
...
static void msdc_dump_all_register(struct msdc_host *host)
{
	...
	for (i = 0; i < byte16_align_cnt; i++)
		pr_err("SDIO reg[%.2x]=0x%.8x reg[%.2x]=0x%.8x reg[%.2x]=0x%.8x reg[%.2x]=0x%.8x\n",
