...
static void mtk_nfc_randomizer_init(struct nand_chip *chip)
{
	struct mtk_nfc *nfc = nand_get_controller_data(chip);
	u32 enable_mask = 0;
	u32 efuse_index = 0;
	...
	if (nfc->chip_data->type == MTK_NAND_MT8516) {
		efuse_index = 0;
		enable_mask = 0x00001000;
	} else
		return;

	if ((get_devinfo_with_index(efuse_index)) & enable_mask) {
		chip->options |= NAND_NEED_SCRAMBLING;
		chip->options |= NAND_NO_SUBPAGE_WRITE;
	}
}

void mtk_nfc_randomizer_enable(struct nand_chip *chip, int page,
		enum mtk_randomizer_operation rand, int repage)
{
	struct mtk_nfc *nfc = nand_get_controller_data(chip);
	u32 reg = 0;
	u32 loop = SS_SEED_NUM;
	u32 page_per_block = 1 << (chip->phys_erase_shift - chip->page_shift);
...
void mtk_nfc_randomizer_disable(struct nand_chip *chip)
{
	struct mtk_nfc *nfc = nand_get_controller_data(chip);
...
static int mtk_nfc_send_command(struct mtk_nfc *nfc, u8 command)
{
	...
	if (ret) {
		dev_err(dev, "nfi core timed out entering command mode\n");
		return -EIO;
	}
...
static int mtk_nfc_send_address(struct mtk_nfc *nfc, int addr)
{
	...
	if (ret) {
		dev_err(dev, "nfi core timed out entering address mode\n");
		return -EIO;
	}
...
static void mtk_nfc_select_chip(struct mtd_info *mtd, int chip)
{
	...
	if (chip < 0) {
		mtk_ecc_disable_clk(nfc->ecc);
		mtk_nfc_disable_clk(&nfc->clk);
		return;
	}

	if (mtk_nfc_enable_clk(nfc->dev, &nfc->clk))
		dev_err(nfc->dev, "failed to enable nfi clk\n");

	if (mtk_ecc_enable_clk(nfc->ecc))
		dev_err(nfc->dev, "failed to enable nfiecc clk\n");
				 ...
				 const u8 *buf, int page, int len)
{
	...
	ret = readl_poll_timeout_atomic(nfc->regs + nfc->chip_data->nfi_regs[NFI_INTR_STA], reg,
				...
				u8 *bufpoi, int page, int raw)
{
	...
	rc = readl_poll_timeout_atomic(nfc->regs + nfc->chip_data->nfi_regs[NFI_INTR_STA], reg,
					reg & INTR_AHB_DONE_EN, 10, MTK_TIMEOUT);
	if (rc)
	...
	if (rc < 0) {
		...
		if (!raw) {
			...
			if (chip->options & NAND_NEED_SCRAMBLING) {
				if (nfi_readl(nfc, NFI_STA) & STA_EMP_PAGE)
					memset(bufpoi, 0xff, mtd->writesize + mtd->oobsize);
			}
		}
	}
				...
				int page)
{
	...
	ret = mtk_nfc_read_page_raw(mtd, chip, NULL, 1, page);
...
static inline void mtk_nfc_hw_init(struct mtk_nfc *nfc)
{
	...
	/* nfi_writel(nfc, 0x10804011, NFI_ACCCON); */

	/* Remove ACCON setting here since lk will adjust timing for nand chip
	 * Just add debug info to check the setting here.
	 */
	pr_info("Check Timing setting NFI_ACCCON:0x%x\n",
				  ...
				  struct device_node *np)
{
	...
	if (nfc->chip_data->type == MTK_NAND_MT8516) {
		if ((mtd->writesize == 4096) && (mtd->oobsize == 224)) {
			mtd->oobsize = 208;
			nand->ecc_strength_ds = 24;
			nand->ecc_step_ds = 1024;
			pr_info("force oobsize 224 to 208 for MT8516\n");
		}
	}
	...
	if (nfc->chip_data->type == MTK_NAND_MT2712)
		...
		ret = mtk_partition_register(mtd, 1);
#else
...
static const struct mtk_nand_type nfc_mt8516 = {
	.nfi_regs = mt8516_nfi_regs,
	.type = MTK_NAND_MT8516,
	.fdm_ecc_size = 8,
	.parity_bit = 14,
	.max_sector = 16,
	.max_sector_spare = 16,
...
static const struct of_device_id mtk_nfc_id_table[] = {
	{
		...
		.compatible = "mediatek,mt8167-nfc",
		.data = &nfc_mt8516,
	}, {
	}
};
...
static int mtk_nfc_probe(struct platform_device *pdev)
{
	...
	mtk_nand_regulator = devm_regulator_get(&pdev->dev, "vcc");
	ret = regulator_set_voltage(mtk_nand_regulator, 3300000, 3300000);
	if (ret != 0)
		dev_err(dev, "regulator set vol failed: %d\n", ret);
