...
/*
	SNOM uses SBASIC-BA enhanced Ethernet descriptors,
	but does not use Jumbo Frames and Implicit chaining
*/
#define ETH_ARCH_SPEARBASIC_BA 
//#define JUMBO_FRAME_SUPPORT 
//#define SPR_DMA_IMPLICIT_CHAIN 
//#define RCV_INTR_EN
...
{                         
	...
	volatile char *DmaMac_Addr1;
	unsigned int address;
	unsigned char *skb_address;
	struct page *skb1;
...
#define SPEAR_MIN_MTU			46

#ifdef JUMBO_FRAME_SUPPORT
#ifdef ETH_ARCH_SPEARBASIC_BA
	#define SPEAR_MAX_MTU			9000
#else
	#define SPEAR_MAX_MTU			4000
#endif

#define SPEAR_MAX_RX_SIZE		4080

#define SPEAR_MTU_OVERHEAD		(6 * 2 + 2 + 4)

static inline int spear_rx_size(int len)
{
	len = (len + 0xf) & ~0xf;
	return len > SPEAR_MAX_RX_SIZE ? SPEAR_MAX_RX_SIZE : len;
}
/* RX BD size for the given MTU */
static inline int spear_eth_rx_size(int mtu)
{
	if (mtu > ETH_DATA_LEN)
		return SPEAR_MAX_RX_SIZE;
	else
		return 2048;
}

#define SPEAR_DMA_ALIGN(x)		ALIGN((x), dma_get_cache_alignment())

#define SPEAR_RX_SKB_HEADROOM		\
	SPEAR_DMA_ALIGN(0)

/* Size of RX skb for the given MTU */
static inline int spear_eth_rx_skb_size(int mtu)
{
	int size = max(mtu + SPEAR_MTU_OVERHEAD, spear_eth_rx_size(mtu));
	return SPEAR_DMA_ALIGN(size + 2) + SPEAR_RX_SKB_HEADROOM;
}

/* RX DMA sync size */
static inline int spear_eth_rx_sync_size(int mtu)
{
	return SPEAR_DMA_ALIGN(spear_eth_rx_size(mtu) + 2);
}

#endif
/* TX/RX descriptor defines */ 
#define SPEAR_DEFAULT_TXD	CONFIG_870E_ETH_SPEAR_MAC_TXB
#define SPEAR_MAX_TXD		256
#define SPEAR_MIN_TXD		16
  
#define SPEAR_DEFAULT_RXD	CONFIG_870E_ETH_SPEAR_MAC_RXB
...
#else
	#ifdef	ETH_ARCH_SPEARBASIC_BA
		#define	MAC_MAX_FRAME_SZ	7168
		#define	MAC_MAX_FRAME_SZ1	2040
	#else
		#define	MAC_MAX_FRAME_SZ	2016
	#endif
#endif
