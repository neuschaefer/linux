/*
* drivers/net/arm/spr_870_ethtool.c
*
...
*
* AUTHOR(s) :  Deepak Sikri <deepak.sikri@st.com> 
	       Bhupesh Sharma <bhupesh.sharma@st.com>
*/

/* Ethtool support for 870 phone */

#include <asm/uaccess.h>
#include "spr_870_eth_syn.h"

extern char spear_rtl_driver_name[];
extern char spear_rtl_driver_version[];
extern uint32_t SPEAr_get_link_status(struct net_device *dev);
...
static const struct spear_stats spear_gstrings_stats[] = {
	...
	//	{ "tx_abort_late_coll", SPEAR_STAT(stats.latecol) },
	{ "tx_deferred_ok", SPEAR_STAT(hw_stats.dc) },
	//	{ "tx_single_coll_ok", SPEAR_STAT(stats.scc) },
	//	{ "tx_multi_coll_ok", SPEAR_STAT(stats.mcc) },
	{ "rx_long_length_errors", SPEAR_STAT(hw_stats.roc) },
	{ "rx_short_length_errors", SPEAR_STAT(hw_stats.ruc) },
	//	{ "rx_align_errors", SPEAR_STAT(stats.algnerrc) },
...
static void emac_force_link_update(struct eth_priv *ndev)
{
...
/****************************************************************************/
	...
	static void
spear_get_ringparam (struct net_device *netdev,
...
/****************************************************************************/
	static void
spear_get_pauseparam (struct net_device *netdev,
...
/****************************************************************************/
	static int
spear_get_regs_len (struct net_device *netdev)
{

#define SPEAR_REGS_LEN 32
	/*	return SPEAR_REGS_LEN * sizeof(uint32_t);*/
	return 0;
}

/****************************************************************************/
/* NAME:  spear_ethtool_gregs                                              */
/*--------------------------------------------------------------------------*/
/* DESCRIPTION:                                                             */
...
/****************************************************************************/
	...
	static void
spear_ethtool_gregs (struct net_device *netdev, struct ethtool_regs *regs,
		void *p)
{
	//    struct eth_priv *adapter = netdev->priv;

	/*	memset(p, 0, SPEAR_REGS_LEN * sizeof(uint32_t));*/
	DEBUGFUNC ("inside gregs\n");
	SPEAR_RTL_ETH_Dump_Registers(netdev);
}

/****************************************************************************/
/* NAME:  spear_nway_reset                                                  */
/*--------------------------------------------------------------------------*/
/* DESCRIPTION:                                                             */
/*                                                                          */
/*                                                                          */
...
/* REENTRANCY: NA                                                           */
/****************************************************************************/
static int spear_nway_reset(struct net_device *netdev)
{
	struct eth_priv *dev = (struct eth_priv *) netdev->priv;
	int res = 0;
...
/****************************************************************************/
	...
	static int
spear_get_stats_count (struct net_device *netdev)
{
	return SPEAR_STATS_LEN;
}

/****************************************************************************/
/* NAME:  spear_get_strings                                                 */
/*--------------------------------------------------------------------------*/
/* DESCRIPTION:                                                             */
/*    called by ethtool -s returns the etherent status                      */
/*                                                                          */
...
/****************************************************************************/
	static void
spear_get_strings (struct net_device *netdev, uint32_t stringset,
		uint8_t * data)
{
	int i;
...
/****************************************************************************/
	static void
spear_get_ethtool_stats (struct net_device *netdev,
		struct ethtool_stats *stats, uint64_t * data)
{
	...
	for (i = 0; i < SPEAR_STATS_LEN; i++)
	{
		...
		data[i] =
			(spear_gstrings_stats[i].sizeof_stat ==
			 sizeof (uint64_t)) ? *(uint64_t *) p : *(uint32_t *) p;
	}
}

/****************************************************************************/
/* NAME:  spear_ethtool_gdrvinfo                                            */
/*--------------------------------------------------------------------------*/
/* DESCRIPTION:                                                             */
...
/****************************************************************************/
	...
	static void
spear_ethtool_gdrvinfo (struct net_device *netdev,
		struct ethtool_drvinfo *drvinfo)
{
	/* struct eth_priv *adapter = netdev->priv;*/

	DEBUGFUNC ("inside gdrv info\n");
	strncpy (drvinfo->driver, spear_rtl_driver_name, 6);
	strncpy (drvinfo->version, spear_rtl_driver_version, 21);
	...
	drvinfo->regdump_len = spear_get_regs_len (netdev);
	//    drvinfo->eedump_len = spear_get_eeprom_len (netdev);
}

/****************************************************************************/
/* NAME:  spear_ethtool_op_get_link                                         */
/*--------------------------------------------------------------------------*/
/* DESCRIPTION:                                                             */
/*    get the link status                                                   */
/*                                                                          */
...
static u32 spear_ethtool_op_get_link(struct net_device *dev)
{
	struct eth_priv *adapter = dev->priv;
	u32 ret;
	/*u32 mii_status;*/

	spin_lock_irq(&adapter->eth_lock);
	ret= SPEAr_get_link_status(dev); 
	spin_unlock_irq(&adapter->eth_lock);
	return ret;
}
/****************************************************************************/
/* NAME:  spear_get_tx_csum                                                 */
/*--------------------------------------------------------------------------*/
/* DESCRIPTION:                                                             */
/*    Set the tx CSUM                                                       */
/*                                                                          */
...
/****************************************************************************/
	static uint32_t
spear_get_tx_csum (struct net_device *netdev)
{
	return (netdev->features & NETIF_F_HW_CSUM) != 0;
}

/****************************************************************************/
/* NAME:  spear_set_tx_csum                                                 */
/*--------------------------------------------------------------------------*/
/* DESCRIPTION:                                                             */
/*    Set the tx CSUM                                                       */
/*                                                                          */
...
/****************************************************************************/
	static int
spear_set_tx_csum (struct net_device *netdev, uint32_t data)
{
...
/****************************************************************************/
	static int
spear_diag_test_count (struct net_device *netdev)
{
...
#ifdef NETIF_F_TSO
	static u32
spear_get_tso (struct net_device *dev)
{
	return (dev->features & NETIF_F_TSO) != 0;
}
	static int
spear_set_tso (struct net_device *dev, u32 sum)
{
...
static struct ethtool_ops spear_ethtool_ops =
{
	...
	.get_drvinfo = spear_ethtool_gdrvinfo,

	.get_regs_len = spear_get_regs_len,
	.get_regs = spear_ethtool_gregs,

	.nway_reset = spear_nway_reset,

	.get_ringparam = spear_get_ringparam,
	.get_pauseparam = spear_get_pauseparam,

	.get_strings = spear_get_strings,
	.get_stats_count = spear_get_stats_count,
	.get_ethtool_stats = spear_get_ethtool_stats,

	.get_link = spear_ethtool_op_get_link,
	.get_tx_csum = spear_get_tx_csum,
	.set_tx_csum = spear_set_tx_csum,
	.get_sg = ethtool_op_get_sg,
	.self_test_count = spear_diag_test_count,
#ifdef NETIF_F_TSO
	.get_tso = spear_get_tso,
	.set_tso = spear_set_tso,
#endif
};
