...
#define DRIVER_AUTHOR "Franko Fang <huawei>"
...
module_param (msg_level, int, S_IRUGO|S_IWUSR);
...
/*****************************************************************************
  ...
  Return:           
*****************************************************************************/
void RNIC_ProcDemDial(struct sk_buff*     pstSkb)
{
    ...
    if (DIAL_MODE_DEMAND_DISCONNECT == g_DialMode.enDialMode)       
    {
...
static inline int ntb_add_dgram(struct ncm_ctx *ctx, struct ntb *n,
    unsigned dgram_len, struct sk_buff *skb, gfp_t flags)
{
    struct ndp_parser_opts_hw *popts = &ctx->popts;
    ...
    gfp_t mem_flags)
{
    ...
    err = ntb_add_dgram(ctx, curr_ntb, skb->len, skb, GFP_ATOMIC);
    switch (err) {
    case 0:
    ...
    case -EINVAL:
        ...
        err = ntb_add_dgram(ctx, curr_ntb, skb->len, skb,
            GFP_ATOMIC);
        if (unlikely(err < 0)) {
...
/* linux kernel > 2.6.37: PowerManager needs disable_depth ==0 */
    ...
    if (deviceisBalong){
        strcpy (net->name, "balong_eth%d");
    }else{
        strcpy (net->name, "qcom_eth%d");
    }
...
next_desc:
    ...
    if (((NULL != info->u)&&(info->u->bMasterInterface0 != info->u->bSlaveInterface0)))
    {
        /* claim data interface and set it up ... with side effects.
         ...
         /*Begin:add for ndis bind error */
        if(info->data->dev.driver != NULL)
        {
            usb_driver_release_interface(driver, info->data);
        }
                /*End:add for ndis bind error */
	 ...
	 || ((NULL != info->u)&&(info->u->bMasterInterface0 == info->u->bSlaveInterface0)))
    {
	 ...
	 && BINTERFACESUBCLASS_HW != intf->cur_altsetting->desc.bInterfaceSubClass) {
	     printk(KERN_ERR"lxz device is Qualcomm device bcdDevice=%x,InterfaceSubClass=%x\n",
		        le16_to_cpu(dev->udev->descriptor.bcdDevice),intf->cur_altsetting->desc.bInterfaceSubClass);
	     deviceisBalong = false;
	 }else{
	     deviceisBalong = true;
