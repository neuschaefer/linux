...
#define DHD_KPARAM_LEN 1024
	...
	if (fp == NULL) {
		DHD_ERROR(("%s: cannot open /proc/cmdline\n", __FUNCTION__));
		goto err;
	}
	...
	DHD_INFO(("%s: err=%d len=%u kparam=%s\n",
		__FUNCTION__, bcmerror, len, len ? kparam : ""));
	if (len > 0) {
...
#if 0 /* Force WHDMI enable regardless of kernel parameters */
	DHD_ERROR(("Forcing WHDMI on regardless of kernel 'wifi=' param!\n"));
	dhd_bcm_whdmi_enable = TRUE;
#endif /* Force WHDMI enable regardless of kernel parameters */
#endif /* DHD_BCM_WIFI_HDMI */
...
dhd_module_init(void)
{
...
#else
		// HACK!!! Get above retry working! -don
		if (1) {
			DHD_ERROR(("%s: Skipping SDIO retry mechanism, FIXME!\n", __FUNCTION__));
#endif 
	...
	if (!chip_up) {
		DHD_ERROR(("%s: failed to power up wifi chip, max retry reached, exits\n", __FUNCTION__));
		return -ENODEV;
	}
#else
	dhd_customer_gpio_wlan_ctrl(WLAN_POWER_ON);
