...
#define CUSTOMER_HW4_OUI		"\x00\x00\xf0"
#define CUSTOMER_HW4_OUI_LEN		3
#define wl_cfgp2p_is_customer_ie(ie, tlvs, len) wl_cfgp2p_has_ie(ie, tlvs, len, \
	(const uint8 *)CUSTOMER_HW4_OUI, 3, 0)
...
wl_cfgp2p_has_ie(u8 *ie, u8 **tlvs, u32 *tlvs_len, const u8 *oui, u32 oui_len, u8 type)
{
	...
	/* If the contents match the CUSTOMER_HW4 OUI */
	if (ie[TLV_LEN_OFF] >= oui_len + 1 &&
	        !bcmp(&ie[TLV_BODY_OFF], oui, oui_len) &&
	        !bcmp(oui, CUSTOMER_HW4_OUI, CUSTOMER_HW4_OUI_LEN)) {
		return TRUE;
	}
...
wl_cfgp2p_find_customer_ie(u8 *parse, u32 *len)
{
	...
	while ((ie = bcm_parse_tlvs(parse, *((int*)len), DOT11_MNG_VS_ID))) {
		if (wl_cfgp2p_has_ie((uint8 *) ie, &parse, len,
			(const uint8 *)CUSTOMER_HW4_OUI, WFA_OUI_LEN, 0)) {
