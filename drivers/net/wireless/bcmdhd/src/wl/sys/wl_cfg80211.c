...
wl_cfg80211_send_pending_tx_act_frm(struct wl_priv *wl)
{
	...
	if (tx_act_frm != NULL) {
		 ...
		 */
#if !defined(CUSTOMER_HW4) && !defined(CONFIG_ARCH_CAPRI)
...
#error inform bss will be done at notify function
			...
			WL_DBG(("connect failed event=%d e->status 0x%x\n",
	...
	const wl_event_msg_t *e, void *data, bool completed)
{
	...
	if (wl_get_drv_status(wl, CONNECTED, ndev)) {
		if (memcmp(curbssid, connect_req_bssid, ETHER_ADDR_LEN) == 0) {
			WL_DBG(("Connected, ignoring connect event.\n"));
			return err;
		}
	}
