...
x_os_drv_get_tick_period()
{
    ...
    //ioctl(i4_fdioctrl,IOCTL_GET_TICK_PERIOD,&ui4_res);
#endif
    return(ui4_res);
}
...
#else
    UINT32 ui4_res;
    ioctl(i4_fdioctrl,IOCTL_GET_FINE_TICK,&ui4_res);
    return (ui4_res);
#endif
}
...
/*#ifdef SW_PLATFORM*/
    ...
    do
    {
        ioctl(i4_fdioctrl,IOCTL_GET_DRV_TICK,0);
        x_os_drv_tick();
    } while (1);
#endif
}
...
os_drv_timer_thread()
{
	...
	i4_fdioctrl=open("/dev/rmmgr",O_RDWR);
	ui4_drv_tick = 0;
	ioctl(i4_fdioctrl,IOCTRL_HW_TICK,&ui4_drv_tick);
   
	do
	{
	   usleep(1);
	   ioctl(i4_fdioctrl,IOCTRL_HW_TICK,&ui4_drv_cur);
	   {
	       if (ui4_drv_cur > ui4_drv_tick)
	       {
	           tick_lost = ui4_drv_tick + (0xffffffff - ui4_drv_cur)  ;
	           
	       }
	       else
	       {
	           tick_lost = ui4_drv_tick - ui4_drv_cur;
	       }
	       ui4_drv_tick = ui4_drv_cur;
	       cnt = tick_lost/1350000;/*n/ms*/
	   while (cnt--)
	       {
	          x_os_drv_tick();
	       } 
