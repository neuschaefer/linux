...
static UINT32 _GfxGetYcFormat(GFX_YCBCR_FORMAT_T eMwYbrFormat)
{
    ...
    if (eVer < IC_VER_AC)   // without 422 linear mode
    {
        switch (eMwYbrFormat)
        {
...
//-------------------------------------------------------------------------
/** _GfxScale
 *  Gfx-Scalar scale (scaling with taps)
 *
 */
//-------------------------------------------------------------------------
static GFX_ERROR_T _GfxScale(const GFX_STRETCH_BITBLT_T *prStretch)
{
    UINT32 usx, udx, usy, udy;
    UINT32 u4LpfEn = 1;
    UINT32 u4ValidRatio = GFXSC_RATIO_LIMIT;
    ...
    if(_GfxOpModeCheck((UINT32)OP_SC_SCALING, 
            (UINT32)prStretch->e_src_cm, (UINT32)prStretch->e_dst_cm))
    {
        return GFX_ERROR_INTERNAL;
    }
    ...
    //------------------------------
    // do gfx flush first and wait its works completely
    GFX_Lock();
    ...
    GFXSC_SetSrc((UINT8*)prStretch->pv_src, 
        _GfxScGetColorMode(prStretch->e_src_cm), prStretch->ui4_src_pitch);

    GFXSC_SetDst((UINT8*)prStretch->pv_dst, 
        _GfxScGetColorMode(prStretch->e_dst_cm), prStretch->ui4_dst_pitch);

    GFXSC_SetScaleOpt(u4LpfEn);
    
    GFXSC_Scale(usx, usy, udx, udy, prStretch->ui4_width, prStretch->ui4_height,
        prStretch->ui4_dst_width, prStretch->ui4_dst_height);
...
*/        
        ...
        case GFX_SET_TYPE_ADV_STRETCH_BITBLT:
            SET_COMMAND(_GfxScale(pv_set_info))
...
INT32 MW_GFX_Init(void)
{
    ...
    if (x_strcpy(ptComp[i].szCompName, szDriverName))
    {
        INT32 i4Ret;
