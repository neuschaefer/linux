...
static UINT8 _ucMjcEnable = DRV_EXTMJC_MODE_OFF;
        ...
        case PLA_MXR_GET_TYPE_BACK_LIGHT:
            {
                ...
                *pz_get_info_len = sizeof(UCHAR);
                ...
                if (VDP_GetArg(VDP_1, ucArgType, &i2Val) != VDP_SET_OK)
                {
                    return (RMR_DRV_INV_GET_INFO);
                }
        ...
        case PLA_MXR_GET_TYPE_MIN_MAX:
            {
                ...
                *pz_get_info_len = sizeof(PLA_MXR_MIN_MAX_INFO_T);
                ...
                if ((ucArgType == VDP_ARG_BACK_LIGHT_LVL) ||
                    (ucArgType == VDP_ARG_ADAPTIVE_BACK_LIGHT))
                {
                    BOOL bRet = VDP_GetUiVqItemMinMaxDftCur(VDP_1, ucArgType,
        ...
        case PLA_MXR_GET_TYPE_RESOLUTION:
            {
                ...
                *pz_get_info_len = sizeof(PLA_MXR_RESOLUTION_INFO_T);
                ...
                prArg->ui2_width = PANEL_GetWidth();
                prArg->ui2_width = PANEL_GetHeight();
        ...
        case PLA_MXR_GET_TYPE_MJC:
            {
                ...
                if ( _ucMjcEnable == DRV_EXTMJC_MODE_OFF )
                {
                    *(BOOL*)pv_get_info = FALSE;
                }
                else if ( _ucMjcEnable == DRV_EXTMJC_MODE_MJC )
                {
        ...
        case PLA_MXR_SET_TYPE_BG:
            {
                ...
                if (PMX_SetBg(ucPmxId, u4Bg) != VDP_SET_OK)
                {
                   return (RMR_DRV_INV_SET_INFO);
                }
            }
        ...
        case PLA_MXR_SET_TYPE_GAMMA:
            {
                ...
                //UNUSED(bApiVideoProc(SV_VP_MAIN , (INT32)VDP_ARG_GAMMA));
                vApiVideoGammaLoadTable(ucArg);
                //vApiRegisterVideoEvent(VDP_ARG_GAMMA, ucArg, SV_ON);
             }
            break;
        ...
        case PLA_MXR_SET_TYPE_BACK_LIGHT:
            {
                ...
                if (VDP_SetArg(VDP_1, ucArgType, (INT16)ucArg) != VDP_SET_OK)
                {
                    return (RMR_DRV_INV_SET_INFO);
                }
            }
        ...
        case PLA_MXR_SET_TYPE_MJC:
            {
                ...
                {
                    _ucMjcEnable = DRV_EXTMJC_MODE_OFF;
                }
                else if ( (BOOL)(UINT32)pv_set_info == TRUE )
                {
                    _ucMjcEnable = DRV_EXTMJC_MODE_MJC;
                }
                else
                {
                    _ucMjcEnable = DRV_EXTMJC_MODE_DNM;
                }
                drv_extmjc_set_mjc_mode(_ucMjcEnable, &u1Msg);
                (void)u1Msg;
...
static void _PmxSetPlaneOrder(UCHAR ucPmxId)
{
    ...
    if (ucPmxId == PMX_COMP_1)
    {
        ...
        u4TargetIdx = 0;
         for (u4ZorderIdx = 0; u4ZorderIdx < PMX_MAX_INPORT_NS; u4ZorderIdx++)
         {
             for (u4PortIdx = 0; u4PortIdx < PMX_MAX_INPORT_NS; u4PortIdx++)
             {
                 if (_au4Zorder[ucPmxId][u4PortIdx] == u4ZorderIdx)
                 {
                     au4HwZorder[u4TargetIdx++] = u4PortIdx;
                     u4Inactive = u4PortIdx;
                     u4PortIdx = PMX_MAX_INPORT_NS;
                 }
             }
         }
        ...
        // refill inactive plane
        for (; u4TargetIdx < PMX_MAX_INPORT_NS; u4TargetIdx++)
        {
            ASSERT(u4Inactive < PMX_MAX_INPORT_NS);
            au4HwZorder[u4TargetIdx] = u4Inactive;
        }
        ...
        VERIFY(PMX_SetPlaneOrderArray(ucPmxId, au4HwZorder) == PMX_SET_OK);
    }
}
...
static UCHAR _PmxMwGetType2ArgType(UINT32 u4GetType)
{
    switch (u4GetType)
    {
        // VDP Back Light Args
        case PLA_MXR_GET_TYPE_BACK_LIGHT:
            return VDP_ARG_BACK_LIGHT_LVL;

        case PLA_MXR_GET_TYPE_ADP_BACK_LIGHT:
            return VDP_ARG_ADAPTIVE_BACK_LIGHT;
    ...
    DISP_QA_MIN_MAX_DFT_T*    pt_min_max_dft_tbl )
{
    ...
    while( !DISP_QA_MIN_MAX_DFT_IS_END(pt_iter) )
    {
        ...
        VDP_SetUiVqMinMaxDft((UINT8)e_drv_src_type, u1UiVqItem, &rUiVqMinMaxDft);
...
INT32 MW_PMX_Init(DRV_CUSTOM_DIR_T* pt_drv_cust_dir)
{
    ...
    _au4ZorderCap[PMX_COMP_1][VDP_COMP_1] = PLA_MXR_Z_ORDER_FLAG_0 + PLA_MXR_Z_ORDER_FLAG_1 + PLA_MXR_Z_ORDER_FLAG_2 + PLA_MXR_Z_ORDER_FLAG_3;
    _au4ZorderCap[PMX_COMP_1][VDP_COMP_2] = PLA_MXR_Z_ORDER_FLAG_0 + PLA_MXR_Z_ORDER_FLAG_1 + PLA_MXR_Z_ORDER_FLAG_2 + PLA_MXR_Z_ORDER_FLAG_3;
