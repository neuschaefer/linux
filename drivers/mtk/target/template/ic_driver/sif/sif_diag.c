...
//---------------------------------------------------------------------------
/// Device address of ADV7301A video encoder by Analog Device.

#define EEPROM_DEV_ADDR     0xA0

/// SIF clock divider of 27 Mhz.
...
static INT32 _TestSIFMaster(VOID)
{
    ...
    u2Ret = SIF_Read(SIF_CLK_DIV, EEPROM_DEV_ADDR, 0x00, u1Value, 0x20);

    if (u2Ret != 0x20)
    {
        LOG(1, "Master I2C test fail.\n");
        return -1;
    }
