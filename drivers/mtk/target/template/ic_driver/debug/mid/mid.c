...
{
    // Group 1
    { MID_AGENT_AUDIO,            "audio" },
    { MID_AGENT_DEMUX,            "demux" },
    { MID_AGENT_VDOIN,            "video_in" },
    { MID_AGENT_OSD,              "osd" },
    { MID_AGENT_PSCAN,            "pscan" },
    { MID_AGENT_B2R,              "b2r" },
    { MID_AGENT_CPU,              "cpu" },
    { MID_AGENT_SCPOS,            "scpos" },
    { MID_AGENT_MPEG1,            "mpeg1" },
    // { MID_AGENT_DEMUX_PARSER,     "demux_parser" },
    // { MID_AGENT_DEMUX_1394,       "demux_1394" },
    // { MID_AGENT_VBI,              "vbi" },
    // { MID_AGENT_VDOOUT,           "video_out" },

    // Group 2
    { MID_AGENT_USB1,             "usb1" },
    { MID_AGENT_MPEG2,            "mpge2" },
    { MID_AGENT_JPEG,             "jpeg" },
    { MID_AGENT_SOFT_PARSER,      "soft_parser" },
    { MID_AGENT_DSP,              "dsp" },
    { MID_AGENT_FCI,              "fci" },
    ...
    { MID_AGENT_USB2,             "usb2" },
    { MID_AGENT_TCM,              "tcm" },
    { MID_AGENT_IDETEST,          "idetest" },
    { MID_AGENT_RS232,            "rs232" },
    // { MID_AGENT_1394,             "1394" },
    // { MID_AGENT_POD,              "pod" },
...
{
    return (u4Region >= MID_MAX_RW_REGIONS);
}
...
static BOOL _ResetRegion(UINT32 u4Region)
{
    ...
    if (u4Region == 4)
    {
        DRAM_WRITE32(REG_REGION4UPPER, 0);
        DRAM_WRITE32(REG_REGION4LOWER, 0);
    }
    else
    {
        DRAM_WRITE32(REG_REGION0UPPER + (u4Region * 8), 0);
        DRAM_WRITE32(REG_REGION0LOWER + (u4Region * 8), 0);
    }

    prRegion = &_arRegionState[u4Region];
    prRegion->fgReadOnly = _IsReadOnly(u4Region);
    ...
    UINT32* pu4Addr)
{
    ...
    for (u4Region = 0; u4Region < MID_MAX_REGIONS; u4Region++)
    {
        ...
        if (u4Region == 4)
        {
            u4IntrudeStatus = DRAM_READ32(REG_INTRUDER_4);
        }

        if ((u4IntrudeStatus & INTRU_INTRUDED) != 0)
        {
            UINT32 u4AgentId, u4Addr;

            u4Addr = (u4IntrudeStatus << INTRU_ADDR_SHIFT) & INTRU_ADDR_MASK;

            u4AgentId = (u4IntrudeStatus >> INTRU_AGENTID_SHIFT) &
                INTRU_AGENTID_MASK;
            ...
            if (u4AgentId == (UINT32)MID_AGENT_GROUP2)
            {
                ...
                ASSERT(u4Group2Id <= ((UINT32)MID_AGENT_TEST1 - GROUP2_FIRST_AGENT_ID));
    ...
    UINT32 u4LowAddr, UINT32 u4HighAddr, UINT32 u4Flags)
{
    ...
    BOOL fgReadOnly;
    UINT32 u4RegionBase;
    MID_AGENT_ID_T eOrgAgentId = eAgentId;
    ...
    // Update region state
    prRegion = &_arRegionState[u4Region];
    prRegion->fgReadOnly = fgReadOnly;
    prRegion->fgSet = TRUE;
    prRegion->fgEnabled = FALSE;
    prRegion->u4LowAddr = u4LowAddr;
    prRegion->u4HighAddr = u4HighAddr;
    prRegion->eAgentId = eOrgAgentId;
    prRegion->u4Flags = u4Flags;
                ...
                UINT32 u4Clk)
{
    ...
    {
        u4Val = (u4Val & ~GPAG1_MASK) | GPAG1_ID((UINT32)eAgentId);
        if (fgEnable)
        {
            u4Val |= GPAG1_EN;
        }
    ...
    {
        u4Val = (u4Val & ~GPAG2_MASK) | GPAG2_ID((UINT32)eAgentId);
        if (fgEnable)
        {
            u4Val |= GPAG2_EN;
        }
    ...
    {
        u4Val = (u4Val & ~GPAG3_MASK) | GPAG3_ID((UINT32)eAgentId);
        if (fgEnable)
        {
            u4Val |= GPAG3_EN;
        }
                ...
                UINT32 *pu4AgentId, UINT32 *pu4Enable)
{
    ...
    {
    case 1:
        *pu4Enable = (u4Val >> 12) & 1;
        *pu4AgentId = (u4Val >> 8) & 0xf;
        u4Val = DRAM_READ32(REG_GPAG1_STAT);
        break;
    case 2:
        *pu4Enable = (u4Val >> 13) & 1;
        *pu4AgentId = ((u4Val >> 4) & 0xf) + 0x10;
        u4Val = DRAM_READ32(REG_GPAG2_STAT);
        break;
    case 3:
        *pu4Enable = (u4Val >> 14) & 1;
        *pu4AgentId = ((u4Val >> 0) & 0xf) + 0x10;
        u4Val = DRAM_READ32(REG_GPAG3_STAT);
        break;
    default:
        return 0;
    }
