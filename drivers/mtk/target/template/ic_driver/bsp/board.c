...
//-----------------------------------------------------------------------------
/** _CalibratePLLs() Calibrate all PLLs except DMPLL
 */
//-----------------------------------------------------------------------------
//static void _CalibratePLLs(void)
//{
#if 0
#if CC_DRIVER_PROGRAM
    if (!BSP_IsFPGA())
#else /* CC_DRIVER_PROGRAM */
    if ((!BSP_IsFPGA()) && (g_u4BootRemap==0))
#endif /* CC_DRIVER_PROGRAM */
    {
        // Setup ckgen default value, and then do calibrate...
        BSP_CkgenInit();

        // Start to do Calibrate... Only CPU, APLL0, and APLL1
        // Printf("Calibrate CPUPLL to %d Hz...\n", CPUPLL_DEFAULT_CLOCK);
        // VERIFY(BSP_Calibrate(CAL_SRC_CPUPLL, CPUPLL_DEFAULT_CLOCK));

        // DMPLL has calibrate in assembly.
        // VERIFY(BSP_Calibrate(CAL_SRC_DMPLL, DMPLL_CLOCK));

        Printf("Set PSPLL to %d Hz...\n", PSPLL_DEFAULT_CLOCK);
        VERIFY(BSP_Calibrate(CAL_SRC_PSPLL, PSPLL_DEFAULT_CLOCK));
        Printf("Set VPLL to %d Hz...\n", VPLL_DEFAULT_CLOCK);
        VERIFY(BSP_Calibrate(CAL_SRC_VOPLL, VPLL_DEFAULT_CLOCK));
        Printf("Set ADCPLL to %d Hz...\n", ADCPLL_DEFAULT_CLOCK);
        VERIFY(BSP_Calibrate(CAL_SRC_ADCPLL, ADCPLL_DEFAULT_CLOCK));
        Printf("Set B2RPLL to %d Hz...\n", B2RPLL_DEFAULT_CLOCK);
        VERIFY(BSP_Calibrate(CAL_SRC_B2RPLL, B2RPLL_DEFAULT_CLOCK));

        Printf("Calibrate APLL1 to %d Hz...\n", APLL0_DEFAULT_CLOCK);
        VERIFY(BSP_Calibrate(CAL_SRC_APLL0, APLL0_DEFAULT_CLOCK));
        Printf("Calibrate APLL2 to %d Hz...\n", APLL1_DEFAULT_CLOCK);
        VERIFY(BSP_Calibrate(CAL_SRC_APLL1, APLL1_DEFAULT_CLOCK));

        //for mt8291 VMID charge early, to avoid power on noise.
	IO_WRITE32(CKGEN_BASE,0x400, 0x14515000);
	IO_WRITE32(CKGEN_BASE,0x404, 0x50140000);
    	IO_WRITE32(CKGEN_BASE,0x408, 0x00581155);
    	IO_WRITE32(AUDIO_BASE,0xb8, 0x204218f2); 
    }
#endif
//}
...
{
//    volatile UINT32* p;
#ifdef CC_PWM1_REGULAR_FOR_TUNER
    UINT32 u4Val, u4Pwmp;

    BSP_PinSet(OPWM1_O, PINMUX_FUNCTION1);

    u4Val = BSP_GetDomainClock(SRC_BUS_CLK);
    u4Pwmp = ((u4Val+(48000*128))/(48000*256)) - 1;
    u4Val = (0xff800001 | (u4Pwmp << 4));
    BIM_WRITE32(REG_RW_PWM1, u4Val);
#endif /* CC_PWM2_REGULAR_FOR_TUNER */
#ifdef CC_PWM2_REGULAR_FOR_TUNER
    UINT32 u4Val, u4Pwmp;

    BSP_PinSet(OPWM2_O, PINMUX_FUNCTION1);

    u4Val = BSP_GetDomainClock(SRC_BUS_CLK);
    u4Pwmp = ((u4Val+(48000*128))/(48000*256)) - 1;
    u4Val = (0xff800001 | (u4Pwmp << 4));
    BIM_WRITE32(REG_RW_PWM2, u4Val);
#endif /* CC_PWM2_REGULAR_FOR_TUNER */

    BIM_WRITE32(REG_RW_TIMEOUT_CTRL, 0xffffffff);
/* Move to GPIO_Init() at gpio.c for MW/Demo/Linux sharing
...
void _BSP_Main(void)
{
    ...
    // Time measure point 7, start of Nucleus initial function.
//    BIM_SetTimeLog((g_u4BootRemap==0) ? 7 : 17);
 ...
 */
//-----------------------------------------------------------------------------
void* BSP_AllocDmaMemory(UINT32 u4Size)
{
    ...
    *p = (UINT32)pMem;
    ...
    return (void*)((UINT32)pMem + u4CacheLineSize + sizeof(UINT32));
}
 ...
 */
//-----------------------------------------------------------------------------
BOOL BSP_FreeDmaMemory(void* pUser)
{
 ...
 */
//-----------------------------------------------------------------------------
void* BSP_AllocAlignedDmaMemory(UINT32 u4Size, UINT32 u4Align)
{
    UINT32 u4CacheLineSize, u4AllocSize;
    void* pMem;
    UINT32* p;
 ...
 */
//-----------------------------------------------------------------------------
BOOL BSP_FreeAlignedDmaMemory(void* pUser)
{
    UINT32* p;
    void* pMem;
...
{
    return (BSP_GetMemSize() - DRVCUST_OptGet(eFbmMemSize)) - 0x4000;
}
...
#if 0
//-----------------------------------------------------------------------------
/** BSP_GetPanelTableOffset()
 *  @return the PanelTable offset in NOR Flash.
 */
//-----------------------------------------------------------------------------
UINT32 BSP_GetPanelTableOffset()
{
//    return (UINT32)(0x28000000U + (UINT32)(NORPART_PartStartOffset(2)));
}
...
//-----------------------------------------------------------------------------
/** BSP_GetPQSmartPicOffset()
 *  @return the PQSmartPic offset in NOR Flash.
 */
//-----------------------------------------------------------------------------
UINT32 BSP_GetPQSmartPicOffset()
{
//    return (UINT32)(0x28000000U + (UINT32)(NORPART_PartStartOffset(2)));
}
...
//-----------------------------------------------------------------------------
/** BSP_GetAQDataOffset()
 *  @return the AQData offset in NOR Flash.
 */
//-----------------------------------------------------------------------------
UINT32 BSP_GetAQDataOffset()
{
//    return (UINT32)(0x28000000U + (UINT32)(NORPART_PartStartOffset(2)));
}
#endif
