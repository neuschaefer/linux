                     ...
                     UINT32  ui4_code)
{
    ...
    {
        /* Print an abort statement and abort the Linux thread / process. */
        printf ("ABORT    %s: %6ld  Code: 0x%08lx\n", psz_name, ui4_line_num, ui4_code);

        exit (EXIT_FAILURE);
    }
    
    /* Make certain that we never return. */
    while (TRUE);
}
