...
# $SWAuthor: Iolo Tsai $
# $MD5HEX: 7cd10f09943a18ae1a6943171a666b7f $
#
# Description:
...
all target lib depend : remove_lock
	@echo "*******************************************************************"
	...
	@echo "*******************************************************************"
	@$(RM) $(RM_FLAG) $(SRC:.c=.o)
	@$(MAKE) -f $(TARGET_ROOT)/all.mak $@ SUB_COMP=
	$(call gen_archieve_fct,$(call path_cnvt,$(TARGET_LIB_DIR)),$(AR),$(AR_FLAG))
	
remove_lock :
	@echo "REMOVE LIBRARY LOCKFILE"
	@find $(call path_cnvt,$(ALL_TARGET_LIB_DIR))/ -name *.lock | xargs $(RM) $(RM_REC_FLAG) $(NULL_DEV)
	@find $(call path_cnvt,$(ALL_MW_LIB_DIR))/ -name *.lock | xargs $(RM) $(RM_REC_FLAG) $(NULL_DEV)
	@find $(call path_cnvt,$(ALL_CUSTOM_LIB_DIR))/ -name *.lock | xargs $(RM) $(RM_REC_FLAG) $(NULL_DEV)
	@$(RM) $(RM_REC_FLAG) $(THIS_ROOT)/warning_*


x_inc c_inc : remove_lock
	@$(MAKE) -C $(MW_ROOT) $@ ENTER_OPT_SUB_COMP=TRUE

mw : remove_lock
	@$(MAKE) -C $(MW_ROOT) $@ ENTER_OPT_SUB_COMP=TRUE
	$(call gen_archieve_fct,$(call path_cnvt,$(MW_LIB_DIR)),$(AR),$(AR_FLAG))

custom :
	@$(MAKE) -C $(CUSTOM_ROOT) $@ ENTER_OPT_SUB_COMP=TRUE
...
lint :
	...
	@echo "* NO_OPT_SUB_COMP: $(NO_OPT_SUB_COMP)"
	@echo "*******************************************************************"
	@$(MAKE) -s -f $(TARGET_ROOT)/lint.mak $@ SUB_COMP=


clean clean_target clean_a clean_o clean_d clean_lib clean_lint clean_force :
	@$(MAKE) -s -i -f $(TARGET_ROOT)/clean.mak $@ SUB_COMP=
