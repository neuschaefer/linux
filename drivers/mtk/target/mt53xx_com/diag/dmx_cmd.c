...
static void _QueryPid(UINT8 u1Pidx)
{
    ...
    x_sprintf(szMsg, "Pidx: %u, TS_idx: %d, PID: 0x%x, %s", u1Pidx, rPid.u1TsIndex, rPid.u2Pid, pszType);
    ...
    {
        x_sprintf(szBuf, ", PES: %u", rCounters.u4PesCount);
        x_strcat(szMsg, szBuf);
    }
    if (rCounters.u4PicCount > 0)
    {
        x_sprintf(szBuf, ", Pic: %u", rCounters.u4PicCount);
        x_strcat(szMsg, szBuf);
    }
#if defined(CC_MT5387) || defined(CC_MT5363)    
    if (rCounters.u4OneByteSecCount > 0)
    {
        x_sprintf(szBuf, ", OBSec: %u", rCounters.u4OneByteSecCount);
        x_strcat(szMsg, szBuf);
    }
#endif
    if (rCounters.u4SecCount > 0)
    {
        x_sprintf(szBuf, ", Sec: %u", rCounters.u4SecCount);
        x_strcat(szMsg, szBuf);
    }

    u4DataSize = DATASIZE(rPid.u4Rp, rPid.u4Wp, rPid.u4BufSize);
    x_sprintf(szBuf, ", Errors: %u, data: %u\n", rCounters.u4TotalErrors,
        u4DataSize);
    x_strcat(szMsg, szBuf);
...
static INT32 _DMXCLI_SetFrontEnd(INT32 i4Argc, const CHAR** aszArgv)
{
    ...
    {
        Printf("%s [FE type]\n", aszArgv[0]);
        Printf("   0: Serial0, 1: Serial1, 2: Parallel, 3: 2bit_0, 4: 2bit_1\n");
        Printf("Current FE type: %u\n", (UINT32)DMX_GetFrontEnd());
        return 0;
    }
...
static void _ShowSetPidUsage(const CHAR* szCmd)
{
    Printf("Usage: %s pid_index(0-%u) pid [pid_type(pes,video,audio,none]"
        ...
        szCmd, DMX_NUM_PID_INDEX - 1, DMX_MAX_CA_MODE, DMX_NUM_KEY_INDEX - 1);
}
...
static INT32 _DMXCLI_CaptureAllocBuf(INT32 i4Argc, const CHAR** aszArgv)
{
    ...
    {
        if(!BSP_FreeAlignedDmaMemory((void*)_u4CaptureBufAddr))
        {
            return -1;
        }
 ...
 */
//-----------------------------------------------------------------------------
static INT32 _DMXCLI_PS_MoveTest(INT32 i4Argc, const CHAR** aszArgv)
{
    UINT32 u4DMAStart, u4DMASize, u4TotalSize, u4RemainedSize, u4ChunkSize;
    DMX_MM_DATA_T rData;

    if (i4Argc != 3)
    {
        Printf("Usage: mt source_addr total_len\n");
        return -1;
    }

    // Warning: The input values are not checked here.
    u4DMAStart = StrToInt(aszArgv[1]);
    u4DMASize = StrToInt(aszArgv[2]);

    rData.u4BufStart = u4DMAStart;
    rData.u4BufEnd = u4DMAStart + u4DMASize;
    rData.u4StartAddr = u4DMAStart;

    u4TotalSize = 0x2000B0;         // transfer about 2MB (a multiple of 188)
    u4RemainedSize = u4TotalSize;
    while (u4RemainedSize > 0)
    {
        u4ChunkSize = random(0x80000);    // 512KB
        if (u4ChunkSize < 0x5000)
        {
            u4ChunkSize = 0x5000;         // 20KB
        }
        if (u4ChunkSize > u4RemainedSize)
        {
            u4ChunkSize = u4RemainedSize;
            if ((u4ChunkSize % 188) != 0)
            {
                Printf("The remaining data size must be a multiple of 188.\n");
                return -1;
            }
        }
        u4ChunkSize -= (u4ChunkSize % 188);   // make it 188*N
        rData.u4FrameSize = u4ChunkSize;
        if (!DMX_DDI_MoveData(&rData))
        {
            Printf("Error!\n");
            return -1;
        }
        u4RemainedSize -= u4ChunkSize;
        rData.u4StartAddr += u4ChunkSize;
    }
...
static CLI_EXEC_T _arDmxPSCmdTbl[] =
{
    ...
    {
        "movetest", "mt", _DMXCLI_PS_MoveTest, NULL, "mt source_addr total_len", CLI_SUPERVISOR
    },
