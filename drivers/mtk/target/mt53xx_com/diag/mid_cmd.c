...
//-----------------------------------------------------------------------------
/** _ShowAgentList() to display all MID agents
 */
//-----------------------------------------------------------------------------
#if defined(CC_MT5387) && !defined(CC_MT5363)
static void _ShowAgentList(void)
{
    UINT32 i;
...
//        MID_AGENT_INVALID
    ...
    for (i = 0; i < sizeof(aeAgents) / sizeof(MID_AGENT_ID_T); i++)
    {
        const CHAR* szAgent;

        szAgent = MID_AgentIdToString(aeAgents[i]);
        Printf("  %2u: %s\n", (UINT32)aeAgents[i], szAgent);
    }
}
...
static INT32 _CmdRegionList(INT32 i4Argc, const CHAR** aszArgv)
{
    ...
    {
        REGION_STATE_T rRegion;

        if (!MID_GetRegionState(u4Region, &rRegion))
        {
            MID_Init();
            MID_GetRegionState(u4Region, &rRegion);
        }

        Printf("\n  Region %u %s %s: ", u4Region,
            ((!rRegion.fgSet) || (rRegion.u4Flags & MID_FLAG_WRITEABLE)) ? "" : "(RO)",
            rRegion.fgEnabled ? "enabled " : "disabled");

        if (rRegion.fgSet)
        {
            if (u4Region < MID_MAX_RW_REGIONS)
            {
                Printf("0x%08x - 0x%08x: %s [%c%c]", rRegion.u4LowAddr,
                    rRegion.u4HighAddr, MID_AgentIdToString(rRegion.eAgentId),
                    (rRegion.u4Flags & MID_FLAG_INVERSE) ? 'I' : 'i',
                    (rRegion.u4Flags & MID_FLAG_CPUACCESS) ? 'C' : 'c');
            }
            else if (u4Region < MID_SINGLE_REGIONS)
            {
                Printf("0x%08x - 0x%08x: %s [%c]", rRegion.u4LowAddr,
                    rRegion.u4HighAddr, MID_AgentIdToString(rRegion.eAgentId),
                    (rRegion.u4Flags & MID_FLAG_WRITEABLE) ? 'R' : 'r');
            }
            else
            {
                Printf("0x%08x - 0x%08x: [%c]", rRegion.u4LowAddr,
                    rRegion.u4HighAddr, (rRegion.u4Flags & MID_FLAG_WRITEABLE) ? 'R' : 'r');
                _CmdPrintAgentList(rRegion.u4AgentList);
            }
        }
        else
        {
