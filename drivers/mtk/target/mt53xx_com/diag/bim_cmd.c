...
EXTERN INT32 CpiMeterQuery(UINT32 u4Action, UINT64 *pu8Instruction, UINT64 *pu8Cycle);
...
static INT32 _CmdCacheMeter(INT32 i4Argc, const CHAR** aszArgv)
{
    ...
    Printf("I-Cache Request = 0x%x(%u)\nI-Cache Miss    = 0x%x(%u)\nI-Cache Penalty = 0x%x(%u)\n", 
            u4IReq, u4IReq,
            u4IMiss, u4IMiss,
            u4IPenalty, u4IPenalty);
    Printf("D-Cache Request = 0x%x(%u)\nD-Cache Miss    = 0x%x(%u)\nD-Cache Penalty = 0x%x(%u)\n", 
            u4DReq, u4DReq,
            u4DMiss, u4DMiss,
            u4DPenalty, u4DPenalty);

    return 0;
}

static INT32 _CmdCpiMeter(INT32 i4Argc, const CHAR** aszArgv)
{
    UINT32 u4Action;
    UINT64 u8Cycle, u8Instruction;

    if (i4Argc == 1)
    {
        Printf("usage: action={0=off, 1=on, 2=reset}\n");
        return 0;
    }

    u4Action = StrToInt(aszArgv[1]);
    CpiMeterQuery(u4Action, &u8Instruction, &u8Cycle);
    if (u4Action != 2)
    {
        Printf("cycle per 100 ins.=%d\n", (UINT32)((u8Instruction!=0)?(u8Cycle*100/u8Instruction):0));
    }
