 ...
 */
//-------------------------------------------------------------------------
static void _DramMesaure(const CHAR* szAgent, UINT32 u4DelaySecond)
{
    UINT32 u4Mod;
    UINT32 u4DelayClock;
    UINT32 u4Result;
    UINT64 u8Result;
    BOOL fg32Bit;
    fg32Bit = IO_READ32MSK(DRAM_BASE, 0x28, 3 << 21) > 0;
    ...
    if (x_strcmp(szAgent, "mpeg") == 0)
    {
        u4Mod = SRM_DRV_MPEG;
    }
    else
    if (x_strcmp(szAgent, "jpeg") == 0)
    {
        u4Mod = SRM_DRV_JPEG;
    }
    else
    ...
    u4DelayClock = (BSP_GetDomainClock(CAL_SRC_MEM_CLK)) * u4DelaySecond;
    #else
    u4DelayClock = (BSP_GetDomainClock(CAL_SRC_DMPLL) >> 1) * u4DelaySecond;
    #endif

    u4Result = SRM_DramMonitor(u4Mod, u4DelayClock, u4DelaySecond * 1000);

    u8Result = (UINT64)u4Result;
    _u8Total += u8Result;
    u8Result *= 1000;
    u8Result = u8Div6432(u8Result, (UINT64)u4DelayClock, NULL);
    ...
    Printf("Agent(%s) = %d o/oo (%d/%d) %d Byte/s\n", szAgent, (UINT32)u8Result,
           u4Result, u4DelayClock, u4Result*((fg32Bit)?16:8)/u4DelaySecond);
...
static INT32 _AvDramMeasureCmd (INT32 i4Argc, const CHAR ** szArgv)
{
    if ((i4Argc >= 2) && (szArgv != NULL) && (szArgv[1] != NULL))
    {
        ...
        for (u4Val = 0; u4Val < u4Loop; u4Val++)
        {
            ...
            {
                _DramMesaure("audio", u4DelaySecond);
                _DramMesaure("demux", u4DelaySecond);
                _DramMesaure("vdoin", u4DelaySecond);
                _DramMesaure("osd", u4DelaySecond);
                _DramMesaure("nr", u4DelaySecond);
                _DramMesaure("b2r", u4DelaySecond);
                _DramMesaure("cpu", u4DelaySecond);
                _DramMesaure("scpos", u4DelaySecond);
                _DramMesaure("mpeg", u4DelaySecond);
                _DramMesaure("jpeg", u4DelaySecond);
                _DramMesaure("gfx_h", u4DelaySecond);
                _DramMesaure("gfx_l", u4DelaySecond);
                _DramMesaure("dsp", u4DelaySecond);
            }
            else
            if (x_strcmp(szArgv[1], "all") == 0)
            {
                ...
                _DramMesaure("mpeg", u4DelaySecond);
                _DramMesaure("jpeg", u4DelaySecond);
                _DramMesaure("usb", u4DelaySecond);
                _DramMesaure("gdma", u4DelaySecond);
                _DramMesaure("ddi", u4DelaySecond);
                _DramMesaure("dsp", u4DelaySecond);
                _DramMesaure("fci", u4DelaySecond);
                _DramMesaure("gfx_h", u4DelaySecond);
                _DramMesaure("gfx_l", u4DelaySecond);
                _DramMesaure("tcm", u4DelaySecond);
                _DramMesaure("idetest", u4DelaySecond);
                _DramMesaure("uart", u4DelaySecond);
            }
            else
            {
                _DramMesaure(szArgv[1], u4DelaySecond);
            }
        }
        
        #if defined(CC_MT8223)
        u4DelayClock = (BSP_GetDomainClock(CAL_SRC_MEM_CLK) * u4DelaySecond) / 1000;
        #else
        u4DelayClock = ((BSP_GetDomainClock(CAL_SRC_DMPLL) >> 1) * u4DelaySecond) / 1000;
        #endif
        _u8Total = u8Div6432(_u8Total, (UINT64)u4DelayClock, NULL);
        Printf("Total = %d, Max = %d\n", _u8Total / u4Loop, _u4MaxValue);
    }
    else
    {
        Printf("av.dram agent [period second] [loop]\n");
        Printf("agent = audio, demux, vdoin, osd, nr, b2r, cpu, scpos, mpeg, jpeg, usb, gdma, ddi, fci, dsp, gfx_h gfx_l, tcm, idetest, uart\n");
    }
