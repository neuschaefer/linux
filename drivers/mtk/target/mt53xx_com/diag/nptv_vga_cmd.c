...
static INT32 _i4SOGSlicerSel(INT32 i4Argc, const CHAR **szArgv); //wu add
#endif
...
CLI_EXEC_T arVgaCmdTbl[] = {
  ...
  #if SUPPORT_SET_SLICER
  {"slicersel", "slicers", _i4SOGSlicerSel, NULL, "slicers [0~15] ", CLI_SUPERVISOR},////wu add
  #endif
...
static INT32 _i4SOGSlicerSel(INT32 i4Argc, const CHAR **szArgv)  //wu add
{
	...
	if(i4Argc==2)
	 {
		if (x_strcmp(szArgv[0], "slicers") == 0)
            wDrvSOGSetSlicer(u1Path,StrToInt(szArgv[1])&0xff);
...
//    return  fgApiVideoIsVgaMode((UINT8)u4VdpId, 0);
//}
