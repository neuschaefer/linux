...
#define    POD_POLLING_MODE    1
#endif

#ifndef POD_POLLING_MODE
#define    POD_POLLING_MODE    0
#endif
...
static INT32 _PcmciaRegDefaultValueTest(UINT32 REG, UINT32 MSK, UINT32 VAL)
{
    ...
    {
        LOG(0, "Reg Default Error (0x%X): Read:0x%X, Predict:0x%X\n", REG, (PCMCIA_RawRead32(REG) & MSK), VAL);
        return -1;
    }
    return 0;
}
...
static INT32 _PcmciaRegRwTest(UINT32 REG, UINT32 MSK)
{
    ...
    {
        LOG(0, "RW Reg Test Failed1(0x%X) : Write:0x%X, Read:0x%X\n", REG, PATN_Fs&MSK, u4RegValue);
        return -1;
    }
    ...
    {
        LOG(0, "RW Reg Test Failed2(0x%X) : Write:0x%X, Read:0x%X\n", REG, PATN_0s&MSK, u4RegValue);
        return -1;
    }
    ...
    {
        LOG(0, "RW Reg Test Failed3(0x%X) : Write:0x%X, Read:0x%X\n", REG, PATN_As&MSK, u4RegValue);
        return -1;
    }
    ...
    {
        LOG(0, "RW Reg Test Failed4(0x%X) : Write:0x%X, Read:0x%X\n", REG, PATN_5s&MSK,  u4RegValue);
        return -1;
    }
...
static BOOL _PcmciaCmdRegDefaultValueTest(void)
{
    ...
    if ( ePcmciaChipType == PCMCIA_CHIP_TYPE_EXTERNAL ) 
    _PcmciaRegDefaultValueTest(REG_VERSION_CODE,        0x000000FF, 0x00000008);    // 0x8
...
static INT32 _PcmciaParseCis(INT32 i4Argc, const CHAR** szArgv)
{
    ...
    if (u4CisLen > 0)
    {
        ...
        if (i4Return == POD_OK)
        {
            LOG(0, "POD module type: %s\n", rCisInfo.fgPodModuleType ? "Yes" : "No");
            LOG(0, "Required 5V Vpp: %s\n", rCisInfo.fgVpp ? "Yes" : "No");
            LOG(0, "COR value = 0x%x\n", rCisInfo.u1CorValue);
            LOG(0, "COR address = 0x%x\n", rCisInfo.u2CorAddr);
        }
    }
    else
    {
...
static BOOL _PcmciaWaitForDataAvailable(void)
{
    ...
    {
#if (POD_POLLING_MODE == 1)
        if (PCMCIA_WaitForModuleStatus(DA, 1) == POD_OK) /* Test DA */
        {
            break;
        }
#else /* Interrupt mode */
        if (PCMCIA_GetPodState(&u4PodState) == POD_OK)
        {
            if (u4PodState & POD_DATA_AVIL) /* Test DA */
            {
                break;
            }
        }
...
static INT32 _PcmciaCmdCreateTC(INT32 i4Argc, const CHAR** szArgv)
{
    ...
    {
        i4Return = PCMCIA_Write(eIoState, &au1CiMsgXmt[POD_CREATE_T_C][MSG_POS],
                                (UINT16)(au1CiMsgXmt[POD_CREATE_T_C][MSG_LEN_POS]));
    }
    else
    {
        i4Return = PCMCIA_Write(eIoState, &au1MsgXmt[POD_CREATE_T_C][MSG_POS],
    ...
    i4Return = PCMCIA_Read(eIoState, pu1MsgRec, &u2MsgRecLen);
    if (i4Return != POD_OK)
    {
        return i4Return;
    }
    ...
    if ( ePcmciaCardType == PCMCIA_CARD_TYPE_CI )
    {
        ...
        {
            LOG(1, "++++++++Diable TC reply fail\n");
        }
    }
    else
    {}
    
    PCMCIA_Dump("Create_T_C_Reply()", pu1MsgRec, u2MsgRecLen);

    return i4Return;
}

static BOOL _PcmciaGetTransSessionNum()
{
    if (!_PcmciaWaitForDataAvailable())
    {
        return FALSE;
    ...
    /* Get T_SB() */
    u2MsgRecLen = MAX_MSG_REC_BUF_SIZE;
    if (PCMCIA_Read(eIoState, pu1MsgRec, &u2MsgRecLen) != POD_OK)
    {
        return FALSE;
    }
...
static BOOL _PcmciaStartPodPollProtocol(BOOL bPoll)
{
    LOG(1, "Polling starts...\n");
    CLI_Parser("p.d_off");
    if (bPoll)
    {
        while (1)
        {
            /* Send T_Data_Last() */
            if (PCMCIA_Write(eIoState, &au1MsgXmt[POD_T_DATA_LAST][MSG_POS],
                          (UINT16)(au1MsgXmt[POD_T_DATA_LAST][MSG_LEN_POS])) != POD_OK)
            {
                return FALSE;
            }

            /* Get T_SB() */
            if (!_PcmciaGetTransSessionNum())
            {
                return FALSE;
            }

            if (pu1MsgRec[u2MsgRecLen-1] & DA) /* DA */
            {
                break;
            }
    ...
    /* Send T_RCV() */
    if (PCMCIA_Write(eIoState, &au1MsgXmt[POD_T_RCV][MSG_POS],
                  (UINT16)(au1MsgXmt[POD_T_RCV][MSG_LEN_POS])) != POD_OK)
    {
        return FALSE;
    }
...
static BOOL _PcmciaOpenASession(UINT16 u2SessionNum)
{
    /* Open a session */
    if (!_PcmciaStartPodPollProtocol(TRUE))
    {
        return FALSE;
    }

    /* Wait for open_session_request() */
    u2MsgRecLen = MAX_MSG_REC_BUF_SIZE;
    if (PCMCIA_Read(eIoState, pu1MsgRec, &u2MsgRecLen) != POD_OK)
    {
        return FALSE;
    ...
    PCMCIA_Dump("open_session_request()", pu1MsgRec, u2MsgRecLen);

    /* Send open_session_response() */
    au1MsgXmt[POD_OPEN_SESSION_RESPONSE][9] = pu1MsgRec[7]; /* resource_identifier() */
    au1MsgXmt[POD_OPEN_SESSION_RESPONSE][10] = pu1MsgRec[8];
    au1MsgXmt[POD_OPEN_SESSION_RESPONSE][11] = pu1MsgRec[9];
    au1MsgXmt[POD_OPEN_SESSION_RESPONSE][12] = pu1MsgRec[10];
    au1MsgXmt[POD_OPEN_SESSION_RESPONSE][13] = (UINT8)(u2SessionNum >> 8); /* session_nb */
    au1MsgXmt[POD_OPEN_SESSION_RESPONSE][14] = (UINT8)(u2SessionNum & 0xFF);
    HalFlushInvalidateDCache();

    if (PCMCIA_Write(eIoState, &au1MsgXmt[POD_OPEN_SESSION_RESPONSE][MSG_POS],
                  (UINT16)(au1MsgXmt[POD_OPEN_SESSION_RESPONSE][MSG_LEN_POS])) != POD_OK)
    {
        return FALSE;
    }

    LOG(5, "Send open_session_response()\n");
    ...
    if (pu1MsgRec[u2MsgRecLen-1] & DA)
    {
        LOG(5, "Unexpected data available!\n");
        return FALSE;
    }
...
static INT32 PcmciaCmdInit(INT32 i4Argc, const CHAR** szArgv)
{
    ...
    /* Update POD current condition */
    PCMCIA_RefreshPodState();

    i4Return = PCMCIA_GetPodState(&u4PodState);
    if (i4Return == POD_OK)
    {
        if ( u4PodState & POD_INSERTED )
        {
            /* Register PCMCIA/POD/CI IRQ handler */
            i4Return = PCMCIA_EnableIsr();
            if (i4Return == POD_OK)
            {
                /* Perform POD init */
...
static INT32 PcmciaCmdPinSet (INT32 i4Argc, const CHAR ** szArgv)
{
    if(i4Argc != 3)
    {
        LOG(0, "Usage: %s [PinIndex][FunctionNum\n", szArgv[0]);
        return 0;
    }
    
    PCMCIA_MT8295PinSet(StrToInt(szArgv[1]), StrToInt(szArgv[2]));
    return 0;
}
...
static INT32 PcmciaCmdSingleRegTest (INT32 i4Argc, const CHAR ** szArgv)
{
    ...
    else    // Read Only
    {
        ...
        for ( i = 0 ; i < i4Loops ; i++ )
        {
            ...
            {
                LOG(0, "Test Failed! Org:0x%X,, Read:0x%X\n", u4OrgValue, u4TempValue);
            }
        }
    }
...
static INT32 PcmciaCmdGetCis(INT32 i4Argc, const CHAR** szArgv)
{
    ...
    PCMCIA_Dump("CIS map", pu1CurCisMap, MAX_CIS_MAP_SIZE);

    {
        UINT16 addrcor;
        UINT8 cor;
        PCMCIA_CheckCIS(pu1CurCisMap, &addrcor, &cor);
        LOG(1, "cor:0x%x, val:0x%x\n", addrcor, cor);
    }
...
static INT32 PcmciaCmdCreateTransConn(INT32 i4Argc, const CHAR** szArgv)
{
    INT32           i4Return;
...
static INT32 PcmciaCmdOpenRmSession(INT32 i4Argc, const CHAR** szArgv)
{
    INT32           i4Return;

    /* Open a session */
    if (!_PcmciaOpenASession(RM_SESSION_NUM))
    {
        return POD_UNKNOWN_COND;
    }

    /* Application layer (Resource Manager) */
    /* Send profile_inq() */
    i4Return = PCMCIA_Write(eIoState, &au1MsgXmt[POD_PROFILE_INQ][MSG_POS],
                         (UINT16)(au1MsgXmt[POD_PROFILE_INQ][MSG_LEN_POS]));
    ...
    if (pu1MsgRec[u2MsgRecLen-1] & DA)
    {
        _PcmciaStartPodPollProtocol(FALSE);
    }
    else
    {
        _PcmciaStartPodPollProtocol(TRUE);
    }

    /* Wait for profile_reply() */
    u2MsgRecLen = MAX_MSG_REC_BUF_SIZE;
    i4Return = PCMCIA_Read(eIoState, pu1MsgRec, &u2MsgRecLen);
    if (i4Return != POD_OK)
    {
    ...
    PCMCIA_Dump("profile_reply()", pu1MsgRec, u2MsgRecLen);

    /* Send profile_changed() */
    i4Return = PCMCIA_Write(eIoState, &au1MsgXmt[POD_PROFILE_CHANGED][MSG_POS],
                         (UINT16)(au1MsgXmt[POD_PROFILE_CHANGED][MSG_LEN_POS]));
    if (i4Return != POD_OK)
    {
        return POD_WRITE_ERROR;
    }

    LOG(5, "Send profile_changed()\n");
    ...
    PCMCIA_Dump("profile_inq()", pu1MsgRec, u2MsgRecLen);

    /* Send profile_reply() */
    i4Return = PCMCIA_Write(eIoState, &au1MsgXmt[POD_PROFILE_REPLY][MSG_POS],
                         (UINT16)(au1MsgXmt[POD_PROFILE_REPLY][MSG_LEN_POS]));
    if (i4Return != POD_OK)
    {
        return POD_WRITE_ERROR;
    }

    LOG(5, "Send profile_reply()\n");
...
static INT32 PcmciaCmdOpenAiSession(INT32 i4Argc, const CHAR** szArgv)
{
    INT32           i4Return;

    /* Open a session */
    if (!_PcmciaOpenASession(AI_SESSION_NUM))
    {
        return POD_UNKNOWN_COND;
    }

    /* Application layer (Application Information) */
    /* Send application_info_req() */
    i4Return = PCMCIA_Write(eIoState, &au1MsgXmt[POD_APPLICATION_INFO_REQ][MSG_POS],
                         (UINT16)(au1MsgXmt[POD_APPLICATION_INFO_REQ][MSG_LEN_POS]));
    if (i4Return != POD_OK)
    {
        return POD_WRITE_ERROR;
    }

    LOG(5, "Send application_info_req()\n");
    ...
    PCMCIA_Dump("application_info_cnf()", pu1MsgRec, u2MsgRecLen);
...
static INT32 PcmciaCmdOpenCaSession(INT32 i4Argc, const CHAR** szArgv)
{
    INT32           i4Return;

    /* Open a session */
    if (!_PcmciaOpenASession(CA_SESSION_NUM))
    {
        return POD_UNKNOWN_COND;
    }

    /* Application layer (Conditional Access) */
    /* Send ca_info_inq() */
    i4Return = PCMCIA_Write(eIoState, &au1MsgXmt[POD_CA_INFO_INQ][MSG_POS],
                         (UINT16)(au1MsgXmt[POD_CA_INFO_INQ][MSG_LEN_POS]));
    if (i4Return != POD_OK)
    {
        return POD_WRITE_ERROR;
    }

    LOG(5, "Send ca_info_inq()\n");
    ...
    PCMCIA_Dump("ca_info()", pu1MsgRec, u2MsgRecLen);
...
static INT32 PcmciaCmdOpenMmiSession(INT32 i4Argc, const CHAR** szArgv)
{
    /* Open a session */
    if (!_PcmciaOpenASession(MMI_SESSION_NUM))
    {
        return POD_UNKNOWN_COND;
    }
...
static INT32 PcmciaCmdDmaTest(INT32 i4Argc, const CHAR** szArgv)
{
    ...
    for (u4I = 0; u4I < u4Loop; u4I++)
    {
        ...
        i4Return = PcmciaCmdInit(0, NULL);
        if (i4Return != POD_OK)
        {
            break;
        }

        i4Return = PcmciaCmdCreateTransConn(0, NULL);
        if (i4Return != POD_OK)
        {
            break;
        }

        i4Return = PcmciaCmdOpenRmSession(0, NULL);
        if (i4Return != POD_OK)
        {
            break;
        }
...
static INT32 PcmciaCmdInteractiveTest(INT32 i4Argc, const CHAR** szArgv)
{
    INT32       i4Return;

    /* Perform POD DMA interactive test */
    i4Return = PcmciaCmdInit(0, NULL);
    if (i4Return != POD_OK)
    {
        return i4Return;
    ...
    i4Return = PcmciaCmdOpenAiSession(0, NULL);
    if (i4Return != POD_OK)
    {
        return i4Return;
    }

    i4Return = PcmciaCmdOpenCaSession(0, NULL);
    if (i4Return != POD_OK)
    {
        return i4Return;
    }

    i4Return = PcmciaCmdOpenMmiSession(0, NULL);
...
static INT32 PcmciaCmdDiag(INT32 i4Argc, const CHAR** szArgv)
{
    INT32  i4Return;
    ...
    LOG(0, "\n================\n");
    LOG(0, " POD diagnostic");
    LOG(0, "\n================\n");
    LOG(0, "\nPlease have your HPNX ready...\n");
    LOG(0, "Host-POD interface test (via HPNX): ");

    i4Return = PcmciaCmdInteractiveTest(0, NULL);
    Printf("%s\n", ((i4Return == POD_OK) ? "Success" : "Fail"));
...
static INT32 PcmciaCmdSpiTest(INT32 i4Argc, const CHAR** szArgv)
{
    INT32   i4Return = POD_OK;
    UINT32  u4Addr;
    UINT32  u4DataLen;
    ...
    {
        LOG(0, "dt [loop][len (1~1024)][1:variant len test]\n");
        return -1;
    }
    ...
    for ( i = 1 ; i <= u4TestLoop ; i++ )
    {
        ...
        for(j=0; j<(u4DataLen*4); j++)
        {
            ...
            {
                LOG(0, "Cmp fail, Length:%d, Predict:0x%08x, Read:0x%08x\n", u4DataLen, pu4GoldBuf[j], pu4TestBuf[j]);
            }
        }
    }
    ...
    if ( u4VariantLenTest == 1 )
    {
        ...
        for ( i = 1 ; i < 1024 ; i++ )
        {
            ...
            for(j=0; j<(u4DataLen*4); j++)
            {
                if ( pu4GoldBuf[j] != pu4TestBuf[j] )
                {
                    LOG(0, "Cmp fail, Length:%d, Predict:0x%x, Read:0x%x\n", u4DataLen, pu4GoldBuf[j], pu4TestBuf[j]);
                }
            }
        }
    }
    LOG(0, "Test Done\n");
    BSP_FreeAlignedDmaMemory((void*)pu4TestBuf);
    BSP_FreeAlignedDmaMemory((void*)pu4GoldBuf);
    return i4Return;
}
...
static CLI_EXEC_T _arSPICmdTbl[] =
{
    ...
    {"dmatest",     "dt",    PcmciaCmdSpiTest,      NULL,   "DMA test",             CLI_SUPERVISOR},
    {NULL,          NULL,    NULL,                  NULL,   NULL,                   CLI_SUPERVISOR}
};
...
static CLI_EXEC_T _arPodSessionCmdTbl[] =
{
    {"tc",      "tc",       PcmciaCmdCreateTransConn,   NULL,   "Create tranport connection (DMA)",         CLI_SUPERVISOR},
    {"rm",      "rm",       PcmciaCmdOpenRmSession,     NULL,   "Open RM session (DMA)",                    CLI_SUPERVISOR},
    {"ai",      "ai",       PcmciaCmdOpenAiSession,     NULL,   "Open AI session (DMA)",                    CLI_SUPERVISOR},
    {"ca",      "ca",       PcmciaCmdOpenCaSession,     NULL,   "Open CA session (DMA)",                    CLI_SUPERVISOR},
    {"mmi",     "mmi",      PcmciaCmdOpenMmiSession,    NULL,   "Open MMI session (DMA)",                   CLI_SUPERVISOR},
    {"t",       "test",     PcmciaCmdDmaTest,           NULL,   "DMA test",                                 CLI_SUPERVISOR},
    {"it",      "itest",    PcmciaCmdInteractiveTest,   NULL,   "DMA interactive test => RM->AI->CA->MMI",  CLI_SUPERVISOR},
    {NULL,      NULL,       NULL,                       NULL,   NULL,                                       CLI_SUPERVISOR}
};
...
static CLI_EXEC_T _arIcVfyCmdTbl[] =
{
    ...
    {"diag",    "diag",     PcmciaCmdDiag,          NULL,   "POD driver diagnostic",                CLI_SUPERVISOR},
    ...
    {"ps",      "ps",       PcmciaCmdPinSet,        NULL,   "Set MT8295 PinMux",                    CLI_SUPERVISOR},
    {"ses",     "ses",      NULL,   _arPodSessionCmdTbl,    "Session operation",                    CLI_SUPERVISOR},
    {NULL,      NULL,       NULL,                   NULL,               NULL,                       CLI_SUPERVISOR}
};
