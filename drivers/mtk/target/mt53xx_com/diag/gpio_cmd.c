...
{
    INT32 i4SelNum, i4FuncNo;

    if (i4Argc < 2)
    {
        Printf("%s [sel num] [[func no.]]\n", aszArgv[0]);
        return 1;
    }

    i4SelNum = StrToInt(aszArgv[1]);
    if (i4Argc < 3)
    {
        i4FuncNo = BSP_PinGet(i4SelNum);
        Printf("SelNum:%d function is %d\n", i4SelNum, i4FuncNo);
        return 0;
    }
    i4FuncNo = StrToInt(aszArgv[2]);
    BSP_PinSet(i4SelNum, i4FuncNo);
    Printf("SelNum:%d function set %d\n", i4SelNum, i4FuncNo);
    return 0;
}
