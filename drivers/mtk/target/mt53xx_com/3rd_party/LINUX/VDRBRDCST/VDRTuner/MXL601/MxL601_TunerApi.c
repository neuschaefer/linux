...
const UINT8 MxLWareDrvVersion[] = {2, 1, 10, 8}; 

// Candidate version number, 0 means for release version 
const UINT8 BuildNumber = 0; 

static XTAL_SPUR_CFG_T xtalSpurCfg[MAX_MXL601_DEVICES] = {{0}};

static IF_FCW_CFG_T ifFcwCfg[MAX_MXL601_DEVICES] = {{0}};
...
static MXL_STATUS MxL601_ConfigDevReset(PMXL_RESET_CFG_T resetCfg)
{
...
static MXL_STATUS MxL601_ConfigDevOverwriteDefault(PMXL_OVERWRITE_DEFAULT_CFG_T overDefaultPtr)
{
  UINT8 status = MXL_TRUE;
  UINT8 readData = 0;
...
static MXL_STATUS MxL601_ConfigDevPowerMode(PMXL_PWR_MODE_CFG_T pwrModePtr)
{
  UINT8 status = MXL_TRUE;
  UINT8 i2cAddr; 
...
static MXL_STATUS MxL601_ConfigDevXtalSet(PMXL_XTAL_SET_CFG_T xtalSetPtr)
{
  UINT8 status = MXL_TRUE;
  UINT8 control = 0;
...
static MXL_STATUS MxL601_ConfigDevIfOutSet(PMXL_IF_OUT_CFG_T ifOutPtr)
{
  UINT16 ifFcw;
  UINT8 status = MXL_TRUE;
  UINT8 readData = 0;
  ...
  MxL_DLL_DEBUG0(("MXL_DEV_IF_OUT_CFG: Manual set = %d \n", ifOutPtr->ManualFreqSet)); 
...
static MXL_STATUS  MxL601_ConfigDevGPO(PMXL_GPO_CFG_T gpoParamPtr)
{
  UINT8 status = MXL_TRUE;
  UINT8 regData = 0;
...
static MXL_STATUS MxL601_GetDeviceInfo(PMXL_DEV_INFO_T DevInfoPtr)
{
  UINT8 status = MXL_TRUE;
  UINT8 readBack = 0;
  UINT8 i2cAddr = 0;
...
static MXL_STATUS  MxL601_GetDeviceGpoState(PMXL_GPO_INFO_T gpoPtr)
{
...
static MXL_STATUS  MxL601_GetDevicePllState(PMXL_PLL_INFO_T pllPtr)
{
  UINT8 status = MXL_TRUE;
  UINT8 condition[3] = {0};
  ...
  MxL_DLL_DEBUG0(("MXL_DEV_PLL_STATE_REQ \n")); 
...
static MXL_STATUS MxL601_ConfigTunerPowerUp(PMXL_POWER_UP_CFG_T topMasterCfgPtr) 
{
  UINT8 status = MXL_TRUE;
  UINT8 regData = 0; 
...
static MXL_STATUS MxL601_ConfigTunerSequenceSet(PMXL_START_TUNE_CFG_T sequnCfgPtr) 
{
  UINT8 status = MXL_TRUE;
  UINT8 regData = 0;
  UINT8 idx = 0;
...
static MXL_STATUS MxL601_ConfigTunerApplicationModeSet(PMXL_TUNER_MODE_CFG_T modeSetPtr)
{
  UINT8 status = MXL_TRUE;
  UINT8 dfeRegData = 0;
  ...
  
  {
    ...
    switch(modeSetPtr->SignalMode)
    {
      ...
      case DIG_ISDBT_ATSC:
        ...
        {
          // Low power IF dependent settings
          status |= Ctrl_WriteRegister(modeSetPtr->I2cSlaveAddr, DIG_ANA_IF_CFG_0, 0xF9);
  ...
  if (MXL_FALSE != status)
  {
    //status |= Ctrl_WriteRegister(modeSetPtr->I2cSlaveAddr, XTAL_EXT_BIAS_REG, 0x00);
...
static MXL_STATUS MxL601_ConfigTunerAgcSet(PMXL_AGC_SET_CFG_T agcSetPtr)
{
  UINT8 status = MXL_TRUE;
  UINT8 regData = 0; 
...
static MXL_STATUS MxL601_ConfigTunerFineTune(PMXL_FINE_TUNE_CFG_T fineTunePtr)
{
  UINT8 status = MXL_TRUE;
  UINT8 regData = 0;
  ...
  if (MXL_FINE_TUNE_STEP_UP == fineTunePtr->ScaleUp)
	...
	MxL_DLL_DEBUG0(("[MXL601]Tuner fine-tune up mode\n"));
  	}
  else 
  	{
    regData |= FINE_TUNE_FREQ_DECREASE;
	MxL_DLL_DEBUG0(("[MXL601]Tuner fine-tune down mode\n"));
  	}
...
static MXL_STATUS MxL601_ConfigSpurShiftAdjustment(PMXL_SPUR_SHIFT_ADJ_CFG_T spurShiftAdjPtr)
{
  UINT8 status = MXL_TRUE;
  UINT8 regData = 0;
...
static MXL_STATUS MxL601_ConfigHlfrfSettings(PMXL_HLFRF_CFG_T hlfrfCfgPtr)
{
  UINT8 status = MXL_TRUE;
  UINT8 regData0 = 0;
  UINT8 regData1 = 0;
...
static MXL_STATUS MxL601_ConfigTunerAfc(PMXL_TUNER_AFC_CFG_T afcCfgPtr)
{
...
static MXL_STATUS MxL601_GetTunerSignalType(PMXL_SIGNAL_TYPE_T tuneSigTypePtr)
{
  MXL_STATUS status = MXL_TRUE;
  UINT8 regData = 0;
...
static MXL_STATUS MxL601_GetTunerLockStatus(PMXL_TUNER_LOCK_STATUS_T tunerLockStatusPtr)
{
  MXL_STATUS status = MXL_TRUE;
  UINT8 regData = 0;
...
static MXL_STATUS MxL601_GetTunerAgcLockStatus(PMXL_TUNER_AGC_LOCK_T agcLockStatusPtr)
{
...
static MXL_STATUS MxL601_GetTunerRxPower(PMXL_TUNER_RX_PWR_T rxPwrPtr)
{
  UINT8 status = MXL_TRUE;
  UINT8 regData = 0;
  UINT16 tmpData = 0;
...
static MXL_STATUS MxL601_GetTunerAfcCorrection(PMXL_TUNER_AFC_CORRECTION_T afcCorrectionPtr)
{
  UINT32 tmpData = 0;
  UINT8 status = MXL_TRUE;
  UINT8 regData = 0;
...
static MXL_STATUS MxL601_SetTeletextSpurMitigation(PMXL_TELETEXT_SPUR_CFG_T teletextSpurCfgPtr)
{
  ...
  MxL_DLL_DEBUG0(("MXL_TUNER_TELETEXT_SPUR_CFG \n"));
