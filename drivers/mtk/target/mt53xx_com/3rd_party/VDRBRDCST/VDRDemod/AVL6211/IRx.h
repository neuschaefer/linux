...
extern "C" {
	...
	/// Return ::AVL_DVBSx_EC_I2CFail if there is an I2C communication problem.
	/// Return ::AVL_DVBSx_EC_Running if the command could not be sent to the Avaiilink device because the device is still processing a previous command.
	...
	/// Disables MPEG manual clock mode.  If MPEG manual clock is disabled, MPEG ouput clock in automatic mode. Then the Availink device outputs an MPEG clock signal only when it is locked to a channel.
	/// 
	/// @param pAVLChip A pointer to the ::AVL_DVBSx_Chip object for which MPEG persistent clock mode is being disabled.
	/// 
	/// @return ::AVL_DVBSx_ErrorCode, 
	...
	/// @param piPhase A pointer to a variable in which to store the phase imbalance in units of degress.  The value is scaled by 100.  For example, a value of 200 means a phase imbalance of 2 degrees.
	/// 
	/// @return ::AVL_DVBSx_ErrorCode, 
