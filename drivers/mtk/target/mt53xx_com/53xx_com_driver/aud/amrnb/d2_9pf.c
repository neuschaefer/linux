...
//   Word16 pos[NB_PULSE];
   ...
   i = (i + sature(i << 2));       /* pos0 =i*5+startPos[j*8+subNr*2] */
   idx = (sature(j << 3) + sature(subNr << 1));
   if (idx >= 2*4*2 || idx < 0)
      idx = 0;
   k = startPos[idx];
   pos[0] = (i + k);
   ...
   i = (i + sature(i << 2));       /* pos1 =i*5+startPos[j*8+subNr*2+1] */
   idx = (sature(j << 3) + sature(subNr << 1)) + 1;
   if (idx >= 2*4*2 || idx < 0)
      idx = 0;
   k = startPos[idx];
   pos[1] = (i + k);
