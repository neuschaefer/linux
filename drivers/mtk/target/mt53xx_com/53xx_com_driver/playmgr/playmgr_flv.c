...
static BOOL _PLAYMGR_SetFlvInfo(PLAYMGR_INFO_T* prMediaInfo)
{
    ...
    MIDXBULD_ELMT_TBL_DESCRIB_T* prVDescrib = &prFlvInfo->rVDescrib;
    MIDXBULD_ELMT_TBL_DESCRIB_T* prADescrib = &prFlvInfo->rADescrib;
    ...
    if(prMediaInfo->u2VidStmNum >0 && prFlvInfo->u2VidIdx<2)
    {
        ...
        switch (prFlvInfo->rStmAttribute[prFlvInfo->u2VidIdx].u.t_flv_video_stm_attr.e_enc)
        {
            case MINFO_INFO_VID_ENC_SORENSON: 
                prMediaStatus->aeVidFormat[0] = PLAYMGR_VDEC_SORENSON;
                break;
...
BOOL _PLAYMGR_FlvStart(VOID* pvMediaInfo)
{
    ...
    {
        SWDMX_SetInfo(eSWDMX_SET_SYSTEMINFO,prFlvInfo->rVDescrib.u_info.t_es.ui4_rate,
          prFlvInfo->rADescrib.u_info.t_es.ui4_sample_sz,prFlvInfo->rStmAttribute[prFlvInfo->u2AudIdx].u.t_flv_audio_stm_attr.ui4_type);
    }
    return TRUE;
}
...
BOOL _PLAYMGR_FlvUpdateRange(VOID* pvMediaInfo)
{
	...
	SWDMX_AddRange(&prMediaInfo->rRangeInfo[0],(UINT32)prMediaInfo->rRangeInfo[0].ui4_range_id);
    return TRUE;
}
