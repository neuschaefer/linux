        ...
        i++)
       {
            switch (prAviInfo->rStmAttribute[i].u.t_avi_audio_stm_attr.e_enc)
            {
                ...
                case MINFO_INFO_AUD_ENC_AAC:
                    ...
                    break;
                default:
                    LOG(3, "Unknown Audio %d\n", (prAviInfo->rStmAttribute[i].u.t_mkv_audio_stm_attr.e_enc));
                    prMediaStatus->aeAudFormat[i - prMediaInfo->u2VidStmNum] = 
    ...
    case MINFO_NFY_EVENT_PARSING_DONE:
	 ...
	 if ((zRecLength != sizeof(MINFO_INFO_T)) || i4Ret)
        ...
        if (prAviInfo->rDivXDrmInfo.u4DrmType == DRM_TYPE_DIVX_DRM)
        {
            ...
            _eDRMType = PLAYMGR_DRMTYPE_PURCHASE;
            _u4DRMRentalLimit = 0;
            _u4DRMRentalCount = 0;
            ...
            {
                _eDRMType = PLAYMGR_DRMTYPE_AUTH_ERR;
                _u4DRMRentalLimit = prAviInfo->rDivXDrmInfo.prDivxDrmBasicInfo->ui1_use_limit;
                _u4DRMRentalCount = prAviInfo->rDivXDrmInfo.prDivxDrmBasicInfo->ui1_use_count;
                LOG(0, "DIVX_DRM_AUTH_ERROR\n");
            }
            else if (prAviInfo->rDivXDrmInfo.prDivxDrmBasicInfo->ui1_flag & DIVX_DRM_RENTAL_EXPIRED)
            {
                _eDRMType = PLAYMGR_DRMTYPE_RENTAL_EXPIRED;
                _u4DRMRentalLimit = prAviInfo->rDivXDrmInfo.prDivxDrmBasicInfo->ui1_use_limit;
                _u4DRMRentalCount = prAviInfo->rDivXDrmInfo.prDivxDrmBasicInfo->ui1_use_count;
            ...
            {
                _eDRMType = PLAYMGR_DRMTYPE_RENTAL;
                _u4DRMRentalLimit = prAviInfo->rDivXDrmInfo.prDivxDrmBasicInfo->ui1_use_limit;
                _u4DRMRentalCount = prAviInfo->rDivXDrmInfo.prDivxDrmBasicInfo->ui1_use_count;
        ...
        {
            _eDRMType = PLAYMGR_DRMTYPE_NONE;
        }
...
#if 1  // moved from _PLAYMGR_Minfo_Notify above
    if (prAviInfo->rDivXDrmInfo.u4DrmType == DRM_TYPE_DIVX_DRM)
    {
        DIVX_DRM_UNION_INFO_T rDivxDrmInfo;
...
BOOL _PLAYMGR_AviUpdateRange(VOID* pvMediaInfo)
{
    ...
    if (&prAviInfo->rAudIDXTBL[0])
    {
        prAudElmtStrm = &prAviInfo->rAudIDXTBL[0];
    }
    else
    {
        return FALSE;
...
BOOL _PLAYMGR_AviGetDivxDRMStatus(PLAYMGR_DRMTYPE *peDRMType, UINT32 *pu4DRMRentalLimit, UINT32 *pu4DRMRentalCount)
{
    *peDRMType = _eDRMType;
    *pu4DRMRentalLimit = _u4DRMRentalLimit;
    *pu4DRMRentalCount = _u4DRMRentalCount;
