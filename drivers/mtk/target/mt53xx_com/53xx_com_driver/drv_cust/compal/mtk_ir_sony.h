...
static void _SetSonyWakeupKey(void)
{
    ...
    u4KeyNum = 0;
    vIO32WriteFldAlign(PDWNC_IREXP_EN, 0x1, FLD_BCEPEN);//PDWNC_WRITE32(IREXP_EN_REG, IREXP_BIT_EN);
    vIO32WriteFldAlign(PDWNC_EXP_BCNT, 0x0c,  FLD_EXP_BITCNT);//PDWNC_WRITE32(IREXP_BITNUM_REG, 0x0c);
    vIO32Write4B(PDWNC_ENEXP_IRL, 0);//PDWNC_WRITE32(IREXP_L_BITMASK_REG, 0);    
    if(DRVCUST_InitGet(eIrrxFlagAllWakeup))
    {
        vIO32Write4B(PDWNC_ENEXP_IRM, 0x00000f80);//PDWNC_WRITE32(IREXP_M_BITMASK_REG, 0x00000f80);

        for (i=0; i < MAX_KEY_ARRAY ; i++)
        {
/*        
            u4Val = PDWNC_READ32(IREXP_EN_REG);
            u4Val |= IREXP_EN(i);
            PDWNC_WRITE32(IREXP_EN_REG, u4Val);    
*/
            vIO32WriteFldAlign(PDWNC_IREXP_EN, IREXP_EN(i), FLD_IREXPEN);
            
            u4Val = _arKeyArray[i].u4GroupId;     
            vIO32Write4B(PDWNC_EXP_IRM0 + (8*i), u4Val);//PDWNC_WRITE32(IREXP_M0_REG + (8*i), u4Val);
            vIO32Write4B(PDWNC_EXP_IRL0 + (8*i), 0);//PDWNC_WRITE32(IREXP_L0_REG + (8*i), 0);    
        }
    }
    else
    {
        vIO32Write4B(PDWNC_ENEXP_IRM, 0x00000fff);//PDWNC_WRITE32(IREXP_M_BITMASK_REG, 0x00000fff);
        for (i=0; i<MAX_KEY_ARRAY; i++)
        {
...
/*                
                    ...
                    PDWNC_WRITE32(IREXP_EN_REG, u4Val);
*/
                    vIO32WriteFldAlign(PDWNC_IREXP_EN, IREXP_EN(u4KeyNum), FLD_IREXPEN);

                    u4Val = (_arKeyArray[i].u4GroupId << 7) |
                            (_arKeyArray[i].prKeyMap[j].u4Key);
//                    PDWNC_WRITE32(IREXP_M0_REG + (8*u4KeyNum), u4Val);
//                    PDWNC_WRITE32(IREXP_L0_REG + (8*u4KeyNum), 0);
...
#define CLOCK_SLOT_TIME		(10000 /_u4CpuClkMhz)
...
void _SonyIrBootupCheck(void)
{
    ...
    vIO32WriteFldAlign(PDWNC_ARM_INTEN, (1U << 7), FLD_EXTIO2_INTEN); //PDWNC_WRITE32(REG_RW_PDWNC_INTEN, PDWNC_READ32(REG_RW_PDWNC_INTEN) | (1U << 1));
    vIO32WriteFldAlign(PDWNC_EXTIO2INTCFG, (1U << 7), FLD_EXINT2_2ED);//PDWNC_WRITE32(REG_RW_EXTINTCFG, PDWNC_READ32(REG_RW_EXTINTCFG) | ((1U << 1) << 16));
    //PDWNC_WRITE32(REG_RW_PINMUX, PDWNC_READ32(REG_RW_PINMUX) | (1U << 16));    
}

void _SonyIrLISR(INT32 i4Vector)
{
    if ((VECTOR_PDWNC == i4Vector) && (u4IO32Read4B(PDWNC_INTSTA) & (1U << 31))/*(PDWNC_READ32(REG_RO_PDWNC_INTST) & PINT_GPIO1)*/)
    {
        //_SonyIrParsing(0, PDWNC_READ32(REG_RO_IOPADIN) & (1 << 1));
        _SonyIrParsing(0, u4IO32Read4B(PDWNC_GPIOIN) & (1 << 15));
        vIO32WriteFldAlign(PDWNC_INTCLR, (1U << 7), FLD_EXTIO2_INTCLR);//PDWNC_WRITE32(REG_WO_PDWNC_INTCLR, _PINT(1));
    }
    else if ((VECTOR_PDWNC == i4Vector) && (/*PDWNC_READ32(REG_RO_PDWNC_INTST)*/u4IO32Read4B(PDWNC_INTSTA)  & PINT_IRRX))
    {
       // _SonyIrParsing(1, PDWNC_READ32(REG_RO_IOPADIN) & (1 << 1));
       _SonyIrParsing(0, u4IO32Read4B(PDWNC_GPIOIN) & (1 << 15));
