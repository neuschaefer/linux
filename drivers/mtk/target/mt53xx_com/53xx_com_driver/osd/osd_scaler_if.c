                   ...
                   UINT32 u4SrcHeight, UINT32 u4DstWidth, UINT32 u4DstHeight)
{
    ...
    else
    {
        ...
        if (u4SrcHeight <= u4DstHeight)
        {
            ...
            if(gfgOSD_Flip)
	     {
                u4RgnID=OSD_PLA_GetFirstRegion(u4Scaler+1);//scaler 2(num 0) -->plane 2(num 1),scaler 3(num 1) -->plane 3(num 2)
                //_OSD_RGN_GetOutputPosY(u4RgnID,&u4Tmp_Y);
                
                if(u4RgnID != (-1))
                {
                   _OSD_BASE_GetScrnVSize(&scn_h);
                   OSD_RGN_GetOriginalY(u4RgnID,&u4Old_Y);
                   _OSD_RGN_GetInputHeight(u4RgnID,&u4Tmp_Y);
                   u4Old_Y=u4Old_Y+u4Tmp_Y;
                   u4New_Y =scn_h- ((u4Old_Y*u4DstHeight)/u4SrcHeight);
	                 _OSD_RGN_SetOutputPosY(u4RgnID, u4New_Y);	
            ...
            if(gfgOSD_Mirror)
	     {
                ...
                if(u4RgnID != (-1))
                {
                   ...
                   OSD_RGN_GetOriginalX(u4RgnID,&u4Old_X);
                   _OSD_RGN_GetInputWidth(u4RgnID,&u4Tmp_X);
                   u4Old_X=u4Old_X+u4Tmp_X;
                   u4New_X =scn_w- ((u4Old_X*u4DstWidth)/u4SrcWidth);
	                 _OSD_RGN_SetOutputPosX(u4RgnID, u4New_X);	
