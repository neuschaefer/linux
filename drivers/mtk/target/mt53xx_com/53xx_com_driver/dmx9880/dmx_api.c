    ...
    PID_STRUCT_T* prPidStruct)
{
    ...
    else
    {
        ...
        if (prPid->ePidType == DMX_PID_TYPE_ES_VIDEOCLIP)
        {
            ...
            //set byte2 to 0xff for default picture search by TS_IDX //for mt5398 fpga emulation
            u4W0 |= 0x00FF0000;
        }
 ...
 */
//-----------------------------------------------------------------------------
static BOOL _DmxSetDescMode(UINT8 u1Idx, DMX_DESC_MODE_T eMode)
{
...
BOOL _DMX_SetPid(UINT8 u1Pidx, UINT32 u4Flags, const DMX_PID_T* prPid, BOOL fgKeepScrmble)
{
    ...
    if ((u4Flags & DMX_PID_FLAG_DESC_MODE) != 0)
    {
        ...
        if(!_DmxSetDescMode(rPidStruct.u1KeyIndex, prPid->eDescMode))
        {
