...
#define MJC_DUMP_MAX_SIZE       (1920*32)  
...
#define MJC_SIZE_TO_NEXT_BLK_2D     (MJC_PACK_SIZE_10BIT_NORMAL*MJC_FRAME_NUM_NORMAL)
...
#define SCL_DUMP_MAX_SIZE       (1920*32)
...
void vDrvMJCDumpNormal(DUMP_3D_INFO_T *pInfo)
{
    ...
    for (i = 0; i < pInfo->u4DesHeight; i++)
    {
        ...
        u4BlkStartR = u4BlkStartL + MJC_PACK_SIZE_10BIT_NORMAL*MJC_FRAME_NUM_NORMAL;
...
{       
    ...
    else if (pDumpInfo->u4SrcHeight >= (384+160)) 
    {        
        pDumpInfo->u4DesHeight = SCL_DES_HEIGHT;                            
        pDumpInfo->u4Subsample = 24;    //384
    }
    else if (pDumpInfo->u4SrcHeight >= (256+128)) 
    {        
        pDumpInfo->u4DesHeight = SCL_DES_HEIGHT;                            
    ...
    pDumpInfo->u4SrcStartY = 
        ((pDumpInfo->u4SrcHeight - pDumpInfo->u4DesHeight*pDumpInfo->u4Subsample)/2);
...
void vDrvSCLDumpNormal(DUMP_3D_INFO_T *pInfo)
{
    ...
    vRegWriteFldAlign(SCPIP_DRAM_M_00, 0, DRAM_M_00_DA_SNAKE_MAPPING);
...
void vDrvLSVModeChange(INT32 iWidth, INT32 iHeight, INT32 iStep_h)
{
    ...
    {  
        ...
        iSclRatio = 0x100;
        #else                 
        if (e3DMode == E_TDTV_DECODER_INPUT_SBS_P) 
        {
...
UINT8 IntToUint8(INT32 in)
{    
    if (in > 0)
        return (in > 128) ? 128 : in;
    else
        return (in < -128) ? 255 : (128-in);
}
...
void AdaptiveMW(INT32 *avg_idx_H, UINT8 SR_H, UINT8 SR_V)
{
	...
	*avg_idx_H=sum_total_H*2/sum_cnt_H;
	...
	x_memset((void *)dv_meter_LH, 0, sizeof(INT32)*256);
}
                ...
                UINT8 SR_H, UINT8 SR_V, UINT8 fcount,UINT8 foreback)
{
	...
	// Generate random MV
	INT32 rand_cnt = fcount;//rand();
	//INT32 RANDOM_NO= 8;
	//INT32	LutT[9] ={1,2,4,7,11,16,22,29,0};
	
	INT32 RANDOM_NO= 8;
	INT32 LutT[9] ={1,-1,5,-5,11,-11,19,-19,0};
	INT32 rand_factor	= ((blk_x+blk_y+rand_cnt)%RANDOM_NO);
	INT32 rand_mv;
	...
	if(foreback==0)
	{
		for(j=0;j<5;j++)
		{
			...
			{	
				if (blk_y==0){//1st line
					if (j<=1)						p_mv_frm_buf_bi = p_mv_frm_buf_bi_tp;
					else if ((j==2)&&(i<=1))		p_mv_frm_buf_bi = p_mv_frm_buf_bi_sp;
					else							p_mv_frm_buf_bi = p_mv_frm_buf_bi_tp;
				}
				else if ((blk_y%2)==0)//forward 
				{	
					if ((j<=1) || ((j==2)&&(i<=1)))	p_mv_frm_buf_bi = p_mv_frm_buf_bi_sp;
					else							p_mv_frm_buf_bi = p_mv_frm_buf_bi_tp;
				}
				else				//backward
				{					
					if ((j<=1) || ((j==2)&&(i>=3)))	p_mv_frm_buf_bi = p_mv_frm_buf_bi_sp;
					else							p_mv_frm_buf_bi = p_mv_frm_buf_bi_tp;
				}
			
				idx = ClipABC((blk_x + i - 2),h_bound-1,0);
				idy = ClipABC((blk_y + j - 2),v_bound-1,0);
				pred_de_dv[i+j*5] = p_mv_frm_buf_bi[idx + idy*h_bound];
			}	
		}
	}
	else
	{
		for(j=0;j<5;j++)
		{
			for(i=0;i<5;i++)
			{	
				if (blk_y==m_depth_height-1)//1st line
				{
					if (j>=3)						p_mv_frm_buf_bi = p_mv_frm_buf_bi_tp;
					else if ((j==2)&&(i<=1))		p_mv_frm_buf_bi = p_mv_frm_buf_bi_sp;
					else							p_mv_frm_buf_bi = p_mv_frm_buf_bi_tp;
				}
				else if ((blk_y%2)==0)//backward 
				{	
					if ((j>=3) || ((j==2)&&(i>=3)))	p_mv_frm_buf_bi = p_mv_frm_buf_bi_sp;
					else							p_mv_frm_buf_bi = p_mv_frm_buf_bi_tp;
				}
				else				//foreward
				{					
					if ((j>=3) || ((j==2)&&(i<=1)))	p_mv_frm_buf_bi = p_mv_frm_buf_bi_sp;
					else							p_mv_frm_buf_bi = p_mv_frm_buf_bi_tp;
				}
	...
	/*if(foreback==0)
	{*/
			 if( blk_x==0			   &&blk_y==0)					rand_mv = pred_de_dv[12];//tmpr
		else if((blk_x==0              &&blk_y!=0&&(blk_y%2)==0)||	
				(blk_x==m_depth_width-1&&blk_y!=0&&(blk_y%2)==1)  )	rand_mv = pred_de_dv[7];//up
		else if((blk_y%2)==0)										rand_mv = pred_de_dv[11];//left
		else														rand_mv = pred_de_dv[13];//right
	//}
	//else
	//{
	//		 if( blk_x==0              &&blk_y==(m_depth_height-1))					rand_mv = pred_de_dv[12];//tmpr
	//	else if((blk_x==m_depth_width-1&&blk_y!=(m_depth_height-1)&&(blk_y%2)==0)||
	//			(blk_x==0              &&blk_y!=(m_depth_height-1)&&(blk_y%2)==1) )	rand_mv = pred_de_dv[17];//down
	//	else if((blk_y%2)==0)														rand_mv = pred_de_dv[13];//right
	//	else																		rand_mv = pred_de_dv[11];//left
	//}
	pred_de_dv[25] = ClipABC( rand_mv + LutT[rand_factor], SR_H, -SR_H);
...
INT32 ORD_Quad(PEL *pCurFull, PEL *pRefFull, INT32 cur_x, INT32 cur_y, INT32 ref_x, INT32 ref_y, UINT8 step_v, UINT8 step_h)
{
	...
	INT32 de_obme=1	;
	INT32 T=36;
    INT32 ord_sel = IO32ReadFldAlign(TTD_FW_REG_02, TTD_ORD_QUAD_SEL); 
	INT32 ord_cost = 0;
	INT32 cur_acc=0,ref_acc=0;
	INT32 i;
	UINT8 ord_cur,ord_ref;
	INT32 ord_pnlty = 0;
    UINT8 obme_num= IO32ReadFldAlign(TTD_FW_REG_05, TTD_OBME_NUM); 
        
	cur_acc+=pCurFull[(cur_y+0*step_v)*m_lsv_width+(cur_x+1*step_h)];
	cur_acc+=pCurFull[(cur_y+0*step_v)*m_lsv_width+(cur_x+2*step_h)];
	cur_acc+=pCurFull[(cur_y+1*step_v)*m_lsv_width+(cur_x+1*step_h)];
	cur_acc+=pCurFull[(cur_y+1*step_v)*m_lsv_width+(cur_x+2*step_h)];
	ref_acc+=pRefFull[(ref_y+0*step_v)*m_lsv_width+(ref_x+1*step_h)];
	ref_acc+=pRefFull[(ref_y+0*step_v)*m_lsv_width+(ref_x+2*step_h)];
	ref_acc+=pRefFull[(ref_y+1*step_v)*m_lsv_width+(ref_x+1*step_h)];
	ref_acc+=pRefFull[(ref_y+1*step_v)*m_lsv_width+(ref_x+2*step_h)];
	
	cur_avg=(cur_acc+(1<<1))>>2;	
	ref_avg=(ref_acc+(1<<1))>>2;

	i_str=(de_obme==1)?-obme_num:0;
	i_end=(de_obme==1)?m_mbSize+obme_num:m_mbSize;
	
	for(i=i_str; i<i_end; i++)
    {
		ord_cur=(cur_avg>pCurFull[(cur_y+0*step_v)*m_lsv_width+(cur_x+i*step_h)])?1:0;
		ord_cur=(cur_avg>pCurFull[(cur_y+1*step_v)*m_lsv_width+(cur_x+i*step_h)])?1:0;
		ord_ref=(ref_avg>pRefFull[(ref_y+0*step_v)*m_lsv_width+(ref_x+i*step_h)])?1:0;
		ord_ref=(ref_avg>pRefFull[(ref_y+1*step_v)*m_lsv_width+(ref_x+i*step_h)])?1:0;

		if(ord_cur!=ord_ref)	ord_cost++;
	}

	if     (ord_sel == 0)		ord_pnlty = 0;
	else if(ord_sel == 1)		ord_pnlty = MIN(ord_cost,T)*1;
	else if(ord_sel == 2)		ord_pnlty = MIN(ord_cost,T)*2;
	else if(ord_sel == 3)		ord_pnlty = MIN(ord_cost,T)*3;
	else if(ord_sel == 4)		ord_pnlty = MIN(ord_cost,T)*4;
	else if(ord_sel == 5)		ord_pnlty = MIN(ord_cost,T)*6;
	else if(ord_sel == 6)		ord_pnlty = MIN(ord_cost,T)*8;
...
INT32  smth_pnlty(INT32 dv_curr, INT32 *dv_in, UINT8 is_even_line)
{
	...
	INT32 dvecand[9];
	INT32 sad_pnlty;
    INT32 me_mvpnlty_sel = IO32ReadFldAlign(TTD_FW_REG_02, TTD_SMTH_SEL);
	INT32 i, T, mv_pred_dif, dv_med;
	INT32 candidate[5]= { 7, 11, 13, 17, 10 };
    
	candidate[4]=(is_even_line)?10:14;
	
	for(i=0; i<5; i++)
    {   
        dvecand[i] = dv_in[candidate[i]];   
    }
	...
	dv_med = dvecand[2];
	T=8;
	mv_pred_dif=MIN(ABS(dv_curr - dv_med),T);//MIN(ABS(dv_curr - dv_med),24*	);
...
{
	INT32 tmpr_pnlty;
    INT32 tmpr_dvpnlty_sel = IO32ReadFldAlign(TTD_FW_REG_02, TTD_TMPR_SEL);
	INT32 tmpr_cost;
	...
	if     (tmpr_dvpnlty_sel == 0)		tmpr_pnlty = 0;
	else if(tmpr_dvpnlty_sel == 1)		tmpr_pnlty = tmpr_cost*1;
	else if(tmpr_dvpnlty_sel == 2)		tmpr_pnlty = tmpr_cost*2;
	else if(tmpr_dvpnlty_sel == 3)		tmpr_pnlty = tmpr_cost*3;
	else if(tmpr_dvpnlty_sel == 4)		tmpr_pnlty = tmpr_cost*4;
	else if(tmpr_dvpnlty_sel == 5)		tmpr_pnlty = tmpr_cost*5;
	else if(tmpr_dvpnlty_sel == 6)		tmpr_pnlty = tmpr_cost*6;
	else if(tmpr_dvpnlty_sel == 7)		tmpr_pnlty = tmpr_cost*7;
...
{
    INT32 me_mvpnlty_sel = IO32ReadFldAlign(TTD_FW_REG_02, TTD_ZERO_SEL);
	INT32 T=1;
	INT32 mv_pred_dif=MIN(ABS(dv_cur),T);
	INT32 sad_pnlty;

	sad_pnlty = 0;
	
	if     (me_mvpnlty_sel == 0)		sad_pnlty = 0;
	else if(me_mvpnlty_sel == 1)		sad_pnlty = mv_pred_dif*1;
	else if(me_mvpnlty_sel == 2)		sad_pnlty = mv_pred_dif*2;
	else if(me_mvpnlty_sel == 3)		sad_pnlty = mv_pred_dif*3;
	else if(me_mvpnlty_sel == 4)		sad_pnlty = mv_pred_dif*4;
	else if(me_mvpnlty_sel == 5)		sad_pnlty = mv_pred_dif*5;
	else if(me_mvpnlty_sel == 6)		sad_pnlty = mv_pred_dif*6;
	else if(me_mvpnlty_sel == 7)		sad_pnlty = mv_pred_dif*7;
...
{
	INT32 rand_pnlty;
    INT32 rand_dvpnlty_sel = IO32ReadFldAlign(TTD_FW_REG_02, TTD_RAND_SEL);
	INT32 rand_cost;
	...
	if     (rand_dvpnlty_sel == 0)		rand_pnlty = 0;
	else if(rand_dvpnlty_sel == 1)		rand_pnlty = rand_cost*1;
	else if(rand_dvpnlty_sel == 2)		rand_pnlty = rand_cost*2;
	else if(rand_dvpnlty_sel == 3)		rand_pnlty = rand_cost*3;
	else if(rand_dvpnlty_sel == 4)		rand_pnlty = rand_cost*4;
	else if(rand_dvpnlty_sel == 5)		rand_pnlty = rand_cost*5;
	else if(rand_dvpnlty_sel == 6)		rand_pnlty = rand_cost*6;
	else if(rand_dvpnlty_sel == 7)		rand_pnlty = rand_cost*7;
...
void DepthBlend(INT32 *pre_depth_buf_R, INT32 *filter_buf_R)
{
	...
	for(y=0;y<m_depth_height;y++)
	{
        ...
        {
            //(filter_buf_R[x+y*width]+pre_depth_buf_R[x+y*width])/2;
		    pre_depth_buf_R[x+y*m_depth_width]=filter_buf_R[x+y*m_depth_width];	
	    }
	}
}
...
{
	INT32 i,j;
	INT32 var_cor_th = IO32ReadFldAlign(TTD_FW_REG_02, TTD_VAR_COR_TH); 
...
{
	INT32 var_en_th = IO32ReadFldAlign(TTD_FW_REG_06, TTD_VAR_EN_TH); 
	INT32 var_pnlty = 0;
    INT32 var_pnlty_sel = IO32ReadFldAlign(TTD_FW_REG_02, TTD_VAR_SEL);
	INT32 var_cost;

	if(var_cur<var_en_th)	var_cost=ABS(dv_cur);
	else					var_cost=0;

	if     (var_pnlty_sel == 0)		var_pnlty = 0;
	else if(var_pnlty_sel == 1)		var_pnlty = var_cost*1;
	else if(var_pnlty_sel == 2)		var_pnlty = var_cost*2;
	else if(var_pnlty_sel == 3)		var_pnlty = var_cost*3;
	else if(var_pnlty_sel == 4)		var_pnlty = var_cost*4;
	else if(var_pnlty_sel == 5)		var_pnlty = var_cost*5;
	else if(var_pnlty_sel == 6)		var_pnlty = var_cost*6;
	else if(var_pnlty_sel == 7)		var_pnlty = var_cost*7;
						 ...
						 UINT8 SR_H, UINT8 SR_V, UINT8 fcount, UINT8 step_v, UINT8 step_h, INT32 str_h)
{
	...
	UINT8 is_even_line;
	INT32 i;
	INT32 seed_1[9]={25,11,13,7,17,6,18,8,16};
	INT32 seed_2[9]={25,13,11,7,17,8,16,6,18};
	...
	for(cur_y=0; cur_y<m_depth_height*V_cand_step; cur_y+=V_cand_step)
	{
		is_even_line=(((cur_y/V_cand_step)%2)==0)?SV_TRUE:SV_FALSE;
		if(is_even_line)
		{
			for(cur_x=str_h; cur_x<(str_h+m_depth_width*H_cand_step); cur_x+=H_cand_step)
			{
				minimum = 65535;	dx=-256;
				prepare_dv((cur_x-str_h)/H_cand_step, cur_y/V_cand_step,cur_depth_buf,pre_depth_buf_cur,SR_H,SR_V, fcount,0);
				i=0;
				while(i<cand_num)
				{
					n=pred_de_dv[seed_1[i]];

					ref_y = cur_y    ;		// Vertical   co-ordinate for reference block in previous frame
					ref_x = cur_x + n;		// Horizontal co-ordinate for reference block in previous frame
					...
					ord_cost  = ORD_Quad(pCurFull, pRefFull, cur_x, cur_y, ref_x, ref_y, step_v, step_h);
				...
				//write histogram
				cur_depth_buf[(cur_x-str_h)/H_cand_step+cur_y/V_cand_step*m_depth_width] = dx;
				if(dx!=-256)
				{
					dv_meter_LH[dx+SR_H]++;
				}
		...
		{
			for(cur_x=(str_h+(m_depth_width-1)*H_cand_step); cur_x>=str_h; cur_x-=H_cand_step)
			{
				minimum = 65535;	dx=-256;
				prepare_dv((cur_x-str_h)/H_cand_step, cur_y/V_cand_step,cur_depth_buf,pre_depth_buf_cur,SR_H,SR_V, fcount,0);
						 ...
						 UINT8 SR_H, UINT8 SR_V, UINT8 fcount, UINT8 step_v, UINT8 step_h, INT32 str_h)
{
	...
	UINT8 is_even_line;
	INT32 i;
	INT32 seed_3[9]={25,11,13,17,7,16,8,18,6};
	INT32 seed_4[9]={25,13,11,17,7,18,6,16,8};
	...
	// Full search disparity estimation										
	for(cur_y=(m_depth_height-1)*V_cand_step; cur_y>=0; cur_y-=V_cand_step)
	{
		is_even_line=(((cur_y/V_cand_step)%2)==1)?SV_TRUE:SV_FALSE;
		if(is_even_line)
		{
			for(cur_x=str_h; cur_x<(str_h+m_depth_width*H_cand_step); cur_x+=H_cand_step)
			{
				...
				prepare_dv((cur_x-str_h)/H_cand_step, cur_y/V_cand_step,cur_depth_buf,pre_depth_buf_cur,SR_H,SR_V, fcount,1);
...
{    
    ...
    if (u1DrvTTDGet3DDepthCtrl() == SV_OFF)
    {        
        lsv_auto_gain = 0;
        lsv_local_gain = 0;
    }
...
{      
	...
	INT32 lsv_global_dspt;
	UINT8 lsv_global_depth;
	UINT8 lsv_final_global_depth;
	UINT8 lsv_final_cid_gain;
	...
	dif_convg=shift_avg_H-cur_convg;
    
	if (cur_convg!=shift_avg_H)
	{

		if (stab_flag==0)	
			cur_convg=cur_convg+dif_convg/8*2;
		else 
		{
    ...
    if (bCount++ >= 7)
    {
 	...
 	lsv_global_dspt = (lsv_focal_plen-128)+(-cur_convg*(((INT32)lsv_auto_gain)-16)/16);
 	lsv_global_depth= dsp2dep(lsv_global_dspt,d2d_convg, d2d_gain, d2d_ratio);

	gen_cid_setting(lsv_global_dspt, lsv_global_depth, lsv_local_gain, 
					 &lsv_final_cid_gain, &lsv_final_global_depth);

	vDrvTTDSetLocalWeight(lsv_final_cid_gain);
	vDrvTTDSetDepthBias(lsv_global_depth);
    //vIO32WriteFldAlign(TTDDG_01, lsv_final_cid_gain, TTDDG_01_DG_LOCAL_WEIGHT);
	//vIO32WriteFldAlign(TTDDG_00, lsv_global_depth , TTDDG_00_DG_DEPTH_BIAS);
