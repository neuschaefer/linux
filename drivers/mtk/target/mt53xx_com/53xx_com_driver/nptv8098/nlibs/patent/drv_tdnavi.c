...
UINT8 g_u1TdNaviDetInFrmNum = 15;
...
CODE TD_NAVI_REGTBL_T TD_NAVI_INIT[] =
{
    ...
    {TDNVAI_01, 0x4009DA0A, 0xFFFFFFFF},
    {TDNVAI_02, 0x03000043, 0x070000FF},
    {TDNVAI_03, 0x21004000, 0x7FFFFFFF},
    {TDNVAI_04, 0x08050000, 0x3FFFFFFF},
...
static void vDrvTdNaviMainloopInt(void)
{
	...
	g_u1TdNaviDetNew = vDrvTdNaviGetNewDetType();
	
    
    if(TD_NAVI_DETECT_CB==eDrvSwNaviDetectedFmtQuery())
    {    
	...
	else if (TD_NAVI_DETECT_SBS == g_u1TdNaviDetCurr)
	{
		if (g_u1TdNaviAdaptOnOff)
		{
	...
	else // (TD_NAVI_DETECT_TAB == g_u1TdNaviDetCurr)
	{
		if (g_u1TdNaviAdaptOnOff)
		{
			vDrvTdNaviSetSimilarityRatio(g_u1Out3DSimilarityRatio);
...
{ 
	return (TD_NAVI_DETECT_CB==eDrvSwNaviDetectedFmtQuery()) ? TD_NAVI_DETECT_CB :
        g_u1TdNaviDetCurr;
}
