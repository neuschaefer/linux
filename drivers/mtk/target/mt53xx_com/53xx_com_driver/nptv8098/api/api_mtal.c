...
{
    if (u4Size != (CONTRAST_SKIN_CURVE_NUM*sizeof(UINT16)))
        return PQ_PARAMETER_ERROR;

    if (ucVdpId != VDP_1)
        return PQ_NOT_SUPPORTED;

    vDrvADLSetSecondLumaCurveHw(prData);
...
static INT32 _PE_SetVdoColorCurve(UCHAR ucVdpId, const VDO_PQ_COLOR_CURVE_T* prData, UINT32 u4Size)
{
    ...
    x_memcpy(&BASE_SCE_TBL[u4Ofst], &prData->u1SCETbl, u2ColorCruveSize[prData->u1Index]*sizeof(UINT8));
    x_memcpy(&DEFAULT_SCE_TBL[u4Ofst], &prData->u1SCETbl, u2ColorCruveSize[prData->u1Index]*sizeof(UINT8));
...
static INT32 _PE_GetVdoPqMotionLevel(UCHAR ucVdpId, UINT16* prData, UINT32 u4Size)
{
    if (u4Size != sizeof(UINT16))
        return PQ_PARAMETER_ERROR;
    if (ucVdpId != VDP_1)
        return PQ_NOT_SUPPORTED;
 
    #if defined(CC_MT5396)
    *prData = u2DrvDIGetMotionLevel();    
    return PQ_OK;
    #else
    return PQ_NOT_SUPPORTED;
    #endif    
...
INT32 PE_GetPqInfo(UCHAR ucVdpId, UINT32 u4PqType, void* pkvPqGetInfo, UINT32 u4Size)
{
    ...
    switch ((VDO_PQ_TYPE_T) u4PqType)
    {
        ...
        case VDO_PQ_MOTION_LEVEL_GET:
            i4Ret = _PE_GetVdoPqMotionLevel(ucVdpId, (UINT16*) pkvPqGetInfo, u4Size);
            break;            
        case VDO_PQ_DIR_GET:
