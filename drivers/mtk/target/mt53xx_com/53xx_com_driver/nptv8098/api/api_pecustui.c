                       ...
                       VOID * pv_get_info, SIZE_T * pz_size)
{
    ...
    case DRV_CUSTOM_DISP_TYPE_PQ_VER:
        ...
        {
            x_strncpy((CHAR *)pv_get_info, (CHAR *)bApiFlashPqGetVersion(FLASHPQ_XML_VER), (*pz_size));
            return 0;
        }
        else
        {
    ...
    case DRV_CUSTOM_DISP_TYPE_GAMMA_VER:
        if (bApiFlashPqVerifyHeader() == SV_TRUE)
        {
            x_strncpy((CHAR *)pv_get_info, (CHAR *)bApiFlashPqGetVersion(FLASHPQ_GAMMA_VER), (*pz_size));
            return 0;
        }
        else
        {
    ...
    case DRV_CUSTOM_DISP_TYPE_SCE_VER:
        if (bApiFlashPqVerifyHeader() == SV_TRUE)
        {
            x_strncpy((CHAR *)pv_get_info, (CHAR *)bApiFlashPqGetVersion(FLASHPQ_SCE_VER), (*pz_size));
            return 0;
        }
        else
        {
    ...
    case DRV_CUSTOM_DISP_TYPE_QTY_VER:
        if (bApiFlashPqVerifyHeader() == SV_TRUE)
        {
            x_strncpy((CHAR *)pv_get_info, (CHAR *)bApiFlashPqGetVersion(FLASHPQ_QTY_VER), (*pz_size));
            return 0;
        }
        else
        {
...
static void bApiLINUXInitVqUiMinMaxDftTbl(VID_QA_MIN_MAX_DFT_D_T* prPQUiMinMaxDataPack)
{
    ...
    if(bFirstInitFinished==0)
    {
        rPEUITable.ptUiPicMinMaxDftTbl = (PE_UI_RANGE_ITEM**)x_mem_alloc(  rPEUITable.bSmartPicNum*sizeof(PE_UI_RANGE_ITEM*)+
                                                                                                         rPEUITable.bSmartPicNum*
                                                                                                         rPEUITable.bUIPQItemNum*
                                                                                                         sizeof(PE_UI_RANGE_ITEM));
