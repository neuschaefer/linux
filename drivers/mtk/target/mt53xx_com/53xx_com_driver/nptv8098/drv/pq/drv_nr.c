...
{
#if 0
    {MCNR_02, 0x240800B0, 0xFFFFFFFF},
    {MCNR_03, 0x007E0000, 0xFFFFFFFF},
    {MCNR_04, 0xC3000000, 0xFFFFFFFF},
    {MCNR_05, 0xC2408420, 0xFFFFFFFF},
    {MCNR_06, 0x80800000, 0xFFFFFFFF},
    {MCNR_07, 0x82083186, 0xFFFFFFFF},
    {MCNR_08, 0xAC600000, 0xFFFFFFFF},
    {MCNR_09, 0xC8F23B80, 0xFFFFFFFF},
    {MCNR_0A, 0x31D09310, 0xFFFFFFFF},
    {MCNR_0B, 0x1852B62C, 0xFFFFFFFF},
    {MCNR_0C, 0x41861528, 0xFFFFFFFF},
    {MCNR_0D, 0x524E6104, 0xFFFFFFFF},
    {MCNR_10, 0x42000428, 0xFFFFFFFF},
    {MCNR_11, 0x00000000, 0xFFFFFFFF},
    {MCNR_12, 0x00000000, 0xFFFFFFFF},
    {MCNR_15, 0x0000FFFF, 0xFFFFFFFF},
    {MCNR_17, 0x00000000, 0xFFFFFFFF},
#endif
    {MCNR_00, 0x00000420, 0x00000FFC},
    //{MCNR_01, 0x48BF8000, 0x000F0000},
...
const NR_REGTBL_T CODE _MCNR_PQ_PARAM[E_MCNR_NUM_TIMINGS][6] =
{
    ...
    {   // HD 720p
        ...
        {MCNR_05, 0xC2408420, 0xFFFFFFFF},
        //{MCNR_0A, 0x3A129394, 0xFFFFFFFF},
        //{MCNR_0B, 0x2052B62C, 0xFFFFFFFF},
        //{MCNR_0C, 0x418614A8, 0xFFFFFFFF},
        //{MCNR_0D, 0x5AD48288, 0xFFFFFFFF},
...
void vDrvNRSetChromaUpSample(BOOL fgOnOff)
{
    ...
    if(SV_TRUE == NR_R(NXNR_00,NX_SD_OFF))
    {
        fgOnOff = SV_OFF;
    }
...
UINT8 u1DrvNRGetNoiseMeterSupport(void)
{
    ...
    case MODE_1080i_50:
    case MODE_1080i:
        u1Ret = (_rNrPrm.u1Ver & NR_SUPPORT_6BIT_NL) ? 1 : 0;
        break;
    default:
        u1Ret = 0;
    }
...
{
    return (_rNrPrm.u1Ver & NR_SUPPORT_STRENTH) ? 1 : 0;
}
...
{
    // SNR gain
    NR_WM(NXNR_04, _P2D(NX_SNR_GAIN_Y)|_P2D(NX_SNR_GAIN_C));
...
{
    NR_W(NXNR_00, (u1OnOff ? 1 : 0), NX_SNR_Y);
}
...
{
    u1OnOff = (u1OnOff && u1DrvNRGetNoiseMeterSupport() ? 1 : 0);
...
{
    u1OnOff = (u1OnOff && u1DrvNRGetNoiseMeterSupport() ? 1 : 0);
    _SWWM(NR_CTRL_00, P_Fld(u1OnOff, EN_AUTO_XNR)|P_Fld(u1OnOff, EN_ADAP_WEIGHT));
    vDrvNRUpdateParam();
}
