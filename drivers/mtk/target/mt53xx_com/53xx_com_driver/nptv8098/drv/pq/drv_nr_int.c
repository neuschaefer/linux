...
static NR_ERFB_T _rNrErfb = {63, 0, 1, 0, 8, -8};

static NR_PEAK_FILT_T _rNrPeakFilt;

static NR_SNR_TH_T _rSnrTh = {0};

static NR_PQ_ADAP_REG_T _rNrPQ = {0x34};
...
const UINT16* pu2DrvNRGetSADHist(void)
{
    return &SADHIST[0];
}

const NR_FLFB_STA_T* prDrvNRGetFlfbSta(void)
{
    return &_rNmSta;
}

UINT16 u2DrvNRGetMVHist(INT32 x, INT32 y)
{
	return MV_HIST(x,y);
}

UINT16 u2DrvNRGetMVHistInd(INT32 x)
{
	return MV_HIST_IND(x);
}
...
static void _QSORT(void *va, long n, long es, INT32 (*cmp)(const void*, const void*))
{
        Sort s;
...
// NR drivers
////////////////////////////////////////////////////////////////////////////////
void vDrvNRUpdateParam(void)
{
    // for those software registers that are updated for changing HW registers.
    if (_rNrPrm.u1ParamUpdate < 0xff)
        _rNrPrm.u1ParamUpdate ++;
}

static UINT8 u1DrvNRGetUpdateParam(void)
{
    return _rNrPrm.u1ParamUpdate;
}
...
{
    static UINT8 u1ScSkip = 0;

    u1ScSkip = _rNmSta.u1ScnChg ? _SWR(NR_NM_07, NM_SC_HIST_SKIP) : (u1ScSkip ? u1ScSkip-1 : 0);
...
{
    static UINT8 _u1ScSkip = 0;

    _u1ScSkip = (_rNmSta.u1ScnChgCnt == 1) ? 18 : (_u1ScSkip ? _u1ScSkip-1 : 0);
...
{
    static UINT8 _u1RawVal[16] = {0};
    static UINT8 _u1FiltVal[16] = {0};
    ...
    for (i = 0; i < 16; i++)
    {
        i4DiffSum += _i2Error((INT32) _u1RawVal[i] - _u1FiltVal[i]);
    }

    if (_SWR(NR_NM_03, RST_SC_ERFB) && _rNmSta.u1NLScnChgCnt == 1)
    {
        _rNrErfb.u1SysSta = _u1DrvNRGetNLScnChg();
        //_rNrPeakFilt.u1FIRResult*4; //_rNmSta.u1PeakBinMo;
        _rNrErfb.i2DiffAccm = 0;
    }
    else
    {
        if ((i4DiffSum > _rNrErfb.i2IncTh) &&
        ...
        else if ((i4DiffSum < _rNrErfb.i2DecTh) && _rNrErfb.u1SysSta > 0)
        {
    ...
    _u1RawVal[_u1Idx] = u1Value;
    _u1FiltVal[_u1Idx] = _rNrErfb.u1SysSta;
...
static UINT8 _u1DrvNRErrorFeedBack(UINT8 u1Value)
{
...
#endif
    if (_SWR(NR_NM_00, FORCE_NOISE_LVL_EN))
    {
        u1Value = _SWR(NR_NM_00, FORCE_NOISE_LVL);
    }
    ...
    {
        _rNrErfb.i2DiffAccm += _i2Error((INT16)u1Value - (INT16)_rNrErfb.u1SysSta);
...
{
    //_rNmSta.u1NoiseLevel = _rNmPrm.u1DefaultNoiseLvl;
    //_rNmSta.u1ValidNoiseLevel = SV_TRUE;
...
static void _vDrvNXNRReadHBlkLineSum(void)
{    
    UINT32 u4HBlkDataSize = MIN(_rNrPrm.u2Height/2, (NX_HEIGHT/2));
    UINT32 u4HBlkAddr, i;
    ...
    u4HBlkAddr = NR_HBLK_START;
    i = 0;

    while (i++ < u4HBlkDataSize)
    {
	    _SWW_4B(u4HBlkAddr, NR_R4B(NXNR_25));
        NR_W(NXNR_15, i, NX_INIT_SRAM_ADDR);
...
static void _vDrvNXNRReadVBlkLineSum(void)
{    
    UINT32 u4VBlkDataSize = MIN(_rNrPrm.u2Width/2, (NX_WIDTH/2));
    UINT32 u4VBlkAddrEnd = NR_VBLK_START + u4VBlkDataSize*4;
    UINT32 u4VBlkAddr;
    ...
    {
        _SWW_4B(u4VBlkAddr, NR_R4B(NXNR_24));
    }
...
static void _vDrvNXNRCalcBlkHist(void)
{
    ...
    UINT32 i;
    UINT32 u4AliasCnt, u4GoodBlkCnt;
    UINT32 u4VBlkAddr;
    UINT32 u4VBlkSum = 0;
    ...
    const UINT32 u4GoodThd = _SWR(NR_BK_METER, NR_BK_GOOD_THD);
    ...
    for (u4VBlkAddr = NR_VBLK_START; u4VBlkAddr < u4VBlkAddrEnd; u4VBlkAddr += 4)
    {
        ...
        if (i == 0)
        {
            ...
            for (j = 0; j < 8; j++)
            {
                ...
                if (arLocalVBlkS8[j] < arLocalVBlkS8[u4HWPhase]) u4GoodCnt++;
            }
            u4AliasCnt += ((arLocalVBlkS8[u4HWPhase] == 0) ? 1 : 0);
            u4GoodBlkCnt += ((u4GoodCnt >= u4GoodThd) ? 1 : 0);
        }
    }

    _QSORT(arVBlkS8Hist, 8, sizeof(NR_BLK_PHASE_SUM_T), _cmpBlk);
    ...
    _SWWM(NR_BLK_STA_0E, P_Fld(u4AliasCnt, NR_BLK_ALIAS_CNT)|P_Fld(u4GoodBlkCnt, NR_BLK_GOOD_BLK_CNT));
...
static void _vDrvMCNRReadSADHist(void)
{
    ...
    UINT32 u4Wei = (_SWR(NR_NM_03, RST_SC_HIST_IIR)&&_rNmSta.u1ScnChg) ? 0 : _rNmPrm.u1SADHistIIR;

    if (_SWR(NR_NM_03, RST_SC_HIST_IIR) && _rNmSta.u1ScnChgCnt)
        return;
...
static void _vDrvMCNRReadHWStatus(void)
{
    ...
    if (_count++ < _rNmPrm.u1SADHistSubSample)
    {
        return;
    }
    ...
    _rNmSta.u1ScnChg = (_SWR(NR_NM_0E, NM_SCNCHG_DET_EN) ? 
        _SWR(NR_NMSTA_0F, NR_NM_HOMO_SCNCHG) : NR_R(MCVP_CS_29, SCENE_CHANGE_STATUS)) || u1ForceChg;
...
void vDrvMCNRSetSADIIRWeight(UINT8 u1Wei)
{
	_rNmPrm.u1SADHistIIR = MIN(u1Wei, 16);
}

void vDrvMCNRSetSADSubSample(UINT8 u1Interval)
{
	_rNmPrm.u1SADHistSubSample = u1Interval;
}
...
static void _vDrvSnrGetParam(void)
{
    ...
    if ((_rSnrTh.u4A != a) || (_rSnrTh.u4B != b) || (_rSnrTh.u4C != c) ||
        ...
        (_rSnrTh.u1MARto != beta) || (_rSnrTh.u1GeneralStr != k) || (_rSnrTh.u1CostDirAdap != u1CostDirAdap))
    {
        _rSnrTh.u4A = a;
        _rSnrTh.u4B = b;
        _rSnrTh.u4C = c;
...
static void _vDrvMCNRUpdateOSDInk(void)
{
    ...
    if (_rNmSta.u1ScnChg && _EN(EN_LOG_SCNCHG))
        LOG(0, "Scene Change: Peak = %d, NL = %d, HOMO = %d, TH = %d\n",
...
{
//	if ( _fgIsSDTiming && (u12DTo3D_flag==0))
//		u1NRStillTexMode = u1NRStillTexMode;
//	else
//		u1NRStillTexMode = 0;
...
static void _vCalcNoiseLevelScnChg(void)
{
    ...
    if (u4ScnChgCnt == 1)
    {
        ...
        _QSORT(u1CentroidQ, 18, sizeof(UINT8), _cmpr);
...
static void _vDrvMCNRSADAnalysis (void)
{
    ...
    if ((_rNrPrm.eMCNRMode == MCNR_MODE_MA) || (_rNrPrm.u1NoiseMeterEn == 0) /*|| valid_noise_level == SV_FALSE*/)  // firmware control
    {
...
static void _vDrvNRSetAdapParam(void)
{
    ...
    UINT8 u1EnCurr = _EN(EN_ADAP_NR) ? 
        (_EN(EN_ADAP_NR) | (_EN(EN_ADAP_WEIGHT)<<1) | (_EN(EN_ADAP_SNR_THM)<<2) | (_EN(EN_ADAP_FORCE_MA)<<3) | (_EN(EN_CLEAN_DF_DCR)<<4) | (_EN(EN_AUTO_XNR)<<5)) : 0;
    ...
    _rNmSta.u2SnrThm = LERP_TGT(_rNmSta.u2SnrThm, _u4EstSNRManualTh(u4NoiseLvl), _SWR(NR_NM_00, SNR_THM_IIR_WEI), 6);
    ...
    if ((u1EnCurr&_EN_ADAP_NR) || (_u1EnPrev&_EN_ADAP_NR))
    {
        ...
        else
        {
            ...
            if ((u1EnCurr&_EN_AUTO_XNR) || (_u1EnPrev&_EN_AUTO_XNR))
            {
                UINT32 u4Enable = (u1EnCurr&_EN_AUTO_XNR) && (u4NLClip >= 8) ? 1 : 0;
    ...
    if ((u1EnCurr&_EN_ADAP_WEI) || (_u1EnPrev&_EN_ADAP_WEI) || u1DrvNRGetUpdateParam())
    {
        UINT32 idx = (u1EnCurr&_EN_ADAP_WEI) ? MIN(((u4NoiseLvl + 2 )/4), (NUM_PARAM_GROUP-1)) : 0;
        UINT32 u4CleanDfLvl =
        ...
        UINT32 u4NewTblTh = _rNrPQ.u1THD8;
        ...
        NR_W(MCNR_0F, u4NewTblTh, MCNR_TBL_THD8);
	}
    NR_WM(MCNR_03,
    ...
    if ((u1EnCurr&_EN_ADAP_SNR_THM) || (_u1EnPrev&_EN_ADAP_SNR_THM) || u1DrvNRGetUpdateParam())
    {
        UINT32 idx = (u1EnCurr&_EN_ADAP_SNR_THM) ? MIN(((u4NoiseLvl + 2 )/4), (NUM_PARAM_GROUP-1)) : 0;
        UINT32 u4SnrThm = (u1EnCurr&_EN_ADAP_SNR_THM) ? _rNmSta.u2SnrThm : _u4EstSNRManualTh(0);
        UINT32 u4K = MIN(8, _rSnrTh.u1GeneralStr);
        UINT32 u4ManualAutoRto = LERP(0, _rSnrTh.u1MARto, MIN(idx, 8), 3);
...
void vDrvNRPQInit(void)
{
    ...
    _SWW_4B(NR_NM_03, 0x0149A010);  // clip noise level at 8 (0x20)
    ...
    _SWW(NR_BK_METER, 7, NR_BK_GOOD_THD);
...
void vDrvNRVBlankProc(void)
{
    ...
    _vDrvNXNRReadVBlkLineSum();
    if (_EN(EN_SUPPORT_HBLK))
        _vDrvNXNRReadHBlkLineSum();

    u4CurrLine = _u4GetCurrLineCnt();
    _SWW_4B(NR_BLK_STA_03, ((u4CurrLine<<16) | u4StartLine));
}
...
void vDrvNRNoiseLevelProc(void)
{
    ...
    u4Dirty = (u4Start <= _SWR(NR_NMSTA_12, NR_VSYNC_PROC_END));
