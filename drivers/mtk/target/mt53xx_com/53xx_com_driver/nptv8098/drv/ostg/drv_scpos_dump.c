...
UINT8 bInitLastStageDump(void)
{
    ...
    wHTotal = RegReadFldAlign(SCPIP_PIP_OA_00, PIP_OA_00_PANEL_HTOTAL);
    wMoSyncLen  = RegReadFldAlign(SCPIP_PIP_OA_01, PIP_OA_01_MOSYNCLEN);
    wVsync_Delsel2 = RegReadFldAlign(OSTG_PTGEN_00, FLD_VSYNC_DELSEL2);
  ...
  //  wVL = wVTotal + 1;
    ...
    _au4ValueStack[_u1StackSize++] = u4RegRead4B(YCBCR2RGB_00);
...
void vGetScalerDRAMInfo(tagDramDumpInfo_T *rDramDumpInfo)
{
    ...
    PreDownScaleFactor = u4ScpipGetHwHPS(SV_VP_MAIN);
...
UINT8 bGetScalerDRAM(UINT32 u2X, UINT32 u2Y, UINT32 *pu2VR, UINT32 *pu2UB, UINT32 *pu2YG)
{
    ...
    u4HpsFactor = u4ScpipGetHwHPS(SV_VP_MAIN);
