...
RRegDwrdTbl const CODE REGTBL_PEI[] =
{
	...
	{        US1_PEI_20, 0x80000000, 0x80000000},    
...
void vDrvSetUpScalerFilter(UINT8 bPath, UINT8 bType, UINT8 bMode)
{
    ...
    if (bType == UPSCALER_HORI)
	    ...
	    vRegWriteFldAlign(SCPIP_US1_09, u4FilterCoeff[HORI_Y_Index + 0], US1_09_R_WDATA_02);
	    vRegWriteFldAlign(SCPIP_US1_08, u4FilterCoeff[HORI_Y_Index + 1], US1_08_R_WDATA_01);
	    vRegWriteFldAlign(SCPIP_US1_07, u4FilterCoeff[HORI_Y_Index + 2], US1_07_R_WDATA_00);
	    }
		#if defined(CC_MT5396) || defined(CC_MT5368)
			// HORI_CbCr  
		    UINT16 HORI_C_Index;  
		    ...
		    for (bSramAddr = 0; bSramAddr < bPhaseNum; bSramAddr++)
		    {      
			    HORI_C_Index = bSramAddr*bCoefLength+bHorCbCrCoefTblOffset + bMode*bUPSCoefTotal;     
			    vRegWriteFldAlign(SCPIP_US1_09, u4FilterCoeff[HORI_C_Index + 0], US1_09_R_WDATA_02);
			    vRegWriteFldAlign(SCPIP_US1_08, u4FilterCoeff[HORI_C_Index + 1], US1_08_R_WDATA_01);
			    vRegWriteFldAlign(SCPIP_US1_07, u4FilterCoeff[HORI_C_Index + 2], US1_07_R_WDATA_00);                
		    }
		#endif	    
    	}
	    ...
	    for (bSramAddr = 0; bSramAddr < bPhaseNum; bSramAddr++)
	    {
		    VER_Index = bSramAddr*bCoefLength+bVerCoefTblOffset + bMode*bUPSCoefTotal;  
		    vRegWriteFldAlign(SCPIP_US1_09, u4FilterCoeff[VER_Index + 0], US1_09_R_WDATA_02);
		    vRegWriteFldAlign(SCPIP_US1_08, u4FilterCoeff[VER_Index + 1], US1_08_R_WDATA_01);
		    vRegWriteFldAlign(SCPIP_US1_07, u4FilterCoeff[VER_Index + 2], US1_07_R_WDATA_00);                                
	    }
    	}
    ...
    if (bType == UPSCALER_HORI)
	    ...
	    for (bSramAddr = 0; bSramAddr < bPhaseNum; bSramAddr++)                                                  
	    {
			...
			for (bSramAddr = 0; bSramAddr < bPhaseNum; bSramAddr++)
			{
				...
				u4SramReadData1 = RegReadFldAlign(SCPIP_US1_0C, US1_0C_R_RDATA_02);
				// Compare Data		                               
				if (u4SramReadData1 != u4FilterCoeff[HORI_C_Index + 0] ||
				    u4SramReadData2 != u4FilterCoeff[HORI_C_Index + 1] ||
				    u4SramReadData3 != u4FilterCoeff[HORI_C_Index + 2])
				{
				    Printf("bSramAddr = %d\n",HORI_C_Index );            
				    Printf("[H_C][0] Read [%d]  Write [%d]\n", u4SramReadData1, u4FilterCoeff[HORI_C_Index + 0]);
	    ...
	    for (bSramAddr = 0; bSramAddr < bPhaseNum; bSramAddr++)
	    {
		    ...
		    u4SramReadData1 = RegReadFldAlign(SCPIP_US1_0C, US1_0C_R_RDATA_02);
		    // Compare Data		                               
		    if (u4SramReadData1 != u4FilterCoeff[VER_Index + 0] ||
		        u4SramReadData2 != u4FilterCoeff[VER_Index + 1] ||
		        u4SramReadData3 != u4FilterCoeff[VER_Index + 2])
		    {
		        Printf("bSramAddr = %d\n",VER_Index );              
		        Printf("[V][0] Read [%d]  Write [%d]\n", u4SramReadData1, u4FilterCoeff[VER_Index + 0]);
