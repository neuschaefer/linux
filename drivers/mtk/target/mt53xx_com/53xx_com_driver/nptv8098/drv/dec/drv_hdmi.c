...
#define HDMI_RES_STABLE 30
#define HDMI_HDCP_STABLE 30
#define ENABLE_ALL_HDMI_LOG 0
...
extern E_HDMI_SWITCH_NUM eActiveHdmiPort = HDMI_SWITCH_INIT;
extern E_HDMI_SWITCH_NUM eCurrHdmiPort = HDMI_SWITCH_INIT;
extern E_HDMI_SWITCH_NUM  eHDMICurrSwitch = HDMI_SWITCH_INIT;
...
#if 0
static UINT32 _au4HArray[80] =
{
   0xe67c1400, 0xe8f3372c, 0x66274da8, 0x132fd0a8, 0xe7977955,
   0x04183787, 0x123b5fd1, 0x6fa4329f, 0xbc8a1c58, 0xcbe37f8c,
   0x45b49e81, 0xcc6a667c, 0x17c85cdd, 0x914390a3, 0x1df0018b,
   0xde1b8aff, 0xf6b6d694, 0x427b156d, 0x041ba213, 0xcc11ddb5,
   0x65662842, 0x65f177f5, 0x2c9b8cc3, 0x7ce9b4ad, 0x4ad6bcd1,
   0x3e45f75d, 0xa8ec222a, 0x575468df, 0x38c8105a, 0xa7a0949f,
   0x67a271a0, 0x8dbd238e, 0x010d8963, 0xba4c9791, 0x73944d5c,
   0x6c126836, 0x51b1fae8, 0xcec693c1, 0x6bc19072, 0x0263f64d,
   0x809ba6d3, 0xffa9b635, 0x6fd9fd8e, 0x4cdba624, 0xcf0f0cd2,
   0xe4191acd, 0x176d9c62, 0xcb39576b, 0x75800d6a, 0x7d69f3fa,
   0xc0e3799f, 0xa8d25a8b, 0x9390d9c9, 0x1e1ae6aa, 0x2d039317,
   0x33afc043, 0x18a96694, 0xf522cc55, 0x37c5c823, 0x96d281f1,
   0xe55a0aaf, 0x63ef138a, 0x3fc64c19, 0xb27a9a6c, 0x5a31d4a6,
   0xcd210130, 0x047486a2, 0x08383e46, 0xb4a757e7, 0x78027341,
   0xce933493, 0xd3465a93, 0x61f7140a, 0x228fb703, 0x983af1d2,
   0x536228ba, 0x191d4cca, 0xf78749ce, 0x000026b5, 0x00000000,
};
...
void vHDMI_Load_Default_HArray(void)
{
    UINT8 i;
    
    vIO32Write4B(HD_ADDR,0);
    
    for (i = 0 ; i < 80 ; i++)
    {
        vIO32Write4B(HD_ADDA,_au4HArray[i]);
    }
    // en/adr
...
#if ENABLE_ALL_HDMI_LOG	
   LOG(0,"!!vSetHdmiEqTracking %d\n",u1Val);
#endif    
}
...
{
    UINT32 u4TmpRegBase = 0xc00;

#if defined(CC_MT5389) || defined(CC_MT5368) || defined(CC_MT5396) //TODO_ES (Daniel, 2011/4/5)
    vRegWriteFldAlign(APLL0 + u4TmpRegBase, 0x3, SDREF);
    vRegWriteFldAlign(APLL1 + u4TmpRegBase, bSDINOVR, SDINOVR);
    vRegWriteFldAlign(CTS_HW_ADP3 + u4TmpRegBase, value1, SDINOVRVAL_31_0);
    vRegWriteFldAlign(CTS_HW_ADP4 + u4TmpRegBase, value2, SDINOVRVAL_39_32);
  ...
  //The following is for software mode DDS Setting, 
    if (value1 == 0x205bc01a)
    {
        ...
        vRegWriteFldAlign(DDS + u4TmpRegBase, 0x131205bc, REG_HADDS2_PCW_SW);        
    }
    else
    {
        //48KHz
        //for DDS mode HDMI setting              
        vRegWriteFldAlign(DDS + u4TmpRegBase, 0x14d867c3, REG_HADDS2_PCW_SW);        
    }
    
    //PCW_NCPO_CHG toggle
    
    vRegWriteFldAlign(ACR_CTRL3 + u4TmpRegBase, 0, FBDIV);   //fbdiv multiply 2
    
    if ((value1==0)&&(value2==0)&&(bSDINOVR==0))
    {
	  vRegWriteFldAlign(ACR_CTRL3 + u4TmpRegBase, 0, REG_HADDS2_PCW_SEL_SW);   // 0: HW mode PCW
    }
    else
    {
	  vRegWriteFldAlign(ACR_CTRL3 + u4TmpRegBase, 1, REG_HADDS2_PCW_SEL_SW);	// 1: SWmode PCW
    }
    
    vRegWriteFldAlign(ACR_CTRL3 + u4TmpRegBase, 1, REG_HADDS2_PCW_CHG_SW);
    vRegWriteFldAlign(ACR_CTRL3 + u4TmpRegBase, 0, REG_HADDS2_PCW_CHG_SW);
     ...
     switch (u1Val)
        {
            case TMDS_CLOCK_LOW1:  // < 30MHz
                     ...
                     vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG2 + u4RegOffset, 1, RG_HDMI_0_RXMODE);                    
            ...
            case TMDS_CLOCK_MIDDLE:  // 40 ~ 140MHz
                     vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG1 + u4RegOffset, 0, RG_HDMI_0_BAND_SEL_SW);
                     vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG2 + u4RegOffset, 3, RG_HDMI_0_RXMODE);
...
UNUSED(u4RegOffset);
    ...
    bCH0EQ = IO32ReadFld(RGS_HDMI_RX_Normal_0_CFG0,RGS_HDMI_0_CH0_EQERR);
    vIO32WriteFld(ANA_INTF_1, bCH0EQ, RG_HDMI_EQ_FILT);
#elif defined(CC_MT536) || defined(CC_MT5395)
    bCH0EQ = RegReadFldAlign(TMDS_CTRL_STATUS1, HDMI_CH0_EQERR);
...
//	#endif
    ...
    /*analog setting*/
     ...
     vIO32Write4B(REG_HDMI_RX_Normal_0_CFG2 + u4AnaRegBase, 0x11210002);
     ...
     vIO32Write4B(REG_HDMI_RX_Normal_0_CFG2 + u4AnaRegBase, 0x11210003);        
...
#if defined(CC_MT5389)//CC_AUD_PLL_HALF
    ...
    vIO32WriteFldAlign(CKGEN_MISC_CKCFG + u4RegBase, 0x0, FLD_HDMI_CLK_Crystral);
    ...
    vRegWriteFldAlign(VID_HRES + u4RegBase, 0x4, HRES_THR); //for Iris project sony DSC T500 and Foxconn project  Setop Box model¡GScienfitic Atlanta 8300HDC flash use
    ...
    vRegWriteFldAlign(VID_BLANK + u4RegBase, 0x2, INTERLACE_LINE_DIFF_TH);//for INTERLACE_DET_SEL
    /*enable I2C deglitch*/
...
#if ENABLE_ALL_HDMI_LOG	
			LOG(0, "Port %d _bHDMIState change from %s to %s\n",ePort, _aszHdmiState[_arHdmiRx[ePort]._bHDMIStateOld], _aszHdmiState[u1HdmiState]);
#endif
...
#if ENABLE_ALL_HDMI_LOG	
			LOG(0, "Port %d _bHDMIState change from %s to UNDEFINED_STATE\n",ePort, _aszHdmiState[_arHdmiRx[ePort]._bHDMIStateOld]);
#endif
                ...
                if (RegReadFldAlign(SRST + u4HdmiRegBase, SCDT))
                {
         		...
         		vHDMIMuteAudio();
	              ...
	              vHDMIVideoOutOff();
                    ...
                    {
                        LOG(0, "Port %d eHdmiPortHDMI_STATE_AUTH wstable sec: %d,ResStable %d hflag: %d, fastSw %d\n", \
...
{
#if CHECHCKDT
    if ((u1RegRead1B(SRST_2 + u4ActiveHdmiRegBase) & 0x01) == 0x01)//modify by ciwu
#else
    if (((u1RegRead1B(SRST_2 + u4ActiveHdmiRegBase) & 0x01) == 0x01) && _fgVideoOn == TRUE)
#endif
    {
        return TRUE;
    }
