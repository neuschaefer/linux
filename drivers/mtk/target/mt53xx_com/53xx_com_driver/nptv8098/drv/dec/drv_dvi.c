...
#define CC_SUPPORT_HDMI_DYNAMIC_WAIT_STABLE
...
#define WIDTH_HEIGHT_FROM_HDMIDEC 1
...
#ifdef CC_SUPPORT_HDMI_DYNAMIC_WAIT_STABLE
UINT8 _bVStableCnt;
UINT8 _bHStableCnt;
...
#endif
#if  ENABLE_ALL_DVI_LOG
		LOG(0, "Main Mode Chg #1 in vDviInitial\n");
#endif	 
...
#endif
#if  ENABLE_ALL_DVI_LOG
		LOG(0, "Pip Mode Chg #1 in vDviInitial\n");
#endif	 
...
void vDVISetModeDone(void)
{
    ...
    vRegWrite4B(INTR_STATE0 + u4ActiveHdmiRegBase, Fld2Msk32(INTR2_HDMI_MODE) | Fld2Msk32(INTR2_CKDT) | Fld2Msk32(INTR2_SCDT) | Fld2Msk32(INTR2_CLK_CHG));
    vRegWrite4B(INTR_STATE1 + u4ActiveHdmiRegBase, Fld2Msk32(INTR5_VRESCHG) | Fld2Msk32(INTR5_HRESCHG) | Fld2Msk32(INTR5_POLCHG) | Fld2Msk32(INTR5_INTERLACEOUT));
    bHdmiPhyReset(eActiveHdmiPort,HDMI_RST_DEEPCOLOR);
    vUtDelay1ms(1);
    vRegWriteFldAlign(MUTE_SET + u4ActiveHdmiRegBase, 1, TDFIFO_SYNC_EN);  // jiewen, 20090114
    vUtDelay1ms(1);
	...
	LOG(0, " The HDMI ColorSpace is %d \n", _bHDMIColorSpace);	
    _bNodifyConnect = 0;
    if (fgIsMainDVI())
    {
...
#endif
    // disable 4xSel
    vRegWriteFldAlign(LPF_PIP_00, 0, CCH_CEN_4XSEL);
    // CCH field out, default non-3D
    vRegWriteFldAlign(PQCRC_02, 0, CCH_3D_FPSEL);
...
UINT8 bHDMI3DIPCheck(void)
{
    ...
    if ((_bDviTiming == MODE_3D_720p_50_FP)||(_bDviTiming == MODE_3D_720p_60_FP)||(_bDviTiming == MODE_3D_1080p_24_FP)||
    	...
    	(_bDviTiming == MODE_3D_720p_25_FP)||(_bDviTiming == MODE_3D_1080p_25_FP))
    {
	...
	if((_bDviTiming == MODE_3D_1080p_30_SBS_FULL)||(_bDviTiming == MODE_3D_1080p_24_SBS_FULL)||(_bDviTiming == MODE_3D_720p_24_SBS_FULL)||
	   (_bDviTiming == MODE_3D_720p_30_SBS_FULL)||(_bDviTiming == MODE_3D_720p_50_SBS_FULL)||(_bDviTiming == MODE_3D_720p_60_SBS_FULL)||
	   (_bDviTiming == MODE_3D_480p_60_SBS_FULL)||(_bDviTiming == MODE_3D_576p_50_SBS_FULL))
	{
...
UINT32 wVprochwidth;
    ...
    if(1==bDviBypassAp3DRule())
    {
      	...
      	else if(FP_interlace==bHDMI3DIPCheck()) //interlace frame packing timing
       {
            ...
            // CCH field out
            vRegWriteFldAlign(PQCRC_02, 1, CCH_3D_FPSEL);            
       }
      	else
       {
            vRegWriteFldAlign(VFE_EF, 0x0, CCH_3D_VACT);
                ...
                (_wDviHclk <= (Get_VGAMODE_IHF(bSearch) + 5)))
                {
                    ...
                    // if (!bMode)
                    {
                                ...
                                (wTmp < (Get_VGAMODE_IHTOTAL(bSearch) + 40)))
                        {
                        	...
                        	{
                        		LOG(1,"bSearch=%d\n",bSearch);
                        		LOG(1,"UNRECRX0_0=%x\n",u1RegRead1B(UNRECRX0_0 + u4ActiveHdmiRegBase));
                       			LOG(1,"INTR_STATE0=%x\n",RegReadFldAlign(INTR_STATE0 + u4ActiveHdmiRegBase, INTR3_NEW_UNREC));
                       			LOG(1,"UNRECRX1_3=%x\n",u1RegRead1B(UNRECRX1_3 + u4ActiveHdmiRegBase) >> 5);
                        	  	if (bHDMI3DPacketVaild())
                        	  	{                   
									if((u1RegRead1B(UNRECRX2_0 + u4ActiveHdmiRegBase) >> 4)==HDMI_3D_Structure_FramePacking)
									{
									...
									else if((u1RegRead1B(UNRECRX2_0 + u4ActiveHdmiRegBase) >> 4)==HDMI_3D_Structure_SideBySideFull)
									{
...
PRIVATE void vDviSetInputCapture(UINT8 bMode)
{
    ...
    if (RegReadFldAlign(VID_CH_MAP + u4ActiveHdmiRegBase, INTERLACEDOUT))
    {
    ...
    {
        if (RegReadFldAlign(VID_VRES + u4ActiveHdmiRegBase, VID_VRES_MUTE))
        {
            vRegWriteFldAlign(VID_VRES + u4ActiveHdmiRegBase, 0x1, VRES_MUTE_CLR);
            vRegWriteFldAlign(VID_VRES + u4ActiveHdmiRegBase, 0x0, VRES_MUTE_CLR);
        }
    }
}
         	...
         	if(1==bDviBypassAp3DRule())
         	{
	            if (fgIsVideoTiming(_bDviTiming))
	            {
	                ...
	                else		//480i 444 has two pixel repeat type, Y0 Y0 Y1 Y1 Y2 Y2 or Y1 Y2 Y3 Y4 Y5 , drop pixel will be decided by PE , Hactive must be 1440
	                {
						...
						if (_bWidthStableFlg)
						{
							return (bDVIGetWidth() >> (RegReadFldAlign(CCIR_00, CCH_CEN_SEL)));
						}
						else
						{
							return (Get_VGAMODE_IPH_WID(_bDviTiming) >> (RegReadFldAlign(CCIR_00, CCH_CEN_SEL)));
					...
					if (_bWidthStableFlg)
					{
						return (bDVIGetWidth() >> (RegReadFldAlign(DVI_00, DVI_CEN_SEL)));
					}
					else
					{
						return (Get_VGAMODE_IPH_WID(_bDviTiming) >> (RegReadFldAlign(DVI_00, DVI_CEN_SEL)));
...
void vDviModeDetect(void)
{
...
#else
	if (!((u1GetHdmiState() == HDMI_STATE_AUTH) || (u1GetHdmiState() == HDMI_STATE_PWOFF)))
	{
		return;
	}
#endif
    ...
    switch (_bDviModeDetState)
    {
        ...
        case DVI_SEARCH_STATE:
            ...
            vRegWrite4BMsk(VID_SET + u4ActiveHdmiRegBase, Fld2Msk32(AV_MUTE_CLR), Fld2Msk32(AV_MUTE_CLR));
            ...
            if (_bDviTiming == MODE_DE_MODE)
            {
                ...
                if ((wDviInputWidth() == 1920) && (wDviInputHeight() == 540) && (bDviRefreshRate() == 60) && (u1RegRead1B(AVIRX1_3 + u4ActiveHdmiRegBase) == 5))
                {
                    _bDviTiming = MODE_1080i;
                }
                if ((wDviInputWidth() == 1920) && (wDviInputHeight() == 540) && (bDviRefreshRate() == 50) && (u1RegRead1B(AVIRX1_3 + u4ActiveHdmiRegBase) == 20))
                {
                    _bDviTiming = MODE_1080i_50;
                }
                if ((wDviInputWidth() == 1440) && (wDviInputHeight() == 240) && (bDviRefreshRate() == 60) && ((u1RegRead1B(AVIRX1_3 + u4ActiveHdmiRegBase) == 6) || (u1RegRead1B(AVIRX1_3 + u4ActiveHdmiRegBase) == 7)))
                {
                    _bDviTiming = MODE_525I;
                }
                if ((wDviInputWidth() == 1440) && (wDviInputHeight() == 288) && (bDviRefreshRate() == 50) && ((u1RegRead1B(AVIRX1_3 + u4ActiveHdmiRegBase) == 21) || (u1RegRead1B(AVIRX1_3 + u4ActiveHdmiRegBase) == 22)))
                {
            ...
            if (_bDviTiming == MODE_NOSUPPORT)
            {
                if (_bNotSupportCnt++ < 90)
                {
                   ...
                   #if  ENABLE_ALL_DVI_LOG
			        LOG(0, "md: MODE_NOSUPPORT %d found\n", _bNotSupportCnt);
                   #endif					
                    break;
                }
            }
            ...
            //if(_bDVIAVIinfoBypass==0x0)//mark by ciwu for davis ask
            {
                ...
                {
                    printf("u1RegRead1B(AVIRX0_0)=%x\n", u1RegRead1B(AVIRX0_0 + u4ActiveHdmiRegBase));
        ...
        case DVI_MODE_CHG_DONE_STATE:
            vRegWrite4BMsk(VID_SET + u4ActiveHdmiRegBase, 0x0, Fld2Msk32(AV_MUTE_CLR));
...
{
    _bCurHDCPstatus = u1RegRead1B(HDCP_STAT_2 + u4ActiveHdmiRegBase) & 0x30; //00: No HDCP; 01: Authenticating; 11: AUTH Done
    ...
    if (_bCurHDCPstatus == 0x30)
    {
        if (_bPreHDCPstatus == 0x30)
        {
            ...
            {
            	vRegWrite4B(INTR_STATE0 + u4ActiveHdmiRegBase, Fld2Msk32(INTR3_NEW_UNREC));
            }
            
//            if (_bHDCPStableCount == 150)
            if (_bHDCPStableCount >= 10)
            {
#if DVI_DEBUG
    ...
    else if (_bCurHDCPstatus == 0x10)
    {
        ...
        {
            _bHDCPStableCount++;
            if(_bHDCPStableCount==30)
            {
            	vRegWrite4B(INTR_STATE0 + u4ActiveHdmiRegBase, Fld2Msk32(INTR3_NEW_UNREC));
            }
            if (_bHDCPStableCount == 60)
            {
#if DVI_DEBUG
...
//    UINT32 u4RegOffset = 0;
    ...
    if (_bDviModeChged)
    {
        ...
        if ((rCurTime.u4Seconds - _rDviModeChgTime.u4Seconds) > wWaitCnt)
        {
            ...
            #if  ENABLE_ALL_DVI_LOG
			  LOG(0, "mc:  Time out to force mode detect done \n");
			#endif
            if (bDVIGetHAct())
            {
                _bDviTiming = MODE_NOSUPPORT;
	...
	if( u1IsSyncLose(eActiveHdmiPort) || u1IsHdmiUnplug(eActiveHdmiPort))
    {
        if (u1IsHdmiUnplug(eActiveHdmiPort))
        {
          ...
          #endif
		  #if ENABLE_ALL_DVI_LOG
		      LOG(0, "mc: from activity detected to no signal when _bUnplugFlag\n");
		  #endif
        }
		else 
		{
		...
		#if DVI_DEBUG
            LOG(6, "mc: from activity detected to no signal when Sync Lose\n");
        #endif
		  #if ENABLE_ALL_DVI_LOG
		    LOG(0, "mc: from activity detected to no signal when Sync Lose\n");
		  #endif
		}
            vDviInitial();
            //vDVISetModeCHG();
...
#endif
#if ENABLE_ALL_DVI_LOG
					LOG(0, "mc: from activity detected to no signal\n");
#endif
    ...
    switch (_bDviChkState)
    {
...
#endif
#if ENABLE_ALL_DVI_LOG
				LOG(0, "mc: from no signal to wait stable\n");
#endif
...
#if ENABLE_ALL_DVI_LOG					
                    LOG(0, "mc: DVI_NO_SIGNAL to force mode done \n");		
#endif
                    _bPWOFFCnt = 0;
                    _bDviTiming = MODE_NOSIGNAL;
                    vDVISetModeDone();
        ...
        case DVI_CHK_MODECHG:
...
#if 0 // move to HDMI #1
            if (RegReadFldAlign(INTR_STATE1 + u4ActiveHdmiRegBase, INTR5_HRESCHG) && _bHResChgIntDetectFlg)
            {
                ...
                if ((fgIsMainDVI() && (_u4DrvVideoGetMute(SV_VP_MAIN) != 0)) || (fgIsPipDVI() && (_u4DrvVideoGetMute(SV_VP_PIP) != 0)))
                {
                    ...
                    vRegWriteFldAlign(MUTE_SET + u4ActiveHdmiRegBase, 0, TDFIFO_SYNC_EN);
                    vRegWrite4B(INTR_STATE1 + u4ActiveHdmiRegBase, Fld2Msk32(INTR5_HRESCHG));
            ...
            {
				vHdmiMute(eActiveHdmiPort);//add by jihua
                if (fgIsMainDVI())
                {
                    vDVIFixColor(SV_VP_MAIN);
                }
...
#if ENABLE_ALL_DVI_LOG					
				LOG(0, "mc: color space change and trigger mode change bHDMIInputType() =%d \n", bHDMIInputType() );		
#endif
		...
		||(aud_ch_info.eSampeFreq!=(HDMI_AUD_FS)bHDMIGetAudSampleFreq()))
		{
...
#if ENABLE_ALL_DVI_LOG					
				LOG(0, "mc:H V total change and trigger mode change....................\n");	
#endif				
            }
...
#if ENABLE_ALL_DVI_LOG					
				LOG(0, "mc:H V active width change and trigger mode change....................\n");
#endif					
            }
...
#if ENABLE_ALL_DVI_LOG					
				LOG(0, "mc:Pixel clock  change and trigger mode change....................\n");
#endif	
            ...
            if (((bHDMI422Input() == 1) && (1 == RegReadFldAlign(SYS_CTRL + u4ActiveHdmiRegBase, IDCK)) && (0 == RegReadFldAlign(CCIR_00, CCH_CLK_DIV)))
                    || ((bHDMI422Input() == 0) && (1 == RegReadFldAlign(SYS_CTRL + u4ActiveHdmiRegBase, IDCK)) && (1 == RegReadFldAlign(CCIR_00, CCH_CLK_DIV))))
            {
            ...
            #if CCIR_decode_444
               //[DTV00082304], change color space between ycbcr422 and ycbcr444, need trigger mode change.
               if (((_bDviMdChgCnt > _bDviMdChgCntTHR)  || (_bDviDeChgCnt > _bDviDeChgCntTHR) || (_bDviPixClkChgCnt > _bDviPixClkChgCntTHR) || (_bCCIRNeedModeChgCnt > _bCCIRNeedModeChgCntTHR)) || (_bHdmiMD != u1IsHdmiMode())
                   #if SUPPORT_SBS_CONVERT
            ...
            #else
               if (((_bDviMdChgCnt > _bDviMdChgCntTHR)  || (_bDviDeChgCnt > _bDviDeChgCntTHR) || (_bDviPixClkChgCnt > _bDviPixClkChgCntTHR)) || (_bHdmiMD != u1IsHdmiMode())
                   #if SUPPORT_SBS_CONVERT
                   || _fgScaleTriggerHdmiMch
                   #endif
                   #if (defined(CC_MT5365) && defined(MT5366_SUPPORT_3D_PR))           
...
#if 0  // move to HDMI #2             
                bHdmiPhyReset(eActiveHdmiPort,HDMI_RST_ALL);
...
#if ENABLE_ALL_DVI_LOG					
				LOG(0, "mc: check mode change to wait stable %d \n", _bDviMdChgCnt);
#endif					
                _bDviChkState = DVI_WAIT_STABLE  ;
                //vRegWriteFldAlign(TMDS_CTRL0, 0, HDMI_RTCK_AUTO);
        ...
        case DVI_WAIT_STABLE:
                    ...
                    ((wvtemp >= (_wDviTmpVtotal - _wDviTmpTHR)) && (wvtemp <= (_wDviTmpVtotal + _wDviTmpTHR))))
            {
...
#ifdef CC_SUPPORT_HDMI_DYNAMIC_WAIT_STABLE
				if (RegReadFldAlign(VID_VRES + u4ActiveHdmiRegBase,VID_VRES_STB))
				{
				    _bVStableCnt++;
				    LOG(6, "+_bVStableCnt %d \n", _bVStableCnt);
#if ENABLE_ALL_DVI_LOG					
				LOG(0, "+_bVStableCnt %d \n", _bVStableCnt);
#endif									
                }
                else
                {
				...
				if (RegReadFldAlign(VID_HRES + u4ActiveHdmiRegBase,VID_HRES_STB))
				{
				    _bHStableCnt++;
				    LOG(6, "+_bHStableCnt %d \n", _bHStableCnt);
#if ENABLE_ALL_DVI_LOG					
				LOG(0, "+_bHStableCnt %d \n", _bHStableCnt);
#endif								
				}
				else
				{
				...
				if ((u1RegRead1B(AVIRX0_0 + u4ActiveHdmiRegBase) == 0x82) && _bHdmiMD)
				{
				    _bAVIStableCnt++;
					_bAVIFlag=1;
				    LOG(6, "+_bAVIStableCnt %d \n", _bAVIStableCnt);
#if ENABLE_ALL_DVI_LOG					
				LOG(0, "+_bAVIStableCnt %d \n", _bAVIStableCnt);
#endif															
				}
				else 
				{
				...
				LOG(0, "-_bAVIStableCnt %d \n", _bAVIStableCnt);
                            ...
                            // check if timing
				if ((_bVStableCnt > _bHDMISTABLECNT && _bHStableCnt > HRES_STB_COUNTS ) || \
					(u1IsHdmiFastSw(eActiveHdmiPort) && _bVStableCnt > (_bHDMISTABLECNT-20)))
				{
				#if WIDTH_HEIGHT_FROM_HDMIDEC
...
#if 0 //#ifdef CC_SUPPORT_HDMI_FBOOTING
			if (((u1IsHdmiSrcIn(eActiveHdmiPort) == 1) && (bDviSigStatus() == SV_VDO_STABLE)) ||(u1HdmiHdcpCnt(eActiveHdmiPort) > 65))
			{
...
#else
				if (_bDviMdChgCnt++ > _wDVI_WAIT_STABLE_COUNT || fgIsHDCPStable)
#endif
                {
...
#ifdef CC_SUPPORT_HDMI_DYNAMIC_WAIT_STABLE				
			Printf("+++ mdchg: %d _bAVIStableCnt %d, WSTBVNT %d, Vstbcnt: %d, VSTBCNT %d, Hstbcnt: %d, HSTBCNT %d\n", \
...
#if ENABLE_ALL_DVI_LOG					
					 LOG(0, "mc:wait stable to timing search \n");
#endif									
                        //vRegWrite4B(INTR_STATE0 + u4ActiveHdmiRegBase, Fld2Msk32(INTR3_NEW_UNREC));
                        vDviInitial();
...
#if ENABLE_ALL_DVI_LOG					
				LOG(0, "_bDviMdChgCnt =%d  H/V Clk Chg, V:%d H:%d \n", _bDviMdChgCnt, wvtemp, whtemp);
                LOG(0, "pre H/V Clk Chg, V:%d H:%d\n", _wDviVTotal, _wDviHtotal);
#endif								
#endif
            }
            else
            {
      		  ...
      		  bHdmiPhyReset(eActiveHdmiPort,HDMI_RST_RTCK);
                bHdmiPhyReset(eActiveHdmiPort,HDMI_RST_DEEPCOLOR);
                //}
