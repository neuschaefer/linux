...
extern const UINT16 CODE DEMO_MODE_BEFORE_SCALER_ARG[][DEMO_MODE_BEFORE_SCALER_END];
extern const UINT16 CODE DEMO_MODE_AFTER_SCALER_ARG[][DEMO_MODE_AFTER_SCALER_END];
...
extern const UINT8 CODE GAMMA_256_BRIGHT[3][257];
extern const UINT8 CODE GAMMA_256_MIDDLE[3][257];
extern const UINT8 CODE GAMMA_256_DARK[3][257];    
...
void vInitDrvPQCustomization(void)
{
    ...
    x_memcpy(GAMMA_256_BRIGHT   ,ADDR_GAMMAE_BRIGHT , SIZE_OF_PER_GAMMA);
    x_memcpy(GAMMA_256_MIDDLE   ,ADDR_GAMMAE_MIDDLE , SIZE_OF_PER_GAMMA);
    x_memcpy(GAMMA_256_DARK     ,ADDR_GAMMAE_DARK   , SIZE_OF_PER_GAMMA);    
}
...
UINT16 wReadDemoModeBeforeScalerArg(UINT8 bTiming,UINT8 bEngine)
{
    return DEMO_MODE_BEFORE_SCALER_ARG[bTiming][bEngine);
}

UINT16 wReadDemoModeAfterScalerArg(UINT8 bTiming,UINT8 bEngine)
{
    return DEMO_MODE_AFTER_SCALER_ARG[bTiming][bEngine);
}
