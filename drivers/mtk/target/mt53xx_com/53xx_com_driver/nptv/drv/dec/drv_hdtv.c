...
#if STORE_POS_CLK_PHASE
extern UINT8 _bOldVgaTiming;
extern UINT16 _wVgaClockOffset;
extern UINT16 _wVgaHPorchOffset;
...
{
#if STORE_POS_CLK_PHASE
    if (wstart != _wVgaHPorchOffset)
    {
        _wVgaHPorchOffset = wstart;
    }
#endif
...
void vHdtvSetInputCapature(UINT8 bmode, UINT8 bIsHdtv) 	//set input start and length
{
    ...
    {
        #if STORE_POS_CLK_PHASE
        if (fgIsVgaTiming(_bVgaTiming))
        {
        	vHdtvSetInput(_wVgaHPorchOffset, Get_VGAMODE_IPH_WID(bmode)); // for the bug: the right side of the VGA picture has the pink garbage
        }
        else
        #endif
        {
    ...
    {
        /* Component 1080i/p on the 1080p panel has the abnormal line at the right side. */
        vHdtvSetInput(Get_VGAMODE_IPH_BP(bmode), Get_VGAMODE_IPH_WID(bmode));
    }
    ...
    if (fgIsMainVga() || fgIsPipVga())
    {
        if (_bOldVgaTiming == _bVgaTiming)
        {
            vDrvAsyncVsyncStart(_wVgaVPorchOffsetS);
        }
        else
        {
            _wVgaVPorchOffsetS =  VS1;
...
#if 0
    //Set Serration-Period Integration VSync Threshold
    vASCSSeparatorThre();
    // Refine 1080i Setting
    if ((bmode==MODE_1080i)||(bmode==MODE_1080i_50))
    {
        //set Schmitt trigger L thre and upper bound
        vRegWriteFldAlign(ASYNC_01,(CS_SEPARATOR_THU_DEFAULT_1080>> 8),CSYNC_CONT_THU_9_8);
        vRegWriteFldAlign(ASYNC_01,(CS_SEPARATOR_THL_DEFAULT_1080>> 8),CSYNC_CONT_THL_9_8);
        vRegWriteFldAlign(ASYNC_01,(UINT8) CS_SEPARATOR_THU_DEFAULT_1080,CSYNC_CONT_THU_7_0);
        vRegWriteFldAlign(ASYNC_01,(UINT8) CS_SEPARATOR_THL_DEFAULT_1080,CSYNC_CONT_THL_7_0);
    }
    //-----------------------------------
#endif

#if STORE_POS_CLK_PHASE
    bPrevPhaseVal = bDrvVGAGetPhase();
#endif
...
#if STORE_POS_CLK_PHASE
    if(_bVgaTiming == _bOldVgaTiming)
    {
        LOG(3, "Set Previous Phase %d\n", bPrevPhaseVal);
        vDrvVGASetPhase(bPrevPhaseVal);
    }
#endif
...
void vHdtvHwInit (void)
{
  ...
  //for decomp2 method
    ...
    // Enable hsync de-glitch
    //vRegWriteFldAlign(ASYNC_00, 1, AS_HSYNC_DGLITCH_SEL);
...
#if SUPPORT_VIDEO_MASK
    vRegWriteFldAlign(HDTV_00, 1, HDTV_AVMASK_EN);
    vRegWriteFldAlign(HDTV_00, 1, HDTV_AVMASK_HEDGE);
#endif
#if 0
    //hlock end/start 3 -> 16 for 5382p scart rgb
    {
        vRegWriteFldAlign(ASYNC_10, 0, AS_HLOCK_SELF_END_12 );
        vRegWriteFldAlign(ASYNC_10, 3, AS_HLOCK_SELF_END_11_0 );        
        vRegWriteFldAlign(ASYNC_10, 0, AS_HLOCK_SELF_ST_12 );
        vRegWriteFldAlign(ASYNC_10, 0, AS_HLOCK_SELF_ST_11_0 );                
    }
#endif    
    ...
    vRegWriteFldAlign(ASYNC_1C, 1, AS_AUTO_HV_SKEW_PIX_EN);    
#if SUPPORT_HDTV_HARDWARE_MUTE
    ...
    {
     vPGA_RESISTOR_BUFFER_COM(SEPERATESYNC);  
    }
#endif    
...
PRIVATE void vHdtvInitial(UINT8 bReason)
{
    ...
    vDrvADCPLLMinPower(FALSE);	////power up adc pll    //ADPART
...
#ifdef SCART_CLK_SOURCE_CVBS
    vIO32WriteFldAlign(CKGEN_AFECFG7, 0x0, FLD_RG_ADCPLL_H_CVBS_SEL);
    vRegWriteFldAlign(SCART_02, 0x0, FLD_CVBS_H_SEL);    
#endif    
...
#endif        
        _IsHdtvDetectDone = TRUE;
        vDrvADCPLLMinPower(TRUE);	//This is ADC PLL power down
...
void vHdtvChkModeChange(void)
{
    ...
    if (fgIsMainYPbPr() || fgIsPipYPbPr())
    {
        ...
        {
         if(bHdtvMCErrorCnt<10)
         {
          vAS2SyncMeasure(SP2_Specific_LVL);
         }
         else if (bHdtvMCErrorCnt>12)
         {
            ...
            case HDTV_CHK_MODECHG:
                ...
                if (fgIsVideoTiming(_bHdtvTiming))
                {
                    ...
                    if (!fgIsCLKLock())
                    {
                        ...
                        {
    #if VGAADCPLL_WORKAROUND
                            if( _bPreVgaTiming == _bHdtvTiming)
                                _bADCPLLDetectCnt++;

                            _bPreVgaTiming = _bHdtvTiming;
                            
                            if(_bADCPLLDetectCnt >= 2)
                            {
                                _ApplyVgaADCPLLWorkAround = TRUE;
                            }
                            else
                            {
                                _ApplyVgaADCPLLWorkAround = FALSE;
                    ...
                    {
                     _bUnLockCnt++;                                        
                     DBG_Printf(DBG_MDET," [SA7] _bUnLockCnt %d vDDS_MAX_PERR %d\n", _bUnLockCnt,wDDS_MAX_PERR_temp);
                    
                      if ((_bUnLockCnt>15) && (!fgIsAutoFlgSet(SP0_AUTO_ALL)))
                      {
                       DBG_Printf(DBG_MDET,"[SA7] Lock but Error to large : vDDS_MAX_PERR %d\n", wDDS_MAX_PERR_temp);
                       bModChg = MCHG_UNLOCK ;
                      }
                    }
#endif                    
                 ...
                 {  
                   ...
                   )
                   {
                       if((_bSetDefaultSlicer_Cnt==_bSetDefaultSlicer_Cnt_th))
                       {
                        ...
                        #if ADAPTIVE_MONITOR_SLICER_MEASURE_DEBUG
                         DBG_Printf(DBG_MCHG,"_bSetDefaultSlicer_loop_Cnt = %d \n",_bSetDefaultSlicer_loop_Cnt);
                        #endif
                        if (_bSetDefaultSlicer_loop_Cnt>=2)
                        {
                         #if ADAPTIVE_MONITOR_SLICER_MEASURE_DEBUG
                         DBG_Printf(DBG_MCHG,"[SA7][HDTV_CHK_MODECHG] vSetDefaultSlicer\n");                                            
                         DBG_Printf(DBG_MCHG,"bReadNewSlicer = 0x%x,bReadMONSlicer = 0x%x \n",bReadNewSlicer(),bReadMONSlicer());
                         DBG_Printf(DBG_MCHG,"SP2_H_LEN_S = %d,whtemp = %d \n",wSP2VCompare[SP2_Specific_LVL].SP2_H_LEN_S,whtemp);        
                         DBG_Printf(DBG_MCHG,"SP2_V_LEN_S = %d,wvtemp = %d \n",wSP2VCompare[SP2_Specific_LVL].SP2_V_LEN_S,wvtemp); 
                         DBG_Printf(DBG_MCHG,"SP2_V_WIDTH_S = %d,wVwidtemp = %d \n",wSP2VCompare[SP2_Specific_LVL].SP2_V_WIDTH_S,wVwidtemp);                                                         
                         DBG_Printf(DBG_MCHG,"[1].SP2_V_LEN_S = %d  \n",wSP2VCompare[SP2_Default_LVL].SP2_V_LEN_S);
                         #endif
                         vSetDefaultSlicer();                                                                                 
                         _bSetDefaultSlicer_loop_Cnt = 0;
                        } 
                        ...
                        if (bSP0HVCheck())
                        {
                            ...
                            _IsHdtvDetectDone = FALSE;
                             ...
                             {                             
                              ...
                              DBG_Printf(DBG_MCHG,"SP2_H_LEN_S = %d,whtemp = %d \n",wSP2VCompare[SP2_Specific_LVL].SP2_H_LEN_S,whtemp);        
                              DBG_Printf(DBG_MCHG,"SP2_V_LEN_S = %d,wvtemp = %d \n",wSP2VCompare[SP2_Specific_LVL].SP2_V_LEN_S,wvtemp); 
                              DBG_Printf(DBG_MCHG,"SP2_V_LEN_S = %d,wVwidtemp = %d \n",wSP2VCompare[SP2_Specific_LVL].SP2_V_WIDTH_S,wVwidtemp);                                                         
                              #if ADAPTIVE_MONITOR_SLICER_MEASURE_ONLINE
                              DBG_Printf(DBG_MCHG,"[1].SP2_V_LEN_S = %d \n",wSP2VCompare[SP2_Default_LVL].SP2_V_LEN_S);                                                                                     
                              #endif
                              if(((wSP2VCompare[SP2_Specific_LVL].SP2_H_LEN_S<=(whtemp+1))&&(wSP2VCompare[SP2_Specific_LVL].SP2_H_LEN_S>=(whtemp-1)))
                              ...
                              &&(_bCurSlicerIdx>=1))                            
                              {
                               DBG_Printf(DBG_MCHG,"[SA7]vSetDefaultSlicer\n");
                               vSetDefaultSlicer(); 
                               vResetSliceTimer();                                                                                  
                              }
                             }        
...
void vDrvScartRGBHdtvInit(void)
{
    ...
    vDrvADCPLLMinPower(FALSE);            ////power up adc pll
    ...
    //vHdtvRGBFormatOn(SV_ON);//RGB or YCbCr color space in HDTV : see above vRegWriteFldAlign(HDTV_00, 0x8 ,HDTV_00_6_0);
...
UINT16 wHdtvGetPorch(UINT8 bPath,UINT8 bPorchType)
{
    switch (bPorchType)
    {
        ...
        case 	SV_HPORCH_CURRENT:
            ...
            return (((RegReadFldAlign(HDTV_01, HDTV_AV_START) - _bPhaseHdelta)>>(RegReadFldAlign(HDTV_00,HDTV_CEN_SEL)))>>1);        
            else
            #endif
            return ((RegReadFldAlign(HDTV_01, HDTV_AV_START) - _bPhaseHdelta)>>(RegReadFldAlign(HDTV_00,HDTV_CEN_SEL)));        
        
        case 	SV_HPORCH_DEFAULT:
            #if (!(YPBPR_480IP_27MHZ))        
            if ((_bHdtvTiming == MODE_525I_OVERSAMPLE)||(_bHdtvTiming == MODE_625I_OVERSAMPLE))        
            return ((Get_VGAMODE_IPH_BP(_bHdtvTiming)>>(RegReadFldAlign(HDTV_00,HDTV_CEN_SEL)))>>1);
            else
            #endif
            return (Get_VGAMODE_IPH_BP(_bHdtvTiming)>>(RegReadFldAlign(HDTV_00,HDTV_CEN_SEL)));        
