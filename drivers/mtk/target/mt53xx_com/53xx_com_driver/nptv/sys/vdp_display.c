...
//static UINT32 _u4NptvScalerPixelBased = 0;

//extern BOOL _fgFRS_ENA;
//extern UINT32 _u4LBoxDetectionInit;
//extern UINT32 _u4LBoxDetectionEnable;
//extern UINT32 _u4LBoxDetectionFixed;
//extern UINT32 _u4LBoxDetectionValid ;
//extern UINT32 _u4LBoxSourceChange;
//extern UINT32 _u4LBoxSrcWidth;
//extern UINT32 _u4LBoxSrcHeight;
//extern UINT8 _u1LBoxSrcInterrupt;
//extern UINT32 _u4LBoxPictureFormatChange;
//extern UINT8 _NRchange;

//extern void LboxDetectionUnlock(void);
static UINT8 CalculateOutputRegion(UINT8 u1Path, const VDP_DISP_REGION_T* prIn, VDP_DISP_REGION_T* prOut);
...
#if 0 // 5387  move to drv_lbox.c
void vApiVideoLbox(UINT8 bPath, UINT8 bOnOff, UINT8 bFixed) 
{
    UNUSED(bPath);
...
static UINT8 CalculateOutputRegion(UINT8 u1Path, const VDP_DISP_REGION_T* prIn, VDP_DISP_REGION_T* prOut)
{
    ...
    #else
    prOut->u4X = (TV_WIDTH * prIn->u4X + (VDP_MAX_REGION_WIDTH >> 1)) / VDP_MAX_REGION_WIDTH;
    prOut->u4Y = (TV_HEIGHT * prIn->u4Y + (VDP_MAX_REGION_HEIGHT >> 1)) / VDP_MAX_REGION_HEIGHT;
    prOut->u4Width = (TV_WIDTH * prIn->u4Width + (VDP_MAX_REGION_WIDTH >> 1)) / VDP_MAX_REGION_WIDTH;
    prOut->u4Height = (TV_HEIGHT * prIn->u4Height + (VDP_MAX_REGION_HEIGHT >> 1)) / VDP_MAX_REGION_HEIGHT;    
    #endif
...
//    UINT32 u4IgnoreOverscanV;
    ...
    u4LBoxVValid = 0;
    u4LBoxHValid = 0;
    bMode = getScalerMode(u1Path);	 

	
#if 0 //5387
    // Calculate Source Size After Kill Letter Box
    u4LBoxY = 0;
    #if SUPPORT_HORIZONTAL_LBOX    
    u4LBoxX = 0;
    #endif
    ...
    if (u1Path == SV_VP_MAIN)
    {
        ...
        if ((_u4LBoxDetectionEnable != 0) && (_u4LBoxDetectionValid != 0) && (!bIsScalerInput444(SV_VP_MAIN)))   // 444 can not use letterbox detection (detect G only!)
        {
                ...
                )
            {
                ...
                if ((u4Height - u4NewHeight) > LETTER_BOX_PROTECT_BOUND)
                {
                    // Handle Low Protect
    ...
    //if (fgApiVideoIsVgaMode(u1Path,0) != 0)
...
//#else    
    ...
    #else
    prOut->u4X = (rOverScaned.u4Width * prIn->u4X + (VDP_MAX_REGION_WIDTH >> 1)) / VDP_MAX_REGION_WIDTH;
    prOut->u4Y = (rOverScaned.u4Height * prIn->u4Y + (VDP_MAX_REGION_HEIGHT >> 1)) / VDP_MAX_REGION_HEIGHT;
    prOut->u4Width = (rOverScaned.u4Width * prIn->u4Width + (VDP_MAX_REGION_WIDTH >> 1)) / VDP_MAX_REGION_WIDTH;
    prOut->u4Height = (rOverScaned.u4Height * prIn->u4Height + (VDP_MAX_REGION_HEIGHT >> 1)) / VDP_MAX_REGION_HEIGHT;
    #endif
 ...
 * @return void
 */
void vApiSetScalerPixelBased(UINT32 u4OnOff)
{
    _u4NptvScalerPixelBased = u4OnOff;
}
#endif
...
void vApiVideoInputChangeNotify(UINT8 bPath) 
{
    ...
    // Get Source Size
    //u4SrcWidth = wDrvVideoInputWidth(bPath);
    u4SrcWidth = wApiVideoGetInputWidth(bPath);
    u4SrcHeight = wDrvVideoInputHeight(bPath);

    if (bPath == SV_VP_MAIN)
    {
        UINT8 u1LBoxSrcInterrupt;

        u1LBoxSrcInterrupt = bDrvVideoIsSrcInterlace(bPath);

        LboxSourceChange(wDrvVideoGetPreScaleWidth(bPath), (u1LBoxSrcInterrupt? (u4SrcHeight>>1): u4SrcHeight), u1LBoxSrcInterrupt);
/*
    ...
    rVdpSrcInfo.u4Interlace= (UINT32)bDrvVideoIsSrcInterlace(bPath);

    if (VDP_SourceChangeNotify(bPath, rVdpSrcInfo))    
    {
    }
    else
    {
...
//    UINT16 u2VPorch;
  ...
  //  VDP_DISP_REGION_T rOutRegion;
    ...
    if (bPath == SV_VP_MAIN) 
    {
        prPicInfo = &_rMPicInfo;
        prPicInfo->rSrcRegion = rInRegion;
 //       if (!(_u4NptvScalerPixelBased||fgIsMainDec()))
    ...
    else 
    {
        prPicInfo = &_rPPicInfo;
        prPicInfo->rSrcRegion = rInRegion;
//        if (!(_u4NptvScalerPixelBased||fgIsPipDec()))
        if (!(VDP_GetScalerPixelBased() ||fgIsPipDec()) ) 
        {
         ...
         // 5387 ToDo
         // seperate following code to another API to update pixel-based source region info
...
//    if (_u4NptvScalerPixelBased == 0)
    if (VDP_GetScalerPixelBased() == 0)
    {
        ...
        if (((wXOff + wWidth) > VDP_MAX_REGION_WIDTH) || ((wYOff + wHeight) > VDP_MAX_REGION_HEIGHT))
        {
            return (SV_FAIL);
        }

        if (CalculateInputRegion(bPath, &(prPicInfo->rOverScan), &rInRegion, &rOutRegion) != SV_SUCCESS)
        {
            return (SV_FAIL);
        }
...
//    if (_u4NptvScalerPixelBased == 0)
    if (VDP_GetScalerPixelBased() == 0)
    {
        if (((wXOff + wWidth) > prPicInfo->wSrcW) || ((wYOff + wHeight) > prPicInfo->wSrcH)) 
        {
                return (SV_FAIL);
        }
    }
    else
    {
        ...
        {    
            // William, Pixel Based Protection
            if (wXOff > prPicInfo->wSrcW)
            {
                wXOff = 0;
            }
    
            if ((wXOff + wWidth) > prPicInfo->wSrcW)
            {
                wWidth = (prPicInfo->wSrcW - wXOff);
            }
        
            if (wYOff > prPicInfo->wSrcH)
            {
                wYOff = 0;
            }
    
            if ((wYOff + wHeight) > prPicInfo->wSrcH)
            {
                wHeight = (prPicInfo->wSrcH - wYOff);
            }
        }
    }
    ...
    if ((prPicInfo->wTargetW == wWidth) &&
        (prPicInfo->wTargetH == wHeight) &&
        (prPicInfo->wXOffset == wXOff) &&
        (prPicInfo->wYOffset == wYOff) &&
        (prPicInfo->u1Interlace == u1Interlace) &&
        (prPicInfo->wHTotal == u2HTotal) &&
        (prPicInfo->wHTotal == u2VTotal) &&
        (prPicInfo->wVPorch == u2VPorch))
 ...
 * @retval void
 */
void vApiVideoSetDispMask(UINT8 bPath, UINT8 bOnOff) 
{
    vDrvVideoSetDispMask(bPath, bOnOff);
}

/**
 * @brief vApiVideoFreeze
 ...
 * @retval void
 */
void vApiVideoFreeze(UINT8 bPath, UINT8 bOnOff) 
{
 ...
 * @retval void
 */
void vApiVideoForceBlueMute(UINT8 bPath, UINT8 bOnOff) 
{
    if (bPath == SV_VP_MAIN) 
    {
        _fgMainBlueMute = bOnOff;

#ifdef CC_SCPOS_MUTE_MASK

        vScposSetMuteMask(bPath, _fgMainBlueMute, CC_MUTE_MASK_BLUE);

#else

        vScposSetMute(bPath, (_fgMainBlueMute | _fgMainBlackMute));
    ...
    {
        _fgSubBlueMute = bOnOff;

#ifdef CC_SCPOS_MUTE_MASK

        vScposSetMuteMask(bPath, _fgSubBlueMute, CC_MUTE_MASK_BLUE);

#else

        vScposSetMute(bPath, (_fgSubBlueMute | _fgSubBlackMute));
 ...
 * @retval void
 */
void vApiVideoForceBlackMute(UINT8 bPath, UINT8 bOnOff) 
{
    if (bPath == SV_VP_MAIN) 
    {
        _fgMainBlackMute = bOnOff;

#ifdef CC_SCPOS_MUTE_MASK

        vScposSetMuteMask(bPath, _fgMainBlackMute, CC_MUTE_MASK_BLACK);
    ...
    {
        _fgSubBlackMute = bOnOff;

#ifdef CC_SCPOS_MUTE_MASK

        vScposSetMuteMask(bPath, _fgSubBlackMute, CC_MUTE_MASK_BLACK);
 ...
 * @retval void
 */
void vApiVideoForceMute(UINT8 bPath, UINT8 bOnOff) 
{
#ifdef CC_SCPOS_MUTE_MASK

    vScposSetMuteMask(bPath, bOnOff, CC_MUTE_MASK_FORCE);

#else

    vScposSetForceMute(bPath, bOnOff);

#endif
}

/**
 * @brief vApiVideoSetBg
 ...
 * @retval void
 */
void vApiVideoSetBg(UINT8 bPath, UINT32 u4Bg) 
{
    ...
    if (prDispInfo->u4BgColor != u4Bg)
    {
        prDispInfo->u4BgColor = u4Bg;
        if (bPath == SV_VP_MAIN)
        {
            vSetScposFlg(MAIN_BGCOLOR_CHG);
        }
        else
        {
            vSetScposFlg(PIP_BGCOLOR_CHG);
        }
    }
}
