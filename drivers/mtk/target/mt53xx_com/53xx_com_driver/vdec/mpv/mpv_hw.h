...
#define VLD_READ32(id, offset)    \
    if (id == 0) {   \
        VLD0_READ32(offset);   }  \
    else { \
        VLD1_READ32(offset);}        

#define VLD_WRITE32(id, offset, value)    \
    if (id == 0) {   \
        VLD0_WRITE32(offset, value);   }  \
    else { \
        VLD1_WRITE32(offset, value);}        

#define MC_READ32(id, offset)    \
    ((id == 0) ? \
        (MC0_READ32(offset))\
    :\
        (MC1_READ32(offset)))
    
#define MC_WRITE32(id, offset, value)    \
    if (id == 0) {   \
...
EXTERN void _MPV_IsrInit(void);
EXTERN void _MPV_VldHWReset(UCHAR ucMpvId);
...
EXTERN void _MPV_McSetFrmBufPtr(UCHAR ucMpvId, UCHAR ucBufIdx, UINT32 u4PtrY, UINT32 u4PtrC);
...
EXTERN void _MPV_SetMcDbkBufAddr(UCHAR ucMpvId, UCHAR ucFbgId, UCHAR ucDbkFbId, 
...
EXTERN BOOL _MPV_ResetSearch(UCHAR ucMpvId, UINT32 u4FifoStart, UINT32 u4FifoEnd, 
