...
#else
#define MUSB_LOADER_MAX_WAIT_TIME        (700) // ms.
#endif
...
static const QUERY_TYPE_T _i4MUSBVbusGpioDef[MUSB_MAX_CONTROLLERS] =
{
    eUSBVbus0Gpio
};
static const QUERY_TYPE_T _i4MUSBVbusEnablePolarityDef[MUSB_MAX_CONTROLLERS] =
{
                ...
                {               
                         ...
                         (_fgMUSBVbusStatus[i] == (BOOL)TRUE))
                    {
                        ...
                        if (GPIO_Output((INT32)u4VbusGpio, (INT32 *)&u4VbusGpioPolarity) 
                            != (INT32)u4VbusGpioPolarity)
                        {
                            Printf("USB%d: Set Vbus GPIO%d error.\n", i, u4VbusGpio);
                        }                        
                    }              
                    ...
                    Printf("USB%d GPIO%d Over current %s !\n", i, 
...
INT32 MUSB_CheckSLT(void)
{
    ...
    */
    ...
    for (i=0; i<(INT32)MUSB_CountPorts(); i++)
    {
        ...
        {
            Printf("USB%d slt fail.\n", i);
            i4Pass = -1;
        }
        else
        {
            Printf("USB%d slt pass.\n", i);
        }
    }
