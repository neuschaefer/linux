...
//#endif
    ...
    #if CC_BOOT_MUTE_LOG
    UINT8 bEnableLog = bApiEepromReadByte(EEP_BOOT_MUTE_LOG);
    if(bEnableLog == 0xaa)
    {
        INT32 i4Log;
...
UINT8 _bCurrentInputMSFor3DPR = SV_VS_ATV1;
#endif 
...
//        vDrvScpipWriteCtrl(SV_VP_PIP,SV_OFF);
    ...
    if (fgSubCombi)
    {
        vDrvSetInternalMux(1,_bMonSubNew);
    }
#endif
