...
{
    if (DRVCUST_PanelGet(ePanelBacklightPwmPort) == 0) //pwm0
    {     
        vRegWriteFldAlign(OUTSTG_OS_D8, u4Start*u2HTotal, OS_D8_REG_PWM_SCAN1_START);
        vRegWriteFldAlign(OUTSTG_OS_D9, u4High*u2HTotal, OS_D9_REG_PWM_SCAN1_HIGH);
        vRegWriteFldAlign(OUTSTG_OS_DA, (u2VTotal-u4Start-u4High)*u2HTotal, OS_DA_REG_PWM_SCAN1_LOW);
    }
    else //pwm1
    {
        vRegWriteFldAlign(OUTSTG_OS_DB, u4Start*u2HTotal, OS_DB_REG_PWM_SCAN2_START);
        vRegWriteFldAlign(OUTSTG_OS_DC, u4High*u2HTotal, OS_DC_REG_PWM_SCAN2_HIGH);
        vRegWriteFldAlign(OUTSTG_OS_DD, (u2VTotal-u4Start-u4High)*u2HTotal, OS_DD_REG_PWM_SCAN2_LOW);     
     }
}
#endif
...
void BACKLT_SetDimming(UINT32 u4Frequency, UINT32 u4Dimming)
{
    ...
    else
    {
        ...
        // set pwm2: control glasses, duty cycle: 50% for CMO glasses
#ifdef CC_SCPOS_3DTV_SUPPORT
        if (IS_LVDS_DISP_3D_SHUTTER)
        {
            #if 0
            vDrvSetPWM(SrcPWM2, (BSP_GetDomainClock(SRC_BUS_CLK) >> 8) / 120, 128, PWM_DUTY_ALL);
            #endif
            if(bOUTPUT_REFRESH_RATE !=0)
            {
                vDrvSetPWM(SrcPWM2, (BSP_GetDomainClock(SRC_BUS_CLK) >> 8) / bOUTPUT_REFRESH_RATE, 128, PWM_DUTY_ALL);
                bInitPWM2 = TRUE;
            }
            else
            {
                LOG(0,"Output Refresh rate is 0\n");
            }
...
void vApiPanelPowerSequence(BOOL fgEnable)
{
    ...
    if (fgEnable == PANEL_ON) //panel power on sequence
    {
        ...
        if (DRVCUST_PanelGet(eFlagBacklightDimmingLock))
        {
            ...
            if (IS_LVDS_DISP_3D_SHUTTER)
            {
                vDrvSetLock(SrcPWM2, SV_ON); // lock pwm2 for CMO glasses
            }
#endif
        }
        else
