...
{     
    ...
    vRegWriteFldAlign(VFE_E0, 0, RG_CTCLAMP_CTMA);//
    vRegWriteFldAlign(VFE_E0, 0, RG_CTCLAMP_CTMB);//
    vRegWriteFldAlign(VFE_E0, 0, RG_CLAMP_SOURCE_ENA);//
    vRegWriteFldAlign(VFE_E0, 0, RG_CLAMP_SOURCE_ENB);//
    vRegWriteFldAlign(VFE_E0, 0, RG_CLAMP_SINK_ENA);//
    vRegWriteFldAlign(VFE_E0, 0, RG_CLAMP_SINK_ENB);//
    vRegWriteFldAlign(VFE_E0, 0x1f, RG_CLAMP_SOURCEA);//
    vRegWriteFldAlign(VFE_E0, 9, RG_CLAMP_SOURCEB);//
    vRegWriteFldAlign(VFE_E1, 5, RG_CLAMP_SINKA);//
    vRegWriteFldAlign(VFE_E1, 5, RG_CLAMP_SINKB);//
    vRegWriteFldAlign(VFE_E1, 1, RG_UPDNA);//
    vRegWriteFldAlign(VFE_E1, 1, RG_UPDNB);//
    ///////////vRegWriteFldAlign(VFE_E1, 0, RG_CALCLAMPA); CY
    ///////////vRegWriteFldAlign(VFE_E1, 0, RG_CALCLAMPB); CY
    vRegWriteFldAlign(VFE_E1, 1, RG_VAGSELA);//
    vRegWriteFldAlign(VFE_E1, 1, RG_VAGSELB);//
    vRegWriteFldAlign(VFE_E1, 0, RG_CALPGAA);//
    vRegWriteFldAlign(VFE_E1, 0, RG_CALPGAB);//
    vRegWriteFldAlign(VFE_E1, 0, RG_C_SELA);//
    vRegWriteFldAlign(VFE_E1, 0, RG_C_SELB);//
    vRegWriteFldAlign(VFE_E1, 0, RG_HPCLAMPA);//
    vRegWriteFldAlign(VFE_E1, 0, RG_HPCLAMPB);//
    vRegWriteFldAlign(VFE_E1, 0, RG_CLAMPC_TSTA);//
    vRegWriteFldAlign(VFE_E1, 0, RG_CLAMPC_TSTB);//
    ...
    vRegWriteFldAlign(VFE_E2, 1, RG_SHIFT2A_PWD);//
    vRegWriteFldAlign(VFE_E2, 1, RG_SHIFT2B_PWD);//
    vRegWriteFldAlign(VFE_E2, 0, RG_BP_PGABUFA);//
    vRegWriteFldAlign(VFE_E2, 0, RG_BP_PGABUFB);//

    vRegWriteFldAlign(VFE_E3, 3, RG_AAF_BWA);//
    vRegWriteFldAlign(VFE_E2, 3, RG_AAF_BWB);//
    vRegWriteFldAlign(VFE_E2, 0, RG_RS_AAFA);//
    vRegWriteFldAlign(VFE_E2, 0, RG_RS_AAFB);//
    //vRegWriteFldAlign(VFE_E3, 1, RG_VCOM_SEL_PGAA); CY
    //vRegWriteFldAlign(VFE_E3, 1, RG_VCOM_SEL_PGAB); CY
    //vRegWriteFldAlign(VFE_E3, 1, RG_VCOM_SEL_FLTA); CY
    //vRegWriteFldAlign(VFE_E3, 1, RG_VCOM_SEL_FLTB); CY
    vRegWriteFldAlign(VFE_E2, 0, RG_CTINA);//
    vRegWriteFldAlign(VFE_E2, 0, RG_CTINB);//

    vRegWriteFldAlign(VFE_E3, 0x20, RG_AISEL);//
    vRegWriteFldAlign(VFE_E4, 0, RG_FE_SELA);//
    vRegWriteFldAlign(VFE_E4, 0, RG_FE_SELB);//
    vRegWriteFldAlign(VFE_E3, 0, RG_AISELR);//

    vRegWriteFldAlign(VFE_E5, 1, RG_MIX_10U);//
    vRegWriteFldAlign(VFE_E4, 1, RG_MIX_60U_EN);//
    //vRegWriteFldAlign(VFE_E5, 0, RG_MIX0_BEN); CY
    //vRegWriteFldAlign(VFE_E5, 0, RG_MIX1_BEN); CY
    vRegWriteFldAlign(VFE_E4, 0, RG_MIX_SHORT);//
    vRegWriteFldAlign(VFE_E4, 2, RG_MIXVOCM_SEL);//
    vRegWriteFldAlign(VFE_E4, 0, RG_BUF_MONEN);//
    vRegWriteFldAlign(VFE_E4, 0, RG_VCM_MONEN);//
    vRegWriteFldAlign(VFE_E4, 0, RG_BIAS_PWD);//
    vRegWriteFldAlign(VFE_E4, 1, RG_40U_PGAOFFA);//
    vRegWriteFldAlign(VFE_E4, 1, RG_40U_PGAOFFB);//
    vRegWriteFldAlign(VFE_E4, 1, RG_30U_BUFA);//
    vRegWriteFldAlign(VFE_E4, 1, RG_30U_BUFB);//
    vRegWriteFldAlign(VFE_E4, 1, RG_20U_CLAMPA);//
    vRegWriteFldAlign(VFE_E4, 1, RG_20U_CLAMPB);//

    vRegWriteFldAlign(VFE_E5, 1, RG_20U_PGAOFFA);//
    vRegWriteFldAlign(VFE_E5, 1, RG_20U_PGAOFFB);//
    vRegWriteFldAlign(VFE_E5, 1, RG_20U_MIXOFF);//
    vRegWriteFldAlign(VFE_E5, 0x0f, RG_SIFGAINA);//
    vRegWriteFldAlign(VFE_E5, 0x0f, RG_SIFGAINB);//
    vRegWriteFldAlign(VFE_E5, 0, RG_SIF_ENA);//
    vRegWriteFldAlign(VFE_E5, 0, RG_SIF_ENB);//
    //vRegWriteFldAlign(VFE_E6, 0, RG_CVBS_TESTNI_EN); CY
    //vRegWriteFldAlign(VFE_E6, 0, RG_CVBS_TESTN_EN);  CY
    //vRegWriteFldAlign(VFE_E6, 0, RG_CVBS_TESTPI_EN); CY
    //vRegWriteFldAlign(VFE_E6, 0, RG_CVBS_TESTP_EN);  CY
    //vRegWriteFldAlign(VFE_E6, 1, RG_CVBS_TESTBUF_PWD); CY

    //vRegWriteFldAlign(VFE_E7, 0, RG_TQI_MON); CY
    vRegWriteFldAlign(VFE_E7, 0, RG_CINRA_EN);//
    vRegWriteFldAlign(VFE_E7, 0, RG_CINRB_EN);//
    //vRegWriteFldAlign(VFE_E7, 0, RG_CVBS2SYNCA_EN); CY
    //vRegWriteFldAlign(VFE_E7, 0, RG_CVBS2SYNCB_EN); CY
    //vRegWriteFldAlign(VFE_E7, 0, RG_BYPASS0_EN); CY
    //vRegWriteFldAlign(VFE_E7, 0, RG_BYPASS1_EN); CY
    vRegWriteFldAlign(VFE_E7, 0x61, RG_CVBS_REV);//
    //vRegWriteFldAlign(VFE_E7, 0, RG_ADC_REV); CY

    vRegWriteFldAlign(VFE_E6, 0, RG_CVBSADC_SEL_CKPLL);// CY 1->0
    vRegWriteFldAlign(VFE_E6, 0, RG_CVBSADC_CKCTRL);//
    vRegWriteFldAlign(VFE_E0, 0, RG_BG_PD);//
    vRegWriteFldAlign(VFE_E6, 0, RG_CVBSADC_VRSEL1); //
    ...
    //vRegWriteFldAlign(VFE_E5, 0x01, RG_MIX0_BEN); CY  // Command Mode = 1/3 * Vdd = 1V
    //vRegWriteFldAlign(VFE_E5, 0x01, RG_MIX1_BEN); CY  // Command Mode = 1/3 * Vdd = 1V
    //CVBS CLK Select
...
void vDrvAvByp(void)
{
    ...
    vRegWriteFldMulti(VFE_E4, P_Fld(1,RG_MIX_60U_EN)|P_Fld(0,RG_MIX_SHORT));//
...
/*
    ...
    vRegWriteFldAlign(VFE_E4, 0, RG_MIX_SHORT);//   //no short because  S-Video= A+B
    vRegWriteFldMulti(VFE_E5, P_Fld(1,RG_MIX_10U));//
    vRegWriteFldMulti(VFE_E4, P_Fld(1,RG_MIX_60U_EN)|P_Fld(0,RG_MIX_SHORT));//
*/
}
...
void vDrvSetTVADC(UINT8 vsrc)
{
	...
	switch (VSS_MAJOR(vsrc))
	{
		...
		case VSS_ATV:
            ...
            vRegWriteFldAlign(VFE_E0, 0x20, RG_CLAMP_SOURCEB);//
            vRegWriteFldAlign(VFE_E0, 1, RG_CLAMP_SINK_ENB);//
			vRegWriteFldAlign(VFE_E0, 1, RG_CLAMP_SOURCE_ENB);//
			//vRegWriteFldAlign(VFE_E0, 0, RG_CALCTB); CY
			vRegWriteFldAlign(VFE_E1, 0, RG_CALPGAB);//
			vRegWriteFldAlign(VFE_E1, 0, RG_C_SELB);//
			vRegWriteFldAlign(VFE_E1, 1, RG_VAGSELB);//
			vRegWriteFldAlign(VFE_E1, 0, RG_UPDNB);//
			vRegWriteFldAlign(VFE_E2, 0, RG_SHIFTB_PWD);//
			vRegWriteFldAlign(VFE_E2, 0, RG_SHIFT2B_PWD);//
			vRegWriteFldAlign(VFE_E2, 0, RG_BP_PGABUFB);//
		...
		case VSS_SV:
			...
			vRegWriteFldAlign(VFE_E1, 1, RG_HPCLAMPB);//
			vRegWriteFldAlign(VFE_E1, 0, RG_CLAMPC_TSTB);//
			vRegWriteFldAlign(VFE_E1, 7, RG_CALCLAMPB);//
			...
			vRegWriteFldAlign(VFE_E2, 0, RG_CTBUFB_PWD);//
			vRegWriteFldAlign(VFE_E2, 0, RG_PGAB_PWD);//
			//vRegWriteFldAlign(VFE_E3, 0, RG_VCOM_SEL_PGAB); CY
			//vRegWriteFldAlign(VFE_E3, 1, RG_VCOM_SEL_FLTB); CY
			vRegWriteFldAlign(VFE_E2, 0, RG_CTINB);//
...
{
   #ifdef CC_MT5395    
		vRegWriteFldAlign(VFE_E3, 0x80, RG_AISELR);//CHB=SIFP   
 	#else     
    vRegWriteFldAlign(VFE_E3, 0x20, RG_AISELR);//CHB=CVBS1
    vRegWriteFldAlign(VFE_E3, 0x10, RG_AISEL);//CHA=CVBS0
	#endif        
    vRegWriteFldAlign(VFE_E2, 0x0, RG_INMUX_PWD);
    vRegWriteFldAlign(VFE_E5, 0x01, RG_SIF_ENB);
    vRegWriteFldAlign(VFE_E2, 0x0, RG_PGABUFPB_PWD);
    vRegWriteFldAlign(VFE_E2, 0x0, RG_PGABUFNB_PWD);
   ...
   vRegWriteFldAlign(VSRC_00, 0x04, TVD3D_MPGAC);
    ...
    vRegWriteFldAlign(VFE_E3, 0x00, RG_OFFCURB);
    vRegWriteFldAlign(VFE_E2, 0x00, RG_PGAB_PWD);
    vRegWriteFldAlign(VFE_E2, 0x01, RG_SHIFTB_PWD);
    vRegWriteFldAlign(VFE_E2, 0x01, RG_SHIFT2B_PWD);
    //AAF Gain
    vRegWriteFldAlign(VFE_E5, 0x0F, RG_SIFGAINB);
    vRegWriteFldAlign(VFE_E1, 0x00, RG_VAGSELB);
    vRegWriteFldAlign(VFE_E1, 0x00, RG_UPDNB);
    vRegWriteFldAlign(VFE_E1, 0x00, RG_HPCLAMPB);
    vRegWriteFldAlign(VFE_E1, 0x00, RG_C_SELB);
    vRegWriteFldAlign(VFE_E0, 0x00, RG_CTCLAMP_CTMB);
    vRegWriteFldAlign(VFE_E0, 0x00, RG_CLAMP_SOURCE_ENB);
    vRegWriteFldAlign(VFE_E0, 0x00, RG_CLAMP_SINK_ENB);
   //CKGEN
    vIO32WriteFldMulti(CKGEN_VPCLK_CFG,P_Fld(0,FLD_ADC_C54M_SEL)|P_Fld(0,FLD_TVD_CK54_SEL));
 #ifdef CC_MT5395   
 	vIO32WriteFldMulti(CKGEN_PADDRVCFG2, P_Fld(0, PADDRV_4MA)|P_Fld(0, PADDRV_2MA));    
 	vIO32WriteFldAlign(CKGEN_MBIST_CTL, 0, FLD_VPCLK_SEL);   
 #else    
    vIO32WriteFldMulti(CKGEN_PMUX5, P_Fld(0, FLD_PMUX5_Bit1)|P_Fld(0, FLD_PMUX5_Bit0));
    vIO32WriteFldAlign(CKGEN_MBISTCTRL, 0, FLD_MBIST_CLK_SEL_Bit3);
 #endif    
    vIO32WriteFldAlign(CKGEN_VPCLK_STOP, 0, FLD_TVD_CK54_STOP);
...
void vDrvCvbsAdcPD(void)
{
     ...
     vRegWriteFldAlign(VFE_E0, 1, RG_BG_PD);
    #endif
     vRegWriteFldAlign(VFE_E0, 1, RG_MIX_PWD);//
     vRegWriteFldAlign(VFE_E4, 1, RG_BIAS_PWD);//
     vRegWriteFldAlign(VFE_E5, 1, RG_CVBSADC_PWD);//
     vRegWriteFldAlign(VFE_E6, 1, RG_ABIST_UBUF_PWD);//
   #endif
}
...
void vDrvCvbsAdcPWON(void)
{
    ...
    vRegWriteFldAlign(VFE_E4, 0, RG_BIAS_PWD);//
    vRegWriteFldAlign(VFE_E5, 0, RG_CVBSADC_PWD);//
    vRegWriteFldAlign(VFE_E6, 0, RG_ABIST_UBUF_PWD);//
  #endif
}
...
void vDrvHDTVADCDefaultSetting(void)
{
    ...
    _bSDDSCaliState = 0;
   #endif
if (((BSP_GetIcVersion() >= IC_VER_5365_AB) && IS_IC_5365()) || ((BSP_GetIcVersion() >= IC_VER_5395_AB) && IS_IC_5395()))
{
  vIO32WriteFldMulti(PDWNC_PDMISC,P_Fld(6,FLD_SOGY_VTL)|P_Fld(1,FLD_SOGY_SEL)|P_Fld(0,FLD_SOGY_CAL)|P_Fld(9,FLD_SOGY_VTH));
}
    //vIO32WriteFldMulti(VGACFG0,P_Fld(0,FLD_RG_VMUX_PWD)|P_Fld(1,FLD_RG_DESPIKE)|P_Fld(1,FLD_RG_RGB_EN));  //5360 Michael checked
    vIO32WriteFldMulti(VGACFG0,P_Fld(0,FLD_RG_VMUX_PWD)|P_Fld(1,FLD_RG_DESPIKE));  //Mt5365/95 kal checked
    ...
    vRegWriteFldMulti(VFE_00, P_Fld(0,ALWAYSAG)|P_Fld(0,ACMONEN)|P_Fld(5,IBG_SEL)|P_Fld(0x0B,VBG_SEL)
                        ...
                        |P_Fld(0,VBG_TEST_EN)|P_Fld(0,IBG_TEST_EN)|P_Fld(0,BG_PWD)); //VBG_SEL C->B, Kal from SC Hwu
    #else   
    vRegWriteFldMulti(VFE_00, P_Fld(0,ALWAYSAG)|P_Fld(0,ACMONEN)|P_Fld(5,IBG_SEL)|P_Fld(0x0C,VBG_SEL)
                        |P_Fld(0,CLKOSEL_3)|P_Fld(0,CLKOSEL_2)|P_Fld(1,CLKOSEL_1)|P_Fld(0,ADC_DEBUG_SEL)
                        |P_Fld(0,ADC_DEBUG_SYNC)| P_Fld(0,ABIST_VGA_DATA_SEL)|P_Fld(0,ABIST_VGA_DOWN_SAMPLE)
    ...
    #ifdef CC_MT5395
    vRegWriteFldMulti(VFE_0C,P_Fld(0x02,ADC_VREFP)|P_Fld(0x02,ADC_VREFN));//MT5395                    
    #else
    vRegWriteFldMulti(VFE_0C,P_Fld(0x02,ADC_VREFP)|P_Fld(0x03,ADC_VREFN));//MT5365
    #endif
...
void vDrvAllHDADCPow(UINT8 bPow)
{
	...
	if (bPow == SV_TRUE)//Power on
	{
		...
		vRegWriteFldAlign(VFE_04,0,ADCBIAS_PWD);
		vRegWriteFldAlign(VFE_04,0,AGGEN_PWD);
		vRegWriteFldAlign(VFE_04,0,VREFGEN_PWD);
        vUtDelay1ms(2);
		for (bCh=0; bCh<3; bCh++)
		{
            ...
            vRegWriteFldAlign(VFE_02+(bCh<<4), 0, GBIAS_1_PWD);
		}
        vUtDelay1ms(2);
	...
	else if (bPow == SV_FALSE)//Power down
	{
		...
		vRegWriteFldAlign(VFE_04,1,ADCBIAS_PWD);
		vRegWriteFldAlign(VFE_04,1,AGGEN_PWD);
		vRegWriteFldAlign(VFE_04,1,VREFGEN_PWD);
		for (bCh=0; bCh<3; bCh++)
		{
            ...
            vRegWriteFldAlign(VFE_02+(bCh<<4), 1, GBIAS_1_PWD);
		}
       #endif 
...
void vDrvSYNCSlicerWakeup(UINT8 bPow)
{
	...
	if (((BSP_GetIcVersion() >= IC_VER_5365_AB) && IS_IC_5365()) || ((BSP_GetIcVersion() >= IC_VER_5395_AB) && IS_IC_5395()))
	{
  		vIO32WriteFldMulti(PDWNC_PDMISC,P_Fld(0,FLD_SOGY_SEL));
	}
        vIO32WriteFldMulti(SYNCFG0, P_Fld(1,FLD_RG_HSYNC_EN));
#ifdef CC_SOURCE_AUTO_DETECT
        vIO32WriteFldMulti(SYNCFG2, P_Fld(1,FLD_RG_VSYNC_EN)|P_Fld(0,FLD_RG_SOGY_SORS_PWD)|P_Fld(0,FLD_RG_SOGY_SINK_PWD));
#else
	...
	if (((BSP_GetIcVersion() >= IC_VER_5365_AB) && IS_IC_5365()) || ((BSP_GetIcVersion() >= IC_VER_5395_AB) && IS_IC_5395()))
	{
  		vIO32WriteFldMulti(PDWNC_PDMISC,P_Fld(1,FLD_SOGY_SEL));
	}
...
{
	UINT8 current_use_adc;
	if(_bEFUSE_AUTOCOLOR_READY)
	    current_use_adc=1;    //use EFUSE
	else if ((_bEEPROM_ready) && (bApiEepromReadByte(EEP_VIDEO_AUTO_COLOR_HDTV_DONE+bType) == SV_TRUE))
	    current_use_adc=2;   //use eeprom
	else
	    current_use_adc=3;   //use FW Default
	return current_use_adc;
}
...
void vScartVfeConnect(void)
{
    ...
    vRegWriteFldMulti(VFE_E5, P_Fld(1,RG_MIX_10U));//
    vRegWriteFldMulti(VFE_E4, P_Fld(1,RG_MIX_60U_EN) |P_Fld(0,RG_MIX_SHORT)|P_Fld(2,RG_MIXVOCM_SEL));//
...
{ //Michael Modify :20100308
    ...
    vIO32WriteFldAlign(SYNCFG1, 0x22   , FLD_RG_SOGY_SINK);//101010
   ...
   {
     vIO32WriteFldAlign(SYNCFG2, 1   , FLD_RG_SOGY_SINK_PWD);//FLD_RG_SOGY_SORS_PWD
   }
   else
   {
     vIO32WriteFldAlign(SYNCFG2, 0   , FLD_RG_SOGY_SINK_PWD);//FLD_RG_SOGY_SORS_PWD
   }
}
