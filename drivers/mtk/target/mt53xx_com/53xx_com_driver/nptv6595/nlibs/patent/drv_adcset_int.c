...
void vDrvChACalonBlankEnb(void)
{
     //make sure chA and ADC are power on???
         ////vRegWriteFldAlign(VFE_23, 1, RG_UPDNA);
         vRegWriteFldAlign(VFE_E1, 1, RG_UPDNA);//
     
         //vWriteVDOINMsk(VFE_23, 0x01, RG_UPDNA, 1);
         ////vRegWriteFldAlign(VFE_22, 0, RG_CALCTA);
         //vRegWriteFldAlign(VFE_E0, 0, RG_CALCTA); CY
     
         //vWriteVDOINMsk(VFE_22, 0x00, RG_CALCTA, 2);
         ////vRegWriteFldAlign(VFE_22, 1, RG_CALPGAA);
         vRegWriteFldAlign(VFE_E1, 1, RG_CALPGAA);//  //MT8222 this is Cal mode.

         //vWriteVDOINMsk(VFE_22, RG_CALPGAA, RG_CALPGAA, 2);
         ////vRegWriteFldAlign(VFE_22, 1, RG_VAGSELA);
         vRegWriteFldAlign(VFE_E1, 1, RG_VAGSELA);//

         //vWriteVDOINMsk(VFE_22, RG_VAGSELA, RG_VAGSELA, 2);
         ////vRegWriteFldAlign(VFE_22, 0, RG_C_SELA);
         vRegWriteFldAlign(VFE_E1, 0, RG_C_SELA);//

         //vWriteVDOINMsk(VFE_22, RG_C_SELA, RG_C_SELA, 2);
         ////vRegWriteFldAlign(VFE_23, 0, RG_CLAMP_SINK_ENA);
         vRegWriteFldAlign(VFE_E0, 0, RG_CLAMP_SINK_ENA);//

         //vWriteVDOINMsk(VFE_23, 0x00, RG_CLAMP_SINK_ENA, 0);
         ////vRegWriteFldAlign(VFE_23, 0, RG_CLAMP_SOURCE_ENA);
         vRegWriteFldAlign(VFE_E0, 0, RG_CLAMP_SOURCE_ENA);//

         //vWriteVDOINMsk(VFE_23, 0x00, RG_CLAMP_SOURCE_ENA, 0);     
         ////vRegWriteFldAlign(VFE_23, 1, RG_BP_PGABUFA);
         vRegWriteFldAlign(VFE_E2, 0, RG_BP_PGABUFA);// // -->Robin 建議 mt8222 改 0

         //vWriteVDOINMsk(VFE_23, RG_BP_PGABUFA, RG_BP_PGABUFA, 1);
         ////vRegWriteFldAlign(VFE_23, 0, RG_SHIFTA_PWD);
         vRegWriteFldAlign(VFE_E2, 0, RG_SHIFTA_PWD);//

         //vWriteVDOINMsk(VFE_23, 0x00, RG_SHIFTA_PWD, 1);
         ////vRegWriteFldAlign(VFE_23, 0, RG_SHIFT2A_PWD);
         vRegWriteFldAlign(VFE_E2, 1, RG_SHIFT2A_PWD);//// -->Robin 建議 mt8222 改 1

         //vWriteVDOINMsk(VFE_23, 0x00, RG_SHIFT2A_PWD, 1);
         ////vRegWriteFldAlign(VFE_20, 1, RG_OFFCAL_EN);
         //vRegWriteFldAlign(VFE_E4, 1, RG_OFFCAL_EN); CY

         //vWriteVDOINMsk(VFE_20, RG_OFFCAL_EN, RG_OFFCAL_EN, 0);//deselect Mux==>need to disconnect???
         ////vRegWrite1BMsk(VFE_20, 0x00, 0xf0);
         vRegWriteFldAlign(VFE_E3, 0, RG_AISEL);//

         //vWriteVDOINMsk(VFE_20, 0x00, 0xf0, 0);//disconnect [9:8] of AISEL and AISELR
         ////vRegWrite2BMsk(VFE_21, 0x0000, 0xffff);
         vRegWriteFldAlign(VFE_E3, 0, RG_AISELR);//
     
     //vWriteVIN2B(VFE_21, 0x0000, 1, 0);//disconnect [7:0] of AISEL and AISELR
     //vRegWriteFldAlign(VFE_22, 0, RG_CALCLAMPA);
         //vWriteVDOINMsk(VFE_22, 0x00, RG_CALCLAMPA, 1);//initialize chA input offset for calibration
     //vRegWriteFldAlign(VFE_23, 16, RG_OFFCURA);
         //vWriteVDOINMsk(VFE_23, 0x10, RG_OFFCURA, 3);//initialize chA output offset for calibration
...
void vDrvChACalonBlankDsb(void)
{
      //recovery the setting of chA in "vDrvTVADCDefaultSetting() ==> operation in clamping on blank"
         //vWriteVDOINMsk(VFE_23, 0x01, RG_UPDNA, 1);
         //vWriteVDOINMsk(VFE_22, 0x00, RG_CALCTA, 2);
         ////vRegWriteFldAlign(VFE_22, 0, RG_CALPGAA);
         vRegWriteFldAlign(VFE_E1, 0, RG_CALPGAA);//
      
         //vWriteVDOINMsk(VFE_22, 0x00, RG_CALPGAA, 2);
         //vWriteVDOINMsk(VFE_22, RG_VAGSELA, RG_VAGSELA, 2);
         //vWriteVDOINMsk(VFE_22, RG_C_SELA, RG_C_SELA, 2);
         //vWriteVDOINMsk(VFE_23, 0x00, RG_CLAMP_SINK_ENA, 0);
         //vWriteVDOINMsk(VFE_23, 0x00, RG_CLAMP_SOURCE_ENA, 0);     
         ...
         //vWriteVDOINMsk(VFE_23, 0x00, RG_SHIFTA_PWD, 1);
         //vWriteVDOINMsk(VFE_23, 0x00, RG_SHIFT2A_PWD, 1);
         ////vRegWriteFldAlign(VFE_20, 0, RG_OFFCAL_EN);
         //vRegWriteFldAlign(VFE_E4, 0, RG_OFFCAL_EN); CY

         //vWriteVDOINMsk(VFE_20, 0x00, RG_OFFCAL_EN, 0);//select Mux
         ////vRegWrite1BMsk(VFE_20, 0x00, 0xf0);
         vRegWriteFldAlign(VFE_E3, 0, RG_AISEL);//

         //vWriteVDOINMsk(VFE_20, 0x00, 0xf0, 0);//disconnect [9:8] of AISEL and AISELR
         ////vRegWrite2BMsk(VFE_21, 0x0110, 0xffff);
         vRegWriteFldAlign(VFE_E3, 0x0110, RG_AISELR);//
         vRegWriteFldAlign(VFE_E2, 0, RG_SHIFTA_PWD);////
         vRegWriteFldAlign(VFE_E2, 0, RG_SHIFT2A_PWD);////

         //vWriteVIN2B(VFE_21, 0x0110, 1, 0);//chA:CVBS0 ; chB:SY
...
void vDrvInputOffsetCal(void)
{
	...
	bCALCLAMP=0x0f;//mt5360, 15->0, slope = positive
       ...
       #if 0 // for loop test only
	while (1)
	{
            vRegWriteFldAlign(VFE_E1, bCALCLAMP, RG_CALCLAMPA); 
            if(bCALCLAMP == 0xff) break; 
	      ...
	      iB= wDrvReadADCBlankAve();
             ...
             #if DBG_TVADC_CAL_5381  
                   LOG(3,"bCALCLAMP = %x, iA = %u, iB = %u\n",bCALCLAMP, iA, iB);
             #endif
                
             bCALCLAMP --;

	    }	
           vRegWriteFldAlign(VFE_E1, 0x0c, RG_CALCLAMPA); 
	#endif
   
       #if 1 //new method of similar triangle to look for cross point .....kal
            vDrvPGAMinGainMauAGC2();
            vRegWriteFldAlign(VFE_E1, 0x0f, RG_CALCLAMPA); //
            vUtDelay1ms(u4Delay);  
            iA= wDrvReadADCBlankAve();
            vRegWriteFldAlign(VFE_E1, 0x00, RG_CALCLAMPA); //
            vUtDelay1ms(u4Delay);  
            iB= wDrvReadADCBlankAve();
    ...
    vRegWriteFldAlign(VFE_E1, bCALCLAMP, RG_CALCLAMPA);////recover to default setting 
    vRegWriteFldAlign(VFE_E1, bCALCLAMP, RG_CALCLAMPB);//
...
#if 1 //TRIANGLE_METHOD_FOR_INOUT_OFFSET_CAL
void vDrvOutputOffsetCalNew(UINT16 wCLAMP)
{
    ...
    vRegWriteFldAlign(VFE_E3, bOFFCUR, RG_OFFCURA);////MT536X //recover to default setting 
    vRegWriteFldAlign(VFE_E3, bOFFCUR, RG_OFFCURB);////MT536X //recover to default setting 
}
#else
void vDrvOutputOffsetCalNew(UINT16 wCLAMP)
{
	UINT8 bOFFCUR,i;
	UINT8 bChangeSlope = 0; //one chance to modify band
	UINT16 wTmp=0, wTmp2=0;	
	UINT16 OUTPUT_OFFSET_TH = (wCLAMP);
       UINT32 u4Delay = 5;//20 save time;//for lint
	//LogS("vDrvOutputOffsetCalNew.....");
	vDrvPGAMidGainMauAGC2();  //Set PGA Manual gain to 1/2 full range	
	
	//***************************************************//
	#if 0 //for loop test only
		for(i=0;i<=31;i++)
                     {  
		        vRegWriteFldAlign(VFE_E3, i, RG_OFFCURA);
			 vUtDelay1ms(u4Delay); //change 250ms to 20ms   		
		        wTmp=wDrvReadADCBlankAve();	//Average of output offset
                      #if DBG_TVADC_CAL_5381		
                            LOG(3," i= %2x, output level is %u\n", i, wTmp);
                      #endif
		        //LogSB("i = ",i);
		        //LogSW("output level is ", wTmp);
			}
     
                     vRegWriteFldAlign(VFE_E3, 0x10, RG_OFFCURA);
                     wTmp = 256;
	#endif	
	...
	//***************************************************//
      	// Output offset Calibration Loop, normal entrance
	i=0;
	...
	{	
              if(i <= 0x0f) 
              {   
                 bOFFCUR = 0x0f-i; //bOFFCUR[4]=0, negtive
              }
              else
              {
                 bOFFCUR=i;  //bOFFCUR[4]=1, positive
              }
              vRegWriteFldAlign(VFE_E3, bOFFCUR, RG_OFFCURA);
		vUtDelay1ms(u4Delay); //change 250ms to 20ms  
		...
		wTmp2=wTmp;
		wTmp=wDrvReadADCBlankAve();	//the value will be difference while warm up

              #if DBG_TVADC_CAL_5381		
                    LOG(3,"output offset is  %u, bOFFCUR=%x \n", wTmp, bOFFCUR);
              #endif

		if (i>=31)	//Boundary case //should be >= 31, not > 31
		{      
			if(bChangeSlope == 0) //one chance to modify band
			{
				bChangeSlope = 1;
                            i = 0;
				wTmp = 0;
				wTmp2 = 0;
                      ...
                      #if DBG_TVADC_CAL_5381        
                            LOG(3,"change band IC, i>=31, but give it once more chance to change\n");
                            #ifdef __MODEL_slt__
                                       //fgCVBSFail = TRUE; //it don't need this flag
                            #endif			
                      #endif
				continue;
			}
                     #if DBG_TVADC_CAL_5381        
			       LOG(3," Fail IC, output offset level=31 , blank is %u\n", wTmp);	
                            #ifdef __MODEL_slt__
                                      fgCVBSFail = TRUE;
                            #endif	
                     #endif
		...
		if (wTmp>=OUTPUT_OFFSET_TH)
		{
			if (i==0) //Boundary case
			{
				if(bChangeSlope == 0) //one chance to modify band
				{
				  ...
				  wTmp2 = 0;
                              vRegWriteFldAlign(VFE_E2, 0, RG_SHIFT2A_PWD);
                              #if DBG_TVADC_CAL_5381        
                                    LOG(3,"change band IC, i=0, but give it once more chance to change\n");
                                    #ifdef __MODEL_slt__
                                               //fgCVBSFail = TRUE; //it don't need this flag
                                    #endif  
                              #endif
				continue;
                              ...
                              #if DBG_TVADC_CAL_5381        
			               LOG(3," Fail IC, output offset level=0 , blank is %u\n", wTmp);	
                                    #ifdef __MODEL_slt__
                                              fgCVBSFail = TRUE;
                                    #endif  
                              #endif
			...
			else if ((wTmp-wTmp2)>10)	//IC defect, FW cover!!
			{
                             #if DBG_TVADC_CAL_5381        
                                   LOG(3,"Fail IC, output offset is abnormal , Diff is too big\n");
                                   #ifdef __MODEL_slt__
                                              fgCVBSFail = TRUE;
                                   #endif  
                             #endif
			...
			{
				if((OUTPUT_OFFSET_TH-wTmp2)<(wTmp-OUTPUT_OFFSET_TH))//shortest distance to OUTPUT_OFFSET_TH
				{
				   wTmp = wTmp2;
  		                 i--;
                               if(i <= 0x0f) 
                            ...
                            #if DBG_TVADC_CAL_5381		
                                LOG(3,"output offset is DONE, blank level is %u, register is %xh\n", wTmp, bOFFCUR);
                            #endif
	...
	if ((wTmp>300)||(wTmp<200))
	{
          #if DBG_TVADC_CAL_5381      
		 LOG(3,"Fail IC, output offset is fail , blank is over range %u\n", wTmp);
               #ifdef __MODEL_slt__
                          fgCVBSFail = TRUE;
               #endif  
          #endif
       ...
       vRegWriteFldAlign(VFE_E3, 0, RG_OFFCURA);//recover to default setting 
       vRegWriteFldAlign(VFE_E3, 0, RG_OFFCURB);
     
      #if DBG_TVADC_CAL_5381		
            LOG(3,"wTARGET_BLANK is %x \n", wTmp);
      #endif
	//return wTmp;
}
#endif
...
{
	// NEW_ADC_CALIBRATION_FLOW
	//BOOL fgRet=0;//fot lint
	//CH1 Calibration 
	//vWriteVINMsk(VFE_0F, 0x00000000, 0x00ff6100); // power on Ch1
	//vDrvTVADC0CH1ClampOnBottomOffsetCal();
	//vDrvTVADCDefaultSetting();
	#if DBG_TVADC_CAL_5371
	//LOG(3,"Channel 1 calibration is done  %u\n", 0);
	#endif

	//CH0 Calibration 
       //0403 ; it has done before calibration
	//vDrvTVADCDefaultSetting();//0403;why do it???		// for demo, moving code ASAP
       //0403------
	//vDrvTVADC0CH1ClampOnBottom();//don't need to cal botom???
        //0403
    
       //vWriteVINMsk(VFE_0D, 0x00000000, 0x00ff6100); // power on Ch0
	//vWriteVINMsk(VFE_0F, 0x001f0000, 0x001f0000); // power down Ch1
          ////kal mark vRegWrite4BMsk(VFE_23, 0, 0x003f0000);
	...
	//vWriteVINMsk(VFE_23, 0x00000000, 0x003f0000); //power on chA
          ////kal mark vRegWrite4BMsk(VFE_25, 0x003f0000, 0x003f0000);
          ...
          vRegWriteFldAlign(VFE_E2, 3, RG_AAFB_PWD);////???
          vRegWriteFldAlign(VFE_E2, 1, RG_CTCLAMPB_PWD);
          vRegWriteFldAlign(VFE_E2, 1, RG_BUFB_PWD);
          
	//vWriteVINMsk(VFE_25,0x003f0000, 0x003f0000);//power down chB ; [7:6]???
	#if DEBUG_CLAMP_ON_BOTTOM
	...
	//vDrvTVADCDefaultSetting();//0403;why do it???	//  for demo, moving code ASAP
	#if DBG_TVADC_CAL_5371
          //LOG(3,"Channel 0 calibration is done  %u\n", 0);
	#endif
	//vWriteVINMsk(VFE_0F, 0x00000000, 0x00ff6100); // power on Ch1

          ////kal mark vDrvAAFCalibration();////kal Robin sai MT8222 has no this

          ////kal mark vRegWrite4BMsk(VFE_25, 0x00000000, 0x003f0000);
	...
	//vWriteVINMsk(VFE_25,0x00000000, 0x003f0000);//power on chB ; [7:6]???
	
    //#if SAVE_TVDC_CAL_TO_EEPROM
          ////fgRet = fgApiEepromWriteByte(wEEP_ADC_OFFSET_TV_CLAMP_DONE, SV_TRUE);
          ////UNUSED(fgRet);
    //#else
          ////fgRet = fgApiEepromWriteByte(wEEP_ADC_OFFSET_TV_CLAMP_DONE, SV_FALSE);
          ////UNUSED(fgRet);
    //#endif
...
void vTvdFT_Chroma_SAT(UINT8 bCmd, UINT32 dArg)
{
	...
	if(vTvdFT_EnvUpdate())
	{
		...
		if((HResNRLvl>1 && HResNRLvl<7)&&(dArg&1))
