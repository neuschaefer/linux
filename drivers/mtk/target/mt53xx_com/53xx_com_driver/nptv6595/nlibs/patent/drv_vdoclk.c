...
//CC_MT5365 according to Walter's suggestion
 vRegWriteFldAlign(VFE_16, 1, SDDS_DATA_SYNC_POST);  
 vRegWriteFldAlign(VFE_16, 1, SDDS_DATA_SYNC_PRE);  
 vUtDelay2us(1);
 vRegWriteFldAlign(VFE_16, 0, SDDS_DATA_SYNC_POST);  
 vRegWriteFldAlign(VFE_16, 0, SDDS_DATA_SYNC_PRE);  
 ...
 vDrvCLKINFreeRun(ENABLE);
    ...
    vIO32WriteFldAlign(CKGEN_DISP_CKCFG1, 1, FLD_RESYNC_PCLK_SEL);
        ...
        vRegWriteFldMulti(VFE_0C, P_Fld(1,CLKINV_EN)|P_Fld(1,RELATCH_EN)); //be careful the define for this bit is different between MT5365 AND MT8223		
    #else//single phase
        ...
        vRegWriteFldMulti(VFE_0C, P_Fld(0,CLKINV_EN)|P_Fld(0,RELATCH_EN)); //be careful the define for this bit is different between MT5365 AND MT8223			
    #endif
...
void vDrvADCPLLSet(UINT16 wPixClk, UINT16 wHtotal)
{
   ...
   //vIO32WriteFldAlign(CKGEN_DTDPLLCFG0, 0, FLD_RG_ADCPLL2_DE_FB_CN);
    ...
    //SC hwu suggest , Michael Modify 20091125
 if (fgIsMainVga()||fgIsPipVga())
 {
       bmode=Get_VGAMODE_OverSample(_bVgaTiming);
 }
 ...
 {
       bmode=Get_VGAMODE_OverSample(_bHdtvTiming);
     }


if ((bmode==1) && wPixClk<=550 )
{
        vPGA_LPF_BW(1);
}
else
...
//new band selection
 ...
 //      vIO32WriteFldAlign(CKGEN_AFEPLLCFG5, 0, FLD_RG_VGAPLL_SDDS_FBKSEL);
   }
    else if((wPixClk<750)&&(wPixClk>=400))			//(SDclk CW)/1, ADCPLLx1 
    {
      bADCPLL_NS = 0;
      vIO32WriteFldAlign(CKGEN_AFEPLLCFG5,0, FLD_RG_VGAPLL_SDDSO_DIV12_SEL);
//    vIO32WriteFldAlign(CKGEN_AFEPLLCFG5, 0, FLD_RG_VGAPLL_SDDS_FBKSEL);
    }
    else if((wPixClk<1400) && (wPixClk>=750))	//(SDclk CW)/2, ADCPLLx2     //wu
    {      
      bADCPLL_NS = 2;
...
//     vIO32WriteFldAlign(CKGEN_AFEPLLCFG5, 0, FLD_RG_VGAPLL_SDDS_FBKSEL);
 ...
 //    vIO32WriteFldAlign(CKGEN_AFEPLLCFG5, 0, FLD_RG_VGAPLL_SDDS_FBKSEL);
    ...
    bMcode = (UINT8) (SYSPLL459 / wPixClk);  //cal the M code
    dTmp = SYSPLL459 - (UINT32) wPixClk *bMcode; //calculate residue        
    ...
    for(delay_count=0;delay_count<50;delay_count++)
    {
        ...
        #else
        if((IO32ReadFldAlign(CKGEN_ACD_DBG_REG0,FLD_RGS_VGA_VCOCALOK)))
        #endif
        {
            delay_count=50;
        }
...
UINT32 dwTmp[MAX_PHASE_ARRAY],dwTmpMAX;
       ...
       if (fgIsMainYPbPr() || fgIsPipYPbPr())                  //wu
       {
       ...
       else
        {
           ...
           {
                case 0:
                vRegWriteFldAlign(VFE_00, 1, CLKOSEL_1);	
                vRegWriteFldAlign(VFE_00, 0, CLKOSEL_2);	
                vRegWriteFldAlign(VFE_00, 0, CLKOSEL_3);	
                case 1:
                vRegWriteFldAlign(VFE_00, 1, CLKOSEL_2);	
                vRegWriteFldAlign(VFE_00, 0, CLKOSEL_1);	
                vRegWriteFldAlign(VFE_00, 0, CLKOSEL_3);	
                case 2:
                default:
                vRegWriteFldAlign(VFE_00, 1, CLKOSEL_3);	
                vRegWriteFldAlign(VFE_00, 0, CLKOSEL_1);	
                vRegWriteFldAlign(VFE_00, 0, CLKOSEL_2);	
                break;
            }
...
void DDDS_SetSpreadSpectrum(UINT32 u4Freq, UINT32 u4Range)
{
    ...
    #else
    u8Step = (UINT32)BSP_GetPLLClock(CAL_SRC_SYSPLL);
    #endif
...
void vDDDSInit(void)
{
    if(!bIsPanelOn())
    {
    #ifdef CC_MT5395 // need to toggle to set control word on freerun mode
    vIO32WriteFldMulti(DDDS_01, P_Fld(0x0B, DDDS_01_INIT_SEL)|
                                P_Fld(0,DDDS_01_HLEN_NUM)|P_Fld(0,DDDS_01_HLEN_INT));
    ...
    // Recover DDDS Initial Issue 432/74.25=0x62E8BA2
    vIO32WriteFldAlign(DDDS_00, 0x62E8BA2, DDDS_00_DDDS_FREQ_CW);    
    ...
    // Recover DDDS Initial Issue 432/74.25=0x62E8BA2
    vIO32WriteFldAlign(DDDS_00, 0x62E8BA2, DDDS_00_DDDS_FREQ_CW);    
    Printf("[LVDS] DVOPLL init control word only\n");    
    #endif
    ...
    vDrvDDDSStepTrackingOnOff(SV_ON);
      ...
      vIO32WriteFldMulti(DDDS_04,P_Fld(0,DDDS_04_SPREAD_CENBAL_EN)|P_Fld(0,DDDS_04_SPREAD_CONT_EN)|
                                 P_Fld(0,DDDS_04_SPREAD_FR_EN));    
      #ifdef CC_MT5395
      vIO32WriteFldAlign(CKGEN_OCLK_TEST, FS_OCLK_SEL_DDDS2_NOSS, FLD_FS_OCLK_SEL);      
      #else
      vIO32WriteFldAlign(CKGEN_PSWCLK_CFG, FS_OCLK_SEL_DDDS2_NOSS, FLD_FS_OCLK_SEL);
      #endif
    #else
      vIO32WriteFldMulti(DDDS_04,P_Fld(0,DDDS_04_SPREAD_CENBAL_EN)|P_Fld(0,DDDS_04_SPREAD_CONT_EN)|
                                 P_Fld(0,DDDS_04_SPREAD_FR_EN));    
      #ifdef CC_MT5395
      vIO32WriteFldAlign(CKGEN_OCLK_TEST, FS_OCLK_SEL_VOPLL, FLD_FS_OCLK_SEL);
      #else
      vIO32WriteFldAlign(CKGEN_PSWCLK_CFG, FS_OCLK_SEL_VOPLL, FLD_FS_OCLK_SEL);
      #endif
    #endif
    ...
    {
     // 1. Change OCLK, OCLK2, TCLK source from GPIO pin.    
     vIO32WriteFldMulti(CKGEN_OCLK_TEST, P_Fld(1,FLD_TCLK_TSTSEL)|P_Fld(1,FLD_SS_OCLK_TSTSEL)|
                                         P_Fld(1,FLD_OCLK_TSTSEL));
     // 2. Output miniLVDS data  structure on LVDS mode                                        
     vIO32WriteFldAlign(LVDSTX_ANA3, 1, LVDSA3_MINI_SEL);                                         
     vIO32WriteFldAlign(LVDSTX_ANA7, 1, LVDSB7_MINI_SEL);       
    }
    #endif
}
