...
#define MAIN_SCPOS_444_MODECHG ((0x1L)<<19)
#define PIP_SCPOS_444_MODECHG ((0x1L)<<20)
...
typedef struct RDispPrm
{
    ...
    UINT16 wVPorchOfst;         // (NOT IN USE). skywalker.todo: check if this can be removed?
    UINT16 wDataLen;            // Read Length
    ...
    UINT32 u4ReadInitP;     // initial point in H mirror mode 
    ...
    UINT32 u4USclVInit;         // V negative initial phase value
    UINT8  u1USclVInitSigned;   // V negative initial phase on/off
    UINT8  u1Mirror422Reg;      
    UINT32 u4USInputWidth;
    UINT8  u1HPaddingLen;       // the padding of each horizontal line       
    RScalerInfo rScalerInfo;    // scaler temporary information
...
typedef struct RDispInfo
{
    ...
    VDP_DISP_REGION_T rOutRegionR;

    // luis@060608
    //UINT32 u4VsyncDelay;
    // luis@2006/8/14...to align vsync when set background color
