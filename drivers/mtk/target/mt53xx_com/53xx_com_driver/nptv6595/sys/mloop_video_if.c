...
void vAllocateSWReg(void)
{
	...
	// for L2 cache issue
	#ifndef __KERNEL__	
	SW_DRAM_BASE_PHY = (UINT32)x_mem_alloc(0x10000);
    ...
    #else
	SW_DRAM_BASE = (SW_DRAM_BASE_PHY | 0x40000000);
    #endif
    #else
...
BOOL VdoMLInit(void)
{
    ...
    if(!_bVdoMLoopInit)
    {
		...
		//Printf("=========source detect: vdoMLinit: cvbsvfe don't pd\n");
#else
        vDrvCvbsVfePD();
#endif
