...
/*
#if SUPPORT_ERROR_LED_CTRL    
    if(tmpCmd.u1Cmd == Cmd_SetClearErrCode)
    {
#if UART_DBG
                LogS("Cmd_SetClearErrCode");
#endif
        gErrCodeCtrl.fgErrCodeLedType = tmpCmd.u1SubCmd;
        gErrCodeCtrl.fgErrCodeLedMsk = 0x1UL <<  tmpCmd.au1Data[0];
        ...
        if(gErrCodeCtrl.fgErrCodeLedInterval > gErrCodeCtrl.fgErrCodeLedPeriod)
        {
            gErrCodeCtrl.fgErrCodeLedInterval -= gErrCodeCtrl.fgErrCodeLedPeriod;        
                ...
                LogSB("set fgErrCodeLedMsk", gErrCodeCtrl.fgErrCodeLedMsk);
...
//    Cmd_ReadT8032WakeUp is the COMMAND most frequestly issued from ARM, so place it at 1st decision...
    ...
    else if(tmpCmd.u1Cmd == Cmd_QuerySwVersion)
    {
        ...
        vCMD_RESONSE(Cmd_QuerySwVersion, FW_VERSION,0xff,count);
    }
...
else if(tmpCmd.u1Cmd == PDWNC_CMD_ARMT8032_CUST_SH_CUSTDATA  )
{
    ...
    else if(tmpCmd.u1Cmd == Cmd_CEC_COPY_CEC_DATA_TO_DRAM)
    {    	
		CEC_DataSize = tmpCmd.u1SubCmd;
		Dram_addr=(((UINT32)(tmpCmd.au1Data[3])<<24)|((UINT32)(tmpCmd.au1Data[2])<<16)|((UINT32)(tmpCmd.au1Data[1])<<8)|((UINT32)(tmpCmd.au1Data[0])));
    	//LogSD("Dram_addr",Dram_addr);
		vCMD_RESONSE(Cmd_CEC_COPY_CEC_DATA_TO_DRAM, tmpCmd.au1Data[0], tmpCmd.au1Data[1], tmpCmd.au1Data[2]);

		if(CEC_DataSize == 0)
			for(count=0;count<=255;count++)
				vWriteDramB((Dram_addr+count),XBYTE[0XD200+count]);//0xaa);//0xaa);//
		else
			for(count=0;count<CEC_DataSize;count++)
				vWriteDramB((Dram_addr+count),XBYTE[0XD200+count]);//tmpCmd.au1Data[count%4]);//XBYTE[0XD200+count]);
    }
 ...
 #define DRAM_ADDR0 0x0042 //[7:0]
#define DRAM_ADDR1 0x0043 //[15:8]
#define DRAM_ADDR2 0x0044 //[23:16]
#define DRAM_ADDR3 0x0045 //[27:24]
#define DRAM_ACC 0x0046
	#define BYTE_ACC 0x01
	#define DRAM_LOCK_EN 0x02
	#define DRAM_BUSY 0x04
#define DRAM_RD0 0x0047
#define DRAM_RD1 0x0048
#define DRAM_RD2 0x0049
#define DRAM_RD3 0x004a
#define DMA_CFG1 0x004b
#define DRAM_WD0 0x003e
#define UP_SINGLE_RW_WA 0
#if UP_SINGLE_RW_WA
#define DUMMY_READ_ONE_BYTE(val, addr)\
    do {\
        vWriteDramAddrReg(addr);\
        DRAM_ACC_STACLR();\
        DRAM_READ_TRIG();\
        MWaitDramRdy();\
        DRAM_ACC_WAIT();\
        val = u1RegRead1B(DRAM_RD0);\
    } while (0)
#define DUMMY_WRITE_ONE_BYTE(val, addr)\
    do {\
        vWriteDramAddrReg(addr);\
        vRegWrite1B(DRAM_WD0, val);\
        MWaitDramRdy();\
    } while (0)
#define DELAY_20_DRAMCLK()\
    do {\
...
void vWriteDramAddrReg(UINT32 dwAddr) reentrant
{
    vRegWrite1B(DRAM_ADDR0, BYTE0(dwAddr));
    vRegWrite1B(DRAM_ADDR1, BYTE1(dwAddr));
    vRegWrite1B(DRAM_ADDR2, BYTE2(dwAddr));
    vRegWrite1B(DRAM_ADDR3, BYTE3(dwAddr));
}
#define DRAM_ACC_STACLR()	{vRegWrite1B(0x00b8, 0);}
#define DRAM_READ_TRIG() \
		do \
		{ \
			XBYTE[0x00fe] = 0x1; \
			vRegWrite1B(0x00b7 ,u1RegRead1B(0x00b7)|0x01); \
		} while (0)
#define DRAM_WRITE_TRIG() \
		do \
		{ \
			XBYTE[0x00fe] = 0x0; \
			vRegWrite1B(0x00b6 ,u1RegRead1B(0x00b6)|0x01); \
		} while (0)
#define DRAM_ACC_WAIT()	{while (u1RegRead1B(0x00b8) != 0x01);}
void MWaitDramRdy(void)
{
    while ((u1RegRead1B(DRAM_ACC))&DRAM_BUSY) {}
}
UINT8 irq1_save(void)
{
    UINT8 ret;
...
/**
 * Read one byte from Dram.
 *
 * @param dwAddr fetch address
 * @return the data
 */
UINT8 bReadDramB(UINT32 dwAddr) reentrant
{
    UINT8 bTmp; // using local register
    UINT8 flags;
    flags = irq1_save();

    vRegWrite1B(DRAM_ACC, BYTE_ACC);
    vWriteDramAddrReg(dwAddr);
    DRAM_ACC_STACLR();	//XBYTE[0x00b8] = 0;
    DRAM_READ_TRIG();		//XBYTE[0x00b7] |= 0x01; // read trigger
    DRAM_ACC_WAIT();		//while (XBYTE[0x00b8] != 0x01);
    MWaitDramRdy();
    bTmp = u1RegRead1B(DRAM_RD0);
    irq_restore(flags);
    return bTmp;
}

/* Wirte one byte to Dram.
 *
 * @param dwAddr write address
 * @param bData write data
 */
void vWriteDramB(UINT32 dwAddr, UINT8 bData) reentrant
{
    #if UP_SINGLE_RW_WA
    UINT8 bTmp;
    #endif
    ...
    vRegWrite1B(DRAM_WD0, bData);

    DRAM_ACC_STACLR();	//XBYTE[0x00b8] = 0;
    DRAM_WRITE_TRIG();	//XBYTE[0x00b6] |= 0x01; // write trigger
    DRAM_ACC_WAIT();		//while (XBYTE[0x00b8] != 0x01);
    #if UP_SINGLE_RW_WA
    //vKrDelay2us(0x20); 
    MWaitDramRdy();
    DUMMY_READ_ONE_BYTE(bTmp, dwAddr);
    #endif
    #if UP_SINGLE_RW_WA
    //vKrDelay2us(0x20); 
    MWaitDramRdy();
