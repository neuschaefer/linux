...
extern UINT8 fgSerialXpressEnable;
#endif

#if P_ERROR_LED
extern PError_Code_Ctrl Phi_err_code_ctrl;
#endif
...
#if CUST_P_SX_SUPPORT
else if(tmpCmd.u1Cmd == PDWNC_CMD_ARMT8032_P_COMPANY_BDSDATA )
{
    fgSerialXpressEnable = tmpCmd.u1SubCmd;    
    vCMD_RESONSE(PDWNC_CMD_ARMT8032_P_COMPANY_BDSDATA, 0, 0, 0);    
}
#endif
#if P_ERROR_LED
    else if(tmpCmd.u1Cmd == PDWNC_CMD_ARMT8032_P_COMPANY_ERRLED_SET )
    {
        vPErrCodeLedReset();
        vPSetCtrlLedEnable();
        if( tmpCmd.u1SubCmd == Cmd_Sub_ClearErrBuf)
        {
            Phi_err_code_ctrl.LED_Array[0] = 0;
            Phi_err_code_ctrl.LED_Array[1] = 0;
            Phi_err_code_ctrl.LED_Array[2] = 0;
            Phi_err_code_ctrl.LED_Array[3] = 0;
            Phi_err_code_ctrl.LED_Array[4] = 0;
        #if UART_DBG
            LogS("Clr Err Buf");
        #endif
        }
        ...
        {
            Phi_err_code_ctrl.LED_Array[4] = tmpCmd.u1SubCmd;
            Phi_err_code_ctrl.LED_Array[3] = tmpCmd.au1Data[3];
            Phi_err_code_ctrl.LED_Array[2] = tmpCmd.au1Data[2];
            Phi_err_code_ctrl.LED_Array[1] = tmpCmd.au1Data[1];
            Phi_err_code_ctrl.LED_Array[0] = tmpCmd.au1Data[0];
#if UART_DBG
            LogS("Set Err Buf");
            LogB(Phi_err_code_ctrl.LED_Array[0]);
            LogB(Phi_err_code_ctrl.LED_Array[1]);
            LogB(Phi_err_code_ctrl.LED_Array[2]);
            LogB(Phi_err_code_ctrl.LED_Array[3]);            
            LogB(Phi_err_code_ctrl.LED_Array[4]);            
#endif
        }
        vCMD_RESONSE(PDWNC_CMD_ARMT8032_P_COMPANY_ERRLED_SET, 0, 0, 0);    
    }
    else if(tmpCmd.u1Cmd == PDWNC_CMD_ARMT8032_P_COMPANY_ERRLED_EN )
    {
        vPErrCodeLedReset();
        vPSetCtrlLedEnable();
        if( tmpCmd.u1SubCmd == Cmd_Sub_EnableErrCode)
        {
            Phi_err_code_ctrl.fgErrCodeLedOn = 1;
            Phi_err_code_ctrl.fgErrCodeLedType = tmpCmd.au1Data[0];
            Phi_err_code_ctrl.fgErrCodeLedMsk = 0x1UL <<  tmpCmd.au1Data[1];
            Phi_err_code_ctrl.fgErrCodeLedPolarity = tmpCmd.au1Data[2];
#if UART_DBG
            LogS("En err Buf");
            LogSB("fgErrCodeLedOn", Phi_err_code_ctrl.fgErrCodeLedOn);
            LogSB("fgErrCodeLedType", Phi_err_code_ctrl.fgErrCodeLedType);
            LogSD("fgErrCodeLedMsk", Phi_err_code_ctrl.fgErrCodeLedMsk);
            LogSB("fgErrCodeLedPolarity", Phi_err_code_ctrl.fgErrCodeLedPolarity);            
#endif

        }
        else if( tmpCmd.u1SubCmd == Cmd_Sub_DisableErrCode)
        {
            Phi_err_code_ctrl.fgErrCodeLedOn = 0;
        }
        vCMD_RESONSE(PDWNC_CMD_ARMT8032_P_COMPANY_ERRLED_EN, 0, 0, 0);    
    }
#endif
