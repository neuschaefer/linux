        ...
        if ((prFbmPicHdr != NULL) && (prFbmSeqHdr != NULL))
        {
...
#endif
            {
                ...
                else
                {
                 ...
                 if (_prVdpConf[prFrcPrm->ucVdpId]->fgCropping)
                {
                    u4PicX = _prVdpConf[prFrcPrm->ucVdpId]->u4CropX;
                    u4PicY = _prVdpConf[prFrcPrm->ucVdpId]->u4CropY;
                    u4PicWidth = _prVdpConf[prFrcPrm->ucVdpId]->u4CropWidth;
                    u4PicHeight = _prVdpConf[prFrcPrm->ucVdpId]->u4CropHeight;
                }
                else
                {
                    u4PicX = 0;
...
static void _VdpCheckOutputMode(FRC_PRM_T* prFrcPrm)
{
    ...
    if (ucProgressive)
    {
        ...
        else
        {
            ...
            if( _prVdpConf[ucVdpId]->u4SrcWidth <1920 &&_prVdpConf[ucVdpId]->u4SrcHeight > 1080)
            {
                switch (prFrcPrm->ucInFrameRate)
                {
                case FBM_FRAME_RATE_24:
