...
static MI_DIF_GFX_MMU_UNION_T *_prGfxMMURegFile = (MI_DIF_GFX_MMU_UNION_T *)_au4GfxMMURegFile;
static GFX_MMU_T _r_gfx_mmu;
static UINT32 _fgGfxMmuInited=0x0;
void _GFX_MMU_Set_Enable(UINT32 u4_enable);

#endif

void vSetGFXFlipMirror(BOOL fgMirrorEn, BOOL fgFlipEn)
{
    #ifdef CC_FLIP_MIRROR_FROM_DDI
        #ifdef CC_FLIP_MIRROR_SUPPORT
        _fgGfxMirror = fgMirrorEn;
        _fgGfxFlip = fgFlipEn;
        #endif
    #endif
    return;
...
#if defined(CC_MT5360B) || defined(CC_MT5392B)
#if defined(CC_MT5368)||defined(CC_MT5396)|| defined(CC_MT5389)|| defined(CC_MT5398)||defined(CC_MT5880)
void _GfxSetBurstReadLength(BOOL fgEnBurstRead, UINT32 fgBurstReadMode)
{
    _fgGfxEnableBurstRead = fgEnBurstRead;
    _fgGfxBurstReadMode = fgBurstReadMode;   // 0 4x16 byte,  1 8X16byte, 2 16x16byte
}

BOOL _GfxGetBurstReadLength(void)
{
    return _fgGfxBurstReadMode;
}
#endif
...
static void _GfxAlphaCompPass(UINT32 u4Width, UINT32 u4Height)
{
    ...
    if(_fgGfxEnableBurstRead)
    {
        ...
        REG(fg_BURST_READ_MODE) = _fgGfxBurstReadMode;
    }
#else
    if (_fgGfxEnableBurstRead)
        REG(fg_BURST_READ) = GFX_ENGINE_BURST_READ;
    ...
    UINT32 u4DstY)
{
   UINT32 ui4_src_addr  = 0x0;
    ...
    ui4_src_addr = (UINT32)(_pu1DstBase + 
...
void _GFX_Flush(void)
{
    ...
    REG(fg_COLOR_KEY_MIN) = 0;
    REG(fg_COLOR_KEY_MAX) = 0;
...
void _GFX_Set_AlwaysRDSrc(UINT32 u4AlwaysRDSrc)
{
    REG(fg_ALWAYS_RD_SRC) = u4AlwaysRDSrc;
    //GFX_CMDENQ(GREG_G_MODE);
}
    ...
    UINT32 u4SrcWidth,UINT32 u4SrcHeight)
{
    ...
    REG(fg_SRC_WIDTH)       = (UINT32)u4DstAdress;
    ...
    UINT32 u4Width, UINT32 u4Height, UINT32 u4Ar, UINT32 u4Mode)
{
        ...
        (GFX_ENABLE == i4MhpPatch))
    {
                 ...
                 (0 == u4Ar))
        {
            ...
            ui4_dst_addr = (UINT32)(_pu1DstBase + (u4DstY * _u4DstPitch) + 
                                    (u4DstX << _u4DstShift));
            ui4_src_addr = (UINT32)(_pu1DstBase + (u4DstY * _u4DstPitch) + 
                                    (u4DstX << _u4DstShift));
            ...
            REG(fg_OSD_WIDTH) = _u4DstPitch >> 4;
            REG(fg_SRC_PITCH) = _u4SrcPitch >> 4;
            ui4_src_addr = (UINT32)(_pu1SrcBase + (u4SrcY * _u4SrcPitch) + (u4SrcX << _u4SrcShift));
            REG(fg_SRC_BSAD)        = ui4_src_addr;
            REG(fg_SRC_BSAD_MSB)    = GFX_REG_ADDR_MSG(ui4_src_addr);
            GFX_SetColor(u4RectColor);
...
BOOL _GFX_ColorConvert_YC(GFX_COLOR_CONVERT_T* prGfxColorConvert)
{
    ...
    {
        _GFX_MMU_Set_Enable(FALSE);
    }
    
    // one pass transform
    _GFX_SetColCnvFmt_(E_YCFMT_420MB, (UINT32)E_SWAP_BLOCK, (UINT32)E_VSTD_BT601, 
          (UINT32)E_VSYS_VID);
    
    _GFX_SetColCnvSrc_(prGfxColorConvert->pu1SrcAddr, prGfxColorConvert->u4DstPitch,
        prGfxColorConvert->pu1CbCr, prGfxColorConvert->u4DstPitch, 0);

    _GFX_SetDst_(prGfxColorConvert->pu1DstAddr, 
        prGfxColorConvert->u4DstCM,prGfxColorConvert->u4Width);
...
void _GFX_SetReadArgbOrder(UINT32 u4Enable, UINT32 u4Aorder, UINT32 u4Rorder, UINT32 u4Gorder, UINT32 u4Border)
{
    if (u4Enable)
    {
...
void _GFX_SetWriteArgbOrder( UINT32 u4Aorder, UINT32 u4Rorder, UINT32 u4Gorder, UINT32 u4Border)
{
...
static void _GFX_Fltr_CbCr(GFX_FLTR_CBCR_T *pt_cbcr)
{
    ...
    /* pass 2: filter cr*/
    REG(fg_SRC_BSAD)    = (pt_cbcr->ui4_cbcr_addr+1);
...
#if defined(CC_MT5368)||defined(CC_MT5396)|| defined(CC_MT5389)|| defined(CC_MT5398)||defined(CC_MT5880)
void _VGFX_GeneralAlphaComSet( UINT32 u4ImgCm, UINT32 u4WbCm, UINT32 u4Ar, UINT32 u4OpCode, BOOL fgAlcomNormal,
    ...
    UINT32 u4AlphaMethod)
{
    ...
    REG(fg_SRC_BSAD_MSB)    = GFX_REG_ADDR_MSG((UINT32)(pu1SrcBase));
    REG(fg_SRC_CM)    = u4SrcCM;
    REG(fg_SRC_PITCH) = (u4SrcPitch >> 4);

    //destination base address
    REG(fg_DST_BSAD)        = (UINT32)(pu1DstBase);
    REG(fg_DST_BSAD_MSB)    = GFX_REG_ADDR_MSG((UINT32)(pu1DstBase));
    REG(fg_CM)        = u4SrcCM;
    REG(fg_OSD_WIDTH) = (u4DstPitch >> 4);
...
void _GFX_MMU_ReInit(void)
{
    if(!_fgGfxMmuInited)
    {
        _GFX_MMU_Init();
        _fgGfxMmuInited = TRUE;     
    }
    else
    {
        _GFX_MMU_Set_Agent0(GFX_MMU_AGENT_EN, GFX_MMU_AGENT_MID0);
...
UINT32 GFX_GetMmuTTB(void)
{
    UINT32 ui4_tb_addr  =0x0;


    #if  (defined(CC_MT5368)||defined(CC_MT5396)|| defined(CC_MT5389)|| defined(CC_MT5398)||defined(CC_MT5880))
    #ifdef __KERNEL__   
    __asm__ ("MRC     p15, 0, %0, c2, c0, 0" : "=r" (ui4_tb_addr));
    #endif
    #else
    //__asm
    //{
        //MRC p15,0,ui4_tb_addr,c2,c0,0;
    //}
    #endif

    return (ui4_tb_addr&0xffffc000);
}

void _GFX_MMU_Set_Cmprss(UINT32 u4_enable)
{
    if(u4_enable)
    {
...
void _GFX_MMU_Set_Dst_MMU(UINT32 u4_enable)
{
    if(u4_enable)
    {
        _GFX_MMU_Set_Agent1(GFX_MMU_AGENT_EN, GFX_MMU_AGENT_MID4);
    ...
    {
        _GFX_MMU_Set_Agent1(GFX_MMU_DISABLE, GFX_MMU_AGENT_MID4);
        _GFX_MMU_Set_Agent2(GFX_MMU_DISABLE, GFX_MMU_AGENT_MID1);
    }
}

void _GFX_MMU_Set_Src_MMU(UINT32 u4_enable)
{
    if(u4_enable)
    {
        _GFX_MMU_Set_Agent0(GFX_MMU_AGENT_EN, GFX_MMU_AGENT_MID0);
    ...
    {
        _GFX_MMU_Set_Agent0(GFX_MMU_DISABLE, GFX_MMU_AGENT_MID0);
    }
}

void _GFX_MMU_Set_Compress_Osd_Cfg(void)
{
    GFX_MMU_T   t_gfx_mmu;
    
    t_gfx_mmu.u4_init       =0x0;
    t_gfx_mmu.u4_enable     =0x1;
    t_gfx_mmu.u4_cmprss     =0x1;
    t_gfx_mmu.u4_src_rw_mmu =0x1;
    t_gfx_mmu.u4_dst_rw_mmu =0x0;
    t_gfx_mmu.u4_vgfx_ord   =0x0;
    t_gfx_mmu.u4_pgt        =0x1;

    _GFX_MMU_Set_Cfg(&t_gfx_mmu);
...
void _GFX_MMU_Process(void)
{
    return;
}

void _GFX_MMU_UPD_PGT(void)
{
    #if 0
    struct thread_info * _pr_gfx_current_task = NULL;
    
    _pr_gfx_current_task = gfx_current_thread_info();
    
    Printf(" pgt[0x%x,0x%x,0x%x]\n",
        _pr_gfx_current_task->task->tgid,
        HalCpuGetPhyPgd());
    #endif
    #ifdef __KERNEL__  
    _GFX_MMU_Set_TabAddr(HalCpuGetPhyPgd());
    #endif
...
void _GFX_MMU_Set_Agent3(UINT32 u4Enable, UINT32  u4Mid)
{
    REG_MMU(fg_MMU_A3_EN) = u4Enable;
    REG_MMU(fg_MMU_A3_MID) = u4Mid;
    REG_MMU(fg_MMU_A3_PREFETCH) = GFX_MMU_PREFETCH_EN;
    REG_MMU(fg_MMU_A3_2D_PREFETCH) = GFX_MMU_2RD_PREFETCH_EN;
    GFX_MMU_CMDENQ(GREG_MMU_CFG3);
}
...
static void GFX_MMU_Enable_CmdQ(UINT32 u4_enable)
{
    ...
    if(u4_enable)
    {
        u4Value = (u4Value)|(GREG_MSK_MMU_EN_DRAMQ);
    }
    else
    {
        u4Value &=  (~GREG_MSK_MMU_EN_DRAMQ);
    }
...
static void _GFX_MMU_RISC_SelfFire(void)
{
    GFX_MMU_WRITE32(GREG_MMU_CFG4,(GFX_MMU_READ32(GREG_MMU_CFG4)|0x80000000));
}

static void _GFX_MMU_RISC_Reset_INT(void)
{
    GFX_MMU_WRITE32(GREG_MMU_CFG4,(GFX_MMU_READ32(GREG_MMU_CFG4)|0x5));
    GFX_MMU_WRITE32(GREG_MMU_CFG4,(GFX_MMU_READ32(GREG_MMU_CFG4)&0xFFFFFFFA));
}

void _GFX_MMU_ERROR_Action(void)
{
    GFX_MMU_Enable_CmdQ(0);
    _GFX_MMU_RISC_SelfFire();
    _GFX_MMU_RISC_Reset_INT();
    GFX_MMU_Enable_CmdQ(1);
    return;
}

static void _GFX_CHG_RISC_Fire(void)
{
    GFX_WRITE32(GFX_REG_G_MODE, GFX_READ32(GFX_REG_G_MODE)|0x800);
}

void _GFX_CHG_RISC_Flush(void)
{
...
void GFX_MMU_Check_Status(void)
{
    ...
    u4Status = GFX_MMU_Get_Status();

    if(u4Status&GREG_MSK_MMU_PGT_ERROR)
    {
        #ifdef LINUX_TURNKEY_SOLUTION
        printk(KERN_ERR"gfx mmu error: pid[%d],tgid[%d],mmu status[0x%x],gfx op[0x%x],fill color[0x%x]\n",
                current->pid,
                current->tgid,
                u4Status,
                GFX_HwGetRegOpMode(),
                GFX_HwGetRegFillColor()
                );
        #else
        printk(KERN_ERR"gfx mmu error: mmu status[0x%x],gfx op[0x%x],fill color[0x%x]\n",
                u4Status,
                GFX_HwGetRegOpMode(),
                GFX_HwGetRegFillColor()
                );
        ...
        printk(KERN_ERR"src[0x%x],sw[%d],sh[%d],spitch[0x%x],scm[%d]\n",
                GFX_HwGetRegSrcBase(),
                GFX_HwGetRegSrcWidth(),
                GFX_HwGetRegSrcHight(),
                GFX_HwGetRegSrcPitch(),
                GFX_HwGetRegSrcCm()
                );
        printk(KERN_ERR"dst[0x%x],dpitch[0x%x],dcm[%d]\n",
                GFX_HwGetRegDstBase(),
                GFX_HwGetRegDstPitch(),
                GFX_HwGetRegDstCm()
                );
        #if 0
        while(u1_gfx_dbg)
...
void GFX_MMU_Risc_Self_Fire_Clr(void)
{
    UINT32 u4Value = 0x0;
#define GFX_REG_OFFSET_CFG4     (0x10)
#define GFX_SELF_FIRE_CLR_MSK   (0x40000000)
 
    GFX_MMU_Enable_CmdQ(FALSE);
    u4Value = GFX_MMU_READ32(GFX_REG_OFFSET_CFG4);
    GFX_MMU_WRITE32(GFX_REG_OFFSET_CFG4,(u4Value|GFX_SELF_FIRE_CLR_MSK));
    GFX_MMU_WRITE32(GFX_REG_OFFSET_CFG4,(u4Value&(~(GFX_SELF_FIRE_CLR_MSK))));   
    GFX_MMU_Enable_CmdQ(TRUE);
    return ;
}
...
void GFX_MMU_Check_Cfg(void)
{
#define GREG_MSK_MMU_SFIRE_RST  ((UINT32)0x20000000)

    while(GREG_MSK_MMU_SFIRE_RST&GFX_MMU_Get_Status())
    {
        x_thread_delay(1);
        if(GREG_MSK_MMU_SFIRE_RST&GFX_MMU_Get_Status())
        {
            GFX_MMU_Risc_Self_Fire_Clr();
        }
    }
...
{  
    #if 0
    Printf("MMU CFG[%d,%d,%d,%d,%d,%d,%d]\n",
            pt_gfx_mmu->u4_init,
            pt_gfx_mmu->u4_enable,
            pt_gfx_mmu->u4_src_rw_mmu,
            pt_gfx_mmu->u4_dst_rw_mmu,
            pt_gfx_mmu->u4_cmprss,
            pt_gfx_mmu->u4_vgfx_opt,
            pt_gfx_mmu->u4_pgt
            );
    #endif
    ...
    if(pt_gfx_mmu->u4_vgfx_ord)
    {
        _GFX_MMU_Set_OverRead(1,0,(pt_gfx_mmu->u4_vgfx_slva&0xFFFFF000)>>12);
    }
...
void GFX_InvalidateDCacheMultipleLine(void)
{   
    #ifdef __KERNEL__    
    HalInvalidateDCacheMultipleLine(VIRTUAL(_u4DirectFBAddr), _u4DirectFBSize);              
    #endif
