...
///////////////////////////////////////////////////////////////////////////
// TunerDVBTOpenStatus & TunerDVBCOpenStatus
// The API for vestel ATV only
// 
///////////////////////////////////////////////////////////////////////////
void TunerATVOpenStatus(UINT8 AStatus)
{
      u1TStatus=AStatus;
	  mcSHOW_DRVERR_MSG(("ATV status is %d\n",u1TStatus));
	  return;
}
/*---------------
...
//    u1DbgLevel = 2 | mcBIT(cpMSG_MONI);
    ...
    for (i=0;i<tTunerCfgTbl.TunerNum;i++){
		...
		if ((res != DRVAPI_TUNER_OK)&&(1==u1TStatus))
			{
            x_mem_free(ptTDCtx);
			continue;
		}
            ...
            x_break_fct _BreakFct, void* pvArg)
{
    ...
    { // Enter TunerDummy mode
        mcSHOW_USER_CRI_MSG(("## TunerOpenFailure ##\n"));
        return fgRetSts;
    }
...
INT16 TunerGetSync(UINT16 TunerID, DRV_CONN_TYPE_T e_conn_type)
{
    ...
    if (bTunerOpenFailure){ // Enter TunerDummy mode        
        mcSHOW_USER_MSG(("## TunerOpenFailure ##\n"));
        return 1;
    }
...
INT32 TunerNimTest(UINT16 TunerID)
{
    ...
    if (bTunerOpenFailure){ // Enter TunerDummy mode        
        mcSHOW_DRVERR_MSG(("## TunerOpenFailure ##\n"));
        return DRVAPI_TUNER_ERROR;
    }
