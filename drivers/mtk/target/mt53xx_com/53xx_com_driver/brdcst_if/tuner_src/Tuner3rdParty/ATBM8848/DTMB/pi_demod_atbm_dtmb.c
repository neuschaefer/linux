...
{
 {0x0009,0x01},//1 bit: 1:Enable DSP self-reset when fatal error occur in DTMB mode,disable
 {0x000A,0x01},//1 bit: 1:Enable DSP self-reset when non-fatal error occur in DTMB mode,disable
...
{
  return  ATBM_Init(pDemodCtx->u1I2cAddr) & ATBMWriteDemodData(pDemodCtx->u1I2cAddr,DTMB_IniTbl,sizeof(DTMB_IniTbl)/sizeof(DTMB_IniTbl[0]));
}
//Frame error ratio
BOOL ATMB_DTMB_GetFER(pDTMB_Demod_Ctx pDemodCtx,UINT16 *u2ErrFrameCount,UINT16 * u2TotalFrameCount)
{
 BOOL fgRet=TRUE;
  ...
  fgRet&=ATBMRead(pDemodCtx->u1I2cAddr,0x0B04,&u1Data);//Number of uncorrectable frames  in the window defined in 0x0B0A,0x0B09
...
BOOL ATBM_DTMB_GetSNR(pDTMB_Demod_Ctx pDemodCtx,UINT16 *u2SNR)
{
 BOOL fgRet=TRUE;
 UINT8 u1InstanReadFlag=1;
 UINT8 u1PNNo,u1snr_noise_sc_sel,u1Tmp_noise,u1signal_l,u1signal_m,u1signal_h,u1noise_l,u1noise_m,u1noise_h;
 ...
 if((u1snr_noise_sc_sel==0) && (u1Tmp_noise!=3))
 {
