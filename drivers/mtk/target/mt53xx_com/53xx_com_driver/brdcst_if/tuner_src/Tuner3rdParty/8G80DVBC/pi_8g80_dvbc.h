...
#ifndef PI_8G80_DVBC_H
#define PI_8G80_DVBC_H
...
enum
{
    DVBC_HIR_PRIORITY_UNKNOWN = 0,
    DVBC_HIR_PRIORITY_NONE,
    DVBC_HIR_PRIORITY_HIGH,
    DVBC_HIR_PRIORITY_LOW
};
...
typedef struct
{
    ...
    UINT32  u4RF_Freq;
    VOID    *psDemodCtx;
    struct  _TUNER_FUNC_T *psTunerFunc;
    void    *psI2cCtx;

    // Porting from MT5133. They can be merged??
    UCHAR   m_Address;  
    UINT8      m_SAW_BW;
    UINT32     m_Freq;         // Channel center frequency; carrier frequency(kHz)
    UINT16     m_IF_Freq;      // momo, 060523, should be fixed-point in C
    UINT8      m_TV_Spec;      // momo, 071112, for combo tuner
    UCHAR   m_aucPara[60];
    UINT8      m_Ana_Top;      // JC, 071227
    INT32     m_s4FreqBoundUpper;   
    INT32     m_s4FreqBoundLower;   
    UINT8      m_DemodVdd;
    UINT8      m_Xtal;         //0: 27M, 1: 26M
    INT8      m_SigLvScan;
    UINT8      m_SigLvTh;
//      m_aucPara[0] : tuner TOP
//      m_aucPara[1] : Reserved for tuner
//      m_aucPara[2] : AGC_IF_SLP
//      m_aucPara[3] : AGC_IF_INT
//      m_aucPara[4] : AGC_IF_MAX
//      m_aucPara[5] : AGC_IF_MIN
} RF_TUNER_CTX_T;
...
// Hardware Configure
    ...
    UINT32             u4ErrRsBlk;
    UINT8              u1LdpcItNum;
    UINT32             u4TotLdpcBlk_SLCH[cMAX_TS_CNT];
    UINT32             u4ErrLdpcBlk_SLCH[cMAX_TS_CNT];
    UINT32             u4TotRsBlk_SLCH[cMAX_TS_CNT];
    UINT32             u4ErrRsBlk_SLCH[cMAX_TS_CNT];
    RECEIVE_MODE_T  u1ReceiveMode;
