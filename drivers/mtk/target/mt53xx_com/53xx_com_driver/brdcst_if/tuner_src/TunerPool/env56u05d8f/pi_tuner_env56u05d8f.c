 ...
 * $Author: dtvbm11 $
 * $Date: 2012/04/27 $
 * $RCSfile: pi_tuner_ENV56U05D8F.c,v $
 * $Revision: #1 $
 *
 *---------------------------------------------------------------------------*/

/** @file pi_tuner_ENV56U05D8F.c
 *  Tuner control for SS ENV56U05D8F
 */
...
#include "eq_script_env56u05d8f.h"


//-----------------------------------------------------------------------------
// Constant definitions
//-----------------------------------------------------------------------------
#define PANASSONIC_ENV56U05D8F_BASE            ((UINT16) 0xC2)
#define PANASSONIC_ENV56U05D8F_IF_FREQUENCY_ANA    ((UINT16) 45750)   /* KHz */
#define PANASSONIC_ENV56U05D8F_IF_FREQUENCY    ((UINT16) 44000)   /* KHz */
#define PANASSONIC_ENV56U05D8F_LO_STEP         ((UINT16) 6250)  /* 10Hz */
#define PANASSONIC_ENV56U05D8F_ANA_LO_STEP         ((UINT16) 6250)  /* 10Hz */
#define PANASSONIC_ENV56U05D8F_STR             "PANASSONIC_ENV56U05D8F 2010-01-01\n 2011-04-06\n v1.0"

//PLL lock check parameters
#define PANASSONIC_ENV56U05D8F_PLL_POLL_INTERVAL      10 //ms
#define PANASSONIC_ENV56U05D8F_PLL_POLL_TIMETOUT      100 //ms
#define PANASSONIC_ENV56U05D8F_PLL_POLL_CNT           PANASSONIC_ENV56U05D8F_PLL_POLL_TIMETOUT/PANASSONIC_ENV56U05D8F_PLL_POLL_INTERVAL //counter
...
STATIC UINT8* pENV56U05D8FEqNormal[MOD_ANA_TYPE_END - MOD_ANA_TYPE_BEGIN] = {NULL};
STATIC UINT8* pENV56U05D8FEqWeak[MOD_ANA_TYPE_END - MOD_ANA_TYPE_BEGIN] = {NULL};

STATIC UINT8 u1AnaTop=4;
//
/** ENV56U05D8F_TunerInit
 *  Tuner initialzation forENV56U05D8F.
 *  @param  pTCtx       Pointer of pi tuner driver context.
 *  @retval void
 */
//-----------------------------------------------------------------------------
VOID ENV56U05D8F_TunerInit(
    ITUNER_CTX_T *pTCtx
    )
{
    ...
    psTunerCtx->I2cAddress = PANASSONIC_ENV56U05D8F_BASE;
    psTunerCtx->u4RF_Freq = 0;
    psTunerCtx->fgRFAGC = FALSE;
    /* Specific part: Only for this tuner model */
    psTunerCtx->u2IF_Freq = PANASSONIC_ENV56U05D8F_IF_FREQUENCY;
    psTunerCtx->u2IF_Freq_A = PANASSONIC_ENV56U05D8F_IF_FREQUENCY_ANA;
    ...
    // init EQ script
    pENV56U05D8FEqNormal[MOD_NTSC_M - MOD_ANA_TYPE_BEGIN]  = EQENV56U05D8FMN;
    pENV56U05D8FEqWeak[MOD_NTSC_M - MOD_ANA_TYPE_BEGIN]  = EQENV56U05D8FMNWeak;

    //pENV56U05D8FEqNormal[MOD_NTSC_M_54M_44M]  = EQPANASSONICENV56U05D8FMN;
    //pENV56U05D8FEqNormal[MOD_NTSC_M_54M_36M]  = EQPANASSONICENV56U05D8FMN;
   psTunerSpecificCtx->u2LO_Step=PANASSONIC_ENV56U05D8F_ANA_LO_STEP;
   
    mcSHOW_USER_MSG(("Init " PANASSONIC_ENV56U05D8F_STR "\n"));
}

//----------------------------------------------------------------------------- 
/**  ENV56U05D8F_TunerGetStatus
 *  Set Tuner PLL staus forENV56U05D8F/ENV56U05D8F
 *  @param  pTCtx       Pointer of pi tuner driver context.
 *  @retval TRUE        PLL lock
 *  @retval FALSE       PLL unlock
 */
//-----------------------------------------------------------------------------
STATIC BOOL ENV56U05D8F_TunerGetStatus(ITUNER_CTX_T *pTCtx)
{
    ITUNER_CTX_T *psTunerCtx = pTCtx;
...
//----------------------------------------------------------------------------- 
/** ENV56U05D8F_TunerSetFreq
 *  Set Tuner PLL forENV56U05D8F/ENV56U05D8F to tune RF frequency.
 *  @param  pTCtx       Pointer of pi tuner driver context.
 ...
 */
//-----------------------------------------------------------------------------
INT16 ENV56U05D8F_TunerSetFreq(
    ITUNER_CTX_T *pTCtx,
...
u1Mod=param->Modulation;
   ...
   mcSHOW_DBG_MSG(("PANASSONIC ENV56U05D8F_TunerSetFreq %dKHz\n",param->Freq));
  if ( u1Mod>= MOD_ANA_TYPE_BEGIN )
    ...
    if (fgAnalogSignal){ //analog IF 45.75MHz    
        Lo = freq + psTunerCtx->u2IF_Freq_A;
        }
    else{                    //digital IF 44MHz
        Lo = freq + psTunerCtx->u2IF_Freq;
        }


    if(!fgAnalogSignal){//digital  mode use digital LO step
      Ndivider = (UINT16) ((Lo * 100) / PANASSONIC_ENV56U05D8F_LO_STEP);
    }
    else{
     Ndivider = (UINT16) ((Lo * 100) /PANASSONIC_ENV56U05D8F_ANA_LO_STEP);
    }
...
/* Assign Control word for specific tuner */
    if (freq < 53 || freq > 865)
    {
       ...
       return (+1);
	     ...
	     CB1 = 0xA3;//Stepsize 62.5KHz  Top 100 104dBuv
	if(fgAnalogSignal)
    {
	  CB1&=0xC7;
	  CB1|=(u1AnaTop<<3);
...
/* LO settings */
 ...
 /* Write to tuner device */
	...
	for(ii=0;ii<PANASSONIC_ENV56U05D8F_PLL_POLL_CNT;ii++)
	{
	  mcDELAY_MS(PANASSONIC_ENV56U05D8F_PLL_POLL_INTERVAL);
	  if(ENV56U05D8F_TunerGetStatus(psTunerCtx))break;
	}
	
	if(ii<PANASSONIC_ENV56U05D8F_PLL_POLL_CNT)
	{
	...
	else
	{
	 mcSHOW_HW_MSG(("PLL unlock !(%d)",ii));
        ...
        if(fgAnalogSignal){
            psTunerCtx->u4RF_Freq=(Ndivider*PANASSONIC_ENV56U05D8F_ANA_LO_STEP)/100-psTunerCtx->u2IF_Freq_A;
        }
        else{
            psTunerCtx->u4RF_Freq=(Ndivider*PANASSONIC_ENV56U05D8F_LO_STEP)/100-psTunerCtx->u2IF_Freq;
        }
 ...
 */
//-----------------------------------------------------------------------------
STATIC CHAR *ENV56U05D8F_TunerGetVer(VOID)
{
    return (PANASSONIC_ENV56U05D8F_STR);
}
/***********************************************************************/
//----------------------------------------------------------------------------- 
 ...
 */
//-----------------------------------------------------------------------------
STATIC INT16 ENV56U05D8F_SignalLevel_Handler(ITUNER_CTX_T * pTCtx,CMD_SIGLVL_PARAM_T * rCmd){
     INT16 ret=ITUNER_OK;
 ...
 */
//-----------------------------------------------------------------------------
INT16 ENV56U05D8F_TunerOP(ITUNER_CTX_T * pTCtx, ITUNEROP_T eOperation, UINT8 SetVal,  VOID * pInOutVal){
         
    switch(eOperation){
    case itGetStatus:
        {
        BOOL * Retval=(BOOL *)pInOutVal;
        *Retval=ENV56U05D8F_TunerGetStatus(pTCtx);
        }
        break;
    case itGetVer:
        {
        CHAR ** RetStr=(CHAR **)pInOutVal;
        *RetStr=ENV56U05D8F_TunerGetVer();
        }
        break;
    case itGetSignalLevel:
        {
        CMD_SIGLVL_PARAM_T * SigLvlParam=(CMD_SIGLVL_PARAM_T *)pInOutVal;
        return ENV56U05D8F_SignalLevel_Handler(pTCtx,SigLvlParam);
        }

    case itGetEqScriptNormal: 
        *(UINT8**)pInOutVal = pENV56U05D8FEqNormal[SetVal];
        break;

    case itGetEqScriptWeak: 
        *(UINT8**)pInOutVal = pENV56U05D8FEqWeak[SetVal];
        break;
