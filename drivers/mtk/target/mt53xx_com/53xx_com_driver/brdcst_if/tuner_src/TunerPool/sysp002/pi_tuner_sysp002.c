 ...
 * $Author: dtvbm11 $
 * $Date: 2012/04/27 $
 * $RCSfile: pi_tuner_SYSP002.c,v $
 * $Revision: #1 $
 *
 *---------------------------------------------------------------------------*/

/** @file pi_tuner_SYSP002.c
 *  Tuner control for SSSYSP002
 */
...
#include "eq_script_sysp002_5366.h"


//-----------------------------------------------------------------------------
// Constant definitions
//-----------------------------------------------------------------------------
#define SANYO_SYSP002_BASE            ((UINT16) 0xC0)
#define SANYO_SYSP002_IF_FREQUENCY_ANA    ((UINT16) 45750)   /* KHz */
#define SANYO_SYSP002_IF_FREQUENCY    ((UINT16) 44000)   /* KHz */
#define SANYO_SYSP002_LO_STEP         ((UINT16) 6250)  /* 10Hz */
#define SANYO_SYSP002_ANA_LO_STEP         ((UINT16) 3125)  /* 10Hz */
#define SANYO_SYSP002_STR             "SANYO_SYSP002 2010-01-01\n 2011-04-07\n v1.0"

//PLL lock check parameters
#define SANYO_SYSP002_PLL_POLL_INTERVAL      10 //ms
#define SANYO_SYSP002_PLL_POLL_TIMETOUT      150 //MAX 150ms SPEC
#define SANYO_SYSP002_PLL_POLL_CNT           SANYO_SYSP002_PLL_POLL_TIMETOUT/SANYO_SYSP002_PLL_POLL_INTERVAL //counter
...
STATIC UINT8* pSYSP002EqNormal[MOD_ANA_TYPE_END - MOD_ANA_TYPE_BEGIN] = {NULL};
STATIC UINT8* pSYSP002EqWeak[MOD_ANA_TYPE_END - MOD_ANA_TYPE_BEGIN] = {NULL};

//
/**SYSP002_TunerInit
 *  Tuner initialzation forsysp002.
 *  @param  pTCtx       Pointer of pi tuner driver context.
 *  @retval void
 */
//-----------------------------------------------------------------------------
VOID SYSP002_TunerInit(
    ITUNER_CTX_T *pTCtx
    )
{
    ...
    psTunerCtx->I2cAddress = SANYO_SYSP002_BASE;
    psTunerCtx->u4RF_Freq = 0;
    psTunerCtx->fgRFAGC = FALSE;
    /* Specific part: Only for this tuner model */
    psTunerCtx->u2IF_Freq = SANYO_SYSP002_IF_FREQUENCY;
    psTunerCtx->u2IF_Freq_A = SANYO_SYSP002_IF_FREQUENCY_ANA;
    ...
    // init EQ script
    pSYSP002EqNormal[MOD_NTSC_M - MOD_ANA_TYPE_BEGIN]  = EQSYSP002MN;
    pSYSP002EqWeak[MOD_NTSC_M - MOD_ANA_TYPE_BEGIN]  = EQSYSP002MNWeak;

    //psysp002EqNormal[MOD_NTSC_M_54M_44M]  = EQSYSP002MN;
    //psysp002EqNormal[MOD_NTSC_M_54M_36M]  = EQSYSP002MN;
   psTunerSpecificCtx->u2LO_Step=SANYO_SYSP002_ANA_LO_STEP;
   
    mcSHOW_USER_MSG(("Init " SANYO_SYSP002_STR "\n"));
}

//----------------------------------------------------------------------------- 
/** SYSP002_TunerGetStatus
 *  Set Tuner PLL staus forsysp002/sysp002
 *  @param  pTCtx       Pointer of pi tuner driver context.
 *  @retval TRUE        PLL lock
 *  @retval FALSE       PLL unlock
 */
//-----------------------------------------------------------------------------
STATIC BOOL SYSP002_TunerGetStatus(ITUNER_CTX_T *pTCtx)
{
    ITUNER_CTX_T *psTunerCtx = pTCtx;
...
//----------------------------------------------------------------------------- 
/**SYSP002_TunerSetFreq
 *  Set Tuner PLL forsysp002/sysp002 to tune RF frequency.
 *  @param  pTCtx       Pointer of pi tuner driver context.
...
u1Mod=param->Modulation;
   ...
   mcSHOW_DBG_MSG(("LGSYSP002_TunerSetFreq %dKHz\n",param->Freq));
  if ( u1Mod>= MOD_ANA_TYPE_BEGIN )
    ...
    if(!fgAnalogSignal){//digital  mode use digital LO step
      Ndivider = (UINT16) ((Lo * 100) / SANYO_SYSP002_LO_STEP);
    }
    else{
     Ndivider = (UINT16) ((Lo * 100) /SANYO_SYSP002_ANA_LO_STEP);
    }
...
/* Assign Control word for specific tuner */
	...
	if(u1Mod==MOD_ATSC_8VSB)
	{
	 CB1=0x83;
	}
	else if(u1Mod==MOD_J83B_64QAM) //64QAM
	{
	 CB1=0x9B;
	}
	else if(u1Mod==MOD_J83B_256QAM)//256QAM
	{
	  CB1=0xA3;
	}
	else//Analog
	{
	    CB1=0xAC;//analog 31.25KHz step
	}
   CB2 = 0xD1;
    if (freq < 162)
        Nband = 0x41;
...
/* LO settings */
 ...
 /* Write to tuner device */
	...
	for(ii=0;ii<SANYO_SYSP002_PLL_POLL_CNT;ii++)
	{
	  mcDELAY_MS(SANYO_SYSP002_PLL_POLL_INTERVAL);
	  if(SYSP002_TunerGetStatus(psTunerCtx))break;
	}
	
	if(ii<SANYO_SYSP002_PLL_POLL_CNT)
	{
	...
	else
	{
	 mcSHOW_HW_MSG(("PLL unlock !(%d)",ii));
        ...
        if(fgAnalogSignal){
            psTunerCtx->u4RF_Freq=(Ndivider*SANYO_SYSP002_ANA_LO_STEP)/100-psTunerCtx->u2IF_Freq_A;
        }
        else{
            psTunerCtx->u4RF_Freq=(Ndivider*SANYO_SYSP002_LO_STEP)/100-psTunerCtx->u2IF_Freq;
        }
 ...
 */
//-----------------------------------------------------------------------------
STATIC CHAR *SYSP002_TunerGetVer(VOID)
{
    return (SANYO_SYSP002_STR);
}
/***********************************************************************/
//----------------------------------------------------------------------------- 
 ...
 */
//-----------------------------------------------------------------------------
STATIC INT16 SYSP002_SignalLevel_Handler(ITUNER_CTX_T * pTCtx,CMD_SIGLVL_PARAM_T * rCmd){
     INT16 ret=ITUNER_OK;
 ...
 */
//-----------------------------------------------------------------------------
INT16 SYSP002_TunerOP(ITUNER_CTX_T * pTCtx, ITUNEROP_T eOperation, UINT8 SetVal,  VOID * pInOutVal){
         
    switch(eOperation){
    case itGetStatus:
        {
        BOOL * Retval=(BOOL *)pInOutVal;
        *Retval=SYSP002_TunerGetStatus(pTCtx);
        }
        break;
    case itGetVer:
        {
        CHAR ** RetStr=(CHAR **)pInOutVal;
        *RetStr=SYSP002_TunerGetVer();
        }
        break;
    case itGetSignalLevel:
        {
        CMD_SIGLVL_PARAM_T * SigLvlParam=(CMD_SIGLVL_PARAM_T *)pInOutVal;
        return SYSP002_SignalLevel_Handler(pTCtx,SigLvlParam);
        }

    case itGetEqScriptNormal: 
        *(UINT8**)pInOutVal = pSYSP002EqNormal[SetVal];
        break;

    case itGetEqScriptWeak: 
        *(UINT8**)pInOutVal = pSYSP002EqWeak[SetVal];
        break;
