 ...
 * $Author: dtvbm11 $
 * $Date: 2012/04/27 $
 * $RCSfile: pi_tuner_tdqu6x521a.c,v $
 * $Revision: #1 $
 *
 *---------------------------------------------------------------------------*/
...
//-----------------------------------------------------------------------------
#define ALPS_TDQU6X521A_BASE            ((UINT16) 0xC2)
#define ALPS_TDQU6X521A_IF_FREQUENCY    ((UINT16) 44)   /* MHz */
#define ALPS_TDQU6X521A_LO_STEP         ((UINT16) 625)  /* 100Hz */
#define ALPS_TDQU6X521A_STR             "ALPS_TDQU6X521A"
...
//----------------------------------------------------------------------------- 
/** TDQU6X521A_TunerInit
 *  Tuner initialzation for TDQU6X521A.
 *  @param  psTunerCtx  Pointer of RF tuner driver context.
 *  @retval void
 */
//-----------------------------------------------------------------------------
VOID TDQU6X521A_TunerInit(RF_TUNER_CTX_T *psTunerCtx, UINT8 I2cAddress)
{
    ...
    psTunerCtx->u1TunerType = cALPS_TDQU6X521A;
    psTunerCtx->u2IF_Freq = ALPS_TDQU6X521A_IF_FREQUENCY;
    psTunerCtx->u2LO_Step = ALPS_TDQU6X521A_LO_STEP;
    psTunerCtx->u2Ana_LO_Step = psTunerCtx->u2LO_Step;

    psTunerCtx->m_aucFunc[TUNER_FUNC_EN_TARGET_LEVEL] = TRUE;
    psTunerCtx->m_aucPara[TUNER_PARA_TARGET_LEVEL_MSB] = 0x30;
    psTunerCtx->m_aucPara[TUNER_PARA_TARGET_LEVEL_LSB] = 0x00;   

    mcSHOW_DBG_MSG(("Init " ALPS_TDQU6X521A_STR "\n"));
}


//----------------------------------------------------------------------------- 
/** TDQU6X521A_TunerSetFreq
 ...
 */
//-----------------------------------------------------------------------------
INT16 TDQU6X521A_TunerSetFreq(RF_TUNER_CTX_T *psTunerCtx, UINT32 Freq, UINT8 u1Mod, UINT16 u2Step)
{
UINT32  Lo;
UINT8   Nband;
UINT16  Ndivider;
...
UINT8   TunerS[6], TunerDataCnt;
    ...
    mcSHOW_DBG_MSG(("ALPS TDQU6X521A_TunerSetFreq\n"));
...
//----------------------------------------------------------------------------- 
/**  TDQU6X521A_TunerGetStatus
 *  Set Tuner PLL staus for TDQU6X521A
 *  @param  psTunerCtx  Pointer of RF tuner driver context.
 *  @retval TRUE        PLL lock
 *  @retval FALSE       PLL unlock
 */
//-----------------------------------------------------------------------------
BOOL TDQU6X521A_TunerGetStatus(RF_TUNER_CTX_T *psTunerCtx)
{
UINT8 TunerS[4];
...
//----------------------------------------------------------------------------- 
/** TDQU6X521A_TunerGetVer
 *  Set Tuner PLL staus for TDQU6X521A.
 *  @param  psTunerCtx  Pointer of RF tuner driver context.
 *  @retval tuner model name
 */
//-----------------------------------------------------------------------------
CHAR *TDQU6X521A_TunerGetVer(RF_TUNER_CTX_T *psTunerCtx)
{
    return (ALPS_TDQU6X521A_STR);
}
/***********************************************************************/
