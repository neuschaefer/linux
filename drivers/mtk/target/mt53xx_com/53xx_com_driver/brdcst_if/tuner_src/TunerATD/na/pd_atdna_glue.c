    	...
    	else
    	{
            ...
            {                                
                ...
                {                  
                    ...
                    ATD_GetReg(psAtdnaDemodCtx, REG_ATD_CTL_RST2, &ucData1, 1);
                    ucData1 = ucData1 & 0xFE;
                    ATD_SetReg(psAtdnaDemodCtx, REG_ATD_CTL_RST2, &ucData1, 1);
            ...
            else
            {
                ...
                {                     
                    ...
                    //ucData1 = 0x26;                    
                    //ATD_SetReg(psAtdnaDemodCtx, REG_ATD_PE_CFG0, &ucData1, 1);
                    //ucData1 = 0xCC;                    
                    //ATD_SetReg(psAtdnaDemodCtx, REG_ATD_PFLL_CFG, &ucData1, 1);
...
/* Move to Pi_demod_atd.c due to it seems PI-code, and both NA and EU are the same by EC_code_d100825
STATIC void SetFMRadio(UINT32  i4CurFreq)
    ...
    ucData1 = 0x6D;
    ATD_SetReg(psAtdnaDemodCtx, REG_ATD_PJ_CFG, &ucData1, 1);
    ucData1 = 0xC9;
    ATD_SetReg(psAtdnaDemodCtx, REG_ATD_PFLL_CFG, &ucData1, 1);
    ucData1 = 0x00;
    ATD_SetReg(psAtdnaDemodCtx, REG_ATD_CPR_CFO_CFG, &ucData1, 1);
    ucData1 = 0x00;
    ATD_SetReg(psAtdnaDemodCtx, REG_ATD_AVS_AOH, &ucData1, 1);
    ucData1 = 0x20;
    ATD_SetReg(psAtdnaDemodCtx, REG_ATD_AVS_AOL, &ucData1, 1);
    ucData1 = 0x81;
    ATD_SetReg(psAtdnaDemodCtx, REG_ATD_AUC_CFG1, &ucData1, 1);
    ucData1 = 0xA1;
    ATD_SetReg(psAtdnaDemodCtx, REG_ATD_AUC_CFG2, &ucData1, 1);
                     ...
                     PTD_SPECIFIC_CTX* pptTDSpecificCtx)
{
    ...
    *pptTDSpecificCtx = psAtdPdCtx;
    ...
    //mcSHOW_USER_MSG(("ATD_Initialize successful (1)\n"));
...
//EC_code_d100825: For MT5365/95 FM Radio flow modification: 
	...
	if(u1ConnMode==ATV_CONN_MODE_CHAN_SCAN)//d110921_Haibo:Move here to fix channel scan time expand issue [DTV00357794]
	{
        ...
        switch (ucCmdId)
        {
            ...
            case 's':
                ...
                //    _SubSysIdx = StrToInt(aszArgv[1]);
                    ...
                    //{
                        psAtdnaDemodCtx->u1SubSysIdx = u1SubSysIdx + MOD_ANA_TYPE_BEGIN;    //modified for get the correct   s4DsmOutFreq
                        i4CurFreq = psAtdPdCtx->Frequency;
					    ATD_SetFreq(psAtdnaDemodCtx, i4CurFreq, FALSE);
                        psAtdPdCtx->u1SubSysIdx = u1SubSysIdx + MOD_ANA_TYPE_BEGIN;
                        ATD_ChipInit(psAtdnaDemodCtx);    //// make sure ATD in normal mode after nim.go dvbt
                        ATD_SetSystem(psAtdnaDemodCtx, psAtdPdCtx->u1SubSysIdx);
            ...
            case 'v':
                ...
                mcSHOW_USER_MSG(("VIF Lock: %d (Freq = %dKHz)\n",
                                 ...
                                 i4CurFreq));
                 mcSHOW_USER_MSG(("TVD Lock: %d\n", fgGetTVDSts(psAtdnaDemodCtx,CHN_LOCK_CHECK_TIME)));                 
                //mcDELAY_MS(10);
                //}
		    ...
		    else if('d'==((CHAR *) aszArgv[0])[1])
			{
              ...
              mcSHOW_USER_MSG(("Re-try TVD delay = %d ms\n",psAtdnaDemodCtx->pTCtx->u2RetryTVDTm)); 
                ...
                ATD_GetReg(psAtdnaDemodCtx, REG_ATD_ACR_CFG, &ucData1, 1);             
                ucData1 = ucData1 & 0xCF;            
                ATD_SetReg(psAtdnaDemodCtx, REG_ATD_ACR_CFG, &ucData1, 1);  //turn-off PN & FN auto-config            

                ucData1 = 0x30 + ucData2;
                ATD_SetReg(psAtdnaDemodCtx, REG_ATD_TRAN_STG, &ucData1, 1);
                ATD_SetReg(psAtdnaDemodCtx, REG_ATD_PRR_PRFK, &ucData1, 1);
                ucData1 = 0x40 + ucData2;
                ATD_SetReg(psAtdnaDemodCtx, REG_ATD_PRR_TRAN, &ucData1, 1);
                ATD_SetReg(psAtdnaDemodCtx, REG_ATD_PRR_DAMRK, &ucData1, 1);
            ...
            {   
	    	  ...
	    	  UINT32 dRegTemp = 0;	
                ...
                for (ii=0; ii<3; ii++)
                {
                    ...
                    ATD_GetReg(psAtdnaDemodCtx, REG_ATD_SLD_PRA_34, &ucData1, 1);
                   mcSHOW_USER_MSG(("REG_ATD_SLD_PRA_34(0x2e4) = 0x%02X\n", ucData1));
#else
                    ATD_GetReg(psAtdnaDemodCtx, REG_ATD_PRA_ST, &ucData1, 1);
                    ...
                    if (s2Data < 0)
                    	s2Data = -s2Data;
                    Record[1] += s2Data;               	                    
                    ATD_GetReg(psAtdnaDemodCtx, REG_ATD_CR_STATE0, &ucData1, 1);
                    mcSHOW_USER_MSG(("REG_ATD_CR_STATE0(0x1a0) = 0x%02X\n", ucData1));
                    Record[2] += ucData1 & 0x3;
                    ATD_GetReg(psAtdnaDemodCtx, REG_ATD_CR_STATE1, &ucData1, 1);
            ...
            case 't':
			 if (i4Argc > 1)
                {
				    i4CurFreq = psAtdPdCtx->Frequency;
                    ucData2 = (UINT8)StrToInt(aszArgv[1]);//TOP
					if((ITuner_OP(psAtdnaDemodCtx->pTCtx, itSetTop, ucData2, NULL))==ITUNER_OK)
					{
					  ATD_SetFreq(psAtdnaDemodCtx, i4CurFreq, FALSE);
					  mcSHOW_USER_MSG(("Set TOP %d ok\n",ucData2)); 
					}
					else
					{
					  mcSHOW_USER_MSG(("Not support!\n")); 
			  ...
			  {
       			if((ITuner_OP(psAtdnaDemodCtx->pTCtx, itGetTop, 0, &ucData2))==ITUNER_OK)
					{
					  mcSHOW_USER_MSG(("Current tuner TOP=%d\n",ucData2)); 
					}  
			  }
			 break;
            default:
