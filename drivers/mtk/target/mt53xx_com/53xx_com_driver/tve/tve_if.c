            ...
            if(TVE_FirstReset2)
            {
                ...
                {	        
                    ...
                    if(bTVECounter2 > 5)
                    {
                        ...
                        TVE_SCPIP_W_STARTP();
                        #endif
                        LOG(6, "TVE detect field invert (Method2)\n");
                        LOG(6, "bTVEFieldInvert1 u2TVEWStartP=%d\n", u2TVEWStartP);
...
{
    LOG(3,"TVE_Resume\n");
    _fgTveResumeFlow = TRUE;
    TVE_Init();
    Bypass_HalClear2Default();
    Bypass_HalSetCommon();
    TVE_SetMode(TVE_1, TVE_MOD_COMPOSITE);
    TVE_SetFmt(TVE_1, TVE_FMT_576I);
    _fgTveResumeFlow = FALSE;
}
#endif
...
// Set TVE format
//--------------------------------------------*/
UINT32 TVE_SetTVMode(UCHAR ucTveId,UCHAR eTveMode)
{
    INT32 i4RetVal = TVE_SET_OK;
   TVE_HalSetTVMode(ucTveId,eTveMode);
    return (UINT32)i4RetVal;
}
