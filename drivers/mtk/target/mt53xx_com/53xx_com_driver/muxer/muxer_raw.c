...
static VOID _MUXER_RawMuxH264Es(UINT32 u4StartAddr, UINT32 u4FrameSize, UINT32 u4Pts)
{
...
static VOID _MUXER_RawMuxAudioEs(UINT32 u4StartAddr, UINT32 u4FrameSize, UINT32 u4Pts)
{
    UINT32 u4MxBufWpBak = _u4MxBufWp;
    ...
    VENC_PES_INFO_T *prVidPesInfo)
{
    ...
    {
        _MUXER_RawMuxH264Es(prVidPesInfo->u4StartAddr, prVidPesInfo->u4FrameSize, prVidPesInfo->u4Pts);
    }
    else
    {
        ...
        _MUXER_RawMuxH264Es((UINT32)_pucTmpFrmBuf, u4FrmSz, prVidPesInfo->u4Pts);
    }
...
{
    _MUXER_RawMuxAudioEs(prAudPesInfo->u4StartAddr, prAudPesInfo->u4FrameSize, prAudPesInfo->u4Pts);
...
MUXER_HDR_STATE_T _MUXER_HdrMuxRaw(UCHAR ucMxId)
{
    ...
    if (prRawMuxInfo->u4CurVidPts < prRawMuxInfo->u4CurAudPts && 0/*|| 1*/)  // video only for now
    {
#if 0    
        ...
        _MUXER_RawHandleVideoPes(prMuxInfo, &prMuxInfo->rVEncPesInfo);

        prMuxInfo->u4VidCnt++;
        //Printf("Video Count = (%ld) \n", prMuxInfo->u4VidCnt);

        prRawMuxInfo->u4CurVidPts = prMuxInfo->rVEncPesInfo.u4Pts;
        //HAL_GetTime(&rTimeRstE);
        //HAL_GetDeltaTime(&rTimeRstDt, &rTimeRstS, &rTimeRstE);
    ...
    else
    {
        ...
        prRawMuxInfo->u4CurAudPts = prMuxInfo->rAEncPesInfo.u4Pts;
    }
