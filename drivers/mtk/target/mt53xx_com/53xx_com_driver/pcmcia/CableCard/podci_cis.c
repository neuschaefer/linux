...
int _PODCI_PHYS_Tuple(unsigned char **pcis, int *len, int *first, int *itpcc_radr, int *dvb)
{
    ...
    *pcis += 2;
    ocis  += 2;
    LOG(7, "PHYS_Tuple: Len passed = %d   Link = %d\n", *len, link );
    switch( tuple )
    {
...
static CI_error_t _PODCI_PHYS_CheckCIS(UINT8 *cis, UINT16 len, UINT16 *addrcor, UINT8 *cor)
{
    ...
    if( IsAddrCor == 1  &&  IsCor == 1 ) {
        LOG(5, "PHYS_CheckCIS: ......CIS OK\n" );
        return CI_SUCCESS;
    } else {
        LOG(5, "PHYS_CheckCIS: ......CIS FAILURE  !!!!!\n" );
...
{
    CI_error_t           error;
    error = _PODCI_PHYS_CheckCIS(pu1Cis, 256 ,addrcor, cor);
    if ( error != CI_SUCCESS ) 
    {
