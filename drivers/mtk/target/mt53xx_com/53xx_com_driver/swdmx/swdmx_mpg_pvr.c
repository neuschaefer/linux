 ...
 */
//-----------------------------------------------------------------------------
static BOOL _SwdmxPVR_Flush(const DMX_PVRPLAY_PTR_T *prPtr)
{
...
BOOL _SWDMX_PVR_Init(void)
{
    ...
    {
        _arPVRPlayPid[i].fgEnable = FALSE;
        _arPVRPlayPid[i].u2PidNum = 0;
        _arPVRPlayPid[i].eType = DMX_PID_TYPE_NONE;
    }
    //-----------------------------------------------------------

    _eSWDMXPVRState = SWDMX_PVR_STATE_IDLE;
    _i4PVRSpeed = 1000;
    _u4Threshold = 0x800000;            // Not use now
 ...
 */
//-----------------------------------------------------------------------------
BOOL _SWDMX_PVR_GetInfo(SWDMX_PROGRAM_INFO_T* prOpenInfo)
{
    UINT32 i;
    ...
    for(i=0; i<DMX_NUM_PID_INDEX; i++)
    {
        ...
        if(_arPVRPlayPid[i].fgEnable)
        {
            if((prOpenInfo->rPgm[0].u2VideoNum == 0) 
               && (_arPVRPlayPid[i].eType == DMX_PID_TYPE_ES_VIDEO))
            {
                prOpenInfo->rPgm[0].u2VideoNum = 1;
                prOpenInfo->rPgm[0].au2VideoId[0] = _arPVRPlayPid[i].u2PidNum;
                prOpenInfo->rPgm[0].aeVideo[0] = ENUM_SWDMX_VDEC_MPEG1_MPEG2;   // temp
            }

            if((prOpenInfo->rPgm[0].u2AudioNum == 0) && 
               (_arPVRPlayPid[i].eType == DMX_PID_TYPE_ES_AUDIO))
            {
                prOpenInfo->rPgm[0].u2AudioNum = 1;
                prOpenInfo->rPgm[0].au2AudioId[0] = _arPVRPlayPid[i].u2PidNum;
               ...
               (_arPVRPlayPid[i].eType == DMX_PID_TYPE_NONE))
            {
                ...
                prOpenInfo->rPgm[0].au2PcrId[0] = _arPVRPlayPid[i].u2PidNum;
            }
        }
    }
    
    if(_u4PVRPlayTime == 0)     // temp
    {
        _u4PVRPlayTime = 20 * 60;
    }
    prOpenInfo->rPgm[0].u4TotalTime = _u4PVRPlayTime * 90000;//(20*60*90000);
...
{    
    ...
    rPlay.u4BufStart = rFeederInfo.u4StartAddr;
    rPlay.u4BufSize = rFeederInfo.u4EndAddr - rFeederInfo.u4StartAddr;    
    rPlay.fgContainTimeStamp = TRUE;
    rPlay.fgIgnoreTimeStamp = FALSE;
    rPlay.u2TimeStampFreqDiv = (UINT16)(8 & 0xFFFF);
    rPlay.eMode = DMX_PVR_PLAY_STREAM;
    rPlay.pfnPVRNotify = NULL;
    rPlay.u4ThresholdSize = _u4Threshold;
 ...
 */
//-----------------------------------------------------------------------------
SWDMX_PVR_SENDDATA_RET_T _SWDMX_PVR_SendData(const DMX_MM_DATA_T *prDmxMMData, UINT32 u4MinBuf, BOOL fgFastMode)
{
...
*/
    ...
    if(fgFastMode)
    {
        if(_eSWDMXPVRState != SWDMX_PVR_STATE_IDLE)
        {
            if(!_SWDMX_PVR_Stop())
            {
                return SWDMX_PVR_SENDDATA_FAIL;
    ...
    // Handle resume
    //----------------------------------------------------------------------
    if(_fgPVRMpgResume && (_eSWDMXPVRState == SWDMX_PVR_STATE_PAUSE))
    {
        _fgCheckHang = FALSE;
        ...
        rResumePtr.u4Wp = u4Wp;
        rResumePtr.u4Rp = _rPVRPauseBufPtr.u4Rp;
        ...
        _eSWDMXPVRState = SWDMX_PVR_STATE_PLAYING;
        _fgPVRMpgResume = FALSE;
    ...
    // Send Data
    //--------------------------------------------------------
    if(_eSWDMXPVRState != SWDMX_PVR_STATE_PLAYING)
    {
...
*/
    ...
    if((rPtr.u4HwRp == _rCurPtr.u4HwRp) && (rPtr.u4HwWp == _rCurPtr.u4HwWp))
    {
        //LOG(0, "xxx %d\n", (INT32)_fgCheckHang);
        if(!_fgCheckHang)
        {
            HAL_GetTime(&_rSwDmxPVRTime);
            _fgCheckHang = TRUE;
        }

        HAL_GetTime(&rTime);
        HAL_GetDeltaTime(&rDiffTime, &_rSwDmxPVRTime, &rTime);
        if((rDiffTime.u4Seconds> 0) || (rDiffTime.u4Micros > 500000))    // 0.5 sec
        //if(rDiffTime.u4Seconds > 0) // 1 sec
        {
            _fgCheckHang = FALSE;

            LOG(0, "%s, timeout. %d:06%d, %d:06%d, %d:06%d\n", __FUNCTION__,
                        rDiffTime.u4Seconds, rDiffTime.u4Micros,
                        _rSwDmxPVRTime.u4Seconds, _rSwDmxPVRTime.u4Micros,
    ...
    {
        _fgCheckHang = FALSE;
        _rCurPtr.u4HwRp = rPtr.u4HwRp;
        _rCurPtr.u4HwWp = rPtr.u4HwWp;
        HAL_GetTime(&_rSwDmxPVRTime);
    }
 ...
 */
//-----------------------------------------------------------------------------
BOOL _SWDMX_PVR_Pause(void)
{
    LOG(3, "%s\n", __FUNCTION__);

    if(_eSWDMXPVRState == SWDMX_PVR_STATE_PLAYING)
    {
        if(!DMX_PVRPlay_Pause(&_rPVRPauseBufPtr))
        {
            LOG(3, "%s pause fail!\n", __FUNCTION__);
            return FALSE;
        }

        _eSWDMXPVRState = SWDMX_PVR_STATE_PAUSE;
    }
 ...
 */
//-----------------------------------------------------------------------------
BOOL _SWDMX_PVR_Resume(void)
{
    _fgPVRMpgResume = TRUE;
 ...
 */
//-----------------------------------------------------------------------------
BOOL _SWDMX_PVR_FlushBuf(void)
{    
    LOG(3, "%s, state:%d\n", __FUNCTION__, (INT32)_eSWDMXPVRState);

    if(_eSWDMXPVRState == SWDMX_PVR_STATE_PLAYING)
    {
        // Get buffer self
        if(!_SwdmxPVR_Flush(NULL))
        {
            LOG(1, "%s:%d, Flush fail\n", __FUNCTION__, __LINE__);
        }
    }
    else if(_eSWDMXPVRState == SWDMX_PVR_STATE_PAUSE)
    {
        // Use pause buffer parameter
        if(!_SwdmxPVR_Flush(&_rPVRPauseBufPtr))
        {
            LOG(1, "%s:%d, Flush fail\n", __FUNCTION__, __LINE__);
        }
    }
...
BOOL _SWDMX_PVR_Stop(void)
{
    ...
    x_memset((void*)&_rPVRPauseBufPtr, 0 , sizeof(DMX_PVRPLAY_PTR_T));
    _eSWDMXPVRState = SWDMX_PVR_STATE_IDLE;
 ...
 */
//-----------------------------------------------------------------------------
BOOL _SWDMX_PVR_SetPid(UINT8 u1Pidx, UINT16 u2PidNum, DMX_PID_TYPE_T ePidType, 
                      BOOL fgEnable)
{
    ...
    if(fgEnable && _arPVRPlayPid[u1Pidx].fgEnable)
    {
        LOG(3, "Pidx %d already enable\n", u1Pidx);
        return FALSE;
    }

    _arPVRPlayPid[u1Pidx].fgEnable = fgEnable;
    _arPVRPlayPid[u1Pidx].u2PidNum = u2PidNum;
    _arPVRPlayPid[u1Pidx].eType = ePidType;
 ...
 */
//-----------------------------------------------------------------------------
void _SWDMX_PVR_SetTime(UINT32 u4TimeSec)
{
    _u4PVRPlayTime = u4TimeSec;
}
 ...
 */
//-----------------------------------------------------------------------------
BOOL _SWDMX_PVR_SetDataMode(INT32 i4PlaySpeed)
{    
    if(_i4PVRSpeed == i4PlaySpeed)
    {
        return TRUE;
    }

    // Speed x1 to others need flush PVR buffer data
    if((_i4PVRSpeed == 1000) && (i4PlaySpeed != 1000))
    {
        _fgPVRMpgResume = FALSE;

        // Flush data
        if(!_SWDMX_PVR_FlushBuf())
        {
            LOG(3, "%s, PVR flush fail\n", __FUNCTION__);
            return FALSE;
        }
    }
    else if((_i4PVRSpeed != 1000) && (i4PlaySpeed == 1000))
    {
        _fgPVRMpgResume = FALSE;
    }

    _i4PVRSpeed = i4PlaySpeed;
