...
static VOID _SIF_WRITE32(UINT32 u4Offset, UINT32 u4Value)
{
    ...
    {
        IO_WRITE32(BIM_BASE, (u4Offset + 0x30), (u4Value));
    }
...
static UINT32 _SIF_READ32(UINT32 u4Offset)
{
    ...
    {
        return IO_READ32(BIM_BASE, (u4Offset + 0x30));
    }
    else if (_rSifDevParm.fgPdwncI2C)
    {
        if ((u4Offset == RW_SM0_CTRL0_REG) || (u4Offset == RW_SM0_CTRL1_REG))
...
static VOID _SIF_WRITE32MSK(UINT32 u4Offset, UINT32 u4Value, UINT32 u4Mask)
{
    ...
    {
        IO_WRITE32MSK(BIM_BASE, (u4Offset + 0x30), (u4Value), (u4Mask));
    }
    else if (_rSifDevParm.fgPdwncI2C)
    {
        if ((u4Offset == RW_SM0_CTRL0_REG) || (u4Offset == RW_SM0_CTRL1_REG))
...
static VOID _SIF_TimeOutReset(VOID)
{
    ...
    {	 
      u4Reg = SIF0_SW_RST;
      u4Msk = SIF0_SW_RST_MSK;
      IO_WRITE32MSK(BIM_BASE, RW_PERMISC_REG, u4Reg, u4Msk);
...
// Lock SIF semaphore
static VOID _SIF_UserLockTimeout(UINT32 u4Timeout)
{
    SIF_Init();
#if SIF_USE_OSAI
    // wait semaphore
    VERIFY(x_sema_lock_timeout(_hSifUserSema, u4Timeout) == OSR_OK);
#endif // SIF_USE_OSAI
}
...
#if SIF_USE_OSAI
#ifdef CC_SIF_USE_INTERRUPT_MODE
        if(x_sema_lock_timeout(_hSifIsrSema, SIF_TIMEOUT_MAX)!=OSR_OK)
        {
            LOG(3,"Sif isr semaphore lock timeout\n");
            _SIF_TimeOutReset();
...
static INT32 _SIF_PutByte(UINT8 u1Data)
{
    ...
    VERIFY(x_sema_lock(_hSifIsrSema, X_SEMA_OPTION_NOWAIT) == OSR_WOULD_BLOCK);
#endif // CC_SIF_USE_INTERRUPT_MODE
#endif // SIF_USE_OSAI
...
static VOID _SIF_Control(VOID)
{
    ...
    u4Msk = (UINT32) (CLK_DIV_MSK | ODRAIN | PSEL | SCL_STRETCH);

    _SIF_WRITE32MSK(RW_SM0_CTRL0_REG, u4Reg, u4Msk);

    // Change OSDA/OSCL function to SIF Master
#if defined(CC_MT8223)
    if (_rSifDevParm.fgSystemI2C)
    {
        #if 0
        BSP_PinSet(TUNER_DATA, PINMUX_FUNCTION0);
        #else
        BSP_PinSet(PINMUX_OSDA0, PINMUX_FUNCTION0);
        CKGEN_WRITE32(REG_PINMUX_SEL2, CKGEN_READ32(REG_PINMUX_SEL2) | 0x04);
        BSP_PinSet(PINMUX_OSDA0_DUP, PINMUX_FUNCTION3);
        #endif
    }
    else if (_rSifDevParm.fgTuner)
    {
    ...
    if (_rSifDevParm.fgSystemI2C)
    {
        BSP_PinSet(OSDA0, PINMUX_FUNCTION0);
    }
    else if (_rSifDevParm.fgTuner)
    {
        BSP_PinSet(TUNER_CLK, PINMUX_FUNCTION0);
    }
    else if (_rSifDevParm.fgSecondI2C)
    {
        BSP_PinSet(OSDA1, PINMUX_FUNCTION0);
    }
    else if (_rSifDevParm.fgPdwncI2C)
    {
        BSP_PinSet(PINMUX_OPCTRL_1, PINMUX_FUNCTION1);
        BSP_PinSet(PINMUX_OPCTRL_2, PINMUX_FUNCTION1);
    }
	else if (_rSifDevParm.fg8295BI2C0)
	{
	#ifndef CC_MTK_LOADER
        PCMCIA_MT8295PinSet(PCMCIA_PINMUX_CI_DATA3,PCMCIA_PINMUX_FUNCTION2);
        #if 0
		UINT32 u4Mask = (3<<14), u4Value = (2<<14);
		PCMCIA_RawWrite32MSK(0xb040, u4Value, u4Mask);
        #endif
	#endif
	}
	else if (_rSifDevParm.fg8295BI2C1)
	{
	#ifndef CC_MTK_LOADER
        PCMCIA_MT8295PinSet(PCMCIA_PINMUX_CI_DATA0,PCMCIA_PINMUX_FUNCTION2);
        #if 0
		UINT32 u4Mask = (3<<16),u4Value = (2<<16);//,u4tmp = 0;
		PCMCIA_RawWrite32MSK(0xb040, u4Value, u4Mask);
        #endif
	#endif
	}
    ...
    if (_rSifDevParm.fgSystemI2C)
    {
        BSP_PinSet(OSDA0, PINMUX_FUNCTION1);
    }
    else if (_rSifDevParm.fgTuner)
    {
        BSP_PinSet(TUNER_CLK, PINMUX_FUNCTION1);
    }
    else if (_rSifDevParm.fgSecondI2C)
    {
        BSP_PinSet(OSDA1, PINMUX_FUNCTION1);
    }
    else if (_rSifDevParm.fgPdwncI2C)
    {
        BSP_PinSet(PINMUX_OPCTRL_0, PINMUX_FUNCTION1);
        BSP_PinSet(PINMUX_OPCTRL_1, PINMUX_FUNCTION1);
    }
	else if (_rSifDevParm.fg8295BI2C0)
	{
...
VOID SIF_Init(VOID)
{
    ...
    if (!_fgSifInitWait)
    {
        ...
        if (_fgEnablePdwncMaster)
        {
            VERIFY(PDWNC_RegIsr(PINTNO_SIF, _SIF_HandleISR, &pfnOldIsr) == PDWNC_OK);
        }
#ifndef CC_MTK_LOADER
		if ((_fgEnable8295bMaster0)||(_fgEnable8295bMaster1))
		{
			PCMCIA_RegIsr(MT8295_ISR_SIF, _SIF_HandleISR, &pfnOldIsr);
		}
#endif
#endif // CC_SIF_USE_INTERRUPT_MODE
...
VOID SIF_Stop(VOID)
{
    ...
    if (_fgSifInit)
    {
		...
		if ((_fgEnable8295bMaster0)||(_fgEnable8295bMaster1))
		{
			PCMCIA_RegIsr(MT8295_ISR_SIF, NULL, &pfnOldIsr);
		}
#endif
...
UINT16 SIF_Read(UINT16 u2ClkDiv, UINT8 u1DevAddr, UINT8 u1WordAddr,
                UINT8 *pu1Buf, UINT16 u2ByteCnt)
{
    return SIF_X_Read(0, u2ClkDiv, u1DevAddr, 1, u1WordAddr, pu1Buf, u2ByteCnt);
}
...
UINT16 SIF_ReadNoSubAddr(UINT16 u2ClkDiv, UINT8 u1DevAddr, UINT8 *pu1Buf,
                         UINT16 u2ByteCnt)
{
    return SIF_X_Read(0, u2ClkDiv, u1DevAddr, 0, 0, pu1Buf, u2ByteCnt);
}
...
UINT16 SIF_TunerReadNoSubAddr(UINT16 u2ClkDiv, UINT8 u1DevAddr, UINT8 *pu1Buf,
                              UINT16 u2ByteCnt)
{
    return SIF_X_Read(1, u2ClkDiv, u1DevAddr, 0, 0, pu1Buf, u2ByteCnt);
}
...
UINT16 SIF_Write(UINT16 u2ClkDiv, UINT8 u1DevAddr, UINT8 u1WordAddr,
                 UINT8 *pu1Buf, UINT16 u2ByteCnt)
{
    return SIF_X_Write(0, u2ClkDiv, u1DevAddr, 1, u1WordAddr, pu1Buf, u2ByteCnt);
}
...
UINT16 SIF_WriteNoSubAddr(UINT16 u2ClkDiv, UINT8 u1DevAddr, UINT8 *pu1Buf,
                          UINT16 u2ByteCnt)
{
    return SIF_X_Write(0, u2ClkDiv, u1DevAddr, 0, 0, pu1Buf, u2ByteCnt);
}
...
UINT16 SIF_TunerWriteNoSubAddr(UINT16 u2ClkDiv, UINT8 u1DevAddr, UINT8 *pu1Buf,
                               UINT16 u2ByteCnt)
{
    return SIF_X_Write(1, u2ClkDiv, u1DevAddr, 0, 0, pu1Buf, u2ByteCnt);
}
...
UINT16 SIF_ReadMultipleSubAddr(UINT16 u2ClkDiv, UINT8 u1DevAddr, UINT8 u1WordAddrNum, UINT32 u4WordAddr,
                               UINT8 *pu1Buf, UINT16 u2ByteCnt)
{
    return SIF_X_Read(0, u2ClkDiv, u1DevAddr, u1WordAddrNum, u4WordAddr, pu1Buf, u2ByteCnt);
}
...
UINT16 SIF_WriteMultipleSubAddr(UINT16 u2ClkDiv, UINT8 u1DevAddr, UINT8 u1WordAddrNum, UINT32 u4WordAddr,
                                UINT8 *pu1Buf, UINT16 u2ByteCnt)
{
    return SIF_X_Write(0, u2ClkDiv, u1DevAddr, u1WordAddrNum, u4WordAddr, pu1Buf, u2ByteCnt);
}
...
{
   _SIF_UserLockTimeout(u4Timeout);
   _fgSIFDisabled = TRUE;
   _SIF_UserUnlock();   
}
