...
/*
extern void* memcpy(void *, const void *, size_t a);
extern void* memset(void *, int, size_t a);

extern void* memcpy(void * a, const void * b, unsigned int c);
extern void* memset(void * a, int b , unsigned int c);
*/
/*
    Debugging:
...
inline void _MLOCK(void)
{
    ...
    if (!_fgInit)
    {
        VERIFY(x_sema_create(&_hApiSem, X_SEMA_TYPE_MUTEX, X_SEMA_STATE_LOCK) == OSR_OK);
        _fgInit = TRUE;
    }

    x_sema_lock(_hApiSem, X_SEMA_OPTION_NOWAIT);
#endif
}
...
int dlmem_init(void *base, int32 size)
{
    ...
    front_misalign = (uint32)chunk2mem(base) & MALLOC_ALIGN_MASK;
    ...
    if ( correction + 2*MALLOC_ALIGNMENT > (unsigned) size )
        ...
        top = (mchunkptr)((uint8*)base + correction);
...
void* dlcalloc(int32 nmemb, int32 size)
{
	int32 realsize = nmemb * size;

	void* ptr = dlmalloc(realsize);
...
void
dlmem_get_status(uint32 flags, mempool_status* status )
{
  ...
  //  LOG(0, "base: 0x%x, size: 0x%x, \ntotal allocated: 0x%x, free: 0x%x blocks 0x%x max: 0x%x\n",
   //     status->arenabase, status->arenasize, status->totalallocated, status->totalfree,
   //     status->freeblocks, status->maxfree);
} // mem_get_status()
