...
UINT32 VDP_SetEnable(UCHAR ucVdpId, UCHAR ucEnable)
{
    ...
    if (ucEnable == 0)
    {
        ...
        {
            _VDP_StatusNofify(ucVdpId, VDP_B2R_NO_SIGNAL);
            LOG(3, "Mute VDP(%d)\n", ucVdpId);
        }
#else
        _VDP_StatusNofify(ucVdpId, VDP_B2R_NO_SIGNAL);
    ...
    else
    {
        ...
        {
            _VDP_StatusNofify(ucVdpId, VDP_B2R_START_PLAY);
        }
    }
#endif
...
UINT32 VDP_SetFreeze(UCHAR ucVdpId, UCHAR ucFreeze)
{
    ...
    if (_arVdpConf[ucVdpId].ucFreeze != ucFreeze)
    {
        ...
        #if defined(CC_MT5365) && defined(DRV_SUPPORT_EXTMJC)
	    if(ucVdpId == SV_VP_MAIN)
	    { 
			vMJCSetMT8283FRZOnOff(ucFreeze);	   // for freezing mjc of mt8283  when mt5365 can't transfer disp_r in 3d fs freezing
		}
		else
		#endif
		{
...
BOOL VDP_IsSupportOverscan(UCHAR ucVdpId)
{
    ...
    if(fgIsScpipTv3dFlgSet(ucVdpId, SCPIP_TV3D_INPUT_IS_3D))
    {
        if(fgIsScpipTv3dFlgSet(ucVdpId, SCPIP_TV3D_FHD_PR)
            || fgIsScpipTv3dFlgSet(ucVdpId, SCPIP_TV3D_3D_TO_2D_SBS_Q)
            || fgIsScpipTv3dFlgSet(ucVdpId, SCPIP_TV3D_3D_TO_2D_SBS_Q_I))
        {
            fgSupported = SV_FALSE;
        }
    }
    #endif
    #if defined(CC_MT5365) && defined(CC_SCPOS_3DTV_SUPPORT) && defined(MT5366_SUPPORT_3D_PR)
    if(fgIsScpipTv3dFlgSet(ucVdpId, SCPIP_TV3D_INPUT_IS_3D))
    {
        if(fgIsScpipTv3dFlgSet(ucVdpId, SCPIP_TV3D_SBSH_PR)
            || fgIsScpipTv3dFlgSet(ucVdpId, SCPIP_TV3D_TNB_PR)
            || fgIsScpipTv3dFlgSet(ucVdpId, SCPIP_TV3D_FP_PR))
        {
            fgSupported = SV_FALSE;
        }
    }    
    ...
    // set source region, output region, overscan, and H/V porch    
...
UINT32 VDP_SetOverScan(UCHAR ucVdpId, VDP_OVERSCAN_REGION_T rRegion)
{
    ...
    if(!VDP_IsSupportOverscan(ucVdpId))
    {
        rRegion.u4Top = 0;
        rRegion.u4Bottom = 0;
        rRegion.u4Left = 0;
...
UINT32 VDP_GetDelayTime(void)
{
    ...
    u4PQDelay += (UINT32)u1DrvDIGetFrameDelay(ucVdpId);
    u4PQDelay += (u4ScpipGetFrameDelay(ucVdpId)/2);
...
#else
#ifdef CC_SCPOS_EN
    vScpipSetGameMode(u4VdpId, u1OnOff);
#else
    UNUSED(u1OnOff);
#endif
#endif
...
UINT32 VDP_SetDownBWMode(UINT8 u1OnOff)
{
    ...
    u1DowmBWMode = u1OnOff;
    vDrvDISeamlessSwtich(u1OnOff); // force DI Bob and 8 bit mode
#endif
    return VDP_SET_OK;
}
#endif
...
UINT32 VDP_ScposDispModeFIFOOverflow(void)
{
	UINT32 u4Return = 0;
#ifdef CC_SCPOS_EN
	u4Return = u4ScpipDispmodeFifoOverflow();
#endif
...
#if defined(CC_MT5395) && defined(CC_SCPOS_3DTV_SUPPORT)
    if(fgIsScpipTv3dFlgSet(SV_VP_MAIN, SCPIP_TV3D_INPUT_IS_3D))
    {
        // support 3d dot by dot only in 
        // 1. frame sequential input and size == output panel size
        // 2. shutter panel output
        if(fgIsScpipTv3dFlgSet(SV_VP_MAIN, SCPIP_TV3D_FS_INPUT)
            && (IS_LVDS_DISP_3D_SHUTTER)
            && (u1ScpipGetDispMode(SV_VP_MAIN) ==VDP_SCPOS_DISPMODE_OFF)
            && (u4InputWidth == wDISPLAY_WIDTH) 
...
UINT32 VDP_UpdateCmdDelay(UINT32 u4VdpId)
{
    ...
    UINT32 u4MJCOutputFrameRate;
    UINT32 u4MJCInputFrameRate; 

     u4MJCOutputFrameRate = (UINT32)vDrvGetLCDFreq();
     u4MJCInputFrameRate = (UINT32)u2DrvGetInternalFrameRate();
     u4MJCFrameDelay = (UINT32)u1DrvMJCGetFrmDelay();
	 
    // TODO: Scaler Need to supply an API for Scaler Delay instand of set delay as 2;
    u4MJCFrameDelay += 2; // scaler delay
    if (u4MJCOutputFrameRate == 0)
	{
		u4MJCOutputFrameRate = 60;
	}
	if (u4MJCInputFrameRate == 0)
	{
		u4MJCInputFrameRate = 60;
	}
    u4MJCFrameDelay = ((u4MJCFrameDelay +1) * u4MJCOutputFrameRate + (u4MJCInputFrameRate - 1))/u4MJCInputFrameRate;
    _u4VdpCmdDelay = (_u4VdpCmdDelay > u4MJCFrameDelay)? _u4VdpCmdDelay: u4MJCFrameDelay;
    VDP_MUTEX_UNLOCK;

    LOG(3, "Update Vdp = (%d) Cmd Delay = %d, u4MJCInputFrameRate = %d, u4MJCOutputFrameRate= %d \n", u4VdpId, _u4VdpCmdDelay,u4MJCInputFrameRate,u4MJCOutputFrameRate);
    #endif
    UNUSED(u4VdpId);    
