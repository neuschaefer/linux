...
INT32 _VDEC_AVSInit(UCHAR ucEsId)
{
    ...
    ASSERT(i < MAX_ES_NUM);
    prVdec = & (rAVSDecInfo[i]);

    //reset data structure
    x_memset((void *)prVdec, 0, sizeof(VDEC_AVS_INFO_T));    

    //reset tracking mode fbid
    prVdec->rDecParam.u1TrackingFbId = FBM_FB_ID_UNKNOWN;
...
INT32 _VDEC_AVSHdrPrsHdr(UCHAR ucVdecId, UCHAR ucEsId)
{
    ...
    switch (prVdec->rPesInfo.ucPicType)
    {
        ...
        case SEQ_END_SC:
            ...
            break;
        case EXTENSION_SC:
            _VDEC_AvsExtHdr(prVdec, &i4RetValue);
            break;
        case USER_DATA_SC:
        case VIDEO_EDIT_SC:
        default:
...
INT32 _VDEC_AVSDecFin(UCHAR ucVdecId, UCHAR ucEsId)
{
                             ...
                             prVdec->rDecParam.bWaitDecLock) > 0)
    {
        ...
        else
        {
            if (prVdec->rPicHdr.u4ProgFrm)
            {
                LOG(5, "Succeed to decode the progressive frame in the interlace bitstream\n");
                i4RetValue = (INT32)E_VDEC_OK;
    ...
    if (i4RetValue != E_VDEC_RETRIGGER)
    {
        ...
        {
            if(fgIsRefPic(prPicHdr->u4PicCodingType)) // reference frame
            {
    ...
    else
    {
        ...
        else
        {
            if ( i4RetValue == (INT32)E_VDEC_RETRIGGER )
            {
                ...
                prVdec->rDecParam.rPicCRC.u4PicCRC_1[6] = 0x0;
                prVdec->rDecParam.rPicCRC.u4PicCRC_1[7] = 0x0;
                LOG(0, "\n^-^ Top Or Bottom Field -> E_VDEC_RETRIGGER ^-^\n");
            }
            if ( i4RetValue == (INT32)E_VDEC_OK )
            {
...
#ifdef LOG_FOR_MC_MEASURE
{
    ...
    if ( i4RetValue != (INT32)E_VDEC_FAIL )
    {
        ...
        else
        {
            ...
            LOG(0, "VDEC_perf_measure:seq_name=%s,pic_idx=%d,pic_width=%d,pic_height=%d\n", _acLoadedBitstream, _u4CurFrmIdx, prVdec->rSeqHdr.u4HSize, prVdec->rSeqHdr.u4VSize);
            LOG(0, "VDEC_perf_measure2:pic_type=%c,framepic=%d,CYCLE_DRAM=%ld\n", ucPicType, ((fgIsFrame == TRUE) ? 1:0), MC_READ32(RO_MC_DECODING_CYCLE));
            LOG(0, "VDEC_perf_measure3:MC_DLE_NUM=%ld,MC_MBX =%ld,MC_MBY =%ld\n", MC_READ32(RO_MC_DLE_NUM), MC_READ32(RO_MC_MBX), MC_READ32(RO_MC_MBY));
            LOG(0, "VDEC_perf_measure4:CYC_SYS=%ld,INTRA_CNT=%ld,Y_BLK_CNT=%ld\n", MC_READ32(RO_MC_CYC_SYS), MC_READ32(RO_MC_INTRA_CNT), MC_READ32(RO_MC_Y_BLK_CNT));
            LOG(0, "VDEC_perf_measure5:C_BLK_CNT=%ld,WAIT_CNT=%ld,REQ_CNT=%ld\n", MC_READ32(RO_MC_C_BLK_CNT), MC_READ32(RO_MC_WAIT_CNT), MC_READ32(RO_MC_REQ_CNT));
...
void _VDEC_AVSStop(UCHAR ucEsId)
{
    ...
    if (!prVdecEsInfo)
    {
        LOG(3, "Error Es info is NULL in _VDEC_AVSStop\n");
        return;
    }
...
BOOL _VDEC_AVSIsPic(UCHAR ucEsId, VDEC_PES_INFO_T *prPes)
{
    ...
    u4Addr = prPes->u4VldReadPtr + 3;
    if (u4Addr >= prPes->u4FifoEnd)
    {
        u4Addr -= (prPes->u4FifoEnd - prPes->u4FifoStart);
    }
...
BOOL _VDEC_AVSEsFlwCtrl(UCHAR ucEsId, UCHAR* pucVldId, INT8 i1EsBufStatus, UCHAR ucSkipMode)
{
    ...
    if (pucPicType)
    {
        ucPicType = (*pucPicType) >> 6;
    }
    else
    {
        LOG(3, "Error Pic Type\n");
        return FALSE;
    }

    //static UINT32 _u4ReferenceCount = 0;
    
    /*if (prVdec->rPesInfo.ucPicType == PB_PICTURE_SC)
    {
        _u4ReferenceCount++;
        if(_u4ReferenceCount ==1 || _u4ReferenceCount ==2)
        {
            return TRUE;
        }
    }*/
