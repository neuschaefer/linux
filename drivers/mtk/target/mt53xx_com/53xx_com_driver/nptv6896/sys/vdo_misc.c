...
UINT8 bIsScalerInput444(UINT8 bPath) reentrant
{
    ...
    {
        bIs444 = fgApiVideoIsVgaMode(bPath, 0) ? SV_TRUE : SV_FALSE;
    }
#else
...
void vDrvVideoHwInit(void)
{
    ...
    vIO32WriteFldAlign(CKGEN_PMUX_MISC, 0x1, FLD_CLK180M_SEL); // select enetpll/2 = 337MHz
    SET_VDOIN_MPCLK(MPCLK_337MHZ);
    ...
    vRegWriteFldAlign(LPF_PIP_00, 1, MAIN_PROG_FLD_EN);
    vRegWriteFldAlign(LPF_PIP_00, 1, PIP_PROG_FLD_EN);
    vRegWriteFldAlign(OMUX_01, 1, PIP_F_SEL);
    vRegWriteFldAlign(OMUX_01, 1, MAIN_F_SEL);
        
#ifdef __MODEL_slt__
    // front 3x3 error diffusion
    vRegWriteFldAlign(TRANS_MAIN_07, 0, ERR_DFN);
    vRegWriteFldAlign(TRANS_PIP_07, 0, ERR_DFN); 

    // post and front dither
    vRegWrite4B(DITHER_00, 0);
    vRegWrite4B(DITHER_03, 0);

    // front scaler dither
    vRegWrite4B(SCPIP_DS1_0B, 0);
    vRegWrite4B(SCPIP_DS2_08, 0);
#endif

    //OD init
    //vDrvODInit(); move to vApiVideoPeInit();
...
//    UINT8 bAppType = sMMAppInfo.u1AppType;
    ...
    {
    case SV_ST_VGA:
        return (bIsScalerInputRGB(bPath)?
	...
	case MODE_3D_640_480p_60_SBS_FULL:
		return SOURCE_TYPE_TIMING_3D_640_480p_60_SBS_FULL;
	case MODE_3D_640_480p_60_LA:
		return SOURCE_TYPE_TIMING_3D_640_480p_60_LA;
	case MODE_3D_480p_60_LA:
    	return SOURCE_TYPE_TIMING_3D_480p_60_LA;
	case MODE_3D_576p_50_LA:
    	return SOURCE_TYPE_TIMING_3D_576p_50_LA;
	case MODE_3D_720p_60_LA:
    	return SOURCE_TYPE_TIMING_3D_720p_60_LA;
	case MODE_3D_720p_50_LA:
    	return SOURCE_TYPE_TIMING_3D_720p_50_LA;
	case MODE_3D_1080p_24_LA:
    	return SOURCE_TYPE_TIMING_3D_1080p_24_LA;
	case MODE_3D_480I_60_FA:
		return SOURCE_TYPE_TIMING_3D_480I_60_FA;
	case MODE_3D_1080I_60_FA:
    	return SOURCE_TYPE_TIMING_3D_1080I_60_FA;
	case MODE_3D_1080I_50_FA:
    	return SOURCE_TYPE_TIMING_3D_1080I_50_FA;
    case MODE_DE_MODE:
...
UINT16 bDrvVideoGetFrameDelay(UINT8 bPath)
{
	...
	u2FrameDelay += u1DrvDIGetFrameDelay(bPath);
    u2FrameDelay += u4ScpipGetFrameDelay(bPath);
    u2FrameDelay += u1DrvTDCGetFrameDelay(bPath);
    #ifdef SUPPORT_PANEL_CUSTOMER_SPEC
    u2FrameDelay += PANEL_GetFrameDelay();
    #endif

    #ifdef CC_MT5396
    u2FrameDelay += u1DrvMJCGetFrmDelay10x();
    #endif
...
{
    return RegReadFldAlign(SCPIP_PIP_OA_RO_08, PIP_OA_RO_08_RD_DISP_OUT_R);  
}
...
void vDrvVideoDramStatusChk(void)
{
    ...
    if ((fgDramStatusChkOnOff & 0xF) == 0)
    {
        ...
        if (u4ScpipCheckFifoOverflow(SV_VP_MAIN) & 0x3) // Scaler for MT5396A
        {
...
void vCheckDeocerStable(UINT8 bState)
{
    ...
    {    
        ...
        case SV_ENABLE_DEC_STABLE_CHK:
            vRegWriteFldAlign(SYS_05, 1, RAND_EN);
            bCheckStable = 1 ;
            bCounter = 0 ;
        break;
        ...
        {
            bCounter++;
            Printf("%d=========Htotal/hde %d %d\n",bCounter, RegReadFldAlign(STA_VFE_04, RANDOM_HTOTAL), RegReadFldAlign(STA_VFE_04, RANDOM_HDELEN));
            Printf("%d=========vtotal/vde %d %d\n", bCounter, RegReadFldAlign(STA_VFE_05, RANDOM_VTOTAL), RegReadFldAlign(STA_CTG_05, VDELEN));
        }
        break;
        default:
        break;
...
void vShowOmuxStatus(void)
{
   ...
   vRegWriteFldAlign(SYS_05, 1, RAND_EN);
  vUtDelay10ms(5);  
  Printf("omux Htotal/hde %d %d\n", RegReadFldAlign(STA_VFE_04, RANDOM_HTOTAL), RegReadFldAlign(STA_VFE_04, RANDOM_HDELEN));
  Printf("omux vtotal/vde %d %d\n",  RegReadFldAlign(STA_VFE_05, RANDOM_VTOTAL), RegReadFldAlign(STA_CTG_05, VDELEN));  
