...
EXTERN UINT32 VDP_CheckDisplayMode(void);
...
static PE_UI_VIDEO_QUALITY_ITEM_T _VdpMwVidQaType2UiVqItem(VID_QA_TYPE_T eVidQaType);
...
LINT_SUPPRESS_BRACE(818)
    ...
    switch (e_get_type)
    {
        ...
        case VID_PLA_GET_TYPE_MJC_EFFECT:
        case VID_PLA_GET_TYPE_MJC_MODE:
        case VID_PLA_GET_TYPE_MJC_DEMO:
            {
        ...
        case VID_PLA_GET_TYPE_REGION_CAPABILITY:
            {
                ...
                else
                {
                    *pz_get_info_len = sizeof(VID_PLA_VID_REGION_CAPABILITY_T);
                    ...
                    {
                        case VID_PLA_GET_TYPE_DISP_REGION:
                            if (VDP_CheckDisplayMode() == SV_FALSE) // non-dispmode
                            {
//                                if (VDP_IsSupportDotByDot())
//                                {
                                prArg->b_is_enable = 1;
//                                }
//                                else
...
//                                prArg->b_is_enable = 0;
		...
		switch (prArg->e_get_type)
		{
		    ...
		    default:
                        ...
                        bRet = PE_GetUiVqItemMinMaxDftCur(ucVdpId, ucArgType,
        ...
        case VID_PLA_GET_TYPE_COLOR_OFFSET_MAX:
            {
                ...
                else
                {
                    *pz_get_info_len = sizeof(VID_PLA_COLOR_GAIN_T);
                    ...
                    {
                        prArg->ui2_r_gain = (UINT16)i2Max;
                        prArg->ui2_g_gain = (UINT16)i2Max;
                        prArg->ui2_b_gain = (UINT16)i2Max;
                    }
                }
            }
            break;
                    ...
                    if (VDP_GetPicInfo(ucVdpId, &rVdpPicInfo) != VDP_SET_OK)
                    {
...
#endif
                        {
                            if((u4Tmp == (UINT32)SWDMX_FMT_H264_VIDEO_ES) ||
                               ...
                               (u4Tmp == (UINT32)SWDMX_FMT_AVS_VIDEO_ES) ||
...
static void _VdpAfdChangeNotify(UINT32 u4VdpId, UINT32 u4Afd, FBM_ASPECT_RATIO_T* prAspectRatio)
{
    ...
    if (_VdpXlatDispFmtCallBackMw(u4VdpId) != 0)
    {
        ...
        if(ucTveEnable != FALSE && u4VdpId == VDP_2)
        {
            ; // Skip OutRegion setting for TVE sub_path case
        }
        else
...
LINT_EXT_HEADER_BEGIN
    ...
    while( !VID_MIN_MAX_DFT_IS_END(pt_iter) )
    {
        ...
        PE_SetUiVqMinMaxDft(ui1_src_idx, u1UiVqItem, &rUiVqMinMaxDft);
...
static BOOL _VdpParseMinMaxDftTblEx(
    UINT8                    ui1_src_idx,
    UINT8                    ui1_pic_mode,
    VID_QA_MIN_MAX_DFT_T*    pt_min_max_dft_tbl )
{
...
static BOOL _VdpParseVidQaTblEx( VID_QA_TBL_EX_T*    pt_vid_qa_tbl )
{
LINT_EXT_HEADER_BEGIN
    BOOL                b_success = FALSE;
    VID_QA_TBL_EX_T*    pt_iter = pt_vid_qa_tbl;
    ...
    while( !VID_QA_TBL_EX_IS_END(pt_iter) )
    {
        UINT8  ui1_idx = _VdpGetSrcTypeIdx( pt_iter->e_src_type );
        UINT8  ui1_mode = (UINT8)pt_iter->e_pic_mode;

        if ( ui1_idx >= (UINT8)EVDP_VIS_MAX ||
             ui1_mode >= (UINT8)VID_PLA_PIC_MODE_MAX )
        {
            return FALSE;
        }

        b_success = _VdpParseMinMaxDftTblEx(
            ui1_idx,
            ui1_mode,
            pt_vid_qa_tbl->pt_min_max_dft_lst );
        if ( !b_success )
...
LINT_EXT_HEADER_BEGIN
    ...
    else if (pt_drv_cust->e_custom_type == DRV_CUSTOM_TYPE_VID_PLA_VID_QA_MIN_MAX_DFT_EX)
    {
        BOOL    b_success;

        b_success = _VdpParseVidQaTblEx( (VID_QA_TBL_EX_T*)(pt_drv_cust->pv_arg) );
        if ( !b_success )
        {
            return DRVP_ERROR;
        }
...
void _VdpEosCallback(UINT32 u4VdpId, VDP_COND_T eCond, BOOL fgFastForward)
{
    ...
    {
       _arMwVdp[u4VdpId].pfAtEosNfy(_arMwVdp[u4VdpId].pvATEosTag,eCond,1,0);
    }
#endif
}
