...
#define DMX_CI_AES_KEY_SWAP_MODE    (0)
...
#ifdef ENABLE_MULTIMEDIA
static BOOL _fgDmxCachedOn = TRUE;

static MW_DMX_PID_INFO_T _arDmxInfo[DMX_NUM_PID_INDEX];

/// The Cache PID structures
static DMX_PID_T _arCachePidStruct[DMX_NUM_PID_INDEX];
static UINT32 _au4CachePidFlag[DMX_NUM_PID_INDEX];
 ...
 */
//-----------------------------------------------------------------------------
static BOOL _MwDmxSetCachePid(UINT8 u1Pidx, UINT32 u4Flags, const DMX_PID_T* prPid)
{
    DMX_PID_T rPid;
    ...
    _MwDmxLock();
    _au4CachePidFlag[u1Pidx] |= u4Flags;
    rPid = _arCachePidStruct[u1Pidx];
    _MwDmxUnlock();
    ...
    if ((u4Flags & DMX_PID_FLAG_TS_INDEX) != 0)
    {
        ...
        rPid.u1TsIndex = prPid->u1TsIndex & 0x3;
    }
    ...
    if ((u4Flags & DMX_PID_FLAG_PID) != 0)
    {
        ...
        rPid.u2Pid = prPid->u2Pid & 0x1fff;
    }

    // PCR
    if ((u4Flags & DMX_PID_FLAG_PCR) != 0)
    {
        rPid.ePcrMode = prPid->ePcrMode;
    }

    // Buffer
    if ((u4Flags & DMX_PID_FLAG_BUFFER) != 0)
    {
        rPid.fgAllocateBuffer = prPid->fgAllocateBuffer;
        rPid.ePidType = prPid->ePidType;
        rPid.u4BufAddr = prPid->u4BufAddr;
        rPid.u4BufSize = prPid->u4BufSize;
    }

    // Steering
    if ((u4Flags & DMX_PID_FLAG_STEER) != 0)
    {
        rPid.u1SteerMode = prPid->u1SteerMode;
    }

    // Descramble mode
    if ((u4Flags & DMX_PID_FLAG_DESC_MODE) != 0)
    {
        rPid.eDescMode = prPid->eDescMode;
    }

    // Key index
    if ((u4Flags & DMX_PID_FLAG_KEY_INDEX) != 0)
    {
        rPid.u1KeyIndex = prPid->u1KeyIndex & 0x7;
    }

    // Device ID
    if ((u4Flags & DMX_PID_FLAG_DEVICE_ID) != 0)
    {
        rPid.u1DeviceId = prPid->u1DeviceId;
    }

    // Notification callback function
    if ((u4Flags & DMX_PID_FLAG_CALLBACK) != 0)
    {
        rPid.pfnNotify = prPid->pfnNotify;
        rPid.pvNotifyTag = prPid->pvNotifyTag;
    }

    // Scramble state callback function
    if ((u4Flags & DMX_PID_FLAG_SCRAMBLE_STATE) != 0)
    {
        rPid.pfnScramble = prPid->pfnScramble;
        rPid.pvScrambleTag = prPid->pvScrambleTag;
    }

    // Primary PID
    if ((u4Flags & DMX_PID_FLAG_PRIMARY) != 0)
    {
        rPid.fgPrimary = prPid->fgPrimary;
    }

    // Valid bit
    if ((u4Flags & DMX_PID_FLAG_VALID) != 0)
    {
        rPid.fgEnable = prPid->fgEnable;
    }

    _MwDmxLock();
    _arCachePidStruct[u1Pidx] = rPid;
    _MwDmxUnlock();
...
#endif
#else
    u4SecAddr = PHYSICAL(prNotifyInfo->u4SecAddr);
#ifndef MW_CLASSIC_DMX_NFY
...
#else
    u4DataAddr = PHYSICAL(prNotifyInfo->u4DataAddr);
#endif
    ...
    const VOID* pv_set_info, UINT8 u1Pidx)
{
    ...
    if(rPidInfo.eConType != MW_DMX_BUFAGENT)
    {
...
{
#ifdef CC_MT5363
    return DMX_SetFrontEnd(eFEType);
#else
    UNUSED(eFEType);    
    return TRUE;
#endif
 ...
 */
//-----------------------------------------------------------------------------
void _MW_DMX_ConnectInfo(UINT8 u1Pidx, UINT32 u4RMType, DRV_TYPE_T eType)
{
#ifdef ENABLE_MULTIMEDIA
    if(u1Pidx >= DMX_NUM_PID_INDEX)
    {
    ...
    {
        _arDmxInfo[u1Pidx].eConType = MW_DMX_BUFAGENT;
    }
    else
    {
        _arDmxInfo[u1Pidx].eConType = MW_DMX_TUNER;
    }
    _MwDmxUnlock();
#else
    UNUSED(u1Pidx);
 ...
 */
//-----------------------------------------------------------------------------
void _MW_DMX_DisConnectInfo(UINT8 u1Pidx)
{
#ifdef ENABLE_MULTIMEDIA
    if(u1Pidx >= DMX_NUM_PID_INDEX)
    {
...
BOOL _MW_DMX_FlushAllCachePid(void)
{
    ...
    {
        _MwDmxLock();
        u4Flags = _au4CachePidFlag[i];
        rPid = _arCachePidStruct[i];
        fgCached = _arDmxInfo[i].fgCached;
        fgValid = _arDmxInfo[i].fgValid;
        _MwDmxUnlock();

        if(fgValid && fgCached)
        {
            if(!_MW_DMX_SetPid(i, u4Flags, &rPid))
            {
                return FALSE;
            }
        }
    ...
    for(i=0; i<DMX_NUM_PID_INDEX; i++)
    {
        _au4CachePidFlag[i] = 0;
        x_memset((void*)&_arCachePidStruct[i], 0, sizeof(DMX_PID_T));
        _arDmxInfo[i].fgCached = FALSE;
    }
    _MwDmxUnlock();
...
{
    _MwDmxLock();
    _fgDmxCachedOn = fgCacheOn;
    _MwDmxUnlock();
}


//-----------------------------------------------------------------------------
/** _MW_DMX_GetCacheStatus
 */
//-----------------------------------------------------------------------------
BOOL _MW_DMX_GetCacheStatus(void)
{
    BOOL fgCacheOn;

    _MwDmxLock();
    fgCacheOn = _fgDmxCachedOn;
    _MwDmxUnlock();
...
//-----------------------------------------------------------------------------
#if defined (CC_MT5387) && !defined (CC_MT5363)
BOOL _MW_DMX_Set5387Pid(UINT8 u1Pidx, UINT32 u4Flags, const DMX_PID_T* prPid)
{
    DMX_DECODER_CALLBACKS_T rDecoderCallbacks;
    DMX_PID_T rPid;
    ...
    // Middleware may set pid in some kind of strange scenario

    if((rPidInfo.eConType == MW_DMX_BUFAGENT) && (rPidInfo.u4RMType == (UINT32)DRVT_DEMUX_TS_SECTION_MEMORY))
    {
        if(fgCacheOn)
        {
            if(!_MwDmxSetCachePid(u1Pidx, u4Flags, prPid))
            {
                return FALSE;
            }

            _MwDmxLock();
            _arDmxInfo[u1Pidx].fgCached = TRUE;
            _MwDmxUnlock();
    ...
    eInputType = DMX_GetInputType();

#ifdef ENABLE_MULTIMEDIA
    if((rPidInfo.eConType == MW_DMX_TUNER) && (eInputType != DMX_IN_BROADCAST_TS))
#else
    ...
    {
        if((rPidInfo.eConType == MW_DMX_BUFAGENT) && (eInputType == DMX_IN_BROADCAST_TS))
        {
            rPid.u1TsIndex = DMX_PVRPLAY_TS_INDEX;
        }

        if((rPidInfo.eConType == MW_DMX_BUFAGENT) && (eInputType == DMX_IN_PLAYBACK_TS))
        {
            if(DMX_PVRPlay_GetUseDescramble())
            {
                rPid.u1TsIndex = DMX_PVRPLAY_TS_INDEX;
            }
        }
...
BOOL _MW_DMX_Set5363Pid(UINT8 u1Pidx, UINT32 u4Flags, const DMX_PID_T* prPid)
{
    DMX_DECODER_CALLBACKS_T rDecoderCallbacks;
    DMX_PID_T rPid;
    DMX_INPUT_TYPE_T eInputType;
...
#ifdef ENABLE_MULTIMEDIA
    if(rPidInfo.eConType == MW_DMX_TUNER)
#endif  //ENABLE_MULTIME
    {
        if(u4Flags & DMX_PID_FLAG_TS_INDEX)
        {
...
{
#if defined (CC_MT5387) && !defined (CC_MT5363)
    return _MW_DMX_Set5387Pid(u1Pidx, u4Flags, prPid);
#elif defined(CC_MT5363)
    return _MW_DMX_Set5363Pid(u1Pidx, u4Flags, prPid);
#endif
}
...
BOOL _MwDmxOnEnableDesc(UINT8 u1CompId, const VOID* pv_set_info)
{
    DMX_PID_T rPid;
    ...
    if(fgEnable) 
    {
        ...
        for(i =0; i< DMX_NUM_PID_INDEX; i++) 
        {
            if(DMX_GetPid(i, DMX_PID_FLAG_PID | DMX_PID_FLAG_TS_INDEX, &rPid))
            {
                if( (rPid.u2Pid == _arDmxDesc[u1CompId].u2PidNum) && (rPid.u1TsIndex != 2)) //ignore PVR PID 
                {
                    //Setup Descrambler
                    if(_arDmxDesc[u1CompId].rDescAlg == DESC_ALG_DES_ECB) 
                    {
                        DMX_SetDesKey(u1CompId, FALSE, _arDmxDesc[u1CompId].fgEven, _arDmxDesc[u1CompId].au4Key);
                    ...
                    {
                        DMX_SetDesKey(u1CompId, TRUE, _arDmxDesc[u1CompId].fgEven, _arDmxDesc[u1CompId].au4Key);
                    ...
                    else if(_arDmxDesc[u1CompId].rDescAlg == DESC_ALG_AES_128_ECB)
                    {
                        ...
                        DMX_SetAesKey(u1CompId, _arDmxDesc[u1CompId].fgEven,  _arDmxDesc[u1CompId].au4Key);
                    ...
                    else if(_arDmxDesc[u1CompId].rDescAlg == DESC_ALG_AES_128_CBC)
                    {
                        ...
                        DMX_CIPlus_SetAesIV(u1CompId, _arDmxDesc[u1CompId].fgEven, _arDmxDesc[u1CompId].au4Iv);                  
                        DMX_SetAesKeyLen(128);
                        DMX_CIPlus_SetAesKey(u1CompId, _arDmxDesc[u1CompId].fgEven,  (UINT8 *)_arDmxDesc[u1CompId].au4Key);                        
    ...
    {        
        ...
        for(i =0; i< DMX_NUM_PID_INDEX; i++) 
        {
            if(DMX_GetPid(i, DMX_PID_FLAG_PID | DMX_PID_FLAG_TS_INDEX, &rPid))
            {
                if(rPid.u2Pid == _arDmxDesc[u1CompId].u2PidNum && (rPid.u1TsIndex != 2)) 
                {                  
                    //Stop Descrambler
                    rPid.eDescMode = DMX_DESC_MODE_NONE;
        ...
        if(i == DMX_DESC_COMP_NUM)
        {
            if(_arDmxDesc[u1CompId].rDescAlg == DESC_ALG_AES_128_CBC)
            {
                DMX_CIPlus_Free(1);
            }
        }
                    ...
                    SIZE_T              z_set_info_len)
{
    ...
    case DESC_SET_TYPE_ODD_KEY:
        ...
        else if(prKeyInfo->e_descrambler_alg == DESC_ALG_AES_128_CBC) 
        {
            ...
            ASSERT((UINT32)prKeyInfo->u_key_info.t_aes_128_cbc_key.z_iv_len <= 16);
...
#ifdef ENABLE_MULTIMEDIA
        _arDmxInfo[i].eConType = MW_DMX_NONE;
