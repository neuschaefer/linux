...
#ifndef ALLOC
# define ALLOC(size) (x_mem_alloc(size))
#endif
#ifndef TRYFREE
# define TRYFREE(p) {if (p) x_mem_free(p);}
#endif
...
extern int ZEXPORT unzStringFileNameCompare (const char* fileName1, const char* fileName2, int iCaseSensitivity)
{
    ...
    if (iCaseSensitivity==1)
        return x_strcmp(fileName1,fileName2);
...
*/
extern int ZEXPORT unzOpen3 (const char *path, unzFile *handle)
{
    unz_s us;
    unz_s *s;
...
extern int ZEXPORT unzLocateFile (unzFile file, const char *szFileName, int iCaseSensitivity)
{
    ...
    if (x_strlen(szFileName)>=UNZ_MAXFILENAMEINZIP)
        return UNZ_PARAMERROR;
...
/* Restore file pos */
extern int ZEXPORT unzRestorePos (unzFile file)
{
...
extern int unzGetDecompressEntryLen(unzFile file, SIZE_T* pz_query_buf_len)
{
    ...
    *pz_query_buf_len = s->cur_file_info.uncompressed_size;
    return UNZ_OK;
}
#endif
