 ...
 *          AEER_AEE_NO_RIGHTS   Permission denied.
 ----------------------------------------------------------------------------*/
INT32 c_aee_create (const AEE_APP_DESC_T*  pt_desc,
                    const CHAR*            ps_name,
                    HANDLE_T*              ph_app)
{
    HANDLE_T  h_aux;
    ...
    switch (i4_res)
    {
        case AEER_OK:
        {
            i4_res = x_aee_create(pt_desc, ps_name, ph_app);
 ...
 *          AEER_INV_HANDLE  Invalid handle.
 ----------------------------------------------------------------------------*/
INT32 c_aee_send_data (HANDLE_T  h_app,
                       UINT32    ui4_type,
                       VOID*     pv_data,
                       SIZE_T    z_data_len)
{
    return x_aee_send_data(h_app, ui4_type, pv_data, z_data_len);
}

/*-----------------------------------------------------------------------------
 * Name: c_aee_get_handle
 *
 ...
 *          AEER_AEE_NO_RIGHTS   Permission denied.
 ----------------------------------------------------------------------------*/
INT32 c_aee_get_handle (const CHAR*  ps_name,
                        HANDLE_T*    ph_app)
{
    HANDLE_T  h_aux;
    INT32     i4_res;
    ...
    switch (i4_res)
    {
        case AEER_OK:
        {
            i4_res = x_aee_get_handle(ps_name, ph_app);
 ...
 *          AEER_INV_ARG  Invalid argument.
 ----------------------------------------------------------------------------*/
INT32 c_aee_get_resource_info (HANDLE_T           h_app,
                               AEE_RESOURCE_ID_T  e_res_id,
                               VOID*              pv_res)
{
    return x_aee_get_resource_info(h_app, e_res_id, pv_res);
}

/*-----------------------------------------------------------------------------
 * Name: c_aee_thread_create
 *
 ...
 *          AEER_INV_ARG  Invalid argument.
 ----------------------------------------------------------------------------*/
INT32 c_aee_thread_create (HANDLE_T*             ph_th_hdl,
                           CHAR*                 ps_name,
                           SIZE_T                z_stacksize,
                           UINT8                 ui1_pri,
                           ...
                           VOID*                 pv_arg)
{
    ...
    i4_res = aee_grab_resources(AEE_FLAG,
                                0,
                                0,
                                0,
                                1, /* one handle */
    ...
    switch (i4_res)
    {
        case AEER_OK:
        {
            i4_res = x_aee_thread_create(ph_th_hdl,
                                         ps_name,
                                         z_stacksize,
                                         ui1_pri,
                                         pf_main,
            ...
            if (i4_res == AEER_OK)
            {
                handle_link_to_aux(h_aux, *ph_th_hdl);
            }
            else
            {
                aee_release_resources(0, 0, 0, 1, 1, 0, 0);
            }
        }
        break;
