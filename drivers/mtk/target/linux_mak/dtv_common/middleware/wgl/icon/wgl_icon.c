...
#include "wgl/icon/x_wgl_icon.h"
#include "wgl/icon/wgl_icon.h"
...
typedef struct _WGL_ICON_CONTROL_LOGIC_T
{
#ifdef CLI_LVL_ALL     
...
static INT32 _wgl_icon_msg_hdr_paint(
    HANDLE_T                        h_widget,
    WGL_ICON_CONTROL_LOGIC_T*       pt_icon,
...
static INT32 _wgl_icon_msg_hdr_size(
    HANDLE_T                        h_widget,
    WGL_ICON_CONTROL_LOGIC_T*       pt_icon,
    UINT16                          ui2_width,
    UINT16                          ui2_height);
...
static INT32 _wgl_icon_msg_hdr_command(
    HANDLE_T                        h_widget,
    WGL_ICON_CONTROL_LOGIC_T*       pt_icon,
...
static INT32 _wgl_icon_cmd_hdr_get_min_size(
    const WGL_ICON_CONTROL_LOGIC_T* pt_icon,
    GL_SIZE_T*                      pt_size);

/*WGL_CMD_GL_GET_PREFERRED_SIZE command handler*/
static INT32 _wgl_icon_cmd_hdr_get_preferred_size(
    const WGL_ICON_CONTROL_LOGIC_T* pt_icon,
    GL_SIZE_T*                      pt_size);

static INT32 _wgl_icon_change_cnt_rect(
    const WGL_ICON_CONTROL_LOGIC_T*     pt_icon,
 ...
 * Returns
 ******************************************************************************/
INT32 x_wgl_icon_init(
    UINT32                      ui4_init_type)
{
    UINT32                      ui4_wgl_type = HT_WGL_WIDGET_ICON;
    WGL_WIDGET_CTRL_INTF_T      t_wgl_intf = { wgl_icon_inst_create, 
                                               wgl_icon_inst_destroy, 
                                               wgl_icon_default_proc,
                                               WGL_ATTR_FOCUSABLE};
    UINT32                      ui4_cnt_type = WGL_CONTENT_ICON_DEF;
    WGL_CONTENT_CTRL_INTF_T     t_cnt_intf = { wgl_icon_cnt_inst_create,
                                               wgl_icon_cnt_inst_destroy,
                                               wgl_icon_cnt_command_proc,
                                               wgl_icon_cnt_paint}; 
    INT32                       i4_ret;

    DBG_API(("{WGL(ICON)} %s: ui4_init_type = 0x%.8x\n", __FUNCTION__, ui4_init_type));

    if(WGL_ICON_INIT_CTRL_LOGIC & ui4_init_type)
    {
        i4_ret = wgl_register_comp_intf(ui4_wgl_type, &t_wgl_intf);
        if(WGLR_OK != i4_ret)
        {
            DBG_ERROR(("{WGL(ICON)} ERR: Fail ot register Component Interface. (%d) at L%d\n\n", i4_ret, __LINE__));
            return i4_ret;
        }
    }
    

    if(WGL_ICON_INIT_DEFAULT_CNT & ui4_init_type)
    {
        i4_ret = wgl_register_content_intf(ui4_cnt_type, &t_cnt_intf);
        if(WGLR_OK != i4_ret)
        {
            DBG_ERROR(("{WGL(ICON)} ERR: Fail to register Content Interface. (%d) at L%d\n\n", i4_ret, __LINE__));
            return i4_ret;
        }
    }
    
    i4_ret = wgl_icon_cli_init();
    WGL_ICON_REPORT(WGLR_OK, i4_ret, DBG_ERROR(("{WGL(ICON)} ERR: Fail to init Icon CLI. (%d) at L%d\n\n", i4_ret, __LINE__)));
    return i4_ret;
}
    ...
    VOID*                       pv_param2)
{
    ...
    WGL_ICON_CONTROL_LOGIC_T*   pt_icon = NULL;
    ...
    DBG_API(("{WGL(ICON)} %s: h_widget = 0x%.8x \tui4_msg = 0x%.8x \tpv_param1 = 0x%.8x \tpv_param2 = 0x%.8x\n", 
            __FUNCTION__, 
            h_widget, 
            ui4_msg,
            pv_param1,
    ...
    case WGL_MSG_PAINT:
        {
            ...
            if(WGLR_OK != i4_ret)
            {
                DBG_ERROR(("{WGL(ICON)} ERR: Fail to get widget's update region. (%d) at L%d\n\n", i4_ret, __LINE__));
                return i4_ret;
            }
        }
        break;
    ...
    case WGL_MSG_GET_FOCUS:
        {
            ...
            WGL_ICON_REPORT(WGLR_OK, i4_ret, DBG_ERROR(("{WGL(ICON)} ERR: Fail to notify widget's parent of getting focus. (%d) at L%d\n\n", i4_ret, __LINE__)));
            return i4_ret;
        }
    ...
    case WGL_MSG_LOSE_FOCUS:
        {
            ...
            WGL_ICON_REPORT(WGLR_OK, i4_ret, DBG_ERROR(("{WGL(ICON)} ERR: Fail to notify widget's parent of losing focus. (%d) at L%d\n\n", i4_ret, __LINE__)));
            return i4_ret;
        }
    ...
    case WGL_MSG_KEY_DOWN:
        {
            ...
            WGL_ICON_REPORT(WGLR_OK, i4_ret, DBG_ERROR(("{WGL(ICON)} ERR: Fail to notify widget's parent of key-down. (%d) at L%d\n\n", i4_ret, __LINE__)));
            return i4_ret;
        }
    ...
    case WGL_MSG_KEY_UP:
        {
            ...
            WGL_ICON_REPORT(WGLR_OK, i4_ret, DBG_ERROR(("{WGL(ICON)} ERR: Fail to notify widget's parent of key-up. (%d) at L%d\n\n", i4_ret, __LINE__)));
            return i4_ret;
        }
    ...
    case WGL_MSG_COMMAND:
        {
            if(WGL_CMD_GL_SET_INSET == (UINT32)pv_param1)   /*need to get widget's rectangle before lock it*/
            {
                ...
                if(WGLR_OK != i4_ret)
                {
                    DBG_ERROR(("{WGL(ICON)} ERR: Fail to get widget's dimension. (%d) at L%d\n\n", i4_ret, __LINE__));
                    return i4_ret;
                }
    ...
    /*Get icon data*/
    i4_ret = wgl_get_instance(h_widget, (WGL_WIDGET_CONTEXT_HANDLE_T*)&pt_icon);
    if(WGLR_OK != i4_ret)
    {
        DBG_ERROR(("{WGL(ICON)} ERR: Fail to get widget's instance. (%d) at L%d\n\n", i4_ret, __LINE__));
        return WGLR_INV_HANDLE;
    }
    if(NULL == pt_icon)
    {
        i4_temp = wgl_release_instance(h_widget, pt_icon);
        WGL_ASSERT(WGLR_OK == i4_temp);

        DBG_ERROR(("{WGL(ICON)} ERR: Widget's instance is NULL. (%d) at L%d\n\n", WGLR_INTERNAL_ERROR, __LINE__));
    ...
    {
    case WGL_MSG_PAINT:
        i4_ret = _wgl_icon_msg_hdr_paint(h_widget, 
                                         pt_icon, 
    ...
    case WGL_MSG_SIZE:
        i4_ret = _wgl_icon_msg_hdr_size(h_widget, 
                                        pt_icon, 
    ...
    case WGL_MSG_ENABLE:
        {
            WGL_UPDATE_STATE(pt_icon->ui2_state, WGL_POINTER_TO_BOOL(pv_param1), WGL_STATE_ENABLED);
            i4_ret = WMPR_DONE;
        }
        break;
    ...
    case WGL_MSG_NOTIFY:
        {
            WGL_ASSERT(0);  /*icon static should not receive this message*/
            i4_ret = WGLR_INTERNAL_ERROR;
            DBG_ERROR(("{WGL(ICON)} ERR: Icon should not receive any notification. (%d) at L%d\n\n", i4_ret, __LINE__));
        }
        break;
    ...
    case WGL_MSG_COMMAND:
        {
            ...
            {
                i4_ret = WGLR_INV_ARG;
                DBG_ERROR(("{WGL(ICON)} ERR: The 2nd parameter of WGL_MSG_COMMAND should not be NULL. (%d) at L%d\n\n", i4_ret, __LINE__));
            }
            else
            {
                i4_ret = _wgl_icon_msg_hdr_command(h_widget, 
                                                   pt_icon, 
                                                   (UINT32)pv_param1, 
    ...
    case WGL_MSG_CHANGE_COLOR_CFG:
        {
            i4_ret = pt_icon->t_cnt_intf.pf_content_cmd_proc(pt_icon->h_cnt_data, WGL_CNT_ICON_UPDATE_CVSST, pv_param1, NULL);
        }
        break;

    default:
        i4_ret = WGLR_INV_MSG;
        DBG_ERROR(("{WGL(ICON)} ERR: Unknown message type. (%d) at L%d\n\n", i4_ret, __LINE__));
        break;
    }
    ...
    WGL_WIDGET_CREATE_INPUT_T       *pt_wgt_create)
{
    ...
    WGL_ICON_CONTROL_LOGIC_T*       pt_ctrl_logic;
    ...
    {
        DBG_ERROR(("{WGL(ICON)} ERR: Invalid arguments. (%d) at L%d\n\n", WGLR_INV_ARG, __LINE__));
        return WGLR_INV_ARG;
    }
    ...
    if(WGLR_OK != i4_ret)
    {
        DBG_ERROR(("{WGL(ICON)} ERR: Fail to get content's interface. (%d) at L%d\n\n", i4_ret, __LINE__));
    ...
    if(WGLR_OK != i4_ret)
    {
        DBG_ERROR(("{WGL(ICON)} ERR: Fail to get border's interface. (%d) at L%d\n\n", i4_ret, __LINE__));
        return WGLR_INV_TYPE;
    }

    /*allocate memory for control logic object*/
    pt_ctrl_logic = (WGL_ICON_CONTROL_LOGIC_T*) WGL_MEM_ALLOC(sizeof(WGL_ICON_CONTROL_LOGIC_T));
    if(NULL == pt_ctrl_logic)
    {
        DBG_ERROR(("{WGL(ICON)} ERR: Fail to alloc memory for control logic. (%d) at L%d\n\n", WGLR_OUT_OF_MEMORY, __LINE__));
        return WGLR_OUT_OF_MEMORY;
    }
    
    /*initialize control logic object*/
    x_memset(pt_ctrl_logic, 0, sizeof(WGL_ICON_CONTROL_LOGIC_T));
    
#ifdef CLI_LVL_ALL
    pt_ctrl_logic->ui4_mem_size = sizeof(WGL_ICON_CONTROL_LOGIC_T);   
#endif
    ...
    if(t_bdr_intf.pf_create_instance) 
    {
        ...
        if(WGLR_OK != i4_ret)
        {
            DBG_ERROR(("{WGL(ICON)} ERR: Fail to create border object. (%d) at L%d\n\n", i4_ret, __LINE__));
            goto _err_out;
        }
        ...
        if(WGLR_OK != i4_ret)
        {
            DBG_ERROR(("{WGL(ICON)} ERR: Fail to get border's inset. (%d) at L%d\n\n", i4_ret, __LINE__));
            goto _err_out;
        }
    ...
    {
        DBG_ERROR(("{WGL(ICON)} ERR: NULL-Border cannot be with WGL_STL_GL_BDR_FILL_CNT_BK style. (%d) at L%d\n\n", i4_ret, __LINE__));
    ...
    if(WGLR_OK != i4_ret)
    {
        DBG_ERROR(("{WGL(ICON)} ERR: Fail to create content object. (%d) at L%d\n\n", i4_ret, __LINE__));
        goto _err_out;
    }
    ...
    WGL_WIDGET_CONTEXT_HANDLE_T     t_ctx_wgt)
{
    ...
    pt_ctrl_logic = (WGL_ICON_CONTROL_LOGIC_T*)t_ctx_wgt;
    ...
    UINT16                          ui2_state)
{
    ...
    {
        DBG_ERROR(("{WGL(ICON)} ERR: GRAPHICS object is NULL. (%d) at L%d\n\n", WGLR_INV_ARG, __LINE__));
        return WGLR_INV_ARG;
    }
    
    if(pt_icon->t_bdr_intf.pf_paint_border)
    {
        /*check border*/
        WGL_ASSERT(pt_icon->h_bdr_data);

        /*paint border*/
        i4_ret = pt_icon->t_bdr_intf.pf_paint_border(pt_icon->h_bdr_data, 
                                                     h_graphics, 
                                                     ui4_upt_rect, 
                                                     pt_upt_rect, 
                                                     ui2_state);
    ...
    /*paint content*/
    WGL_CHECK_INTERNAL(pt_icon->t_cnt_intf.pf_paint_content);
    WGL_CHECK_INTERNAL(pt_icon->h_cnt_data);
    return pt_icon->t_cnt_intf.pf_paint_content(pt_icon->h_cnt_data, 
                                                h_graphics, 
                                                ui4_upt_rect, 
                                                pt_upt_rect, 
                                                ui2_state);
 ...
 *      
 * Description
 *          WGL_MSG_SIZE message handler. Not check h_widget and pt_icon
 * Input arguments
 * Output arguments
 * Returns
 ******************************************************************************/
    ...
    UINT16                          ui2_height)
{
    ...
    if(pt_icon->t_bdr_intf.pf_border_cmd_proc)
    {
        /*check border*/
        WGL_ASSERT(pt_icon->h_bdr_data);
        
        /*notify border to update its rectangle*/
        i4_ret = pt_icon->t_bdr_intf.pf_border_cmd_proc(pt_icon->h_bdr_data, WGL_BDR_GL_SET_SIZE, (VOID*)(INT32)ui2_width, (VOID*)(INT32)ui2_height);
        if(WGLR_OK != i4_ret)
        {
            return i4_ret;
        }
    ...
    return _wgl_icon_change_cnt_rect(pt_icon, &t_widget_rect, FALSE, NULL);
}
    ...
    VOID*                           pv_internal_cmd_data)
{
    ...
    DBG_API(("{WGL(ICON)} %s: h_widget = 0x%.8x \tui4_command_id = 0x%.8x \tpv_param1 = 0x%.8x \tpv_param2 = 0x%.8x\n", 
            __FUNCTION__, 
            h_widget, 
            ui4_command_id,
            pv_param1,
    ...
    case WGL_CMD_GL_GET_ATTACH_DATA:
        {
            ...
            {
                DBG_ERROR(("{WGL(ICON)} ERR: Invalid arguments. For WGL_CMD_GL_GET_ATTACH_DATA, its 1st input argument should not be NULL. (%d) at L%d\n\n", WGLR_INV_ARG, __LINE__));
                return WGLR_INV_ARG; 
            }

            *((VOID**)pv_param1) = pt_icon->pv_attach;
        }
        break;
	case WGL_CMD_GL_GET_PREFERRED_SIZE:
		return _wgl_icon_cmd_hdr_get_preferred_size(pt_icon, (GL_SIZE_T*)pv_param1);
    case WGL_CMD_ICON_GET_MIN_SIZE:
        return _wgl_icon_cmd_hdr_get_min_size(pt_icon, (GL_SIZE_T*)pv_param1);

    case WGL_CMD_GL_SET_INSET:
        {
            DBG_INFO(("{WGL(ICON)} set-inset(l, t, r, b) = 0x%.8x (%d, %d, %d, %d)\n", 
                        pv_param1,
            ...
            return _wgl_icon_change_cnt_rect(pt_icon, (GL_RECT_T*)pv_internal_cmd_data, TRUE, (WGL_INSET_T*)pv_param1);
        }
#ifdef CLI_LVL_ALL
        case WGL_CMD_GL_GET_ALLOC_MEM:
        {
            UINT32 ui4_wdg_mem = pt_icon->ui4_mem_size;
            WGL_MTX_OBJ_T*  pt_wgt_mem;
            if(NULL == pv_param1)
            {
                    DBG_ERROR(("{WGL(ICON)} ERR: Invalid arguments. For WGL_CMD_GL_GET_ALLOC_MEM, its 1st input argument should not be NULL. (%d) at L%d\n\n", WGLR_INV_ARG, __LINE__));
                    return WGLR_INV_ARG; 
            }
            
            pt_wgt_mem = (WGL_MTX_OBJ_T* )pt_icon->h_cnt_data;
            ui4_wdg_mem += pt_wgt_mem->ui4_mem_size;
            if(pt_icon->t_bdr_intf.pf_create_instance != NULL)
            {
                pt_wgt_mem = (WGL_MTX_OBJ_T* )pt_icon->h_bdr_data;
                ui4_wdg_mem += pt_wgt_mem->ui4_mem_size;   
            }
    ...
    default:
        {
            ...
            {
                /*check content*/
                WGL_CHECK_INTERNAL(pt_icon->t_cnt_intf.pf_content_cmd_proc);
                WGL_CHECK_INTERNAL(pt_icon->h_cnt_data);
                
                return pt_icon->t_cnt_intf.pf_content_cmd_proc(pt_icon->h_cnt_data, ui4_command_id, pv_param1, pv_param2);
            }
            else if(WGL_IS_BORDER_CMD(ui4_command_id))/*border command*/
            {
                if(NULL == pt_icon->t_bdr_intf.pf_border_cmd_proc)
                {
                    DBG_ERROR(("{WGL(ICON)} ERR: Border's command procedure is NULL. (%d) at L%d\n\n", WGLR_NO_BORDER, __LINE__));
                    return WGLR_NO_BORDER;
                }

                /*check border*/
                WGL_CHECK_INTERNAL(pt_icon->h_bdr_data);
                return pt_icon->t_bdr_intf.pf_border_cmd_proc(pt_icon->h_bdr_data, ui4_command_id, pv_param1, pv_param2);
            }
            else
            {
                DBG_ERROR(("{WGL(ICON)} ERR: Unknown command. (%d) at L%d\n\n", WGLR_INV_CMD, __LINE__));
    ...
    GL_SIZE_T*                          pt_size)
{
    ...
    if(pt_icon->t_bdr_intf.pf_border_cmd_proc)
    {
        ...
        if(WGLR_OK != i4_ret)
        {
            DBG_ERROR(("{WGL(ICON)} ERR: This command isn't exported in this version. (%d) at L%d\n\n", i4_ret, __LINE__));
            return i4_ret;
        }
    }
    
    /*Get Content Size*/
    i4_ret = pt_icon->t_cnt_intf.pf_content_cmd_proc(pt_icon->h_cnt_data, WGL_CNT_ICON_GET_PREFERRED_SIZE, pt_size, NULL);
    if(WGLR_OK != i4_ret)
    {
        return i4_ret;
    }
    ...
    GL_SIZE_T*                          pt_size)
{
    ...
    /*Get Content Size*/
    i4_ret = pt_icon->t_cnt_intf.pf_content_cmd_proc(pt_icon->h_cnt_data, WGL_CNT_ICON_GET_MIN_SIZE, pt_size, NULL);
    if(WGLR_OK != i4_ret)
    {
        return i4_ret;
    }
    ...
    const WGL_INSET_T*                  pt_inset)
{
    ...
    /*get cnt rect*/
    i4_ret = wgl_calc_cnt_rect(pt_icon->t_bdr_intf.pf_border_cmd_proc, pt_icon->h_bdr_data, pt_widget_rect, b_new_inset, pt_inset, &t_cnt_rect);
    if(WGLR_OK != i4_ret)
    {
        DBG_ERROR(("{WGL(ICON)} ERR: Fail to calc content rectangle. (%d) at L%d\n\n", i4_ret, __LINE__));
        return i4_ret;
    }
    
    /*update content rect*/
    return pt_icon->t_cnt_intf.pf_content_cmd_proc(pt_icon->h_cnt_data, WGL_CNT_ICON_SET_RECT, &t_cnt_rect, NULL);
}
