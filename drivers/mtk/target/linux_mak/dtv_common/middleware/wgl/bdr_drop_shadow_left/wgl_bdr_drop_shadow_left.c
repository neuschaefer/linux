 ...
 *---------------------------------------------------------------------------*/
/*-----------------------------------------------------------------------------
 * $RCSfile: wgl_bdr_drop_shadow_left.c,v $
 * $Revision: #1 $
 * $Date: 2012/04/27 $
 * $Author: dtvbm11 $
...
#include "wgl/bdr_drop_shadow_left/wgl_bdr_drop_shadow_left.h"
#include "wgl/bdr_drop_shadow_left/u_wgl_bdr_drop_shadow_left.h"

/*-----------------------------------------------------------------------------
                    constants
 ----------------------------------------------------------------------------*/
#define BDR_DPSDWLFT_DFT_HEIGHT                 6
#define BDR_DPSDWLFT_DFT_SHADOW_ALPHA_FACTOR    128


static const WGL_INSET_T BDR_DPSDWLFT_DFT_INSET =
    {
        BDR_DPSDWLFT_DFT_HEIGHT,
        0, 
        0,
        BDR_DPSDWLFT_DFT_HEIGHT
    };

static const WGL_INSET_T BDR_DPSDWLFT_MIN_INSET =
    {
        0,
        0,
        0,
...
/*-----------------------------------------------------------------------------
                    structures
 ----------------------------------------------------------------------------*/
typedef struct _BDR_DPSDWLFT_CTX_T
{
#ifdef CLI_LVL_ALL     
...
/*-----------------------------------------------------------------------------
 ...
 ----------------------------------------------------------------------------*/
static INT32 _bdr_dpsdwlft_cmd_hdr_set_size(
    IN OUT  BDR_DPSDWLFT_CTX_T          *pt_ctx,
    IN      VOID                        *pv_param1,
    IN      VOID                        *pv_param2);
    
static INT32 _bdr_dpsdwlft_cmd_hdr_set_inset(
    IN OUT  BDR_DPSDWLFT_CTX_T          *pt_ctx,
    IN      VOID                        *pv_param1);

static INT32 _bdr_dpsdwlft_cmd_hdr_get_inset(
    IN      BDR_DPSDWLFT_CTX_T          *pt_ctx,
    OUT     VOID                        *pv_param1);
    
static INT32 _bdr_dpsdwlft_cmd_hdr_get_min_inset(
    IN      BDR_DPSDWLFT_CTX_T          *pt_ctx,
    OUT     VOID                        *pv_param1);
 
static INT32 _bdr_dpsdwlft_cmd_hdr_set_is_bdr_draw_interior(
    IN OUT  BDR_DPSDWLFT_CTX_T          *pt_ctx,
    IN      VOID                        *pv_param1);

static INT32 _bdr_dpsdwlft_cmd_hdr_get_is_bdr_draw_interior(
    IN      BDR_DPSDWLFT_CTX_T          *pt_ctx,
    OUT     VOID                        *pv_param1);

static INT32 _bdr_dpsdwlft_cmd_hdr_set_bdr_color(
    IN OUT  BDR_DPSDWLFT_CTX_T          *pt_ctx,
    IN      VOID                        *pv_param1,
    IN      VOID                        *pv_param2);

static INT32 _bdr_dpsdwlft_cmd_hdr_get_bdr_color(
    IN      BDR_DPSDWLFT_CTX_T          *pt_ctx,
    IN      VOID                        *pv_param1,
    OUT     VOID                        *pv_param2);
    
static INT32 _bdr_dpsdwlft_cmd_hdr_get_preferred_inset(
    IN      BDR_DPSDWLFT_CTX_T          *pt_ctx,
    OUT     VOID                        *pv_param1);
    
static INT32 _bdr_dpsdwlft_cmd_hdr_set_height(
    IN OUT  BDR_DPSDWLFT_CTX_T          *pt_ctx,
    IN      VOID                        *pv_param1);
    
static INT32 _bdr_dpsdwlft_cmd_hdr_get_height(
    IN      BDR_DPSDWLFT_CTX_T          *pt_ctx,
    OUT     VOID                        *pv_param1);
    
static INT32 _bdr_dpsdwlft_cmd_hdr_set_shadow_alpha_factor(
    IN      BDR_DPSDWLFT_CTX_T          *pt_ctx,
    IN      VOID                        *pv_param1);
    
static INT32 _bdr_dpsdwlft_cmd_hdr_get_shadow_alpha_factor(
    IN      BDR_DPSDWLFT_CTX_T          *pt_ctx,
    OUT     VOID                        *pv_param1);
    
static VOID _bdr_dpsdwlft_tool_copy_from_color_info_to_extend_color_set(
    OUT     WGL_COLOR_EXT_SET_T         *pt_clrs_ex,
    IN      WGL_COLOR_INFO_T            *pt_clr_info_from);
    
static VOID _bdr_dpsdwlft_tool_copy_from_extend_color_set_to_color_info( 
    OUT     WGL_COLOR_INFO_T            *pt_clr_info_out, 
    IN      WGL_COLOR_SET_TYPE_T        e_clrs_type,
    IN      WGL_COLOR_EXT_SET_T         *pt_clrs_ex_from);
    
static VOID _bdr_dpsdwlft_tool_load_sys_def_clrs_bk(
    IN OUT  BDR_DPSDWLFT_CTX_T              *pt_ctx);
    
static VOID _bdr_dpsdwlft_tool_load_sys_def_clrs_fg(
    IN OUT  BDR_DPSDWLFT_CTX_T              *pt_ctx);
    
static VOID _bdr_dpsdwlft_tool_select_color(
    IN      WGL_COLOR_EXT_SET_T         *pt_clrs_src,
    IN      UINT16                      ui2_state, 
    OUT     GL_COLOR_T                  **ppt_clr_out);

static VOID _bdr_dpsdwlft_tool_apply_shadow_alpha_factor(
    IN OUT  BDR_DPSDWLFT_CTX_T              *pt_ctx);
...
/*-----------------------------------------------------------------------------
                    exported functions 
 ----------------------------------------------------------------------------*/
INT32 x_wgl_bdr_drop_shadow_left_init(VOID)
{
    WGL_BORDER_CTRL_INTF_T      t_bdr_intf = 
        {
            wgl_bdr_drop_shadow_left_inst_create,
            wgl_bdr_drop_shadow_left_inst_destroy,
            wgl_bdr_drop_shadow_left_cmd_proc,
            wgl_bdr_drop_shadow_left_paint
        };
    
    return wgl_register_border_intf(WGL_BORDER_DROP_SHADOW_LEFT, &t_bdr_intf);
}
...
/*-----------------------------------------------------------------------------
                    public functions 
 ----------------------------------------------------------------------------*/
INT32 wgl_bdr_drop_shadow_left_inst_create(
    WGL_BORDER_CONTEXT_HANDLE_T         *ppv_ctx_bdr, 
    WGL_BORDER_CREATE_INPUT_T           *pt_bdr_create)
{
    BDR_DPSDWLFT_CTX_T  *pt_ctx = NULL;
    INT32               i4_ret;

    *ppv_ctx_bdr    = NULL;
    
    pt_ctx = (BDR_DPSDWLFT_CTX_T *)WGL_MEM_ALLOC(sizeof(BDR_DPSDWLFT_CTX_T));
    if(pt_ctx==NULL)
    {
...
#ifdef CLI_LVL_ALL   
    pt_ctx->ui4_mem_size = sizeof(BDR_DPSDWLFT_CTX_T);
#endif
    ...
    WGL_COPY_INSET(&pt_ctx->t_inset_bdr, &BDR_DPSDWLFT_DFT_INSET);
    pt_ctx->ui2_height              = BDR_DPSDWLFT_DFT_HEIGHT;
    pt_ctx->ui1_shadow_alpha_factor = BDR_DPSDWLFT_DFT_SHADOW_ALPHA_FACTOR;
    wgl_cvsst_copy(&pt_ctx->t_cvsst, pt_bdr_create->pt_cvsst);
    _bdr_dpsdwlft_tool_load_sys_def_clrs_bk(pt_ctx);
    _bdr_dpsdwlft_tool_load_sys_def_clrs_fg(pt_ctx);
    
    *ppv_ctx_bdr = (WGL_BORDER_CONTEXT_HANDLE_T)pt_ctx; 
    return WGLR_OK;    
}

INT32 wgl_bdr_drop_shadow_left_inst_destroy(
...
INT32 wgl_bdr_drop_shadow_left_paint(
    WGL_BORDER_CONTEXT_HANDLE_T         pv_ctx_bdr,
    GL_HGRAPHICS_T                      h_g,
    UINT32                              ui4_upt_rect,
    ...
    UINT16                              ui2_state)
{
    BDR_DPSDWLFT_CTX_T  *pt_ctx = (BDR_DPSDWLFT_CTX_T *)pv_ctx_bdr;
    INT32               i4_ret;
    GL_RECT_T           t_rc_cnt;
    GL_RECT_T           t_rc_upt;
    ...
    if(pt_ctx->b_is_draw_intr)
    {
        ...
        _bdr_dpsdwlft_tool_select_color(&pt_ctx->t_clrs_bk, ui2_state, &pt_clr_bk);
    ...
    if(pt_ctx->ui2_height > 0)
    {
        ...
        _bdr_dpsdwlft_tool_select_color(&pt_ctx->t_clrs_fg, ui2_state, &pt_clr_shadow);
...
INT32 wgl_bdr_drop_shadow_left_cmd_proc(
    WGL_BORDER_CONTEXT_HANDLE_T         pv_ctx_bdr,
    UINT32                              ui4_cmd_id, 
    VOID                                *pv_param1, 
    VOID                                *pv_param2)
{
    ...
    if(!(WGL_IS_CMD_BELONG_BORDER(WGL_BORDER_BASE, ui4_cmd_id) ||
         WGL_IS_CMD_BELONG_BORDER(WGL_BORDER_DROP_SHADOW_LEFT, ui4_cmd_id)) )
    {
    ...
    switch(ui4_cmd_id) 
    {
        ...
        case WGL_BDR_GL_SET_SIZE:
            i4_ret_val = _bdr_dpsdwlft_cmd_hdr_set_size(pt_ctx, pv_param1, pv_param2);
            break;

        /*Common Border Commands*/
        case WGL_CMD_GL_SET_INSET:
            i4_ret_val = _bdr_dpsdwlft_cmd_hdr_set_inset(pt_ctx, pv_param1);
            break;
        
        case WGL_CMD_GL_GET_INSET:
            i4_ret_val = _bdr_dpsdwlft_cmd_hdr_get_inset(pt_ctx, pv_param1);
            break;

        case WGL_CMD_GL_GET_MIN_INSET:
            i4_ret_val = _bdr_dpsdwlft_cmd_hdr_get_min_inset(pt_ctx, pv_param1);
            break;
    
        case WGL_CMD_GL_SET_IS_BDR_DRAW_INTERIOR:
            i4_ret_val = _bdr_dpsdwlft_cmd_hdr_set_is_bdr_draw_interior(pt_ctx, pv_param1);
            break;

        case WGL_CMD_GL_GET_IS_BDR_DRAW_INTERIOR:
            i4_ret_val = _bdr_dpsdwlft_cmd_hdr_get_is_bdr_draw_interior(pt_ctx, pv_param1);
            break;

        case WGL_CMD_GL_SET_BDR_COLOR: /*pv_param1: (UINT8)target, pv_param2:(WGL_COLOR_INFO_T *)*pt_color_info*/
            i4_ret_val = _bdr_dpsdwlft_cmd_hdr_set_bdr_color(pt_ctx, pv_param1, pv_param2);
            break;

        case WGL_CMD_GL_GET_BDR_COLOR:
            i4_ret_val = _bdr_dpsdwlft_cmd_hdr_get_bdr_color(pt_ctx, pv_param1, pv_param2);
            break;

        /* Specific Border Commands */
        case WGL_CMD_BDR_DPSDWLFT_GET_PREFERRED_INSET:
            i4_ret_val = _bdr_dpsdwlft_cmd_hdr_get_preferred_inset(pt_ctx, pv_param1);
            break;
        
        case WGL_CMD_BDR_DPSDWLFT_SET_HEIGHT:
            i4_ret_val = _bdr_dpsdwlft_cmd_hdr_set_height(pt_ctx, pv_param1);
            break;

        case WGL_CMD_BDR_DPSDWLFT_GET_HEIGHT:
            i4_ret_val = _bdr_dpsdwlft_cmd_hdr_get_height(pt_ctx, pv_param1);
            break;
        
        case WGL_CMD_BDR_DPSDWLFT_SET_SHADOW_ALPHA_FACTOR:
            i4_ret_val = _bdr_dpsdwlft_cmd_hdr_set_shadow_alpha_factor(pt_ctx, pv_param1);
            break;
        
        case WGL_CMD_BDR_DPSDWLFT_GET_SHADOW_ALPHA_FACTOR:
            i4_ret_val = _bdr_dpsdwlft_cmd_hdr_get_shadow_alpha_factor(pt_ctx, pv_param1);
            break;
...
/*-----------------------------------------------------------------------------
 ...
 ----------------------------------------------------------------------------*/
/*-----------------------------------------------------------------------------
 * Name: _bdr_dpsdwlft_cmd_hdr_set_size
 *
...
/*-----------------------------------------------------------------------------
 * Name: _bdr_dpsdwlft_cmd_hdr_set_inset
 *
...
/*-----------------------------------------------------------------------------
 * Name: _bdr_dpsdwlft_cmd_hdr_get_inset
 *
...
/*-----------------------------------------------------------------------------
 * Name: _bdr_dpsdwlft_cmd_hdr_get_min_inset
 *
    ...
    OUT     VOID                        *pv_param1)
{
    ...
    CHECK_NULL(pt_inset_out, WGLR_INV_ARG);
    _INSET_COPY(pt_inset_out, &BDR_DPSDWLFT_MIN_INSET);
        
    return WGLR_OK;
}        

/*-----------------------------------------------------------------------------
 * Name: _bdr_dpsdwlft_cmd_hdr_set_is_bdr_draw_interior
 *
...
/*-----------------------------------------------------------------------------
 * Name: _bdr_dpsdwlft_cmd_hdr_get_is_bdr_draw_interior
 *
...
/*-----------------------------------------------------------------------------
 * Name: _bdr_dpsdwlft_cmd_hdr_set_bdr_color
 *
    ...
    else
    {
        ...
        switch(ui1_target)
        {
            ...
            case WGL_CLR_BDR_BK:
                ...
                _bdr_dpsdwlft_tool_copy_from_color_info_to_extend_color_set(&pt_ctx->t_clrs_bk, pt_clr_info);
            break;
            
            case WGL_CLR_BDR_FG:
                pt_ctx->e_clrs_type_src_fg = pt_clr_info->e_type;
				_bdr_dpsdwlft_tool_copy_from_color_info_to_extend_color_set(&pt_ctx->t_clrs_fg, pt_clr_info);
                ...
                _bdr_dpsdwlft_tool_apply_shadow_alpha_factor(pt_ctx);
            break;
...
/*-----------------------------------------------------------------------------
 * Name: _bdr_dpsdwlft_cmd_hdr_get_bdr_color
 *
    ...
    OUT     VOID                        *pv_param2)
{
    ...
    switch(ui1_target)
    {
        case WGL_CLR_BK:
        case WGL_CLR_BDR_BK:
            _bdr_dpsdwlft_tool_copy_from_extend_color_set_to_color_info(pt_clr_info_out, pt_ctx->e_clrs_type_src_bk, &pt_ctx->t_clrs_bk);
        break;
        
        case WGL_CLR_BDR_FG:
            _bdr_dpsdwlft_tool_copy_from_extend_color_set_to_color_info(pt_clr_info_out, pt_ctx->e_clrs_type_src_fg, &pt_ctx->t_clrs_fg);
        break;
...
/*-----------------------------------------------------------------------------
 * Name: _bdr_dpsdwlft_cmd_hdr_get_preferred_inset
 *
    ...
    OUT     VOID                        *pv_param1)
{
    ...
    CHECK_NULL(pt_inset_out, WGLR_INV_ARG);
    pt_inset_out->i4_left   = (INT32) pt_ctx->ui2_height;
    pt_inset_out->i4_top    = 0;
    pt_inset_out->i4_right  = 0;
...
/*-----------------------------------------------------------------------------
 * Name: _bdr_dpsdwlft_cmd_hdr_set_height
 *
...
/*-----------------------------------------------------------------------------
 * Name: _bdr_dpsdwlft_cmd_hdr_get_height
 *
...
/*-----------------------------------------------------------------------------
 * Name: _bdr_dpsdwlft_cmd_hdr_set_shadow_alpha_factor
 *
...
/*-----------------------------------------------------------------------------
 * Name: _bdr_dpsdwlft_cmd_hdr_get_shadow_alpha_factor
 *
...
/*-----------------------------------------------------------------------------
 * Name: _bdr_dpsdwlft_tool_copy_from_color_info_to_extend_color_set
 *
...
/*-----------------------------------------------------------------------------
 * Name: _bdr_dpsdwlft_tool_copy_from_extend_color_set_to_color_info
 *
...
/*-----------------------------------------------------------------------------
 * Name: _bdr_dpsdwlft_tool_load_sys_def_clrs_bk
 *
...
/*-----------------------------------------------------------------------------
 * Name: _bdr_dpsdwlft_tool_select_color
 *
...
/*-----------------------------------------------------------------------------
 * Name: _bdr_dpsdwlft_tool_apply_shadow_alpha_factor
 *
