...
#include "wgl/bdr_depressed/wgl_bdr_depressed.h"
#include "wgl/bdr_depressed/u_wgl_bdr_depressed.h"

/*-----------------------------------------------------------------------------
                    constants
 ----------------------------------------------------------------------------*/
#define BDR_DPRS_DFT_DEPTH  3

static const WGL_INSET_T  BDR_DPRS_DFT_INSET = 
    {
        BDR_DPRS_DFT_DEPTH,
        BDR_DPRS_DFT_DEPTH,
        BDR_DPRS_DFT_DEPTH,
...
static const WGL_INSET_T  BDR_DPRS_MIN_INSET = 
    {
        0,
        0,
        0,
...
#define DBG_MODULE_NAME "BDRDPRS"

#define IN
#define OUT

#define GET_LT_START_R(_ui1_r)          \
    _bdr_dprs_tool_multiply_factor_uint8((_ui1_r), 172, 236)

#define GET_LT_START_G(_ui1_g)          \
    _bdr_dprs_tool_multiply_factor_uint8((_ui1_g), 168, 233)

#define GET_LT_START_B(_ui1_b)          \
    _bdr_dprs_tool_multiply_factor_uint8((_ui1_b), 153, 216)
    
#define GET_LT_END_R(_ui1_r)            \
    _bdr_dprs_tool_multiply_factor_uint8((_ui1_r), 113, 236)

#define GET_LT_END_G(_ui1_g)            \
    _bdr_dprs_tool_multiply_factor_uint8((_ui1_g), 111, 233)

#define GET_LT_END_B(_ui1_b)            \
    _bdr_dprs_tool_multiply_factor_uint8((_ui1_b), 100, 216)

#define GET_RB_START_R(_ui1_r)          \
    _bdr_dprs_tool_multiply_factor_uint8((_ui1_r), 255, 236)

#define GET_RB_START_G(_ui1_g)          \
    _bdr_dprs_tool_multiply_factor_uint8((_ui1_g), 255, 233)

#define GET_RB_START_B(_ui1_b)          \
    _bdr_dprs_tool_multiply_factor_uint8((_ui1_b), 255, 216)

#define GET_RB_END_R(_ui1_r)            \
    _bdr_dprs_tool_multiply_factor_uint8((_ui1_r), 241, 236)

#define GET_RB_END_G(_ui1_g)            \
    _bdr_dprs_tool_multiply_factor_uint8((_ui1_g), 239, 233)

#define GET_RB_END_B(_ui1_b)            \
    _bdr_dprs_tool_multiply_factor_uint8((_ui1_b), 226, 216)
    

/*-----------------------------------------------------------------------------
                    structures
 ----------------------------------------------------------------------------*/
typedef struct _BDR_DPRS_CTX_T
{
#ifdef CLI_LVL_ALL     
...
/*-----------------------------------------------------------------------------
 ...
 ----------------------------------------------------------------------------*/
static INT32 _bdr_dprs_cmd_hdr_set_size(
    IN OUT  BDR_DPRS_CTX_T              *pt_ctx,
    IN      VOID                        *pv_param1,
    IN      VOID                        *pv_param2);
    
static INT32 _bdr_dprs_cmd_hdr_set_inset(
    IN OUT  BDR_DPRS_CTX_T              *pt_ctx,
    IN      VOID                        *pv_param1);

static INT32 _bdr_dprs_cmd_hdr_get_inset(
    IN      BDR_DPRS_CTX_T              *pt_ctx,
    OUT     VOID                        *pv_param1);
    
static INT32 _bdr_dprs_cmd_hdr_get_min_inset(
    IN      BDR_DPRS_CTX_T              *pt_ctx,
    OUT     VOID                        *pv_param1);
 
static INT32 _bdr_dprs_cmd_hdr_set_is_bdr_draw_interior(
    IN OUT  BDR_DPRS_CTX_T              *pt_ctx,
    IN      VOID                        *pv_param1);

static INT32 _bdr_dprs_cmd_hdr_get_is_bdr_draw_interior(
    IN      BDR_DPRS_CTX_T              *pt_ctx,
    OUT     VOID                        *pv_param1);

static INT32 _bdr_dprs_cmd_hdr_set_bdr_color(
    IN OUT  BDR_DPRS_CTX_T              *pt_ctx,
    IN      VOID                        *pv_param1,
    IN      VOID                        *pv_param2);

static INT32 _bdr_dprs_cmd_hdr_get_bdr_color(
    IN      BDR_DPRS_CTX_T              *pt_ctx,
    IN      VOID                        *pv_param1,
    OUT     VOID                        *pv_param2);

static INT32 _bdr_dprs_cmd_hdr_get_preferred_inset(
    IN      BDR_DPRS_CTX_T              *pt_ctx,
    OUT     VOID                        *pv_param1);
    
static INT32 _bdr_dprs_cmd_hdr_set_depth(
    IN OUT  BDR_DPRS_CTX_T              *pt_ctx,
    IN      VOID                        *pv_param1);
    
static INT32 _bdr_dprs_cmd_hdr_get_depth(
    IN      BDR_DPRS_CTX_T              *pt_ctx,
    OUT     VOID                        *pv_param1);
        
static UINT8 _bdr_dprs_tool_interpolate_uint8(
    IN      UINT8                       ui1_start,
    IN      UINT8                       ui1_end,
...
static VOID _bdr_dprs_tool_copy_from_color_info_to_extend_color_set(
    OUT     WGL_COLOR_EXT_SET_T         *pt_clrs_ex,
    IN      WGL_COLOR_INFO_T            *pt_clr_info_from);
    
static VOID _bdr_dprs_tool_copy_from_extend_color_set_to_color_info( 
    OUT     WGL_COLOR_INFO_T            *pt_clr_info_out, 
    IN      WGL_COLOR_SET_TYPE_T        e_clrs_type,
    IN      WGL_COLOR_EXT_SET_T         *pt_clrs_ex_from);
    
static VOID _bdr_dprs_tool_load_sys_def_clrs_bk(
    IN OUT  BDR_DPRS_CTX_T              *pt_ctx);
    
static VOID _bdr_dprs_tool_load_sys_def_clrs_fg(
    IN OUT  BDR_DPRS_CTX_T              *pt_ctx);
    
static VOID _bdr_dprs_tool_select_color(
    IN      WGL_COLOR_EXT_SET_T         *pt_clrs_src,
    IN      UINT16                      ui2_state, 
    OUT     GL_COLOR_T                  **ppt_clr_out);

static UINT8 _bdr_dprs_tool_multiply_factor_uint8(
    IN      UINT8                       ui1_value,
 ...
 *          WGLR_OUT_OF_MEMORY  Out of memory
 ----------------------------------------------------------------------------*/
INT32 x_wgl_bdr_depressed_init(VOID)
{
    WGL_BORDER_CTRL_INTF_T      t_bdr_intf = 
        {
            wgl_bdr_depressed_inst_create,
            wgl_bdr_depressed_inst_destroy,
            wgl_bdr_depressed_cmd_proc,
            wgl_bdr_depressed_paint
        };
    
    return wgl_register_border_intf(WGL_BORDER_DEPRESSED, &t_bdr_intf);
}
...
/*-----------------------------------------------------------------------------
 ...
 ----------------------------------------------------------------------------*/
/*-----------------------------------------------------------------------------
 * Name: wgl_bdr_depressed_inst_create
 *
 ...
 *          WGLR_OUT_OF_MEMORY  Out of memory
 ----------------------------------------------------------------------------*/
INT32 wgl_bdr_depressed_inst_create(
    WGL_BORDER_CONTEXT_HANDLE_T         *ppv_ctx_bdr, 
    WGL_BORDER_CREATE_INPUT_T           *pt_bdr_create)
{
    BDR_DPRS_CTX_T  *pt_ctx = NULL;
    INT32           i4_ret;

    *ppv_ctx_bdr    = NULL;
    
    pt_ctx = (BDR_DPRS_CTX_T *)WGL_MEM_ALLOC(sizeof(BDR_DPRS_CTX_T));
    if(pt_ctx==NULL)
    {
...
#ifdef CLI_LVL_ALL   
    pt_ctx->ui4_mem_size = sizeof(BDR_DPRS_CTX_T);
#endif
    ...
    WGL_COPY_INSET(&pt_ctx->t_inset_bdr, &BDR_DPRS_DFT_INSET);
    pt_ctx->ui2_depth          = BDR_DPRS_DFT_DEPTH;
    wgl_cvsst_copy(&pt_ctx->t_cvsst, pt_bdr_create->pt_cvsst);
    _bdr_dprs_tool_load_sys_def_clrs_bk(pt_ctx);
    _bdr_dprs_tool_load_sys_def_clrs_fg(pt_ctx);
...
/*-----------------------------------------------------------------------------
 * Name: wgl_bdr_depressed_inst_destroy
 *
 ...
 * Returns: WGLR_OK             Routine successful.
 ----------------------------------------------------------------------------*/
INT32 wgl_bdr_depressed_inst_destroy(
...
INT32 wgl_bdr_depressed_paint(
    ...
    UINT16                              ui2_state)
{
    BDR_DPRS_CTX_T  *pt_ctx = (BDR_DPRS_CTX_T *)pv_ctx_bdr;
    INT32           i4_ret;
    ...
    if(pt_ctx->b_is_draw_intr)
    {
        ...
        _bdr_dprs_tool_select_color(&pt_ctx->t_clrs_bk, ui2_state, &pt_clr_bk);
    ...
    /* prepare colors */
    _bdr_dprs_tool_select_color(&pt_ctx->t_clrs_fg, ui2_state, &pt_clr_fg);
    ...
    for(i4_depth=0; i4_depth<((INT32)pt_ctx->ui2_depth); i4_depth++)
    {
        ...
        if(b_render_left || b_render_top)
        {
            ...
            t_dclr_lt.u1.r = _bdr_dprs_tool_interpolate_uint8(t_dclr_lt_start.u1.r, t_dclr_lt_end.u1.r, 0, (INT32)(pt_ctx->ui2_depth)-1, i4_depth);
            t_dclr_lt.u2.g = _bdr_dprs_tool_interpolate_uint8(t_dclr_lt_start.u2.g, t_dclr_lt_end.u2.g, 0, (INT32)(pt_ctx->ui2_depth)-1, i4_depth);
            t_dclr_lt.u3.b = _bdr_dprs_tool_interpolate_uint8(t_dclr_lt_start.u3.b, t_dclr_lt_end.u3.b, 0, (INT32)(pt_ctx->ui2_depth)-1, i4_depth);
        ...
        if(b_render_right || b_render_bottom)
        {
            ...
            t_dclr_rb.u1.r = _bdr_dprs_tool_interpolate_uint8(t_dclr_rb_start.u1.r, t_dclr_rb_end.u1.r, 0, (INT32)(pt_ctx->ui2_depth)-1, i4_depth);
            t_dclr_rb.u2.g = _bdr_dprs_tool_interpolate_uint8(t_dclr_rb_start.u2.g, t_dclr_rb_end.u2.g, 0, (INT32)(pt_ctx->ui2_depth)-1, i4_depth);
            t_dclr_rb.u3.b = _bdr_dprs_tool_interpolate_uint8(t_dclr_rb_start.u3.b, t_dclr_rb_end.u3.b, 0, (INT32)(pt_ctx->ui2_depth)-1, i4_depth);
...
/*-----------------------------------------------------------------------------
 * Name: wgl_bdr_depressed_cmd_proc
 *
 ...
 *          < 0                 Other errors
 ----------------------------------------------------------------------------*/
INT32 wgl_bdr_depressed_cmd_proc(
    WGL_BORDER_CONTEXT_HANDLE_T         pv_ctx_bdr,
    UINT32                              ui4_cmd_id, 
    VOID                                *pv_param1, 
    VOID                                *pv_param2)
{
    ...
    if(!(WGL_IS_CMD_BELONG_BORDER(WGL_BORDER_BASE, ui4_cmd_id) ||
         WGL_IS_CMD_BELONG_BORDER(WGL_BORDER_DEPRESSED, ui4_cmd_id)) )
    {
    ...
    switch(ui4_cmd_id) 
    {
        ...
        case WGL_BDR_GL_SET_SIZE:
            i4_ret_val = _bdr_dprs_cmd_hdr_set_size(pt_ctx, pv_param1, pv_param2);
            break;

        /*Common Border Commands*/
        case WGL_CMD_GL_SET_INSET:
            i4_ret_val = _bdr_dprs_cmd_hdr_set_inset(pt_ctx, pv_param1);
            break;
        
        case WGL_CMD_GL_GET_INSET:
            i4_ret_val = _bdr_dprs_cmd_hdr_get_inset(pt_ctx, pv_param1);
            break;

        case WGL_CMD_GL_GET_MIN_INSET:
            i4_ret_val = _bdr_dprs_cmd_hdr_get_min_inset(pt_ctx, pv_param1);
            break;
    
        case WGL_CMD_GL_SET_IS_BDR_DRAW_INTERIOR:
            i4_ret_val = _bdr_dprs_cmd_hdr_set_is_bdr_draw_interior(pt_ctx, pv_param1);
            break;

        case WGL_CMD_GL_GET_IS_BDR_DRAW_INTERIOR:
            i4_ret_val = _bdr_dprs_cmd_hdr_get_is_bdr_draw_interior(pt_ctx, pv_param1);
            break;

        case WGL_CMD_GL_SET_BDR_COLOR: /*pv_param1: (UINT8)target, pv_param2:(WGL_COLOR_INFO_T *)*pt_color_info*/
            i4_ret_val = _bdr_dprs_cmd_hdr_set_bdr_color(pt_ctx, pv_param1, pv_param2);
            break;

        case WGL_CMD_GL_GET_BDR_COLOR:
            i4_ret_val = _bdr_dprs_cmd_hdr_get_bdr_color(pt_ctx, pv_param1, pv_param2);
            break;

        /*Specific Border Commands*/
        case WGL_CMD_BDR_DPRS_GET_PREFERRED_INSET:
            i4_ret_val = _bdr_dprs_cmd_hdr_get_preferred_inset(pt_ctx, pv_param1);
            break;

        case WGL_CMD_BDR_DPRS_SET_DEPTH:
            i4_ret_val = _bdr_dprs_cmd_hdr_set_depth(pt_ctx, pv_param1);
            break;

        case WGL_CMD_BDR_DPRS_GET_DEPTH:
            i4_ret_val = _bdr_dprs_cmd_hdr_get_depth(pt_ctx, pv_param1);
            break;
...
/*-----------------------------------------------------------------------------
 ...
 ----------------------------------------------------------------------------*/
/*-----------------------------------------------------------------------------
 * Name: _bdr_dprs_cmd_hdr_set_size
 *
...
/*-----------------------------------------------------------------------------
 * Name: _bdr_dprs_cmd_hdr_set_inset
 *
...
/*-----------------------------------------------------------------------------
 * Name: _bdr_dprs_cmd_hdr_get_inset
 *
...
/*-----------------------------------------------------------------------------
 * Name: _bdr_dprs_cmd_hdr_get_min_inset
 *
    ...
    OUT     VOID                        *pv_param1)
{
    ...
    CHECK_NULL(pt_inset_out, WGLR_INV_ARG);
    _INSET_COPY(pt_inset_out, &BDR_DPRS_MIN_INSET);
    
    return WGLR_OK;
}        

/*-----------------------------------------------------------------------------
 * Name: _bdr_dprs_cmd_hdr_set_is_bdr_draw_interior
 *
...
/*-----------------------------------------------------------------------------
 * Name: _bdr_dprs_cmd_hdr_get_is_bdr_draw_interior
 *
...
/*-----------------------------------------------------------------------------
 * Name: _bdr_dprs_cmd_hdr_set_bdr_color
 *
    ...
    else
    {
        ...
        switch(ui1_target)
        {
            ...
            case WGL_CLR_BDR_BK:
                ...
                _bdr_dprs_tool_copy_from_color_info_to_extend_color_set(&pt_ctx->t_clrs_bk, pt_clr_info);
            break;
            
            case WGL_CLR_BDR_FG:
                pt_ctx->e_clrs_type_src_fg = pt_clr_info->e_type;
                _bdr_dprs_tool_copy_from_color_info_to_extend_color_set(&pt_ctx->t_clrs_fg, pt_clr_info);
            break;
...
/*-----------------------------------------------------------------------------
 * Name: _bdr_dprs_cmd_hdr_get_bdr_color
 *
    ...
    OUT     VOID                        *pv_param2)
{
    ...
    switch(ui1_target)
    {
        case WGL_CLR_BK:
        case WGL_CLR_BDR_BK:
            _bdr_dprs_tool_copy_from_extend_color_set_to_color_info(pt_clr_info_out, pt_ctx->e_clrs_type_src_bk, &pt_ctx->t_clrs_bk);
        break;
        
        case WGL_CLR_BDR_FG:
            _bdr_dprs_tool_copy_from_extend_color_set_to_color_info(pt_clr_info_out, pt_ctx->e_clrs_type_src_fg, &pt_ctx->t_clrs_fg);
        break;
...
/*-----------------------------------------------------------------------------
 * Name: _bdr_dprs_cmd_hdr_get_preferred_inset
 *
...
/*-----------------------------------------------------------------------------
 * Name: _bdr_dprs_cmd_hdr_set_depth
 *
 * Description: This command handler sets the depth 
 *
 * Inputs:  pt_ctx      Contains the context of the border
 *          pv_param1   (UINT16) Contains the depth
 *
 * Outputs: -
 *                    
 * Returns: WGLR_OK         Routine successful.
...
/*-----------------------------------------------------------------------------
 * Name: _bdr_dprs_cmd_hdr_get_depth
 *
 * Description: This command handler gets the depth
 *
 * Inputs:  pt_ctx      Contains the context of the border
 *
 * Outputs: pv_param1   (UINT16 *) Returns the depth
 *                    
 * Returns: WGLR_OK         Routine successful.
 *          WGLR_INV_ARG    The argument pv_param1 is NULL
 ----------------------------------------------------------------------------*/
...
/*-----------------------------------------------------------------------------
 * Name: _bdr_dprs_tool_interpolate_uint8
 *
...
/*-----------------------------------------------------------------------------
 * Name: _bdr_dprs_tool_copy_from_color_info_to_extend_color_set
 *
...
/*-----------------------------------------------------------------------------
 * Name: _bdr_dprs_tool_copy_from_extend_color_set_to_color_info
 *
...
/*-----------------------------------------------------------------------------
 * Name: _bdr_dprs_tool_load_sys_def_clrs_bk
 *
...
/*-----------------------------------------------------------------------------
 * Name: _bdr_dprs_tool_select_color
 *
...
/*-----------------------------------------------------------------------------
 * Name: _bdr_dprs_tool_multiply_factor_uint8
 *
