...
#include "file_mngr/dlnafs/fm_dlnadev.h"
#include <errno.h>
#include <stdio.h>
#include <unistd.h>
#include <string.h>
...
#ifdef DLNA_SUPPORT
static FM_XENTRY_OP_T t_dlnadev_xent_ops =
{
    rootfs_lookup,
    dlnadev_read,
    dlnadev_write,
...
#endif
#ifdef SMB_SUPPORT
static FM_XENTRY_OP_T t_smbdev_xent_ops =
{
    rootfs_lookup,
    NULL,
    NULL,
...
#endif
static FM_XENTRY_OP_T t_chrdev_xent_ops =
{
    rootfs_lookup,
    chrdev_read,
    chrdev_write,
    ...
    ROOTFS_ENTRY_T  *pt_rent)
{
    ...
    *ppt_rent = pt_rent;
    ...
    if (pt_xent != NULL)
    {
        ...
        {
            pt_xent->pt_xent_ops = &t_chrdev_xent_ops;
        }
#ifdef DLNA_SUPPORT
        else if (IS_XENTRY_DLNA_DMS(pt_xent))
        {
            pt_xent->pt_xent_ops = &t_dlnadev_xent_ops;
        }
#endif
#ifdef SMB_SUPPORT
        else if (IS_XENTRY_SMB(pt_xent))
        {
            pt_xent->pt_xent_ops = &t_smbdev_xent_ops;
        }
#endif
        else
        {
...
static INT32 rootfs_make_entry(VOID *pv_data)
{
    ...
    if (FM_IS_BLK(pt_arg->ui4_mode) || FM_IS_CHR(pt_arg->ui4_mode))
    {
        DBG_INFO(("[FM]%s Make BLK or CHR entry shall always be existed\n", __FUNCTION__));
    }
    else
    {
        CHAR            *ps_name;
    ...
    switch (pt_arg->ui4_mode & FM_MODE_TYPE_MASK)
    {
        ...
        case FM_MODE_TYPE_PTP_MTP:
#ifdef DLNA_SUPPORT
        case FM_MODE_TYPE_DLNA_DMS:
#endif
#ifdef SMB_SUPPORT
        case FM_MODE_TYPE_SMB:
#endif
    ...
    pt_new_rent->ui8_create_time = GET_CUR_TIME(NULL);
    ...
    if (FM_IS_BLK(pt_arg->ui4_mode) || FM_IS_PTP_MTP(pt_arg->ui4_mode) || FM_IS_DLNA_DMS(pt_arg->ui4_mode) || FM_IS_SMB(pt_arg->ui4_mode))
    {
...
static INT32 rootfs_write(VOID *pv_data)
{
    ...
    {
        ui4_more_bytes = (UINT32)(pt_arg->ui8_offset + pt_arg->ui4_count -
                         pt_rent->z_file_size);
    ...
    pt_rent->ui8_modify_time = GET_CUR_TIME(NULL);
...
static INT32 rootfs_read(VOID *pv_data)
{
    ...
    {
        ui4_read_bytes = (UINT32)(pt_rent->z_file_size - pt_arg->ui8_offset);
        i4_ret = FMR_EOF;
    }
    ...
    pt_rent->ui8_access_time = GET_CUR_TIME(NULL);
...
static INT32 rootfs_lseek(VOID *pv_data)
{
    ...
    if (pt_arg->i8_offset > 0)
    {
        ...
        {
            if (pt_arg->i8_offset <= (INT64)ROOTFS_SIZE)  /* max FAT size is 4G */
            {
        ...
        {
            if ((pt_arg->i8_offset + pt_arg->ui8_cur_pos) <= (INT64)ROOTFS_SIZE)  /* max FAT size is 4G */
            {
                pt_arg->ui8_cur_pos += (UINT64)pt_arg->i8_offset;
            }
            else
        ...
        {
            if ((pt_arg->i8_offset + (UINT64)(pt_rent->z_file_size)) <= (INT64)ROOTFS_SIZE)  /* max FAT size is 4G */
            {
                pt_arg->ui8_cur_pos = (UINT64)pt_arg->i8_offset + (UINT64)(pt_rent->z_file_size);
            }
            else
            {
                return FMR_FILE_TOO_LARGE;
...
static INT32 rootfs_set_info(VOID *pv_data)
{
        ...
        (pt_info->ui8_size   != FM_SET_SIZE_NOVAL))
    {
        ...
        {
            z_new_size = (SIZE_T)(pt_info->ui8_size);
            ui8_offset = 0;
        }
...
static INT32 rootfs_read_dir(VOID *pv_data)
{
    ...
    ui8_offset = pt_arg->ui8_byte_offset;
    
    if ( pt_arg->ui4_ent_cnt >= 2)
    {
        if ( pt_arg->ui8_byte_offset == 0)
        { /* there are 2 rooms for ".", and ".." */
            x_strcpy(pt_dir_ent->s_name, ".");
            pt_dir_ent->ui1_len = (UINT8)x_strlen(".");
        ...
        else if ( pt_arg->ui8_byte_offset == 1)
        {
            x_strcpy(pt_dir_ent->s_name, "..");
            pt_dir_ent->ui1_len = (UINT8)x_strlen( "..");
         ...
         pt_child_rent = pt_child_rent->pt_next)
    {
        ...
        pt_dir_ent->ui1_len = (UINT8)x_strlen(pt_child_rent->ps_name);
    ...
    pt_arg->ui8_byte_offset  += ui4_read_cnt;
...
static INT32 rootfs_delete(VOID *pv_data)
{
    ...
    if (FM_IS_BLK(pt_xent->ui4_mode) || FM_IS_CHR(pt_xent->ui4_mode))
    {
        DBG_INFO(("[FM]%s BLK or CHR entry shall always be existed\n", __FUNCTION__));
    }
    else
    {
        CHAR            *ps_name;
        ...
        DBG_INFO(("[FM]%s rmdir:%s\n", __FUNCTION__, ps_name));
        if (rmdir(ps_name) < 0)
        {
            DBG_INFO(("[FM]%s rmdir:%s return %s\n", __FUNCTION__, ps_name, strerror(errno)));
        }
        else
        {
            DBG_INFO(("[FM]%s rmdir:%s success\n", __FUNCTION__, ps_name));
        }
        x_mem_free(ps_name);
        ps_name = NULL;
    }
...
FM_FS_DESC_T *rootfs_init(VOID *pv_data)
{
    ...
    if (pt_fs_desc != NULL)
    {
        ...
        pt_root_rent->ui8_create_time = GET_CUR_TIME(NULL);
