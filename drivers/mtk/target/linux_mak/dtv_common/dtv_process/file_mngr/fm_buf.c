...
#include "file_mngr/dlnafs/fm_dlnafs.h"
#include "inet/dlna/x_dlna_dmp_api.h"
#include "file_mngr/fm_util.h"
#include "file_mngr/fm_ufs.h"
#include "handle/handle.h"
#include "inet/dlna/u_dlna_dmp.h"
#ifdef DLNA_SUPPORT
#include "file_mngr/dlnafs/dlnafs_common.h"
#endif
/*-----------------------------------------------------------------------------
    ...
    FM_XENTRY_T     *pt_xent)
{
    ...
    else if (IS_XENTRY_DLNA_DMS(pt_xent->pt_mount->pt_dev_xent))
    {
        ui4_shift = 9;
    }
    #endif
...
#if 0
static FM_BUF_T *_fm_buf_alloc_new_ex(VOID)
{
    FM_BUF_T *pt_buf;

#if !FM_BUF_CB_MEM_POOLED
    pt_buf = x_mem_calloc(1, sizeof(FM_BUF_T));
#else
    pt_buf = x_mem_part_calloc(h_fm_mem_pool, 1, sizeof(FM_BUF_T));
#endif
...
#ifdef DLNA_SUPPORT
    t_io.h_file         = pt_buf->pt_rw_req->h_file;
#endif
...
#ifdef DLNA_SUPPORT
                    /*zink DLNA DMS*/
                if ((pt_rw_req->pt_xent != NULL) &&
                    (pt_rw_req->pt_xent->pt_mount != NULL) &&
                    (pt_rw_req->pt_xent->pt_mount->pt_dev_xent != NULL) &&
                    IS_XENTRY_DLNA_DMS(pt_rw_req->pt_xent->pt_mount->pt_dev_xent))
                {
                    if (IS_XENTRY_VIRT(pt_rw_req->pt_xent->pt_mount->pt_dev_xent))   /* virtual device needs offset */
                    {
                        DLNA_DENTRY_T* pt_dent;

                        pt_dent = DLNA_GET_DENTRY(pt_rw_req->pt_xent);

                        ui8_data_rlbn  = FM_BUF_LBN(pt_dent->ui8_size+FM_BUF_BLOCK_SIZE(pt_xent)-1, pt_xent);
                        ui8_new_size  = (UINT64)((ui8_data_rlbn - ui8_data_llbn) * FM_BUF_BLOCK_SIZE(pt_xent));
                    }
                    else
                    {
                        ASSERT(0);
        ...
        if (pt_rw_req->ui4_done_bytes == pt_rw_req->ui4_count)
        {
            ...
            else if (pt_rw_req->pf_nfy_fct != NULL)
            {
                if ((e_async_cond != FM_ASYNC_COND_FAIL) && 
                    (e_async_cond != FM_ASYNC_COND_BLANK_SECTOR) &&
                    (e_async_cond != FM_ASYNC_COND_CMD_TIMEOUT))
                {
                    if (pt_buf->ui2_flags & FM_BUF_FLAG_COPY_IN)
                    {
    ...
    UINT32          ui4_prefetch_limit)
{
    ...
    else if (IS_XENTRY_DLNA_DMS(pt_rw_req->pt_xent->pt_mount->pt_dev_xent))
    {
        if (IS_XENTRY_VIRT(pt_rw_req->pt_xent->pt_mount->pt_dev_xent))   /* virtual device needs offset */
        {
            /*pt_rw_req->ui8_offset = pt_rw_req->ui8_offset;*/
        }
        ui4_offset = 0;
    }
    else
 ...
 * Name: fm_buf_recycle
 *
 * Description: Purge all used buffer blocks of the specified xentry and free 
 ...
 *              Fail.
 ----------------------------------------------------------------------------*/
INT32 fm_buf_recycle(FM_XENTRY_T *pt_xent)
{
    FM_BUF_T *pt_buf;
    INT32    i4_ret  = FMR_OK;
    ...
    while ((pt_buf = DLIST_HEAD(&pt_xent->t_buf_list)) != NULL)
    {
        ...
        else if (IS_BUF_RETRY(pt_buf))
        {
            ...
            i4_ret = _fm_buf_flush_data(pt_buf, FALSE);
            ...
            if (i4_ret != FMR_OK)
            {
                ...
                _fm_buf_add_cache(pt_xent, pt_buf);
...
INT32 fm_buf_init(VOID)
{
    ...
    i4_ret = x_thread_create(&h_buf_thrd,
                             FM_BUF_THREAD_NAME,
                             FM_BUF_THREAD_DEFAULT_STACK_SIZE,
                             FM_BUF_THREAD_DEFAULT_PRIORITY,
