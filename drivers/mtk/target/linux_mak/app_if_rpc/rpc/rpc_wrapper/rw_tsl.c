...
/*-------------------------------------------------------------------------
 ...
 --------------------------------------------------------------------------*/
static INT32 _hndlr_x_tsl_nfy_fct(
    RPC_ID_T     t_rpc_id,
...
{  
    ...
    pt_return->e_type   = ARG_TYPE_VOID;
      ...
      ((x_tsl_nfy_fct)pv_cb_addr)(
            (HANDLE_T)pt_args[0].u.ui4_arg,
          (TSL_COND_T)pt_args[1].u.ui4_arg,
                      pt_args[2].u.ui4_arg,
                      pt_args[3].u.pv_arg,
                      pt_args[4].u.ui4_arg);
...
)
{
    ...
    RPC_CHECK(RPC_ADD_REF_STR(RPC_DEFAULT_ID, ps_tsl_name));
    
    RPC_ARG_INP(ARG_TYPE_REF_STR,    ps_tsl_name);
    ...
    RPC_ARG_OUT(ARG_TYPE_REF_UINT32,      ph_tsl);

    RPC_DO_OP("x_tsl_create");

    RPC_RETURN(ARG_TYPE_INT32, TSLR_INV_ARG);
}

static INT32 _x_tsl_fs_load
(
    HANDLE_T            h_dir,
...
)
{
    ...
    RPC_CHECK(RPC_ADD_REF_STR(RPC_DEFAULT_ID, ps_tsl_path));
    RPC_CHECK(RPC_ADD_REF_STR(RPC_DEFAULT_ID, ps_tsl_name));
    
    RPC_ARG_INP(ARG_TYPE_UINT32,        h_dir);
    RPC_ARG_INP(ARG_TYPE_REF_STR, ps_tsl_path);
    RPC_ARG_INP(ARG_TYPE_UINT16,   ui2_tsl_id);
    RPC_ARG_INP(ARG_TYPE_REF_STR, ps_tsl_name);
    RPC_ARG_INP(ARG_TYPE_REF_VOID,     pv_tag);
    RPC_ARG_INP(ARG_TYPE_REF_FUNC,     pf_nfy);
    ...
    RPC_DO_OP("x_tsl_fs_load");

    RPC_RETURN(ARG_TYPE_INT32, TSLR_INV_ARG);
}

static INT32 _x_tsl_open
(
    UINT16              ui2_tsl_id,
    VOID*               pv_tag,
    x_tsl_nfy_fct       pf_nfy,
...
)
{
    ...
    RPC_DO_OP("x_tsl_open");

    RPC_RETURN(ARG_TYPE_INT32, TSLR_INV_ARG);
}

static INT32 _x_tsl_open_by_name
(
    const CHAR*         ps_tsl_name,
    VOID*               pv_tag,
    x_tsl_nfy_fct       pf_nfy,
...
)
{
    ...
    i4_rc = _x_tsl_create(ps_tsl_name, ui2_tsl_id, e_mem_policy,
...
)
{
    ...
    RPC_DO_OP("x_tsl_clean");

    RPC_RETURN(ARG_TYPE_INT32, TSLR_INV_ARG);
}

/*----------------------------------------------------------------------
 * Name: c_tsl_set_src_name 
 *
 * Description: This function sets the tuner source name for all the 
 *              transport stream records in this TSL.     
 *
 * Inputs: 
 *   h_tsl        Handle to the TSL object.
 ...
 *                (MAX_TSL_SRC_NAME_LEN+1) bytes.
 *
 * Outputs: None
 *
 * Returns: 
 *   TSLR_OK           Tuner source name is successfully stored.
 *   TSLR_INV_HANDLE   Bad TSL handle.
 *   TSLR_INV_ARG      'ps_src_name' is null. 
 -----------------------------------------------------------------------*/
INT32 c_tsl_set_src_name 
(
    HANDLE_T            h_tsl,
...
)
{
    ...
    RPC_CHECK(RPC_ADD_REF_DESC(RPC_DEFAULT_ID, pt_tsl_rec,  RPC_DESC_TSL_REC_T, NULL));
    
    RPC_ARG_INP(ARG_TYPE_UINT32,        h_tsl);
    RPC_ARG_INP(ARG_TYPE_REF_DESC, pt_tsl_rec);

    RPC_DO_OP("x_tsl_add_rec");
...
)
{
    ...
    RPC_ARG_INP(ARG_TYPE_UINT32,           h_tsl);
    RPC_ARG_INP(ARG_TYPE_UINT8, e_tsl_qry_option);
    RPC_ARG_OUT(ARG_TYPE_REF_UINT16, pui2_rec_id);

    RPC_DO_OP("x_tsl_qry_rec_id");
...
)
{
    ...
    i4_rc = _x_tsl_fs_load(h_dir, ps_tsl_path, ui2_tsl_id,
...
)
{
    ...
    RPC_ARG_OUT(ARG_TYPE_REF_UINT16, pui2_tsl_id);
    RPC_ARG_OUT(ARG_TYPE_REF_STR,    ps_tsl_name);

    RPC_DO_OP("x_tsl_fs_qry");
...
)
{
    ...
    i4_rc = _x_tsl_open(ui2_tsl_id, pv_tag, pf_nfy, ph_tsl);
...
)
{
    ...
    i4_rc = _x_tsl_open_by_name(ps_tsl_name, pv_tag, pf_nfy, ph_tsl);
...
)
{
    ...
    RPC_DO_OP("x_tsl_get_handle_info");
...
)
{
    ...
    RPC_ARG_INP(ARG_TYPE_UINT32,           h_tsl);
    RPC_ARG_INP(ARG_TYPE_UINT16,  ui2_tsl_rec_id);
    RPC_ARG_OUT(ARG_TYPE_REF_DESC,    pt_tsl_rec);
    RPC_ARG_OUT(ARG_TYPE_REF_UINT32, pui4_ver_id);

    RPC_DO_OP("x_tsl_get_rec");

    RPC_RETURN(ARG_TYPE_INT32, TSLR_INV_ARG);
}

/*----------------------------------------------------------------------
 * Name: c_tsl_get_src_name 
 *
 * Description: This API returns the source name associated with all 
 *              the transport stream records for this TSL.
 *
 * Inputs: 
 *   h_tsl        Handle to the TSL object.
 *
 ...
 *                MAX_TSL_NAME_LEN+1 bytes).  On return, this buffer 
 *                will be filled with TSL source name.
 * Returns: 
 *   TSLR_OK           TSL source name is returned. 
 *   TSLR_INV_HANDLE   Bad 'h_tsl' handle.
 *   TSLR_INV_ARG      'pt_src_name' is null. 
 -----------------------------------------------------------------------*/
INT32 c_tsl_get_src_name 
(
    HANDLE_T            h_tsl,
    CHAR*               ps_src_name 
)
...
)
{
    ...
    RPC_DO_OP("x_tsl_sort_rec");

    RPC_RETURN(ARG_TYPE_INT32, TSLR_INV_ARG);
}

INT32 c_tsl_init_mem_dump(TSL_MEM_DUMP_T * pt_mem_dump)
{
    RPC_DECL(1, INT32);

    RPC_CHECK(RPC_ADD_REF_DESC(RPC_DEFAULT_ID, pt_mem_dump,  RPC_DESC_TSL_MEM_DUMP_T, NULL));

    RPC_ARG_OUT(ARG_TYPE_REF_DESC, pt_mem_dump);

    RPC_DO_OP("x_tsl_init_mem_dump");

    RPC_RETURN(ARG_TYPE_INT32, TSLR_FAIL);
}

INT32 c_tsl_free_mem_dump(TSL_MEM_DUMP_T * pt_mem_dump)
{
    RPC_DECL(1, INT32);
    ...
    RPC_DO_OP("x_tsl_free_mem_dump");

    RPC_RETURN(ARG_TYPE_INT32, TSLR_FAIL);
}

INT32 c_tsl_dump_to_mem(HANDLE_T h_tsl,
                        TSL_MEM_DUMP_T * pt_mem_dump)
{
    ...
    RPC_DO_OP("x_tsl_dump_to_mem");

    RPC_RETURN(ARG_TYPE_INT32, TSLR_FAIL);
}

INT32 c_tsl_load_from_mem(HANDLE_T h_tsl,
                          TSL_MEM_DUMP_T * pt_mem_dump)
{
    RPC_DECL(2, INT32);
    ...
    RPC_DO_OP("x_tsl_load_from_mem");

    RPC_RETURN(ARG_TYPE_INT32, TSLR_FAIL);
}

INT32 c_tsl_get_rec_by_rec_idx(HANDLE_T h_tsl,
                               UINT16 ui2_idx,
                               TSL_REC_T * pt_tsl_rec,
                               UINT32 * pui4_ver_id)
{
    ...
    RPC_DO_OP("x_tsl_get_rec_by_rec_idx");

    RPC_RETURN(ARG_TYPE_INT32, TSLR_FAIL);
}

INT32 c_tsl_get_num_rec_by_sat(HANDLE_T h_tsl,
                               UINT32 ui4_mask,
                               UINT16 ui2_satl_id,
                               ...
                               UINT32 * pui4_ver_id)
{
    ...
    RPC_ARG_INP(ARG_TYPE_UINT32, h_tsl);
    RPC_ARG_INP(ARG_TYPE_UINT32, ui4_mask);
    RPC_ARG_INP(ARG_TYPE_UINT16, ui2_satl_id);
    RPC_ARG_INP(ARG_TYPE_UINT16, ui2_satl_rec_id);
    RPC_ARG_OUT(ARG_TYPE_REF_UINT16, pui2_num_recs);
    RPC_ARG_OUT(ARG_TYPE_REF_UINT32, pui4_ver_id);

    RPC_DO_OP("x_tsl_get_num_rec_by_sat");

    RPC_RETURN(ARG_TYPE_INT32, TSLR_FAIL);
}

INT32 c_tsl_get_rec_by_sat(HANDLE_T h_tsl,
                           UINT32 ui4_mask,
                           UINT16 ui2_satl_id,
                           UINT16 ui2_satl_rec_id,
                           UINT16 ui2_idx,
                           ...
                           UINT32 * pui4_ver_id)
{
    ...
    RPC_DO_OP("x_tsl_get_rec_by_sat");

    RPC_RETURN(ARG_TYPE_INT32, TSLR_FAIL);
}

INT32 c_rpc_reg_tsl_cb_hndlrs()
{
    RPC_REG_CB_HNDLR(x_tsl_nfy_fct);    
    return RPCR_OK;
}
