...
)
{
    ...
    switch (i4_rc)
    {
        case AEER_OK:
        {
            i4_rc = x_db_create(ui2_max_record,
                                e_memory_policy,
                                s_db_name,
                                ui1_count,
                                at_field_info,
            ...
            if (i4_rc == DBR_OK)
            {
                handle_link_to_aux(h_aux, *ph_client);
            }
            else
            {
        ...
        {
            i4_rc = DBR_AEE_OUT_OF_RESOURCES;
        }
        break;

        default:
        {
            i4_rc = DBR_AEE_NO_RIGHTS;
        }
        break;
    }
    return i4_rc;
...
)
{
    ...
    i4_rc = x_db_delete(h_client,s_db_name);
...
{
    return x_db_clean(h_client);
}

/*-----------------------------------------------------------------------------
 * Name:  c_db_get_handle_by_name
 *
...
)
{
    ...
    switch (i4_rc)
    {
        case AEER_OK:
        {
            i4_rc = x_db_get_handle_by_name(s_db_name,
                                            pv_tag,
                                            pf_nfy,
                                            ph_client);
            if (i4_rc == DBR_OK)
...
{
    return x_db_update_rec(h_client,
...
{
    return x_db_write_rec(h_client,
                          apv_key_field_values,
                          apv_field_values,
                          aui2_addr_len,
                          ui1_write_mode);
...
{
    return x_db_delete_rec(h_client,
                           ui1_key_field_count,
                           aui4_field_types,
                           apv_field_values,
                           ui2_record_index);
 ...
 *            DBR_INV_HANDLE Bad handle.
 ----------------------------------------------------------------------------*/
INT32 c_db_write_lock
(
    HANDLE_T h_client
)
{
    return x_db_write_lock(h_client);
}

/*-----------------------------------------------------------------------------
 * Name:  c_db_write_unlock
 *
 ...
 *            DBR_INV_HANDLE Bad handle.
 ----------------------------------------------------------------------------*/
INT32 c_db_write_unlock
(
    HANDLE_T h_client
)
{
    return x_db_write_unlock(h_client);
}
 ...
 *            DBR_INV_HANDLE Bad handle (or the database object is deleted.)
 ----------------------------------------------------------------------------*/
INT32 c_db_read_lock
(
    HANDLE_T h_client
)
{
    return x_db_read_lock(h_client);
}

/*-----------------------------------------------------------------------------
 * Name: c_db_read_unlock 
 *
 ...
 *            DBR_INV_HANDLE Bad handle (or the database object is deleted.)
 ----------------------------------------------------------------------------*/
INT32 c_db_read_unlock
(
    HANDLE_T h_client
)
{
    return x_db_read_unlock(h_client);
}
