...
#include "unicode/uc_str.h"
#include "unicode/x_uc_str.h"

#include "unicode/uc_cmn/unicode/utypes.h"
#include "unicode/uc_cmn/unicode/ustring.h"
 ...
 *          UCR_OK                  Success.
 ----------------------------------------------------------------------------*/
INT32 c_uc_ps_to_w2s(
    const CHAR*     ps_src,
    UTF16_T*        w2s_dst,
    UINT32          ui4_dst_len)
{
    return x_uc_ps_to_w2s(ps_src, w2s_dst, ui4_dst_len);
}
 ...
 *          UCR_OK                  Success.
 ----------------------------------------------------------------------------*/
INT32 c_uc_w2s_to_ps(
    const UTF16_T*  w2s_src,
    CHAR*           ps_dst,
    UINT32          ui4_dst_len)
{
    return x_uc_w2s_to_ps(w2s_src, ps_dst, ui4_dst_len);
}


/*-----------------------------------------------------------------------------
 * Name:  c_uc_w2s_strlen
 *
 ...
 * Returns: string length (UTF16_T characters).
 ----------------------------------------------------------------------------*/
SIZE_T c_uc_w2s_strlen(
    const UTF16_T*  w2s_str)
{
    return x_uc_w2s_strlen(w2s_str);
}


/*-----------------------------------------------------------------------------
 * Name:  c_uc_w2s_strcmp
 *
 ...
 *          < 0, w2s_str2 is larger than w2s_str1
 ----------------------------------------------------------------------------*/
INT32 c_uc_w2s_strcmp(
    const UTF16_T*  w2s_str1,
    const UTF16_T*  w2s_str2)
{
    return x_uc_w2s_strcmp(w2s_str1, w2s_str2);
}


/*-----------------------------------------------------------------------------
 * Name:  c_uc_w2s_strncmp
 *
 ...
 *          < 0, w2s_str2 is larger than w2s_str1
 ----------------------------------------------------------------------------*/
INT32 c_uc_w2s_strncmp(
    const UTF16_T*  w2s_str1,
    const UTF16_T*  w2s_str2,
    UINT32          ui4_len)
{
    return x_uc_w2s_strncmp(w2s_str1, w2s_str2, ui4_len);
}


/*-----------------------------------------------------------------------------
 * Name:  c_uc_w2s_strcat
 *
 ...
 * Returns: pointer of combined string
 ----------------------------------------------------------------------------*/
UTF16_T* c_uc_w2s_strcat(
    UTF16_T*        w2s_dst,
    const UTF16_T*  w2s_src)
{
    return x_uc_w2s_strcat(w2s_dst, w2s_src);
}


/*-----------------------------------------------------------------------------
 * Name:  c_uc_w2s_strncat
 *
 ...
 * Returns: pointer of combined string
 ----------------------------------------------------------------------------*/
UTF16_T* c_uc_w2s_strncat(
    UTF16_T*        w2s_dst,
    const UTF16_T*  w2s_src,
    UINT32          ui4_len)
{
    return x_uc_w2s_strncat(w2s_dst, w2s_src, ui4_len);
}


/*-----------------------------------------------------------------------------
 * Name:  c_uc_w2s_strcpy
 *
 * Description: The c_uc_w2s_strcpy function copies w2s_src, including the
 ...
 * Returns: pointer of w2s_dst
 ----------------------------------------------------------------------------*/
UTF16_T* c_uc_w2s_strcpy(
    UTF16_T*        w2s_dst,
    const UTF16_T*  w2s_src)
{
    return x_uc_w2s_strcpy(w2s_dst, w2s_src);
}


/*-----------------------------------------------------------------------------
 * Name:  c_uc_w2s_strncpy
 *
 * Description: The c_uc_w2s_strncpy function copies the initial ui4_len
 ...
 * Returns: pointer of w2s_dst
 ----------------------------------------------------------------------------*/
UTF16_T* c_uc_w2s_strncpy(
    UTF16_T*        w2s_dst,
    const UTF16_T*  w2s_src,
    UINT32          ui4_len)
{
    return x_uc_w2s_strncpy(w2s_dst, w2s_src, ui4_len);
}
 ...
 * Returns: The character in the specified index.
 ----------------------------------------------------------------------------*/
UTF16_T c_uc_w2s_get(
    const UTF16_T*  w2s_str,
    UINT32          ui4_idx)
{
    return x_uc_w2s_get(w2s_str, ui4_idx);
}


/*-----------------------------------------------------------------------------
 * Name:  c_uc_w2s_get_ptr
 *
 ...
 * Returns: The pointer of the character in the specified index.
 ----------------------------------------------------------------------------*/
UTF16_T* c_uc_w2s_get_ptr(
    const UTF16_T*  w2s_str,
    UINT32          ui4_idx)
{
    return x_uc_w2s_get_ptr(w2s_str, ui4_idx);
}
...
INT32 c_uc_w2s_set(
    ...
    UTF16_T         w2_char)
{
    return x_uc_w2s_set(w2s_str, ui4_idx, w2_char);
}
...
INT32 c_uc_w2s_reverse(
    UTF16_T*  w2s_str)
{
    return x_uc_w2s_reverse(w2s_str);
}
 ...
 *          if not                  NULL
 ----------------------------------------------------------------------------*/
UTF16_T* c_uc_w2s_chr(
    const UTF16_T*  w2s_str,
    UTF16_T         w2_ch)
{
    return x_uc_w2s_chr(w2s_str, w2_ch);
}


/*-----------------------------------------------------------------------------
 * Name:  c_uc_w2s_chr_ex
 *
 * Description: Find a character in a string.
 *
 * Inputs:  w2s_str                 String to be searched in.
 ...
 *          if not                  NULL
 ----------------------------------------------------------------------------*/
UTF16_T* c_uc_w2s_chr_ex(
    const UTF16_T*  w2s_str,
    UTF16_T         w2_ch,
    UINT32*         pui4_pos)
{
    return x_uc_w2s_chr_ex(w2s_str, w2_ch, pui4_pos);
}
 ...
 *          if not                  NULL
 ----------------------------------------------------------------------------*/
UTF16_T* c_uc_w2s_rchr(
    const UTF16_T*  w2s_str,
    UTF16_T         w2_ch)
{
    return x_uc_w2s_rchr(w2s_str, w2_ch);
}


/*-----------------------------------------------------------------------------
 * Name:  c_uc_w2s_toupper
 *
 ...
 *          UCR_OK                  Success
 ----------------------------------------------------------------------------*/
INT32 c_uc_w2s_toupper(
    UTF16_T*        w2s_dst,
    UINT32          ui4_dst_len,
    const UTF16_T*  w2s_src,
    const CHAR*     ps_locale)
{
    return x_uc_w2s_toupper(w2s_dst, ui4_dst_len, w2s_src, ps_locale);
}


/*-----------------------------------------------------------------------------
 * Name:  c_uc_w2s_tolower
 *
 ...
 *          UCR_OK                  Success
 ----------------------------------------------------------------------------*/
INT32 c_uc_w2s_tolower(
    UTF16_T*        w2s_dst,
    UINT32          ui4_dst_len,
    const UTF16_T*  w2s_src,
    const CHAR*     ps_locale)
{
    return x_uc_w2s_tolower(w2s_dst, ui4_dst_len, w2s_src, ps_locale);
}

INT32 c_multi_byte_to_unicode(UINT16* pui2_multi_byte, UINT32 ui4_len, 
                           E_MUL_BYTE_TYPE e_multi_type, UINT16* pui2_unicode)
{
    return x_multi_byte_to_unicode(pui2_multi_byte, ui4_len, 
                           e_multi_type, pui2_unicode);
}

INT32 c_multi_byte_to_unicode_ex(VOID* pv_multi_byte, UINT32 ui4_src_len, UINT32* pui4_cut_pos, 
                           E_MUL_BYTE_TYPE e_multi_type, UINT16* pui2_unicode, UINT32* pui4_dst_len)
{
    return x_multi_byte_to_unicode_ex(pv_multi_byte, ui4_src_len, pui4_cut_pos,
                                    e_multi_type, pui2_unicode, pui4_dst_len);
}
