...
THIS_LIB := lib$(strip $(THIS_COMPONENT)).a
...
ADD_DIR_WORD = _SRC_DIR_NEED_NEED_NEED


BUILD_DIR_LIST := $(addsuffix $(ADD_DIR_WORD),$(SUB_COMPONENTS) $(OPTIONAL_SUB_COMPONENTS))
...
.PHONY : all target lib depend mw x_inc c_inc custom sys sub_comp lint clean clean_target clean_a clean_o clean_d clean_lib clean_lint clean_mw clean_x clean_c clean_custom clean_sys clean_force $(SUB_COMP) $(ENT_SUB_COMP) $(ENT_OPT_SUB_COMP)


all target lib depend:
...
ifeq "$(LEVEL_TOP)" "TRUE"
	$(call if_dir_exist_fct,$(call path_cnvt,$(TARGET_LIB_DIR)), find $(call path_cnvt,$(TARGET_LIB_DIR))/ -name *.lock | xargs $(RM) $(RM_REC_FLAG) $(NULL_DEV);)
endif
	@$(MAKE) -f $(TARGET_ROOT)/all.mak $@ SUB_COMP= LEVEL_TOP=FALSE

x_inc c_inc :
	@$(MAKE) -C $(MW_ROOT) $@ ENTER_OPT_SUB_COMP=true



phase2_init : $(BUILD_DIR_LIST)
	@$(MAKE) -f $(TARGET_ROOT)/all.mak $@ SUB_COMP= LEVEL_TOP=FALSE

$(BUILD_DIR_LIST) :
	@$(if $(filter "$(findstring project_x/custom,$@)","project_x/custom"),,\
		$(shell if [ -e "$(ORIG_SRC_ROOT)/$(subst $(ADD_DIR_WORD),,$@)/$(TARGET_COMPONENT).mak" ]; then \
			$(MKDIR) $(MKDIR_FLAG) $(notdir $(subst $(ADD_DIR_WORD),,$@)); \
			$(LN)    $(LN_FLAG) $(ORIG_SRC_ROOT)/$(subst $(ADD_DIR_WORD),,$@)/$(TARGET_COMPONENT).mak $(notdir $(subst $(ADD_DIR_WORD),,$@))/Makefile; \
		else \
			$(LN)    $(LN_FLAG) $(subst $(ADD_DIR_WORD),,$@)/$(TARGET_COMPONENT).mak   $(notdir $(subst $(ADD_DIR_WORD),,$@))/Makefile; \
		fi \
		) \
	)
	
clean_init :
	$(foreach i,$(SUB_COMPONENTS),$(shell $(RM) $(RM_REC_FLAG) $(notdir $(i))))
...
ifeq "$(FAST_SUPPORT)" "false"
	...
	@$(MAKE) -f $(TARGET_ROOT)/all.mak all SUB_COMP=
endif
endif
