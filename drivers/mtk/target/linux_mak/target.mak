...
DFBLIB_PWD=$(VM_LINUX_ROOT)/chiling/ramdisk/rootfs/lib


LIB2 := 

ifeq ($(strip $(findstring -DANDROID, $(DEFINES))),)

INC += -I$(PROJECT_ROOT)/middleware/mheg5/OceanBlue/glue/gui_render/png/inc

INC += -I$(VM_LINUX_ROOT)/chiling/directfb/Source/DirectFB/include
INC += -I$(LINUX_PORTING_DTV_LINUX_PRJ_PATH)/dtv_process/include/ioctl_inc

else
INC += -I$(ANDROID_TOP)/bionic/libc/arch-arm/include
INC += -I$(ANDROID_TOP)/bionic/libc/include
INC += -I$(ANDROID_TOP)/bionic/libc/kernel/common
INC += -I$(ANDROID_TOP)/bionic/libc/kernel/arch-arm
INC += -I$(ANDROID_TOP)/bionic/libm/include
endif
...
CC_FLAG += $(OPTIMIZE_LVL) -Wall -Wno-long-long -std=gnu99 -fsigned-char
ifeq ($(strip $(findstring -DANDROID, $(DEFINES))),)
CC_FLAG += -fshort-wchar
else
CC_FLAG += -fno-short-wchar -fno-short-enums
endif
# -Werror
#CC_FLAG += -ansi
...
ASM_DEFINES += -march=armv6z -mtune=arm1176jz-s -mfloat-abi=softfp -mfpu=vfp
endif

ifeq "$(ENABLE_CA9)" "true"
CC_FLAG += -march=armv7-a -mtune=cortex-a9 -mfloat-abi=softfp -mfpu=vfpv3-d16 -fPIC
ASM_DEFINES += -march=armv7-a -mtune=cortex-a9 -mfloat-abi=softfp -mfpu=vfpv3-d16 -fPIC
endif

export ASM_DEFINES:=$(ASM_DEFINES)


ifeq "$(ENABLE_MUDFLAP_DTV_SVC)" "true"
CC_FLAG += -fmudflapth -Wno-mudflap -g
endif

ifeq "$(CODE_STRIP)" "true"

STRIP = $(lastword $(CROSS_COMPILE)strip)
...
LD_FLAG   = -Wl,--hash-style=gnu
LD_OUTPUT = -o
GCC_LD_FLAG = -Wl,--no-wchar-size-warning,--no-enum-size-warning -Wl,--hash-style=gnu
GCC_LD_FLAG_B = -Wl,--no-wchar-size-warning,--no-enum-size-warning -Wl,--hash-style=gnu
ifeq "$(FFUNCTION_BUILD)" "true"
GCC_LD_FLAG_B = -Wl,--gc-sections --no-wchar-size-warning,--no-enum-size-warning -Wl,--hash-style=gnu
endif
...
# CPU's endianess
#
DEFINES += -D_CPU_LITTLE_ENDIAN_ $(CHIP_DEFINES)
