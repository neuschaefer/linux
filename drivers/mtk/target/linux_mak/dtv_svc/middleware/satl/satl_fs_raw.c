 ...
 *    SATL object to a raw file (or a regular file). It uses fs_raw_*** API's
...
/*-------------------------------------------------------------------------
            SATL file system related functions implementation
 --------------------------------------------------------------------------*/
/*----------------------------------------------------------------------
 * Name: satl_fs_raw_get_magic_for_write_ops
 *
 ...
 *   
 -----------------------------------------------------------------------*/
static
INT32 satl_fs_raw_get_magic_for_write_ops
(
    HANDLE_T  h_file,
...
)
{
        ...
        )
    {
        ...
        */
        if ( ui4_magic != SATL_MAGIC_NUMBER &&
             ui4_magic != (~SATL_MAGIC_NUMBER)
            )
        {
            *pui4_beg_magic = SATL_MAGIC_NUMBER;
            *pb_new_pos     = TRUE;
        }
        else
        {
...
/*----------------------------------------------------------------------
 * Name: satl_fs_raw_close_hdle
 *
 * Description: This API unlock and close the file handle. 
 *
 * Inputs: 
 *   h_satl     Handle to the SATL file.
 *
 * Outputs: None
 *
 * Returns: 
...
)
{
    ...
    /* Translate FM err code to SATL err code. */
    i4_rc=satl_fm_to_satl_err_code(i4_rc);

    return i4_rc;
}

/*----------------------------------------------------------------------
 * Name: satl_fs_raw_close_hdle_no_fm_close
 *
 * Description: This API unlock and close the file handle. 
 *
 * Inputs: 
 ...
 *   
 -----------------------------------------------------------------------*/
INT32 satl_fs_raw_close_hdle_no_fm_close
(
    HANDLE_T  h_file
)
{
    ...
    /* Translate FS_RAW err code to SATL err code. */
    i4_rc=satl_fsraw_to_satl_err_code(i4_rc);

    return i4_rc;
}

/*----------------------------------------------------------------------
 * Name: satl_fs_raw_get_read_hdle
 *
 * Description: This API return a file handle to the RAW file, the read
 *              pointer is position at the SATL object specified by
 *              the ui2_satl_id.
 *
 *              This API will check the begin and end SATL magic number
 *              to detect any corruption.            
 *
 * Inputs: 
 *   ps_satl_path Path to the RAW file.
 *   ui2_satl_id  Specify the SATL id to load
 *
 * Outputs: 
 *   ph_file:     File handle to the SATL object on the RAW file.
 *
 * Returns: 
 *   SATLR_OK          Record is successfully added. 
 *   SATLR_NOT_FOUND   SATL object not found.
 *   SATLR_INV_FILE_PATH specified h_dir and ps_satl_path is not a valid RAW
 *                      file.
 -----------------------------------------------------------------------*/
INT32 satl_fs_raw_get_read_hdle
(
    HANDLE_T           h_dir,
...
)
{
    ...
    *ph_file=NULL_HANDLE;
    ...
    if ( fs_raw_is_inited(h_dir, ps_satl_path) == FALSE )
    {
        return SATLR_INV_FILE_PATH;
    }    
    /*
      Open the RAW file.
    */
    ...
    {
        /*
          Set the file offset to the SATL object found.
        */
        i4_rc=fs_raw_seek_obj(h_file, &t_meta,
                              (UINT32) FS_RAW_MAKE_OBJ_ID(SATL_GROUP_ID, ui2_satl_id),
                              &ui4_offset, &ui4_obj_sz);
        if (  i4_rc == FSRAWR_OK )
        {
            /*
              Check the begin and ending magic number for the SATL object.
            ...
            */
            ...
            {
                if ( (
                         ui4_beg_magic_num == SATL_MAGIC_NUMBER ||
                         ui4_beg_magic_num == (~SATL_MAGIC_NUMBER)
                     )   &&
                   ...
                   )
                {
                    /*
                      ...
                      position of the SATL object is inside
                ...
                {
                    /* Bad magic, this SATL object is corrupted. */
            ...
            {
                /* Translate FSRAW err code to SATL err code. */
                i4_rc = satl_fsraw_to_satl_err_code(i4_rc);
...
/*----------------------------------------------------------------------
 * Name: satl_fs_raw_get_read_hdle_no_fm_open
 *
 * Description: This API return a file handle to the RAW file, the read
 *              pointer is position at the SATL object specified by
 *              the ui2_satl_id.
 ...
 *                      file.
 -----------------------------------------------------------------------*/
INT32 satl_fs_raw_get_read_hdle_no_fm_open
(
    UINT16		       ui2_satl_id,
    HANDLE_T*		   ph_file	
)
    ...
    if ( (i4_rc=fs_raw_read_lock(h_file)) == FSRAWR_OK )
    {
        ...
        */
        ...
        if (  i4_rc == FSRAWR_OK )
        {
            ...
            DBG_API(("{SATL} locating object for read operation> object id: %d size: %d offset: %d\n",
                       ui2_satl_id, ui4_obj_sz, ui4_offset));
            ...
            */
            ...
            if ( i4_rc == FSRAWR_OK )
            {
                ...
                DBG_API(("{SATL} begin_magic: %x end_magic: %x\n",
                           ui4_beg_magic_num, ui4_end_magic_num));
...
/*----------------------------------------------------------------------
 * Name: satl_fs_raw_get_write_hdle
 *
 * Description: This API return a file handle to the RAW file, the read
 *              pointer is position at the SATL object specified by
 *              the ui2_satl_id. It also return a write mask value (e.g.
 ...
 *                      file.
 -----------------------------------------------------------------------*/
INT32 satl_fs_raw_get_write_hdle
(
    HANDLE_T           h_dir,
    const CHAR*	       ps_satl_path,
    SATL_OBJ_T*        pt_satl,
...
)
{
    ...
    ui2_satl_id = pt_satl->ui2_satl_id;
    *ph_file=NULL_HANDLE;

    if ( fs_raw_is_inited(h_dir, ps_satl_path) == FALSE )
    {
        if ( fs_raw_init(h_dir, ps_satl_path) != FSRAWR_OK )
        {
            return SATLR_INV_FILE_PATH;
        }
    }    
    ...
    */
    ...
    {
        /*
          Get the size of the SATL object to write to
          file
        */
        ui4_obj_space_needed=satl_persistent_size(pt_satl);
        

        DBG_API(("{SATL} object spaced required: %d bytes\n",ui4_obj_space_needed));
        ...
        if ( ui4_obj_space_needed > 0 )
        {
            ...
            */
            ...
            if ( i4_rc == FSRAWR_OK )
            {
                ...
                */
                if ( satl_fs_raw_get_magic_for_write_ops(h_file,
                                                         ui4_write_offset,
                                                         pui4_beg_magic,
                                                         pb_new_pos)
                     == SATLR_OK
...
/*----------------------------------------------------------------------
 * Name: satl_fs_raw_get_write_hdle_no_fm_open
 *
 * Description: This API return a file handle to the RAW file, the read
 *              pointer is position at the SATL object specified by
 *              the ui2_satl_id.
 ...
 *                      file.
 -----------------------------------------------------------------------*/
INT32 satl_fs_raw_get_write_hdle_no_fm_open
(
    SATL_OBJ_T*         pt_satl,
    HANDLE_T*          ph_file,
    UINT32*            pui4_beg_magic,
...
/*----------------------------------------------------------------------
 * Name: satl_fs_raw_del
 *
 * Description: This API delete an SATL object from the RAW file.
 *             
 *
 * Inputs: 
 *   ps_satl_path Path to the RAW file.
 *   ui2_satl_id  Specify the SATL id to delete.
 *
 * Outputs: None
 *
 * Returns: 
...
/*----------------------------------------------------------------------
 * Name: satl_fs_raw_qry
 *
 * Description: This query the SATL ID and NAME of a SATL object store
 *              on the raw file.   
 *
 * Inputs: 
        ...
        )
    {
        ...
        { 
            ...
            */
            ...
            if ( i4_rc == FSRAWR_OK )
            {
                if ( FS_RAW_GET_MODULE_ID(ui4_obj_id) == SATL_GROUP_ID )
                {
                    if ( ui2_nb_matched == ui2_idx )
                    {
                        /* Read meta data of the SATL object. */
                        if ( satl_read_file_info(h_file, &t_finfo, NULL) == SATLR_OK )
                        {
                            /* Assertion check, object id and satl id must be the same. */
                            if (  FS_RAW_GET_ID(ui4_obj_id) == t_finfo.ui2_satl_id )
                            {
                                *pui2_satl_id=t_finfo.ui2_satl_id;
                                x_strncpy(ps_satl_name, t_finfo.ps_satl_name,MAX_SATL_NAME_LEN);
                                i4_rc = SATLR_OK;
                                break;
                            }
                            else
