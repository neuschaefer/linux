...
/*-----------------------------------------------------------------------------
 * Name: satl_reset_rec_id
 *
 * Description: This function reset the SATL record ID library. 
 *
 * Inputs: 
 *    pt_satl:  SATL structure containing the smallest and largest record id
 ...
 *
 ----------------------------------------------------------------------------*/
BOOL satl_reset_rec_id(SATL_OBJ_T*  	pt_satl)
{
    /*
    ...
    */
    ...
    /*
      Reset the SATL id values.
    */
    aui2_min_max[0]=SATL_SMALLEST_INIT_VAL;
    aui2_min_max[1]=SATL_NULL_REC_ID;

    /*
      Save the min and max record id value to the
      database object.
    */
    i4_rc=db_write_common(pt_satl->h_cdb,aui2_min_max,
...
/*-----------------------------------------------------------------------------
 * Name: satl_get_next_rec_id
 *
 * Description: This function get the next available SATL record ID.
 *
 * Inputs:  
 *    pt_satl:  SATL structure containing the smallest and largest record id
 *              currently in used.
 ...
 *   SATLR_OUT_OF_RES Can not add the new SATL ID/Name.
 ----------------------------------------------------------------------------*/
BOOL satl_get_next_rec_id(const SATL_OBJ_T* pt_satl, UINT16* pui2_next_id)
{
    UINT16  aui2_min_max[2];
    UINT16  ui2_i;
    ...
    */
    db_read_common(pt_satl->h_cdb, aui2_min_max,
                   (UINT16) sizeof(aui2_min_max));
 ...
 *   FALSE     A Null record id is returned.
 ----------------------------------------------------------------------------*/
BOOL satl_get_smallest_rec_id(const SATL_OBJ_T* pt_satl, UINT16* pui2_next_id)
{
    UINT16  aui2_min_max[2];
    ...
    */
    ...
    if ( aui2_min_max[0] != SATL_SMALLEST_INIT_VAL )
    {
 ...
 *   FALSE     Invalid SATL ID/Name.
 ----------------------------------------------------------------------------*/
BOOL satl_set_smallest_rec_id(SATL_OBJ_T* pt_satl, UINT16  ui2_next_id)
{
    UINT16  aui2_min_max[2];
    ...
    */
    ...
    db_write_common(pt_satl->h_cdb, aui2_min_max,
                    (UINT16) sizeof(aui2_min_max));
    return TRUE;
}

/*-----------------------------------------------------------------------------
 * Name: satl_get_largest_rec_id
 *
 ...
 *
 ----------------------------------------------------------------------------*/
BOOL satl_get_largest_rec_id(const SATL_OBJ_T* pt_satl, UINT16* pui2_next_id)
{
    UINT16  aui2_min_max[2];
    ...
    */
    ...
    if ( aui2_min_max[1] != SATL_NULL_REC_ID )
    {
 ...
 *
 ----------------------------------------------------------------------------*/
BOOL satl_set_largest_rec_id(SATL_OBJ_T* pt_satl, UINT16  ui2_next_id)
{
    UINT16  aui2_min_max[2];
