 ...
 *    LOL object to a raw file (or a regular file). It uses fs_raw_*** API's
...
/*-------------------------------------------------------------------------
            LOL file system related functions implementation
 --------------------------------------------------------------------------*/

/*----------------------------------------------------------------------
 * Name: lol_fs_raw_get_magic_for_write_ops
 *
 ...
 *
 * Inputs: 
 *   h_file     Handle to the LOL file.
 *              After return from this API, the file handle is positioned
 *              at the file location where the LOL object will be
 *              written.
 *              
 ...
 *                existing LOL object stored on filesystem.
 *
 *                TRUE: the location points to by the file handle is not
 *                      a LOL object.
 *                FALSE: the location points to by the file handle contains
 *                      an existing LOL object.
 *
 * Returns: 
 *   LOLR_OK      magic number is returned.
 *   
 -----------------------------------------------------------------------*/
static
INT32 lol_fs_raw_get_magic_for_write_ops
(
    HANDLE_T  h_file,
...
)
{
        ...
        )
    {
        ...
        */
        if ( ui4_magic != LOL_MAGIC_NUMBER  &&
             ui4_magic != (~LOL_MAGIC_NUMBER)
            )
        {
            *pui4_beg_magic = LOL_MAGIC_NUMBER;
            *pb_new_pos     = TRUE;
        }
        else
        {
...
/*----------------------------------------------------------------------
 * Name: lol_fs_raw_close_hdle
 *
 * Description: This API unlock and close the file handle. 
 *
 * Inputs: 
 *   h_lol      Handle to the LOL file.
 *
 * Outputs: None
 *
 * Returns: 
 *   LOLR_OK           file handle is closed.
 *   
 -----------------------------------------------------------------------*/
INT32 lol_fs_raw_close_hdle
(
    HANDLE_T  h_file
)
{
    ...
    /* Translate FM err code to LOL err code. */
    i4_rc=lol_fm_to_lol_err_code(i4_rc);

    return i4_rc;
}

/*----------------------------------------------------------------------
 * Name: lol_fs_raw_close_hdle_no_fm_close
 *
 * Description: This API unlock and close the file handle. 
 *
 * Inputs: 
 ...
 *
 * Returns: 
 *   LOLR_OK           file handle is successfully closed. 
 *   
 -----------------------------------------------------------------------*/
INT32 lol_fs_raw_close_hdle_no_fm_close
(
    HANDLE_T  h_file
)
{
    ...
    /* Translate FS_RAW err code to LOL err code. */
    i4_rc=lol_fsraw_to_lol_err_code(i4_rc);

    return i4_rc;
}

/*----------------------------------------------------------------------
 * Name: lol_fs_raw_get_read_hdle
 *
 * Description: This API return a file handle to the RAW file, the read
 *              pointer is position at the LOL object specified by
 *              the ui2_lol_id.
 *
 *              This API will check the begin and end LOL magic number
 *              to detect any corruption.
 *
 * Inputs: 
 *   ps_lol_path  Path to the RAW file.
 *   ui2_lol_id   Specify the LOL id to load
 *
 * Outputs: 
 *   ph_file:     File handle to the LOL object on the RAW file.
 *
 * Returns: 
 *   LOLR_OK           File handle is returned.
 *   LOLR_NOT_FOUND    LOL object not found.
 *   LOLR_INV_FILE_PATH specified h_dir and ps_lol_path is not a valid RAW
 *                      file.
 -----------------------------------------------------------------------*/
INT32 lol_fs_raw_get_read_hdle
(
    HANDLE_T           h_dir,
...
)
{
    ...
    *ph_file=NULL_HANDLE;
    ...
    if ( fs_raw_is_inited(h_dir, ps_lol_path) == FALSE )
    {
        return LOLR_INV_FILE_PATH;
    }
    /*
      Open the RAW file.
    */
    ...
    {
        /*
          Set the file offset to the LOL object found.
        */
        i4_rc=fs_raw_seek_obj(h_file, &t_meta,
                              (UINT32)(FS_RAW_MAKE_OBJ_ID(LOL_GROUP_ID,ui2_lol_id)),
                              &ui4_offset, &ui4_obj_sz);
        if (  i4_rc == FSRAWR_OK )
        {

            DBG_API(("{LOL} locating object for read operation> object id: %d size: %d offset: %d\n",
                       ui2_lol_id, ui4_obj_sz, ui4_offset));

            /*
              Check the begin and ending magic number for the LOL object.
            ...
            */
            ...
            if ( i4_rc == FSRAWR_OK )
            {
                ...
                DBG_API(("{LOL} begin_magic: %x end_magic: %x\n",
                           ui4_beg_magic_num, ui4_end_magic_num));

                if ( (
                         ui4_beg_magic_num == LOL_MAGIC_NUMBER  ||
                         ui4_beg_magic_num == (~LOL_MAGIC_NUMBER)
                     )   &&
                     ( ui4_beg_magic_num == (~ui4_end_magic_num) )
                   )
                {
                    /*
                      ...
                      position of the LOL object is inside
                ...
                {
                    /* Bad magic, this LOL object is corrupted. */
            ...
            {
                /* Translate FSRAW err code to LOL err code. */
                i4_rc = lol_fsraw_to_lol_err_code(i4_rc);
...
/*----------------------------------------------------------------------
 * Name: lol_fs_raw_get_read_hdle_no_fm_open
 *
 * Description: This API return a file handle to the RAW file, the read
 *              pointer is position at the LOL object specified by
 *              the ui2_lol_id.
 ...
 *                      file.
 -----------------------------------------------------------------------*/
INT32 lol_fs_raw_get_read_hdle_no_fm_open
(
    UINT16		       ui2_lol_id,
    HANDLE_T*		   ph_file	
)
...
/*----------------------------------------------------------------------
 * Name: lol_fs_raw_get_write_hdle
 *
 * Description: This API return a file handle to the RAW file, the read
 *              pointer is position at the LOL object specified by
 *              the ui2_lol_id.
 ...
 *
 * Returns: 
 *   LOLR_OK           File write handle is returned 
 *   LOLR_NOT_FOUND    LOL object not found.
 *   LOLR_INV_FILE_PATH specified h_dir and ps_lol_path is not a valid RAW
 *                      file.
 -----------------------------------------------------------------------*/
INT32 lol_fs_raw_get_write_hdle
(
    HANDLE_T           h_dir,
    const CHAR*	       ps_lol_path,
    LOL_OBJ_T*         pt_lol,
...
)
{
    ...
    ui2_lol_id = pt_lol->ui2_lol_id;
    *ph_file=NULL_HANDLE;

    if ( fs_raw_is_inited(h_dir, ps_lol_path) == FALSE )
    {
        if ( fs_raw_init(h_dir, ps_lol_path) != FSRAWR_OK )
        {
            return LOLR_INV_FILE_PATH;
        }
    }
    ...
    */
    ...
    {
        /*
          Get the size of the LOL object to write to
          file
        */
        ui4_obj_space_needed=lol_persistent_size(pt_lol);
        

        DBG_API(("{LOL} object spaced required: %d bytes\n",ui4_obj_space_needed));
        ...
        {   
            ...
            */
            ...
            if ( i4_rc == FSRAWR_OK )
            {
                ...
                */
                if ( lol_fs_raw_get_magic_for_write_ops(h_file,
                                                        ui4_write_offset,
                                                        pui4_beg_magic,
                                                        pb_new_pos)
                     == LOLR_OK
...
/*----------------------------------------------------------------------
 * Name: lol_fs_raw_get_write_hdle_no_fm_open
 *
 * Description: This API return a file handle to the RAW file, the read
 *              pointer is position at the LOL object specified by
 *              the ui2_lol_id.
 ...
 *                      file.
 -----------------------------------------------------------------------*/
INT32 lol_fs_raw_get_write_hdle_no_fm_open
(
    LOL_OBJ_T*         pt_lol,
    HANDLE_T*          ph_file,
    UINT32*            pui4_beg_magic,
 ...
 * Name: lol_fs_raw_del
 *
 * Description: This API delete an LOL object from the RAW file.
 *             
 *
 * Inputs: 
 *   ps_lol_path  Path to the RAW file.
 *   ui2_lol_id   Specify the LOL id to delete.
 *
 * Outputs: None
 *
 * Returns: 
 *   LOLR_OK           LOL object on file system is deleted 
 *   LOLR_NOT_FOUND    LOL object specified is not found on file
 *                     system location
 *   LOLR_INV_FILE_PATH specified h_dir and ps_lol_path is not a valid RAW
 *                      file.
 -----------------------------------------------------------------------*/
INT32 lol_fs_raw_del
(
    HANDLE_T        h_dir,
    const CHAR*	    ps_lol_path,
    UINT16		    ui2_lol_id
 ...
 * Name: lol_fs_raw_qry
 *
 * Description: This query the LOL ID and NAME of a LOL object store
 *              on the raw file.   
 *
 * Inputs: 
 ...
 *
 * Returns: 
 *   LOLR_OK           Query is success. 
 *   LOLR_INV_HANDLE   Bad LOL handle.
 *   LOLR_OUT_OF_MEM   Can not allocate memory, add record failed.
 *   LOLR_INV_ARG      'pui2_lol_id' or 'ps_lol_name' is a NULL.
 *   LOLR_INV_FILE_PATH specified h_dir and ps_lol_path is not a valid RAW
 *                      file.
 -----------------------------------------------------------------------*/
INT32 lol_fs_raw_qry (
    HANDLE_T        h_dir,
    const CHAR*	    ps_lol_path,
    UINT16		    ui2_idx,
    UINT16*	   	    pui2_lol_id,
        ...
        )
    {
        ...
        { 
            ...
            */
            ...
            if ( i4_rc == FSRAWR_OK )
            {
                if ( FS_RAW_GET_MODULE_ID(ui4_obj_id) == LOL_GROUP_ID )
                {
                    if ( ui2_nb_matched == ui2_idx )
                    {
                        /* Read meta data of the LOL object. */
                        if ( lol_read_file_info(h_file, &t_finfo, NULL) == LOLR_OK )
                        {
                            /* Assertion check, object id and lol id must be the same. */
                            if (  FS_RAW_GET_ID(ui4_obj_id) == t_finfo.ui2_lol_id )
                            {
                                *pui2_lol_id=t_finfo.ui2_lol_id;
                                x_strncpy(ps_lol_name, t_finfo.ps_lol_name,MAX_LOL_NAME_LEN-1);
                                i4_rc = LOLR_OK;
                                break;
                            }
                            else
