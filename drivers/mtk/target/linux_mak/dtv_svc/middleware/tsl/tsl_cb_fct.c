 ...
 *---------------------------------------------------------------------------*/
/*-----------------------------------------------------------------------------
 * $RCSfile: tsl_cb_fct.c,v $
 * $Revision: #1 $
 * $Date: 2012/04/27 $
 * $Author: dtvbm11 $
 *
 * Description: 
 *    This file implements TSL callback functions
 *    
 *   
 *         
 *---------------------------------------------------------------------------*/
...
/**********************************************************************
   Following are callback functions for the TSL library.
***********************************************************************/

/*-----------------------------------------------------------------------------
 * Name: tsl_handle_free_fct
 *
 * Description: This function is called whenever a TSL database client handle
 *              is freed.
 *
 * Inputs:  h_obj         Contains the handle to core db client.
 *          e_type        Contains the core db client handle type and must be 
 *                        set to TSLT_TRANSPORT_STREAM_LIST.
...
)
{
    ...
    /* If no client database object  has been pass into this
       simply return FALSE . */
    if ( ( pt_tsl = ( (TSL_OBJ_T*) pv_obj ) ) != NULL ) 
    {
        /* Abort if this is not the correct type. */
        if (e_type == TSLT_TRANSPORT_STREAM_LIST )
        {
            /*
              Free the CDB handle and its control structure
            */
            if ( pt_tsl->h_cdb != NULL_HANDLE )
            {
                handle_free(pt_tsl->h_cdb, TRUE);
            }
 ...
 *              It acts as an intermediate to invoke TSL client's
 *              notification function.
 *
 *              When this function is called by the CoreDB database object,
 *              it needs to get the address of the TSL client object. 
 *              The TSL client object structure contains the address of
 *              the notification function for the TSL client.  Using
 *              that address, this function then call the SCDE client
 *              notification fucntion.
 *
 * Inputs:  h_client:  CDB handle
 *          e_cond:    condition TSL object of call back.
...
)
{
    ...
    TSL_OBJ_T*        pt_tsl;
    TSL_COND_T        e_tsl_cond;
    HANDLE_T          h_tsl_client;
    UINT32            ui4_tsl_reason;
    x_tsl_nfy_fct     pf_tsl_nfy_fct;
    ...
    */
    ...
    {
        /* cast pv_tag to the address of TSL client object. */
        pt_tsl=(TSL_OBJ_T*) pv_tag;

        /* Check if the client handle passed into this function
           is really CDB client for this TSL control block.
           ...
           valid handle.
        */
        if ( pt_tsl->h_cdb == h_client && pt_tsl->pf_nfy_fct != NULL )
        {
            /* Get the handle to the tsl object. */
            h_tsl_client = pt_tsl->h_tsl;

            /* Get the private tag for the TSL handle. */
            if ( (h_tsl_client != NULL_HANDLE)                           &&
                 (x_handle_get_type( h_tsl_client, &pe_type) == HR_OK )  &&
                 (pe_type == TSLT_TRANSPORT_STREAM_LIST )                
                )
            {
                /*
                  Get the TSL client tag.
                */
                if (x_handle_get_tag(h_tsl_client,((VOID**) &pv_tag))
                    != HR_OK)
                {
                    pv_tag=NULL;
                }
                ...
                {
                    e_tsl_cond = TSL_UPDATING;
                }
                else if ( e_cond == DB_UPDATED )
                {
                    e_tsl_cond = TSL_UPDATED;
                }
                else if ( e_cond == DB_CLOSED )
                {
                    e_tsl_cond = TSL_CLOSED;
                }
                else
                {
                    e_tsl_cond = TSL_UNKNOWN;
                }
                ...
                {
                    ui4_tsl_reason = ui4_tsl_reason | TSL_REASON_UNKNOWN;
                }
                if ( ui4_reason & DB_RECORD_ADD )
                {
                    ui4_tsl_reason = ui4_tsl_reason | TSL_RECORD_ADD;
                }
                if ( ui4_reason & DB_RECORD_DEL )
                {
                    ui4_tsl_reason = ui4_tsl_reason | TSL_RECORD_DEL;
                }
                if ( ui4_reason & DB_RECORD_MOD )
                {
                    ui4_tsl_reason = ui4_tsl_reason | TSL_RECORD_MOD;
                }

                /* Save the notification function pointer */
                pf_tsl_nfy_fct=pt_tsl->pf_nfy_fct;
                        
                /* If the reason for the notification is that TSL
                   is shutting down, then we need to invalidate all
                   other handles to this TSL
                */
                if ( e_tsl_cond == TSL_CLOSED )
                {
                    /* Free the TSL handle. */
                    handle_free(h_tsl_client, TRUE);
                }
                
                /* Notify the TSL client that the TSL's records
                   have changed or it has been deleted. */
                pf_tsl_nfy_fct(h_tsl_client,
                               e_tsl_cond,
                               ui4_tsl_reason,
                               pv_tag,
                               ui4_db_version);
...
#ifdef DEBUG
                if ( (h_tsl_client != NULL_HANDLE) &&
                     x_handle_valid(h_tsl_client)
                    )
                {
                    ABORT (DBG_CAT_HANDLE, DBG_ABRT_INVALID_HANDLE_TYPE);
                }
#endif
                /* Invalid TSL handle or NULL notification function,
