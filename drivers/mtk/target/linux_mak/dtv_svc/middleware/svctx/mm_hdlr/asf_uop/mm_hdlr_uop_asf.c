 ...
 *
 * Description: 
 *         This file contains the implementation of asf uop handler 
...
#include "svctx/mm_hdlr/asf_uop/mm_hdlr_uop_asf.h"
/*-----------------------------------------------------------------------------
...
/*-----------------------------------------------------------------------------
 ...
 ----------------------------------------------------------------------------*/
static INT32 _asf_uop_strema_id_2_ele_tbl_desc(
...
/*-----------------------------------------------------------------------------
 * Name:  asf_uop_hdlr_open
 *
 * Description:This function prepare data for later use when open asf uop handler
 *
 ...
 *          NON-Zero     If fail
 ----------------------------------------------------------------------------*/
INT32 asf_uop_hdlr_open (MEDIA_DESCRIBT_T* pt_obj)
{
    MM_HDLR_UOP_ASF_OBJ*  pt_asf_uop_hdlr_obj;
    INT32                 i4_ret;
    MINFO_INFO_T          t_asf_minfo_obj; 
    UINT16                ui2_rec_nums;
    UINT32                ui4_rec_sz;
    ...
    pt_asf_uop_hdlr_obj = (MM_HDLR_UOP_ASF_OBJ*)
                          _mm_util_mem_alloc(sizeof(MM_HDLR_UOP_ASF_OBJ));
    if(NULL == pt_asf_uop_hdlr_obj)
    {
        return SVCTXR_NO_RESOURCE;
    }
    pt_obj->pv_uop_hdr_private_obj = (VOID*)pt_asf_uop_hdlr_obj;

    ui4_rec_sz = sizeof(MINFO_INFO_T);
    x_memset(&t_asf_minfo_obj, 0, ui4_rec_sz);
    ...
    i4_ret = x_minfo_get_info (pt_obj->h_media_info,    
                               MINFO_INFO_TYPE_GENERIC,
                               0,          
                               (VOID*)&t_asf_minfo_obj,       
                               (SIZE_T*)&ui4_rec_sz);
    if (MINFOR_OK != i4_ret)
    {
        _mm_util_mem_free(pt_asf_uop_hdlr_obj);
        pt_obj->pv_uop_hdr_private_obj = NULL;
        return SVCTXR_FAIL;
    }
    pt_asf_uop_hdlr_obj->t_minfo_generic = t_asf_minfo_obj.u.t_general_info;                    
    return SVCTXR_OK;
}

/*-----------------------------------------------------------------------------
 * Name:  asf_uop_hdlr_is_hdlr
 *
 ...
 *          SVCTXR_FAIL , If do not support the format
 ----------------------------------------------------------------------------*/
INT32 asf_uop_hdlr_is_hdlr(MEDIA_DESCRIBT_T* pt_obj,
...
/*-----------------------------------------------------------------------------
 * Name:  asf_uop_hdlr_close
 *
 * Description:This function free used data  when close asf uop handler
 *
 * Inputs:  pv_obj       specify the media description object  
 *
 * Outputs: NULL 
 ...
 *                       operation has been completed.
 ----------------------------------------------------------------------------*/
INT32 asf_uop_hdlr_close (MEDIA_DESCRIBT_T* pt_obj)
{
    MM_HDLR_UOP_ASF_OBJ* pt_asf_uop_hdlr_obj;
    ...
    pt_asf_uop_hdlr_obj = (MM_HDLR_UOP_ASF_OBJ*)pt_obj->pv_uop_hdr_private_obj;
    if(NULL != pt_asf_uop_hdlr_obj)
    {
        _mm_util_mem_free(pt_asf_uop_hdlr_obj);
        pt_obj->pv_uop_hdr_private_obj = NULL;
 ...
 *          Non-Zero ,    If fail
 ----------------------------------------------------------------------------*/
INT32 asf_uop_hdlr_get_tm_code (
   ...
   /*check paprameter*/
    ...
    switch(e_tm_code_type)
    {
        ...
        case MM_SVC_TM_CODE_TYPE_ALL:
            ...
            /*[DTV00125878]dirver don't calculate preroll time,so here set zero*/
            pt_time_info->ui8_time = 0;
                         /*pt_asf_uop_hdlr_obj->t_minfo_generic.ui8_preroll;*/
        break;
        default:
        break;
    }
 ...
 *          Non-Zero ,    If fail
 ----------------------------------------------------------------------------*/
INT32 asf_uop_hdlr_get_dur (MEDIA_DESCRIBT_T*  pt_obj , 
    ...
    switch(e_dur_type)
    {
        ...
        case MM_SVC_DUR_TYPE_ALL:
            ...
            pt_time_dur->ui4_duration = (UINT32)
                         pt_asf_uop_hdlr_obj->t_minfo_generic.ui8_pb_duration;*/
        break;
...
/*-----------------------------------------------------------------------------
 * Name:  asf_uop_handler_search_fct
 *
 ...
 *          Non-Zero ,    If fail
 ----------------------------------------------------------------------------*/
INT32 asf_uop_handler_search_fct (MEDIA_DESCRIBT_T*  pt_obj , 
 ...
 *          Non-Zero ,    If fail
 ----------------------------------------------------------------------------*/
INT32 asf_uop_handler_trick_fct (
                                 MEDIA_DESCRIBT_T*       pt_obj, 
...
INT32 asf_uop_handler_btn_fct ( MEDIA_DESCRIBT_T* pt_obj , 
...
/*-----------------------------------------------------------------------------
 * Name:  asf_uop_handler_get_range_fct
 *
 ...
 *          Non-Zero ,    If fail
 ----------------------------------------------------------------------------*/
INT32 asf_uop_handler_get_range_fct(
                                  MEDIA_DESCRIBT_T*               pt_obj ,
                                  ...
                                  MIDXBULD_ELMT_TBL_ALLOC_INFO_T* pt_range_info)
{
    ...
    {   
        /*if not 3d only playback first video*/
    ...
    i4_ret = _asf_uop_strema_id_2_ele_tbl_desc(
                    t_vid_stm_info.u.t_stm_attribute.t_stm_id.u.t_asf_stm_id,
                    pt_range_info,
                    &pt_tbl_desc);   
    if (SVCTXR_OK == i4_ret)
    {
    ...
    for(ui2_i = 1 ; ui2_i <= ui2_rec_nums ; ui2_i++)
    {
        ...
        i4_ret = _asf_uop_strema_id_2_ele_tbl_desc(
                        t_aud_stm_info.u.t_stm_attribute.t_stm_id.u.t_asf_stm_id,
                        pt_range_info,
                        &pt_tbl_desc);
        if (SVCTXR_OK == i4_ret)
        {
            ...
            (UINT32)t_aud_stm_info.u.t_stm_attribute.u.t_asf_audio_stm_attr.i4_samples_per_sec;  
