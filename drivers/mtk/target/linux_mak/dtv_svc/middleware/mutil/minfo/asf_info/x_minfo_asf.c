 ...
 *
 * Description: 
 *         This file contains the implementation of the asf media info function
...
/*-----------------------------------------------------------------------------
 * Name:  x_minfo_asf_init
 *
 * Description: asf media info initial function
 *
 * Inputs:  NULL
 *          
 * Outputs: NULL
 ...
 *   (1)2008-3-12 : initial
 ----------------------------------------------------------------------------*/                            
INT32 x_minfo_asf_init ( VOID )
{
    INT32                       i4_ret = MINFOR_OK; /*return code*/
    ...
    if (i4_ret != MINFOR_OK)
    {
        DBG_ERROR(("x_minfo_asf_init:minfo_init failed(%d)\r\n",
                   i4_ret));
        return MINFOR_INITIAL_ERR;
    }
    t_fct_tab.pf_is_hdlr = asf_minfo_is_handler;
    t_fct_tab.pf_fopen = asf_minfo_fopen;
    t_fct_tab.pf_mopen = asf_minfo_mopen;
    t_fct_tab.pf_close = asf_minfo_close;
    t_fct_tab.pf_get_info = asf_minfo_get_info;
    t_fct_tab.pf_parse = asf_minfo_parse;
    t_fct_tab.pf_get_rec_nums = asf_minfo_get_num_recs;
    t_fct_tab.pf_minfo_cli_init = asf_minfo_cli_init;
    
    t_format.e_media_type = MEDIA_TYPE_CONTAINER;
    t_format.t_media_subtype.u.e_contnr_subtype = MEDIA_CONTNR_SUBTYPE_ASF;

    /* register asf media info */
    i4_ret = minfo_register_handler(ASF_MEDIA_INFO_NAME, 
                                    &t_format, 
                                    &t_fct_tab);
    if (i4_ret != MINFOR_OK)
    {
        DBG_ERROR(("x_minfo_asf_init: minfo_register_handler failed(%d)\r\n", 
                   i4_ret));
        return MINFOR_INITIAL_ERR;
    }  
      
    /* register wma media info */
    t_format.e_media_type = MEDIA_TYPE_AUDIO_ES;
    t_format.t_media_subtype.u.e_aud_subtype = MEDIA_AUD_SUBTYPE_WMA;
    i4_ret = minfo_register_handler(WMA_MEDIA_INFO_NAME,
                                    &t_format,
                                    &t_fct_tab);
    if (i4_ret != MINFOR_OK)
    {
