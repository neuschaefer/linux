 ...
 *
 * Description: 
 *         This file contains the implementation of theavi media index builder function
 *---------------------------------------------------------------------------*/
...
#include "mutil/midxbuld/avi_idxbuld/x_midxbuld_avi.h"
...
/*-----------------------------------------------------------------------------
 ...
 ----------------------------------------------------------------------------*/
/*-----------------------------------------------------------------------------
 * Name:  x_midxbuld_avi_init
 *
 * Description: avi media index builder initial function
 *
 * Inputs:  -
 *          
 * Outputs: -
 ...
 *          MIDXBULDR_INITIAL_ERR        The library initialize failed.
 ----------------------------------------------------------------------------*/                            
INT32 x_midxbuld_avi_init (void)
{
    INT32                       i4_ret;
    ...
    t_fct_tab.pf_fopen = avi_midxbuld_fopen;
    t_fct_tab.pf_mopen = avi_midxbuld_mopen;
    t_fct_tab.pf_close = avi_midxbuld_close;
    t_fct_tab.pf_start_indexing = avi_midxbuld_start_indexing;
    t_fct_tab.pf_get_info= avi_midxbuld_get_info;
    t_fct_tab.pf_set_info= avi_midxbuld_set_info;
    t_fct_tab.pf_midxbuld_cli_init = avi_midxbuld_cli_init;

    t_format.e_media_type = MEDIA_TYPE_CONTAINER;
    t_format.t_media_subtype.u.e_contnr_subtype = MEDIA_CONTNR_SUBTYPE_AVI;

    /* register avi media index builder */
    i4_ret = midxbuld_register_handler(AVI_MEDIA_IDX_BUILDER_NAME, &t_format, &t_fct_tab);
    if (i4_ret != MIDXBULDR_OK)
    {
        return MIDXBULDR_INITIAL_ERR;
    }  
