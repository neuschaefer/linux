 ...
 *
 * Description: 
 *         This file contains the implementation of the rm media index builder 
 *         function
 *---------------------------------------------------------------------------*/
...
#include "mutil/midxbuld/rm_idxbuld/x_midxbuld_rm.h"
...
/*-----------------------------------------------------------------------------
 * Name:  x_midxbuld_rm_init
 *
 * Description: rm media index builder initial function
 *
 * Inputs:  NULL 
 *          
 * Outputs: NULL 
...
INT32 x_midxbuld_rm_init (VOID)
{
    ...
    t_fct_tab.pf_fopen = rm_midxbuld_fopen;
    t_fct_tab.pf_mopen = rm_midxbuld_mopen;
    t_fct_tab.pf_close = rm_midxbuld_close;
    t_fct_tab.pf_start_indexing = rm_midxbuld_start_indexing;
    t_fct_tab.pf_get_info = rm_midxbuld_get_info;
    t_fct_tab.pf_set_info = rm_midxbuld_set_info;
    t_fct_tab.pf_midxbuld_cli_init = rm_midxbuld_cli_init;

    t_format.e_media_type = MEDIA_TYPE_CONTAINER;
    t_format.t_media_subtype.u.e_contnr_subtype = MEDIA_CONTNR_SUBTYPE_RM;

    /* register rm media index builder */
    i4_ret = midxbuld_register_handler(RM_MEDIA_IDX_BUILDER_NAME, 
                                       &t_format, 
                                       &t_fct_tab);
    if (i4_ret != MIDXBULDR_OK)
    {
