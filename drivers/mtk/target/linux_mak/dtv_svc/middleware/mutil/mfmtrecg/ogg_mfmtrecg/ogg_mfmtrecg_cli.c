...
#include "mutil/mfmtrecg/ogg_mfmtrecg/ogg_mfmtrecg.h"
...
#ifdef CLI_LVL_ALL
static INT32 mfmtrecg_cli_ogg_fconfirm_fct(INT32 i4_argc, const CHAR** pps_argv);     
static INT32 mfmtrecg_cli_ogg_mconfirm_fct(INT32 i4_argc, const CHAR** pps_argv);     


/* mfmtrecg command table */

static CLI_EXEC_T at_ogg_mfmtrecg_cmd_tbl[] =
{
    {"ogg fconfirm" ,   "ogg",                     mfmtrecg_cli_ogg_fconfirm_fct,  
     NULL,              "OGG format file check",    CLI_SUPERVISOR},
    {"ogg mconfirm" ,   "ogg",                     mfmtrecg_cli_ogg_mconfirm_fct,  
     NULL,              "OGG format mem check",     CLI_SUPERVISOR},
    END_OF_CLI_CMD_TBL
};
#endif
...
/*-----------------------------------------------------------------------------
 * Name: mfmtrecg_cli_ogg_fconfirm_fct
 *
 * Description: This API returns the media format for file handle.
 *
 * Inputs: -command format : ogg  /mnt/usb/Mass-000/file_path
 *
 * Outputs: -
 *
 * Returns:
...
static INT32 mfmtrecg_cli_ogg_fconfirm_fct(INT32 i4_argc, const CHAR** pps_argv)
{
    ...
    if ((i4_argc != 2) || (pps_argv == NULL))
    {
        ...
        x_dbg_stmt("Usage: ogg FilePathName \r\n");
        return CLIR_INV_ARG;
    }
    ...
    do
    {
        ...
        i4_ret = ogg_handler_file_confirm_fct(h_file, &t_format);
        ...
        x_dbg_stmt("Media Format is OGG!\r\n");
    }while(0);
    x_fm_close(h_file);   
    return i4_ret;
...
{  
    ...
    {
        x_dbg_stmt("Invalid argument! \r\n");
        x_dbg_stmt("Usage: oggm FilePathName mem_length(0/1) \r\n");
        return CLIR_INV_ARG;
    }
    ps_file_path = (CHAR*)pps_argv[1];
    ...
    if (0 == x_strncmp(pps_argv[2], "0", (SIZE_T)1))
    {
        ...
        i4_ret = ogg_handler_mem_confirm_fct((VOID*)&t_feeder, 
                                             (SIZE_T)0, 
                                             &t_format);
    }
    else
    {
        ...
        ui8_size = 100;
        pui1_buf = (UINT8*)_mm_util_mem_alloc(sizeof(UINT8) * ui8_size);
        if(pui1_buf == NULL)
        {
        ...
        i4_ret = x_fm_read(h_file, (VOID*)pui1_buf, ui8_size, &ui4_read);
        if (i4_ret != FMR_OK)
        {
            x_fm_close(h_file);   
            _mm_util_mem_free(pui1_buf);
        ...
        i4_ret = ogg_handler_mem_confirm_fct((VOID*)pui1_buf, 
                                             (SIZE_T)ui8_size, 
                                             &t_format);
        _mm_util_mem_free(pui1_buf);
    }
...
INT32 ogg_mfmtrecg_cli_init(CLI_EXEC_T** ppt_command_array)
{
#ifdef CLI_LVL_ALL

    *ppt_command_array = at_ogg_mfmtrecg_cmd_tbl;
