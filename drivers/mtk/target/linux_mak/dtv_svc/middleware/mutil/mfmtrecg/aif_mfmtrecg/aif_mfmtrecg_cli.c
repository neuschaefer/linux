...
#include "mutil/mfmtrecg/aif_mfmtrecg/aif_mfmtrecg.h"
...
#ifdef CLI_LVL_ALL
static INT32 mfmtrecg_cli_aif_fconfirm_fct(INT32 i4_argc, const CHAR** pps_argv);     
static INT32 mfmtrecg_cli_aif_mconfirm_fct(INT32 i4_argc, const CHAR** pps_argv);     

/* mfmtrecg command table */
static CLI_EXEC_T at_aif_mfmtrecg_cmd_tbl[] =
{
    {"aif fconfirm" ,   "aiff",                     mfmtrecg_cli_aif_fconfirm_fct,  
     NULL,              "AIF format file check",    CLI_SUPERVISOR},
    {"aif mconfirm" ,   "aifm",                     mfmtrecg_cli_aif_mconfirm_fct,  
     NULL,              "AIF format mem check",     CLI_SUPERVISOR},
    END_OF_CLI_CMD_TBL
};
...
/*-----------------------------------------------------------------------------
 * Name: mfmtrecg_cli_aif_fconfirm_fct
 *
 * Description: This API returns the media format for file handle.
 *
 * Inputs: -command format : aiff  /mnt/usb/Mass-000/file_path
 *
 * Outputs: -
 *
 * Returns:
...
static INT32 mfmtrecg_cli_aif_fconfirm_fct(INT32 i4_argc, const CHAR** pps_argv)
{
    ...
    if ((i4_argc != 2) || (pps_argv == NULL))
    {
        ...
        DBG_INFO(("Usage: aiff FilePathName \r\n"));
        return CLIR_INV_ARG;
    }
    ...
    do
    {
        ...
        i4_ret = aif_handler_file_confirm_fct(h_file, &t_format);
        ...
        x_dbg_stmt("Media Format is AIF!\r\n");
    }while(0);
    x_fm_close(h_file);   
    return i4_ret;
...
{  
    ...
    {
        x_dbg_stmt("Invalid argument! \r\n");
        DBG_INFO(("Usage: aifm FilePathName mem_length(0/1) \r\n"));
        return CLIR_INV_ARG;
    }
    ps_file_path = (CHAR*)pps_argv[1];
    ...
    if (0 == x_strncmp(pps_argv[2], "0", (SIZE_T)1))
    {
        ...
        i4_ret = aif_handler_mem_confirm_fct((VOID*)&t_feeder, 
                                             (SIZE_T)0, 
                                             &t_format);
    }
    else
    {
        ...
        i4_ret = aif_handler_mem_confirm_fct((VOID*)pui1_buf, 
                                             (SIZE_T)ui8_size, 
                                             &t_format);
        _mm_util_mem_free(pui1_buf);
    }
 ...
 * Returns: 
 ----------------------------------------------------------------------------*/
INT32 aif_mfmtrecg_cli_init(CLI_EXEC_T** ppt_command_array)
{
#ifdef CLI_LVL_ALL

    *ppt_command_array = at_aif_mfmtrecg_cmd_tbl;
