...
#include "mutil/mfmtrecg/flv_mfmtrecg/flv_mfmtrecg.h"
...
#ifdef CLI_LVL_ALL
static INT32 mfmtrecg_cli_flv_fconfirm_fct(INT32 i4_argc, const CHAR** pps_argv);     
static INT32 mfmtrecg_cli_flv_mconfirm_fct(INT32 i4_argc, const CHAR** pps_argv);     

/* mfmtrecg command table */
static CLI_EXEC_T at_flv_mfmtrecg_cmd_tbl[] =
{
    {"flv fconfirm" ,       "flvf",     mfmtrecg_cli_flv_fconfirm_fct,  NULL,       "FLV format file check",                   CLI_SUPERVISOR},
    {"flv mconfirm" ,       "flvm",     mfmtrecg_cli_flv_mconfirm_fct,  NULL,       "FLV format mem check",                    CLI_SUPERVISOR},
    END_OF_CLI_CMD_TBL
};
...
/*-----------------------------------------------------------------------------
 * Name: mfmtrecg_cli_flv_fconfirm_fct
 *
 * Description: This API returns the media format for file handle.
 *
 * Inputs: -command format : flvf  /mnt/usb/Mass-000/file_path
 *
 * Outputs: -
 *
 * Returns:
...
static INT32 mfmtrecg_cli_flv_fconfirm_fct(INT32 i4_argc, const CHAR** pps_argv)
{
    ...
    do
    {
        ...
        i4_ret = flv_handler_file_confirm_fct(h_file, &t_format);
        if(i4_ret != MFMTRECGR_OK)
        {
            x_dbg_stmt("File could not be recognized, i4_ret=%d\r\n", i4_ret);
            i4_ret =  CLIR_INV_ARG;
            break;
        }
        x_dbg_stmt("Media Format is FLV!\r\n");
    } while(0);
...
static INT32 mfmtrecg_cli_flv_mconfirm_fct(INT32 i4_argc, const CHAR** pps_argv)
{
    ...
    {
        x_dbg_stmt("Invalid argument! \r\n");
        DBG_INFO(("Usage: flvm filepath mem_length(0/1) \r\n"));
        return CLIR_INV_ARG;
    }
    ...
    if (0 == x_strncmp(pps_argv[2], "0", (SIZE_T)1))
    {
        ...
        i4_ret = flv_handler_mem_confirm_fct((VOID*)&t_feeder, 
                                             (SIZE_T)0, 
                                             &t_format);
    }
    else
    {
        ...
        x_dbg_stmt("ui8_len_hi =%u\n",(UINT32)(ui8_len>>32));
        x_dbg_stmt("ui8_len_low =%u\n",(UINT32)(ui8_len&0x00000000FFFFFFFF));
        i4_ret = flv_handler_mem_confirm_fct((VOID *) pui1_buf,
                                             (SIZE_T) ui8_len,
                                             &t_format);
        x_mem_free(pui1_buf);
    }
    if(i4_ret != MFMTRECGR_OK)
    {
        x_dbg_stmt("x_minfo_mopen() failed returns: %d\n",i4_ret);
        dbg_abort(DBG_MOD_QA);
    }
    else 
    {
        x_dbg_stmt("x_mfmtrecg_get_mem_fmt() passed \r\n");
        x_dbg_stmt("Media Format is FLV \r\n");
    }
    return CLIR_OK;
#else
 ...
 * Returns: 
 ----------------------------------------------------------------------------*/
INT32 flv_mfmtrecg_cli_init(CLI_EXEC_T **ppt_command_array)
{
#ifdef CLI_LVL_ALL

    *ppt_command_array = at_flv_mfmtrecg_cmd_tbl;
