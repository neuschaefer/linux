...
#include "mutil/mfmtrecg/rm_mfmtrecg/rm_mfmtrecg.h"
...
#ifdef CLI_LVL_ALL
static INT32 mfmtrecg_cli_rm_fconfirm_fct(INT32 i4_argc, const CHAR** pps_argv);     
static INT32 mfmtrecg_cli_rm_mconfirm_fct(INT32 i4_argc, const CHAR** pps_argv);     

/* mfmtrecg command table */
static CLI_EXEC_T at_rm_mfmtrecg_cmd_tbl[] =
{
    {"rm fconfirm" ,   "rmf",                     mfmtrecg_cli_rm_fconfirm_fct,  
     NULL,              "RM format file check",    CLI_SUPERVISOR},
    {"rm mconfirm" ,   "rmm",                     mfmtrecg_cli_rm_mconfirm_fct,  
     NULL,              "RM format mem check",     CLI_SUPERVISOR},
    END_OF_CLI_CMD_TBL
};
...
/*-----------------------------------------------------------------------------
 * Name: mfmtrecg_cli_rm_fconfirm_fct
 *
 * Description: This API returns the media format for file handle.
 *
 * Inputs: -command format : rmf  /mnt/usb/Mass-000/file_path
 *
 * Outputs: -
 *
 * Returns:
...
static INT32 mfmtrecg_cli_rm_fconfirm_fct(INT32 i4_argc, const CHAR** pps_argv)
{
    ...
    if ((i4_argc != 2) || (pps_argv == NULL))
    {
        ...
        x_dbg_stmt("Usage: rmf FilePathName \r\n");
        return CLIR_INV_ARG;
    }
    ...
    do
    {
        ...
        i4_ret = rm_handler_file_confirm_fct(h_file, &t_format);
...
{  
    ...
    {
        x_dbg_stmt("Invalid argument! \r\n");
        x_dbg_stmt("Usage: rmm FilePathName mem_length(0/1) \r\n");
        return CLIR_INV_ARG;
    }
    ps_file_path = (CHAR*)pps_argv[1];
    ...
    if (0 == x_strncmp(pps_argv[2], "0", (SIZE_T)1))
    {
        ...
        i4_ret = rm_handler_mem_confirm_fct((VOID*)&t_feeder, 
                                             (SIZE_T)0, 
                                             &t_format);
    }
    else
    {
        ...
        i4_ret = rm_handler_mem_confirm_fct((VOID*)pui1_buf, 
                                             (SIZE_T)ui8_size, 
                                             &t_format);
        _mm_util_mem_free(pui1_buf);
    }
 ...
 * Returns: 
 ----------------------------------------------------------------------------*/
INT32 rm_mfmtrecg_cli_init(CLI_EXEC_T** ppt_command_array)
{
#ifdef CLI_LVL_ALL

    *ppt_command_array = at_rm_mfmtrecg_cmd_tbl;
