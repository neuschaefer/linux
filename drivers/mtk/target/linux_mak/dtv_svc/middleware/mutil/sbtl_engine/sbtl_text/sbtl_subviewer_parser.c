 ...
 *---------------------------------------------------------------------------*/
/*-----------------------------------------------------------------------------
 * $RCSfile: sbtl_subviewer_parser.c,v $
 * $Revision: #1 $
 * $Date: 2012/04/27 $
 * $Author: dtvbm11 $
 * $CCRevision: $
 ...
 * Returns: -
 ----------------------------------------------------------------------------*/
INT32 svsub_seek_time_stamp(SBTL_PAGE_INST_MNGR_T*      pt_mngr,
                            PTS_T                       t_pts_current,
                            PTS_T*                      pt_pts_start,
                            PTS_T*                      pt_pts_end,
                            BOOL*                       pb_found)
    ...
    while(1)
    {
        ...
        if (pt_private->b_UTF16)
        {
               ...
               ('.' == aui2_buf[20]))
            {
                ...
                *pt_pts_start = (UINT64)_mm_div64(PTS_FREQ*(UINT64)ui4_ms, 1000, NULL)
                ...
                ui4_hour = sbtl_wasci2decimal(&aui2_buf[12], 2);
           ...
           ('.' == aui1_buf[20]))
        {
            ...
            *pt_pts_start = (UINT64)_mm_div64(PTS_FREQ*(UINT64)ui4_ms, 1000, NULL)
            ...
            ui4_hour = sbtl_asci2decimal(&aui1_buf[12], 2);
 ...
 * Returns: -
 ----------------------------------------------------------------------------*/
INT32 svsub_load_string(SBTL_PAGE_INST_MNGR_T*      pt_mngr)
{
    MEDIA_FEEDER_T*                 pt_feeder;
    UINT8                           au1MatchCode[4] = {(UINT8)'[', (UINT8)'b', (UINT8)'r', (UINT8)']'};
    UINT8                           ui1_idx = 0;
    UINT8                           ui1_idx2 = 0;
    UINT8                           ui1_tmp;
    UINT16                          ui2_tmp;
    ...
    while(1)
    {
        ...
        {
            sbtl_feeder_setpos(pt_mngr->pt_engine, i8_offset, MEDIA_SEEK_CUR, MM_SBTL_FEEDER_SUBTITLE);
            break;
        }
    }
 ...
 * Returns: -
 ----------------------------------------------------------------------------*/
UINT32 svsub_timer_period(SBTL_PAGE_INST_MNGR_T*    pt_mngr)
{
 ...
 * Returns: -
 ----------------------------------------------------------------------------*/
INT32 svsub_init(SBTL_PAGE_INST_MNGR_T*     pt_mngr)
{
    return MMSBTLR_OK;
}
 ...
 * Returns: -
 ----------------------------------------------------------------------------*/
INT32 svsub_deinit(SBTL_PAGE_INST_MNGR_T*   pt_mngr)
{
    return MMSBTLR_OK;
}
 ...
 * Returns: -
 ----------------------------------------------------------------------------*/
VOID svsub_start(SBTL_PAGE_INST_MNGR_T*     pt_mngr)
{
}
 ...
 * Returns: -
 ----------------------------------------------------------------------------*/
VOID svsub_stop(SBTL_PAGE_INST_MNGR_T*      pt_mngr)
{
}
...
00:04:35.03,00:04:38.82
*/
BOOL svsub_parser_verify(SBTL_ENGINE_T*     pt_engine)
{
    INT32   i4_ret;
    UINT16  ui1_idx;
    UINT8   aui1_buf[24];
    ...
    for(ui1_idx=0; ui1_idx<256; ui1_idx++)
    {
        if (b_UTF16)
        {
            ...
            if((':' == aui2_buf[0]) && ('.' == aui2_buf[3]) && 
               (',' == aui2_buf[6]) && (':' == aui2_buf[9]) && 
               (':' == aui2_buf[12]) && ('.' == aui2_buf[15]))
            {
                return TRUE;
            }
        }
        else
        {
            ...
            if((':' == aui1_buf[0]) && ('.' == aui1_buf[3]) && 
               (',' == aui1_buf[6]) && (':' == aui1_buf[9]) && 
               (':' == aui1_buf[12]) && ('.' == aui1_buf[15]))
            {
                return TRUE;
            }
        }
