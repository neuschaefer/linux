 ...
 *---------------------------------------------------------------------------*/
/*-----------------------------------------------------------------------------
 * $RCSfile: _sm_bypass_comp_vd.c,v $
 * $Revision: #2 $
 * $Date: 2012/11/03 $
 * $Author: xingju.wang $
...
static SELECT_T _bypass_rm_comp_sel_fct_vd( DRV_TYPE_T     e_type,
                                            UINT16         ui2_id,
...
static VOID _bypass_rm_nfy_fct_vd( HANDLE_T     h_obj,
                                   UINT8        ui1_port,
...
/*-----------------------------------------------------------------------------
 * Name: _bypass_rm_comp_sel_fct_vd
 *
                                   ...
                                   UINT32       ui4_data )
{
    ...
    {
        SM_BYPASS_DBG_ERR(("[SM] Bypass, _bypass_rm_nfy_fct_vd(), h_stream(0x%.8X) is invalid!\r\n",
                            h_stream ));
        return;
    }    
...
/*-----------------------------------------------------------------------------
 * Name: _sm_bypass_comp_vd_ctrl_nfy
 *
 * Description: <function description>
 *
 ...
 *
 ----------------------------------------------------------------------------*/
VOID _sm_bypass_comp_vd_ctrl_nfy( 
    VOID*             pv_nfy_tag,
    ...
    UINT32            ui4_data_2 )
{
    ...
    if( !x_handle_valid(h_stream))
    {
        SM_BYPASS_DBG_ERR(("[SM] Bypass, _sm_bypass_comp_vd_ctrl_nfy(), h_stream(0x%.8X) is invalid!\r\n",
                            h_stream ));
        return;
    }    
    
    t_msg.h_stream = h_stream;
    t_msg.e_msg_type = SM_BYPASS_HDLR_MSG_VD_CTRL_NFY;
...
static VOID _sm_bypass_codec_convert_scdb_to_vd( 
    VID_ENC_T           e_enc,
...
static CHAR* _sm_bypass_hdlr_set_codec_str(
 ...
 *
 ----------------------------------------------------------------------------*/
INT32 sm_bypass_comp_vd_open( HANDLE_T                h_stream,
                              HANDLE_T                h_source, 
                              SM_DRV_COMP_DESCR_T*    pt_comp_descr )
{
    ...
    i4_ret = sm_drv_comp_open( h_source,
                               ...
                               (VOID*)pt_comp_descr,
                               &_bypass_rm_comp_sel_fct_vd,
                               (VOID*)h_stream,
                               &_bypass_rm_nfy_fct_vd,
                               pt_comp_descr );
...
/*-----------------------------------------------------------------------------
 * Name: sm_bypass_comp_vd_connect
 *
 * Description: <function description>
 *
 * Inputs:  h_stream      References.
 ...
 *
 ----------------------------------------------------------------------------*/
INT32 sm_bypass_comp_vd_connect( HANDLE_T               h_stream, 
                                 SM_BYPASS_STREAM_T*    pt_bypass_obj )
{
    ...
    i4_ret = pm_get_component_info( pt_bypass_obj->t_base.h_pm_comp,
                                    &h_dmx );
    if ( i4_ret != PMR_OK )
    {
    ...
    h_source = h_dmx;
#else
    h_source = pt_bypass_obj->t_base.h_source;
#endif
...
/*-----------------------------------------------------------------------------
 * Name: sm_bypass_comp_vd_disconnect
 *
 * Description: <function description>
 *
 ...
 *
 ----------------------------------------------------------------------------*/
INT32 sm_bypass_comp_vd_disconnect( HANDLE_T                h_stream, 
...
/*-----------------------------------------------------------------------------
 * Name: sm_bypass_comp_vd_set_callback
 *
 * Description: <function description>
 *
 * Inputs:  h_stream      References.
 ...
 *
 ----------------------------------------------------------------------------*/
VOID sm_bypass_comp_vd_set_callback( HANDLE_T               h_stream,
                                     SM_BYPASS_STREAM_T*    pt_bypass_obj )
{
    INT32                 i4_ret;
...
#endif
    {
        t_set_nfy_info.pf_vid_dec_nfy = _sm_bypass_comp_vd_ctrl_nfy;
        ...
        {
            SM_BYPASS_DBG_ERR(( 
                "[SM] BYPASS, rm_set(VID_DEC_SET_TYPE_NFY_FCT) fail, i4_ret = %d\r\n",
                i4_ret ));
            SM_ASSERT( 0 );
            return;
        }
...
/*-----------------------------------------------------------------------------
 * Name: sm_bypass_comp_vd_set_codec
 *
 * Description: <function description>
 *
 * Inputs:  h_stream      References.
 ...
 *
 ----------------------------------------------------------------------------*/
INT32 sm_bypass_comp_vd_set_codec( HANDLE_T               h_stream,
                                   SM_BYPASS_STREAM_T*    pt_bypass_obj )
{
    ...
    if ( e_cond == RM_COND_CONN_ORIGINATOR )
    {
        if ( !pt_bypass_obj->b_codec_not_support )
        {
            return SMR_OK;
        }
    
        _sm_bypass_codec_convert_scdb_to_vd( pt_bypass_obj->e_actual_enc, 
                                             &e_vd_codec_type );
        
        x_dbg_stmt( "[SM] BYPASS(0x%.8X), set video codec (%s)\r\n", 
                      h_stream, _sm_bypass_hdlr_set_codec_str((VID_ENC_T)pt_bypass_obj->e_actual_enc));
        ...
        {
            SM_BYPASS_DBG_ERR(( 
                "[SM] BYPASS, rm_set(VID_DEC_SET_TYPE_CODEC) fail, i4_ret = %d\r\n",
                i4_ret ));
            sm_bypass_hdlr_send_msg_do_event( 
                h_stream,
                SM_EVN_BYPASS_CODEC_NOT_SUPPORT,
                (UINT32)0 );
            return SMR_INTERNAL_ERROR;
        }
        pt_bypass_obj->b_codec_not_support = FALSE;
    }
    return SMR_OK;
}

/*-----------------------------------------------------------------------------
 * Name: sm_bypass_comp_vd_get_actual_enc
 *
 * Description: <function description>
 *
 ...
 *          
 *
 * Outputs: pt_bypass_obj  References
 *
 * Returns: -
 *
 ----------------------------------------------------------------------------*/
VOID sm_bypass_comp_vd_get_actual_enc( SM_BYPASS_STREAM_T*    pt_bypass_obj )
{
    switch ( pt_bypass_obj->t_base.t_scdb_rec.e_rec_type )
    {
    case SCDB_REC_TYPE_VIDEO_MPEG:
        pt_bypass_obj->e_actual_enc = pt_bypass_obj->t_base.t_scdb_rec.u.t_video_mpeg.e_enc;
        break;

    case SCDB_REC_TYPE_VIDEO_AVI:
        pt_bypass_obj->e_actual_enc = pt_bypass_obj->t_base.t_scdb_rec.u.t_video_avi.e_enc;
        break;

    case SCDB_REC_TYPE_VIDEO_MP4:
        pt_bypass_obj->e_actual_enc = pt_bypass_obj->t_base.t_scdb_rec.u.t_video_mp4.e_enc;
        break;
    
    case SCDB_REC_TYPE_VIDEO_ASF:
        pt_bypass_obj->e_actual_enc = pt_bypass_obj->t_base.t_scdb_rec.u.t_video_asf.e_enc;
        break;

    case SCDB_REC_TYPE_VIDEO_MKV:
        pt_bypass_obj->e_actual_enc = pt_bypass_obj->t_base.t_scdb_rec.u.t_video_mkv.e_enc;
        break;

    case SCDB_REC_TYPE_VIDEO_RM:
        pt_bypass_obj->e_actual_enc = pt_bypass_obj->t_base.t_scdb_rec.u.t_video_rm.e_enc;
        break;
    
    /*add for flv*/
     case SCDB_REC_TYPE_VIDEO_FLV:
        pt_bypass_obj->e_actual_enc = pt_bypass_obj->t_base.t_scdb_rec.u.t_video_flv.e_enc;
        break;
    
    case SCDB_REC_TYPE_VIDEO_PS:
        pt_bypass_obj->e_actual_enc = pt_bypass_obj->t_base.t_scdb_rec.u.t_video_ps.e_enc;
        break;

    default:
        pt_bypass_obj->e_actual_enc = VID_ENC_UNKNOWN;
    }
}
 ...
 *
 ----------------------------------------------------------------------------*/
VOID sm_bypass_comp_convert_vd_codec_to_scdb_enc( 
    VID_DEC_CODEC_T     e_vd_codec_type,
    VID_ENC_T*          pe_enc )
{
