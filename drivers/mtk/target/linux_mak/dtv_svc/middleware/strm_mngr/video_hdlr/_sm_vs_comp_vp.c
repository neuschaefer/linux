 ...
 *---------------------------------------------------------------------------*/
/*-----------------------------------------------------------------------------
 * $RCSfile: _sm_vs_comp_vp.c,v $
 * $Revision: #3 $
 * $Date: 2012/11/03 $
 * $Author: xingju.wang $
 * $CCRevision: /main/DTV_X/DTV_X_HQ_int/12 $
 * $SWAuthor: Alec Lu $
 * $MD5HEX: d9c26da5d95b5005743f3fd1a9459df2 $
 *
 * Description: 
 *---------------------------------------------------------------------------*/
...
/*-----------------------------------------------------------------------------
 ...
 ----------------------------------------------------------------------------*/
static SELECT_T _rm_comp_sel_fct_vp( 
                    DRV_TYPE_T   e_type,
                    UINT16       ui2_id,
...
static VOID _rm_nfy_fct_vp( HANDLE_T   h_obj,
                            UINT8      ui1_port,
                            ...
                            UINT32     ui4_data)
{
    ...
    {
        SM_VS_DBG_ERR((
            "[SM] VS, _rm_nfy_fct_vp(), h_stream(0x%.8X) is invalid!\r\n", h_stream ));
        return;
    }

    /* 
      Return before send message to queue, because not handle the OPERATION info in sm_slctr, 
      except VID_PLA_SET_TYPE_MODE.
    */
    e_cond = rm_cond_chg( e_nfy_cond,
                          RM_COND_IGNORE,
                          COND_GRP_OPERATION );
    if( e_cond != RM_COND_NO_CHG && ui4_data != VID_PLA_SET_TYPE_MODE ) 
    {
        return;
    }
 ...
 *
 ----------------------------------------------------------------------------*/
INT32 sm_vs_comp_vp_open( HANDLE_T             h_stream,
                          HANDLE_T             h_source, 
...
/*-----------------------------------------------------------------------------
 * Name: sm_vs_comp_vp_set_op_monitor
 *
 * Description: <function description>
 *
 ...
 *
 ----------------------------------------------------------------------------*/
INT32 sm_vs_comp_vp_set_op_monitor( 
    ...
    UINT32               ui4_data )
{
    ...
    if ( x_handle_valid(pt_vs_obj->t_vs_comp_grp.t_vp.h_comp) == FALSE )
    {
        return SMR_DRV_COMP_ERROR;
    }    
    if ( ui4_data == VID_PLA_SET_TYPE_MODE )
    {
        z_get_size = sizeof(VID_PLA_MODE_T);
        i4_ret = rm_get( pt_vs_obj->t_vs_comp_grp.t_vp.h_comp,
                         FALSE,
                         ANY_PORT_NUM,
                         VID_PLA_GET_TYPE_MODE,
                         (VOID*)&pt_vs_obj->e_vp_mode,
                         &z_get_size,
                         &pt_vs_obj->t_vs_comp_grp.t_vp.e_comp_cond);
        if ( i4_ret != RMR_OK )
        {
            SM_VS_DBG_ERR((
                "[SM] VS: rm_get(TV_PLA_GET_TYPE_MODE) fail, i4_ret = %d\r\n", 
                 i4_ret ));
            return SMR_DRV_COMP_ERROR;
        }
    }
    ...
    UINT32              ui4_data_2 )
{
    ...
    {
        SM_VS_DBG_ERR((
            "[SM] VS, _sm_vs_comp_vp_ctrl_nfy(), h_stream(0x%.8X) is invalid!\r\n", h_stream ));
        return;
    }
    
    t_msg.h_stream = h_stream;
    t_msg.e_msg_type = SM_VIDEO_HDLR_MSG_VP_CTRL_NFY;
    
    t_msg.u.t_msg_vp_ctrl.e_vp_nfy_cond = e_nfy_cond;
    t_msg.u.t_msg_vp_ctrl.ui4_data_1 = ui4_data_1;
    t_msg.u.t_msg_vp_ctrl.ui4_data_2 = ui4_data_2;
...
/*-----------------------------------------------------------------------------
 * Name: sm_vs_comp_vp_set_callback
 *
 * Description: <function description>
 *
 * Inputs:  h_stream  References.
 ...
 *
 ----------------------------------------------------------------------------*/
VOID sm_vs_comp_vp_set_callback( HANDLE_T              h_stream,
                                 SM_VIDEO_STREAM_T*    pt_vs_obj )
{
    INT32                 i4_ret;
    VID_PLA_NFY_INFO_T    t_set_nfy_info;
    
    t_set_nfy_info.pf_vid_pla_nfy = _sm_vs_comp_vp_ctrl_nfy;
    t_set_nfy_info.pv_tag = (VOID*)h_stream;
    
    i4_ret = rm_set( pt_vs_obj->t_vs_comp_grp.t_vp.h_comp,
                  FALSE,
                  ANY_PORT_NUM,
                  VID_PLA_SET_TYPE_NFY_FCT,
                  (VOID*)&t_set_nfy_info,
                  sizeof(VID_PLA_NFY_INFO_T),
                  &pt_vs_obj->t_vs_comp_grp.t_vp.e_comp_cond );
    if ( i4_ret != RMR_OK )
    {
        SM_VS_DBG_ERR(( 
            "[SM] VS, rm_set(VID_PLA_SET_TYPE_NFY_FCT) fail, i4_ret = %d\r\n",
            i4_ret ));
        /*SM_ASSERT( 0 );*/
        return;
    }
