 ...
 *    SVL object to a raw file (or a regular file). It uses fs_raw_*** API's
...
/*-------------------------------------------------------------------------
            SVL file system related functions implementation
 --------------------------------------------------------------------------*/

/*----------------------------------------------------------------------
 * Name: svl_fs_raw_get_magic_for_write_ops
 *
 ...
 *
 * Inputs: 
 *   h_file     Handle to the SVL file.
 *              After return from this API, the file handle is positioned
 *              at the file location where the SVL object will be
 *              written.
 *              
 ...
 *                existing SVL object stored on filesystem.
 *
 *                TRUE: the location points to by the file handle is not
 *                      a SVL object.
 *                FALSE: the location points to by the file handle contains
 *                      an existing SVL object.
 *
 * Returns: 
 *   SVLR_OK           file handle is successfully closed. 
 *   
 -----------------------------------------------------------------------*/
static
INT32 svl_fs_raw_get_magic_for_write_ops
(
    HANDLE_T  h_file,
...
)
{
        ...
        )
    {
        ...
        */
        if ( ui4_magic != SVL_MAGIC_NUMBER  &&
             ui4_magic != (~SVL_MAGIC_NUMBER)
            )
        {
            *pui4_beg_magic = SVL_MAGIC_NUMBER;
            *pb_new_pos     = TRUE;
        }
        else
        {
...
/*----------------------------------------------------------------------
 * Name: svl_fs_raw_close_hdle
 *
 * Description: This API unlock and close the file handle. 
 *
 * Inputs: 
 *   h_svl      Handle to the SVL file.
 *
 * Outputs: None
 *
 * Returns: 
...
)
{
    ...
    /* Translate FM err code to SVL err code. */
    i4_rc=svl_fm_to_svl_err_code(i4_rc);

    return i4_rc;
}

/*----------------------------------------------------------------------
 * Name: svl_fs_raw_close_hdle_no_fm_close
 *
 * Description: This API unlock and close the file handle. 
 *
 * Inputs: 
 ...
 *   
 -----------------------------------------------------------------------*/
INT32 svl_fs_raw_close_hdle_no_fm_close
(
    HANDLE_T  h_file
)
{
    ...
    /* Translate FS_RAW err code to SVL err code. */
    i4_rc=svl_fsraw_to_svl_err_code(i4_rc);

    return i4_rc;
}

/*----------------------------------------------------------------------
 * Name: svl_fs_raw_get_read_hdle
 *
 * Description: This API return a file handle to the RAW file, the read
 *              pointer is position at the SVL object specified by
 *              the ui2_svl_id.
 *
 *              This API will check the begin and end SVL magic number
 *              to detect any corruption.
 *
 * Inputs: 
 *   ps_svl_path  Path to the RAW file.
 *   ui2_svl_id   Specify the SVL id to load
 *
 * Outputs: 
 *   ph_file:     File handle to the SVL object on the RAW file.
 *
 * Returns: 
 *   SVLR_OK           Record is successfully added. 
 *   SVLR_NOT_FOUND    SVL object not found.
 *   SVLR_INV_FILE_PATH specified h_dir and ps_svl_path is not a valid RAW
 *                      file.
 -----------------------------------------------------------------------*/
INT32 svl_fs_raw_get_read_hdle
(
    HANDLE_T           h_dir,
...
)
{
    ...
    *ph_file=NULL_HANDLE;
    ...
    if ( fs_raw_is_inited(h_dir, ps_svl_path) == FALSE )
    {
        return SVLR_INV_FILE_PATH;
    }
    /*
      Open the RAW file.
    */
    ...
    {
        /*
          Set the file offset to the SVL object found.
        */
        i4_rc=fs_raw_seek_obj(h_file, &t_meta,
                              (UINT32) FS_RAW_MAKE_OBJ_ID(SVL_GROUP_ID,ui2_svl_id),
                              &ui4_offset, &ui4_obj_sz);
        if (  i4_rc == FSRAWR_OK )
        {

            DBG_API(("{SVL} locating object for read operation> object id: %d size: %d offset: %d\n",
                       ui2_svl_id, ui4_obj_sz, ui4_offset));

            /*
              Check the begin and ending magic number for the SVL object.
            ...
            */
            ...
            if ( i4_rc == FSRAWR_OK )
            {
                ...
                DBG_API(("{SVL} begin_magic: %x end_magic: %x\n",
                           ui4_beg_magic_num, ui4_end_magic_num));

                if ( (
                         ui4_beg_magic_num == SVL_MAGIC_NUMBER  ||
                         ui4_beg_magic_num == (~SVL_MAGIC_NUMBER)
                     )   &&
                     ( ui4_beg_magic_num == (~ui4_end_magic_num) )
                   )
                {
                    /*
                      ...
                      position of the SVL object is inside
                ...
                {
                    /* Bad magic, this SVL object is corrupted. */
            ...
            {
                /* Translate FSRAW err code to SVL err code. */
                i4_rc = svl_fsraw_to_svl_err_code(i4_rc);
...
/*----------------------------------------------------------------------
 * Name: svl_fs_raw_get_read_hdle_no_fm_open
 *
 * Description: This API return a file handle to the RAW file, the read
 *              pointer is position at the SVL object specified by
 *              the ui2_svl_id.
 ...
 *                      file.
 -----------------------------------------------------------------------*/
INT32 svl_fs_raw_get_read_hdle_no_fm_open
(
    UINT16		       ui2_svl_id,
    HANDLE_T*		   ph_file	
)
...
/*----------------------------------------------------------------------
 * Name: svl_fs_raw_get_write_hdle
 *
 * Description: This API return a file handle to the RAW file, the read
 *              pointer is position at the SVL object specified by
 *              the ui2_svl_id.
 ...
 *                      file.
 -----------------------------------------------------------------------*/
INT32 svl_fs_raw_get_write_hdle
(
    HANDLE_T           h_dir,
    const CHAR*	       ps_svl_path,
    SVL_OBJ_T*         pt_svl,
...
)
{
    ...
    ui2_svl_id = pt_svl->ui2_svl_id;
    *ph_file=NULL_HANDLE;

    if ( fs_raw_is_inited(h_dir, ps_svl_path) == FALSE )
    {
        if ( fs_raw_init(h_dir, ps_svl_path) != FSRAWR_OK )
        {
            return SVLR_INV_FILE_PATH;
        }
    }
    ...
    */
    ...
    {
        /*
          Get the size of the SVL object to write to
          file
        */
        ui4_obj_space_needed=svl_persistent_size(pt_svl);
        

        DBG_API(("{SVL} object spaced required: %d bytes\n",ui4_obj_space_needed));
        ...
        {   
            ...
            */
            ...
            if ( i4_rc == FSRAWR_OK )
            {
                ...
                */
                if ( svl_fs_raw_get_magic_for_write_ops(h_file,
                                                        ui4_write_offset,
                                                        pui4_beg_magic,
                                                        pb_new_pos)
                     == SVLR_OK
...
/*----------------------------------------------------------------------
 * Name: svl_fs_raw_get_write_hdle_no_fm_open
 *
 * Description: This API return a file handle to the RAW file, the read
 *              pointer is position at the SVL object specified by
 *              the ui2_svl_id.
 ...
 *                      file.
 -----------------------------------------------------------------------*/
INT32 svl_fs_raw_get_write_hdle_no_fm_open
(
    SVL_OBJ_T*         pt_svl,
    HANDLE_T*          ph_file,
    UINT32*            pui4_beg_magic,
 ...
 * Name: svl_fs_raw_del
 *
 * Description: This API delete an SVL object from the RAW file.
 *             
 *
 * Inputs: 
 *   ps_svl_path  Path to the RAW file.
 *   ui2_svl_id   Specify the SVL id to delete.
 *
 * Outputs: None
 *
 * Returns: 
 ...
 * Name: svl_fs_raw_qry
 *
 * Description: This query the SVL ID and NAME of a SVL object store
 *              on the raw file.   
 *
 * Inputs: 
        ...
        )
    {
        ...
        { 
            ...
            */
            ...
            if ( i4_rc == FSRAWR_OK )
            {
                if ( FS_RAW_GET_MODULE_ID(ui4_obj_id) == SVL_GROUP_ID )
                {
                    if ( ui2_nb_matched == ui2_idx )
                    {
                        /* Read meta data of the SVL object. */
                        if ( svl_read_file_info(h_file, &t_finfo, NULL) == SVLR_OK )
                        {
                            /* Assertion check, object id and svl id must be the same. */
                            if (  FS_RAW_GET_ID(ui4_obj_id) == t_finfo.ui2_svl_id )
                            {
                                *pui2_svl_id=t_finfo.ui2_svl_id;
                                x_strncpy(ps_svl_name, t_finfo.ps_svl_name,MAX_SVL_NAME_LEN);
                                i4_rc = SVLR_OK;
                                break;
                            }
                            else
