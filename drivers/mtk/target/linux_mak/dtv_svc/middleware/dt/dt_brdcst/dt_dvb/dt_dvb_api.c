 ...
 *   DTR_FAIL        SST table rejected.
 -------------------------------------------------------------------------*/
static INT32 dt_parse_dvb_tdt_tot
(
    HANDLE_T       h_obj,
        ...
        */     
        if ( ui1_tbl_id == DVB_TOT_TBL_ID )
        {
            ...
            if ( b_tot_rc == TRUE )
            {
                ...
                /*
                  Set the Timezone offset value from the TOT table.
                */
                if (  x_dt_get_tz_ctrl() == TRUE )
                {
                    ...
                    if ( curr_tz_offset != t_tot.t_local_tz_offset )
                    {
                        x_dt_set_tz (t_tot.t_local_tz_offset);
 ...
 *   DTR_FAIL        SST table rejected.
 -------------------------------------------------------------------------*/
INT32 x_dt_reg_dvb(VOID)
{
    static const DT_BRDCST_DVB_API_T  t_table =
        {
            dt_parse_dvb_tdt_tot,
    ...
    static const DT_BRDCST_API_T     t_brdcst_table =
        {
            dt_load_brdcst_time_info,
            dt_unload_brdcst_time_info
        };
    ...
    */
    ...
    dt_reg_brdcst_functions(&t_brdcst_table);

    /* register clock specific API. */
    x_dt_clock_reg_dvb();
    
    return dt_reg_dvb_functions(&t_table);
}
