 ...
 *---------------------------------------------------------------------------*/
/*-----------------------------------------------------------------------------
 * $RCSfile: rm_drv_mc_nfy_vid_dec.c,v $
 * $Revision: #1 $
 * $Date: 2012/04/27 $
 * $Author: dtvbm11 $
 *
...
#define MAKE_TAG(_ptr)  ((VOID*) ((((UINT32) (_ptr - pt_vid_dec_dir_head)) & 0x0000ffff) | ((((UINT32) _ptr->ui1_count) << 16) & 0x00ff0000)))
...
#define MAX_NUM_INST  8

/* Instance control structure. */
typedef struct _VID_DEC_INST_CTRL_T
{
    x_vid_dec_nfy_fct     pf_vid_dec_nfy;  /* Status notification. */
    VOID*  pv_nfy_tag;
...
typedef struct _VID_DEC_DEV_CTRL_T
{
    VID_DEC_INST_CTRL_T  at_vid_dec_inst [MAX_NUM_INST];

    union
    {
        VID_DEC_NFY_INFO_T  vid_dec_nfy_info;           /* The following fields carry the notify info */
    }   u;
...
typedef struct _VID_DEC_DIR_CTRL_T
{
    VID_DEC_DEV_CTRL_T*  pt_vid_dec_dev;                /* References the device control object. */
...
static VID_DEC_DIR_CTRL_T*  pt_vid_dec_dir_head;
...
static VID_DEC_DIR_CTRL_T* alloc_dir (VOID);
static VID_DEC_DIR_CTRL_T* find_dir (UINT16  ui2_dev_id);

static VID_DEC_INST_CTRL_T* alloc_inst (VID_DEC_DEV_CTRL_T*  pt_vid_dec_dev);
static VID_DEC_INST_CTRL_T* find_inst (VID_DEC_DEV_CTRL_T*  pt_vid_dec_dev,
                                       HANDLE_T             h_obj);

static INT32 vid_decoder_add_dev (UINT16  ui2_dev_id);
static VOID vid_decoder_del_dev (UINT16  ui2_dev_id);
static VOID vid_decoder_rst_dev (UINT16  ui2_dev_id);
static INT32 vid_decoder_add_inst (HANDLE_T  h_obj,
                                   UINT16    ui2_dev_id);
static VOID vid_decoder_del_inst (HANDLE_T  h_obj,
                                  UINT16    ui2_dev_id);
static BOOL vid_decoder_set (HANDLE_T        h_obj,
                             UINT16          ui2_dev_id,
...
static VOID vid_dec_nfy_fct (VOID*           pv_nfy_tag,
...
const static MC_NFY_FCT_TBL_T  t_mc_nfy_vid_decoder =
{
    DRVT_VID_DEC,
    ...
    vid_decoder_del_dev,
    vid_decoder_rst_dev,
    vid_decoder_add_inst,
    vid_decoder_del_inst,
    vid_decoder_set
};
...
static VID_DEC_DIR_CTRL_T* alloc_dir (VOID)
{
    ...
    VID_DEC_DIR_CTRL_T* pt_vid_dec_dir;
    VID_DEC_DIR_CTRL_T* pt_vid_dec_dir_del;
    ...
    while ((ui2_idx < ui2_curr_dir_entry)                        &&
           (pt_vid_dec_dir_head [ui2_idx].pt_vid_dec_dev != NULL))
    {
        ui2_idx++;
    }
    ...
    if (ui2_idx >= ui2_curr_dir_entry)
    {
        if ((ui2_idx = ui2_curr_dir_entry) >= ui2_num_dir_entries)
        {
            ...
            if ((pt_vid_dec_dir = (VID_DEC_DIR_CTRL_T*) x_mem_alloc ((ui2_num_dir_entries + NUM_ALLOC_DIR) * sizeof (VID_DEC_DIR_CTRL_T))) != NULL)
            {
                x_memset (((VOID*) pt_vid_dec_dir), 0, ((ui2_num_dir_entries + NUM_ALLOC_DIR) * sizeof (VID_DEC_DIR_CTRL_T)));

                /* Copy the existing data. */
                if (pt_vid_dec_dir_head != NULL)
                {
                    x_memcpy (((VOID*) pt_vid_dec_dir), ((VOID*) pt_vid_dec_dir_head), (ui2_num_dir_entries * sizeof (VID_DEC_DIR_CTRL_T)));
                }
                ...
                pt_vid_dec_dir_del  = pt_vid_dec_dir_head;
                pt_vid_dec_dir_head = pt_vid_dec_dir;
                ...
                if (pt_vid_dec_dir_del != NULL)
                {
                    x_mem_free ((VOID*) pt_vid_dec_dir_del);
                }
    ...
    {
        pt_vid_dec_dir = &(pt_vid_dec_dir_head [ui2_idx]);
    }

    return (pt_vid_dec_dir);
}
...
static VID_DEC_DIR_CTRL_T* find_dir (UINT16  ui2_dev_id)
{
    ...
    while ((ui2_idx < ui2_num_dir_entries)                          &&
           (pt_vid_dec_dir_head [ui2_idx].ui2_dev_id != ui2_dev_id))
    {
        ui2_idx++;
    }
...
{
    UINT16               ui2_idx;
    VID_DEC_INST_CTRL_T* pt_vid_dec_inst;

    pt_vid_dec_inst = NULL;
    ui2_idx         = 0;

    while ((ui2_idx < MAX_NUM_INST)                                        &&
           (pt_vid_dec_dev->at_vid_dec_inst [ui2_idx].h_obj != NULL_HANDLE))
    {
        ui2_idx++;
    }
    ...
    {
        pt_vid_dec_inst = &(pt_vid_dec_dev->at_vid_dec_inst [ui2_idx]);
    }

    return (pt_vid_dec_inst);
}
 ...
 *
 * Inputs:  pt_vid_dec_dev  Refernces the device control object.
                                       ...
                                       HANDLE_T             h_obj)
{
    ...
    while ((ui2_idx < MAX_NUM_INST)                                  &&
           (pt_vid_dec_dev->at_vid_dec_inst [ui2_idx].h_obj != h_obj))
    {
        ui2_idx++;
    }
...
static INT32 vid_decoder_add_dev (UINT16  ui2_dev_id)
{
    INT32               i4_return;
    CRIT_STATE_T        t_state;
    VID_DEC_DEV_CTRL_T* pt_vid_dec_dev;
    VID_DEC_DIR_CTRL_T* pt_vid_dec_dir;
    ...
    if (find_dir (ui2_dev_id) == NULL)
    {
        ...
        if (((pt_vid_dec_dir = alloc_dir ())                                                  != NULL)  &&
            ((pt_vid_dec_dev = (VID_DEC_DEV_CTRL_T*) x_mem_alloc (sizeof (VID_DEC_DEV_CTRL_T))) != NULL))
        {
            i4_return = RMR_OK;

            x_memset (((VOID*) pt_vid_dec_dev), 0, sizeof (VID_DEC_DEV_CTRL_T));

            /* The assignment must be done inside a critical section since the */
            /* device control blcok may be accessed from an ISR context.       */
            t_state = x_crit_start ();

            pt_vid_dec_dir->pt_vid_dec_dev = pt_vid_dec_dev;
            pt_vid_dec_dir->ui2_dev_id     = ui2_dev_id;
            ...
            if ((UINT16) (pt_vid_dec_dir - pt_vid_dec_dir_head) == ui2_curr_dir_entry)
            {
                t_state = x_crit_start ();
...
static VOID vid_decoder_del_dev (UINT16  ui2_dev_id)
{
    ...
    if ((pt_vid_dec_dir = find_dir (ui2_dev_id)) != NULL)
    {
        ...
        pt_vid_dec_dev = pt_vid_dec_dir->pt_vid_dec_dev;

        t_state = x_crit_start ();

        pt_vid_dec_dir->pt_vid_dec_dev = NULL;
        pt_vid_dec_dir->ui2_dev_id     = 0;
        pt_vid_dec_dir->ui1_count++;

        x_crit_end (t_state);

        /* And now, free up the device control object. */
        if (pt_vid_dec_dev != NULL)
        {
            x_mem_free ((VOID*) pt_vid_dec_dev);
        }
        ...
        if ((UINT16) ((pt_vid_dec_dir - pt_vid_dec_dir_head) + 1) == ui2_curr_dir_entry)
        {
            t_state = x_crit_start ();
...
static VOID vid_decoder_rst_dev (UINT16  ui2_dev_id)
{
    ...
    if ((pt_vid_dec_dir = find_dir (ui2_dev_id)) != NULL)
    {
        if ((pt_vid_dec_dev = pt_vid_dec_dir->pt_vid_dec_dev) == NULL)
        {
        ...
        while (ui2_idx < MAX_NUM_INST)
        {
            pt_vid_dec_dev->at_vid_dec_inst [ui2_idx].pf_vid_dec_nfy    = NULL;
            pt_vid_dec_dev->at_vid_dec_inst [ui2_idx].pv_nfy_tag        = NULL;
...
/*-----------------------------------------------------------------------------
 * Name: vid_decoder_add_inst
 *
 * Description: This API will allocate an instance for a single Video Decoder
 *              driver component.
 *
                                   ...
                                   UINT16    ui2_dev_id)
{
    ...
    if ((pt_vid_dec_dir = find_dir (ui2_dev_id)) != NULL)
    {
        ...
        if (find_inst (pt_vid_dec_dev, h_obj) == NULL)
        {
            i4_return = RMR_NOT_ENOUGH_MEM;

            if ((pt_vid_dec_inst = alloc_inst (pt_vid_dec_dev)) != NULL)
            {
                i4_return = RMR_OK;
                ...
                pt_vid_dec_inst->pf_vid_dec_nfy    = NULL;
                pt_vid_dec_inst->pv_nfy_tag        = NULL;
                pt_vid_dec_inst->h_obj             = h_obj;

                /* And now, update the number of entries if required. */
                if ((UINT16) (pt_vid_dec_inst - &(pt_vid_dec_dev->at_vid_dec_inst [0])) >= pt_vid_dec_dev->ui2_curr_inst_entry)
                {
                    t_state = x_crit_start ();

                    pt_vid_dec_dev->ui2_curr_inst_entry++;
...
/*-----------------------------------------------------------------------------
 * Name: vid_decoder_del_inst
 *
 * Description: This API will delete an instance for a single Video Decoder
 *              driver component.
 *
 * Inputs:  h_obj       Contains a unique handle which identifies the
 *                      instance.
                                  ...
                                  UINT16    ui2_dev_id)
{
    ...
    if ((pt_vid_dec_dir = find_dir (ui2_dev_id)) != NULL)
    {
        ...
        if ((pt_vid_dec_inst = find_inst (pt_vid_dec_dev, h_obj)) != NULL)
        {
            ...
            pt_vid_dec_inst->pv_nfy_tag    = NULL;
            pt_vid_dec_inst->h_obj         = NULL_HANDLE;

            /* And update the number of entries if required. */
            if ((UINT16) (pt_vid_dec_inst - &(pt_vid_dec_dev->at_vid_dec_inst [0])) >= pt_vid_dec_dev->ui2_curr_inst_entry)
            {
                /* And now clear out all the old deleted instances. */
                while ((pt_vid_dec_dev->ui2_curr_inst_entry > 0)                                                       &&
                       (pt_vid_dec_dev->at_vid_dec_inst [pt_vid_dec_dev->ui2_curr_inst_entry - 1].h_obj == NULL_HANDLE))
                {
                    t_state = x_crit_start ();
...
/*-----------------------------------------------------------------------------
 * Name: vid_decoder_set
 *
 * Description: This API will intercept the set notify functions to the Video
                             ...
                             INT32*          pi4_return)
{
    ...
    if (e_set_type == VID_DEC_SET_TYPE_NFY_FCT)
    {
                ...
                switch (e_set_type)
                {
                    case VID_DEC_SET_TYPE_NFY_FCT:
                            ...
                            (pz_set_info_len != NULL))
                        {
                            ...
                            pt_vid_dec_dev->u.vid_dec_nfy_info = (*((VID_DEC_NFY_INFO_T*) (*ppv_set_info)));
                            ...
                            pt_vid_dec_inst->pf_vid_dec_nfy = ((VID_DEC_NFY_INFO_T*) (*ppv_set_info))->pf_vid_dec_nfy;
                            pt_vid_dec_inst->pv_nfy_tag     = ((VID_DEC_NFY_INFO_T*) (*ppv_set_info))->pv_tag;

                            x_crit_end (t_state);

                            if ((pt_vid_dec_dev->ui1_flags & DEV_FLAG_NFY_FCT_SET) == 0)
                            {
                                ...
                                pt_vid_dec_dev->u.vid_dec_nfy_info.pf_vid_dec_nfy = vid_dec_nfy_fct;
                                pt_vid_dec_dev->u.vid_dec_nfy_info.pv_tag        = MAKE_TAG (pt_vid_dec_dir);

                                (*ppv_set_info)    = (VOID*) &(pt_vid_dec_dev->u.vid_dec_nfy_info);
                                (*pz_set_info_len) = sizeof (VID_DEC_NFY_INFO_T);

                                /* And signal that the notify function has been set. */
                                t_state = x_crit_start ();

                                pt_vid_dec_dev->ui1_flags |= DEV_FLAG_NFY_FCT_SET;
 ...
 * Returns: -
 ----------------------------------------------------------------------------*/
static VOID vid_dec_nfy_fct (VOID*           pv_tag,
                             VID_DEC_COND_T  e_nfy_cond,
                             UINT32          ui4_data_1,
                             UINT32          ui4_data_2)
{
    ...
    if (((ui2_idx = GET_IDX (pv_tag)) < ui2_curr_dir_entry)                          &&
        ((ui1_count = GET_COUNT (pv_tag)) == pt_vid_dec_dir_head [ui2_idx].ui1_count) &&
        ((pt_vid_dec_dev = pt_vid_dec_dir_head [ui2_idx].pt_vid_dec_dev) != NULL)       &&
        ((pt_vid_dec_dev->ui1_flags & DEV_FLAG_NFY_FCT_SET) != 0))
    {
        /* Tag is valid. Now notify all interested parties. */
        x_crit_end (t_state);

        pt_vid_dec_dir  = &(pt_vid_dec_dir_head [ui2_idx]);
        pt_vid_dec_inst = &(pt_vid_dec_dev->at_vid_dec_inst [0]);
        ...
        while ((ui1_count == pt_vid_dec_dir->ui1_count)         &&
               (ui2_idx < pt_vid_dec_dev->ui2_curr_inst_entry))
        {
            if ((pf_vid_dec_nfy = pt_vid_dec_inst->pf_vid_dec_nfy) != NULL)
            {
                pv_nfy_tag = pt_vid_dec_inst->pv_nfy_tag;

                x_crit_end (t_state);

                /* And exeute the clients real callback function. */
                pf_vid_dec_nfy (pv_nfy_tag, e_nfy_cond, ui4_data_1, ui4_data_2);
            }
            else
            {
                x_crit_end (t_state);
 ...
 *                                        for component type exists alreday.
 ----------------------------------------------------------------------------*/
INT32 rm_drv_mc_nfy_vid_dec_init (VOID)
{
    ui2_num_dir_entries = 0;
    ui2_curr_dir_entry  = 0;

    pt_vid_dec_dir_head = NULL;

    return (rm_reg_mc_nfy (&t_mc_nfy_vid_decoder));
}
