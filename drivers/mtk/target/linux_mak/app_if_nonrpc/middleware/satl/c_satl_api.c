 ...
 *---------------------------------------------------------------------------*/
/*-----------------------------------------------------------------------------
 * $RCSfile: c_satl_api.c,v $
 * $Revision: #1 $
 * $Date: 2012/04/27 $
 * $Author: dtvbm11 $
 *
 * Description: 
 *    This file contains implmentation of the Satellite list (SATL)
...
)
{
    ...
    switch (i4_rc)
    {
        case AEER_OK:
        {
            i4_rc = x_satl_create(ps_satl_name, ui2_satl_id, e_mem_policy,
                                  ui2_max_nb_rec, pv_tag, pf_nfy, ph_satl);
            
            if (i4_rc == SATLR_OK)
            {
                handle_link_to_aux(h_aux, *ph_satl);
            }
            else
            {
        ...
        {
            i4_rc = SATLR_AEE_OUT_OF_RESOURCES;
        }
        break;

        default:
        {
            i4_rc = SATLR_AEE_NO_RIGHTS;
        }
        break;
    }
...
{
    return x_satl_add_rec(h_satl, pt_satl_rec);
}
...
{
    return x_satl_del_rec(h_satl, ui2_satl_rec_id);
}
...
{
    return x_satl_update_rec(h_satl, pt_satl_rec, b_must_exist);
}
...
{
    return x_satl_qry_rec_id(h_satl, e_satl_qry_option, pui2_rec_id);
}
...
{
    return x_satl_read_lock(h_satl);
}
...
{
    return x_satl_read_unlock(h_satl);
}
...
)
{
    ...
    switch (i4_rc)
    {
        case AEER_OK:
        {
            i4_rc = x_satl_fs_load(h_dir, ps_satl_path, ui2_satl_id,
...
{
    return x_satl_fs_store(h_dir, ps_satl_path, ui2_satl_id);
}
...
{
    return x_satl_fs_del(h_dir, ps_satl_path, ui2_satl_id);
}
...
{
    return x_satl_fs_qry(h_dir, ps_satl_path, ui2_idx, pui2_satl_id, ps_satl_name);
}
...
{
    return x_satl_fs_sync(h_satl, h_dir, ps_satl_path, ui2_satl_id);
}
...
)
{
    ...
    switch (i4_rc)
    {
        case AEER_OK:
        {
            i4_rc = x_satl_open(ui2_satl_id, pv_tag, pf_nfy, ph_satl);
...
)
{
    ...
    switch (i4_rc)
    {
        case AEER_OK:
        {
            i4_rc = x_satl_open_by_name(ps_satl_name, pv_tag, pf_nfy, ph_satl);
...
{
    return x_satl_get_handle_info(h_satl, pui2_satl_id, ps_satl_name);
}
...
{
    return x_satl_get_rec(h_satl, ui2_satl_rec_id, pt_satl_rec, pui4_ver_id);
}
...
{
    return x_satl_get_num_recs(h_satl, pui2_num_recs, pui4_ver_id);
}

/*----------------------------------------------------------------------
 * Name: c_satl_get_rec_by_rec_idx
 *
 ...
 *              by calling c_satl_get_num_recs.
 *             
 * Inputs: 
 *    h_satl            Handle to the SATL object.
 ...
 *   
 -----------------------------------------------------------------------*/
INT32 c_satl_get_rec_by_rec_idx
(
    HANDLE_T            h_satl,
    UINT16              ui2_idx,
    SATL_REC_T*         pt_satl_rec,
...
{
    return x_satl_get_rec_by_rec_idx(h_satl, ui2_idx,
                                     pt_satl_rec, pui4_ver_id);
}
...
{
    return x_satl_rec_init(pt_satl_rec);
}
...
{
    return x_satl_set_cursor(h_satl, e_pos);
}
...
{
    return x_satl_read_cursor(h_satl, pt_satl_rec, pui4_ver_id);
}
...
{
    return x_satl_sort_rec(h_satl, pui4_ver_id);
}

/*----------------------------------------------------------------------
 * Name: c_satl_init_mem_dump
 *
 * Description: This API initializes the memory dump structure, which 
 *              is used for c_satl_dump_to_mem.
 *
 * Inputs: 
 *   None
 *
 ...
 *   
 -----------------------------------------------------------------------*/
INT32 c_satl_init_mem_dump
(
    SATL_MEM_DUMP_T*       pt_mem_dump
)
{
    return x_satl_init_mem_dump(pt_mem_dump);
}

/*----------------------------------------------------------------------
 * Name: c_satl_free_mem_dump
 *
 ...
 *   
 -----------------------------------------------------------------------*/
INT32 c_satl_free_mem_dump
(
    SATL_MEM_DUMP_T*       pt_mem_dump
)
{
    return x_satl_free_mem_dump(pt_mem_dump);
}

/*----------------------------------------------------------------------
 * Name: c_satl_dump_to_mem
 *
 * Description: This API dumps the specified SATL database into the specified
 *              structure, which could be later used for c_satl_load_from_mem.
 ...
 *   
 -----------------------------------------------------------------------*/
INT32 c_satl_dump_to_mem
(
    HANDLE_T               h_satl,
    SATL_MEM_DUMP_T*       pt_mem_dump
)
{
    return x_satl_dump_to_mem(h_satl, pt_mem_dump);
}

/*----------------------------------------------------------------------
 * Name: c_satl_load_from_mem
 *
 ...
 *   
 -----------------------------------------------------------------------*/
INT32 c_satl_load_from_mem
(
    HANDLE_T               h_satl,
    SATL_MEM_DUMP_T*       pt_mem_dump
)
{
    return x_satl_load_from_mem(h_satl, pt_mem_dump);
}
