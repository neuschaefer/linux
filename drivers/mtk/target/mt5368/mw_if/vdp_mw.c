...
static HANDLE_T _hMutex = NULL_HANDLE;            // Thread safe protection
...
#ifdef CC_3D_MM_DS_SUPPORT        
        fgFrameSeq = _arMwComp3dInfo[_arMwVdp[ucVdpId].u2SrcCompId];
        _arMwVdp[ucVdpId].fgFrameSeq = fgFrameSeq;
        VDP_Set3DInfo(ucVdpId,1,TRUE);
#else
        VDP_Set3DInfo(ucVdpId,0,FALSE);
#endif        
    ...
    switch (e_get_type)
    {
		...
		case VID_PLA_GET_TYPE_OVERSCAN_ENABLE:
		{
			...
			else
			{
				...
				*pfgArg = (BOOL)SV_TRUE;  //mt5368 always support 3d overscan 
			}
		}
		break;
        ...
        // [DTV00023573] Display mode information pass to application for features, Peggy 20080227
...
#ifdef CC_3D_MM_DS_SUPPORT    
    if(_arMwVdp[ucVdpId].fgFrameSeq)
    {
        ucEsId =0;
    }
    else
