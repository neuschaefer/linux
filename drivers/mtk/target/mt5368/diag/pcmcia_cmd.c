...
static INT32 PcmciaCmdSltAll (INT32 i4Argc, const CHAR ** szArgv)
{
    INT32 i4Result;
    UINT32 u4Index, u4Loop = 1;
    
    if(i4Argc > 1)
    {
        u4Loop = StrToInt(szArgv[1]);
    }
    for (u4Index = 0; u4Index < u4Loop; u4Index++)
    {
        LOG(0, "Loop: %d\n", u4Index);

        LOG(0, "SPI register R/W Test ... %d\n");
        PCMCIA_ResetPcmciaHw();

        LOG(0, "    (1-a) Register Default Values Test : \n");
        i4Result = _PcmciaCmdRegDefaultValueTest(); // return TRUE if pass.
        if (!i4Result)
        {
            LOG(0, "CIFAIL:7\n");
            return 0;
        ...
        _PcmciaCmdRwRegTest(); // return TRUE if pass.
        if (!i4Result)
        {
            LOG(0, "CIFAIL:8\n");
            return 0;
        ...
        _PcmciaCmdRegNoMapTest(); // return TRUE if pass.
        if (!i4Result)
        {
            LOG(0, "CIFAIL:9\n");
            return 0;
        ...
        /*i4Result = PcmciaCmdCreateTC(0, NULL);
        if (i4Result != POD_OK)
        {
            LOG(0, "CIFAIL:0\n");
            return 0;
...
static CLI_EXEC_T _arIcVfyCmdTbl[] =
{
    ...
    {"sltall",  "sa",       PcmciaCmdSltAll,        NULL,   "SLT all",                              CLI_SUPERVISOR},    
