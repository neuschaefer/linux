...
static void _vDiRunOverscanPrelenTest(void);
static INT32 _i4DiSetOvsPrlnTest(INT32 i4Argc, const CHAR ** szArgv);
static INT32 _i4DiVerifyPrln(INT32 i4Argc, const CHAR ** szArgv);
static UINT32 u4CaseParam[30][10];
static UINT16 u2CaseNum = 0;
...
#ifdef PSCAN_AUTO_VERIFY
    {"Verify PrlnVal", "verifyprln", _i4DiVerifyPrln, NULL, "Set Prelen & Overscan test", CLI_SUPERVISOR},
...
static INT32 _i4DiHPSPrelenTest(INT32 i4Argc, const CHAR ** szArgv)
{
    ...
    UINT16 u2HpsHigh, u2HpsLow, u2MaxCntPrln;
    UINT16 u2Width, u2WidthPre=0;
    ...
    vDrvDIPrlnSetParam(StrToInt(szArgv[5]), StrToInt(szArgv[6]));
            ...
            if (((u4VideoWidth - u2Width)%u2Step) == 0)
            {
                ...
                //Printf("\nTest Width = %d  ratio = 0x%x  prelen high %d prelen low %d \n", u2Width, u2Cnt, u2PlenHigh, u2PlenLow);
            	  ...
            	  for (u2Prln = u2PrlnLow; u2Prln <= u2PrlnHigh; u2Prln++)
            	  {
                    ...
                    x_thread_delay(50);
                    vDrvDIRstPrelenChkSta();
            	      while (1)
            	      {
            	          E_DI_SC_PRELEN_STATE ePrlnstate = eGetPrlnChkSta();
            	          
            	          x_thread_delay(50);
            	          if (ePrlnstate == E_DI_PRELEN_FAULT)
            	          {
                            //Printf("\nPrelen Fail !!!! Width= %d  prelen value= %d\n", u2Width, u2Prln);
                            if (u4GetPrlnChkCnt() > u4MaxFailCnt)
                            {
                                u4MaxFailCnt = u4GetPrlnChkCnt();
                                u2MaxCntPrln = u2Prln;
                            }
                            Printf(",O");
            	              break;
            	          }
            	          else if (ePrlnstate == E_DI_PRELEN_NORMAL)
            	          {
            	              Printf(",");
            	              break; 
            	          }
...
static INT32 _i4DiSetOvsPrlnTest(INT32 i4Argc, const CHAR ** szArgv)
{
    ...
    if (i4Argc == 2 && (x_strcmp(szArgv[1], "r") == 0))
    {
        u2CaseNum = 0;
    	 return 0;
    }
    else if (i4Argc == 2 && (x_strcmp(szArgv[1], "q") == 0))
    {
        ...
        Printf("PrelenHigh PrelenLow Max-Min_thrld TestCnt StartV EndV  Step TestMode FixedV  FixedCutSide \n");
        for (u2I = 0; u2I < u2CaseNum; u2I++)
        {
            Printf("%10d %9d %13d %7d %6d %4d %4d %8d %6d %12d\n", 
                u4CaseParam[u2I][0], u4CaseParam[u2I][1], u4CaseParam[u2I][2], u4CaseParam[u2I][3], 
                u4CaseParam[u2I][4], u4CaseParam[u2I][5], u4CaseParam[u2I][6], u4CaseParam[u2I][7], 
                u4CaseParam[u2I][8], u4CaseParam[u2I][9]);
        }
    	 return 0;
    }
    else if (i4Argc == 12 && (x_strcmp(szArgv[1], "s") == 0))
    {
        for (u2I = 0; u2I < 10; u2I++)
        {
            u4CaseParam[u2CaseNum][u2I] = StrToInt(szArgv[u2I + 2]);
        }
        u2CaseNum ++;
        return 0;
    }
    ...
    {
        _vDiRunOverscanPrelenTest();
        return 0;
    }

    Printf("n.di.prlnovs go => execute all test cases \n");
    Printf("%s q => query all test cases \n", szArgv[0]);
    Printf("%s r => reset all test cases  remove all \n", szArgv[0]);
    Printf("%s s => set test cases \n", szArgv[0]);
    Printf("%s s PrelenHigh PrelenLow Max-Min_Threshold TestCount StartValue EndValue Step TestMode FixedValue FixedCutSide  | \n", szArgv[0]);
    Printf(" Max Cases: 30 TestMode:  1-top, 2-bottom, 3-left, 4-right, 5-all; 3,4 -> step at least 2 \n");
...
static void _vDiRunOverscanPrelenTest(void)
{
    ...
    UINT8 u1VdpId = SV_VP_MAIN, u1Mode = 0, u1FixedCutSide;
    //UINT16 u2Width, u2WidthPre=0;
    UINT16 u2PrlnHigh, u2PrlnLow, u2Prln, u2MaxCntPrln = 0;
    UINT32 u4MaxFailCnt = 0, u4Step = 100, u4StartV, u4EndV; 
    UINT32 u4VideoWidth = wDrvVideoInputWidth(VDP_1);
    UINT32 u4VideoHeight= wDrvVideoInputHeight(VDP_1);
    UINT32 u4CutValue, u4CutWidthRatio, u4CutHeightRatio, u4FixedCutV, u4I;
    UINT16 u2CaseI;
    

    //pipOffset = getOffset(u1VdpId);  

    for(u2CaseI = 0; u2CaseI < u2CaseNum; u2CaseI++)
    {
        u2PrlnHigh = u4CaseParam[u2CaseI][0];
        u2PrlnLow = u4CaseParam[u2CaseI][1];
        vDrvDIPrlnSetParam(u4CaseParam[u2CaseI][2], u4CaseParam[u2CaseI][3]);

	 u4StartV = u4CaseParam[u2CaseI][4];
	 u4EndV = u4CaseParam[u2CaseI][5];
	 u4Step = u4CaseParam[u2CaseI][6];
	 u1Mode = u4CaseParam[u2CaseI][7];
	 u4FixedCutV = u4CaseParam[u2CaseI][8]; 
        u1FixedCutSide = u4CaseParam[u2CaseI][9]; 
	     
        for (u4I = u4StartV; u4I >= u4EndV; u4I -= u4Step)
        {
	 
            if(u1Mode == 1 || u1Mode == 2 || u1Mode == 5)
            {
                u4CutValue = u4VideoHeight - u4I;
            }
            else if(u1Mode == 3 || u1Mode == 4)
            {
                u4CutValue = u4VideoWidth - u4I;
            }	
            u4CutValue = (u1Mode == 5) ? u4CutValue / 2 : u4CutValue;
            u4CutHeightRatio = (u4CutValue)  * 10000 / u4VideoHeight;
            u4CutWidthRatio = (u4CutValue + 1)  * 10000 / u4VideoWidth;
            ...
            if (u1Mode == 1 || u1Mode==5)
            {
                rXdtaOverscan.u4Top = u4CutHeightRatio;
            }
            else if (u1FixedCutSide == 1)
            {
                rXdtaOverscan.u4Top = 10000 - ((u4FixedCutV) * 10000 / u4VideoHeight);
            }
            
            if (u1Mode == 2 || u1Mode==5)
            {
                rXdtaOverscan.u4Bottom = u4CutHeightRatio;
            }
            else if (u1FixedCutSide == 2)
            {
                rXdtaOverscan.u4Bottom = 10000 - ((u4FixedCutV) * 10000 / u4VideoHeight);
            }
            
            if (u1Mode == 3 || u1Mode==5)
            {
                rXdtaOverscan.u4Left = u4CutWidthRatio;
            }
            else if (u1FixedCutSide == 3)
            {
                rXdtaOverscan.u4Left = 10000 - ((u4FixedCutV) * 10000 / u4VideoWidth);
            }
                 
            if (u1Mode == 4 || u1Mode==5)
            {
                rXdtaOverscan.u4Right = u4CutWidthRatio;
            }
           else if (u1FixedCutSide == 4)
            {
                rXdtaOverscan.u4Right = 10000 - ((u4FixedCutV) * 10000 / u4VideoWidth);
            }
                 
            VDP_SetOverScan(u1VdpId, rXdtaOverscan);
            x_thread_delay(1000);

            Printf("\nHDEW%d(%d),VDEW%d(%d),Failed prelen", 
            	u2DrvDIGetVdpWidth(VDP_1), u4CutWidthRatio, u2DrvDIGetVdpHeight(VDP_1), u4CutHeightRatio);
...
static INT32 _i4DiVerifyPrln(INT32 i4Argc, const CHAR ** szArgv)
{
    UINT32 u4W, u4H, u4MaxFailCnt = 0;
    UINT32 u4WStart, u4WEnd, u4HStart, u4HEnd;
    UINT32 u4WStep, u4HStep;
    UINT32 u4VideoWidth = wDrvVideoInputWidth(VDP_1);
    UINT32 u4VideoHeight= wDrvVideoInputHeight(VDP_1);
    UINT32 u4WCut, u4HCut;
    UINT32 u4CutWidthRatio, u4CutHeightRatio;
    VDP_OVERSCAN_REGION_T rXdtaOverscan;
    UINT16 u2Prln;
    //UINT32 u4LCut = 0, u4RCut = 0, u4TCut = 0, u4BCut = 0;
    
    if (i4Argc < 8)
    {
    	Printf("Usage: %s WidthStart WidthEnd WidthStep HeightStart HeightEnd HeightStep CheckCnt \n", szArgv[0]);
    	return 1;
    }

    u4WStart = StrToInt(szArgv[1]);
    u4WEnd = StrToInt(szArgv[2]);
    u4WStep = StrToInt(szArgv[3]);
    u4HStart = StrToInt(szArgv[4]);
    u4HEnd = StrToInt(szArgv[5]);
    u4HStep = StrToInt(szArgv[6]);
    vDrvDIPrlnSetParam(7, StrToInt(szArgv[7]));

    for (u4W = u4WStart; u4W <= u4WEnd; u4W += u4WStep)
    {
        u4WCut = u4VideoWidth - u4W;
        u4CutWidthRatio = (u4WCut + 1)  * 10000 / u4VideoWidth;
        
        for (u4H = u4HStart; u4H <= u4HEnd; u4H += u4HStep)
        {
            u4HCut = u4VideoHeight - u4H;
            u4CutHeightRatio = (u4HCut)  * 10000 / u4VideoHeight;     
            ...
            x_thread_delay(250);
            u2Prln = u2GetMainPrelenValue(VDP_1);
            vScpipSetPscanPrelen(VDP_1, u2Prln);
            x_thread_delay(50);
                ...
                vDrvDIRstPrelenChkSta();
            	  ...
            	  while (1)
            	  {
            	      ...
            	      if (ePrlnstate == E_DI_PRELEN_FAULT)
            	      {
                        ...
                        Printf("\n Failed!!  ");
                        Printf("HDEW%d(%d),VDEW%d(%d)  ", 
            	              u2DrvDIGetVdpWidth(VDP_1), u4CutWidthRatio, u2DrvDIGetVdpHeight(VDP_1), u4CutHeightRatio);
                        Printf("FaliedCnt %d Prelen %d  ", u4GetPrlnChkCnt(), u2GetMainPrelenValue(VDP_1));
            	          break;
            	      }
            	      else if (ePrlnstate == E_DI_PRELEN_NORMAL)
            	      {
            	          /*Printf("HDEW%d(%d),VDEW%d(%d)  ", 
            	              u2DrvDIGetVdpWidth(VDP_1), u4CutWidthRatio, u2DrvDIGetVdpHeight(VDP_1), u4CutHeightRatio);
                        Printf("Prelen %d  ", u2GetMainPrelenValue(VDP_1));*/
            	          break; 
            	      }
