...
//-----------------------------------------------------------------------------
#include "loader_if.h"
#if defined(CC_MT5395) || defined(CC_MT5368)
...
//for use extender gpio control backlight issue while press reset
    #if defined(CC_MT5365) || defined(CC_MT5368)
    // turn off backlight GPIO with QFP (controlled by extender)
    if (DRVCUST_InitGet(eFlagDDRQfp))
    {                  
        UINT32 u4Val = (DRVCUST_PanelGet(eBackLightOnVal)==1)? 0 : 1;
...
static void GPIO_MtkPowerOnFunc()
{
...
#ifdef CC_MTK_LOADER
    #ifdef CC_MT5368
    //Audio AD82581 amp PD# change in loader
    //A_MUTE pin
    if (DRVCUST_InitQuery(eAudioMuteGpioNum, &i) == 0)
    {
        GPIO_SetOut(i, 0);
        HAL_Delay_us(5000); //delay 5ms
        GPIO_SetOut(i, 1);
        Printf("\nA_Mute GPIO=%x release\n",i);
    }
    //AMP mute GPIO
    if (DRVCUST_InitQuery(eAudioAmpEnableGpioNum, &i) == 0)
    {
        GPIO_SetOut(i, 1);
        HAL_Delay_us(5000); //delay 5ms
        GPIO_SetOut(i, 0);
        Printf("AMP mute GPIO=%x release\n",i);
    }
    #endif
    ...
    // Set PAD_OSCL2 as low for ethernet link down issue
    // ES1 IC will resolve this issue
    #if defined(CC_MT5368)
    if(DRVCUST_InitGet(eFlagDDRQfp))
        GPIO_SetOut(GPIO(17), 0);
    #endif
}
