...
static UINT8 pu1CurCisMap[MAX_CIS_MAP_SIZE];
...
static INT32 _PODCI_CheckCis(void)
{
    ...
    printf("_PODCI_CheckCis: PCMCIA_Reset\n");
    x_thread_delay(200);
    PCMCIA_Reset(1);
    x_thread_delay(10);
    PCMCIA_Reset(0);
    ...
    while( (PCMCIA_TestCardReady()!=1) )
    {
        ...
        {
            printf("Ready pin is low... (card not reset?) \n");
            return -2;
        }
        x_thread_delay(5);
    }

    printf("_PODCI_CheckCis: PCMCIA_PcmciaToPod(FALSE) \n");
    PCMCIA_PcmciaToPod( FALSE );
    x_memset((void*)pu1CurCisMap, 0, MAX_CIS_MAP_SIZE);
    printf("_PODCI_CheckCis: PCMCIA_ReadCor\n");
...
INT32 PODCI_LowLevelInit(void)
{
    ...
    LOG(1,"PODCI_LowLevelInit() begin\n");
    
    LOG(1,"_PODCI_CheckCis() begin\n");
    _PODCI_CheckCis();
    LOG(1,"_PODCI_CheckCis() end\n");

    LOG(1,"PCMCIA_PcmciaToPod(FALSE) begin\n");
    PCMCIA_PcmciaToPod( FALSE );
    x_thread_delay(100);
    LOG(1,"PCMCIA_PcmciaToPod(FALSE) end\n");
    
    LOG(1,"PCMCIA_WriteCor(0x%x, 0x%x) begin\n", u2addrcor, u1cor);
    PCMCIA_WriteCor(u2addrcor, u1cor);
    LOG(1,"PCMCIA_WriteCor(0x%x, 0x%x) end\n", u2addrcor, u1cor);

    LOG(1,"PODCI_SetPodCiAccessMode(POD_DATA_CHANNEL) begin\n");
    PODCI_SetPodCiAccessMode(POD_DATA_CHANNEL);
    PODCI_ResetChannel();
    x_thread_delay(100);
    LOG(1,"PODCI_ResetChannel() end\n");
    
    VERIFY(PODCI_SetPodCiAccessMode(POD_DATA_CHANNEL) == POD_OK);
    LOG(1,"PODCI_NegotiateBufSize() begin\n");
    i4Result = PODCI_NegotiateBufSize(&u2NegoBufSize);
    LOG(1,"PODCI_NegotiateBufSize() end\n");
    if (i4Result == FALSE)
    {
    LOG(0, "PODCI_LowLevelInit, PODCI_NegotiateBufSize fail\n");
    }
    
    LOG(1,"PODCI_LowLevelInit() end\n");
    return i4Result;
}
