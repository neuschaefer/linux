...
BOOL PCMCIA_SltAll()
{
    ...
    /*LOG(0, "PCMCIA_ResetCam()\n");
    PCMCIA_ResetCam();
    LOG(0, "PCMCIA_ResetPcmciaHw()\n");
    PCMCIA_ResetPcmciaHw();*/
    LOG(0, "PCMCIA_Init\n");
    PCMCIA_Init();

    CKGEN_WRITE32(0x40c, (CKGEN_READ32(0x40c) & 0xffffffe3) | 0x4); 
    CKGEN_WRITE32(0x220, 1); 
    
    u1RegVal = 0;
    SPI_DemodSingleWrite(&u1RegVal, 0x881);
    SPI_DemodSingleWrite(&u1RegVal, 0x882);
    SPI_DemodSingleWrite(&u1RegVal, 0x883);
    SPI_DemodSingleWrite(&u1RegVal, 0x884);
    SPI_DemodSingleWrite(&u1RegVal, 0x885);
    SPI_DemodSingleWrite(&u1RegVal, 0x886);
    SPI_DemodSingleWrite(&u1RegVal, 0x887);
    SPI_DemodSingleWrite(&u1RegVal, 0x888);
    
    // CI card plug in for all test case 
    // 1. serial in / serial out
    PCMCIAHW_WRITE32(REG_CTRL, PCMCIAHW_READ32(REG_CTRL) & 0xfff3ffff | 0x000c0000);
    PCMCIAHW_WRITE32(REG_POD_GPR_CNTL_SET0, 0x80000034);
    PCMCIAHW_WRITE32(REG_POD_GPR_CNTL_SET3, 0);
    u1RegVal = 0xc0;
    SPI_DemodSingleWrite(&u1RegVal, 0xdd2);
    u1RegVal = 0x3;
    SPI_DemodSingleWrite(&u1RegVal, 0xa85);
    u1RegVal = 0x24;
    SPI_DemodSingleWrite(&u1RegVal, 0xa84);
    SPI_DemodSingleRead(&u1RegVal, 0xe00);
    u1RegVal |= 0x12;
    SPI_DemodSingleWrite(&u1RegVal, 0xe00);
    u1RegVal = 0x24;
    SPI_DemodSingleWrite(&u1RegVal, 0xa80);
    
    u1RegVal = 0xb;
    SPI_DemodSingleWrite(&u1RegVal, 0xe5f);
    u1RegVal = 0x1;
    SPI_DemodSingleWrite(&u1RegVal, 0xe60);
    u1RegVal = 0xb;
    SPI_DemodSingleWrite(&u1RegVal, 0xe61);
    u1RegVal = 0x1;
    SPI_DemodSingleWrite(&u1RegVal, 0xe62);

    LOG(0, "p.v.slt 0 27 1 -- bypass card \n");
    if (FALSE == PCMCIA_SltStart(0, 27, 1))
    {
        LOG(0, "CIFail:%d\n", u4SltIndex);
        return FALSE;
    }
    ...
    PCMCIAHW_WRITE32(REG_POD_GPR_CNTL_SET0, 0x80000034);
    LOG(0, "p.v.slt 1 27 1 -- bypass card \n");
    if (FALSE == PCMCIA_SltStart(1, 27, 1))
    {
        LOG(0, "CIFail:%d\n", u4SltIndex);
        return FALSE;
    }
    ...
    u4SltIndex++;
/*
    PCMCIAHW_WRITE32(REG_POD_GPR_CNTL_SET0, 0x34);
    u1RegVal = 0x0;
    SPI_DemodSingleWrite(&u1RegVal, 0xa80);
    x_thread_delay(100);
    LOG(0, "p.v.slt 0 27 1 -- go through card \n");
    if (FALSE == PCMCIA_SltStart(0, 27, 1))
    {
        LOG(0, "CIFail:%d\n", u4SltIndex);
        return FALSE;
    ...
    u1RegVal = 0x0;
    SPI_DemodSingleWrite(&u1RegVal, 0xa80);
    LOG(0, "p.v.slt 1 27 1 -- go through card \n");
    if (FALSE == PCMCIA_SltStart(1, 27, 1))
    {
        LOG(0, "CIFail:%d\n", u4SltIndex);
        return FALSE;
...
*/    
    ...
    PCMCIAHW_WRITE32(REG_POD_GPR_CNTL_SET0, 0xC0000034);
    PCMCIAHW_WRITE32(REG_POD_GPR_CNTL_SET3, 0x20000);
    x_thread_delay(100);
    u1RegVal = 0xfb;
    SPI_DemodSingleWrite(&u1RegVal, 0xdd2);
    u1RegVal = 0x80;
    SPI_DemodSingleWrite(&u1RegVal, 0xdd3);
    u1RegVal = 0x24;
    SPI_DemodSingleWrite(&u1RegVal, 0xdd0);
    u1RegVal = 0x24;
    SPI_DemodSingleWrite(&u1RegVal, 0xdd4);
    LOG(0, "p.v.slt 0 27 2\n");
    if (FALSE == PCMCIA_SltStart(0, 27, 2))
    {
        LOG(0, "CIFail:%d\n", u4SltIndex);
        return FALSE;
    }
    x_thread_delay(100);
    u4SltIndex++;
    LOG(0, "p.v.slt 1 27 2\n");
    if (FALSE == PCMCIA_SltStart(1, 27, 2))
    {
        LOG(0, "CIFail:%d\n", u4SltIndex);
        return FALSE;
    }
...
BOOL PCMCIA_SltStart(UINT8 u1TsIndex, UINT8 u1TsClkRate, UINT8 u1TsDataWidth)
{
#ifdef __MODEL_slt__
    UINT32 u4Reg;    
...
//        PCMCIAHW_WRITE32(REG_POD_GPR_CNTL_SET3, PCMCIAHW_READ32(REG_POD_GPR_CNTL_SET3) | 0xC000);
    ...
    {
        case 1: 
            eTsDataWidth = DMX_TSOUT_DATAWIDTH_1;
            break;
        case 2: 
            eTsDataWidth = DMX_TSOUT_DATAWIDTH_2;
            break;
        case 8: 
            eTsDataWidth = DMX_TSOUT_DATAWIDTH_8;
            break;
        default: 
