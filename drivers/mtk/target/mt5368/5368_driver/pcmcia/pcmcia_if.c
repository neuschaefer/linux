...
//-----------------------------------------------------------------------------
static void _PCMCIA_AutoSetTsRoutingPath_5368(void)
{
    ...
    UNUSED(_PCMCIA_AutoSetTsRoutingPath_5368);
    ...
    if (fgExternalDemod)    // External CI
    {
        ...
        if (eExternalIcVersion == MT8295_VERSION_BNE) 
        {
            if (_rTsFmt.SerOrPar == SP_SERIAL)
            {
                ...
                DMX_SetFrontEndEx(0, DMX_FE_EXT_1_S);
            }
...
static void _PCMCIA_AutoSetTsRoutingPath(void)
{
    ...
    _PCMCIA_AutoSetTsRoutingPath_5368();
...
BOOL PCMCIA_SLTAll()
{
    LOG(0, "PCMCIA SLT All Start\n");
    return (PCMCIA_SltAll());
}
...
BOOL    PCMCIA_SLTAll(){return TRUE;}
...
#if defined(CC_EXTERNAL_IC_MT5135) || defined(PCMCIA_ENABLED)
