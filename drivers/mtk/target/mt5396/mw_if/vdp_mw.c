...
static VOID set_vdp_ex_nfy(UINT32                               ucVdpId,
                           ...
                           VID_PLA_VID_REGION_T*                pt_updated_disp_region)    /* OUT */
#else
                           VID_PLA_VID_REGION_T*                pt_disp_region)    /* OUT */
#endif
{
...
#ifdef SYS_OVERRIDE_DISP_REGION_SUPPORT
    dst.tOut.t_disp_reg_override = *pt_updated_disp_region;
#endif
    #else
...
#ifdef SYS_OVERRIDE_DISP_REGION_SUPPORT
    *pt_updated_disp_region = dst.tOut.t_disp_reg_override;
#endif
    #else    
...
static VOID vid_pla_source_region_fct (
            UINT16                        ui2_vdp_id,
            UINT32                              u4X,      
            UINT32                              u4Y,    
            UINT32                              u4Width,    
...
)
{
    ...
    UINT32 addr;
    #endif
    PARMA_GET_CUR_SRC_REGION dst;
    
    dst.ui2_vdp_id = ui2_vdp_id;
    dst.ui4X = u4X;
    dst.ui4Y = u4Y;
    dst.ui4Width = u4Width;
    dst.ui4Height = u4Height;
    dst.pf_nfy = _pfVideoSrcRegionFct;

    #if 1
    _CB_PutEvent_Adpt(CB_DRV_VDP_GET_CUR_SRC_REGION, sizeof(PARMA_GET_CUR_SRC_REGION), &dst);  
    #else    
    lock_nfy_param_buf(PARMA_GET_CUR_SRC_REGION);
    pv_dst = (PARMA_GET_CUR_SRC_REGION *)get_nfy_param_buf(PARMA_GET_CUR_SRC_REGION_NFY, sizeof(PARMA_GET_CUR_SRC_REGION));
    rm_copy_to_user(pv_dst,&dst,sizeof(PARMA_GET_CUR_SRC_REGION));
    addr = pv_dst;
    _CB_PutEvent_Adpt(CB_DRV_VDP_GET_CUR_SRC_REGION, sizeof(UINT32), &addr);
    unlock_nfy_param_buf(PARMA_GET_CUR_SRC_REGION_NFY);
    #endif
}
static VOID vid_pla_output_region_fct (
            UINT16                        ui2_vdp_id,
            UINT32                              u4X,      
            UINT32                              u4Y,    
            UINT32                              u4Width,    
...
)
{
    ...
    dst.ui4Width = u4Width;
    dst.ui4Height = u4Height;
    dst.pf_nfy = _pfVideoOutRegionFct;

    #if 1
    _CB_PutEvent_Adpt(CB_DRV_VDP_GET_CUR_OUT_REGION, sizeof(PARMA_GET_CUR_OUT_REGION), &dst);  
    #else    
    lock_nfy_param_buf(PARMA_GET_CUR_OUT_REGION);
    pv_dst = (PARMA_GET_CUR_OUT_REGION *)get_nfy_param_buf(PARMA_GET_CUR_OUT_REGION_NFY, sizeof(PARMA_GET_CUR_OUT_REGION));
    rm_copy_to_user(pv_dst,&dst,sizeof(PARMA_GET_CUR_OUT_REGION));
    addr = pv_dst;
    _CB_PutEvent_Adpt(CB_DRV_VDP_GET_CUR_OUT_REGION, sizeof(UINT32), &addr);
    unlock_nfy_param_buf(PARMA_GET_CUR_OUT_REGION_NFY);
    #endif
}
        ...
        //case VID_PLA_GET_TYPE_3D_NAV:           
            {
                ...
                else
                {
                    ...
                    *pu4Arg = (UINT32)eDrvTDTVTDNavimModeQuery();
                }                
                break; 
            }
        ...
        //case VID_PLA_GET_TYPE_3D_NAV:           
            {
                ...
                else
                {
                    ...
                    *pu4Arg = (UINT32)eDrvTDTVTDImgSafetyLvlQuery();
                }                
                break;             
            }
                    ...
                    *pz_get_info_len = sizeof(VID_PLA_VID_REGION_CAPABILITY_T);
                    ...
                    switch (prArg->e_get_type)
                    {
                        ...
                        case VID_PLA_GET_TYPE_SRC_REGION:
                            ...
                            {
                                //if(VDP_GetDispmode(ucVdpId) == VDP_SCPOS_LINESYNC_DISPMODE)    //line sync
                                {
                                    #if defined(SUPPORT_TV3D_OVERSCAN)
                                    if(vScpipIs3DDoneByMJC() == SV_TRUE)    //3d & overscan enabled
                                    {
                                        //For zoom limitation.
                                        prArg->ui4_height_max = VDP_MAX_REGION_HEIGHT;
                                    ...
                                    {
                                        prArg->ui4_height_max = VDP_MAX_REGION_HEIGHT;
                                        prArg->ui4_height_min = VDP_GetMinSrcRegionHeight();
                                    }
                                }
                                /*
                                                        else   //other mode
...
LINT_SUPPRESS_BRACE(507)
        ...
        //case VID_PLA_SET_TYPE_3D_NAV:
        {
            E_TDTV_UI_NAVI uNaviUI = (E_TDTV_UI_NAVI) (UINT32)pv_set_info; 
            vDrvTDTVTDNAVI(uNaviUI);
            break;
        }
...
//            #endif
        ...
        case VID_PLA_SET_TYPE_COLORSPACE:
        {
            switch((UINT32)pv_set_info)
            {
                ...
                default:
                    LOG(2, "Valid color space param for VGA mode \n");
                break;
            }
        }
        break;
...
static TV_DEC_TIMING_TYPE_T  _bVDPGetTimingType(UINT8 u1Path)
{
    ...
    {
        e_timing_type = ( (fgApiVideoIsVgaModeEx( u1Path, 0))?
   ...
   if ((_pfVideoSrcRegionFct != NULL) && fgEnableMwCb)
    {
        ...
        LOG(0,"in _VdpDriverSrcRegionChgNotify and (u4X,u4Y,u4Width,u4Height)(%d,%d,%d,%d)\n",u4X,u4Y,u4Width,u4Height);
        #ifndef LINUX_TURNKEY_SOLUTION
   ...
   if ((_pfVideoOutRegionFct != NULL) && fgEnableMwCb)
    {
        ...
        LOG(0,"in _VdpDriverOutputRegionChgNotify and (u4X ,u4Y,u4Width,u4Height)(%d,%d,%d,%d)\n",u4X,u4Y,u4Width,u4Height);
        #ifndef LINUX_TURNKEY_SOLUTION
...
static UINT32 _VdpXlatDispFmtCallBackMw(UCHAR ucVdpId)
{
    ...
    if ((_pfXlatDispFmtExFct[(UINT32)_arMwVdp[ucVdpId].eFmt] != NULL) && fgEnableMwCb)
    {
		...
		if (_arMwVdp[ucVdpId].ucDtv != 0)
		{
            ...
            rVidSrcInfo.u.t_mpeg.b_is_cropping = rHdrInfo.fgCropping;
            rVidSrcInfo.u.t_mpeg.u4_cropping_x = rHdrInfo.u4CropX;
            rVidSrcInfo.u.t_mpeg.u4_cropping_y = rHdrInfo.u4CropY;
            rVidSrcInfo.u.t_mpeg.u4_cropping_width = rHdrInfo.u4CropWidth;
            rVidSrcInfo.u.t_mpeg.u4_cropping_height = rHdrInfo.u4CropHeight;
            ...
            {
            	rVidSrcInfo.u.t_mpeg.b_is_display_aspect_ratio = TRUE;
            }
...
static void _VdpCalculateOutputRegion(UCHAR ucVdpId)
{
    ...
    {
        // X
        _arMwVdp[ucVdpId].rFinalOutput.ui4_x = _arMwVdp[ucVdpId].rTrueFullOutput.ui4_x + ((_arMwVdp[ucVdpId].rXlatOutput.ui4_x * _arMwVdp[ucVdpId].rTrueFullOutput.ui4_width) / VDP_MAX_REGION_WIDTH);

        // Y
        _arMwVdp[ucVdpId].rFinalOutput.ui4_y = _arMwVdp[ucVdpId].rTrueFullOutput.ui4_y + ((_arMwVdp[ucVdpId].rXlatOutput.ui4_y * _arMwVdp[ucVdpId].rTrueFullOutput.ui4_height) / VDP_MAX_REGION_HEIGHT);

        // Width
        _arMwVdp[ucVdpId].rFinalOutput.ui4_width = (_arMwVdp[ucVdpId].rXlatOutput.ui4_width * _arMwVdp[ucVdpId].rTrueFullOutput.ui4_width) / VDP_MAX_REGION_WIDTH;

        // Height
        _arMwVdp[ucVdpId].rFinalOutput.ui4_height = (_arMwVdp[ucVdpId].rXlatOutput.ui4_height * _arMwVdp[ucVdpId].rTrueFullOutput.ui4_height) / VDP_MAX_REGION_HEIGHT;    
    }
#endif
}
...
//    VERIFY(x_sema_create(&_hMutex, X_SEMA_TYPE_MUTEX, X_SEMA_STATE_UNLOCK) == OSR_OK);
    ...
    VDP_RegCbFunc(VDP_CB_FUNC_DRIVER_SRC_RANGE_CHG_IND, (UINT32)_VdpDriverSrcRegionChgNotify, 0, 0);
    VDP_RegCbFunc(VDP_CB_FUNC_DRIVER_OUT_RANGE_CHG_IND, (UINT32)_VdpDriverOutputRegionChgNotify, 0, 0);
...
void MW_VDP_CallbackTrigger(UCHAR ucVdpId)
{
    ...
    if (_arMwVdp[ucVdpId].ucSrcChgNfy)
    {
        ...
        {
            VERIFY(x_sema_unlock(_hMutex) == OSR_OK);
			PrintfOverscan("MW_VDP_CallbackTrigger overscan  %d %d %d %d\n", rOverScan.u4Top, rOverScan.u4Bottom, rOverScan.u4Left,rOverScan.u4Right);
