...
UINT8 fgZoomMJCModeChg = SV_FALSE;
...
void vDrvMJCSetSystemMode(UINT32 u4MjcSystemMode)
{
    ...
    if (u1Dram444)
    {
        if((IO32ReadFldAlign(MJC_PR_MODE, MJC_3DMODE_IN) == E_VDO_MJC_2D) && (arMJCPrm.u1Dram422RPT == SV_OFF))
        {
            // 444->422 Off
...
void vDrvMJCSet32PdInfoShiftProc(UINT8 fgIsInterlace, UINT8 fgLineSyncMode)
{
	...
	u4DiDone = ((!fgLineSyncMode) && ((arMJCPrm.arFRCConfig.u13DMode == E_VDO_MJC_SBS) || (arMJCPrm.arFRCConfig.u13DMode == E_VDO_MJC_TAB)));
    ...
    else
    {
        ...
        else
        {
            ...
            MJC_WRITE_FLD(MJC_SYS_00, u4PdInfo32Shift, MJC_PD_INFO_32_SHIFT);
            vDrvMJCFrmSchSetPdInfoDly(fgLineSyncMode ? 1 : 0);
        }
    ...
    vIO32WriteFldAlign(MJC_FW_SCH_01, u4DiDone, MJC_FW_FRM_SCH_FD_IN_PS);
}
...
void vDrvMJCSetFrmSch(MJC_FRC_CONFIG_T * arMJCFRC)
{
    ...
    vIO32WriteFldAlign(MJC_FW_SCH_03, (arMJCFRC->u13DMode == E_VDO_MJC_TTD), MJC_50TO120_TTD);
    vIO32WriteFldAlign(MJC_FW_SCH_03,
        ((fgProgFrmRateEn && arMJCFRC->fgIs50To120 && (arMJCFRC->u13DMode || arMJCFRC->u1PanelType == E_MJC_60HZ)) ||
         (!fgProgFrmRateEn && arMJCFRC->fgIs50To120 && ((arMJCFRC->u13DMode == E_VDO_MJC_2D && arMJCFRC->u1PanelType == E_MJC_120HZ) || arMJCFRC->u13DMode == E_VDO_MJC_TTD))) ? 1 : 0,
        MJC_50TO120_DOUBLE);
    vIO32WriteFldAlign(MJC_FW_SCH_03, (fgProgFrmRateEn && arMJCFRC->fgIs50To120 && ((arMJCFRC->u13DMode == E_VDO_MJC_2D && arMJCFRC->u1PanelType == E_MJC_120HZ) || arMJCFRC->u13DMode == E_VDO_MJC_TTD)), MJC_50TO120_TTD_25P);
    vIO32WriteFldMulti(MJC_FW_SCH_01,
        P_Fld(arMJCFRC->fgIs50To120 ? 1 : (MJC_SUPPORT_MULTI_CADENCE&&(arMJCFRC->u1InFrmRate == 60)&&(arMJCFRC->u13DMode == E_VDO_MJC_2D)? 2 : 0) /* if muticadence, set 2*/, MJC_FW_FRM_SCH_SWITCH)|
        ...
        P_Fld(((arMJCFRC->u1InFrmRate == 60)&&(arMJCFRC->u13DMode == E_VDO_MJC_2D)&&(arMJCFRC->u1PanelType == E_MJC_120HZ)? 1 : 0), MJC_FW_FRM_SCH_TST_DLY));
...
void vDrvMJCSetPRCtrl(MJC_FRC_CONFIG_T* arMJCFRC , MJC_PIC_SIZE_T * arMJCPicSize)
{
    ...
    if(MJC_READ_FLD(MJC_SYS_PR_0, MJC_PR_SYNC_REGEN_BYPASS) == SV_OFF)
    {
        ...
        else
        {
			...
			MJC_WRITE_FLD(MJC_SYS_PR_1 , SV_ON, MJC_PR_FOR_VSYNC_MJC_TWO_PULSE);
			MJC_WRITE_FLD(MJC_SYS_PR_1 , ((arMJCPicSize->u2OutVTotal / 2) - (2 *  ALIGN_8_CEIL(PANEL_GetPanelHeight()/2)/ arMJCPicSize->u2InHeight)),
				          MJC_PR_SYNC_REGEN_MJC_VSYNC_SEL);
        }
        MJC_WRITE_FLD(MJC_PR_03, (arMJCFRC->u13DMode == E_VDO_MJC_2D), MJC_PR_SG_LR);
        ...
        {
            MJC_WRITE_FLD(MJC_PR_04, ((arMJCPrm.arPicSize.fgUdp4 == SV_ON) && (fgDrvMJCGetFlipMirror() > 1))?
                                       SV_ON : SV_OFF, MJC_PR_HPT_TAB_FLIP_ECO);
			MJC_WRITE_FLD(MJC_PR_00 , (((MJC_READ_FLD(MJC_PR_04 , MJC_PR_HPT_TAB_FLIP_ECO) == SV_ON) ||
		                      (LVDS_DISP_3D == LVDS_DISP_3D_POLARIZED_RLRL))? 1 : 0), MJC_PR_LR_SWITCH);
            MJC_WRITE_FLD(MJC_PR_03 , ((MJC_READ_FLD(MJC_SYS_DB, MJC_TTD_BYPASS) == 1)? 1 : 0), MJC_PR_IN_LR_INV);
        }
    }
    else
    {
        ...
        else
        {
			...
			MJC_WRITE_FLD(MJC_SYS_PR_1 , SV_OFF, MJC_PR_FOR_VSYNC_MJC_TWO_PULSE);
			MJC_WRITE_FLD(MJC_SYS_PR_1 , 0, MJC_PR_SYNC_REGEN_MJC_VSYNC_SEL);
        }
    }
...
void vDrvMJCSet3DOverScan(const MJC_PIC_SIZE_T * arMjcPicSize)
{
    ...
    {
        MJC_WRITE_FLD(MJC_SYS_01, arMjcPicSize->rClipInfo.wTop >> 1, MJC_IN_V_START);
...
{    
    ...
    arMJCFRC->u1InFrmRate = u2DrvGetInternalFrameRate();
    #else
    arMJCFRC->u1InFrmRate = bDrvVideoGetRefreshRate(SV_VP_MAIN);
    #endif

    arMJCFRC->fgIs50To120 = ((arMJCFRC->u1InFrmRate % 25 == 0) &&
        ((arMJCFRC->u13DMode != E_VDO_MJC_2D && (u1DrvMJCIsSupport50To120()&MJC_SUPPORT_50TO120_3D)) ||
         (arMJCFRC->u13DMode == E_VDO_MJC_2D && (u1DrvMJCIsSupport50To120()&MJC_SUPPORT_50TO120_2D))));
...
static BOOL fgDrvMJCGetPicSize(MJC_PIC_SIZE_T * arPicSize)
{
    ...
    if (arPicSize->rClipInfo.wTargetHeight != 0)
    {
        ...
        #endif
        arPicSize->u2InHeight  = ((u1PRMode == E_MJC_HPR) && (arMJCPrm.arFRCConfig.u13DMode != E_VDO_MJC_2D) && (IO32ReadFldAlign(MJC_PR_MODE, MJC_3DMODE_OUT) != E_VD0_MJC_OUT_NoPR)) || (arMJCPrm.arFRCConfig.u1FHPRMode == E_MJC_HPR2160) ?
...
UINT32 u4DrvMJCBufferInit(void)
{
    ...
    {
        u4MJCBaseAddress = (BSP_GetIcVersion() != IC_VER_5396_AA) ? (prPool->u4Addr+0x40) : (prPool->u4Addr);
...
void vDrvMJCSrmCb(UINT32 u4VdpId, UINT32 u4Mode, UINT32 u4PDSize, UINT32 u4SramMode)
{
    ...
    if ((arMJCDramAlloc.u2DebugMode != 0xFFFF) || (arMJCDramAlloc.u2Mode != u4DrvMJCRemapMJCFbmMode(u4Mode)) || 
        ((u1MJC3DModeIn != arMJCPrm.arFRCConfig.u13DMode) && (arMJCPrm.u1Dram444)) || (bIsScalerInput444(SV_VP_MAIN) != u1PreScaler444))
    {
        u1PreScaler444 = bIsScalerInput444(SV_VP_MAIN);
...
void vDrvMJCMainloop(void)
{
	...
	fgMJCIs3DFSMode = (IO32ReadFldAlign(MJC_PR_MODE, MJC_3DMODE_IN) == E_VDO_MJC_FS);
	
	if ((fgMJCModeChg) || (IO32ReadFldAlign(MJC_MISC_00, MJC_MODE_CHG_TRIG_AGAIN) == SV_ON))
    {
        if (fgDrvMJCModeChgProc(SV_VP_MAIN))
        {
		   u1ModeChangeRst = (!SRM_IsPipVideo() && !SRM_IsPopVideo() && (fgZoomMJCModeChg != SV_TRUE))? 3 : 0;
		   fgZoomMJCModeChg = SV_FALSE;
	...
	if ((arMJCPrm.u1Dram444 == SV_OFF) && 
		 (((arMJCPrm.arFRCConfig.u13DMode == E_VDO_MJC_TAB) && ((MJC_READ_FLD(MJC_STA_SYS_01, MJC_STA_V_TOTAL) >> 2) > arMJCPrm.arPicSize.u2InHeight)) || 
		  ((arMJCPrm.arFRCConfig.u13DMode == E_VDO_MJC_SBS) && ((MJC_READ_FLD(MJC_STA_SYS_01, MJC_STA_V_TOTAL) >> 1) > arMJCPrm.arPicSize.u2InHeight))))
    {
        
		MJC_WRITE_FLD_MULTI(MJC_FB_09, P_Fld(1, MJC_MID_VSYNC_IN_CNT) |  P_Fld(SV_ON, MJC_MID_VSYNC_IN_PROG_EN));	 
	}
    else 
	#endif
    {
        MJC_WRITE_FLD_MULTI(MJC_FB_09, P_Fld(0, MJC_MID_VSYNC_IN_CNT) |  P_Fld(SV_OFF, MJC_MID_VSYNC_IN_PROG_EN));   
    }
