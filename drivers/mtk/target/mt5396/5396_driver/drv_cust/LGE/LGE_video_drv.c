...
{
    // Set YCbCr 709 for HD input source
    vIO32WriteFldAlign(MATRIX_00, 0x1, MATRIX_HD_709);
...
{
    vDrvLcdimSpiCommandGen(E_SPI_PAGE_WRITE);
    return;
}
...
{    
    ...
    {
        vSetHDMIRangeMode(SV_HDMI_RANGE_FORCE_LIMIT);
        SET_MATRIX_PED(!bLevel);    // BLK_LVL=1=no ped,  BLK_LVL=0=ped
    }
    else
    {
        SET_MATRIX_PED(SV_OFF);
    ...
    switch (bColorSys)
    {
        ...
        case SV_COLOR_HDMI_444_709:
            ...
            else
            {
                ...
                {
                    bMonColorTransform = bIsVideoTiming ? 
                        SV_COLORTRANS_YCBCR601L_2_YCBCR601L : SV_COLORTRANS_YCBCR601L_2_YCBCR601L;
                }
            }
            break;
        case SV_COLOR_HDMI_RGB:
            ...
            {
                bMonColorTransform = bIsVideoTiming ? 
                    (bHdtv ? SV_COLORTRANS_RGBL_2_YCBCR709L : SV_COLORTRANS_RGBL_2_YCBCR601L) :
                    (bHdtv ? SV_COLORTRANS_RGBF_2_YCBCR709L : SV_COLORTRANS_RGBF_2_YCBCR601L);
            }
            break;
        case SV_COLOR_RGB:
        default:
