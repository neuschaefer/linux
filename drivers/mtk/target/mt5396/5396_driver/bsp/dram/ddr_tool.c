...
#define GATING_CAL_CTLX400    (0x46)
#define GATING_CAL_CTLX800    (0x10)
#endif
...
#ifdef DRAM_GATING_SCAN
#ifdef CC_SUPPORT_DDR3
UINT32 _DDR_DeltaCalibrate(void)
{
    INT32 i4PLL, i4MCK;
    UINT32 u4Val;
    UINT16 u2PllJitter;
    UINT16 u2MckJitter;
    UINT32 u4LowBound = 0, u4HighBound = 95;
    UINT32 u4Delta;

    Printf("Jitter checking...\n");  

    // check PLLCK first.
    for (i4PLL=0xA; i4PLL >= 0; i4PLL--)
    {       
        u4Val = 0x80000FFF;
        u4Val |= (UINT32)(i4PLL << 24);
        IO_WRITE32(DRAM_BASE,0x228, u4Val);
        IO_WRITE32(DRAM_BASE,0x228, u4Val | 0x00800000);

        CHIP_Delay_us(10);

        u2PllJitter = (UINT16)(IO_READ32(DRAM_BASE,0x2B4) & 0x0FFF);
        IO_WRITE32(DRAM_BASE,0x228, 0);
        #if 0
    ...
    // check MCK.
    for (i4MCK=u4LowBound + 30; i4MCK<=0x5F; i4MCK++)
    {        
        u4Val = 0x80000FFF;
        ...
        u2MckJitter = (UINT16)(IO_READ32(DRAM_BASE,0x2B4) & 0x0FFF);
        IO_WRITE32(DRAM_BASE,0x228, 0);
        #if 0
    ...
    u4Delta = u4LowBound + u4HighBound; 
    Printf("LowBound = %03d, HighBound = %03d, Delta = %3d .\n", u4LowBound, u4HighBound, u4Delta);
    return u4Delta;
...
void _DDR_GatingScan(void)
{
...
#ifdef CC_SUPPORT_DDR3
    UINT32 u4Delta, u4DqsCtl;
    u4Delta = _DDR_DeltaCalibrate();
    u4DqsCtl = DRAM_DDRPHY_READ32(0x380);
    u4DqsCtl &= 0xFFFF;
    
    for (u4Byte = 0; u4Byte<(u4ChipNum*BYTE_PER_CHIP); u4Byte ++)   
    {
        u4HwDQSth = (IS_DDR_SWAP16BIT()) ? u4Swap16Bit[u4Byte] : u4Byte;
        u4Val = u4DqsCtl | (u4DqsCtl << 16);
        DRAM_DDRPHY_WRITE32(0x380, u4Val);
        DRAM_DDRPHY_WRITE32(0x384, u4Val);
        u4Val = DRAM_READ32(0x388) & (~(0x7F << (8*u4HwDQSth)));
        u4Val |= GATING_CAL_CTLX400 << 8*u4HwDQSth;
        DRAM_WRITE32(0x388, u4Val);
        
        u4OldVal = (u4Reg388 >> (8*u4HwDQSth)) & 0x7F;
        // Check DQSCTL=0x400 lower bound.
        for(u4Step=u4GatingStep; u4Step<0x60; u4Step=u4Step+u4GatingStep)
        {
            if (u4OldVal > u4Step)
            {
        ...
        // Check DQSCTL=0x0800 upper bound.
        u4Val = (u4DqsCtl*2) | ((u4DqsCtl*2) << 16);
        DRAM_DDRPHY_WRITE32(0x380, u4Val);
        DRAM_DDRPHY_WRITE32(0x384, u4Val);
        u4Val = DRAM_READ32(0x388) & (~(0x7F << (8*u4HwDQSth)));
        u4Val |= GATING_CAL_CTLX800 << 8*u4HwDQSth;
        DRAM_WRITE32(0x388, u4Val);
        ...
        // Update 0x388 register new value.
        u4Avg = u4Min + u4Max > u4Delta ? (u4Min + u4Max - u4Delta)/2 : 0;
        u4Val = u4Avg;

#ifdef __MODEL_slt__
        if(!(u4HwDQSth%2))
        {
            u4Val = DRAM_DDRPHY_READ32(0x380+4*(u4HwDQSth/2)) & (~(0xFFFF << (16*(u4HwDQSth%2))));
            u4Val |= (u4Avg > 20 )? (0x0800 << 16*(u4HwDQSth%2)) : ((0x0400 << 16*(u4HwDQSth%2)));
            DRAM_DDRPHY_WRITE32(0x380+4*(u4HwDQSth/2), u4Val);
            u4Val = (u4Avg > 20) ? (u4Avg-20) : (u4Avg+u4Delta);           
        }
        else
        {
            u4Val = u4Avg + 20;
...
#endif
#ifdef DRAM_DEBUG
        Printf("Byte %d : Gating(%d(0x%04x) ~ %d(0x%04x)), Size=%d, Mid=%d(0x%04x), Set=%d(0x%04x).\n", \
                u4Byte, u4Min, u4DqsCtl, u4Max, u4DqsCtl*2, u4Delta+u4Max-u4Min, u4Avg, u4DqsCtl*2, u4Val, u4DqsCtl*2);
#endif
        if ((u4Delta+u4Max-u4Min) < 30 )
        {
            CHIP_DisplayString("Gating size too small !!\n");        
            // Use default setting.
            _DDR_ResetPhyOnly();                
    ...
    DRAM_WRITE32(0x388, u4Reg388);
    Printf("Current gating setting: \n");
    Printf("DQSCTL0 = 0x%08x \n", DRAM_DDRPHY_READ32(0x380));
    Printf("DQSCTL1 = 0x%08x \n", DRAM_DDRPHY_READ32(0x384));
    Printf("DQSIEN = 0x%08x \n", DRAM_READ32(0x388));

    return;
#endif //CC_SURPPORT_DDR3
}
#endif
