...
INT32 MUSB_Read(UINT64 u8Offset, UINT32 u4MemPtr, UINT32 u4MemLen)
{
    ...
    #ifdef __linux__
    {
        _rMUSBData.u8BlkNum = u8Offset;
        do_div(_rMUSBData.u8BlkNum, pMediumInfo->dwBlockSize);
    }
    #else
...
INT32 MUSB_GetInsert(VOID)
{
    ...
    LOG(0, "USB: Vbus turn up time = %d ms.\n", u4Diff);        
    ...
    LOG(0, "USB: Wait device insert time = %d ms.\n", u4UsbLoaderWaitTime);        
