...
static void _NonOSSetupUsbTimerIsr(void)
{
    ...
    u4BusClock = u4BusClock*10; // set hw timer to 10ms

    // Setup Timer 0 and triger 1st interrupt.
    BIM_WRITE32(REG_RW_TIMER0_LLMT, u4BusClock);
    BIM_WRITE32(REG_RW_TIMER0_HLMT, 0);
    BIM_WRITE32(REG_RW_TIMER0_LOW , u4BusClock);
...
{
    if (u4IrqMask() & ((UINT32)0x1 << u4Vector))
    {
        return TRUE;
    }
    else
