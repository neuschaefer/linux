...
// Static variables
//-----------------------------------------------------------------------------
static UINT32 _u4FvrOverflowCount = 0;
static UINT32 _u4FvrBufPeakSize = 0;
static UINT32 _u4FvrTimeIsrSerialNum = 0;
...
static void _FVRTestTimeSwap(UINT32 u4StartAddr)
{
    ...
    VERIFY(_FVR_GetGBL(FVR_GBL_FLAG_BUFFER, &rGBL));
...
static void _FVRPacketCountInt(UINT32 u4PacketAddr)
{
    ...
    else
    {
        ...
        {
            if(!_DMX_IDE_SendPacket(0, u4BufStart, u4BufEnd, prGBL->u4Rp, u4Wp))
            {
                LOG(1, "IDE send fail\n");
                return;
            }
...
{  
    ...
    if(!_FVR_GetGBL(FVR_GBL_FLAG_BUFFER, &rGBL))
    {
        return;
    }
 ...
 */
//-----------------------------------------------------------------------------
static void _FVRTimerPktInt(UINT32 u4PktCount, UINT32 u4EndAddr)
{
    FVR_GBL_STRUCT_T *prGBL;
    ...
    u4VirEndAddr = VIRTUAL(u4EndAddr);
    ...
    _FVR_Lock();
    if(u4PeakSize > _u4FvrBufPeakSize)
    {
        _u4FvrBufPeakSize = u4PeakSize;
    }
    _FVR_Unlock();
...
void _FVR_uPErrorInt(void)
{
    ...
    {
        prGBLStruct->pfnNotifyFunc(FVR_GBL_NFY_OVERFLOW, 0, 
                                   prGBLStruct->pvNotifyTag);
    }

    LOG(3, "FVR uP error happen\n");
}
...
BOOL _FVR_DispatchuPInterrupt(UINT32 u4Status, UINT32 u4Status2)
{
    ...
    if(u1Pidx >= DMX_NUM_ALL_PID_INDEX)
    {
        return FALSE;
    }
    ...
    case 66:    // Timer notification (End Addr + 1)
        _u4EndAddr = u4Status2;
        _u4FvrTimeIsrSerialNum = u1Pattern;
        break;

    case 67:    // Timer notification (long packet count)
        if(_u4FvrTimeIsrSerialNum == u1Pattern)
        {
            _FVRTimerPktInt(u4Status2, _u4EndAddr);
        }
        break;
 ...
 */
//-----------------------------------------------------------------------------
void _FVR_SetIsOverflow(BOOL fgIsOverflow)
{
    if(fgIsOverflow)
    {
        _FVR_Lock();
...
void _FVR_QueryStatus(FVR_QUERY_INFO_T *prInfo)
{
    ...
    // Ring bufer range
    _FVR_Lock();
    prInfo->u4BufStart = VIRTUAL(FVR_GBL_PID_W(0));
    prInfo->u4BufEnd = VIRTUAL(FVR_GBL_PID_W(1));
    prInfo->u4Wp = VIRTUAL(FVR_GBL_PID_W(3));
    prInfo->u4Rp = VIRTUAL(FVR_GBL_PID_W(4));
    
    prInfo->u4PktCount = FVR_GBL_PID_W(6);
    prInfo->u4OverflowCount = _u4FvrOverflowCount;
    prInfo->u4BufPeakSize = _u4FvrBufPeakSize;
    _FVR_Unlock();
}
