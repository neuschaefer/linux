...
#define MM_PES_HEADER_BUF_SIZE          0x1000
...
static BOOL _DMXMMAllocBuf(UINT8 u1Idx, const DMX_MM_T *prMM)
{
    ...
    else if((prPidStruct->ePidType == DMX_PID_TYPE_PES) ||
            (prPidStruct->ePidType == DMX_PID_TYPE_ES_DTCP))
    {
        u4W0 = (UINT32)(1 << 1);
    }
       ...
       (prPidStruct->ePidType == DMX_PID_TYPE_ES_AUDIO))
    {
        ...
        //_DMX_SetMemoryUsage(u1Pidx, MM_PES_HEADER_BUF_SIZE);
    ...
    if(prMM->fgAllocBuf)
    {
        ...
        if (u4BufStart == 0)
        {
            ...
            // Header buffer is not calculated in memory usage.
            //_DMX_ClearMemoryUsage(u1Pidx, MM_PES_HEADER_BUF_SIZE);
            LOG(3, "Can't allocate memory!\n");
            return FALSE;
        }
        _DMX_SetMemoryUsage(u1Idx, prMM->u4BufSize);
    }
    else
    {
...
static BOOL _DMXMMFreeBuf(UINT8 u1Idx)
{
    ...
    if (prPidStruct->fgAllocateBuffer)
    {
        ...
        _DMX_ClearMemoryUsage(u1Idx, prPidStruct->u4BufLen);
    }
...
BOOL _DMX_MM_Set(UINT8 u1Idx, UINT32 u4Flags, const DMX_MM_T *prMM)
{
    ...
    if(u4Flags & DMX_MM_FLAG_TYPE)
    {
        ...
        if(prMM->ePidType == DMX_PID_TYPE_ES_VIDEO)
        {
            UINT8 u1Channel;

            // Map pidx to MPV channel
            if (!_DMX_AllocateVideoChannel(u1Idx, &u1Channel))
            {
                return FALSE;
            }
        }
...
BOOL _DMX_MM_SetAES(UINT8 u1Idx, UINT32 u4Flags, const DMX_MM_DRM_AES_T *prDRM)
{
    ...
    if ((u4Flags & DMX_MM_DRM_FLAG_KEY) != 0)
    {
        ...
        for(i=0; i<8; i++)
        {
            PID_S_W((u1Idx + DMX_MM_DRM_STRUCT_OFFSET), 7 - i) =
                                             prDRM->au1Key[(i*4) + 3] |
...
BOOL _DMX_MM_FlushBuffer(UINT8 u1Idx)
{
    ...
    UINT8 u1MaxIndex = DMX_NUM_PID_INDEX;
    ...
    {
        u1MaxIndex = DMX_NUM_PID_INDEX;
    }
    else if (eInputType == DMX_IN_PLAYBACK_MM)
    {
        u1MaxIndex = DMX_MM_STRUCT_NUM;
    }
    if (u1Idx >= u1MaxIndex)
    {
    ...
    // Delay for 1 ms so that no data is left in the internal buffer.
    if((eInputType == DMX_IN_PLAYBACK_TS) || (eInputType == DMX_IN_BROADCAST_TS))
    {
    ...
    {
        if ((PID_W(3) & 0xFF) == (UINT32)u1Idx)
        {
            UINT32 u4uPState = (PID_W(17) >> 24) & 0xFF;
            if ((5 < u4uPState) && (u4uPState <= 17))
            {
                PID_W(17) = (PID_W(17) & 0x00FFFFFF) | ((UINT32)0x05 << 24);
                LOG(1, "(%u) u4uPState = %u\n", u1Idx, u4uPState);
            }
        }
    }
    ...
    // DMX_IN_BROADCAST_TS is for time-shift case
    if ((eInputType == DMX_IN_BROADCAST_TS) || (eInputType == DMX_IN_PLAYBACK_TS) || 
        (eInputType == DMX_IN_PLAYBACK_PS))
    {
        PID_S_W(u1Idx, 1) = 0;
        //PID_S_W(u1Idx, 2) = (PID_S_W(u1Idx, 2) & 0xFF) | 0x0000ff00;  // Scramble state = unknown
        PID_S_W(u1Idx, 3) = 0xffff0001;         // For PES (TS/PS uCode)
        PID_S_W(u1Idx, 4) = 0xffffffff;
    }
    else
    {
