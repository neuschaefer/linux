...
void _DMX_PanicReset(void)
{
...
#ifdef TIME_SHIFT_SUPPORT
    if(!_FVR_GetGBL(FVR_GBL_FLAG_ALL, &rGBL))
    {
        _u4DmxRecoveryErr = 9;
        goto PanicExit;
    }
    ...
    eFrontEnd = _DMX_GetFrontEnd();
    eInputType = _DMX_GetInputType();
    eVideoType = _DMX_GetVideoType();
    ...
    {
        UNUSED(_DMX_SetFrontEnd(eFrontEnd));
    }
    UNUSED(_DMX_SetVideoType(eVideoType));
    ...
    if(!_FVR_SetGBL(FVR_GBL_FLAG_ALL, &rGBL))
    {
...
void _DMX_PanicDisplay(void)
{
    ...
    {
        LOG(1, "Latest panic time: %d:%06d\n", 
            _rDmxRecoverTime.u4Seconds, _rDmxRecoverTime.u4Micros);
    }

    if(_u4dmxMicroPanicTimes > 0)
    {
        LOG(1, "Latest micro-code panic time: %d:%06d\n", 
                _rDmxMicroRecovertime.u4Seconds, _rDmxMicroRecovertime.u4Micros);
    }
