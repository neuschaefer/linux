...
#define DMX_REG_DES_IV_LO						27      // 0x6C
#define DMX_REG_DES_IV_HI						28      // 0x70
...
#define DMX_REG_AES_IV_0                        121
#define DMX_REG_AES_IV_1                        122
#define DMX_REG_AES_IV_2                        123
#define DMX_REG_AES_IV_3                        124
...
#define PID_INDEX_TABLE(pidx)   ((volatile UINT32*)(DEMUX5_BASE + 0xc00))[(pidx)]
    ...
    /// Function entry log
    #define FUNC_ENTRY              LOG(5, "+++ Enter %s +++\n", __FUNCTION__)

    /// Function exit log
    #define FUNC_EXIT               LOG(5, "--- Exit %s ---\n", __FUNCTION__)
...
#define CT_SETTING              (*(volatile UINT32*)(DEMUX2_BASE + 0xAE0))
#define MICROCODE_WATCHDOG      (*(volatile UINT32*)(DEMUX2_BASE + 0xAE4))
...
EXTERN void _DMX_CkgenToggle(void);

EXTERN BOOL _DMX_LoadIMem(BOOL fgForceWrite, const UINT32* pu4IData, UINT32 u4Len);

EXTERN BOOL _DMX_TSSoftInit(void);

EXTERN BOOL _DMX_SetFrontEnd(DMX_FRONTEND_T eFEType);

EXTERN DMX_FRONTEND_T _DMX_GetFrontEnd(void);

EXTERN void _DMX_ResetFramer(void);
...
EXTERN BOOL _DMX_SelectInputType(DMX_INPUT_TYPE_T rInputType);

EXTERN DMX_INPUT_TYPE_T _DMX_GetInputType(void);

EXTERN void _DMX_SetInputMode(DMX_INPUT_MODE_T rMode);

EXTERN BOOL _DMX_TestFTuPLatch(void);

EXTERN BOOL _DMX_SetPacketSize(UINT8 u1PacketSize);

EXTERN UINT32 _DMX_GetMinFreeBufferSize(DMX_PID_TYPE_T ePidType);
...
EXTERN BOOL _DMX_SetVideoType(DMX_VIDEO_TYPE_T eVDecType);

EXTERN DMX_VIDEO_TYPE_T _DMX_GetVideoType(void);
...
EXTERN void _DMX_SetDesIV(UINT32 u4IvHi, UINT32 u4IvLo);

EXTERN BOOL _DMX_SetAesKey(UINT8 u1Index, BOOL fgEven, const UINT32 au4Key[8]);

EXTERN void _DMX_SetAesIV(const UINT32 au4Iv[4]);

EXTERN BOOL _DMX_SetAesKeyLen(UINT32 u4KeyLen);
...
EXTERN BOOL _DMX_SetCapture2(BOOL fgEnable, const DMX_CAPTURE2_T* prCapture);
...
EXTERN BOOL _DMX_AllocateVideoChannel(UINT8 u1Pidx, UINT8* pu1Channel);
...
EXTERN void _DMX_SetDecoderCallbacks(
    const DMX_DECODER_CALLBACKS_T* prCallbacks);

EXTERN DMX_DECODER_CALLBACKS_T* _DMX_GetDecoderCallbacks(void);
...
EXTERN BOOL _DMX_UpdateVideoWritePointer(UINT8 u1Channel, UINT32 u4Wp);

EXTERN BOOL _DMX_SendPictureHeader(const DMX_PES_MSG_T* prPes);

EXTERN BOOL _DMX_SendVideoChunk(const DMX_PES_MSG_T* prPes);

EXTERN BOOL _DMX_SendAudioPes(const DMX_AUDIO_PES_T* prPes);
...
EXTERN void _DMX_CheckVCT(UINT32 u4BufStart, UINT32 u4BufEnd, UINT32 u4SecRp,
                            UINT32 u4SecLen);
// --- End (for J-Chip SLT program) ---
//-----------------------------------------------------------------------------

EXTERN BOOL _DMX_GetLockState(void);

EXTERN BOOL _DMX_GetLockStateEx(UINT8 u1FramerIdx);
...
EXTERN BOOL _DMX_IsTo1394(void);

EXTERN void _DMX_SetDebugPictureFinder(BOOL fgEnable);

EXTERN BOOL _DMX_IsDebugPictureFinder(void);
...
// Memory tracker
//
EXTERN void _DMX_InitMemoryTracker(void);

EXTERN void _DMX_SetMemoryUsage(UINT8 u1Pidx, UINT32 u4Size);

EXTERN void _DMX_ClearMemoryUsage(UINT8 u1Pidx, UINT32 u4Size);

EXTERN void _DMX_GetMemoryUsage(DMX_MEM_TRACKER_T *pMemTracker);
...
EXTERN INT32 _DMX_AwEraserCheck_Get(UINT32 u4Timeout);
