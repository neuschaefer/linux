...
#if 0
/// Program Stream
#define DMX_STREAM_FLAG_VALID              0x00000001  ///< Enabled or not
#define DMX_STREAM_FLAG_ID                 0x00000002  ///< ID value
#define DMX_STREAM_FLAG_BUFFER             0x00000004  ///< Buffer control
#define DMX_STREAM_FLAG_NONE               0           ///< None
#define DMX_STREAM_FLAG_ALL                0xffffffff
#endif

#define DMX_MM_STRUCT_NUM                   16//6
#define DMX_DTCP_MM_PIDX                    15
#define DMX_MM_DRM_STRUCT_OFFSET            16
...
typedef enum
{
    DMX_DDI_PORT_FRAMER,        // to Demux framer 0
    DMX_DDI_PORT_NO_FRAMER      // to Demux TS index 1 (DDI; bypass framer)
} DMX_DDI_PORT_T;
...
typedef BOOL (*PFN_DMX_SEND_TO_VDEC)(const DMX_MM_VDEC_MSG_T* prMsg);
typedef BOOL (*PFN_DMX_SEND_TO_ADEC)(const DMX_MM_ADEC_MSG_T* prMsg);

/// Decoder callback handlers
typedef struct
{
    PFN_DMX_SEND_TO_VDEC pfnSendToVdec;
    PFN_DMX_SEND_TO_ADEC pfnSendToADec;
...
EXTERN BOOL DMX_PVRPlay_EnableTSOut(BOOL fgEnableTSOut);
EXTERN BOOL DMX_PVRPlay_SetUseDescramble(BOOL fgEnable);
EXTERN BOOL DMX_PVRPlay_GetUseDescramble(void);
...
EXTERN void DMX_DDI_SetPort(DMX_DDI_PORT_T ePort);
EXTERN DMX_DDI_PORT_T DMX_DDI_GetPort(void);

EXTERN BOOL DMX_DDI_StreamStart(void);
EXTERN BOOL DMX_DDI_StreamStop(void);

EXTERN BOOL DMX_DDI_FlushBuf(void);
EXTERN UINT32 DMX_DDI_GetFreeBufSize(void);
EXTERN BOOL DMX_DDI_FillBuf(UINT32 u4DataAddr, UINT32 u4DataSize, UINT32 *pu4FilledSize);
EXTERN DMX_DDI_STATE_T DMX_DDI_GetStreamState(void);
EXTERN BOOL DMX_DDI_SetPacketSize(UINT8 u1PacketSize);
EXTERN BOOL DMX_DDI_SetSyncOffset(UINT8 u1Offset);
EXTERN BOOL DMX_DDI_SingleMove(UINT32 u4BufStart, UINT32 u4BufEnd,
                               UINT32 u4Addr, UINT32 u4Size);
EXTERN BOOL DMX_DDI_MoveData(const DMX_MM_DATA_T *prData);
EXTERN BOOL DMX_DDI_PowerDown(VOID);
EXTERN BOOL DMX_DDI_RequestReset(VOID);
...
EXTERN DMX_AUDIO_TYPE_T DMX_PS_GetAudioType(void);
EXTERN BOOL DMX_PS_SetDataFormat(DMX_DATA_FORMAT_T eDataFormat);
EXTERN DMX_DATA_FORMAT_T DMX_PS_GetDataFormat(void);
...
EXTERN BOOL DMX_MM_MoveData(UINT8 u1Idx, const DMX_MM_DATA_T* prData, UINT32 u4TimeOut);
EXTERN UINT32 DMX_MM_GetBufEmptySize(UINT8 u1Idx);
