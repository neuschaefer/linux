...
#define DECODE_OUT_OFFSET 0x80000
...
* Function prototype
******************************************************************************/
static INT32 _SetDispBuffer(IMG_LIB_DISP_INFO_T* prDisp, IMG_REGION_T rDst, UINT32 u4DispColorMode, ENUM_IMG_ROTATE_PHASE ePhase);
...
******************************************************************************/
//static 
IMG_DEBUG_INFO_T _rImgDecDbg;
//static UINT32 _au4BytePerPixel[] = {2, 0, 0, 2};		///< bit depth (left shift value), index=ENUM_IMG_DISP_COLOR_MODE
static UINT32 _au4BPP[] = {4, 2, 2, 3};				///< bit depth, index=ENUM_IMG_DISP_COLOR_MODE
...
IMGHANDLE _IMG_Init(ENUM_IMG_BUFFER_TYPE eMemClass, void *pvTag)
{
    ...
    if (eMemClass >= RING_IN_SINGLE_OUT)			//Input buffer provide by jpg driver
    {
       ...
       JDEC_MwSetRingBuf(prImg->hCurDec, VIRTUAL_UINT8((UINT32)prFbmPool->u4Addr + BS_OFFSET),(UINT32)BS_SIZE);
    }
...
void _IMG_GetProgess(IMGHANDLE hCurImg, UINT32* pu4Status)
{
    ...
    *pu4Status = JDEC_GetProgress(prImg->hCurDec,&prImg->rHdrInfo);
}
...
INT32 _IMG_DecPrepare(IMGHANDLE hCurImg, IMG_PARAM_T rInfo)
{
    ...
    _pu1TempStartAddr = rInfo.rDstRegion.pu1Addr;
#endif


    //--- set display parameter------------------------------------------
    if (_SetDispBuffer(prDispInfo, rInfo.rDstRegion, (UINT32)rInfo.eDispColorMode, rInfo.eRotateDegree) < 0)
    {
        return (INT32)E_IMG_DISP_INFO_ERROR;
    }

    if(rInfo.pu1InBuffer)
    {
    ...
    if ((rInfo.eRotateDegree == CLOCKWISE_0) || (rInfo.eRotateDegree == CLOCKWISE_180) || (rInfo.eRotateDegree == CLOCKWISE_0_WITH_FLIP) || (rInfo.eRotateDegree == CLOCKWISE_180_WITH_FLIP))
    {
    ...
    if (prDispInfo->u4DispPitch == 0)
    {
        ...
        if (prDispInfo->u4DisplayClrMode != (UINT32)AYCBCR8888_DIRECT32)
        {
            if (prImg->u1MemClass != (UINT8)EXTERNAL_RING_IN_SINGLE_OUT)
            {
    ...
    _rImgDecDbg.u4ScW = prDispInfo->u4DisplayW;
    _rImgDecDbg.u4ScH = prDispInfo->u4DisplayH;
    _rImgDecDbg.u4Pitch=prDispInfo->u4DispPitch;
    _rImgDecDbg.u4OrigImgW = prJpgHdrInfo->u4Width;
    _rImgDecDbg.u4OrigImgH = prJpgHdrInfo->u4Height;
    _rImgDecDbg.u4StartX= prDispInfo->u4StartX[0];
    _rImgDecDbg.u4StartY= prDispInfo->u4StartY[0];
	_rImgDecDbg.u4Rotate=rInfo.eRotateDegree;
...
VOID _IMG_RotateJpg(ENUM_IMG_ROTATE_PHASE eRotatePhase, IMG_LIB_DISP_INFO_T* prDispInfo, IMG_LIB_MEM_LAYOUT_T* prImgLibMem)
{
    IMG_REGION_T rSrc, rDst;
    UINT32 u4DisplayClrMode, u4ScaleW, u4ScaleRowH;
    ...
    if ((eRotatePhase == CLOCKWISE_0_WITH_FLIP) || (eRotatePhase == CLOCKWISE_90_WITH_FLIP) || (eRotatePhase == CLOCKWISE_270_WITH_FLIP))
    {
        ...
        rSrc.u4Pitch = ALIGN16(u4DispPitch);
        rSrc.u4StartX = 0;
        rSrc.u4StartY = 0;
...
static INT32 _SetDispBuffer(IMG_LIB_DISP_INFO_T* prDisp, IMG_REGION_T rDst, UINT32 u4DispColorMode, ENUM_IMG_ROTATE_PHASE ePhase)
{
...
#ifdef JPG_SEMI_HOST
    if (u4DispColorMode == (UINT32)AYCBCR8888_DIRECT32)
    {
    ...
    if (prDisp->u4DisplayClrMode == (UINT32)AYCBCR8888_DIRECT32)
    {
