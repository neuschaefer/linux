...
STATIC void MonitorProcess(VOID *pvArg)
{
    ...
    psTDDTDCtx->t_monitor_thread = NULL;
...
UINT8 u1Status, fgParallel = 0;
    ...
    if (i4Argc < 1)
    {
        ...
        mcSHOW_USER_MSG(("\t s: [1(Parallel) / 0(Serial)]: Set TS format\n"));
...
//                         &_Freq1, &u4SymbolRate, &_Mode);
                         &_Freq1, &u4SymbolRate, &_Mode, &_OobSpecInv);

    if (i4State != RMR_OK)
    {
        mcSEMA_UNLOCK_HAL(psTDDTDCtx->hHalLock);

        mcSHOW_DBG_MSG((" Invalid input (%d) to DTD_TunerAcq!\n", i4State));
        return fgRetSts;
    }
...
#if fcADD_ACQ_CHK
    #if fcMEASURE_DRIVER_TIME != 2
    //mcSHOW_DBG_MSG((" DTD_TunerAcq = %d ", fgRetSts));
...
//CHAR        char2;
    ...
    switch (e_tuner_get_type)
    {
        ...
        case DRV_CUSTOM_TUNER_GET_TYPE_DIAG_CONTENT:
            ...
            //char1 = *ptmpch;
            //Printf("pg_glue begin, first char = %c\n", *ptmpch);
