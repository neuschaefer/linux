 ...
 * $Author: dtvbm11 $
 * $Date: 2012/04/27 $
 * $RCSfile: pi_tuner_va1e1bf.c,v $
 * $Revision: #1 $
 *
 *---------------------------------------------------------------------------*/

/** @file pi_tuner_va1e1bf.c
 *  Tuner control for sahrp va1e1bf
 */
...
#include "eqscript_va1e1bf_mt5363.h"
#include "sif_if.h"
...
//-----------------------------------------------------------------------------
#define SHARP_VA1E1BF_BASE            ((UINT8) 0xC0)
#define SHARP_VA1E1BF_IF_FREQUENCY    ((UINT16) 44000)   /* KHz */
//#define SHARP_VA1E1BF_LO_STEP         ((UINT16) 14286)  /* 10Hz */
#define SHARP_VA1E1BF_ANA_LO_STEP         ((UINT16) 5000)  /* 10Hz */
#define SHARP_VA1E1BF_STR             "SHARP_VA1E1BF"
...
STATIC UINT8* pVA1E1BFEqNormal[MOD_ANA_TYPE_END - MOD_ANA_TYPE_BEGIN] = {NULL};
STATIC UINT8* pVA1E1BFEqWeak[MOD_ANA_TYPE_END - MOD_ANA_TYPE_BEGIN] = {NULL};
...
//
/** VA1E1BF_TunerInit
 *  Tuner initialzation forVA1E1BF.
 *  @param  pTCtx       Pointer of pi tuner driver context.
 *  @retval void
 */
//-----------------------------------------------------------------------------
VOID VA1E1BF_TunerInit(
    ITUNER_CTX_T *pTCtx
    )
{
    ...
    psTunerCtx->I2cAddress = SHARP_VA1E1BF_BASE;
    psTunerCtx->u4RF_Freq = 0;
    psTunerCtx->fgRFAGC = TRUE;//Externel RF_AGC
    /* Specific part: Only for this tuner model */
    psTunerCtx->u2IF_Freq = SHARP_VA1E1BF_IF_FREQUENCY;
  ...
  //  psTunerSpecificCtx->m_aucPara[TUNER_FUNC_EN_TARGET_LEVEL] = TRUE;
   ...
   // psTunerSpecificCtx->m_aucPara[TUNER_PARA_TARGET_LEVEL_LSB] = 0x00;  
    ...
    // init EQ script
  pVA1E1BFEqNormal[MOD_NTSC_M - MOD_ANA_TYPE_BEGIN]  = EQSHARPVA1E1BFMN;
  pVA1E1BFEqWeak[MOD_NTSC_M - MOD_ANA_TYPE_BEGIN]  = EQSHARPVA1E1BFMNWeak;
    //pVA1E1BFEqNormal[MOD_NTSC_M_54M_44M]  = EQSHARPVA1E1BFMN;
    //pVA1E1BFEqNormal[MOD_NTSC_M_54M_36M]  = EQSHARPVA1E1BFMN;
   psTunerSpecificCtx->u2LO_Step=SHARP_VA1E1BF_ANA_LO_STEP;
   
    mcSHOW_USER_MSG(("Init " SHARP_VA1E1BF_STR " EXTERNEL RF_AGC3\n"));
}


//----------------------------------------------------------------------------- 
/** VA1E1BF_TunerSetFreq
 *  Set Tuner PLL forVA1E1BF/VA1E1BF to tune RF frequency.
 *  @param  pTCtx       Pointer of pi tuner driver context.
...
u1Mod=param->Modulation;
 ...
 mcSHOW_DBG_MSG(("define USE_ATD_IF_DEMOD\n"));
#endif

   mcSHOW_DBG_MSG(("SHARP VA1E1BF_TunerSetFreq %dKHz Mode=%d \n",param->Freq,u1Mod));
  if ( u1Mod>= MOD_ANA_TYPE_BEGIN )
    ...
    if(!fgAnalogSignal){//digital  mode use digital LO step
     ...
     Ndivider = (UINT16) ((Lo * 100) /SHARP_VA1E1BF_ANA_LO_STEP);
    }
...
/* Assign Control word for specific tuner */
   ...
   CB2=0xD1;
    ...
    if((freq<88) && (freq>=82)) Nband |= 0x08;//CH6 FM trap ON
   
    if (fgAnalogSignal)                     // For NTSC
    	{
    		 CB1 = 0xBD;//RFAGC disable(Externel)
    	}
  //  else
  //  	{
  //  	Nband|=0x90;
...
/*    TunerS[2] = (UINT8) 0x7F & (Ndivider >> 8);
 ...
 /*if(fgEnableATP==0)
 {
		 mcSHOW_DBG_MSG(("Directly write tuner\n"));
 		TunerDataCnt = 5;
*/ 
 if (ICtrlBus_I2cTunerWrite(270,TOSHIBA_DEMOD_BASE, TunerS, TunerDataCnt))
   ...
   //  mcSHOW_DBG_MSG(("LO_%02X(%d): ",psTunerCtx->I2cAddress,TunerDataCnt));
 // }
/*  else
  	{

  		 mcSHOW_DBG_MSG(("Write tuner through demod\n"));
  		TunerS[0]=0xFE;
    ...
    if (ICtrlBus_I2cTunerWrite(270,TOSHIBA_DEMOD_BASE, TunerS, TunerDataCnt))
    {
       ...
       mcSHOW_DBG_MSG(("LO_%02X(%d): 0x%02X-0X%02X-",TunerDataCnt,TOSHIBA_DEMOD_BASE,TunerS[0],TunerS[1]));
  	}
  */
//    else//I2C bus  connect
//    {
        if(fgAnalogSignal){
            psTunerCtx->u4RF_Freq=(Ndivider*SHARP_VA1E1BF_ANA_LO_STEP)/100-psTunerCtx->u2IF_Freq-1750;
        }
        else{
...
//----------------------------------------------------------------------------- 
/**  VA1E1BF_TunerGetStatus
 *  Set Tuner PLL staus forVA1E1BF/VA1E1BF
 *  @param  pTCtx       Pointer of pi tuner driver context.
 *  @retval TRUE        PLL lock
 *  @retval FALSE       PLL unlock
 */
//-----------------------------------------------------------------------------
STATIC BOOL VA1E1BF_TunerGetStatus(ITUNER_CTX_T *pTCtx)
{
    //ITUNER_CTX_T *psTunerCtx = pTCtx;
   ...
   // if (ICtrlBus_I2cTunerRead(270,psTunerCtx->I2cAddress, TunerS, 1))
    if(!SIF_X_Read(1,0x100,TOSHIBA_DEMOD_BASE,2,u4WordAddr,&ucData,1))
	{
 ...
 */
//-----------------------------------------------------------------------------
STATIC CHAR *VA1E1BF_TunerGetVer(VOID)
{
    return (SHARP_VA1E1BF_STR);
}
//------------------------------------------------------- 
/** VA1E1BF_TunerOP
 *  Set/Get function
 ...
 */
//-----------------------------------------------------------------------------
INT16 VA1E1BF_TunerOP(ITUNER_CTX_T * pTCtx, ITUNEROP_T eOperation, UINT8 SetVal,  VOID * pInOutVal){
         
    switch(eOperation){
    case itGetStatus:
        {
        BOOL * Retval=(BOOL *)pInOutVal;
        *Retval=VA1E1BF_TunerGetStatus(pTCtx);
        }
        break;
    case itGetVer:
        {
        CHAR ** RetStr=(CHAR **)pInOutVal;
        *RetStr=VA1E1BF_TunerGetVer();
        }
        break;
    case itGetEqScriptNormal: 
        *(UINT8**)pInOutVal = pVA1E1BFEqNormal[SetVal];
        break;

    case itGetEqScriptWeak: 
        *(UINT8**)pInOutVal = pVA1E1BFEqWeak[SetVal];
        break;
