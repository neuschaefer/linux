...
#include "eq_script_f20wt_8223.h"

// *** Release Note ***
// Ver     Date         Description
// 1.00    2009.03.25   file established
#define C_F20WT_VERSION				"F20WT v1.00"

#define C_F20WT_PLL_TIMEOUT_CNT     10
#define C_F20WT_I2C_CLK_DIV         0x100
#define C_F20WT_PLL_LOCK_BIT        6
 
#define C_F20WT_LO_ADDRRESS           ((U16)  0xC2)
#define C_F20WT_IF_FREQUENCY_ANA      ((U16)  38000)  /* kHz */
#define C_F20WT_IF_FREQUENCY_L1       ((U16)  32900)  /* kHz */
#define C_F20WT_LO_DIVIDER_STEP_ANA   ((U16)  62500) //50000)  /* Hz */
...
//    SPECIFIC_MEMBER_EU_CTX *psTunerSpecificCtx = &(psTunerCtx->specific_member.eu_ctx);
	...
	psTunerCtx->u2IF_Freq = C_F20WT_IF_FREQUENCY_ANA;
	psTunerCtx->u4RF_Freq = 0;
    psTunerCtx->u1AtdPatchSwitch = (MASK_PATCH_CR|MASK_PATCH_DRO|MASK_PATCH_SENS|MASK_PATCH_STCHR);
...
// AGC para
    ...
    mcSHOW_USER_MSG(("Tuner F20WT for mtk board init done!\n"));
}

S16 F20WT_TunerSetFreq(ITUNER_CTX_T *pTCtx, PARAM_SETFREQ_T* param)
{
	UCHAR TunerS[4];
    U16   Ndivider;
    UCHAR CB1, BB; 
    ...
    if( Mode == MOD_SECAM_L1 )
    {
        ...
        Ndivider = (U16) ( (Lo*1000+C_F20WT_LO_DIVIDER_STEP_ANA/2) / C_F20WT_LO_DIVIDER_STEP_ANA);
    ...
    for (jj=0; jj<2; jj++)
    {
        if (ICtrlBus_I2cTunerWrite(C_F20WT_I2C_CLK_DIV, pTCtx->I2cAddress, TunerS, 4))
        {
		...
		{
		    mcSHOW_DRVAPI_MSG(("Tuner F20WT Write PLL success!\n"));
		}
		...
		for (s1PllPollCnt=0; s1PllPollCnt<C_F20WT_PLL_TIMEOUT_CNT; s1PllPollCnt++)
		{
            ...
            if (ICtrlBus_I2cTunerRead(C_F20WT_I2C_CLK_DIV, pTCtx->I2cAddress, &ucPllValue, 1))
            {
            ...
            //mcDELAY_MS(10);			
            // check if PLL lock
            if (mcCHK_BIT1(ucPllValue, C_F20WT_PLL_LOCK_BIT)) {
                break;
            }											
		}
