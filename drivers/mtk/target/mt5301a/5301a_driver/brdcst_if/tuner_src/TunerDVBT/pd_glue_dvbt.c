...
static UINT8 checkTunerTypeFromEeprom(void)
{
	  ...
	  UINT16 ucByteCount =1;
	   ...
	   if(ICtrlBus_I2cTunerRead(270, 0xc0, pucBuffer, ucByteCount) == 0)
		 ...
		 {
			 Printf("--defaut I2C Addr = C2->Open tuner ENV57S------------\n");
			 return 0x5d;
		 }	
#else
                     ...
                     PTD_SPECIFIC_CTX* pptTDSpecificCtx)
{
    ...
    *pptTDSpecificCtx = pPdCtx;
    ...
    Printf("Register DVBT Tunerall type = %d\n",tunerType);		
 #endif
...
/*            mcSHOW_USER_MSG(("DTD PLL CFG (0x2000d180) = 0x%x\n", mcRISC_Reg(CKGEN_BASE, 0x180)));
    ...
    default:
        mcPRINTF("Cmd: \n");
        mcPRINTF("\t s [1/2/3/4/5]: Set TunerID to EEPROM (1: ENV57S, 2: FT2101, 3: TDTWS810D,4: FH2607, 5: Query TunerType in EEPROM)\n");
        mcPRINTF("\t monon:    (period level) turn on monitor for Toolx info update\n");
        mcPRINTF("\t monoff:   turn off monitor for Toolx info update\n");
        mcPRINTF("\t pecon:    turn on  PEC\n");
        mcPRINTF("\t pecoff:   turn off PEC\n");
        mcPRINTF("\t sh:       set hierarchical (1: HP, 0: LP)\n");
        mcPRINTF("\t stop:     Set TOP\n");  
        mcPRINTF("\t checkreg:  Basic register settings check");
        mcPRINTF("\t ar [0/1]: AR Coefficient [Off/On]\n");
