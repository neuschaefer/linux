...
//for new scan step
#define cNEXT_STEP_TVD_UNLOCK           2000000 //Hz, original value
...
static BOOL AtdPd_CheckTVDLock(ATD_PD_CTX_T *psAtdPdCtx, UINT16 u4Freq, CHAR cMsgInd)
{
BOOL    fgLockSts = TRUE;
    #if fcTIMING_MEASURE
HAL_TIME_T  rCurTime;
...
/* do TVD check when auto search find VIF, LC Chien 070302*/
    ...
    //if ((psAtdeuDemodCtx->u1TunerType == cALPS_TEQE3) || (psAtdeuDemodCtx->u1TunerType == cALPS_TEQE3_SUNBOARD))
    {
        ...
        if(psAtdPdCtx->u1SubSysIdx >= MOD_NTSC_M) //first detect TVD lock
        {
            ...
            {
                fgLockSts = TRUE;
                mcSHOW_DBG_MSG(("Burst 358 is detected!\n"));
            }
            else if (fgSameFreq)
            {
                fgLockSts = TRUE;
            ...
            {
                //fgLockSts = FALSE;
                //mcSHOW_DBG_MSG(("Burst 443 or unknown!\n"));
                GPIO_SetOut(21, 1); //set SAW BW=8M
    ...
    while (fgMonitorThreadRunning)
    {
    	...
    	else
    	{
            ...
            {                                
                ...
                {
                    ucData1 = 0x08;
                    ATD_SetReg(psAtdeuDemodCtx, 0x980, &ucData1, 1);
                    ucData1 = 0x04;                    
                    ATD_SetReg(psAtdeuDemodCtx, 0x9B6, &ucData1, 1);
                    ucData1 = 0x21;                    
                    ATD_SetReg(psAtdeuDemodCtx, 0x9B1, &ucData1, 1);
                    //ucData1 = 0x8C;                    
            ...
            else
            {
                ...
                {                     
                    ...
                    ATD_SetReg(psAtdeuDemodCtx, 0x980, &ucData1, 1);
                    ucData1 = 0x09;                    
                    ATD_SetReg(psAtdeuDemodCtx, 0x997, &ucData1, 1);
                    //ucData1 = 0x26;                    
                    //ATD_SetReg(psAtdeuDemodCtx, 0x9B4, &ucData1, 1);
                    //ucData1 = 0xCC;                    
                    //ATD_SetReg(psAtdeuDemodCtx, 0x999, &ucData1, 1);
...
STATIC void SetFMRadio(UINT32  i4CurFreq)
{
    ...
    ucData1 = 0x6D;
    ATD_SetReg(psAtdeuDemodCtx, 0x9B7, &ucData1, 1);
    ucData1 = 0xC9;
    ATD_SetReg(psAtdeuDemodCtx, 0x999, &ucData1, 1);
    ucData1 = 0x00;
    ATD_SetReg(psAtdeuDemodCtx, 0x9D6, &ucData1, 1);
    ucData1 = 0x00;
    ATD_SetReg(psAtdeuDemodCtx, 0x9C2, &ucData1, 1);
    ucData1 = 0x20;
    ATD_SetReg(psAtdeuDemodCtx, 0x9C3, &ucData1, 1);
    ucData1 = 0x81;
    ATD_SetReg(psAtdeuDemodCtx, 0xA20, &ucData1, 1);
    ucData1 = 0xA1;
    ATD_SetReg(psAtdeuDemodCtx, 0xA21, &ucData1, 1);                                
...
static void setTunerTypeToEeprom(UINT8 u1TunerType){
    ...
    Printf("read TunerType from EEPROM, addr0x%x=0x%x, addr0x%x=0x%x, addr0x%x=0x%x\r\n",u4Ofst0,u1Val1,u4Ofst1,u1Val2,u4Ofst2,u1Val3);
    ...
    VERIFY(0 == EEPROM_Read((UINT64)u4Ofst2, (UINT32)(void *)&u1Val3, 1));
    Printf("Modify TunerType in EEPROM, addr0x%x=0x%x, addr0x%x=0x%x, addr0x%x=0x%x\r\n",u4Ofst0,u1Val1,u4Ofst1,u1Val2,u4Ofst2,u1Val3);
...
static UINT8 checkTunerTypeFromEeprom(void)
{
  ...
  UINT16 ucByteCount =1;
   ...
   if(ICtrlBus_I2cTunerRead(270, 0xc0, pucBuffer, ucByteCount) == 0)
	 ...
	 {
	     Printf("--tuner I2C Addr = C0->Open tuner DT70WI------------\n");
		 return 0x62;
	 }
    else if(ICtrlBus_I2cTunerRead(270, 0xc2, pucBuffer, ucByteCount) == 0)
     {
	     Printf("--tuner I2C Addr = C2->Open tuner ENV57S------------\n");
	     return 0x5d;
	 }
    else
     {
	     Printf("--default I2C Addr = C2->Open tuner ENV57S------------\n");
	     return 0x5d;
	 }	
#else
    ...
    Printf("ofst0=0x%x, ofst1=0x%x, ofst2=0x%x, def=0x%x\r\n",u4Ofst0, u4Ofst1, u4Ofst2, u4Default);
...
ATD_PD_CTX_T   *psAtdPdCtx;
 ...
 //090720, LC, move ITuner init b4 ATD init
    ...
    Printf("Register ATD Tunerall type = %d\n",tunerType);	
 #endif
#endif
...
//#endif
...
#endif		
        u1ConnMode = GetAtdConnMode(((TUNER_TER_ANA_TUNE_INFO_T *) pv_conn_info)->e_mod);
    ...
    else if (e_conn_type == TUNER_CONN_TYPE_CAB_ANA) 
    {
        ...
        u1ConnMode = GetAtdConnMode(((TUNER_CAB_ANA_TUNE_INFO_T *) pv_conn_info)->e_mod);
        psAtdPdCtx->u1SubSysIdx = vGetTvSubSysIdx(psAtdPdCtx, psAtdPdCtx->u4TvSubSysMask); 
    }
    else
    {
    ...
    {        
        ucData1 = 0xF9;
        ATD_SetReg(psAtdeuDemodCtx, 0x997, &ucData1, 1); //EC_code_d9A28: Enable CR auto-reacq for Fine tune mode
    ...
    if (u1ConnMode == ATV_CONN_MODE_CHAN_CHANGE)   
    {
		...
		{
		   ucData1 = 0xFF; 
	       ATD_SetReg(psAtdeuDemodCtx, 0x934, &ucData1, 1); //realese data path register,for TDTWS810D tuner LNA
		}
    }
    if (ATD_SetFreq(psAtdeuDemodCtx, u4Freq, _fgAutoSearch) != TRUE) /*jackon: why need use SV_TRUE?*/
    {
        printf("Freq %dKHz setting fail!\n", u4Freq);
        AtdPd_ExitTunerAcq(FALSE);

        mcSEMA_UNLOCK_HAL(psAtdPdCtx->hHalLock);
        return FALSE;
    }
	else   //add by liuqu,for AGC-Frequency relationship!
	{
    ...
    if (u1ConnMode == ATV_CONN_MODE_CHAN_SCAN)
    {
    	if(((TUNER_TER_ANA_TUNE_INFO_T *) pv_conn_info)->b_fine_tune == TRUE ) {
    ...
    if ( (u1ConnMode == ATV_CONN_MODE_CHAN_SCAN) && 
         (((e_conn_type == TUNER_CONN_TYPE_TER_ANA) && ((TUNER_TER_ANA_TUNE_INFO_T *) pv_conn_info)->b_fine_tune) || 
          ((e_conn_type == TUNER_CONN_TYPE_CAB_ANA) && ((TUNER_CAB_ANA_TUNE_INFO_T *) pv_conn_info)->b_fine_tune) ))
    {
        UINT32 u2FFF = u4Freq;
    ...
    else
    {
        ...
        if(((TUNER_TER_ANA_TUNE_INFO_T *) pv_conn_info)->e_mod == MOD_QAM_896)        
            psAtdeuDemodCtx->fgCheckCR = TRUE;
        else
        ...
        switch(u1ConnMode)
        {
            ...
            case ATV_CONN_MODE_CHAN_SCAN:
                ...
                if ( (u1ATVStatus == 2) || ((u1ATVStatus == 1) && (((TUNER_TER_ANA_TUNE_INFO_T *) pv_conn_info)->e_mod == MOD_UNKNOWN)) )
                {
...
//#ifdef ANAEU_ENABLE_MONITOR
    ...
    {    	
        ...
        //Printf("Enable monitor process!!, fgAFT = %d\n", psAtdeuDemodCtx->fgAFT);
...
//#endif
    ...
    if(u1ConnMode != ATV_CONN_MODE_CHAN_SCAN) //LC Chien, 20090813
    {
        ...
        //Printf("Tuner_DRV call AUD_TriggerDspPlay(AUX_DEC)!\n\n");
    }
#endif
...
*/
    ...
    mcSEMA_UNLOCK_HAL(psAtdPdCtx->hHalLock);
    AtdEU_TunerAtdCommand(ptTDSpecificCtx, i4Argc, aszArgv);
}
			    ...
			    if (i4Argc ==2)
				  {
					 ...
					 ITuner_OP(psAtdeuDemodCtx->pTCtx, itSetTop, i4CurTop, &ucData1); 
					 ATD_SetFreq(psAtdeuDemodCtx, i4CurFreq, FALSE);
		 ...
		 {   //only available in nim.atd p 4(bit 2 =1)
		  ...
		  else if (gATP_TEST_ITEM_BIT & 0x01)
		  {
            ...
            case 'r':
				...
				#else
				if (ATD_GetReg(psAtdeuDemodCtx, u2RegAddr+0x800, ucData, ucPar))
				#endif
            ...
            case 'w':
                ...
                #else
				 if (ATD_SetReg(psAtdeuDemodCtx, u2RegAddr+0x800, &ucPar, 1))
                #endif
            ...
            case 'g':
                ...
                ATD_GetReg(psAtdeuDemodCtx, 0x9A6, &ucData1, 1);
                ATD_GetReg(psAtdeuDemodCtx, 0x9A7, &ucData2, 1);
                s2Data = (S32)mcSIGN_2BYTE(ucData1, ucData2, 5, 8);
                s2Data = (S32)s2Data * 439 / 100000;
                mcSHOW_USER_MSG(("CPO = %d degree\n", s2Data));
                ATD_GetReg(psAtdeuDemodCtx, 0x9A0, &ucData1, 1);                
                mcSHOW_USER_MSG(("OVM idx = %d\n", (ucData1 & 0x70) >> 4 ));
                mcSHOW_USER_MSG(("0x1A0 = 0x%02X\n", ucData1));
                ATD_GetReg(psAtdeuDemodCtx, 0x9A1, &ucData1, 1);
                mcSHOW_USER_MSG(("0x1A1 = 0x%02X\n", ucData1));
                mcSHOW_USER_MSG(("PN indicator = %d\n", (ucData1 & 0x08) >> 3 ));
                mcSHOW_USER_MSG(("FN indicator = %d\n", (ucData1 & 0x04) >> 2 ));
                //ATD_GetReg(psAtdeuDemodCtx, 0x197, &ucData1, 1);
                //mcSHOW_USER_MSG(("0x197 = 0x%02X\n", ucData1));
                //ATD_GetReg(psAtdeuDemodCtx, 0x198, &ucData1, 1);
                //mcSHOW_USER_MSG(("0x198 = 0x%02X\n", ucData1));
                ATD_GetReg(psAtdeuDemodCtx, 0x99C, &ucData1, 1);
                ATD_GetReg(psAtdeuDemodCtx, 0x99D, &ucData2, 1);
                u2Data = mcUNSIGN_2BYTE(ucData1, ucData2, 8, 8);
                mcSHOW_USER_MSG(("CR CMetrix = 0x%04X\n", u2Data));
                ATD_GetReg(psAtdeuDemodCtx, 0x00B, &ucData1, 1);
                switch (ucData1)
                {
                    case 0:
                ...
                // DRO
                ATD_GetReg(psAtdeuDemodCtx, 0x96F, &ucData1, 1);
                ATD_GetReg(psAtdeuDemodCtx, 0x970, &ucData2, 1);
                u2Data = mcUNSIGN_2BYTE(ucData1, ucData2, 8, 8);
                mcSHOW_USER_MSG(("DRO gain = 0x%04X\n", u2Data));
                //ATD_GetReg(psAtdeuDemodCtx, 0x164, &ucData1, 1);
                //mcSHOW_USER_MSG(("0x164 = 0x%02X\n", ucData1));
                //ATD_GetReg(psAtdeuDemodCtx, 0x166, &ucData1, 1);
                //mcSHOW_USER_MSG(("0x166 = 0x%02X\n", ucData1));
                //ATD_GetReg(psAtdeuDemodCtx, 0x17A, &ucData1, 1);
                //mcSHOW_USER_MSG(("0x17A = 0x%02X\n", ucData1));
                
                // PRA
                ATD_GetReg(psAtdeuDemodCtx, 0x94D, &ucData1, 1);
                ATD_GetReg(psAtdeuDemodCtx, 0x94E, &ucData2, 1);
                u2Data = mcUNSIGN_2BYTE(ucData1, ucData2, 8, 8);
                mcSHOW_USER_MSG(("RF gain = 0x%04X\n", u2Data));
                //ATD_GetReg(psAtdeuDemodCtx, 0x94F, &ucData1, 1);
                //ATD_GetReg(psAtdeuDemodCtx, 0x950, &ucData2, 1);
                //u2Data = mcUNSIGN_2BYTE(ucData1, ucData2, 8, 8);
                u2Data = AtdEU_GetIFAGC();
                mcSHOW_USER_MSG(("IF gain = 0x%04X\n", u2Data));
                ucData1 = IO_READ8(VIDEO_IN0_BASE, 0x80);
                mcSHOW_USER_MSG(("TVD noise level = %d\n", ucData1));
                //ATD_GetReg(psAtdeuDemodCtx, 0xA42, &ucData1, 1);
                u2Data = AtdEU_GetNoiseLevel();
                mcSHOW_USER_MSG(("ATD noise level = 0x%08X\n", u2Data));
                //ATD_GetReg(psAtdeuDemodCtx, 0x249, &ucData1, 1);
                //mcSHOW_USER_MSG(("Impulse noise indicator = %d\n", (ucData1 & 0x80) >> 7 ));
                ucData1 = 0x2E;
                ATD_SetReg(psAtdeuDemodCtx, 0x96C, &ucData1, 1);
                ATD_GetReg(psAtdeuDemodCtx, 0x96D, &ucData1, 1);
                ATD_GetReg(psAtdeuDemodCtx, 0x96E, &ucData2, 1);
                u2Data = mcUNSIGN_2BYTE(ucData1, ucData2, 8, 8);
                mcSHOW_USER_MSG(("DRO gain jitter = 0x%04X\n", u2Data));
                ATD_GetReg(psAtdeuDemodCtx, 0xA39, &ucData1, 1);
                ATD_GetReg(psAtdeuDemodCtx, 0xA3A, &ucData2, 1);
                u2Data = mcUNSIGN_2BYTE(ucData1, ucData2, 8, 8);
                mcSHOW_USER_MSG(("DRO signal swing = 0x%04X\n", u2Data));
                ATD_GetReg(psAtdeuDemodCtx, 0xA3B, &ucData1, 1);
                ATD_GetReg(psAtdeuDemodCtx, 0xA3C, &ucData2, 1);
                u2Data = mcUNSIGN_2BYTE(ucData1, ucData2, 8, 8);
                ...
                ATD_GetReg(psAtdeuDemodCtx, 0x95E, &ucData1, 1);
            ...
            case 'p':
              ...
              mcSHOW_USER_MSG(("Re-try TVD delay = %d ms\n",psAtdeuDemodCtx->pTCtx->u2RetryTVDTm)); 
		       ...
		       else
			   {
                ...
                ATD_SetReg(psAtdeuDemodCtx, 0x998, &ucData1, 1);
                ATD_SetReg(psAtdeuDemodCtx, 0x9B2, &ucData1, 1);
                ucData1 = 0x40 + ucData2;
                ATD_SetReg(psAtdeuDemodCtx, 0x9A9, &ucData1, 1);
                ATD_SetReg(psAtdeuDemodCtx, 0x9B3, &ucData1, 1);
...
TUNER_TYPE_DIAG_CONTENT_T *pDiagContent = (TUNER_TYPE_DIAG_CONTENT_T *) pv_get_info;
    ...
    switch (e_tuner_get_type)
    {
        ...
        // Record 2
            ...
            sprintf(pDiagContent->cDisplayValue, "%6d", psAtdPdCtx->Frequency);
            pDiagContent++;
