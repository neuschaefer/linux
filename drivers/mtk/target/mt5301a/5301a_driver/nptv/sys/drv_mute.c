...
static VOID vDrvDumpVideoMuteModuleInfo(UINT32 u4Path, MUTE_MODULE_ID eModule)
{
    ...
    LOG(3, "Path=%d Module=%2d (%s) \tInvalid=%d Count=%d Info=0x%x\n",
           u4Path, eModule, _aszMuteModuleStr[eModule],
           _rVideoMuteInfo[u4Path][eModule].bInvalid, 
...
VOID _vDrvDumpVideoMuteModule(UINT32 u4Path, UINT32 u4ForceDumpInfo)
{
    ...
    for (u4Module = 0; u4Module < MUTE_MODULE_MAX; u4Module++)
    {
        if (_rVideoMuteInfo[u4Path][u4Module].bInvalid)
            continue;

        if (u4ForceDumpInfo)
        {
            Printf("Module = %2d (%s) \tDelay conter = %d\n",
               u4Module, _aszMuteModuleStr[u4Module],
               _rVideoMuteInfo[u4Path][u4Module].u4Delay);
        }
        else
        {            
            LOG(3, "Module = %2d (%s) \tDelay conter = %d\n",
               u4Module, _aszMuteModuleStr[u4Module],
               _rVideoMuteInfo[u4Path][u4Module].u4Delay);
        }
    }
...
UINT32 _u4DrvVideoGetMute(UINT32 u4Path)
{
    ...
    return _au4LastMuteState[u4Path];
}
...
*/
    ...
    bInvalid = _rVideoMuteInfo[u4Path][eModule].bInvalid;

    
    if (!bInvalid && (u4UnMuteDelay > 0))
    {
        VIDEO_MUTE(u4Path, SV_ON);
                                  ...
                                  UINT32 u4Path, BOOL bInvalid)
{
    ...
    rCritState = x_crit_start();
    _rVideoMuteInfo[u4Path][eModule].bInvalid = bInvalid;
    x_crit_end(rCritState);
}
#if 0
...
void vDrvSetForceUmute(INT32 u4Path, UINT8 u1MuteType, BOOL bEnable)
{
	switch(u1MuteType)
	{
		case SV_FORCE_UNMUTE_TYPE_ON_NO_SIG:
		{
			...
			_vDrvVideoSetMuteInvalidFlag(MUTE_MODULE_MODECHG, u4Path, bEnable);
			_vDrvVideoSetMuteInvalidFlag(MUTE_MODULE_API_FORCE, u4Path, bEnable);
		}
		break;
		case SV_FORCE_UNMUTE_TYPE_ATV_CHG_CH:
		{
			...
			_vDrvVideoSetMuteInvalidFlag(MUTE_MODULE_MODECHG, u4Path, bEnable);
			_vDrvVideoSetMuteInvalidFlag(MUTE_MODULE_API_FORCE, u4Path, bEnable);
			_vDrvVideoSetMuteInvalidFlag(MUTE_MODULE_MDDI01, u4Path, bEnable);
		}
		break;
		default:
		break;
