...
#else
        force_color_sys = _bMAIN_COLOR_SYS;
#endif
...
#else
    RVChannel * pt_rv_channel = &_rMChannel;
#endif
...
//    UINT8 bRefreshRate;
    ...
    if ((bRefreshRate == 25) || (bRefreshRate == 50))
    {
        bLCDFreqType = SV_DCLK_50HZ;
    }
    else if ((bRefreshRate == 30) || (bRefreshRate == 60))
    {
        bLCDFreqType =SV_DCLK_60HZ;
    }
    else if (bRefreshRate == 24)
    {
    ...
    else
    {
        ...
        if (bRefreshRate < 55)
        {
            u4Clk = dwSI_DISPLAY_DCLK_50HZ/50 * bRefreshRate;
            bLCDFreqType = SV_DCLK_50HZ;
        }
        else
        {
            u4Clk = dwSI_DISPLAY_DCLK_TYPICAL/60 * bRefreshRate;
            bLCDFreqType = SV_DCLK_60HZ;
        }
...
#ifdef TIME_MEASUREMENT
        TMS_DIFF_EX(TMS_FLAG_CHG_CHL, TMS_CHL_CHE_TIME, "VIDEO_STABLE");
        ...
        case VDO_STATE_WAIT_MODE_DET:
            if (fgIsMainDec())
            {
		      ...
		      bSigMode = bDrvVideoSignalStatus(SV_VP_MAIN);
                    ...
                    if (fgIsSrcAtv(SV_VP_MAIN) && (bSigMode == (UINT8)SV_VDO_NOSIGNAL))
                    {
                        //_u4MainDelayUnmute=DELAY_MODE_CHG_UNMUTE;
                        // mute  more time to wait AP mute 
                        _vDrvVideoSetMute(MUTE_MODULE_MODECHG, SV_VP_MAIN, _u4MainDelayUnmute*2, FALSE);
                    }
                    else 
                    #endif // end of SUPPORT_ATV_SNOWSCREEN
                   ...
                   // if (IS_COMPANION_CHIP_ON())
                    ...
                    if (fgIsMainDec())
                    {
						...
						{
							bSigMode =_rTvd3dStatus.bSigStatus ;	
						}
#endif	
		 ...
		 if(_fgFirstPIP==FALSE)
               {
                ...
                if (flag)
                {
                    ...
                    // ---Handle mode change unmute--------
                    #if SUPPORT_ATV_SNOWSCREEN && SUPPORT_POP
                    if (fgIsSrcAtv(SV_VP_MAIN) && (bSigMode == (UINT8)SV_VDO_NOSIGNAL))
                    {
                        //_u4MainDelayUnmute=DELAY_MODE_CHG_UNMUTE;
                        // mute  more time to wait AP mute
                        _vDrvVideoSetMute(MUTE_MODULE_MODECHG, SV_VP_PIP, _u4PipDelayUnmute*2, FALSE);
                    }
                    else 
                    #endif // end of SUPPORT_ATV_SNOWSCREEN
                    if (bSigMode == (UINT8)SV_VDO_STABLE)
 ...
 */
#if SUPPORT_CEC
extern void _CEC_Mainloop(void);
#endif
...
void vApiVideoMainloop(void)
{
    ...
    #if SUPPORT_CEC
    if ((_u4BypassNptvMloop & BYPASS_NPTV_CEC) == 0)
    {
        //CEC state machine
        CEC_Mainloop();
    }
