...
UINT8 bInputType;                  //MC20081115 add for SCART
    ...
    vDrvGetHistogramInfoInVsync();        
    vDrvTriggerHistogramInVsync(); 
    ...
    if (fgIsVdoIntMainDet())
    {
        vApiApplyVideoEventInputVSync(SV_VP_MAIN);
    }
#if SUPPORT_POP
    if (fgIsVdoIntPipDet())
    {
         vApiApplyVideoEventInputVSync(SV_VP_PIP);
    }
#endif
    DRVCUST_VdoInISR();
}
...
PRIVATE void vOutVSyncISR(void)
{
		...
		vDrvDummyToRegister();
//        vDrvSCEFindColor(); remove for 8223 porting
#if 0// reserved for 120Hz Panel
    if ( (!fgIsSupport120HzPanel()) || (_u1VSyncCount==0))
    {
...
#endif
        #if CC_SCPOS_MUTE_RELATCH_WA
        if ((_u4PreviousMuteMask[SV_VP_MAIN] == 0) && dwDrvGetMuteMask(SV_VP_MAIN))
        {
            LOG(3, "Mute Take Effect in Main\n");
            vDrvScposSetHwMuteStatus(SV_VP_MAIN, SV_ON);
        }
        else if (dwDrvGetMuteMask(SV_VP_MAIN) == 0)
        {
            vDrvScposSetHwMuteStatus(SV_VP_MAIN, SV_OFF);
        }
        _u4PreviousMuteMask[SV_VP_MAIN] = dwDrvGetMuteMask(SV_VP_MAIN);
        if ((_u4PreviousMuteMask[SV_VP_PIP] == 0) && dwDrvGetMuteMask(SV_VP_PIP))
        {
            LOG(3, "Mute Take Effect in Sub\n");
            vDrvScposSetHwMuteStatus(SV_VP_PIP, SV_ON);
        }
        else if (dwDrvGetMuteMask(SV_VP_PIP) == 0)
        {
            vDrvScposSetHwMuteStatus(SV_VP_PIP, SV_OFF);
        }
        _u4PreviousMuteMask[SV_VP_PIP] = dwDrvGetMuteMask(SV_VP_PIP);
        #endif
