...
extern void vApiNotifySigChg(UINT8 u1Path,UINT8 u1SigState,UINT8 u1ReportState)
{
    ...
    {
        _arSigNfyInfo[u1Path].pfSigNfy((void*)u1Path, u1SigState, 0);
    }
#if 0    
...
extern void vAPiNotifyVPSCNIData(UINT8 au1Data[])
{
	if(_pfnVpsNfy != NULL)
	{
	    _pfnVpsNfy(au1Data);
...
extern void TvdNotifyVPSCNIData(UINT16 u2Data) __attribute__((weak));
...
void vAPiNotifyVPSCNIData(UINT16 u2data)
{
	if(TvdNotifyVPSCNIData!=0)
	{
		TvdNotifyVPSCNIData(u2data);
	}
#if 0 /*VPSCNI interface is different from MW_IF */
    if(_pfnVpsNfy != NULL)
    {
        _pfnVpsNfy(u2data);
    }
