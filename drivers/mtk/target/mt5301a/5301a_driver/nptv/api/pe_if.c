...
void vApiVideoDemoMode(UINT8 bmode, Region DemoRec, UINT8 bDEMOModules)
{
    ...
    switch (bmode)
    {
    case VDP_PQ_DEMO_ON_LEFT:
        bOnOff = SV_ON;
        bRegion = SV_LEFT;
        break;
    case VDP_PQ_DEMO_ON_RIGHT:
        bOnOff = SV_ON;
        bRegion = SV_RIGHT;
        break;
...
//    vDrvHSharpDemo(bOnOff, bRegion, (wInputWidth/2)/8);//remove Hsharp for 8223 porting
    vDrvNRDemo(bOnOff, bRegion,(wInputWidth/2));
    vDrvDIDemo(0, bOnOff, bRegion,(wInputWidth/2)+0x44);   // 0x44 is for MEMA Demo position alignment. why 0x44? just test the position, and got this difference value.

    vDrv2DSharpDemo(bOnOff, bRegion, wDispCenter);
    vDrvECTIDemo(bOnOff, bRegion, wDispCenter);
    vDrvColorDemo(bOnOff, bRegion, wDispCenter);
#endif
*/
}
...
//        case PE_ARG_BACK_LIGHT_LVL:
    ...
    case PE_ARG_CDS:
        ...
        if (i2Cur == 0)
        {
            vHalVideoCDSOnOff(SV_OFF);//remove CDS for 8223 porting
        }
        else
        {
            vHalVideoCDSOnOff(SV_ON);//remove CDS for 8223 porting
            vHalVideoCDSParam();//remove CDS for 8223 porting
        }
#endif
        break;
    case PE_ARG_LTI:
    ...
    case PE_ARG_G_GAIN:
		...
		Printf("G_gain after mapping = %d\n", bApiQtyMapping(u1SrcTypTmg, QUALITY_G_GAIN_LSB,
                                          &i2Min, &i2Max, &i2Dft, &i2Cur));
        if (_fgCallFromIsr == SV_OFF)
        {
    ...
    case PE_ARG_R_OFFSET:
		...
		Printf("R_offset after mapping = %d\n", ((UINT16) i2Cur -
                            (UINT16) ((i2Min + i2Max + 1) / 2)));
