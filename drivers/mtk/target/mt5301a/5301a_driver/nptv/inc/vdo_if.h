...
/**
 * @brief bApiVideoFixColorCurState(UINT8 bPath)
 * get video path's fix color space current state
 * @param  bPath :main/sub path (SV_VP_MAIN / SV_VP_PIP)
 */
EXTERN UINT8 bApiVideoFixColorCurState(UINT8 bPath);

/**
 * @brief vApiVideoFixColorSpace(UINT8 bPath,UINT8 bMode)
 * force(fix) video path's color space setting
 * @param  bPath :main/sub path (SV_VP_MAIN / SV_VP_PIP)
 * @param  bMode : 0,1,2,17,18 (ref sv_const.h)
 */
EXTERN void vApiVideoFixColorSpace(UINT8 bPath, UINT8 bMode);
