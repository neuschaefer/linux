...
#define TARGET_Y_LEVEL_N216 0x203//515 for SONY 16~216
...
static UINT8 _sbTvdModeDeglish(BOOL *pfgVPres , UINT8 *pbTvdMode, BOOL *pfgIs525 )
{
		...
		(fgPreIs525_1 != fgHwTvd525()))
	{
		...
		bPreTvdMode_1 = bHwTvdMode();
		fgPreIs525_1 = fgHwTvd525();
		LOG(6,"fgPreVPres_1=%d\n",fgPreVPres_1);
		LOG(6,"bPreTvdMode_1=%d\n",bPreTvdMode_1);
		LOG(6,"fgPreIs525_1=%d\n",fgPreIs525_1);
	}
		...
		)
	{
		if(fgDrvTvdIsPreModeCnt(TVD_PreMCDONE_CNT_Vpres))
		{
			if((_sfgPreVPres_2 != fgPreVPres_1)||(_sfgPreIs525_2 != fgPreIs525_1))
			{
				...
				LOG(6,"TVD_PreMCDONE_CNT_Vpres mode change\n");
			}
		}
		if(fgDrvTvdIsPreModeCnt(TVD_PreMCDONE_CNT_Mode))
		{
			...
			_sbPreTvdMode_2 = bPreTvdMode_1;
			_sfgPreIs525_2 = fgPreIs525_1;
			LOG(6,"TVD_PreMCDONE_CNT_Mode mode change\n");
		}
	}
	else
	{
...
{    
    ...
    if (fgDrvTvdIsChanchgCnt(CHANCHG_DONE))
    {
        if (weer_vlen > 20)
        {
        	vRegWriteFldAlign(TG_08,SV_ON,FAST_VALIGN);
...
{	 
  ...
  if ((fgTrick && (!(pTvd3dStatus->eSourceType==SV_ST_TV))) ||
	...
	{
		//To fix PICO Vpres toggle.//VPRES1 && VPRES2 for SHARP tuner
...
void vTvd3dConnect(UINT8 bPath, UINT8 bOnOff)
{
	...
	vRegWriteFldAlign(CDET_0F, CDET_SB4CTG_TH, SB4CTG_TH);//for SONY VCR issue
...
//LOG(1,"DCODE END\n");
		...
		_swY_Avg_Lvl = (_swY_Avg_Lvl * 27) >> 6;//YUV2YCbCr
...
void vSVPresDet(void)
{
	...
	if (fgSVPres == _rTvd3dStatus.fgSVPres)
	{
		...
		if (fgDrvTvdIsSFirstCnt(TVD_SFIRST_CNT))
		{
...
UINT8 bTvdCtrl(UINT8 bItem,UINT8 bCmd,UINT32 bArg) reentrant
{
	...
	switch(bItem)
	{
    	...
    	case TCTL_BP_MODECHG:
    		_srTvdCtrl[bItem].val=_srTvdCtrl[bItem].en;
    		break;
    	case TCTL_CKILL:
    		if(bCmd==TC_ENABLE)
    		{
