...
void vScposInit(UINT8 bPath)
{
    ...
    // init dram burst mode, yulia
    #if SCALER_DRAM_BURST_MODE
    vRegWriteFldAlign(SCPIP_PIP1_05, 1, PIP1_05_DRAM_BURST_MODE_1);
    #else
    ...
    if (bPath == SV_VP_MAIN)
    {
        ...
        vRegWriteFldAlign(SCPIP_PIP1_07, 6, PIP1_07_DISPMODE_H_POS_1);
        vRegWriteFldAlign(SCPIP_US1_10, 1, US1_10_REG_PSCAN_DISP_EN_1);
        vRegWriteFldAlign(SCPIP_US1_10, 6, US1_10_REG_CLEAR_SEL_1);
        vRegWriteFldAlign(SCPIP_US1_10, 26, US1_10_REG_DISP_PRETFETCH_1);
...
void vScpipPScanDispmodeWaterCtrl(UINT8 bPath, UINT8 bOnOff)
{
    ...
    if(bOnOff==SV_ON)
    {
        if (bPath == SV_VP_MAIN)
        {
            ...
            vRegWriteFldAlign(SCPIP_PIP_OA_0A, 0x70, PIP_OA_0A_WTHR31);
	    vRegWriteFldAlign(SCPIP_PIP_OA_0A, 0x68, PIP_OA_0A_WTHR21);
...
#else
    bCline = RegReadFldAlign(SCPIP_PIP_OA_06,  PIP_OA_06_C_LINE1);
    bCfgCline = RegReadFldAlign(SCPIP_PIP_OA_06,  PIP_OA_06_CFG_LINE1);
#endif
...
void vDrvConfigHardwareMute(void)
{
	...
	if (fgIsSrcMM())//in mm quick off mute should off too.
	{
	      //disable quick off mute
            vDrvConfigQuickMute(SV_VP_MAIN,SV_OFF);
	}
