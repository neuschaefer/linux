...
#define LETTER_BOX_IIR_VALID_DRIFT_1        2       // 2%
#define LETTER_BOX_IIR_VALID_DRIFT_2        4       // 4%
#define LETTER_BOX_IIR_TAP_FRACTION         3
...
#define LETTER_BOX_FCNT_BOUND               0x10
#define LETTER_BOX_Y_THRESHOLD              0x28
#define LETTER_BOX_LOOP_DELAY               100
#define LETTER_BOX_HARDWARE_DELAY           90
#define LETTER_BOX_APATIVED_THD_REFLASH         20
#define LETTER_BOX_APATIVED_THD_DARK            16       // 80%, Too Dark
#define LETTER_BOX_APATIVED_THD_NS              4
#define LETTER_BOX_APATIVED_THD_DEFAULT         0       // 32
#define LETTER_BOX_APATIVED_THD_TOO_DARK        2       // 0~48 > 75%
#define LETTER_BOX_CHROMA_MAX_DELTA             20    
...
#endif
UINT32 _u4LBoxDetectionInit = 0;
UINT32 _u4LBoxDetectionEnable = 0;
UINT32 _u4LBoxDetectionValid = 0;
UINT32 _u4LBoxDetectionFixed = 0;
...
UINT32 _u4LBoxSrcWidth = 0;
UINT32 _u4LBoxSrcHeight = 0;
UINT8 _u1LBoxSrcInterrupt = 0;
...
static UINT32 _u4LBoxBound1Low = 0;
static UINT32 _u4LBoxBound1Up = 0;
static UINT32 _u4LBoxBound1UpCounter = 0;
static UINT32 _u4LBoxBound1LowCounter = 0;
static UINT32 _u4LBoxBound2Low = 0;
static UINT32 _u4LBoxBound2Up = 0;
static UINT32 _u4LBoxBound2UpCounter = 0;
static UINT32 _u4LBoxBound2LowCounter = 0;

// Bound & Final & Current
static UINT32 _u4LboxInvalidDraft1 = 0;
static UINT32 _u4LboxInvalidDraft2 = 0;
static UINT32 _u4LBoxUpBound = 0;
static UINT32 _u4LBoxLowBound = 0;
static UINT32 _u4LBoxUpMax = 0;
static UINT32 _u4LBoxLowMax = 0;
static UINT32 _u4LBoxUpFinal = 0;
static UINT32 _u4LBoxLowFinal = 0;
static UINT32 _u4LBoxUpCurrent = 0;
static UINT32 _u4LBoxLowCurrent = 0;

// External Stable Time
static UINT32 _u4LBoxUpFinalBackup[3];
static UINT32 _u4LBoxLowFinalBackup[3];
...
static UINT32 _u4LBoxThresholdDarkCount = 0;
static UINT32 _au4LboxThresholdCount[LETTER_BOX_APATIVED_THD_NS] = { 0, 0, 0, 0 };
const UINT32 _au4LboxThresholdValue[LETTER_BOX_APATIVED_THD_NS] = { 32, 48, 64, 80 };     // 4 Threshold    

// Stable Time
static UINT32 _u4LBoxStableTime;
...
typedef struct
{
    UINT32 u4Bound1Counter;
    UINT32 u4Bound2Counter;
    UINT32 u4Bound1;
    UINT32 u4Bound2;
    UINT32 u4Final;
}LBOX_BOUND_STATUS;
static LBOX_BOUND_STATUS _arSta;
...
{            
    ...
    UINT32 u4LBoxX, u4LBoxY,u4LBoxWidth, u4LBoxHeight;
    #if SUPPORT_HORIZONTAL_LBOX
    u4InputWidth = (UINT32)wDrvVideoInputWidth(SV_VP_MAIN);
    u4LBoxX =_u4LBoxLeftFinal * u4InputWidth/_u4LBoxSrcWidth;
    u4LBoxWidth = (_u4LBoxRightFinal- _u4LBoxLeftFinal) * u4InputWidth/_u4LBoxSrcWidth; 
    //VDP_LBoxChangeNotify(u4InputWidth, u4InputHeight, u4LBoxWidth, u4LBoxHeight, u4LBoxX,u4LBoxY);
    LOG(3,"VDP_LBoxChangeNotify SrcW %d SrcH %d\n BoxW %d BoxH %d, BoxX %d, BoxY %d\n\n",
                u4InputWidth, u4InputHeight, u4LBoxWidth, u4LBoxHeight, u4LBoxX,u4LBoxY);
    #endif
    
    u4InputHeight = (UINT32)wDrvVideoInputHeight(SV_VP_MAIN);   
    u4LBoxY  = (_u1LBoxSrcInterrupt)? (_u4LBoxUpFinal<<1):(_u4LBoxUpFinal);
    u4LBoxHeight = (_u1LBoxSrcInterrupt)? ((_u4LBoxLowFinal-_u4LBoxUpFinal)<<1):(_u4LBoxLowFinal-_u4LBoxUpFinal);
    LOG(3,"VDP_LBoxChangeNotify SrcH %d\n  BoxH%d,  BoxY %d\n\n",
                 u4InputHeight,  u4LBoxHeight, u4LBoxY);
#endif     
...
static void LboxCheckHorizontalChroma(void)
{
    ...
    vRegWriteFldAlign(SIDE_LBOX_01, _u4LBoxLeftFinal - TONE_DETECT_RANGE, C_LEFT_BOUND);
    vRegWriteFldAlign(SIDE_LBOX_01, _u4LBoxRightFinal + TONE_DETECT_RANGE, C_RIGHT_BOUND);
    vRegWriteFldAlign(SIDE_LBOX_05, _u4LBoxLeftFinal, C_LEFT_TONE_SEPERATE);
    vRegWriteFldAlign(SIDE_LBOX_05, _u4LBoxRightFinal, C_RIGHT_TONE_SEPERATE);
...
static void LboxCheckVerticalChroma(void)
{
    ...
    // Set Tone detect bound based on Y detected bound.
    vRegWriteFldAlign(SIDE_LBOX_01, _u4LBoxLeftFinal, C_LEFT_BOUND);
    vRegWriteFldAlign(SIDE_LBOX_01, _u4LBoxRightFinal, C_RIGHT_BOUND);
    vRegWriteFldAlign(SIDE_LBOX_05, _u4LBoxRightFinal, C_LEFT_TONE_SEPERATE);
    vRegWriteFldAlign(SIDE_LBOX_05, _u4LBoxLeftFinal, C_RIGHT_TONE_SEPERATE);
...
static void LboxAdaptiveThreshold(void)
{
    ...
    {    
            ...
            (u4Best == 0))
        {
            ...
            if (u4Idx < LETTER_BOX_APATIVED_THD_NS)
            {
                UINT32 u4Target;
        ...
        if ((au2Hist[u4Idx] > (au2Hist[u4Idx + 1] + _u4LBoxThresholdEdge)) &&
            (u4Best == 0))
        {
            // edge detect
...
/*
    if (_u4UpdateFlag)
    {
        // During source selection, reset lbox detection in fast mode
        HAL_GetTime(&_rStartTime);
        _u4UpdateModeDetectTime = 1;
        _u4UpdateFlag = 0;
    }
    
    // After 30 seconds, reset lbox detection in slow mode
    HAL_GetTime(&rCountTime);
    HAL_GetDeltaTime(&rDeltaTime, &_rStartTime, &rCountTime);
    if ((rDeltaTime.u4Seconds >= LETTER_BOX_MODE_CHANGE_TIME) && (_u4UpdateModeDetectTime))
    {
        VDP_LBoxDetectMode(SV_VP_MAIN, 1); // slow mode
        LOG(1, "Slow Mode Detect!\n");
        _u4UpdateModeDetectTime = 0;
    }
*/
    // When change notify happen, bDrvVideoIsSrcInterlace return value NOT valid
    if (_u1LBoxSrcInterrupt != bDrvVideoIsSrcInterlace(SV_VP_MAIN))
    {
        UINT16 u2SrcHeight;
        _u1LBoxSrcInterrupt = bDrvVideoIsSrcInterlace(SV_VP_MAIN);

        u2SrcHeight = wDrvVideoInputHeight(SV_VP_MAIN);
        u2SrcHeight = _u1LBoxSrcInterrupt ? (u2SrcHeight>>1): u2SrcHeight;
        LOG(3, "New LBOX source height: %d --> %d\n", _u4LBoxSrcHeight, u2SrcHeight);
        _u4LBoxSrcHeight = u2SrcHeight;
    ...
    if (_u4LBoxSrcWidth != wDrvVideoGetPreScaleWidth(SV_VP_MAIN))
    {
        LOG(3, "New LBOX source width: %d --> %d\n", _u4LBoxSrcWidth, wDrvVideoGetPreScaleWidth(SV_VP_MAIN));
        _u4LBoxSrcWidth = wDrvVideoGetPreScaleWidth(SV_VP_MAIN);
    ...
    if ((_rMPicInfo.rOverScan.u4Top != _rLBoxOverScan.u4Top) ||
        (_rMPicInfo.rOverScan.u4Bottom != _rLBoxOverScan.u4Bottom) ||
        (_rMPicInfo.rOverScan.u4Left != _rLBoxOverScan.u4Left) ||
        (_rMPicInfo.rOverScan.u4Right != _rLBoxOverScan.u4Right))
    {
        _rLBoxOverScan = _rMPicInfo.rOverScan;
    ...
    if (u4ReConfig != 0)
    {
        ...
        #if 0
        u4LeftBound = (_u4LBoxSrcWidth * u4DetectBoundRatio) / 100;
        u4RightBound = _u4LBoxSrcWidth - u4LeftBound;
        #endif
...
//-----------------------------------------------------------------------------
/** Brief of LboxUpdateStatus.
 */
//-----------------------------------------------------------------------------
static void LboxUpdateStatus(UINT32 _u4Bound1Counter, UINT32 _u4Bound2Counter, UINT32 _u4Bound1, UINT32 _u4Bound2, UINT32 _u4Final)
{
    _arSta.u4Bound1Counter  = _u4Bound1Counter;
    _arSta.u4Bound2Counter  = _u4Bound2Counter;
    _arSta.u4Bound1 = _u4Bound1;
    _arSta.u4Bound2 = _u4Bound2;    
    _arSta.u4Final = _u4Final;
}
...
//-----------------------------------------------------------------------------
#if SUPPORT_HORIZONTAL_LBOX    
static void LboxHorizontalBoundSelect(UINT8 fgIsLeftRight, UINT32 _u4Bound1Counter, UINT32 _u4Bound2Counter, UINT32 _u4Bound1, UINT32 _u4Bound2, UINT32 _u4Final)
{
    UINT32 u4SmallBoundNs;
    u4SmallBoundNs = 0;

    LboxUpdateStatus(_u4Bound1Counter, _u4Bound2Counter, _u4Bound1, _u4Bound2, _u4Final);
    
    // choose left bound
    if (_u4LBoxFastRecovery != 0)
    {
        // choose left bound
        if ((_arSta.u4Bound1Counter == 0) &&
            (_arSta.u4Bound2Counter == 0))
        {
            // do nothing
        }
        else
        if (_arSta.u4Bound1Counter == 0)
        {
            ...
            _arSta.u4Final = _arSta.u4Bound2;
        }
        else
        if (_arSta.u4Bound2Counter == 0)
        {
            // Bound#2 NOT detected! Use Bound#1
            
            _arSta.u4Final = _arSta.u4Bound1;
        }
        else
        {
            ...
            {
                if (_arSta.u4Bound1 > _arSta.u4Bound2)
                {
                    _arSta.u4Final = _arSta.u4Bound2;
                }
                else
    ...
    {
        u4SmallBoundNs = (_arSta.u4Bound1Counter + _arSta.u4Bound2Counter) / LETTER_BOX_SMALL_BOUND;
        ...
        if ((_arSta.u4Bound1Counter < u4SmallBoundNs) &&
            (_arSta.u4Bound2Counter > u4SmallBoundNs))
        {
            // Bound#1 NOT detected! Use Bound#2
        ...
        if ((_arSta.u4Bound2Counter < u4SmallBoundNs) &&
            (_arSta.u4Bound1Counter > u4SmallBoundNs))
        {
            // Bound#2 NOT detected! Use Bound#1
        ...
        else
        {
            // Two Bound detected! Caption/Subtitle Case
            // Pick envelop of Bound#1 & Bound#2
            LOG(4, "Two Up Bound detected! Caption/Subtitle Case! B1(%d, %d), B2(%d, %d)\n", _arSta.u4Bound1, _arSta.u4Bound1Counter, _arSta.u4Bound2, _arSta.u4Bound2Counter);
    ...
    if (fgIsLeftRight) // Left
    {
        _u4LBoxBound1LeftCounter = _arSta.u4Bound1Counter;
        _u4LBoxBound2LeftCounter = _arSta.u4Bound2Counter;
        _u4LBoxBound1Left = _arSta.u4Bound1;
        _u4LBoxBound2Left = _arSta.u4Bound2;
        _u4LBoxLeftFinal  = _arSta.u4Final;
    }
    else // Right
    {
        _u4LBoxBound1RightCounter = _arSta.u4Bound1Counter;
        _u4LBoxBound2RightCounter = _arSta.u4Bound2Counter;
        _u4LBoxBound1Right = _arSta.u4Bound1;
        _u4LBoxBound2Right = _arSta.u4Bound2;
        _u4LBoxRightFinal  = _arSta.u4Final;
    }
}
#endif
 ...
 */
//-----------------------------------------------------------------------------
static void LboxVerticalBoundSelect(UINT8 fgIsDirect, UINT32 _u4Bound1Counter, UINT32 _u4Bound2Counter, UINT32 _u4Bound1, UINT32 _u4Bound2, UINT32 _u4Final)
{
    UINT32 u4SmallBoundNs;
    u4SmallBoundNs = 0;
    ...
    if (fgIsDirect) // fgIsDirect:1 -> Up
    {
        _u4LBoxBound1UpCounter = _arSta.u4Bound1Counter;
        _u4LBoxBound2UpCounter = _arSta.u4Bound2Counter;
        _u4LBoxBound1Up = _arSta.u4Bound1;
        _u4LBoxBound2Up = _arSta.u4Bound2;
        _u4LBoxUpFinal  = _arSta.u4Final;
    }
    else // fgIsDirect:0 -> Low
    {
        _u4LBoxBound1LowCounter = _arSta.u4Bound1Counter;
        _u4LBoxBound2LowCounter = _arSta.u4Bound2Counter;
        _u4LBoxBound1Low = _arSta.u4Bound1;
        _u4LBoxBound2Low = _arSta.u4Bound2;
        _u4LBoxLowFinal  = _arSta.u4Final;
    }
}
 ...
 */
//-----------------------------------------------------------------------------
static void LboxBoundUpdate(UINT8 fgIsDirect, UINT32 _u4Bound1Counter, UINT32 _u4Bound2Counter, UINT32 _u4Bound1, UINT32 _u4Bound2, UINT32 _u4Final, UINT32 _u4Draft, UINT32 u4New, UINT32 u4Small)
{
    ...
    LboxUpdateStatus(_u4Bound1Counter, _u4Bound2Counter, _u4Bound1, _u4Bound2, _u4Final);
    LOG(4, "_u4Bound1Counter: %d _u4Bound2Counter: %d _u4Bound1: %d _u4Bound2: %d _u4Final: %d\n", _u4Bound1Counter, _u4Bound2Counter, _u4Bound1, _u4Bound2, _u4Final);

    u4MatchBound1 = 0;
    u4MatchBound2 = 0;
    if (LboxBoundStable(u4New, _arSta.u4Bound1, _u4Draft))
    {
        u4MatchBound1 = 1;
    }

    if (LboxBoundStable(u4New, _arSta.u4Bound2, _u4Draft))
    {
        u4MatchBound2 = 1;
    }
    ...
    if (u4MatchBound1 == u4MatchBound2)
    {
        ...
        if (u4MatchBound1 != 0)
        {
            ...
            if (u4New > _arSta.u4Bound1)
            {
                u4Delta1 = (u4New - _arSta.u4Bound1);
            }
            else
            {
                u4Delta1 = (_arSta.u4Bound1 - u4New);
            }

            if (u4New > _arSta.u4Bound2)
            {
                u4Delta2 = (u4New - _arSta.u4Bound2);
            }
            else
            {
                u4Delta2 = (_arSta.u4Bound2 - u4New);
            }

            if (u4Delta1 > u4Delta2)
            {
                _arSta.u4Bound2 = u4New;
            }
            else
            {
                _arSta.u4Bound1 = u4New;
            }
        }
        else
        if (_arSta.u4Bound1Counter > _arSta.u4Bound2Counter)
        {
            // Update Bound#2
            _arSta.u4Bound2 = u4New;
    ...
    if (fgIsDirect == 1) // fgIsDirect:1 -> Up
    {
        ...
        if ((_arSta.u4Bound1 < _arSta.u4Final) &&
            ((_arSta.u4Bound1 + LETTER_BOX_FAST_RECOVERY_BOUND) < _arSta.u4Final) &&
            (_arSta.u4Bound1Counter > LETTER_BOX_SMALL_BOUND_MIN) && (u4Small == 0) && (u4New == _arSta.u4Bound1))
        {
            _u4LBoxFastRecovery = 1;
        }

        if ((_arSta.u4Bound2 < _arSta.u4Final) &&
            ((_arSta.u4Bound2 + LETTER_BOX_FAST_RECOVERY_BOUND) < _arSta.u4Final) &&
            (_arSta.u4Bound2Counter > LETTER_BOX_SMALL_BOUND_MIN) && (u4Small == 0) && (u4New == _arSta.u4Bound2))
        {
            _u4LBoxFastRecovery = 1;
        }
 ...
 */
//-----------------------------------------------------------------------------
static void LboxUploadCounter(UINT8 fgIsDirect, UINT32 _u4Bound1Counter, UINT32 _u4Bound2Counter, UINT32 _u4Bound1, UINT32 _u4Bound2, UINT32 _u4Draft, UINT32 u4New, UINT32 u4Ori)
{
    LboxUpdateStatus(_u4Bound1Counter, _u4Bound2Counter, _u4Bound1, _u4Bound2, _arSta.u4Final);
    
    // Upload Counters
    if (u4New == _arSta.u4Bound1)
    {
        // Bound#1
        _arSta.u4Bound1Counter++;
    }
    else
    if (u4Ori == _arSta.u4Bound1)
    {
        // Bound#1
        _arSta.u4Bound1Counter++;
    }
    ...
    if (LboxBoundStable(u4Ori, _arSta.u4Bound1, _u4Draft))
    {
        // Bound#1
        _arSta.u4Bound1Counter++;
    }
    ...
    {
        // Bound#2
        _arSta.u4Bound2Counter++;
    }
    else
    if (u4Ori == _arSta.u4Bound2)
    {
        // Bound#2
        _arSta.u4Bound2Counter++;
    }
    ...
    if (LboxBoundStable(u4Ori, _arSta.u4Bound2, _u4Draft))
    {
        // Bound#2
        _arSta.u4Bound2Counter++;
    }
...
static void LboxDetectionGetResult(void)
{
    ...
    if (++_u4LBoxIIRCount > LETTER_BOX_IIR_TAP)
    {
        ...
        if ((rDeltaTime.u4Seconds >= _u4LBoxStableTime) ||
            (_u4LBoxFastRecovery != 0))
        {
            ...
            // Check Chroma
            if ((_u4LBoxUpFinal == _u4LBoxUpBound) &&
                (_u4LBoxLowFinal == _u4LBoxLowBound)
#if SUPPORT_HORIZONTAL_LBOX        
                && (_u4LBoxLeftFinal == _u4LBoxLeftBound) 
                && (_u4LBoxRightFinal == _u4LBoxRightBound)
            ...
            (rDeltaTime.u4Seconds >= LETTER_BOX_BOUND_TIME))            
        {
            ...
            if (_u4LBoxFastRecovery != 0)
            {
                LOG(4, "Fast Recovery B1(%d, %d), B2(%d, %d), FINAL(%d %d)\n",
                    _u4LBoxBound1Up, _u4LBoxBound1Low,
                    _u4LBoxBound2Up, _u4LBoxBound2Low,
                    _u4LBoxUpFinal, _u4LBoxLowFinal);
            }
#endif
        }
        ...
        LOG(6, "H_NEW(%d %d), IIR(%d %d), B1(%d, %d), B2(%d, %d)\n",
            u4LeftMin, u4RightMax,
            u4NewLeft, u4NewRight,
            _u4LBoxBound1Left, _u4LBoxBound1Right,
            _u4LBoxBound2Left, _u4LBoxBound2Right);
#endif
...
static void LboxDetectionLoop(void *pvArgs)
{
    ...
    while (1)
    {
        while (_u4LBoxDetectionEnable != 0)
        {
            ...
            if ((_rMChannel.bIsChannelOn != 0) &&
                (bDrvVideoSignalStatus(SV_VP_MAIN) == (UINT8) SV_VDO_STABLE) &&
                (_u4LBoxDetectionFixed == 0) && 
                (u1Dispmode != VDP_SCPOS_DISPMODE_AUTO) && (u1Dispmode != VDP_SCPOS_DISPMODE_MANUAL))
            {
                // reconfig
                LboxDetectionConfig();
            ...
            if ((_u4LBoxDetectionFixed != 0) || (u1Dispmode == VDP_SCPOS_DISPMODE_AUTO) || (u1Dispmode == VDP_SCPOS_DISPMODE_MANUAL))
            {
...
UINT8 LboxQueryRegion(UINT32 *pu4LBoxX, UINT32 *pu4LBoxY, UINT32 *pu4Width, UINT32 *pu4Height, UINT32 *pu4HValid, UINT32 *pu4VValid)
{
  ...
  //  UINT32 u4IgnoreOverscanV;
    ...
    *pu4VValid = 0;
    ...
    #endif
    u4LBoxY = 0;
   // u4IgnoreOverscanV = *pu4IgnoreOverscanV;
    
    //[DTV00019983] Can't Auto detect letter box in HDMI mode, Peggy20071220 
    if ((_u4LBoxDetectionEnable != 0) && (_u4LBoxDetectionValid != 0) && (!bIsScalerInput444(SV_VP_MAIN)))   // 444 can not use letterbox detection (detect G only!)
    {
...
#endif
    *pu4LBoxY = u4LBoxY;
//    *pu4IgnoreOverscanV = u4IgnoreOverscanV;
...
void LboxSourceChange(UINT32 u4SrcWidth, UINT32 u4SrcHeight, UINT8 u1SrcInterlace)
{
        ...
        _u1LBoxSrcInterrupt = u1SrcInterlace;
