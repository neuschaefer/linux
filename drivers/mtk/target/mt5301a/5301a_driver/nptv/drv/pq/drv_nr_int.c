...
//    {NR_2DNR20,     0x80000000, 0x80000000},  //detail extend    
    ...
    {NR_2DNR63,     0x00000200, 0x00000700},
    {NR_REGTBL_END, 0x00000000, 0x00000000}
};
...
void vDrvMPEGNRParam(UINT8 bCurUI)
{
	...
	vRegWriteFldAlign(NR_2DNR66 , (wReadQualityTable(QUALITY_2DNR_NEAREDGE_EDGE_THR) *2) /3, c_nearedge_edge_thr1);
}
...
BOOL IsGrayPattern(UINT8 bVLineCnt, UINT8 bSize8, UINT8 bSize16, UINT8 bSize24, UINT8 bSize32)
{
    ...
    wSatHistSum = 0;
    for(i=1;i<8;i++)        //compute saturation item 1 ~ 7, skip item 0
    {
        ...
        if( LumaIsStep )
    {
        ...
        else if( (bSize8>=(bSize8+bSize16+bSize24+bSize32-2)) &&  (bVLineCnt>=0x3A) )      //802BT, time=480i/480p, Pat=Grays64
        {                                                                                                                                        // -2: original case=> bSize8!=0, bSize8+bSize16+bSize32=0. but maybe not stable, so set variation = 2
