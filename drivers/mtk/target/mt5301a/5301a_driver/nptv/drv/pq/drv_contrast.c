...
volatile UINT8 bUpdateHistogramInfo = SV_FALSE;
volatile UINT8 bIsHistogramUpdating = SV_FALSE;
...
{
    if( bUpdateHistogramInfo )
    {
        // while updateing, avoid adaptive luma from retrieving information
        bIsHistogramUpdating = SV_TRUE;

        if(bDrvIsHistReady())
        {                        
            // while histogram ready, get histogram info and trigger for next read.
            vDrvGetHistInfo();

            vRegWriteFldAlign(METRIC_00, 0, METRIC_ENA);
            vRegWriteFldAlign(METRIC_00, 1, METRIC_ENA);
        }
        bUpdateHistogramInfo = SV_FALSE;
        
        bIsHistogramUpdating = SV_FALSE;        
    }   
}
...
void vDrvAutoContrast(void)
{
	...
	if (!bIsHistogramUpdating)
	{
        ...
        // notifying update histogram info in next input vsync
        // see vDrvGetHistogramInfoInVsync()
		bUpdateHistogramInfo = SV_TRUE;
	}
}
