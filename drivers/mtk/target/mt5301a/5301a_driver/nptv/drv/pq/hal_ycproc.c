...
//    vDrvPreSharpOnOff(bPath, bOnOff);//remove Hsharp for 8223 porting
...
void vHalVideoSharpnessParam(UINT8 bPath)
{
    if (bIsScalerInput444(bPath))
    {
//		vDrvPreSharpOnOff(bPath, SV_OFF);//remove Hsharp for 8223 porting
		if (bPath == SV_VP_MAIN)
		{
...
//    vDrvPreSharpParam(bPath);//remove Hsharp for 8223 porting
	if (bPath == SV_VP_MAIN)
	{
	    vDrvPostSharpParam();
	}
...
void vHalVideoCTIOnOff(UINT8 bPath, UINT8 bOnOff)
{
    ...
    if (bPath == SV_VP_MAIN)
    {
//        vDrvCTIFOnOff(bOnOff);  //remove ECTIF for 8223 porting
...
void vHalVideoCTIParam(UINT8 bPath)
{
    if (bIsScalerInput444(bPath))
    {
//        vDrvCTIFOnOff(SV_OFF);//remove ECTIF for 8223 porting
    ...
    if (bPath == SV_VP_MAIN)
    {
//        vDrvCTIFParam();//remove ECTIF for 8223 porting
        vDrvCTIRParam();
    }
}
...
//	vDrvCTIFOnOff(bOnOff);//remove ECTIF for 8223 porting
	vDrvLTIOnOff(bOnOff);
//    vDrvPreSharpOnOff(SV_VP_MAIN, bOnOff);//remove Hsharp fro 8223 porting
    vDrvPostSharpOnOff(bOnOff);
//    vDrvCDSOnOff(bOnOff);//remove CDS for 8223 porting
