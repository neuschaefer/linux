...
//#define HSHARP_OFFSET           0x40//remove Hsharp for 8223 porting
...
#endif
#if 0 //remove Hsharp for 8223 porting
...
#if 0 //remove PBC2~4 fro 8223 porting
...
#if 0 //remove CDS for 8223 porting
...
RRegDwrdTbl const REGTBL_LTI[] =
{
	...
	{REG_END, 0x00000000, 0x00000000},
#if 0 //remove HLTI for 8223 porting
...
{
//    vDrvPreSharpInit();//remove Hsharp for 8223 porting
    vDrvPostSharpInit();
//    vDrvCTIFInit();//remove ECTIF for 8223 porting
    vDrvCTIRInit();
//    vDrvCDSInit();//remove CDS for 8223 porting
    vDrvLTIInit();
...
//    vDrvSetHueHistBoundary(u1HueHistBoundary);//remove HUE Hist for 8223 porting
...
//    vDrvCDSOnOff(SV_OFF);//remove CDS for 8223 porting
...
#if 0 //remove SCEfindcolor for 8223 porting
 ...
 * @param bOnOff SV_ON/SV_OFF
 */
 #if 0 //remove ECTIF for 8223 porting
 ...
 * @param bOnOff SV_ON/SV_OFF
 */
 #if 0 //remoe CDS for 8223 porting
...
//	vRegWriteFldAlign(HLTI_00, bOnOff, HLTI_EN);//remove HLTI for 8223 porting
}
...
//		vRegWriteFldAlign(PBC_SHP2_01, 0, PBC_EN_2);//remove PBC2~4 for 8223 porting
//		vRegWriteFldAlign(PBC_SHP3_01, 0, PBC_EN_3);//remove PBC2~4 for 8223 porting
//		vRegWriteFldAlign(PBC_SHP4_01, 0, PBC_EN_4);//remove PBC2~4 for 8223 porting
	}
}
...
//PBC_1
	...
	vRegWriteFldAlign(PBC_SHP1_02,
	                  ...
	                  PBC_UPPER_BOUND_1);
	#if 0 //remove PBC2~4 for 8223 porting
//PBC_2
...
/**********************************   DEMO MODE   ****************************************/
/*****************************************************************************************/
//UINT16 wHSharpCenter;//remove Hsharp for 8223 porting
//UINT8 bHSharpRegion;//remove Hsharp for 8223 porting
UINT16 w2DSharpCenter;
UINT8 b2DSharpRegion;

#if 0 //remove Hsharp for 8223 porting
void vDrvHSharpDemoOnOff(UINT8 bPath, UINT8 bOnOff)
{
	if (bOnOff == SV_OFF)
	{
        vRegWriteFldAlign(HSHARP_MAIN_0D, 0, HSHARP_DEMO_EN);
	}
	else
	{
        vRegWriteFldAlign(HSHARP_MAIN_0D, 1, HSHARP_DEMO_EN);
        vRegWriteFldAlign(HSHARP_MAIN_0D,
		                  ((bHSharpRegion == SV_LEFT) ? 0 : 1), HBOUND_INV);
        vRegWriteFldAlign(HSHARP_MAIN_0E, wHSharpCenter, DEMO_RBOUND);
	}
}

void vDrvHSharpDemoSwitchRegion(UINT8 bRegion)
{
	bHSharpRegion = bRegion;
}

void vDrvHSharpDemoRegionParam(UINT16 wXCenter)
{
	wHSharpCenter = wXCenter;
}

void vDrvHSharpDemo(UINT8 bOnOff, UINT8 bRegion, UINT16 wXCenter)
{
	vDrvHSharpDemoSwitchRegion(bRegion);
	vDrvHSharpDemoRegionParam(wXCenter);
	vDrvHSharpDemoOnOff(SV_VP_MAIN, bOnOff);
}
#endif
void vDrvPostSharpDemoOnOff(UINT8 bOnOff)
{
	if (bOnOff == SV_OFF)
	{
		vRegWriteFldAlign(SP_Mask_03, 0, DEMO_EN);
        vRegWriteFldAlign(SP_Mask_00, 0, VERTICAL_PEAKING_BYPASS);
	}
	else
	{
        vRegWriteFldAlign(SP_Mask_03, 1, DEMO_EN);
        vRegWriteFldAlign(SP_Mask_00, 1, VERTICAL_PEAKING_BYPASS);
        vRegWriteFldAlign(SP_Mask_03, ((b2DSharpRegion == SV_LEFT) ? 1 : 0), DEMO_SX);
        vRegWriteFldAlign(SP_Mask_02, w2DSharpCenter, DEMO_HEND);
	}
}

void vDrvPostSharpDemoSwitchRegion(UINT8 bRegion)
{
	b2DSharpRegion = bRegion;
}

void vDrvPostSharpDemoRegionParam(UINT16 wXCenter)
{
	w2DSharpCenter = wXCenter;
}

void vDrv2DSharpDemo(UINT8 bOnOff, UINT8 bRegion, UINT16 wXCenter)
{
	vDrvPostSharpDemoSwitchRegion(bRegion);
	vDrvPostSharpDemoRegionParam(wXCenter);
	vDrvPostSharpDemoOnOff(bOnOff);
}

void vDrvECTIDemo(UINT8 bOnOff, UINT8 bRegion, UINT16 wXCenter)
{
	if (bOnOff == SV_ON) 
	{
		vRegWriteFldAlign(ECTI_0A, ((b2DSharpRegion == SV_LEFT) ? 1 : 0),
		                  ECTI_DEMO_SX);
        
		vRegWriteFldAlign(ECTI_09, 0, ECTI_HDEMO_START);
		vRegWriteFldAlign(ECTI_09, wXCenter, ECTI_HDEMO_END);
		vRegWriteFldAlign(ECTI_0A, 0, ECTI_VDEMO_START);
		vRegWriteFldAlign(ECTI_0A, 0xFFF, ECTI_VDEMO_END);
	}
    vRegWriteFldAlign(ECTI_0A, bOnOff, ECTI_DEMO_ENA);
}
/*****************************************************************************************/
/*********************************   3x3 Matrix   ****************************************/
/*****************************************************************************************/
...
{
    UINT8 bColorSys, bMonColorTransform, bHdtv;
    ...
    switch (bColorSys)
    {
        ...
        case SV_COLOR_HDMI_444_709:
            if (bIsScalerInput444(bPath))
            {
                if (bHdtv == SV_FALSE)
                {   // SDTV
                    if (bHDMIRgbRange() == SV_HDMI_RANGE_FULL) // full range
                    {
                        bMonColorTransform = SV_COLORTRANS_YCBCR601F_2_RGBF;
                    }
                    else     // limit range / default
                    {
...
/***************************************  Ambilight **************************************/
/*****************************************************************************************/
#if 0 //remove ambilight for 8223 porting
...
#if 0 //remove HUE Hist for 8223 porting
...
void vDrvColorDemoOnOff(UINT8 bOnOff)
{
	vRegWriteFldAlign(DBG_CFG, bOnOff, SPLIT_EN);
    vRegWriteFldAlign(0x781C, 0x80, AC_FULLB1); // Set Demo mode brightness in demo window.
}

void vDrvColorDemoSwitchRegion(UINT8 bRegion)
{
	vRegWriteFldAlign(DBG_CFG, ((bRegion == SV_LEFT) ? 0 : 1), SPLIT_SWAP);
}

void vDrvColorDemoRegionParam(UINT16 wXCenter)
{
	vRegWriteFldAlign(WIN_X_MAIN, wXCenter, WIN_X_START);
}

void vDrvColorDemo(UINT8 bOnOff, UINT8 bRegion, UINT16 wXCenter)
{
	vDrvColorDemoSwitchRegion(bRegion);
	vDrvColorDemoRegionParam(wXCenter);
	vDrvColorDemoOnOff(bOnOff);
}
                          ...
                          UINT16 bSatValue, UINT16 bHueValue)
{
    ...
    COLOR_TRANSFORM_ADJ[12] = wTableY2R[12] + ((bBriValue - 0x80) << DRVCUST_OptGet(eBrightInterval));
...
void vDrvPreprocSetFieldResync(UINT8 bPath)
{
	...
	#if 0 //remove PIP for 8223 porting
	else
	{
...
void vDrvYcprocProc()
{
	...
	//SHARPNESS adaptive control : Auto SHARPNESS
//	vDrvSharpProc();//remove Hsharp adaptive for 8223 porting
