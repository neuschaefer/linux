...
void vDrvCSOnOff(UINT8 bPath, UINT8 u1OnOff)
{
    if (DiPQMode.bCfgCSOnOff != u1OnOff)
    {
        ...
        MDDI_WRITE_FLD(bPath, MDDI_KC_00, u1OnOff, SD_5_LINE);
        MDDI_WRITE_FLD(bPath, MDDI_MULTI_00, u1OnOff, EN_MVR);
        MDDI_WRITE_FLD(bPath, MDDI_MULTI_00, u1OnOff, EN_MVW);
...
void vDrvCSOnOffEx(UINT8 bPath, UINT8 u1OnOff)
{
    ...
    vDrvCSOnOff(VDP_1, _arMDDiPrm[VDP_1].u1CsEnable);    
}
...
{
    UINT16 u2InputWidth = MDDI_READ_FLD(bPath, MDDI_SC_00, HDEW);
    UINT16 u2InputHeight = MDDI_READ_FLD(bPath, MDDI_MULTI_00, VDEW);
...
static void _vDrvDIGetPQStatusISR()
{
    ...
    {
        MDDI_WRITE_FLD(SV_VP_MAIN, MDDI_KC_04, i, HISTOGRAM_SEL);
...
void vDrvDISetInk(UINT8 bMode,UINT16 u2Type)
{
    ...
    MDDI_WRITE_FLD(SV_VP_MAIN, MDDI_OC_04, 0x10, OSD_Y_POS);
    MDDI_WRITE_FLD(SV_VP_MAIN, MDDI_OC_04, 0x10, OSD_X_POS);

    switch (bMode)
    {
        case 0:
            MDDI_WRITE_FLD(SV_VP_MAIN, MDDI_OC_04, SV_OFF, HIST_INK_ON);
            MDDI_WRITE_FLD(SV_VP_MAIN, MDDI_OC_04, SV_OFF, OSD_MODE);
            break;
        ...
        case 4: //CS ink            
            ...
            MDDI_WRITE_FLD(SV_VP_MAIN, MDDI_OC_04, SV_ON, HIST_INK_ON);
            MDDI_WRITE_FLD(SV_VP_MAIN, MDDI_OC_04, 3, OSD_MODE);
        ...
        case 6: //Film Ink 
            MDDI_WRITE_FLD(SV_VP_MAIN, MDDI_OC_04, 1, OSD_MODE);
            break;
...
void vDrvDICMotionStatusPolling(void *pvArgv)
{
    ...
    while (fgDIStatusPolling)
    {
        ...
        for (u4HistSel = 0; u4HistSel < DI_MOTION_HIST_BIN ; u4HistSel++)
        {
            MDDI_WRITE_FLD(SV_VP_MAIN, MDDI_KC_04, u4HistSel, HISTOGRAM_SEL);
