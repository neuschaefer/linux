...
//kal8225
//#ifdef VGA_AUTOCOLOR_TABLE_INCLUDE_FILE   //kal8225
//#include VGA_AUTOCOLOR_TABLE_INCLUDE_FILE
//#endif
...
PRIVATE void vDrvOnChipModeOnOff(UINT8 bOnOff)
{
    ...
    for (bCh = 0; bCh < 3; bCh++)
    {
        vRegWriteFldAlign(VFE_01 + (bCh << 4), 0, CLAMP_1_PWD); //don't power down clamp no matter what bOnOff=1 or 0
        if (bOnOff == 0)
        {
...
#endif
        vRegWriteFldAlign(VFE_03, 1, RGBREV_EN_BUFFER_V1_V2);   //to power down "ON CHIP auto color buffer and disable 825/450mv,
        vRegWriteFldAlign(VFE_03, 0, RGBREV_EN_CH123); //[SA7_Nick] 080520 add for Power Dwon channel buffer of ON CHIP AUTO
    }
    else
    {
        vIO32WriteFldAlign(CKGEN_AFECFG7, 0x0, FLD_RG_ADCPLL_CKO_SEL);
        //CKGEN_WRITE32(REG_ADCPLL_CFG0, ((CKGEN_READ32(REG_ADCPLL_CFG0) & 0xFFFFFF3F) | 0x00000000)); //Set ADC clock=27MHz
        vRegWriteFldAlign(VFE_03, 0, RGBREV_EN_BUFFER_V1_V2);   //to power down "ON CHIP auto color buffer and disable 825/450mv,
    }
...
{
       vRegWriteFldAlign(VFE_03, (1<<(bOrder)), RGBREV_EN_CH123); //5360 select channel buffer of ON CHIP AUTO COLOR 
}
...
PRIVATE void vDrvSelV1V2(UINT8 bLoopCnt_V1V2)
{
    if (bLoopCnt_V1V2 == 0)
    {
        vRegWriteFldAlign(VFE_03, 0, RGBREV_EN_V1); //turn off V1
        vRegWriteFldAlign(VFE_03, 1, RGBREV_EN_V2); //Select V2=825mv
    }
    else
    {
        vRegWriteFldAlign(VFE_03, 0, RGBREV_EN_V2); //turn off V2
        vRegWriteFldAlign(VFE_03, 1, RGBREV_EN_V1); //turn on V1=450mv
    }
}
...
#if AUTO_BLANK_DEBUG
static vDrvShowDigGainOffset(UINT8 bOrder)
{
...
static vDrvShowAnalogGainOffset(UINT8 bOrder)
{
    UINT8 bAgain;
    UINT8 bAOffset;
...
UINT8 bDigital_gain;
   ...
   if(_bOrder<3)  //for klocwork check
   {
...
#if PGA_4bit_offset
                  Printf("before bAdjOffset=0x%x\n",bAdjOffset);
            bAdjOffset = bAdjOffset >> 3;
            bAdjOffset = bAdjOffset << 3;
                  Printf("after bAdjOffset=0x%x\n",bAdjOffset);
            if (bAdjOffset == 0)
            {
                _bGainOffset_channel |= (0x01 << _bOrder);  //compensate limit
            }
            else if (bTempOffset [_bOrder] < (0xff - bAdjOffset))
            {
    ...
    else if (_wData_v1v2[_bOrder] < (_wADCTarget[_bOrder] - _bDiffOffset))
    {
        _bDoubleChkFlag[_bOrder] = 0;
        if (bTempOffset [_bOrder] > 0) //(_bInitOffset-MAX_ALLOWED_OFFSET_RANGE))
        {
            ...
            if (bAdjOffset == 0)
            {
                _bGainOffset_channel |= (0x01 << _bOrder);  //compensate limit
                  ...
                  else if (bTempOffset [_bOrder]>(1+bAdjOffset/2)
            {
...
{
#if CHK_PGA_LINEARITY    
     _bVFE_VERIFY_CASE = _bVFE_VERIFY_CASE|0x01;
...
void bDrvPrintAutocolorResult(void)
{
//UINT32 dTmpMax[3];
                //--------- to show the PGA offset /gain /blank levl /maximum level --------------
                // 1. read the blank level and maximum level
                for (_bOrder = 0;  _bOrder < 3; _bOrder++)
                {
                    vDrvOnChipAutoColorGetADCData(OFFSET_MODE);
                    //_wDiff_mim[_bOrder]=_wData_v1v2[_bOrder]>>1;              //Blank level (12 bits base)
                    _wDiff_mim[_bOrder]=_wData_v1v2[_bOrder]>>3;              //Blank level (10 bits base)
                     //dTmpMax[_bOrder] = bDrvOnChipGetADCMaxMinValue(_bOrder, AS_PHASE_MAX_SEL); //maximum level (8 bit base)
                    _wADC_Diff_value[_bOrder] = bDrvGetHDADCGain(_bOrder);   //Gain 
                    _wData_v1v2[_bOrder] = bDrvGetHDADCOffset(_bOrder);        //offset
                }
                #if 0// kal5301a
                mPrintf(" \n");
               if(_bType==INT_HDTV)
               {
                     mPrintf("HDTV: Offset_1= %uh ,Offset_2= %uh ,Offset_3= %uh\n",_wData_v1v2[0],_wData_v1v2[1],_wData_v1v2[2]);
                     mPrintf("HDTV: Gain1= %uh ,Gain2= %uh ,Gain3= %uh\n",_wADC_Diff_value[0],_wADC_Diff_value[1],_wADC_Diff_value[2]);
                     mPrintf("HDTV: 10 bits base: Blank1= %uh ,Blank2= %uh ,Blank3= %uh\n",_wDiff_mim[0],_wDiff_mim[1],_wDiff_mim[2]);
                     mPrintf("HDTV: Max1= %3ul ,Max2= %3ul ,Max3= %3ul\n",dTmpMax[0],dTmpMax[1],dTmpMax[2]);
               }
               else if(_bType==INT_VGA)
               {
                     mPrintf("VGA: Offset_1= %uh ,Offset_2= %uh ,Offset_3= %uh\n",_wData_v1v2[0],_wData_v1v2[1],_wData_v1v2[2]);
                     mPrintf("VGA: Gain1= %uh ,Gain2= %uh ,Gain3= %uh\n",_wADC_Diff_value[0],_wADC_Diff_value[1],_wADC_Diff_value[2]);
                     mPrintf("VGA: 10 bits base: Blank1= %uh ,Blank2= %uh ,Blank3= %uh\n",_wDiff_mim[0],_wDiff_mim[1],_wDiff_mim[2]);
                     mPrintf("VGA: Max1= %3ul ,Max2= %3ul ,Max3= %3ul\n",dTmpMax[0],dTmpMax[1],dTmpMax[2]);
                     
               }
               else if(_bType==INT_SCART)
               {
                     mPrintf("SCART: Offset_1= %uh ,Offset_2= %uh ,Offset_3= %uh\n",_wData_v1v2[0],_wData_v1v2[1],_wData_v1v2[2]);
                     mPrintf("SCART: Gain1= %uh ,Gain2= %uh ,Gain3= %uh\n",_wADC_Diff_value[0],_wADC_Diff_value[1],_wADC_Diff_value[2]);
                     mPrintf("SCART: 10 bits base: Blank1= %uh ,Blank2= %uh ,Blank3= %uh\n",_wDiff_mim[0],_wDiff_mim[1],_wDiff_mim[2]);
                     mPrintf("SCART: Max1= %3ul ,Max2= %3ul ,Max3= %3ul\n",dTmpMax[0],dTmpMax[1],dTmpMax[2]);
               }
               #endif
}
               ...
               bDrvPrintAutocolorResult();
                ...
                if (_bType == _bOrgType)
                {
                 ...
                 vDrvEnableADCLinearityVerify(16);
                    _bAutoColorState0 = VDO_AUTO_COLOR_END;
#elif (CHK_GAIN_OFFSET_ITERATION || CHK_BLANK_VARIATION )
                    _bAutoColorTimeOutCNT = 0;
                 Printf("--- To check Gain/offset iteration ---\n");
                    vDrvSetPGAGainOffsetFromEEPROM();
                    for (_bOrder = 0; _bOrder < 3; _bOrder++)
                    {
                        bTmp2 = bDrvGetHDADCGain(_bOrder) - 0x10;
                        bDrvSetHDADCGain(_bOrder, bTmp2);
                     Printf("channel=%x, initial gain=0x%x, initial offset=0x%x\n",_bOrder,bTmp2,_bInitOffset);
                    }
...
#if (CHK_GAIN_OFFSET_ITERATION || CHK_BLANK_VARIATION)
        case VDO_AUTO_COLOR_5_START:
            if (fgIsSP0FlgSet(SP0_AUTOCOLOR_FLG))
            {
//                vClrSP0Flg(SP0_AUTOCOLOR_FLG);
                _bAutoColorTimeOutCNT ++;
                if (_bAutoColorTimeOutCNT < 0x40)
                {
                    for (_bOrder = 0;  _bOrder < 3; _bOrder++)
                    {
                        vDrvOnChipAutoColorGetADCData(OFFSET_MODE);
...
{
#if CHK_ADC_LINEARITY
     _bVFE_VERIFY_CASE = (_bVFE_VERIFY_CASE|0x02);
     vDrvAsyncPreDataActive(0x20, 0x20);  //vDrvSetMeasureWindow(0xff);
    vDrvClearMAXvariable();  // to clear _dwMaxLevelAvg[];
    _bLocation = 0;
     _bADCLinearity = 0;
    ...
    if (_bType == INT_VGA)
    {
        switch (_bADC_LINEARITY_state)
        {
             ...
             case ADC_VERIFY_1:
                ...
                else
                {
                    ...
                    _dwAutoColorMax[2] = _dwMaxLevelAvg[2];
                     ...
                     _bADCLinearity =1;
                     Printf("---- The top level of 3 CH after auto color is: %x,  %x, %x\n",_dwAutoColorMax[0] ,_dwAutoColorMax[1] ,_dwAutoColorMax[2] );
                }
                break;
             ...
             case ADC_VERIFY_4:
                ...
                {
                     Printf("The level of 3 CH at location %2u are CH1:0x%4x,  CH2:0x%4x, CH3:0x%4x\n",_bLocation,_dwMaxLevelAvg[0] ,_dwMaxLevelAvg[1] ,_dwMaxLevelAvg[2] );
                    _dwMaxDiffTemp = 0;
                    for (_bOrder = 0; _bOrder < 3; _bOrder++)
                    {
                    ...
                    else
                    {
                        ...
                        for (_bOrder = 0; _bOrder < TOTAL_POINT; _bOrder++)
                        {
                           ...
                           Printf("Position %2u =0x%2x\n",_bOrder,_dwMaxDiff[_bOrder]);
                        }
                            
                                Printf("The maximum code difference (12 bit) between channel to channel is %ul\n", _dwMaxDiffLocal);
                ...
                else
                {
                    for (_bOrder = 0; _bOrder < 3; _bOrder++)
                    {
                        _dwMaxDiffTemp = _dwDeltaValue[_bOrder] * (TOTAL_POINT - 2 - _bLocation) + _dwEndPoint[_bOrder];
                           #if PGA_LINEARITY_DEBUG
                           Printf("Channel=%u,  point %u  target level=0x%x, Current level=0x%x\n",_bOrder,_bLocation,_dwMaxDiffTemp,_dwMaxLevelAvg[_bOrder]);
                           #endif
                        if (_dwMaxDiffTemp > _dwMaxLevelAvg[_bOrder])
                        {
                      ...
                      Printf("The channel different at position %2u are 0x%2x,  0x%2x,  0x%2x\n",_bLocation, _dwMaxDiff[0],_dwMaxDiff[1],_dwMaxDiff[2]);
