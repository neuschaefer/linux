...
UINT32 SRM_DramMonitor(UINT32 u4Mod, UINT32 u4DelayClock, UINT32 u4MonitorDelay)
{
    ...
    volatile HAL_DRAMC_T* _prDramcReg = (HAL_DRAMC_T*) (DRAM_BASE + 0x180);
    ...
    switch (u4Mod)
    {
        ...
        u4AgendId = 0x10;   // invalid in MT8223
        u4GroupId = 1;
        break;
    ...
    IO_REG32(DRAM_BASE, 0x18C) = u4DelayClock;
    ...
    *_prDramcReg = rDramcReg;
    ...
    while (IO_REG32(DRAM_BASE, 0x19C) != IO_REG32(DRAM_BASE, 0x18C))
    {
            x_thread_delay(1000);
    }
    ...
    if (u4GroupId == 1)
    {
        u4Result = IO_REG32(DRAM_BASE, 0x190);
    }
    else
    if (u4GroupId == 2)
    {
        u4Result = IO_REG32(DRAM_BASE, 0x194);
    }
    else
    if (u4GroupId == 3)
    {
        u4Result = IO_REG32(DRAM_BASE, 0x198);
    }
