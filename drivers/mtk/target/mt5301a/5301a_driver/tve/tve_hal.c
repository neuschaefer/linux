...
void TVE_HalColorBar(UCHAR ucTveId, UCHAR ucOn)
{
    ...
    if (ucTveId == TVE_1)
    {
        ...
        //rTveMainReg.uSyncCtrl.rSyncCtrl.fgAdjSyncEn = ucOn;
        /* Set TVE HW MAIN Registers */
        TVE_WRITE32(TVE1_REG_ENCMOD, rTveMainReg.uEncMode.u4EncMode);
...
//            rTveMainReg.uCbCrDelay.u4CbCrDelay = 0x0053B30C;
            ...
            if ((ucFmt == TVE_FMT_480I) || (ucFmt == TVE_FMT_576I))
            {
                ...
                if (ucFmt == TVE_FMT_480I)
                {
                    ...
                    rTveMainReg.uChromaGain.rChromaGain.u4Y2ActiveEnd = 0xb;
                    rTveMainReg.uChromaGain.rChromaGain.u4C2ActiveEnd = 0x5;
                    #endif
                }
                else
                {
...
UCHAR TVE_HalSend2TTXQue(UCHAR ucTveId, const UCHAR *pu1Buf, UINT32 length)
{
    ...
    if (ucTveId == TVE_1)
    {
                ...
                if (_u4MainTTXWp >= TVE_TTX_BUF_NUM)
        {
            ...
            if((ucLineOffset - rTveMainReg.uTeltxt3.rTeltxt3.u4TTBgnLine) < TVE_TTX_BUF_LINENUM)
            {
                //line_offset info
...
// Power on/off DAC
//-------------------------------------------- */
UINT8 TVE_HalDACPowerOn(UCHAR ucTveId, UCHAR ucDAC, UCHAR ucOn)
{
    UINT8 u4RetVal = TVE_SET_OK;
    ...
    if (ucTveId == TVE_1)
    {
        if(ucDAC == DAC_1)
        {
            vIO32WriteFldAlign(AFE_10, ucOn ? 0x3 : 0x0, VDAC_MODE);	 
        }
        else
        {
            LOG(7, "Only 1 VDAC\n");
	...
	if(IO32ReadFldAlign(AFE_10, VDAC_MODE) == 0x00)
	{
            vIO32WriteFldAlign(AFE_10, 1, BGREF_OFF);	 
	}
	else
	{
            vIO32WriteFldAlign(AFE_10, 0, BGREF_OFF);	 
	}
    }
    return u4RetVal;
}
...
//--------------------------------------------*/
UINT8 TVE_HalSetMixerSrc(UCHAR Mixer, UCHAR bSrc)
{
    if(Mixer == TVE_Mixer1)
    {
        if(bSrc == TVE_VDOIN_MIXER)
        {
            vIO32WriteFldAlign(AFE_0F, 0x0, MIXER_SEL);	 
        }
        else if(bSrc == TVE_DEMOD_MIXER)
        {
            vIO32WriteFldAlign(AFE_0F, 0x1, MIXER_SEL);	 
        }
    }
    else
    {
...
// DAC 0: DAC1; 1: DAC2
// bSrc 0: demod audio test out mode; 1: normal mode
//--------------------------------------------*/
UINT8 TVE_HalSetMixerDacSel(UCHAR DAC, UCHAR bSrc)
{
    if(DAC == DAC_1)
    {
        vIO32WriteFldAlign(AFE_0F, bSrc, MIXER_DAC_SEL);	 
    }
    else
    {
        LOG(7, "Only 1 VDAC\n");
...
void Bypass_HalSrcBypass(UCHAR DAC, UCHAR bSrc)
{
    ...
    if(DAC == DAC_1)
    {
        ...
        TVE_HalSetMixerDacSel(DAC_1, 1);//normal mode
        if(bSrc == TVE_VDOIN_MIXER) //BYPASS_CVBS
        {
            vIO32WriteFldAlign(CKGEN_PSWCLK_CFG, 0x1, FLD_VDAC1_SEL); //tvd54_ck
        }
        else if(bSrc == TVE_DEMOD_MIXER)//BYPASS_DEMOD
        {
            vIO32WriteFldAlign(CKGEN_PSWCLK_CFG, 0x0, FLD_VDAC1_SEL); //demod54_ck
        }
    }
    else
    {
