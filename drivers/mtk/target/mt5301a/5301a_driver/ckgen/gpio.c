...
#define GPIO_INTR_REG(gpio)     \
    ((GPIO_RISE_REG(gpio >> 5) |GPIO_FALL_REG(gpio >> 5) |GPIO_EDGE_HIGH_REG(gpio >> 5) | GPIO_EDGE_LOW_REG(gpio >> 5)) & (1U << (gpio & GPIO_INDEX_MASK)))
#define GPIO_INTR_FALL_REG(gpio)    \
    (GPIO_FALL_REG(gpio >> 5) & (1U << (gpio & EDGE_INDEX_MASK)))
#define GPIO_INTR_CLR(gpio)     \
    GPIO_STATUS_WRITE(gpio >> 5, GPIO_STATUS_REG(gpio >> 5) & (~(1U << (gpio & EDGE_INDEX_MASK))))
...
#define GPIO_INTR_RISE_SET(gpio)    \
    (GPIO_RISE_WRITE(gpio >> 5, GPIO_RISE_REG(gpio >> 5)  | (1U << (gpio & EDGE_INDEX_MASK))))
...
#define GPIO_INTR_FALL_SET(gpio)    \
    (GPIO_FALL_WRITE(gpio >> 5, GPIO_FALL_REG(gpio >> 5) | (1U << (gpio & EDGE_INDEX_MASK))))

#define GPIO_INTR_FALL_CLR(gpio)    \
    (GPIO_FALL_WRITE(gpio >> 5, GPIO_FALL_REG(gpio >> 5) & (~ (1U << (gpio & EDGE_INDEX_MASK)))))

// Edge triggered interrupt status register
#define GPIO_INTR_EDGE_HIGH_REG(gpio)    \
    (GPIO_EDGE_HIGH_REG(gpio >> 5) & (1U << (gpio & EDGE_INDEX_MASK)))

#define GPIO_INTR_EDGE_HIGH_SET(gpio)    \
    (GPIO_EDGE_HIGH_WRITE(gpio >> 5, GPIO_EDGE_HIGH_REG(gpio >> 5) | (1U << (gpio & EDGE_INDEX_MASK))))

#define GPIO_INTR_EDGE_HIGH_CLR(gpio)    \
    (GPIO_EDGE_HIGH_WRITE(gpio >> 5, GPIO_EDGE_HIGH_REG(gpio >> 5) & (~(1U << (gpio & EDGE_INDEX_MASK)))))

#define GPIO_INTR_EDGE_LOW_REG(gpio)    \
    (GPIO_EDGE_LOW_REG(gpio >> 5) & (1U << (gpio & EDGE_INDEX_MASK)))

#define GPIO_INTR_EDGE_LOW_SET(gpio)    \
    (GPIO_EDGE_LOW_WRITE(gpio >> 5, GPIO_EDGE_LOW_REG(gpio >> 5) | (1U << (gpio & EDGE_INDEX_MASK))))

#define GPIO_INTR_EDGE_LOW_CLR(gpio)    \
        (GPIO_EDGE_LOW_WRITE(gpio >> 5, GPIO_EDGE_LOW_REG(gpio >> 5) & (~(1U << (gpio & EDGE_INDEX_MASK)))))
...
static INT32 _CKGEN_GPIO_Output(INT32 i4GpioNum, INT32 *pfgSet)
{
    ...
    //VERIFY(_CKGEN_GPIO_Enable(i4GpioNum, &i4Idx)==i4Idx); //modify for QAC
    UNUSED(_CKGEN_GPIO_Enable(i4GpioNum, &i4Idx));
...
static INT32 _CKGEN_GPIO_Intrq(INT32 i4GpioNum, INT32 *pfgSet)
{
  ...
  //i4Idx = ((UINT32)i4GpioNum >> 5);
    ...
    // cyj.ask: no way to disable interrupt?
    UINT32 u4Val;
    INT32 i4Idx;
    INT32 i4GpioIntNum = _GPIO2BitOffsetOfIntReg[i4GpioNum];
    INT32 i4IntCfg = 0;
    if(i4GpioIntNum == -1)
        return -1;
    i4Idx = ((UINT32)i4GpioIntNum >> 5);
    
    if (pfgSet == NULL)
    {
        u4Val = GPIO_RISE_REG(i4Idx);
        i4IntCfg += ((u4Val & (1U << (i4GpioIntNum & GPIO_INDEX_MASK))) ? GPIO_TYPE_INTR_RISE/*1*/ : 0);

        u4Val = GPIO_FALL_REG(i4Idx);
        i4IntCfg += ((u4Val & (1U << (i4GpioIntNum & GPIO_INDEX_MASK))) ? GPIO_TYPE_INTR_FALL /*2*/: 0);

        u4Val = GPIO_EDGE_HIGH_REG(i4Idx);
        i4IntCfg += ((u4Val & (1U << (i4GpioIntNum & GPIO_INDEX_MASK))) ? GPIO_TYPE_INTR_LEVEL_HIGH/*4*/ : 0);

        u4Val = GPIO_EDGE_LOW_REG(i4Idx);
        i4IntCfg += ((u4Val & (1U << (i4GpioIntNum & GPIO_INDEX_MASK))) ? GPIO_TYPE_INTR_LEVEL_LOW/*8*/ : 0);
        return i4IntCfg;
    }

    if((*pfgSet) & GPIO_TYPE_INTR_RISE)
    {
        u4Val = GPIO_RISE_REG(i4Idx);
        u4Val = (u4Val | (1U << (i4GpioIntNum & GPIO_INDEX_MASK))) ;
        GPIO_RISE_WRITE(i4Idx, u4Val);            
    }
    
    if((*pfgSet) & GPIO_TYPE_INTR_FALL)
    {
        u4Val = GPIO_FALL_REG(i4Idx);
        u4Val = (u4Val | (1U << (i4GpioIntNum & GPIO_INDEX_MASK))) ;
        GPIO_FALL_WRITE(i4Idx, u4Val);            
    }

    if((*pfgSet) & GPIO_TYPE_INTR_LEVEL_HIGH)
    {
        u4Val = GPIO_EDGE_HIGH_REG(i4Idx);
        u4Val = (u4Val | (1U << (i4GpioIntNum & GPIO_INDEX_MASK))) ;
        GPIO_EDGE_HIGH_WRITE(i4Idx, u4Val);            
    }

    if((*pfgSet) & GPIO_TYPE_INTR_LEVEL_LOW)
    {
        u4Val = GPIO_EDGE_LOW_REG(i4Idx);
        u4Val = (u4Val | (1U << (i4GpioIntNum & GPIO_INDEX_MASK))) ;
        GPIO_EDGE_LOW_WRITE(i4Idx, u4Val);            
    }
...
//    UINT32 u4Val;
    ...
    CKGEN_WRITE32(REG_RINTEN0, 0);
    CKGEN_WRITE32(REG_RINTEN1, 0);
    CKGEN_WRITE32(REG_FINTEN0, 0);
    CKGEN_WRITE32(REG_FINTEN1, 0);
    CKGEN_WRITE32(REG_HINTEN0, 0);
    CKGEN_WRITE32(REG_HINTEN1, 0);
    CKGEN_WRITE32(REG_LINTEN0, 0);
    CKGEN_WRITE32(REG_LINTEN1, 0);
    ...
    CKGEN_WRITE32(REG_EDGEINT0, 0);
    CKGEN_WRITE32(REG_EDGEINT1, 0);
...
//remove warning for klocwork checking
    ...
    case GPIO_TYPE_INTR_LEVEL_LOW:
        if(_GPIO2BitOffsetOfIntReg[i4Gpio] != -1)
        {
            ...
            GPIO_INTR_EDGE_LOW_SET((UINT32)_GPIO2BitOffsetOfIntReg[i4Gpio]);
        }
        break;
    case GPIO_TYPE_INTR_LEVEL_HIGH:
        if(_GPIO2BitOffsetOfIntReg[i4Gpio] != -1)
        {
            ...
            GPIO_INTR_EDGE_HIGH_SET((UINT32)_GPIO2BitOffsetOfIntReg[i4Gpio]);
        }
        break;
#if GPIO_THREAD_ENABLE
    ...
    case GPIO_TYPE_NONE:
        ...
        if(_GPIO2BitOffsetOfIntReg[i4Gpio] != -1)
        {
            ...
            GPIO_INTR_RISE_CLR((UINT32)(_GPIO2BitOffsetOfIntReg[i4Gpio]));
            GPIO_INTR_FALL_CLR((UINT32)(_GPIO2BitOffsetOfIntReg[i4Gpio]));
            GPIO_INTR_EDGE_HIGH_CLR((UINT32)(_GPIO2BitOffsetOfIntReg[i4Gpio]));
            GPIO_INTR_EDGE_LOW_CLR((UINT32)(_GPIO2BitOffsetOfIntReg[i4Gpio])); 
        }            
#ifndef CC_MTK_LOADER        
...
static INT32 _CKGEN_GPIO_Stop()
{
    ...
    {
        GPIO_RISE_WRITE(i4Idx, 0);//CKGEN_WRITE32(REG_RINTEN0 + 4 * i4Idx, 0);  
        GPIO_FALL_WRITE(i4Idx, 0);//CKGEN_WRITE32(REG_FINTEN0 + 4 * i4Idx, 0); 
        GPIO_EDGE_HIGH_WRITE(i4Idx, 0);//CKGEN_WRITE32(REG_HINTEN0 + 4 * i4Idx, 0); 
        GPIO_EDGE_LOW_WRITE(i4Idx, 0);        //CKGEN_WRITE32(REG_LINTEN0 + 4 * i4Idx, 0); 
    }
#ifndef CC_UP8032_ATV
...
//#if GPIO_THREAD_ENABLE
/*
	interrupt will check and call the callback function here
*/
...
*/
    ...
    UINT32 u4MaskRise;
    UINT32 u4MaskFall;    
    UINT32 u4MaskLevelHigh;
    UINT32 u4MaskLevelLow;    
    UINT32 u4Status;
    ...
    {
        // get edge triggered interrupt status.       
        u4Status = GPIO_STATUS_REG(i4Idx);

        // clear edge triggered interrupt status.       
    	GPIO_STATUS_WRITE(i4Idx, 0);
        u4MaskRise = u4Status & GPIO_RISE_REG(i4Idx);        
        if (u4MaskRise)
        {
            _GPIO_CheckCallback(u4MaskRise, i4Idx, NORMAL_GPIO_NUM, 1);
        }

        u4MaskFall = u4Status & GPIO_FALL_REG(i4Idx);
        if (u4MaskFall)
        {
            _GPIO_CheckCallback(u4MaskFall, i4Idx, NORMAL_GPIO_NUM, 0);                
        }

        u4MaskLevelHigh = u4Status & GPIO_EDGE_HIGH_REG(i4Idx);        
        if (u4MaskLevelHigh)
        {
            _GPIO_CheckCallback(u4MaskLevelHigh, i4Idx, NORMAL_GPIO_NUM, 1);
        }

        u4MaskLevelLow = u4Status & GPIO_EDGE_LOW_REG(i4Idx);        
        if (u4MaskLevelLow)
        {
            _GPIO_CheckCallback(u4MaskLevelLow, i4Idx, NORMAL_GPIO_NUM, 0);
        }
    }
}
