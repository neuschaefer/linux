...
* Return        : None
******************************************************************************/
//#define _ADSP_GetDRAMDec1_Addr(addr)   ( (UINT32*)(NONCACHE(_u4DspDramBuf[TYPE_DEC1_DRAM_IDX]+((addr)<<2))) )    //Andrew Wen : DTV00005752 Remove usage of NONCACHE macro
...
static void vDspLoadTestPattern(UINT8 u1Type)
{
    ...
    fgLoadCode  = FALSE;
    dwFlashOfst = u4GetSLTDspImgAddr ();   // set to dsp image addr
    switch (u1Type)
    {
    ...
    *pu4ADSPslt_rslt = 0;
    ...
    {
        // Flush Cache
        vFlash2DramDma ((dwDECICAddr * 4), u4GetSLTDspImgAddr () + (dwOfst * 4),
                        (dwSz - 1) * 4);
        // Ram Boot
        vSetDspBootFromRam ();
        // Let DSP go
