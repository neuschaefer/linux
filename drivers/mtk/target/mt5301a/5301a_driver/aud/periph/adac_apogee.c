...
static void _APOGEE_RegWriteEx(UINT8 u1Addr, UINT8 u1Data, UINT8 u1AmpID);
static UINT8 _APOGEE_RegReadEx(UINT8 u1Addr, UINT8 u1AmpID);
...
static void _APOGEE_Init(void)
{
    ...
    if (u4reset != 0xffffffff)
    {
        ...
        UNUSED(GPIO_Output((INT32)u4reset,(const INT32 *)&u4polarity)); //digital ampliferT
        x_thread_delay(50);
        u4polarity = !u4polarity;
        UNUSED(GPIO_Output((INT32)u4reset,(const INT32 *)&u4polarity)); //digital ampliferT
    }
