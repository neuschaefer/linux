...
#include "x_typedef.h"
#include "aud_if.h"
#include "codec_internal.h" // For environment register read & write
...
static  CODEC_FUNCTBL_T _rInterAdcFunTbl = 
{
	_AADC_InterAdcInit,				//pfnInit
	_AADC_InterAdcMute,			//pfnMute 
	...
	NULL,					// pfnGetAdcGain   
   	NULL,			// pfnSetHpGain
   	NULL,					// pfnGetHpGain
   	_AADC_InterAdcChlSel,			// pfnSetAdcMuxSel
   	NULL,			// pfnRegWrite
   	NULL			// pfnRegRead
};

static CODEC_T _rInterAadc = 
{
	"INTERADC",				// szName[32];
	0,							// i2MaxHpGain
	-72,							// i2MinHpGain
 ...
 */
//-----------------------------------------------------------------------------
void _AADC_InterAdcChlSel(UINT8 u1Indx)
{
    UINT32 u4Reg;
    ...
    AADC_WRITE32(REG_INT_SDADC2, (AADC_READ32(REG_INT_SDADC2) & ~(0x1fU<<23)) |u4Reg);
 ...
 */
//-----------------------------------------------------------------------------
void _AADC_InterAdcEnable(BOOL fgEnable)
{    
/*
    if (fgEnable)
    {
        ADAC_WRITE32(REG_ADC_CFG1, ADAC_READ32(REG_ADC_CFG1) &
        	                       (~CORE_POWER_DOWN_EN)     &
        	                       (~REF_POWER_DOWN_EN));
    }
    else
 ...
 */
//-----------------------------------------------------------------------------
void _AADC_InterAdcInit(void)
{
    AUD_INTER_DAC_CFG_T* prInterDacUsage;
    
    AADC_WRITE32(REG_INT_SDADC0, 0x01020000);
    AADC_WRITE32(REG_INT_SDADC1, 0x21a30000);
    AADC_WRITE32(REG_INT_SDADC2, 0x00031f20);
 ...
 */
//-----------------------------------------------------------------------------
void _AADC_InterAdcMute(BOOL fgEnable)
{
/*
    if (fgEnable)
    {
        ADAC_WRITE32(0x30c, 0x15);
    }
    else
    {
 ...
 */
//-----------------------------------------------------------------------------
CODEC_T*  _ADAC_GetInterAdcTbl(void)
{
    return &_rInterAadc;
}
