...
void _MT8292_WRITE(UINT8 u1Addr, UINT8 u1Data)
{
    ...
    u4WriteCount = SIF_Write(SIF_CLK_DIV, MT8292_DEV_ADDR, u1Addr, &u1Data, 1);
    ...
    {
    	LOG(5, "Error MT8292 register setting\n");
    }	
}
...
UINT8 _MT8292_READ(UINT8 u1Addr) 
{
    ...
    u4ReadCount = SIF_Read(SIF_CLK_DIV, MT8292_DEV_ADDR, u1Addr, &u1Data, 1);
...
void _MT8292_Init(void)
{
    ...
    _MT8292_WRITE(REG_MT8292_HP_UPDATE, 0x04);       

    //Output PGA unmute
    //Due to open later, keep mute longer.
    //_MT8292_WRITE(REG_OUTPUT_PGA_MUTE, 0x00); 

    //Set PGA gain 
    UNUSED(DRVCUST_OptQuery(eAudioMT8292PGA0GAIN, (UINT32 *)(void *) &u4PGA0Gain));
    UNUSED(DRVCUST_OptQuery(eAudioMT8292PGA1GAIN, (UINT32 *)(void *) &u4PGA1Gain));
    UNUSED(DRVCUST_OptQuery(eAudioMT8292PGA2GAIN, (UINT32 *)(void *) &u4PGA2Gain));
    UNUSED(DRVCUST_OptQuery(eAudioMT8292HPGAIN, (UINT32 *)(void *) &u4HPGain));

    _MT8292_WRITE(REG_MT8292_PGA0L_GAIN, (UINT8)u4PGA0Gain); 
    _MT8292_WRITE(REG_MT8292_PGA0R_GAIN, (UINT8)u4PGA0Gain); 
    
    _MT8292_WRITE(REG_MT8292_PGA1L_GAIN, (UINT8)u4PGA1Gain); 
    _MT8292_WRITE(REG_MT8292_PGA1R_GAIN, (UINT8)u4PGA1Gain);     

    _MT8292_WRITE(REG_MT8292_PGA2L_GAIN, (UINT8)u4PGA2Gain); 
    _MT8292_WRITE(REG_MT8292_PGA2R_GAIN, (UINT8)u4PGA2Gain); 

    u1HPGainSt = _MT8292_READ(REG_MT8292_HP_GAIN);
    if ((UINT8)u4HPGain > u1HPGainSt)
    {
        while (u1HPGainSt < (UINT8)u4HPGain)
        {
            _MT8292_WRITE(REG_MT8292_HP_GAIN, ++u1HPGainSt); 
        }
    }
    else if ((UINT8)u4HPGain < u1HPGainSt)
    {
        while (u1HPGainSt > (UINT8)u4HPGain)
        {
            _MT8292_WRITE(REG_MT8292_HP_GAIN, --u1HPGainSt); 
