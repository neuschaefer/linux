...
static HANDLE_T _rProtectSema = NULL;
...
static void _AudNotifyInit(void)
{
    ...
    if (!fgInit)
    {
        ...
        for (i=0; i<NOTIFY_FUNC_NUM; i++)
        {
            x_memset((VOID *)&_arNotifyFuncInfo[i], 0, sizeof(NOTIFY_FUNC_INFO_T));
    ...
    for (i=0; i<NOTIFY_FUNC_NUM; i++)
    {
        if ((_arNotifyFuncInfo[i].fgRegistered ) && (_arNotifyFuncInfo[i].pfNotifyFunc != NULL))
        {
            ...
            if ((eNfyCond == AUD_COND_CTRL_DONE) && ((AUD_NFY_TAG_T)u4NfyTag == AUD_NFY_STREAM))
            {
                ...
                ((eNfyCond == AUD_COND_AUD_INFO_CHG) && ((AUD_NFY_TAG_T)u4NfyTag == AUD_NFY_STREAM)) &&
                AUD_IsDualDecState())
            {
                AUD_DEC_STREAM_FROM_T StrStr;
                StrStr = AUD_GetDualDecStrSrc();
                if (StrStr == AUD_STREAM_FROM_ANALOG_TUNER)
                {
                ...
                else if (StrStr == AUD_STREAM_FROM_DIGITAL_TUNER)
                {
#ifdef CC_AUD_SETUP_MELODY
					if (!fgIgnoreSetupPlayNotify)
#endif			
...
#ifdef CC_AUD_SETUP_MELODY
		if (eNfyCond == AUD_COND_AUD_CLIP_DONE)
		{
			fgIgnoreSetupPlayNotify = FALSE;
...
void vApiNotifyAfSoundPropertiesChanged(void)
{
    void (*pfnNotifyFunc)(void) = NULL; 
    if (DRVCUST_OptQuery(eNotifyAFSoundProperties, (UINT32 *)(void *)&pfnNotifyFunc) == 0)
    {        
       if (pfnNotifyFunc)
        {            
            pfnNotifyFunc();
        ...
        {
            //LOG(5, "undefine NOTIFY_OnSoundPropertiesChanged\n");
        }
    }
}
void vApiNotifyAfDRCAllowedChanged(void)
{
    void (*pfnNotifyFunc)(void) = NULL; 
    if (DRVCUST_OptQuery(eNotifyAFDRCAllowed, (UINT32 *)(void *)&pfnNotifyFunc) == 0)
    {        
       if (pfnNotifyFunc)
        {            
            pfnNotifyFunc();
