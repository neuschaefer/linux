...
/** @file assert.c
 *  assert.c provides Assert function for assertion.
 */
...
#if defined (FBM_FB_LOG) ||defined (FBM_CS_LOG)    
#include "fbm_drvif.h"
#endif
...
#ifdef FBM_FB_LOG
    //Flush FB log
    FBM_FlushFbLog(10, FBM_FB_ID_UNKNOWN);
#endif
...
#if defined(CC_MTK_PRELOADER) && defined(NDEBUG)
    CHIP_DisplayString("\nAssertion fails at:\nFile: ");
    CHIP_DisplayString((const INT8 *)szFile);
    CHIP_DisplayString(", line ");
    CHIP_DisplayHex(i4Line);
    CHIP_DisplayString("\n\t");
    CHIP_DisplayString((const INT8 *)szExpress);
    CHIP_DisplayString("\n\nProgram terminated.\n");
