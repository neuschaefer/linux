...
//============================================================================
#if 0
#if defined(CC_NAND_BOOT) || defined(CC_NAND_DRIVER)

#include "x_hal_5381.h"

//#if !defined(CC_LARGE_NAND)
//#define CC_LARGE_NAND
//#endif
 ...
 */
//-----------------------------------------------------------------------------
INT32 _NANDHW_ReadPageToDRAM(UINT32 *pu4Buf, UINT32 u4PageIdx, UINT32 u4ByteCnt)

{
    ...
    for(i = 0; i < (u4ByteCnt)/4; i++)
    {
        ...
        *pu4Buf = NAND_READ32(NAND_U4_RD_BUF_REG);
        pu4Buf++;
    }
...
//-----------------------------------------------------------------------------
/** _NANDHW_ReadAttrLarge()
 */
//-----------------------------------------------------------------------------
static INT32 _NANDHW_ReadAttrLarge(UINT8 *pu1Buf, UINT32 u4PgIdx, UINT32 u4ByteCount)
{
    UINT32 i, u4Val;
    ...
    /* Write command. */
    _NANDHW_Command(NAND_CMD_READ1_00);
    _NANDHW_Addr((u4PgIdx << 16) + 2048, 4);
    _NANDHW_Command(0x30);
    ...
    for (i=0; i<u4ByteCount; i++)
    {
        ...
        {
            u4Val = NAND_READ32(NAND_FIFO_CTRL_REG);
        } while (u4Val & NAND_FIFO_RB_EMPTY);
        pu1Buf[i] = NAND_READ32(NAND_U1_RD_BUF_REG);
    }
 ...
 */
//-----------------------------------------------------------------------------
INT32 _NAND_CheckBlockLarge(UINT8 *pu1Buf, UINT32 *pu4PgIdx, UINT32 u4LogIdx)
{
    do
    {
        _NANDHW_ReadAttrLarge(pu1Buf, *pu4PgIdx, 64);
        if ( (pu1Buf[0] != 0xFF) || ((*((UINT32*)(pu1Buf + 12))) != u4LogIdx) )
        {
            *pu4PgIdx += 64;
        }
        else
...
//-----------------------------------------------------------------------------
/** nand boot for lhzs decompression.
 */
//-----------------------------------------------------------------------------
...
//    UINT8 *pu1Buf;
    ...
    pu4Buf = pu4DstBuf;
//    pu1Buf = (UINT8 *)pu4DstBuf;
    u4LogPageIdx = u4ByteOffset/2048;
    u4PhyPageIdx = u4LogPageIdx;
    u4OrgBlkIdx = u4LogPageIdx/64;
    u4CurrBlkIdx = 0xFFFFFFFF;
    do
    {
        if (u4OrgBlkIdx != u4CurrBlkIdx)
        {
// wait for MTK tool
//            _NAND_CheckBlockLarge(pu1Buf, &u4PhyPageIdx, u4LogPageIdx);
            u4OrgBlkIdx = u4PhyPageIdx/2048;
            u4CurrBlkIdx = u4OrgBlkIdx;
        }
        if (u4ByteOffset & 0x7FF)
        {
            u4ReadByte = (2048 - (u4ByteOffset & 0x7FF) );
        }
        else
        {
            u4ReadByte = 2048;
        ...
        _NANDHW_ReadPageToDRAM(pu4Buf, u4PhyPageIdx, u4ReadByte);


        u4ByteOffset += u4ReadByte;
        u4Len -= u4ReadByte;
        u4PhyPageIdx++;
        u4LogPageIdx++;
        pu4Buf+= (u4ReadByte/4);
        u4CurrBlkIdx = u4PhyPageIdx/2048;
    }while(u4Len);
#if 0
    {UINT32 i;
        for(i = 0; i < 8; i++)
...
#else
INT32 _NANDHW_ReadPageToDRAM(UINT32 *pu4Buf, UINT32 u4PageIdx, UINT32 u4ByteCnt, UINT16 u2FromHead)
{
    UINT32 u4Val, i;
    UINT8 *pu1Buf;
    UINT32 u4Size;
    ...
    if (u4ByteCnt%4)
    {
        ...
        if (u2FromHead)
        {
            for (i = 0; i < (u4ByteCnt); i++)
            {
                NAND_WRITE32(NAND_OP_CTRL_REG, NAND_NOB(NAND_NOB_1B) | NAND_SINGLE_RD_MODE);
                _NANDHW_WaitBusy(NAND_STATUS_DTRD);
                *pu1Buf = (NAND_READ32(NAND_U1_RD_BUF_REG));
                 pu1Buf++;
            }
        }
        else
        {
            for (i = 0; i < (u4Size-u4ByteCnt); i++)
            {
                NAND_WRITE32(NAND_OP_CTRL_REG, NAND_NOB(NAND_NOB_1B) | NAND_SINGLE_RD_MODE);
                _NANDHW_WaitBusy(NAND_STATUS_DTRD);
                *pu1Buf = (NAND_READ32(NAND_U1_RD_BUF_REG));
            }
            for (i = (u4Size-u4ByteCnt); i < u4Size; i++)
            {
                NAND_WRITE32(NAND_OP_CTRL_REG, NAND_NOB(NAND_NOB_1B) | NAND_SINGLE_RD_MODE);
                _NANDHW_WaitBusy(NAND_STATUS_DTRD);
                *pu1Buf = (NAND_READ32(NAND_U1_RD_BUF_REG));
    ...
    else
    {
        ...
        {
            for(i = 0; i < (u4Size-u4ByteCnt)/4; i++)
            {
                do
                {
                    u4Val = NAND_READ32(NAND_FIFO_CTRL_REG);
            ...
            for(i = (u4Size-u4ByteCnt)/4; i < (u4Size)/4; i++)
            {
                do
                {
                    u4Val = NAND_READ32(NAND_FIFO_CTRL_REG);
...
//-----------------------------------------------------------------------------
/** _NANDHW_ReadAttrSmall()
 */
//-----------------------------------------------------------------------------
static INT32 _NANDHW_ReadAttrSmall(UINT8 *pu1Buf, UINT32 u4PgIdx, UINT32 u4ByteCount)
{
    UINT32 i, u4Val;
    NAND_WRITE32(NAND_FIFO_CTRL_REG, NAND_FIFO_RESET);
    NAND_WRITE32(NAND_CTRL_REG, NAND_CTRL_PROGSPARE);
 ...
 */
//-----------------------------------------------------------------------------
INT32 _NAND_CheckBlockSmall(UINT8 *pu1Buf, UINT32 *pu4PgIdx, UINT32 u4LogIdx)
{
    do
    {
        _NANDHW_ReadAttrSmall(pu1Buf, *pu4PgIdx, 16);
        if ( (pu1Buf[5] != 0xFF) || ((*((UINT32*)(pu1Buf + 12))) != u4LogIdx) )
        {
            *pu4PgIdx += 32;
        }
        else
...
void CHIP_NANDCopyToDRAMLZHS(UINT32 *pu4DRAMBuf, UINT32 u4ByteOffset, UINT32 u4Len)
{
    UINT32 *pu4Buf;
    UINT32 u4LogPageIdx;
    UINT32 u4PhyPageIdx;
    ...
    UINT32 u4ReadByte;
    pu4Buf = pu4DRAMBuf;
    pu1Buf = (UINT8 *)pu4DRAMBuf;
    u4LogPageIdx = u4ByteOffset/512;
    u4PhyPageIdx = u4LogPageIdx;
    u4OrgBlkIdx = u4LogPageIdx/32;
    UINT16 u2FromHead;
    u4CurrBlkIdx = 0xFFFFFFFF;
    do
    {
        ...
        {
// wait for MTK tool
//            _NAND_CheckBlockSmall(pu1Buf, &u4PhyPageIdx, u4LogPageIdx);
            u4OrgBlkIdx = u4PhyPageIdx/512;
            u4CurrBlkIdx = u4OrgBlkIdx;
        }
        if (u4ByteOffset & 0x1FF)
        {
            u4ReadByte = (512 - (u4ByteOffset & 0x1FF) );
        ...
        _NANDHW_ReadPageToDRAM(pu4Buf, u4PhyPageIdx, u4ReadByte, u2FromHead);
        u4ByteOffset += u4ReadByte;
        u4Len -= u4ReadByte;
        u4PhyPageIdx++;
        u4LogPageIdx++;
        pu4Buf+= (u4ReadByte/4);
        u4CurrBlkIdx = u4PhyPageIdx/512;
    }while(u4Len);
#if 0
    {UINT32 i;
        for(i = 0; i < 8; i++)
