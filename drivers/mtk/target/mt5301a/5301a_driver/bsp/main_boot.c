...
VOID BSP_Start()
{
#if !defined(CC_NAND_BOOT_BIN0) && !defined(CC_NAND_BOOT_BIN1) && !defined(CC_NAND_BOOT_BIN2) && !defined(CC_NAND_BOOT_BIN3)
    CHIP_BootEntry0();
    if (!BSP_IsFPGA())
    {
        CHIP_BootEntry1();
...
Restart:
    ...
    #ifdef CC_NAND_BOOT_BIN0
        CHIP_BootEntry0();
        #define    u4Offset  (1024*5)
    #endif /* CC_NAND_BOOT_BIN0 */

    #ifdef CC_NAND_BOOT_BIN1
        CHIP_BootEntry1();
        #ifndef u4Offset
            #define    u4Offset  (1024*9)
        #endif
    #endif /* CC_NAND_BOOT_BIN1 */

    #ifdef CC_NAND_BOOT_BIN2
        CHIP_BootEntry2();
        #ifndef u4Offset
            #define    u4Offset  (1024*13)
        #endif
    #endif /* CC_NAND_BOOT_BIN2 */

    #ifdef CC_NAND_BOOT_BIN3
        CHIP_BootEntry3();
        #ifndef u4Offset
            #define    u4Offset  (1024*16)
        #endif
    #endif /* CC_NAND_BOOT_BIN3 */

    #ifndef CC_MTK_PRELOADER
        BSPLinkSymbol(0);
        // read nand offset to LZHS buffer
        CHIP_NANDCopyToDRAM((UINT32 *)0x2B000400, u4Offset, 1024*3);
        goto Restart;
    #endif
#endif /* CC_NANDBOOT */
}
