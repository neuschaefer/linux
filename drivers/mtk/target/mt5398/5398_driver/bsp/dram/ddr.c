...
#else
	eRODT = DDR_RTT_DISABLE;   //read ODT always on
    u4Val =	DRAM_DDRPHY_READ32(0x314, u4Val);
	u4Val = (u4Val & ~0x1F) | 0x1A;
	DRAM_DDRPHY_WRITE32(0x314, u4Val);
	u4Val =	DRAM_DDRPHY_READ32(0x31c, u4Val);
	u4Val = (u4Val & ~0x1F) | 0x1A;
	DRAM_DDRPHY_WRITE32(0x31c, u4Val);
#endif
    // Set DQS default ODTP, ODTN, RTT.
    u4Val = DRAM_READ32(0x310);
    u4Val &= ~ (0x00770007);
    u4Val |= (((UINT32)eRODT) | 0x00330000);
