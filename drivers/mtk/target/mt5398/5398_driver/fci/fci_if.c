...
#ifdef CC_DTV_FCI
extern INT32 i4FciCardDetInit(VOID);
#endif
...
static REG_TEST_T arFCIRgtList [] =
{
    ...
    { 0x0008, eRD_ONLY, 4, 0xffff00ff, 1, 0x00060000 }, // MT5387 FCI 0x08[0] DCE default value to 0
    { 0x000c, eRD_WR,    4, 0xf93fffe0, 1, 0x80300000 },
    { 0x0010, eRD_ONLY, 4, 0xff1fffef, 1, 0x480A0000 }, /* Boot change from 0x10 to 0x400A0000 */
#elif (CONFIG_CHIP_VER_CURR == CONFIG_CHIP_VER_MT5365) || (CONFIG_CHIP_VER_CURR == CONFIG_CHIP_VER_MT5395)     
...
#else
  #if(CONFIG_CHIP_VER_CURR == CONFIG_CHIP_VER_MT8550) //added mhzhang
    { 0x0008, eRD_WR,    4, 0xffff01ff, 1, 0x00060001}, 
  #elif(CONFIG_CHIP_VER_CURR == CONFIG_CHIP_VER_MT8555) // added on 20009/11/6 by mhzhang
    { 0x0008, eRD_WR,    4, 0xffff11ff, 1, 0x00060001}, // because in 8555 , 0x0008[12] is defined
  #else
    { 0x0008, eRD_ONLY, 4, 0xffff00ff, 1, 0x00060001 }, // MT5362B FCI 0x08[0] DCE default value to 0, MT5362A FCI 0x08[0] DCE default value is 1    
  #endif

  #if (FCI_INTERRUPT_ARCHITECTURE == FCI_NEW_INTERRUPT_ARCHITECTURE)    
    { 0x000c, eRD_WR,    4, 0xFD3FC02C, 1, 0x80300000 },
  #else
    { 0x000c, eRD_WR,    4, 0xf93fffef, 1, 0x80300000 },
  #endif
    { 0x0010, eRD_ONLY, 4, 0xff1fffef, 1, 0x400A0000 }, /* Boot change from 0x10 to 0x400A0000 */
#endif
    ...
    { 0x001c, eRD_WR,    4, 0x0000403f, 1, 0x00000000 },
#elif (CONFIG_CHIP_VER_CURR == CONFIG_CHIP_VER_MT5363)
    { 0x001c, eRD_ONLY, 4, 0xf3ffc037, 1, 0x70010000 },
#elif (CONFIG_CHIP_VER_CURR == CONFIG_CHIP_VER_MT5365) || (CONFIG_CHIP_VER_CURR == CONFIG_CHIP_VER_MT5395)
    { 0x001c, eRD_ONLY, 4, 0xf3ffc037, 1, 0x00000000 },
#else
    { 0x001c, eRD_WR,    4, 0xf3ffc037, 1, 0x70010000 },
#endif
...
#if(CONFIG_CHIP_VER_CURR == CONFIG_CHIP_VER_MT8550)  //added mhzhang
    { 0x0054, eRD_ONLY,  4, 0x001f03ff, 1, 0x00120320},
    { 0x0058, eRD_WR,     4, 0x0000ffff, 1, 0x0000f000},
#endif
    /* XXX */
...
{
    INT32 i4Ret;
    UINT32 u4CardType = FCI_TYPE_NONE;
...
#ifdef CC_BD_FCI
    // Enable FCI Clock
    CKGEN_AgtOnClk(e_CLK_SD);
    CKGEN_AgtOnClk(e_CLK_MS);    
#endif
...
#endif    
    {
        ...
        if (i4Ret == 0)
        {
                ...
                u4CardType = FCI_TYPE_SD;
        }
    }

#if !(CONFIG_CHIP_VER_CURR == CONFIG_CHIP_VER_MT5387) && !(CONFIG_CHIP_VER_CURR == CONFIG_CHIP_VER_MT5365) && !(CONFIG_CHIP_VER_CURR == CONFIG_CHIP_VER_MT5395)
...
#endif    
    {
        ...
        if (i4Ret == S_OK)
        {
            ...
            u4CardType |= FCI_TYPE_MS;
        }
    }
#endif // #if !(CONFIG_CHIP_VER_CURR == CONFIG_CHIP_VER_MT5387)

    if(u4CardType == FCI_TYPE_NONE)
    {
...
INT32 FCI_Stop(void)
{
    ...
    // de-register HISR to FCI SDIO interrupt handler.
    FCIHW_SetSDIOINTFct(NULL);
#endif
...
INT32 FCI_UTIL_RegDefChk(UINT32 u4BaseAddr, REG_TEST_T *psrgtRegList)
{
    ...
    for (psrgt=psrgtRegList; psrgt->eRegType!=eNO_TYPE; psrgt++)
    {
        ...
        if ((u4Tmp & psrgt->u4Mask) !=psrgt->u4DfVal) {
            UTIL_Printf("Reg ofst[0x%08x]: dftval should be [0x%x &]:0x%x but is 0x%x\n", u4BaseAddr + psrgt->u4Addr,
...
INT32 FCI_UTIL_RegRWTest(UINT32 u4BaseAddr, REG_TEST_T *psrgtRegList)
{
    ...
    for (psrgt=psrgtRegList; psrgt->eRegType!=eNO_TYPE; psrgt++) {
        ...
        case eRD_ONLY:
            switch(psrgt->iRegLen) {
            case 1: u1Tmp = IO_READ8(u4BaseAddr, psrgt->u4Addr); x_sprintf(szBuf, "0x%02x", u1Tmp); break;
            case 2: u2Tmp = IO_READ16(u4BaseAddr, psrgt->u4Addr); x_sprintf(szBuf, "0x%04x", u2Tmp); break;
            case 4: u4Tmp = IO_READ32(u4BaseAddr, psrgt->u4Addr); x_sprintf(szBuf, "0x%08x", u4Tmp); break;
            default: return 1;
            } /* end of switch iRegType */
            // UTIL_Printf("ROReg ofst[0x%04x]: %s(%d)\n", psrgt->u4Addr, szBuf, psrgt->iRegLen);
            break;
        case eWR_ONLY:
            for (i=0; i<6; i++) {
        ...
        case eRD_WR:
            ...
            for (i=0; i<6; i++) {
                ...
                case 1:
                    ...
                    if (u1Tmp!=u1Pre) { x_sprintf(szBuf, "0x%02x->0x%02x", u1Pre, u1Tmp); }
                    break;
                case 2:
                    u2Tmp = IO_READ16(u4BaseAddr, psrgt->u4Addr);
                    if (u2Tmp!=u2Pre) { x_sprintf(szBuf, "0x%04x->0x%04x", u2Pre, u2Tmp); }
                    break;
                case 4:
                    u4Tmp = IO_READ32(u4BaseAddr, psrgt->u4Addr);
                    if (u4Tmp!=u4Pre) { x_sprintf(szBuf, "0x%08x->0x%08x", u4Pre, u4Tmp); }
                    break;
                default: return 1;
                } /* end of switch iRegType */
                if (szBuf[0]!=0) { /* read/write value different */
                    UTIL_Printf("RWReg ofst[0x%08x]: r/w diff %s(%d)\n", u4BaseAddr + psrgt->u4Addr, szBuf, psrgt->iRegLen);
                    i4Ret = 1;
                }
            } /* for test patter loop */
            switch(psrgt->iRegLen) {
...
INT32 FCI_Diag(void)
{
    ...
    i4Val |= i4Ret;
    UTIL_Printf("Register default value check .............. %s\n", (i4Ret ? "FAIL" : "PASS"));
    i4Ret = FCI_UTIL_RegRWTest(FCI_IO_BASE_ADDR, arFCIRgtList);
    i4Val |= i4Ret;
    UTIL_Printf("Register read/write test .................. %s\n", (i4Ret ? "FAIL" : "PASS"));
    i4Ret = _Fci_AllSpaceRWTest(FCI_IO_BASE_ADDR, FCI_REG_LENGTH);
    i4Val |= i4Ret;
    UTIL_Printf("Memory space read/write test .............. %s\n", (i4Ret ? "FAIL" : "PASS"));
    ...
    UTIL_Printf("FCI detect %d device ....................... %s\n", ((i4DevNum >= 0) ? i4DevNum : 0),
