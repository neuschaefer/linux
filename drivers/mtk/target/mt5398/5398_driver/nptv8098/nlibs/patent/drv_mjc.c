...
CODE MJC_REGTBL_T MJC_TBA_INIT[] =
{
    {MJC_ME_TBA_0, 0x00232A06, 0xFFFFFFFF}, 
    {MJC_ME_TBA_1, 0x34680568, 0xFFFFFFFF}, 
    #ifndef MJC_TODO
    {MJC_MV_28, 0x00000000, 0xFFFFFFFF}, 
    {MJC_MV_29, 0x00000000, 0xFFFFFFFF}, 
    #endif
    {MJC_MV_2A, 0x00000000, 0xFFFFFFFF}, 
    {MJC_MV_TBA_0, 0x08070808, 0xFFFFFFFF},
    {MJC_MV_TBA_1, 0x023F0404, 0xFFFFFFFF}, 
    {MJC_MV_TBA_2, 0x02371F08, 0xFFFFFFFF}, 
    {MJC_MV_TBA_3, 0x35460256, 0xFFFFFFFF}, 
    {MJC_MV_TBA_4, 0x666625AD, 0xFFFFFFFF}, 
    {MJC_MV_TBA_5, 0x01606000, 0xFFFFFFFF}, 
    {MJC_MV_TBA_6, 0x00609FFF, 0xFFFFFFFF}, 
    {MJC_MV_TBA_7, 0x04303323, 0xFFFFFFFF}, 
    {MJC_MV_TBA_8, 0x0F12F636, 0xFFFFFFFF}, 
    {MJC_MV_TBA_9, 0xA0300505, 0xFFFFFFFF}, 
    {MJC_CA_TBA_0, 0x00686845, 0xFFFFFFFF}, 
    {MJC_CA_TBA_1, 0x00441F58, 0xFFFFFFFF}, //tba enable
    {MJC_REGTBL_END, 0x00000000, 0x00000000}
};
...
UINT8 u1MJCTriggerModeChg = SV_FALSE;
...
UINT32 u4DrvMJCRemapMJCFbmMode(UINT32 u4FbmMode)
{
    ...
    default:
    case FBM_POOL_MODE_FRC_Y10_C10_444: return (((bIsScalerInput444(SV_VP_MAIN) == SV_FALSE) && (arMJCPrm.arFRCConfig.u1PanelType == E_MJC_60HZ))?
		                                        FRC_Y10_C10_422RPT : FRC_Y10_C10_444);
    case FBM_POOL_MODE_FRC_Y08_C08_444: return (((bIsScalerInput444(SV_VP_MAIN) == SV_FALSE) && (arMJCPrm.arFRCConfig.u1PanelType == E_MJC_60HZ))?
		                                        FRC_Y08_C08_422RPT : FRC_Y08_C08_444);         
...
void vDrvMJCInit(UINT8 bForceInit)
{
    ...
    vDrvMJCLoadRegTbl(MJC_TBA_INIT);
    #endif
    ...
    vDrvMJCSet50To120(MJC_SUPPORT_50TO120);
    ...
    //init PR
    vDrvMJCInitPR();
	vDrvMJCSet32PdInfoShiftProc(0, 1);
}
...
{
    #ifndef CC_FPGA
    MJC_WRITE_FLD_MULTI(MJC_SYS_CLK_CTRL,
...
void vDrvMJCSetBuf(const MJC_DRAM_ALLOC_T * prDramAlloc)
{
	...
	if ((!IS_LVDS_DISP_3D_SHUTTER) && (!IS_PANEL_PDP))
	{
		u4FrameDramSize >>=1;
		u4PRDramSize    >>=1;
	}
	...
	if(fgDram444 && !PANEL_IsSupport120Hz())
	{
...
UINT8 u1MJCIs1R1WMode(void)
{
    return  (MJC_READ_FLD(MJC_FB_08, MJC_DRAM_BASE_ADDR) == MJC_READ_FLD(MJC_PR_00, MJC_PR_BASE_ADDR));
}
...
void vDrvMJCSetSystemMode(UINT32 u4MjcSystemMode)
{
    ...
    if (u1Dram444)
    {
        if ((arMJCPrm.ar3DConfig.u1MJC3DModeIn == E_VDO_MJC_2D) && (arMJCPrm.u1Dram422RPT == SV_OFF))
        {
            // 444->422 Off
...
void vDrvMJCSetFlipMirror(UINT32 u4FlipOnOff, UINT32 u4MirrorOnOff)
{
    ...
    if((arMJCPrm.arPicSize.u2OutHeight == 544) && ((arMJCPrm.arFRCConfig.u13DModeIn== E_VDO_MJC_SBS )||
		(arMJCPrm.arFRCConfig.u13DModeIn== E_VDO_MJC_FS)))
    {
...
void vDrvMJCSet32PdInfoShiftProc(UINT8 fgIsInterlace, UINT8 fgLineSyncMode)
{
    ...
    arMJCPrm.fgLineSyncMode = fgLineSyncMode;

    if (fgIsInterlace == SV_TRUE)
    {
        u4PdInfo32Shift = (fgLineSyncMode != TRUE) ? 0 : 1;
        //u4PdInfo32Shift = (SV_VD_TVD3D == bGetVideoDecType(SV_VP_MAIN))? 1 : u4PdInfo32Shift;
    ...
    else
    {
        ...
        {
            MJC_WRITE_FLD(MJC_SYS_00, ((fgLineSyncMode == TRUE) ? SV_OFF : SV_ON), MJC_PD_INFO_DLY);
            u4PdInfo32Shift = 1;
            MJC_WRITE_FLD(MJC_SYS_00, u4PdInfo32Shift, MJC_PD_INFO_32_SHIFT);
        ...
        {
            u4PdInfo32Shift = (fgLineSyncMode != TRUE) ? 2 : 3;
            MJC_WRITE_FLD(MJC_SYS_01, SV_OFF, MJC_IN_TEST_V_DLY);
            MJC_WRITE_FLD(MJC_SYS_00, SV_OFF, MJC_PD_INFO_DLY);
            MJC_WRITE_FLD(MJC_SYS_00, u4PdInfo32Shift, MJC_PD_INFO_32_SHIFT);
            vDrvMJCFrmSchSetPdInfoDly(1);
        }
		vRegWriteFldAlign(MJCFD_00, SV_OFF , MJCFD_C_FD_INFO_SEL);
        vRegWriteFldAlign(MJCFD_00, SV_OFF , MJCFD_C_FD_SRC_SEL);
    }
	vIO32WriteFldAlign(MJC_PD32_SHIFT_INFO, arMJCPrm.fgIsInterlace , MJC_PD32_SHIFT_INTERLACE);
	vIO32WriteFldAlign(MJC_PD32_SHIFT_INFO, arMJCPrm.fgLineSyncMode , MJC_PD32_SHIFT_LINE_SYNC_MODE);
}
...
void vDrvMJCSetFrmSch(MJC_FRC_CONFIG_T * arMJCFRC)
{
    ...
    case E_VDO_MJC_TTD:
		...
		MJC_WRITE_FLD(MJC_PR_03, SV_OFF, MJC_PR_MJC1W_PR1R_MODE);
        break;
    case E_VDO_MJC_FS:
		...
		MJC_WRITE_FLD(MJC_PR_03, (((u1MJCIs1R1WMode() == SV_TRUE) && (arMJCFRC->u1PanelType == E_MJC_60HZ))? SV_ON : SV_OFF), MJC_PR_MJC1W_PR1R_MODE);
        break;
    case E_VDO_MJC_SBS:
    case E_VDO_MJC_TAB:
    ...
    if ((arMJCFRC->u13DModeIn == E_VDO_MJC_TTD) && ((arMJCFRC->u1InFrmRate == 60) || (arMJCFRC->u1InFrmRate == 24)))
    {
        fgFallback64 = SV_ON;
        fgFallback55p2 = SV_OFF;
    }
    ...
    u4DramFrmCnt = (fgProgFrmRateEn && !(arMJCFRC->fgIs50To120)) ? 5 : 8;
        
    MJC_WRITE_FLD(MJC_SYS_04, (fgProgFrmRateEn && !(arMJCFRC->fgIs50To120)), MJC_PROG_FRM_EN);
    ...
    if (arMJCFRC->u13DModeIn == E_VDO_MJC_TTD)
    {
    ...
    else if (arMJCFRC->u13DModeIn)
    {
        MJC_WRITE_FLD_MULTI(MJC_FB_LR3D,
            P_Fld(SV_ON, MJC_3D_MODE) |
            P_Fld(arMJCFRC->u13DModeIn - 1, MJC_3D_TYPE));
        MJC_WRITE_FLD(MJC_FB_09, (arMJCFRC->u13DModeIn != E_VDO_MJC_FS), MJC_MID_VSYNC_IN_EN);
        MJC_WRITE_FLD(MJC_FM_00, (arMJCFRC->u13DModeIn != E_VDO_MJC_FS), MJC_LR3D_IN_FREERUN);
    ...
    vDrvMJCSetFrmMask(((arMJCFRC->fgIs50To120)? 5 : 0), 0);
    vIO32WriteFldAlign(MJC_FW_SCH_03, (arMJCFRC->u13DModeIn == E_VDO_MJC_TTD), MJC_50TO120_TTD);
    vIO32WriteFldAlign(MJC_FW_SCH_03, ((fgProgFrmRateEn && arMJCFRC->fgIs50To120 && arMJCFRC->u13DModeIn) || 
		                                (!fgProgFrmRateEn && arMJCFRC->fgIs50To120 && (arMJCFRC->u13DModeIn == E_VDO_MJC_2D ||
		                                 arMJCFRC->u13DModeIn == E_VDO_MJC_TTD))) ? 1 : 0, MJC_50TO120_DOUBLE);
    vIO32WriteFldAlign(MJC_FW_SCH_03, (fgProgFrmRateEn && arMJCFRC->fgIs50To120 && (arMJCFRC->u13DModeIn == E_VDO_MJC_2D ||
		                               arMJCFRC->u13DModeIn == E_VDO_MJC_TTD)), MJC_50TO120_TTD_25P);
    vIO32WriteFldMulti(MJC_FW_SCH_01, P_Fld(arMJCFRC->fgIs50To120 ? 1 : (MJC_SUPPORT_MULTI_CADENCE&&(arMJCFRC->u1InFrmRate == 60)&&
		                              (arMJCFRC->u13DModeIn == E_VDO_MJC_2D)? 2 : 0) /* if muticadence, set 2*/, MJC_FW_FRM_SCH_SWITCH)|
                                      P_Fld(arMJCFRC->u1PanelType == E_MJC_60HZ, MJC_FW_FRM_SCH_1X)|
                                      P_Fld(fgProgFrmRateEn ? 0 : ((arMJCFRC->u1InFrmRate % 50) == 0 ? 0x10 : 0x1F), MJC_FW_FRM_SCH_AUTO_DET));
    ...
    MJC_WRITE_FLD(MJC_SYS_00, ((arMJCFRC->u13DModeIn == E_VDO_MJC_TTD) ? SV_ON : SV_OFF), MJC_GRP_RPT_EN);
...
void vDrvMJCSetPicSize(const MJC_PIC_SIZE_T * arMjcPicSize)
{
    ...
    MJC_WRITE_FLD(MJC_SYS_0C, ((arMjcPicSize->u2InHeight == 544) && (arMJCPrm.arFRCConfig.u13DModeIn == E_VDO_MJC_SBS)) ? 4 : 0, MJC_IN_BLACK_H);
...
#ifdef TTD_TODO
void vDrvMJCSetTTDCtrl(void)
{
#if 0
    if (arMJCFRC->u13DModeIn == E_VDO_MJC_2D)
    {
        MJC_WRITE_FLD(MJC_FB_LR3D, SV_OFF, MJC_2D_TO_3D_MODE);
        MJC_WRITE_FLD(MJC_SYS_DB, SV_ON, MJC_TTD_BYPASS);
        
        MJC_WRITE_FLD(MJC_SYS_CLK_CTRL, SV_OFF, MJC_CLK_O_EN);
    ...
    MJC_WRITE_FLD(MJC_SYS_CLK_CTRL, SV_ON, MJC_CLK_O_EN);
    
    MJC_WRITE_FLD(MJC_SYS_CLK_CTRL, SV_ON, MJC_OUTPUT_RST);
    MJC_WRITE_FLD(MJC_SYS_CLK_CTRL, SV_OFF, MJC_OUTPUT_RST);
    ...
    if (u4TTD != u4MJCTTDPrev)
    {
        ...
        vDrvTTDSetEnable(SV_OFF);
    #else
		if ((u1MJCIs1R1WMode() == SV_TRUE) && (!PANEL_IsSupport120Hz()))
	    {
		...
		else
		{
            ...
            else if ((eTTDIn == E_TD_IN_2D_P) && (eTTDOut == E_TD_OUT_3D_LI))
            {
                MJC_WRITE_FLD(MJC_FB_LR3D, SV_OFF, MJC_2D_TO_3D_MODE);
                MJC_WRITE_FLD(MJC_SYS_DB, SV_OFF, MJC_TTD_BYPASS);          
                MJC_WRITE_FLD(MJC_SYS_CLK_CTRL, SV_ON, MJC_CLK_O_EN);         
            ...
            else if ((eTTDIn == E_TD_IN_LI_P) && (eTTDOut == E_TD_OUT_3D_LI))
            {
                MJC_WRITE_FLD(MJC_FB_LR3D, SV_OFF, MJC_2D_TO_3D_MODE);
                MJC_WRITE_FLD(MJC_SYS_DB, SV_OFF, MJC_TTD_BYPASS);          
                MJC_WRITE_FLD(MJC_SYS_CLK_CTRL, SV_ON, MJC_CLK_O_EN);         
...
void vDrvMJCSetPRCtrl(MJC_FRC_CONFIG_T* arMJCFRC , MJC_PIC_SIZE_T * arMJCPicSize)
{
    ...
    fgPRCtrl = ((arMJCFRC->u13DModeIn == E_VDO_MJC_2D) || (arMJCFRC->u13DModeIn == E_VD0_MJC_OUT_NoPR) || (fgPROnOff == SV_OFF));
    if (fgPRCtrl && (arMJCFRC->u1FHPRMode != E_MJC_HPR2160))
    { 
    ...
    if(MJC_READ_FLD(MJC_SYS_PR_0, MJC_PR_SYNC_REGEN_BYPASS) == SV_OFF)
    {
        ...
        MJC_WRITE_FLD(MJC_PR_03, (arMJCFRC->u13DModeIn == E_VDO_MJC_2D), MJC_PR_SG_LR);
    ...
    MJC_WRITE_FLD(MJC_SYS_PR_0, (((arMJCFRC->u1FHPRMode == E_MJC_HPR120) && (arMJCFRC->u13DModeIn != E_VDO_MJC_2D))? 1 : 0), MJC_PR_SYNC_REGEN_CEN_DCM);
}

void vDrvMJC1W_PR1RCtrl(void)
{
    static UINT8 u1fgcheck = 0;
    if((u1DrvMJCGet1W1Rmode() == 1) && (MJC_READ_FLD(MJC_PR_03, MJC_PR_MJC1W_PR1R_MODE) == SV_ON))
    {
        MJC_WRITE_FLD_MULTI(MJC_FB_01,
                P_Fld(1, MJC_DRAM_RPT)|P_Fld(1, MJC_DRAM_RPT_422)|P_Fld(0, MJC_FB_TWO_FRM_EN));
...
void vDrvMJCSetPRMode(UINT8 u1PRMode , UINT8 u1FHPRMode)
{
    if(u1PRMode == E_MJC_FPR)       
    {
	    ...
	    vIO32WriteFldAlign(MJC_PR_MODE, E_MJC_NOHPR, MJC_HALF_PR_MODE);
    }
    else
    {
...
void vDrvMJCSet3DLRDly(const MJC_PIC_SIZE_T * arMjcPicSize)
{
    UINT32 u4LR3DDly;
    UINT8 bTTDBypassed;

    bTTDBypassed = (MJC_READ_FLD(MJC_SYS_DB, MJC_TTD_BYPASS) == 1)? SV_TRUE : SV_FALSE;
    
    u4LR3DDly = (bTTDBypassed == SV_TRUE)? 3 : 0;    

	if(LVDS_DISP_3D == LVDS_DISP_3D_SHUTTER)
	{
        if(bTTDBypassed == SV_FALSE)
        {
            LOG(1, "Set IN_HEIGHT(%d) OUT_HEIGHT (%d)\n", arMjcPicSize->u2InHeight, arMjcPicSize->u2OutHeight);
            if(arMjcPicSize->u2OutHeight >= arMjcPicSize->u2InHeight)
            {
                u4LR3DDly = (u4LR3DDly + 1) % 4;
            }
        }
        arMJCPrm.u1DispRDly = u4LR3DDly;
        LOG(1, "vDrvMJCSet3DLRDly::Set MJC LR 3D dely to (%d)\n", u4LR3DDly);
        if (arMJCPrm.fgDispRSwp)
        {
            u4LR3DDly = (u4LR3DDly + 1) % 4;
        }
	}
    vDrvMJCSetDispRDly(u4LR3DDly);
}
...
{    
    arMJCFRC->u1FHPRMode = u1DrvMJCGetHPRMode();
    arMJCPrm.arMJCPRConfig.u1OutPRFrameRate = vDrvGetLCDFreq();
    if(fgMJC3DModeDbg == 1)
    {
        arMJCFRC->u13DModeIn = u1MJC3DMode;
    }
    else
    {
        arMJCFRC->u13DModeIn = arMJCPrm.ar3DConfig.u1MJC3DModeIn; 
		arMJCFRC->u13DModeOut = arMJCPrm.ar3DConfig.u1MJC3DModeOut;
    }
	
    arMJCFRC->u1OutFrmRate = (((arMJCFRC->u1FHPRMode > E_MJC_HPR120) && (arMJCFRC->u13DModeIn != E_VDO_MJC_2D) && 
		                       (arMJCFRC->u13DModeOut != E_VD0_MJC_OUT_NoPR))? (arMJCPrm.arMJCPRConfig.u1OutPRFrameRate * 2) : 
		                       arMJCPrm.arMJCPRConfig.u1OutPRFrameRate);  
							   
    arMJCFRC->u1InFrmRate = bDrvVideoGetRefreshRate(SV_VP_MAIN);

    arMJCFRC->fgIs50To120 = ((arMJCFRC->u1InFrmRate % 25 == 0) &&
        ((arMJCFRC->u13DModeIn != E_VDO_MJC_2D && (u1DrvMJCIsSupport50To120()&MJC_SUPPORT_50TO120_3D)) ||
         (arMJCFRC->u13DModeIn == E_VDO_MJC_2D && (u1DrvMJCIsSupport50To120()&MJC_SUPPORT_50TO120_2D))));

    if (arMJCPrm.arMJCPRConfig.u1OutPRFrameRate < 96)
    {
        arMJCFRC->u1PanelType = E_MJC_60HZ;
        arMJCFRC->u1OutFrmRate = ((arMJCFRC->fgIs50To120 ) ? 60 : arMJCFRC->u1OutFrmRate);
    }
    else if (arMJCPrm.arMJCPRConfig.u1OutPRFrameRate < 192)
    {
        arMJCFRC->u1PanelType = E_MJC_120HZ;
        arMJCFRC->u1OutFrmRate = ((arMJCFRC->fgIs50To120) ? 120 : arMJCFRC->u1OutFrmRate);
    }
    else
    {
        arMJCFRC->u1PanelType = E_MJC_240HZ;
        arMJCFRC->u1OutFrmRate = ((arMJCFRC->fgIs50To120) ? 240 : arMJCFRC->u1OutFrmRate);
    }
    ...
    if ((arMJCFRC->u1OutFrmRate != arMJCPrm.arFRCConfig.u1OutFrmRate) ||
        ...
        (arMJCFRC->fgIs50To120 != arMJCPrm.arFRCConfig.fgIs50To120))
    {
        return SV_TRUE;
    }
    else
...
static BOOL fgDrvMJCGetPicSize(MJC_PIC_SIZE_T * arPicSize)
{
    ...
    arPicSize->u2OutWidth = (arMJCPrm.arFRCConfig.u13DModeIn == E_VDO_MJC_SBS) ? (u2Width >> 1) : u2Width;
    arPicSize->u2InWidth  = ALIGN_16_CEIL(arPicSize->u2OutWidth);

    arPicSize->u2OutHeight = u2Height;
    arPicSize->u2InHeight  = ((u1PRMode == E_MJC_HPR) && (arMJCPrm.arFRCConfig.u13DModeIn != E_VDO_MJC_2D) && (arMJCPrm.arFRCConfig.u13DModeOut != E_VD0_MJC_OUT_NoPR)) 
		                     || (arMJCPrm.arFRCConfig.u1FHPRMode == E_MJC_HPR2160) ? (u2Height >> 1) : u2Height;
    arPicSize->u2InHeight  = ALIGN_8_CEIL(arPicSize->u2InHeight);

    arPicSize->u2OutHTotal = (arMJCPrm.arFRCConfig.u13DModeIn == E_VDO_MJC_SBS) ? 
        (wDrvGetOutputHTotal() >> 1) : wDrvGetOutputHTotal();         
    arPicSize->u2OutVTotal = wDrvGetOutputVTotal();          

    if ((arMJCPrm.arFRCConfig.u13DModeIn == E_VDO_MJC_TAB) && (arPicSize->u2OutHeight & 0xF) && (u1PRMode == E_MJC_HPR) && (arMJCPrm.arFRCConfig.u13DModeOut != E_VD0_MJC_OUT_NoPR))
    {
        arPicSize->fgUdp4 = SV_ON;
    }

    if ((arMJCPrm.u1Flip == 1) && (arPicSize->u2OutHeight & 0xF)  && (arMJCPrm.arFRCConfig.u13DModeOut != E_VD0_MJC_OUT_NoPR) &&
        ((arMJCPrm.arFRCConfig.u13DModeIn == E_VDO_MJC_SBS )||(arMJCPrm.arFRCConfig.u13DModeIn == E_VDO_MJC_FS) || (arMJCPrm.arFRCConfig.u13DModeIn == E_VDO_MJC_TTD)))
    {
        arPicSize->u2OutVStart += 8;
    }
    
	u4Prelen = IO32ReadFldAlign(PSPIP1_0A, PSPIP1_0A_VS_PSCAN_PRELEN_1) - 2;
	arPicSize->u1DispVpos = (u4Prelen - arPicSize->u2OutVStart);
...
UINT32 fgDrvMJCModeChgProc(UINT32 u4Path)
{
    ...
    if (fgDrvMJCGetFRCConfig(&arMJCFRC) || IO32ReadFldAlign(MJC_MISC_00, MJC_FLMR_CHG_TRIG))
    {
        if (arMJCPrm.u1Dram444 == SV_FALSE)
        {
            ...
            u4MJCMuteCount = (arMJCFRC.u13DModeIn || arMJCPrm.arFRCConfig.u13DModeIn) ? (u4MJCMuteCount << 1) : u4MJCMuteCount;
        ...
        vDrvMJCSetProgME(arMJCFRC.u1PanelType, arMJCFRC.u1FrmSchMode == FALLBACK_MODE_55P2); 
        vIO32MJCWriteFldAlign(MJC_FBCK_13, 1, FBCK_THR_TRIG);
		vIO32MJCWriteFldAlign(MJC_FW_SCH_04, ((arMJCPrm.arFRCConfig.fgIs50To120 != arMJCFRC.fgIs50To120) && 
			                                 (arMJCPrm.arFRCConfig.u1InFrmRate == arMJCFRC.u1InFrmRate)), MJC_50TO120_CHA_CLK_FLG);
	    arMJCPrm.arFRCConfig = arMJCFRC;
        fgModeChg = SV_TRUE;
        LOG(2, "vDrvMJCModeChgProc: fgDrvMJCGetFRCConfig mode change \n");
    }
    ...
    if (fgDrvMJCGetPicSize(&arMJCPicSize))
    {
        ...
        {
            u4MJCMuteCount = IO32ReadFldAlign(MJC_MISC_00, MODE_CHG_MUTE_COUNT);
            u4MJCMuteCount = arMJCPrm.arFRCConfig.u13DModeIn ? (u4MJCMuteCount << 1) : u4MJCMuteCount;
            _vDrvVideoSetMute(MUTE_MODULE_MJC, SV_VP_MAIN, u4MJCMuteCount, SV_TRUE);
        }
        vDrvMJCSetPicSize(&arMJCPicSize);
		#ifdef TTD_TODO
        vDrvTTDSetPicSize(((arMJCPicSize.u2InWidth == 1440)? 1456 : arMJCPicSize.u2InWidth), arMJCPicSize.u2InHeight);
		#endif
        vDrvMJCSetOutputTiming(&arMJCPicSize);
        arMJCPrm.arPicSize = arMJCPicSize;
        fgModeChg = SV_TRUE;
        LOG(2, "vDrvMJCModeChgProc: fgDrvMJCGetPicSize mode change\n");
    }
    
    if (fgModeChg)
    {
		vDrvMJCSet3DLRDly(&arMJCPicSize);
...
void vDrvMJCModeChangeDone(UINT32 u4Path)
{
    if (u4Path == SV_VP_MAIN) 
    {
		...
		if (u1MJCTriggerModeChg == SV_ON)
		{
            u1MJCTriggerModeChg = SV_OFF;
		}
		else
		{
...
UINT32 u4DrvMJCBufferInit(void)
{
    ...
    {
        u4MJCBaseAddress =  prPool->u4Addr;
        if (prPool->u4Addr>= 0x20000000)    // MJC dram in Bank B
            u4MJCBaseAddress = (UINT32)BSP_MapReservedMem((void *)u4MJCBaseAddress, prPool->u4Size);
        else
            u4MJCBaseAddress = VIRTUAL(u4MJCBaseAddress);
    }
	...
	arMJCDramAlloc.u4Height    = (((IS_LVDS_DISP_3D_SHUTTER) || (IS_PANEL_PDP))? ALIGN_8_CEIL(prPool->u4Height) : ALIGN_16_CEIL(prPool->u4Height)); // for mjc height align to 8 => prDramAlloc->u4Height must align to 16 before /2 in polarized case
...
{
    MJC_3D_CONFIG_T arMJC3D ;
	UINT8 u1TDTV3DModeIn ; 
	UINT8 u1TDTV3DModeOut ;
	UINT8 fgMJC3DModeChg;
    ...
    if (IO32ReadFldAlign(MJC_MISC_00, MJC_PR_MODE_CHG_OFF) == SV_OFF)
    {
        ...
        arMJC3D.u1MJC3DModeIn = (u1TDTV3DModeIn == E_TD_IN_TTD_P) ? E_VDO_MJC_TTD:     
        ...
        arMJC3D.u1MJC3DModeOut = (u1TDTV3DModeOut == E_TD_OUT_3D_FPR) ? E_VD0_MJC_OUT_FPR : 
                               ...
                               ((u1TDTV3DModeOut == E_TD_OUT_3D_FS) || (u1TDTV3DModeOut == E_TD_OUT_NATIVE)) ? 
							   	E_VD0_MJC_OUT_NoPR : E_VD0_MJC_OUT_OTHER;
        ...
        if(arMJC3D.u1MJC3DModeIn != arMJCPrm.ar3DConfig.u1MJC3DModeIn)
        { 
			arMJCPrm.ar3DConfig.u1MJC3DModeIn = arMJC3D.u1MJC3DModeIn;
			fgMJC3DModeChg = SV_ON;
            fgMJCModeChg = SV_TRUE;
        }
		
        if(arMJC3D.u1MJC3DModeOut != arMJCPrm.ar3DConfig.u1MJC3DModeOut)
        {
            if(IS_PANEL_PDP)
            {
                vDrvMJCSetPRMode(E_MJC_HPR , E_MJC_HPR2160);         //set half pr 2160 for pdp panel 
            }
            if (arMJC3D.u1MJC3DModeOut == E_VD0_MJC_OUT_HPR)
            {
                if(vDrvGetLCDFreq() < 96)
                {
            ...
            if (arMJC3D.u1MJC3DModeOut == E_VD0_MJC_OUT_FPR) 
            {
            ...
            arMJCPrm.ar3DConfig.u1MJC3DModeOut = arMJC3D.u1MJC3DModeOut;
			fgMJC3DModeChg = SV_ON;
            fgMJCModeChg = SV_TRUE;
        }
    }
...
void vDrvMJCSrmCb(UINT32 u4VdpId, UINT32 u4Mode, UINT32 u4PDSize, UINT32 u4SramMode)
{
	...
	{
		LOG(3,"vDrvMJCSrmCb mjc  3d format mode change;\n");
	}
    ...
    if ((arMJCDramAlloc.u2DebugMode != 0xFFFF) || (arMJCDramAlloc.u2Mode != u4DrvMJCRemapMJCFbmMode(u4Mode)))
    {
        ...
        LOG(3, "vDrvMJCSrmCb: %x  , bIsScalerInput444:%d\n", arMJCDramAlloc.u2Mode, u1PreScaler444);
...
void vDrvMJCEnablePatGen(UINT32 u4InOut, UINT32 u4OnOff)
{
    if (u4InOut == 0)
    {
        MJC_WRITE_FLD(MJC_SYS_0A, arMJCPrm.arFRCConfig.u13DModeIn == E_VDO_MJC_FS ? 1 : 0, MJC_PTG_3D_MODE);
        MJC_WRITE_FLD(MJC_SYS_0F, u4OnOff, MJC_PTG_PAT_EN);
    }
    else
    {
        MJC_WRITE_FLD(MJC_SYS_0A, arMJCPrm.arFRCConfig.u13DModeIn != E_VDO_MJC_2D ? (arMJCPrm.arFRCConfig.u1OutFrmRate >= 192 ? 2 : 1) : 0, MJC_PTG_3D_MODE);
...
void vDrvMJCFrmsch50To120Patch(void)
{
	if ((u1TrigSet50TO120TRG ) && (bDrvVideoSignalStatus(SV_VP_MAIN) == SV_VDO_STABLE))
	{
	    u1TrigSet50TO120TRG --;
		vIO32WriteFldAlign(MJC_FW_SCH_03, SV_OFF, MJC_50TO120_EN);
		if ((u1TrigSet50TO120TRG == 0) && (IO32ReadFldAlign(STA_DDDS_00, STA_DDDS_00_DDDS_LOCK) == SV_ON))
		{
			vIO32WriteFldAlign(MJC_FW_SCH_03, arMJCPrm.arFRCConfig.fgIs50To120, MJC_50TO120_EN);
			u1TrigSet50TO120TRG = 0;
			LOG(2,"++++++vDrvMJCFrmsch50To120Patch\n");
		}
	}
}
...
void vDrvMJCMainloop(void)
{
	...
	UINT8 fgLineSyncMode = 1;
	UINT8 fgMJCIs3DFSMode = 0;

	fgIsInterlace = bDrvVideoIsSrcInterlace(SV_VP_MAIN);
	fgLineSyncMode = (getScalerMode(SV_VP_MAIN) == VDP_SCPOS_LINESYNC_DISPMODE);
	fgMJCIs3DFSMode = (arMJCPrm.ar3DConfig.u1MJC3DModeIn == E_VDO_MJC_FS);
	
	if ((fgMJCModeChg) || (IO32ReadFldAlign(MJC_MISC_00, MJC_MODE_CHG_TRIG_AGAIN) == SV_ON))
    {
        if (fgDrvMJCModeChgProc(SV_VP_MAIN))
        {
		   u1TrigSet50TO120TRG = ((arMJCPrm.fgSupport50To120)? (fgLineSyncMode ? 0x36 : 0x18) : 0);
		   LOG(2,"++++_u1TrigSet50TO120TRG \n");
        }
        fgMJCModeChg = SV_FALSE;
		vIO32MJCWriteFldAlign(MJC_MISC_00, SV_OFF , MJC_MODE_CHG_TRIG_AGAIN);
    }

    if(arMJCPrm.fgSupport50To120)
	{
        if ((bDrvVideoSignalStatus(SV_VP_MAIN) == SV_VDO_STABLE) &&
		     (IO32ReadFldAlign(MJC_FW_SCH_04,  MJC_50TO120_CHA_CLK_FLG) == SV_ON))
        {
		    vIO32MJCWriteFldAlign(MJC_FW_SCH_04, SV_OFF , MJC_50TO120_CHA_CLK_FLG);
            vSetMainFlg(MAIN_FLG_MODE_CHG);
            vSetMainFlg(MAIN_FLG_MODE_DET_DONE);
		    vDrvSetMJC50TO120ModeChangeFlg(SV_ON);
			u1MJCTriggerModeChg = SV_ON;
	        LOG(2,"++++vSetMainFlg\n");
	    }
	}
    ...
    if (IO32ReadFldAlign(MJC_MISC_00, MJC_MODE_CHG_TRIG))
    {
   	...
   	if((fgIsInterlace != IO32ReadFldAlign(MJC_PD32_SHIFT_INFO, MJC_PD32_SHIFT_INTERLACE)) || 
	   (fgLineSyncMode != IO32ReadFldAlign(MJC_PD32_SHIFT_INFO, MJC_PD32_SHIFT_LINE_SYNC_MODE)) ||
	   (fgMJCIs3DFSMode != IO32ReadFldAlign(MJC_PD32_SHIFT_INFO, MJC_PD32_SHIFT_IS_FS)))
	{
	    LOG(1, "vDrvMJCSet32PdInfoShift: fgIsInterlace :	%d fgLineSyncMode:%d\n",fgIsInterlace,fgLineSyncMode);
        vDrvMJCSet32PdInfoShiftProc(fgIsInterlace, fgLineSyncMode);		
		vIO32WriteFldAlign(MJC_PD32_SHIFT_INFO, fgMJCIs3DFSMode, MJC_PD32_SHIFT_IS_FS);
	}
...
void vDrvSetMJC50TO120ModeChangeFlg(UINT8 u1OnOff)
{   
	vIO32MJCWriteFldAlign(MJC_FW_SCH_04, u1OnOff , MJC_50TO120_CHA_CLK_TTDFLG);
}

UINT8 u1DrvGetMJC50TO120ModeChangeFlg(void)
{
    return(IO32ReadFldAlign(MJC_FW_SCH_04,  MJC_50TO120_CHA_CLK_TTDFLG));
}
...
void vDrvMJCSetFrmFreeze(UINT32 u4OnOff)
{
    ...
    MJC_WRITE_FLD(MJC_FM_07, arMJCPrm.arFRCConfig.u13DModeIn ? u4OnOff : SV_OFF, MJC_FREEZE_3D_EN);
...
void vDrvMJCISR(UINT16 u2Vector)
{
    ...
    {        
        ...
        if(arMJCPrm.fgSupport50To120)
	    {
            vDrvMJCFrmsch50To120Patch();
        }
		#ifndef MJC_TODO
        vScpipOnMJCOutputVSync();   // check to scaler owner  need or not?
		#endif
...
UINT8 u1DrvMJCGetFrmDelay10x(void)
{
    ...
    if (u1DrvMJCIsOnOff())
    {
        ...
        {
            if (bDrvVideoGetRefreshRate(SV_VP_MAIN) <= 30)
            {
                u1Delay = 30;
            }
            else
...
UINT8 u1DrvMJCIsSupport50To120(void)
{
    return arMJCPrm.fgSupport50To120;
}

void vDrvMJCSet50To120(UINT8 u1Support50To120)
{
    arMJCPrm.fgSupport50To120 = u1Support50To120;
}
...
{
    UINT8 u12D3DMode;
    u12D3DMode = arMJCPrm.ar3DConfig.u1MJC3DModeIn;                     
...
void vDrvMJCPRQueryCtrl(void)
{
    ...
    Printf("\tTDTV PR ................: %d\n", arMJCPrm.ar3DConfig.u1MJC3DModeOut);
