...
// Static variables
//---------------------------------------------------------------------------
volatile static unsigned int _u4MsdcAccuVect = 0;
...
static int MsdcWaitClkStable (void)
{
	UINT32 i;
...
static void MsdcContinueClock (int i4ContinueClock)
{
    if (i4ContinueClock)
    {
...
static int MsdcSetClkfreq(unsigned int clkFreq) 
{
...
static int MsdcWaitIntr(unsigned int vector, unsigned int timeoutCnt) 
{
    UINT32 i;
...
static int MsdcSendCmd (sd_cmd_t *pCmd) 
{
    unsigned int respType = RESPONSE_NO;
    ...
    if (PIO_DATA_MODE == pCmd->dataMode) 
    {
        ...
        if (!(sdcCmd & SDC_CMD_WRITE))
        {
            if(_u4StartOffsetAlign)
            {
                do
                {
                    ...
                    while ((sizeof(int) <= u4RxFifoCnt) && (_u4StartOffsetAlign > 0))
                    {
                        // Read Data
                        MSDC_READ32(MSDC_RXDATA);                
                        u4RxFifoCnt -= sizeof(int);
                        _u4StartOffsetAlign -= sizeof(int);
                    }
            ...
            while (u4ActualEnd > (UINT32)pBufData) 
            {
                ...
                while ((u4ActualEnd > (UINT32)pBufData) && (sizeof(int) <= u4RxFifoCnt))
                {
                    // Read Data
                    *pBufData = MSDC_READ32(MSDC_RXDATA);
                     ...
                     if (u4RxCnt == 128)   // Check CRC error happens or not every 512 Byte
                     {
           ...
           if (_u4EndOffsetAlign)
           {
                do
                {
                    ...
                    while (sizeof(int) <= u4RxFifoCnt && (_u4EndOffsetAlign > 0))
                    {
                        // Read Data
                        MSDC_READ32(MSDC_RXDATA);                
                        u4RxFifoCnt -= sizeof(int);
                        _u4EndOffsetAlign -= sizeof(int);
                    }
		...
		else
		{
            while (u4BufEnd > (UINT32)pBufData) 
            {
                ...
                else if(sizeof(int) > u4TxFifoCnt)
				{
...
static int cmd0_go_idle_state(unsigned int arg)
{
	sd_cmd_t cmd;
...
static int cmd1_mmc_send_op_cond(void)
{
	sd_cmd_t cmd;
...
static int cmd2_all_send_cid(void)
{
	sd_cmd_t cmd;
...
static int cmd3_send_relative_addr(void) 
{
	sd_cmd_t cmd;
...
static int cmd6_mmc_set_bus_width(int busWidth) 
{
	sd_cmd_t cmd;
...
static int cmd7_select_card(unsigned int sel) 
{
	sd_cmd_t cmd;
...
static int cmd17_read_single_block(unsigned int addr, unsigned char *pBuff, unsigned int buffLen)
{
	sd_cmd_t cmd;
...
static int cmd18_read_multiple_block(unsigned int addr, unsigned char *pBuff, unsigned int buffLen) 
{
	sd_cmd_t cmd;
	UINT32 u4Tmp;
	...
	{
        cmd.arg = addr >> 9;    // Block addressing mode
        _u4StartOffsetAlign = addr - (addr & (~0x1FF));
	}
	else
	{
        cmd.arg = addr & (~0x1FF);           // Byte addressing mode
        _u4StartOffsetAlign = addr - cmd.arg;
	}
	
	u4Tmp = addr + buffLen - 1;
	u4Tmp &= (~0x1FF);
	u4Tmp += 0x200;
	cmd.buffLen = u4Tmp - (addr & (~0x1FF)); 
	
	_u4EndOffsetAlign = u4Tmp - (addr + buffLen);
	
	return MsdcSendCmd(&cmd);
}

static int cmd12_stop_transmission(void) 
{
	sd_cmd_t cmd;
...
static int MsdcCardTypeChk(void)
{
    // CMD2
...
static int MsdcSetBusWidth (int busWidth) 
{
...
static int MsdcMemRead (unsigned int addr, void *pBuff, unsigned int buffLen) 
{
...
static int MsdcEmmcBootStop (BOOL fgResetMsdc)
{    
    UINT32 i;
...
static void MSDC_PinMux()
{
    ...
    {  	    
	 ...
	 else                 //MSDC Controller 0, Base Address: 0xF006d000, pinmux register d414[21:20]/d40c[31:30]/d40c[17], function 2
	 {
...
//---------------------------------------------------------------------------
static int MsdcCardIdentify (void)
{    
...
static int MsdcIdentyCard (BOOL fgForceIdentCard)
{
...
static int MsdcReadCard (UINT32 u4Addr, UINT32 *pu4Buf, UINT32 u4Length)
{
    int i4Ret, i4RetryCnt = 0;

    do
    {
        i4Ret = MsdcMemRead(u4Addr, (void*)pu4Buf, u4Length);
...
static int MsdcIdentReadMMC (UINT32 *pu4Buf, UINT32 u4Addr, UINT32 u4Length)
{
    int i4Ret, i4RetryCnt = 0;
...
int CHIP_eMMCCopyToDRAM(UINT32 *pu4DstBuf, UINT32 u4ByteOffset, UINT32 u4Len)
{
    int i4Ret;
    UINT32 i4RetryCnt = 0;
	...
	return MsdcIdentReadMMC(pu4DstBuf, u4ByteOffset, u4Len);
}
