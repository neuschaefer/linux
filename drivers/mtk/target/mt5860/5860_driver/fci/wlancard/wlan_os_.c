...
static int wlan_probe(void)
{
    ...
    #include <asm/arch/clocks.h>
        ...
        UTIL_Printf("fclk: %lu, pclk: %lu, hclk: %lu, clkratio: 0x%08X\n", fclk, pclk, hclk, clkratio);
        UTIL_Printf("BWSCON: 0x%08X\n", BWSCON);
        UTIL_Printf("BANKCON5: 0x%08X\n", BANKCON5);
        UTIL_Printf("BANKCON3: 0x%08X\n", BANKCON3);
        UTIL_Printf("MPLLCON: 0x%08X\n", MPLLCON);
        UTIL_Printf("CLKDIVN: 0x%08X\n", CLKDIVN);
    ...
    if ((result = check_mem_region(MEM_MAPPED_ADDR, MCR_MEM_MAPPED_LENGTH)) < 0){
        UTIL_Printf(KERN_ALERT "IPN2126 check_mem_region fail\n");
        goto fail;
    }

    if (!kal_request_mem_region(MEM_MAPPED_ADDR, MCR_MEM_MAPPED_LENGTH, "IPN2126STA")) {
        UTIL_Printf(KERN_ALERT "IPN2126 request_mem_region fail \n");
        result = -EIO;
        goto fail;
    }
    membase = (u32)ioremap(MEM_MAPPED_ADDR, MCR_MEM_MAPPED_LENGTH);
    //UTIL_Printf(KERN_ALERT "IPN2126 remapped I/O base: [0x%08X] \n", membase);
    ...
    *((volatile unsigned long *)(0x80000000 + 0x0150)) |= 0x00040000;
    ...
    if (request_irq(MT5901IRQ, wlan_hw_interrupt, SA_INTERRUPT, "mt5911 loopback test", NULL)){
        UTIL_Printf("MT5911 request_irq(%d) failed\n", MT5901IRQ);
        result = -EIO;
        goto fail;
    }else{
        CON_MSG("use irq#%d\n", MT5901IRQ);
...
void sdio_notify_add(struct mmc_slot *slot)
{
    ...
    UTIL_Printf("SDIO notify card added!\n");
    x_slot = slot;
    result = slot->set_block_size(slot, 1, SDIO_BLKSIZE);

    if (result < 0){
        UTIL_Printf("WLAN: init SDIO Card, set block size fail, use byte mode\n");
    }else{
        UTIL_Printf("WLAN: init SDIO Card, set block size:[%d]\n", SDIO_BLKSIZE);
    }

    if (wlan_probe() < 0) {
        UTIL_Printf(KERN_ALERT "Failed to initialize SDIO card!\n");
        wlan_remove();
    }
    ...
    UTIL_Printf(KERN_ALERT "vTestLoopBack\n");
...
{
    UTIL_Printf("SDIO notify card removed!\n");
    x_slot = NULL;
...
int init_wlan_module(void)
{
    // We use UTIL_Printf only here and in cleanup_module, so that the module
...
// Max Liao, wlan card porting, 2006/03/02.
    ...
    wlan_remove();
#endif
    UTIL_Printf(KERN_ALERT "Unload .... %s\n", "MT5911 LoopBack Test");
...
void WLAN_Main(BOOL  fg4Bits)
{
    ...
    if ( i4Ret != S_OK)
    {
        UTIL_Printf("(%s):SDIO_EnableIO fail.\n", __FUNCTION__);
        return;
    }
    ...
    while (1)
    {
        ...
        if (u4Count > 1000)
        {
            ...
            // Avoid clock larger than 25 Mhz
            while(u4ClockSel[i4ClockIndex] > 25)
            {
              i4ClockIndex++;
            }
            ...
            UTIL_Printf("@@@@@ Change Clock to %d Mhz (index = %d)@@@@@ \n", u4ClockSel[i4ClockIndex], i4ClockIndex);
