...
static INT32 _CmdBootTime(INT32 i4Argc, const CHAR** aszArgv)
{
    ...
    TIME_STAMP_T *prTimeStamp;
    pu4Time = (UINT32 *)0xf00080e0;
    _CmdDisplayTimeItem(*pu4Time, "Preloader start time");
    pu4Time++;
    _CmdDisplayTimeItem(*pu4Time, "U-Boot start time");
    pu4Time++;
    _CmdDisplayTimeItem(*pu4Time, "Kernel start time");
    pu4Time++;
    _CmdDisplayTimeItem(*pu4Time, "Kernel end time");
...
{
#if CPU_TEST_ENABLE
    UINT32 u4DstSize, u4DstAddr, u4SrcSize, u4SrcAddr, u4GoldenAddress=0, u4Loop=1, u4Ret;
    if (i4Argc < 3)
    {
        Printf("lzma dst_addr src_addr {dst_size} {src_size}\n");
        return -1;
    }

#ifdef CC_MT5880
    u4DstAddr = StrToInt(aszArgv[1]);
    u4SrcAddr = StrToInt(aszArgv[2]);
    u4DstSize = StrToInt(aszArgv[3]);
    u4SrcSize = StrToInt(aszArgv[4]);
    if (i4Argc == 7)
    {
        u4GoldenAddress = StrToInt(aszArgv[5]);
        u4Loop = StrToInt(aszArgv[6]);
    }
#else
    UINT32 u4ImageAddr = StrToInt(aszArgv[2]);
    ...
    do
    {
        u4Ret = LZMA_Dec(u4DstAddr, u4DstSize, u4SrcAddr, u4SrcSize, NULL);
        if (u4GoldenAddress)
        {
            ASSERT(x_memcmp(u4GoldenAddress, u4DstAddr, u4DstSize) == 0);
        }
    }
    while (--u4Loop);
#else
...
{
    TZ_RunUT(StrToInt(aszArgv[1]), StrToInt(aszArgv[2]), StrToInt(aszArgv[3]));
    return 0;
}

static INT32 _CmdTzEnableDebug(INT32 i4Argc, const CHAR** aszArgv)
{
    TZ_EnableDebug((BOOL)StrToInt(aszArgv[1]));
    return 0;
}
