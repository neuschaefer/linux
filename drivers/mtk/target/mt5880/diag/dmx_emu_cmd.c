...
static INT32 _CmdDdiTransfer0(INT32 i4Argc, const CHAR** aszArgv)
{
    ...
    if (i4Argc == 1)
    {
        Printf("Usage: %s type[0-1] [loop(0=infinite)] [mode(0 full fush,1 half, 2 full pull)] [max_size=0x%x]"
    ...
    while (1)
    {
        ...
        if (!DMX_EMU_DDI_Transfer0(u1Type, _u1TsIndex, u4Size, eMode))
        {
            break;
        }
...
static INT32 _CmdPullTsTest0(INT32 i4Argc, const CHAR** aszArgv)
{
    ...
    while (1)
    {
        ...
        if (!DMX_EMU_PullTsTransfer0(_u1TsIndex, u4Size, u2Pid, eMode))
        {
            break;
        }
...
static INT32 _CmdPullTsTest3(INT32 i4Argc, const CHAR** aszArgv)
{
    ...
    if (i4Argc == 1)
    {
        Printf("Usage: %s [loop(0=infinite)][TsIndex][mode(0 full fush,1 half, 2 full pull)] [max_size=0x%x]"
            " [min_size=0x%x]\n", aszArgv[0], u4MaxSize, u4MinSize);
        return -1;
    }
    ...
    Printf("\nStart Pull TS test 0\n");
    Printf("TS index: %u, round: %u, max size: 0x%x, min size: 0x%x\n\n",
        u1TsIndex, u4MaxLoop, u4MaxSize, u4MinSize);
    ...
    while (1)
    {
        ...
        if (!DMX_EMU_PullTsTransfer3(u1TsIndex, u4Size, u2Pid,eMode))
        {
            break;
        }
...
static INT32 _CmdPullEsTest0(INT32 i4Argc, const CHAR** aszArgv)
{
    ...
    while (1)
    {
        ...
        if (!DMX_EMU_PullEsTransfer0(_u1TsIndex, u4Size, u2Pid,eMode))
        {
            break;
        }
...
static INT32 _CmdPullEsTest1(INT32 i4Argc, const CHAR** aszArgv)
{
    ...
    while (1)
    {
        ...
        if (!DMX_EMU_PullEsTransfer1(_u1TsIndex, u4Size, u2Pid,eMode))
        {
            break;
        }
...
#endif //CC_MT5880 mt5880 don't support multi-2
...
static INT32 _CmdAesTest0(INT32 i4Argc, const CHAR** aszArgv)
{
    ...
    while (1)
    {
        ...
        u4Size =  0x500c;//DMX_Align(random(u4MaxSize - u4MinSize) + u4MinSize - 188,188);

        u2Pid = 0x100; //random(0x2000)
...
static INT32 _DMXCLI_TsOutput(INT32 i4Argc, const CHAR ** aszArgv)
{
    ...
    if ((i4Argc < 3) || (i4Argc > 10))
    {
        ...
        Printf("    route: loopback | extloop | ext(ex CI) | int(int CI) -> ckgen\n");
        Printf("     sync: 8 | 1\n");
        Printf("    valid: 0-3\n");
    ...
    if (i4Argc >= 4)
    {
        ...
        else if (x_strncmp(aszArgv[3], "extloop", sizeof("extloop"))==0)
        {
...
static INT32 _CmdDMAFIFOTest(INT32 i4Argc, const CHAR** aszArgv)
{
    ...
    while (1)
    {
        ...
        if(fgDDI)
        {
            if (!DMX_EMU_DDI_ResetTransfer(_u1TsIndex, u4Size, fgIntr))
            {
                break;
            }
        }
...
extern BOOL _DMX_EMU_DBM_Auto_SwitchTest(BOOL fgEnable_AutoSwitch);
static INT32 _CmdDBM_Auto_SwitchTest(INT32 i4Argc, const CHAR** aszArgv)
{
    ...
    if(i4Argc != 3)
    {
        Printf("%s loop enable/disable\n", aszArgv[0]);
        return 0;
    }

    u4Loop = StrToInt(aszArgv[1]);
	
	if (x_strncmp(aszArgv[2], "enable", sizeof("enable"))==0)
        {
            fgEnable_AutoSwitch = TRUE;
        }
	else if (x_strncmp(aszArgv[2], "disable", sizeof("disable"))==0)
        {
    ...
    {
        Printf(" -----Test %d -----\n", i+1);
        if(_DMX_EMU_DBM_Auto_SwitchTest(fgEnable_AutoSwitch))
        {
            Printf("OK\n");
        }
        else
...
static INT32 _Cmd130byte_TsTest(INT32 i4Argc, const CHAR** aszArgv)
{
	...
	if ((u1Tsindex != 0x0) && (u1Tsindex != 0x3))
	{
		Printf("    Tsindex : 0 or 3.\n");
        return -1;
	}
 ...
 */
//-----------------------------------------------------------------------------
extern BOOL DMX_Emu_Tsout188_TransmitData( UINT8 u1Tsindex );
static INT32 _Cmd188byte_TsoutTest(INT32 i4Argc, const CHAR** aszArgv)
{
    UINT32 i, u4Loop;
	UINT8 u1Tsindex;
    ...
    {
        Printf(" -----Test %d -----\n", i+1);
        if(DMX_Emu_Tsout188_TransmitData(u1Tsindex))
        {
            Printf("OK\n");
        }
        else
...
extern BOOL DMX_Emu_Tsout130_TransmitData(BOOL fgExtSync);
static INT32 _Cmd130byte_TsoutTest(INT32 i4Argc, const CHAR** aszArgv)
{
    ...
    {
        Printf(" -----Test %d -----\n", i+1);
        if(DMX_Emu_Tsout130_TransmitData((u4Sync == 1)))
        {
            Printf("OK\n");
        }
        else
 ...
 */
//-----------------------------------------------------------------------------
extern BOOL _DMX_EMU_Multi_PacketSize_Test( BOOL fgFramer0Reset, BOOL fgFramer1Reset);
static INT32 _CmdMulti_PacketSizeTest(INT32 i4Argc, const CHAR** aszArgv)
{
	...
	if(i4Argc != 4)
	{
		Printf("%s loop fgFramer0Reset fgFramer1Reset\n", aszArgv[0]);
	...
	u4Loop = StrToInt(aszArgv[1]);
	u4Framer0Reset = StrToInt(aszArgv[2]);
	u4Framer1Reset = StrToInt(aszArgv[3]);
	...
	{
		Printf(" -----Test %d -----\n", i+1);
		if(_DMX_EMU_Multi_PacketSize_Test((u4Framer0Reset == 1),(u4Framer1Reset == 1)))
		{
			Printf("OK\n");
		}
		else
...
*/
    ...
    {
        "188byte_test", "188pvrtest", _Cmd188byte_TsoutTest   , NULL, "188byte_pvrplayback_test", CLI_SUPERVISOR
    },
    {
