...
EXTERN void vDrvMJCSetProgME(BOOL fgIs120Hz, UINT8 fgIs55p2);
void vDrvMJCMVEXPProc(void)
{
    ...
    else if (u1MVEXP == 0 && _u1PrevEn)
    {
        ...
        vDrvMJCSetProgME(arMJCPrm.arFRCConfig.u1OutFrmRate < 192, arMJCPrm.arFRCConfig.u1FrmSchMode == FALLBACK_MODE_55P2);
    }
...
static void _vDrvMJCIntpCtrl(void)
{
    ...
    vDrvMJCSetRdcType(u1MCRgnNum == 3);
...
UINT32 u4FilmModeStatus(void)
{
    return (MJC_READ_FLD(MJC_STA_FM_02, MJC_STA_ME_FILM_FLG_HW) || MJC_READ_FLD(MJC_SYS_04, MJC_PROG_FRM_EN));
}
...
void vDrvMJCBadEdit(void)
{
    ...
    if (IO32ReadFldAlign(MJC_BADEDIT_00, BADEDIT_EN) == SV_FALSE)
    {
        return;
    }
    
    fgCurrFilmMode = (u4FilmModeStatus() != MJC_FM_VDO) ? SV_TRUE : SV_FALSE;
    ...
    {
        MJC_WRITE_FLD_MULTI(MJC_FM_00,
            P_Fld(SV_ON, MJC_PSAUTO_32) |
            P_Fld(SV_ON, MJC_PSAUTO_22));
    }
}
...
void vDrvMJCSetQualityISR(void)
{
    ...
    if ((arMJCPrm.eMjcEffect != E_MJC_EFFECT_OFF) || (fgMCOnOff == 0))
    {
        ...
        if (u1DrvMJCGetOutLR() == 0)
        {
            u4CurFilmStatus = u4FilmModeStatus();
