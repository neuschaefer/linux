 ...
 */
INT32 rijndaelSetupEncrypt(UINT32 *rk, const UINT8 *key, int keybits)
{
  INT32 i = 0;
  UINT32 temp;
 ...
 * @return the number of rounds for the given cipher key size.
 */
int rijndaelSetupDecrypt(UINT32 *rk, const UINT8 *key, INT32 keybits)
{
  INT32 nrounds, i, j;
  UINT32 temp;
...
void rijndaelEncrypt(const UINT32 *rk, INT32 nrounds, const UINT8 plaintext[16],
  UINT8 ciphertext[16])
{
...
void rijndaelDecrypt(const UINT32 *rk, int nrounds, const UINT8 ciphertext[16],
  UINT8 plaintext[16])
{
  UINT32 s0, s1, s2, s3, t0, t1, t2, t3;
  #ifndef FULL_UNROLL
                    ...
                    UINT8 *output )
{
    ...
    if( mode == AES_ENCRYPT )
    {
        aes_crypt_ecb( ctx, mode, iv, output );
        for( i = 0; i < length; i++ )
            output[i] = (unsigned char)( input[i] ^ output[i] );
    }
    else
    {
        //ASSERT(0);
 ...
 * Checkup routine
 */
int aes_self_test( INT32 verbose )
{
    INT32 i, j, u, v;
    aes_context ctx;
    UINT8 buf[32];
