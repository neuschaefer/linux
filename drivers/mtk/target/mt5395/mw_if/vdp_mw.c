...
static void _VdpEosCallback(UINT32 u4VdpId, VDP_COND_T eCond, BOOL fgFastForward,UINT32 u4AttachedSrcId );
#else
...
LINT_SUPPRESS_BRACE(818)
    ...
    //    ucEsId = MW_MpvCompIdtoEsId(prMpvCompReg->ui2_id);        // prMpvCompReg->ui2_id to ucEsId
        ...
        else
        {
            ...
            else
            {
				...
				//single b2r , should not connect sd path
				ASSERT(0);
#endif
                if(ucVdpId == VDP_2)
                {
	...
	if(t_scart_pin_8 != AVC_SCART_PIN_8_UNKNOWN)
        {
...
extern UINT8 u1DrvTDTVMJCEnableQuery(void);
//-----------------------------------------------------------------------------
/** Brief of _VdpGet.
 */
//-----------------------------------------------------------------------------
        ...
        case VID_PLA_GET_TYPE_OVER_SCAN_CLIPPER:
        {
            ...
            else
            {
                *pz_get_info_len = sizeof(VID_PLA_OVER_SCAN_CLIPPER_T);
                ...
                if (VDP_GetOverScan(ucVdpId, &rOverScan) != VDP_SET_OK)
                {
                    i4ReturnValue = RMR_DRV_INV_GET_INFO;
                }
                else
        ...
        case VID_PLA_GET_TYPE_DRAMINF:
        {
            ...
            else
            {
                *pz_get_info_len = sizeof(CAP_INPUT_T);
                ...
                {
                    #ifdef LINUX_TURNKEY_SOLUTION
					peArg->puac_src_buffer = (VOID *)adapt_fbm_knl_to_usr(VIRTUAL(draminf.scaler_inf.startAddr));
                    #else
                ...
                {                	
								...
								#ifdef LINUX_TURNKEY_SOLUTION
								peArg->ui4_u_start_lsb_add = (VOID *)adapt_fbm_knl_to_usr(VIRTUAL(draminf.DI_inf.u4UStartLSBAddr));
								peArg->ui4_u_start_msb_add = (VOID *)adapt_fbm_knl_to_usr(VIRTUAL(draminf.DI_inf.u4UStartMSBAddr));
								
								peArg->ui4_y_start_lsb_add = (VOID *)adapt_fbm_knl_to_usr(VIRTUAL(draminf.DI_inf.u4YStartLSBAddr));
								peArg->ui4_y_start_msb_add = (VOID *)adapt_fbm_knl_to_usr(VIRTUAL(draminf.DI_inf.u4YStartMSBAddr));
								
								peArg->ui4_v_start_lsb_add = (VOID *)adapt_fbm_knl_to_usr(VIRTUAL(draminf.DI_inf.u4VStartLSBAddr));
								peArg->ui4_v_start_msb_add = (VOID *)adapt_fbm_knl_to_usr(VIRTUAL(draminf.DI_inf.u4VStartMSBAddr));
								#else
                ...
                {
								#ifdef LINUX_TURNKEY_SOLUTION
								peArg->ui4_u_start_msb_add = (VOID *)adapt_fbm_knl_to_usr(VIRTUAL(draminf.B2R_inf.u4CStart));
								peArg->ui4_y_start_msb_add = (VOID *)adapt_fbm_knl_to_usr(VIRTUAL(draminf.B2R_inf.u4YStart));
								#else
...
LINT_SUPPRESS_BRACE(507)
        ...
        break;
        case VID_PLA_SET_TYPE_STATIC_GAMMA:
        {
            vApiSetStaticGamma((UINT32)pv_set_info);
            break;
        }
        case VID_PLA_SET_TYPE_PEDESTAL:
        {
            UINT32 u4OnOff;
            u4OnOff = (UINT32)pv_set_info;
            vDrvSetPedOnOff((UINT8)u4OnOff);
            vDrvColorTransform((UINT8)(UINT32)ucVdpId);
            break;
        }
        case VID_PLA_SET_TYPE_HDMI_RANGE:
        {
            UINT32 u4Mode;
            u4Mode = (UINT32)pv_set_info;
            vSetHDMIRangeMode((UINT8)u4Mode);
            vDrvColorTransform((UINT8)(UINT32)ucVdpId);
            break;
        }
...
//            #endif
		...
		case VID_PLA_SET_TYPE_FORCE_UNMUTE:
			...
			break;
        case VID_PLA_SET_TYPE_ANGULAR_SCE:
            {
                if (pv_set_info == NULL)
                {
                    i4ReturnValue = RMR_DRV_INV_SET_INFO;
                ...
                {
                    VID_PLA_ANGULAR_SCE_T* prArg;
                    prArg = (VID_PLA_ANGULAR_SCE_T*) pv_set_info;

                    if (prArg->ui1_color == 0xFF)
                    {
                        vHalSCEAngularReset();
                    }
                    else
                    {                
                        vHalSetSCEAngular(prArg->ui1_ysh, prArg->ui1_color, prArg->ui1_value);
                    }
                }
                break;
            }        
...
#ifdef CC_53XX_SWDMX_V2
void _VdpEosCallback(UINT32 u4VdpId, VDP_COND_T eCond, BOOL fgFastForward,UINT32 u4AttachedSrcId)
{
