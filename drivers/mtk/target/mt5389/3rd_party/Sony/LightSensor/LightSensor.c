...
VOID CM3217_Init(void)
{
    ...
    switch (u4ModelIndex)
    {
        case DRV_CUSTOM_MODEL_OPTION_P:
            buff = 0x2E;
    	    uiRet = SIF_WriteNoSubAddr(0x100,0x20,&buff,1);
            if(uiRet)
            ...
            buff = 0x60;
            uiRet = SIF_WriteNoSubAddr(0x100,0x22,&buff,1);
            if(uiRet)
...
UINT8 Get_CM3217_Step(void)
{
    ...
    uiRet = SIF_ReadNoSubAddr(0x100,0x23,&CM3217_value_LSB,1);	
    if(uiRet==0)        return 0xff;
					
    //Read CM3217 MSB data	
    uiRet = SIF_ReadNoSubAddr(0x100,0x21,&CM3217_value_MSB,1);	
    if(uiRet==0)        return 0xff;
