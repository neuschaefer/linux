/*------------------------------------------------------------------------------

 Copyright 2009-2010 Sony Corporation

 This is UNPUBLISHED PROPRIETARY SOURCE CODE of Sony Corporation.
 No part of this file may be copied, modified, sold, and distributed in any
 form or by any means without prior explicit permission in writing from
 Sony Corporation.

 Date: 2010/06/18
 Revision: 1.0.1.0

------------------------------------------------------------------------------*/

#include "rb211_tuner_i2c.h"

#include "rb211_stdlib.h" /* for memcpy */

#define BURST_WRITE_MAX 128 /* Max length of burst write */

rb211_result_t rb211_tuner_i2c_CommonReadRegister(rb211_tuner_i2c_t* pI2c, UINT8 deviceAddress, UINT8 subAddress, UINT8* pData, UINT32 size)
{
	rb211_result_t result = RB211_RESULT_OK;

	RB211_TRACE_I2C_ENTER("rb211_tuner_i2c_CommonReadRegister");

	if(!pI2c){
		RB211_TRACE_I2C_RETURN(RB211_RESULT_ERROR_ARG);
	}

	result = pI2c->Write(pI2c, deviceAddress, &subAddress, 1, RB211_TUNER_I2C_START_EN);
	if(result == RB211_RESULT_OK){
		result = pI2c->Read(pI2c, deviceAddress, pData, size, RB211_TUNER_I2C_START_EN | RB211_TUNER_I2C_STOP_EN);
	}

	RB211_TRACE_I2C_RETURN(result);
}

rb211_result_t rb211_tuner_i2c_CommonWriteRegister(rb211_tuner_i2c_t* pI2c, UINT8 deviceAddress, UINT8 subAddress, const UINT8* pData, UINT32 size)
{
	rb211_result_t result = RB211_RESULT_OK;
	UINT8 buffer[BURST_WRITE_MAX + 1];

	RB211_TRACE_I2C_ENTER("rb211_tuner_i2c_CommonWriteRegister");

	if(!pI2c){
		RB211_TRACE_I2C_RETURN(RB211_RESULT_ERROR_ARG);
	}
	if(size > BURST_WRITE_MAX){
		/* Buffer is too small... */
		RB211_TRACE_I2C_RETURN(RB211_RESULT_ERROR_ARG);
	}
	
	buffer[0] = subAddress;
	rb211_memcpy(&(buffer[1]), pData, size);

	/* send the new buffer */
	result = pI2c->Write(pI2c, deviceAddress, buffer, size+1, RB211_TUNER_I2C_START_EN | RB211_TUNER_I2C_STOP_EN);
	RB211_TRACE_I2C_RETURN(result);
}

rb211_result_t rb211_tuner_i2c_CommonWriteOneRegister(rb211_tuner_i2c_t* pI2c, UINT8 deviceAddress, UINT8 subAddress, UINT8 data)
{
	rb211_result_t result = RB211_RESULT_OK;

	RB211_TRACE_I2C_ENTER("rb211_tuner_i2c_CommonWriteOneRegister");

	if(!pI2c){
		RB211_TRACE_I2C_RETURN(RB211_RESULT_ERROR_ARG);
	}
	result = pI2c->WriteRegister(pI2c, deviceAddress, subAddress, &data, 1);
	RB211_TRACE_I2C_RETURN(result);
}

/* For Read-Modify-Write */
rb211_result_t rb211_tuner_i2c_SetRegisterBits(rb211_tuner_i2c_t* pI2c, UINT8 deviceAddress, UINT8 subAddress, UINT8 data, UINT8 mask)
{
	rb211_result_t result = RB211_RESULT_OK;

	RB211_TRACE_I2C_ENTER("rb211_tuner_i2c_SetRegisterBits");

	if(!pI2c){
		RB211_TRACE_I2C_RETURN(RB211_RESULT_ERROR_ARG);
	}
	if(mask == 0x00){
		/* Nothing to do */
		RB211_TRACE_I2C_RETURN(RB211_RESULT_OK);
	}
	
	if(mask != 0xFF){
		UINT8 rdata = 0x00;
		result = pI2c->ReadRegister(pI2c, deviceAddress, subAddress, &rdata, 1);
		if(result != RB211_RESULT_OK){ RB211_TRACE_I2C_RETURN(result); }
		data = (UINT8)((data & mask) | (rdata & (mask ^ 0xFF)));
	}

	result = pI2c->WriteOneRegister(pI2c, deviceAddress, subAddress, data);
	RB211_TRACE_I2C_RETURN(result);
}

