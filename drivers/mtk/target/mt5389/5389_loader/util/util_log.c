...
static INT32 _UTIL_SetLog(INT32 i4ModLvl, CHAR *szMod, const CHAR *szFmt, VA_LIST t_ap, INT32 i4Level)
{
    ...
    if ((i4Level & _S) && (_u4SLogIndex < (MAX_SLOG_NUM-1))) {
        x_memset(_aszSLogBuf[_u4SLogIndex], 0, LOCAL_BUFSZ);
        if (i4ModLvl & _M) {
            LINT_SUPPRESS_NEXT_EXPRESSION(534);
            x_strncat(_aszSLogBuf[_u4SLogIndex], "[", 1);
            LINT_SUPPRESS_NEXT_EXPRESSION(534);
            x_strncat(_aszSLogBuf[_u4SLogIndex], szMod, 8);
            LINT_SUPPRESS_NEXT_EXPRESSION(534);
            x_strncat(_aszSLogBuf[_u4SLogIndex], "]", 1);
        }
        if (i4ModLvl & _T) {
            LINT_SUPPRESS_NEXT_EXPRESSION(534);
            x_strncat(_aszSLogBuf[_u4SLogIndex], szTId, 12);
        }
        if (i4ModLvl & _X) {
            LINT_SUPPRESS_NEXT_EXPRESSION(534);
            x_strncat(_aszSLogBuf[_u4SLogIndex], szTime, 24);
        }
        LINT_SUPPRESS_NEXT_EXPRESSION(534);
        x_strncat(_aszSLogBuf[_u4SLogIndex++], szStr, 80);
    }
#endif /* CC_MINI_DRIVER */
