...
#ifdef CC_MT5396
#include "drv_mjc.h"
EXTERN void vDrvMJCFrmSchFilmDetect(void);
EXTERN 	UINT8 fgDrvMJCFilmDetectDIOrMJC(void);
#endif
...
EXTERN void vDrvDISetFrcLine_Ext(UINT8 u1VdpId);
...
static void _vDrvDISetDramCtrl(UINT8 u1VdpId, E_MIB_DMODE eDataMode)
{
    ...
    LOG(1,"============_arMDDiPrm[u1VdpId].u1OnOff = %d,dram=%d\n",_arMDDiPrm[u1VdpId].u1OnOff,u4DRAM_RW_EN);
    ...
    LOG(1,"============u1FreezeWriteFilter= %d,dram=%d\n",_arMDDiPrm[u1VdpId].u1FreezeWriteFilter,u4DRAM_RW_EN);
...
static void _vDrvDIISR(UINT16 u2Vector)
{
    ...
    {        
        ...
        vDrvNRISR();
		#ifdef CC_MT5396
		if (fgDrvMJCFilmDetectDIOrMJC() == SV_TRUE)
		{
...
static inline void _vDrvDISetActiveWindow(UINT8 u1VdpId, UINT16 wSrcXOffset, UINT16 wSrcYOffset, UINT16 wSrcWidth, UINT16 wSrcHeight)
{
	...
	extern UINT16 DrvGetScalerForMIBHOffsetWA(UINT8 u1VdpId);
    ...
    if (IS_MIB_FRC(u1VdpId) && (wSrcXOffset + wSrcWidth > u4PreDownWidth) && (DrvGetScalerForMIBHOffsetWA(u1VdpId)==0))
...
static UINT8 _u1DrvDIModeChange(UINT8 u1VdpId)
{
            ...
            )
    {
        ...
        {
            _arMDDiPrm[u1VdpId].u1SRCWAutoStopDisableCnt= 1;  //For  H/V position change after DTV pause. Temporary solution.
            vDrvDISetAutoStop(u1VdpId, SV_OFF);
        }
