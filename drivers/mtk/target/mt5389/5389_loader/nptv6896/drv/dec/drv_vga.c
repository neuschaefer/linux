...
UINT8   bForceModeIndex = SYNCINVALID;
 ...
 */
UINT8 bVgaGetCurrSyncMode(void)
{
   return bModeIndex;
}
 ...
 * @return the current sync mode.
 */
UINT8 bVgaForceSyncMode(UINT8 bForceSyncMode)
{

   if(bForceSyncMode < SYNCINVALID)
   {
       bForceModeIndex = bForceSyncMode;
       switch(bForceModeIndex)
       {
           case SYNCONGREEN:
               vASSetSOGSync();
           break;
       ...
       bModeIndex = bForceModeIndex;
   }
   else
   {
       bForceModeIndex = SYNCINVALID;
   }
...
//reset clamp win and blank start
    if (fgIsMainVga()||fgIsPipVga()) //only for VGA
    {
        ...
        else
        {
            ...
            if(wWindow<50)
            {
                vHdtvClampWin(0, 20);      // START=0, 	END= 12
                vHdtvBlankStart(26);       //26~42 
            }
            else if (wWindow<60)
            {
    ...
    {	
    	...
    	if(fgIsVdoIntSp0Mute()&& _IsVgaDetectDone)
    	{
			...
			if(fgIsMainVga())
			{
				...
				vRegWriteFldAlign(HDTV_01,0,HDTV_AV_WIDTH);
			}	
