...
{
#if !defined(CC_MTK_LOADER) || !defined(CC_LOADER_SHRINK_EDID) //loader shrink
...
void vDrvLoadHdmiEdidTable(void)
{
    UINT32 u4MemLen = 256;
    UINT32 i = 0;
    UINT64 u8CurOff;

    UINT32 u4EdidPartition = 3;
    UINT32 u4EdidOffset = 768;
#if 1
    INT32 ret = 0;
    
    ret = DRVCUST_InitQuery(eHDMIEDIDNorPartId, &u4EdidPartition);
    if(0 != ret)
    {
        Printf("edid error 1\n");
        return ;
    ...
    ret = DRVCUST_InitQuery(eHDMIEDIDNorOffset, &u4EdidOffset);
    if(0 != ret)
    {
        Printf("edid error 2\n");
        return ;
...
#endif    
    for(i = 0; i<10; i++)
    Printf("edid[i] = 0x%x, i = %d, u4MemLen = %d++++------\n", HDMIEDID_table[i], i, u4MemLen);  
    
    u8CurOff = (UINT64)(u4EdidOffset);
    u8CurOff |= (((UINT64)(u4EdidPartition)) << 32);
    Printf("Read partB current flag: 0X%x,0X%x,0X%x_________________________\n", u8CurOff, u4EdidPartition, u4EdidOffset);
    NORPART_Read(u8CurOff, (UINT32)(&HDMIEDID_table[0]), u4MemLen);
...
void vHDMI_INTERNAL_EDID_Init(void)
{
	...
	*/
    ...
    vIO32Write1B(PDWNC_EDID_ADDR,u1PAOffset);
    vIO32Write4B(PDWNC_EDID_DATA,0x20);
    vIO32Write1B(PDWNC_EDID_ADDR,255);
    vIO32Write4B(PDWNC_EDID_DATA,u1Buf);
#endif
    
	if (u1Checksum >= 0x10)
	{
	    u1Buf = u1Checksum - 0x10;
	}
	else
	{
	    u1Buf = u1Checksum + (0x100 - 0x10);
	}

    Printf("HDMI1 PA 0x%x\n",0x10);	
    Printf("HDMI1 checksum 0x%x\n",u1Buf);

	vIO32WriteFldAlign(PDWNC_EDID_CTL1, 0x10, FLD_PHY_ADDR1);
