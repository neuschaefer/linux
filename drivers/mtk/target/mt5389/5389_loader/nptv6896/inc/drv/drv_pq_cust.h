...
#define SCE_INDEX 960   // 22*28+8=624, plus 8 for FlashPQ size alignment (16x)
...
//CUST_QTY_TBL    
    extern UINT8 bCustomQtyTbl[FLASHPQ_NUM][17][CUSTOM_QTY_TBL_DIM];
    
    #define READ_CUSTOM_QTY_TBL(timing,item) bCustomQtyTbl[item>>QTY_TBL_REF_INDEX_SHIFT][(timing)][item &0x3FFF]
    
    #define WRITE_CUSTOM_QTY_TBL(timing,item,value) {bCustomQtyTbl[item>>QTY_TBL_REF_INDEX_SHIFT][(timing)][(item &0x3FFF]=(value);}

	
//CUST_QTY_TBL 2ND PART Caihua Lei mtk70646
	//extern UINT8 bCustom2ndPartQtyTbl[][CUSTOM_QTY_TBL_DIM];
	//extern UINT16 const CODE wCustom2ndPartQtyItem[CUSTOM_QTY_TBL_DIM + TBL_HEAD_HASH];
	#define READ_CUST_2ND_PART_QTY_ITEM(index) 1
	//wCustom2ndPartQtyItem[(index)]
	//#define READ_CUSTOM_2ND_PART_QTY_TBL(timing,item) bCustom2ndPartQtyTbl[(timing)][(item)]
	#define WRITE_2ND_PART_DEFUALT_QTY_TBL_REF(index,value) {aDefaultQtyTbl[(index)].wDftQtyRefenence=(value);}
