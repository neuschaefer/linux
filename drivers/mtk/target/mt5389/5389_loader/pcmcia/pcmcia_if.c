...
#include "tuner_if.h"
#include "dmx_if.h"
#if defined(PCMCIA_ENABLED) || defined(CC_EXTERNAL_IC_MT5135)
TS_FMT_T _rTsFmt;
#endif
...
//-----------------------------------------------------------------------------
static void _PCMCIA_AutoSetTsRoutingPath_5389(void)
{
    ...
    UNUSED(_PCMCIA_AutoSetTsRoutingPath_5389);
    ...
    //Get demod type from ts format
    if (_rTsFmt.fgExternalDemod == BOOL_TRUE)
    {
        fgExternalDemod = TRUE;
    }
    else
    {
        fgExternalDemod = FALSE;
    }

    LOG(1, "PCMCIA_AutoRouteTs: CamConnected=%d, External Demod=%d, TSGoThroughCam=%d\n", fgCamConnected, fgExternalDemod, fgTSGoThroughCam);

    if ( fgIsInternalCi )   // Internal CI
    {
        LOG(0, "    Is Internal CI ???\n");
        return;
    }
    if (fgExternalDemod)    // External CI
    {
        // External Demod (DTMB,DVBS, DVBT, DVBC)
        if (eExternalIcVersion == MT8295_VERSION_BNE) 
        {
            if (_rTsFmt.SerOrPar == SP_SERIAL)
                {
                    // P1. Demod : internal/external, parallel/serial, tri-state ---
                    LOG(1, "    TunerSetTsSP(SP_SERIAL)\n");
                    TunerSetTsSP(SP_SERIAL);
                // P2. PCMCIA : parallel/serial, bypass ----------------------------
                LOG(1, "    Set TS Type : Serial\n");
                PCMCIA_ParallelTsOnOff(FALSE);
                    LOG(1, "    TunerSetTsTristate(BOOL_FALSE)\n");
                    TunerSetTsTristate(BOOL_FALSE);
        
                if ( fgCamConnected && fgTSGoThroughCam )
                {
                    LOG(1, "    Set PCMCIA Bypass : FALSE\n");
                    PCMCIA_SetBypass(FALSE);
                }
                else
                {
                    LOG(1, "    Set PCMCIA Bypass : TRUE\n");
                    PCMCIA_SetBypass(TRUE);
                }
                LOG(1, "    Set d.sfe 0x%x 0\n", DMX_FE_EXT_S);
                DMX_SetFrontEndEx(0, DMX_FE_EXT_S);
                }
            else // dvbt2, parallel ts input to mt8295
            {
                LOG(1, "    TunerSetTsSP(SP_PARALLEL)\n");
                TunerSetTsSP(SP_PARALLEL);

            // P2. PCMCIA : parallel/serial, bypass ----------------------------
            LOG(1, "    Set TS Type : Parallel\n");
        ...
        {
            LOG(1, "    TunerSetTsOutput(BOOL_FALSE)\n");
            TunerSetTsOutput(BOOL_FALSE);
            PCMCIA_SetTsRouting(fgCamConnected && fgTSGoThroughCam, &_rTsFmt);
            LOG(1, "    TunerSetTsOutput(BOOL_TRUE)\n");
            TunerSetTsOutput(BOOL_TRUE);
        }
    }
    else
    {
        // Internal Demod (DTMB,DVBS, DVBT, DVBC)
        if (eExternalIcVersion == MT8295_VERSION_BNE) 
        {
            if ( fgCamConnected && fgTSGoThroughCam )
            {
                ...
                LOG(1, "    TunerSetTsTristate(BOOL_FALSE)\n");
                TunerSetTsTristate(BOOL_FALSE);
                CKGEN_WRITE32(0x40c, (CKGEN_READ32(0x40c) & 0xffffffe3) | 0x4); 
                LOG(1, "    Set PCMCIA Bypass : FALSE\n");
                PCMCIA_SetBypass(FALSE);
                LOG(1, "    Set d.sfe 0x%x 0\n", DMX_FE_EXT2_0_S);
...
static void _PCMCIA_AutoSetTsRoutingPath(void)
{
    ...
    _PCMCIA_AutoSetTsRoutingPath_5389();
...
void PCMCIA_UpdateTsStatus2(TS_FMT_T* prTsFmt)
{
    LOG(1, "PCMCIA_UpdateTsStatus2\n");
    //PCMCIA_Init();
    //TunerGetTsFmt(&_rTsFmt);
    _rTsFmt.SerOrPar = prTsFmt->SerOrPar;
    _rTsFmt.ClockPolarity = prTsFmt->ClockPolarity;
    _rTsFmt.fgTristate = prTsFmt->fgTristate;
    _rTsFmt.u4TsClockInKHz = prTsFmt->u4TsClockInKHz;
    _rTsFmt.fgGatedClock = prTsFmt->fgGatedClock;
    _rTsFmt.fgExternalDemod = prTsFmt->fgExternalDemod;
    
    LOG(1, "TsFmt:SerOrPar=%d,u4TsClockInKHz=%d,ClockPolarity=%d,fgExternalDemod=%d\n", 
        _rTsFmt.SerOrPar, _rTsFmt.u4TsClockInKHz, _rTsFmt.ClockPolarity, _rTsFmt.fgExternalDemod);
...
INT32 PCMCIA_Read(POD_IO_STATE_T eIoState, UINT8* pu1Data, UINT16* pu2DataLen)
{
    ...
    //i4Result = PODCI_ReadCardReg(eIoState, 0, pu1Data, *pu2DataLen);
    //ASSERT(0);    
    LOG(3, "PCMCIA_Read : %d\n", i4Result);
    return i4Result;
}
...
INT32 PCMCIA_ReadCor(UINT16 u2CorAddress)
{
    ...
    LOG(3, "PCMCIA_ReadCor(%d) begin\n", u2CorAddress);
    i4Result = PODCI_ReadCor(u2CorAddress);
    LOG(3, "PCMCIA_ReadCor : %d\n", i4Result);
    return i4Result;
}
...
INT32 PCMCIA_PcmciaEnableTS(BOOL fgPcmciaEnableTS)
{
    ...
    PCMCIA_AutoSetTsRoutingPath();
//    Printf("PCMCIA_PcmciaEnableTS call PCMCIA_AutoSetTsRoutingPath_______________________________________\n");
...
#endif
void PCMCIA_UpdateTsStatus(TS_FMT_T TsFmt)
{
    LOG(1, "PCMCIA_UpdateTsStatus\n");
    //PCMCIA_Init();
    //TunerGetTsFmt(&_rTsFmt);
    ...
    _rTsFmt.SerOrPar = TsFmt.SerOrPar;
    _rTsFmt.ClockPolarity = TsFmt.ClockPolarity;
    _rTsFmt.fgTristate = TsFmt.fgTristate;
    _rTsFmt.u4TsClockInKHz = TsFmt.u4TsClockInKHz;
    _rTsFmt.fgGatedClock = TsFmt.fgGatedClock;
    _rTsFmt.fgExternalDemod = TsFmt.fgExternalDemod;
...
{
    return BIM_GetDeviceData(pu1DeviceID, u1Size);
}

INT32 PCMCIA_BIM_GetDeviceID(BYTE *pu1DeviceID, BYTE u1Size)
{
    return BIM_GetDeviceID(pu1DeviceID, u1Size);
}

INT32 PCMCIA_BIM_GetSecureID(BYTE *pu1SecureID, BYTE u1Size)
{
    return BIM_GetSecureID(pu1SecureID, u1Size);
}
...
#endif //CC_MTK_LOADER
#else
PCMCIA_EXTERNAL_IC_VERSION eExternalIcVersion = MT5135_VERSION_AE;
#ifndef CC_MTK_LOADER
...
void PCMCIA_UpdateTsStatus(TS_FMT_T TsFmt)
{
	...
	#if 1
	UINT8  u1RegValue;
		UINT32 u4PositiveEdge = 1, u4TsFreq,  u4Clock;
		...
		// mt5135 demod
		 // SP_PARALLEL -- DVBS ... 3rd party demod
		{
