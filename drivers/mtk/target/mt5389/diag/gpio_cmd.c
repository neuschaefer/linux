...
static INT32 _GPIOCLI_T8032_Enable(INT32 i4Argc, const CHAR** aszArgv)
{
    ...
    //BSP_PinGpioSel(i4Gpio);//mark it for 5368, because pinmux not ready 
    GPIO_Reg(i4Gpio, GPIO_TYPE_NONE, NULL);
    PDWNC_T8032GpioReg(i4Gpio, GPIO_TYPE_NONE, NULL);
    Printf("GPIO %d - ", i4Gpio);
    if (i4Intr) {
        PDWNC_T8032GpioReg(i4Gpio, GPIO_TYPE_INTR_BOTH, _GPIO_Callback);
        Printf(" INTR ");
    }
    if (i4Fall) {
        PDWNC_T8032GpioReg(i4Gpio, GPIO_TYPE_INTR_FALL, _GPIO_Callback);
        Printf(" INTRFALL ");
    }
    if (i4Rise) {
        PDWNC_T8032GpioReg(i4Gpio, GPIO_TYPE_INTR_RISE, _GPIO_Callback);
        Printf(" INTRRISE ");
    }
    if (i4Low) {
        PDWNC_T8032GpioReg(i4Gpio, GPIO_TYPE_INTR_LEVEL_LOW, _GPIO_Callback);
        Printf(" INTRLOW");
    }
    if (i4High) {
        PDWNC_T8032GpioReg(i4Gpio, GPIO_TYPE_INTR_LEVEL_HIGH, _GPIO_Callback);
        Printf(" INTRHIGH ");
    }
...
static INT32 _GPIOCLI_TestWatchdog(INT32 i4Argc, const CHAR ** aszArgv)
{
    ...
    UINT32 u4Priority=0, u4Seconds=0;
    INT32 i4Ret;
...
static CLI_EXEC_T arGpioCmdTbl[] =
{
    ...
    DECLARE_CMD(_GPIOCLI_T8032_Enable, upenable, upen, "gpio.upen [-i/-r/-f/-L/-H][gpio num]"),    
