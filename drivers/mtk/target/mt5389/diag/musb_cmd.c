...
#if 0
extern int TestQMUBulkMultiGPD(int i4Argc, const char **szArgv);
extern int TestQMUNGPD(int i4Argc, const char **szArgv);
extern int TestMultiQueue(int i4Argc, const char **szArgv);
extern int TestMultiEndpoint(int i4Argc, const char **szArgv);
extern int TestQMUTxStop(int i4Argc, const char **szArgv);
extern int TestQMU2TxStop(int i4Argc, const char **szArgv);
extern int TestQMURxStop(int i4Argc, const char **szArgv);
extern int TestMultiIxiaLoopbackQMU(int i4Argc, const char **szArgv);
extern int TestQMUIsoLoopback(int i4Argc, const char **szArgv);
extern int TestRiscDramFifoAccess(int i4Argc, const char **szArgv);
#endif
//extern unsigned int _u4UsbQDebugLevel;
#endif
...
static INT32 _MUSBHostMsdCmd(INT32 i4Argc, const CHAR **szArgv)
{
    ...
    if (i4Argc < 4)
    {
        MGC_Printf("Arg: [LUN] [StartSector] [NumSector] [Option: RW=0/RO=1/KILL=2].\n");
        return 0;
    }
    ...
    MGC_Printf("LUN= 0x%X.\n", lun);
    MGC_Printf("sector_start_offset = 0x%X.\n", sector_start_offset);
    MGC_Printf("sector_num = 0x%X.\n", sector_num);
    ...
    {
        MGC_Printf("Write and read test.\n");
            stress = 0;
    }
...
MUSB_InitTimeMonitor();
    ...
    //                do_div(u8ReadRate, MUSB_GetTimeMonitor());
        ...
        {
            UINT32 u4Counter = 0;
        MGC_Printf("Read Only Stress Test Start ... \n");
        MGC_Printf("    Write one time, read/compare forever.\n");
        MGC_Printf("    There will be one line log for every 100000 tries.\n");
            ...
            while(1)
            {
                ...
                if ( u4Counter == 100000 )
                {
                MGC_Printf("loop %d times.", u4Counter);
                    u4Counter = 0;
                }
        ...
        {        
                ...
                if (stress)
                {
            MGC_Printf("[%d] offset = 0x%X, test_size = 0x%X.\n", lun, sector_offset, test_size);
                }
            ...
            if (test_size > 65536)
            {
                if (!stress)
                {
                MGC_Printf("Write/Read data check ok !\n");
                    break;                
                }
                test_size = block_size;
            }
...
static INT32 _USBDevISOGetCmd(INT32 i4Argc, const CHAR **szArgv)
{       
        UINT32 ep = 2;
        UINT32 pktsize = 0;
        UINT8* buf;
...
//        UINT32 i;
        ...
        if (!_pvMusbMemPool)
        {
            _pvMusbMemPool = x_mem_alloc(0x20000);
            ASSERT(_pvMusbMemPool);
        }
      ...
      //  for (i=0; i<loop; i++)
        {
            pktsize = MGC_Dev_GetISOPkt(ep, (UINT8 *)buf, loop);
...
extern uint8_t bADataEnable;

static INT32 _USBDevISOSendCmd(INT32 i4Argc, const CHAR **szArgv)
    {       
        UINT32 ep = 2;
        UINT32 pktsize = 0;
        UINT8* buf;
    ...
    //    UINT32 i;
        ...
        if (i4Argc < 4)
        {
            MGC_Printf("Arg: [loop] [send data length] [1: enable / 0: disable] .\n");
            return 0;
        }
        
        loop = StrToInt(szArgv[1]);
        pktsize = StrToInt(szArgv[2]);
        bADataEnable = StrToInt(szArgv[3]);
        // test data is balance in multi-packet.
        balance = 1;
        MGC_Printf("balance = %d test start...\n", balance);
      ...
      //  for (i=0; i<loop; i++)
        {
            ...
            MGC_Dev_SendISOPkt(ep, pktsize, (UINT8 *)buf, loop, balance);
...
#ifdef MUSB_QMU
static INT32 _MUSBQmuHostISOTest(INT32 i4Argc, const CHAR **szArgv)
{       
    UINT32 pktsize = 3072;
    UINT8 * src;
    UINT32 framenum = 1;
    ...
    for (i = 1; i <= pktsize; i ++)
    {
        CLI_Parser_Arg("musb.qmus %d 2 1", i);
        // In fpga, cpu clock is slower, we need to wait device is ready to send.
        //x_thread_delay(10);
        CLI_Parser_Arg("musb.qmus %d 3 1", i);
    }           
...
{       
    ...
    if (i4Argc < 3)
    {
        ...
        //MGC_Printf("[TX/RX] not 0: RX; 1: TX\n");
        MGC_Printf("[framenum]\n");
        return 0;
    }
...
#ifdef MUSB_QMU
#if 0
static INT32 _MUSBTestQDebugLevel(INT32 i4Argc, const CHAR **szArgv)
{
    if (i4Argc < 2)
    {
        MGC_Printf("Arg: [Debug Level]\n");
        return 0;
    }

    _u4UsbQDebugLevel = StrToInt(szArgv[1]);

    UTIL_Printf("Set USBQ Debug Level:%d\n", _u4UsbQDebugLevel);
    return 0;
}
#endif
...
#if 0
static INT32 _MUSBTestQMUBulkMultiGPD(INT32 i4Argc, const CHAR **szArgv)
{
    return TestQMUBulkMultiGPD(i4Argc, szArgv);
}

static INT32 _MUSBTestQMUNGPD(INT32 i4Argc, const CHAR **szArgv)
{
    return TestQMUNGPD(i4Argc, szArgv);
}

static INT32 _MUSBTestMultiQueue(INT32 i4Argc, const CHAR **szArgv)
{
    return TestMultiQueue(i4Argc, szArgv);
}

static INT32 _MUSBTestMultiEndpoint(INT32 i4Argc, const CHAR **szArgv)
{
    return TestMultiEndpoint(i4Argc, szArgv);
}

static INT32 _MUSBTestQMUTxStop(INT32 i4Argc, const CHAR **szArgv)
{
    return TestQMU2TxStop(i4Argc, szArgv);
    //return TestQMUTxStop(i4Argc, szArgv);
}

static INT32 _MUSBTestQMURxStop(INT32 i4Argc, const CHAR **szArgv)
{
    return TestQMURxStop(i4Argc, szArgv);
}

static INT32 _MUSBTestMultiIxiaLoopbackQMU(INT32 i4Argc, const CHAR **szArgv)
{
    return TestMultiIxiaLoopbackQMU(i4Argc, szArgv);
}

static INT32 _MUSBTestQMUIsoLoopback(INT32 i4Argc, const CHAR **szArgv)
{
    //printk("Remove TestQMUIsoLoopback temporality\n");
    return TestQMUIsoLoopback(i4Argc, szArgv);
}

static INT32 _MUSBTestRiscDramFifoAccess(INT32 i4Argc, const CHAR **szArgv)
{
    return TestRiscDramFifoAccess(i4Argc, szArgv);
}

#if 0
static INT32 _MUSBTestQMUDevIsoLoopback(INT32 i4Argc, const CHAR **szArgv)
{
    return TestQMUDevIsoLoopback(i4Argc, szArgv);
}
#endif
#endif
#endif
...
/*Isochroous mode test cli*/
    ...
    { "dis", "dis", _USBDevISOSendCmd, NULL, "USB Device isochronous OUT packet test", CLI_GUEST},
    { "dig", "dig", _USBDevISOGetCmd, NULL, "USB device isochronous IN packet test", CLI_GUEST},
#ifdef MUSB_QMU
    {"qmuh", "qmuh", _MUSBQmuHostISOTest, NULL, "USB QMU host iso OUT packet", CLI_GUEST}, 
