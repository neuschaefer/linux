...
/* the following should be enough for float? */
#define PRINT_FLOAT_BUF_LEN 64

// pow and powf doesn't work...
static UINT64 ten_power(UINT32 y)
{
    UINT64 x = 1;
    while (y > 0)
    {
...
INT32 print_float(CHAR **out, DOUBLE f, INT32 b, INT32 sg, INT32 width, INT32 frac_width, INT32 pad, INT32 letbase)
{
    ...
    if (sg && b == 10 && f < 0) {
        neg = 1;
        u = -(UINT64)f;
        frac = -((f + u) * ten_power(frac_width+1));
    }
    else
    {
        u = (UINT64)f;
    ...
    s = print_buf + PRINT_FLOAT_BUF_LEN-1;
    *s = '\0';
...
{
    register INT32 width, pad, frac_width;
    register INT32 pc = 0;
    ...
    for (; *format != 0; ++format) {
        if (*format == '%') {
            ...
            if( *format == 'f' ) {
                varg = (INT32 *)(((UINT32)varg + 0x7) & ~0x7);
                DOUBLE d = *(DOUBLE*)varg;
                varg = (INT32 *)((UINT32)varg + 0x8);   // Org: ((DOUBLE*)varg)++;
                //DOUBLE d = (*(DOUBLE*)varg)++;
                pc += print_float (out, d, 10, 1, width, frac_width, pad, 'a');
                continue;
            }
        }
        else {
