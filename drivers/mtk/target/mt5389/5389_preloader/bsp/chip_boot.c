...
void CHIP_ClockSetting()
{
    ...
    vIO32WriteFldMulti(PDWNC_CONCFG0,P_Fld(1,FLD_RG_CON_XTAL2PLLGP_EN)|P_Fld(1,FLD_RG_XTAL_AR_EN)|P_Fld(6,FLD_RG_CON_RESERVE));

    // for at-speed romcode, set CPU/BUS/SRAM to xtal before PLL setting
    ...
    // 0x1B=756mhz
    // 0x1A=729mhz
    vIO32WriteFldAlign(REG_PLL_GROUP_CFG1, 0x1A, FLD_RG_MJCPLL_FBDIV);
    //vIO32WriteFldAlign(REG_PLL_GROUP_CFG1, 0x18, FLD_RG_MJCPLL_FBDIV);    
    vIO32WriteFldAlign(REG_PLL_GROUP_CFG2, 0x02, FLD_RG_MJCPLL_DIVEN);

    // Set VDECPLL for 378MHz
...
#else
    // Load PLL 2592MHz default
    vIO32Write4B(REG_IFPLL_CFG0, 0x97342100);
    vIO32Write4B(REG_IFPLL_CFG1, 0x4C180700);
    vIO32Write4B(REG_IFPLL_CFG2, 0xF0000000);    
#endif

    // Enable IFPLL watch dog
    vIO32WriteFldAlign(REG_IFPLL_CFG2, 0x02, FLD_RG_ULJPLL_RESERVE);
    ...
    vIO32WriteFldAlign(REG_IFPLL_CFG1, 0x1, FLD_RG_ULJPLL_DIV3_RSTB);   // ULJPLL_DIV3_RSTB=0b

    // 3. init USB PLL
    // already do this in step0
    CHIP_Delay_us(500);
    vIO32Write4B(0xf000d25c,0x00010001);
    vIO32Write4B(0xf000d320,0x00000001);
   ...
   // vIO32Write4B(0xf0059a00,(u4IO32Read4B(0xf0059a00)|0x00004000)); //RG_USB20_INTR_EN=1b
    ...
    // cpu clock = cpupll (756Mhz), bus clock = 216Mhz
    /*
    Clock source selection[3:0]
        ...
        2: enetpll_ck (675MHZ)
        3: syspll_ck (648MHz)
        4: usbpll_d1_ck (480MHz)
        5: sawlesspll_d1_ck (432MHz)
        6: cpupll_d2_ck (378MHz)
        7: usbpll_d2_ck (240MHz )
        8: sawlesspll_d2_ck (216MHz )
        9: sawlesspll_d3_ck (144MHz )
       10: usbpll_d4_ck (120MHz )
       11: sawlesspll_d4_ck (108MHz)
       12: sawlesspll_d8_ck (54MHz)
       13: MEM_CK
       14: slow_ck
       15: bist_ck
    BUS_CK selection[10:8]
        0: bus_ck = xtal_ck
        1: bus_ck = syspll_d3_ck
        2: bus_ck = usbpll_d2p5_ck
        3: bus_ck = syspll_d4_ck
        4: bus_ck = sawlesspll_d3_ck
        5: bus_ck = usbpll_d4_ck
        6: bus_ck = swalesspll_d6_ck
        7: bus_ck = swalesspll_d2_ck
    */
    vIO32WriteFldMulti(CKGEN_CPU_CKCFG, P_Fld(7,FLD_BUS_CK_SEL)|P_Fld(1,FLD_CPU_CK_SEL));
	...
	IO_WRITE32(0, 0xF00087B0, 6);
	IO_WRITE32(0, 0xF00087B4, 0x41);

    // 9. setup sram clock
    /*
    abist_sram_ck select
    00: xtal_ck
    01: usbpll_d2_ck
    10: sawlesspll_d2_ck
    11: usbpll_d3_ck
    */
    ...
    15: bist_ck
    */
    vIO32WriteFldAlign(CKGEN_NFI_CKCFG, 0x4, FLD_NFI_CK_SEL);      
...
#ifdef CC_MT5389
	vIO32WriteFldAlign(CKGEN_PADPD, 0, FLD_PADPD_16);//workaround for 5389 SCL_SYSTEM(oscl2) default pull low issue
	//set XTAL's gm up to 0x3 , with setting 0xf000d330[1:0]=2b'01. workaround for reduce demod spur 
	vIO32WriteFldAlign(PDWNC_CONCFG0, 0x3, FLD_RG_XTAL_GM);
#endif

#ifdef NEED_TO_FIX
#ifdef CC_ENABLE_HDMI_JTAG
    // switch JTAG to gpio
    IO_WRITE32MSK(CKGEN_PMUX1, 0, 0x003ffc00, 0x003ffc00);
    // switch HDMI to JTAG
    IO_WRITE32MSK(PDWNC_PINMUX1, 0, 0x00040000, 0x03cfc000);
#endif
#endif
}
