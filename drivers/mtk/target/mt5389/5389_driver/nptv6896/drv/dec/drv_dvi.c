...
void vDVISetModeDone(void)
{
    ...
    LOG(6, " The HDMI ColorSpace is %d \n", _bHDMIColorSpace);	
    _bNodifyConnect = 0;
    if (fgIsMainDVI())
    {
...
UINT8 bHDMI3DIPCheck(void)
{
    ...
    if ((_bDviTiming == MODE_3D_720p_50_FP)||(_bDviTiming == MODE_3D_720p_60_FP)||(_bDviTiming == MODE_3D_1080p_24_FP)||
    	...
    	(_bDviTiming == MODE_3D_2880_240p_60_FP))
    {
    ...
    if((_bDviTiming == MODE_3D_1080I_60_FP)||(_bDviTiming == MODE_3D_480I_60_FP)||(_bDviTiming == MODE_3D_576I_60_FP)||
	   ...
	   (_bDviTiming == MODE_3D_2880_480I_60_FP))
    {
        return FP_interlace;
    }
	if((_bDviTiming == MODE_3D_1080p_30_SBS_FULL)||(_bDviTiming == MODE_3D_1080p_24_SBS_FULL)||(_bDviTiming == MODE_3D_720p_24_SBS_FULL)||
	   ...
	   (_bDviTiming == MODE_3D_480p_60_SBS_FULL)||(_bDviTiming == MODE_3D_576p_50_SBS_FULL)||(_bDviTiming == MODE_3D_640_480p_60_SBS_FULL))
	{
...
UINT8 bHDMI3DPacketVaild(void)
{
    ...
    else
    {
		...
		{
			_fgFake3D = TRUE;
			//LOG(3, "------ 3D timing, but have NO 3D InfoFrame ------\n");
			return 1;
		}
		else
		{
...
UINT32 wVprochwidth;
    ...
    if(1==bDviBypassAp3DRule())
    {
      	...
      	else if(FP_interlace==bHDMI3DIPCheck()) //interlace frame packing timing
       {
            ...
            {
                vRegWriteFldAlign(VFE_EF, wVprochwidth, CCH_3D_VACT_SP2);
            } else {
    ...
    switch (_bDviChkState)
    {
		...
		||(aud_ch_info.eSampeFreq!=(HDMI_AUD_FS)bHDMIGetAudSampleFreq()))
		{
            ...
            else
            {
               ...
               if (((_bDviMdChgCnt > _bDviMdChgCntTHR)  || (_bDviDeChgCnt > _bDviDeChgCntTHR) || (_bDviPixClkChgCnt > _bDviPixClkChgCntTHR) || (_bCCIRNeedModeChgCnt > _bCCIRNeedModeChgCntTHR)) || (_bHdmiMD != u1IsHdmiMode() || u1IsSyncLose(eActiveHdmiPort))
                   #if SUPPORT_SBS_CONVERT
        ...
        case DVI_WAIT_STABLE:
                    ...
                    ((wvtemp >= (_wDviTmpVtotal - _wDviTmpTHR)) && (wvtemp <= (_wDviTmpVtotal + _wDviTmpTHR))))
            {
				...
				if ((_bVStableCnt > VRES_STB_COUNTS && _bHStableCnt > HRES_STB_COUNTS ) || \
					(u1IsHdmiFastSw(eActiveHdmiPort) && _bVStableCnt > 10))
				{
				#if WIDTH_HEIGHT_FROM_HDMIDEC
				   ...
				   //  Printf("++ pre search V:%d H:%d clk: %d, timing %d\n", _wDviVTotal, _wDviHtotal, _wHDMIPixelClk,bDviStdTimingSearch(1));
				    if (bDviStdTimingSearch(1) == MODE_NOSUPPORT)
				    {
				        ...
				        {
				            if (_bHStableCnt > 120 && _bVStableCnt > 120)
				            {
              			         _bHVStableFlag = 1;
				            }
				        }
...
#ifdef CC_SUPPORT_HDMI_DYNAMIC_WAIT_STABLE				
			LOG(6,"+++ mdchg: %d _bAVIStableCnt %d, WSTBVNT %d, Vstbcnt: %d, VSTBCNT %d, Hstbcnt: %d, HSTBCNT %d\n", \
