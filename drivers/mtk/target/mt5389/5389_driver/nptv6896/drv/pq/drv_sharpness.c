...
#include "drv_pq_cust.h"    //Work around for sharpness
...
UINT8 TDS_YLEV_VALUE[17] =      //Work around for sharpness
{
    0x40,0x4D,0x5A,0x66,0x73,0x76,0x7A,0x7D,
    0x80,0x80,0x80,0x80,0x80,0x7D,0x7A,0x76,
    0x72  
...
static RRegDwrdTbl const REGTBL_TDSHARP[] =
{
	...
	{        TDSHP_0C, 0x00000000, 0xFFF00FFF},
...
{
#if defined(CC_MT5396) || defined(CC_MT5368)
	vRegWriteFldAlign(ECTIF_00, wReadQualityTable(QUALITY_CTIF_ENA), ECTIF_ENA);
...
{
	vRegWriteFldAlign(ECTI_00, wReadQualityTable(QUALITY_CTIR_ENA), ECTI_ENA);
...
{
#if defined(CC_MT5389) || defined(CC_MT5368)		// LI case : TDS_FLT_LTI_V = 0	
...
#if defined(CC_MT5389) || defined(CC_MT5368)		// LI case : TDS_FLT_LTI_V = 0
    if((E_TD_OUT_3D_LI==eDQCOut)||(E_TD_OUT_3D_LN_REP==eDQCOut))
	{
...
{
    vIO32WriteFldAlign(GLOBAL_SHARPNESS_GAIN, wReadQualityTable(QUALITY_OSD_MAPPING_SHARPNESS), SHARPNESS_GAIN);       //for vestel sharpness                                                                              
    vIO32WriteFldAlign(GLOBAL_SHARPNESS_GAIN,wReadQualityTable(QUALITY_OSD_MAPPING_3D_SHARPNESS) , SHARPNESS_GAIN_3D); //for vestel sharpness 
    // Coring Zero
...
{  
        vRegWriteFldAlign(TDSHP_2A, TDS_YLEV_VALUE[0], TDS_YLEV_P000);
        vRegWriteFldAlign(TDSHP_2A, TDS_YLEV_VALUE[1], TDS_YLEV_P064);
        vRegWriteFldAlign(TDSHP_2A, TDS_YLEV_VALUE[2], TDS_YLEV_P128);
        vRegWriteFldAlign(TDSHP_2A, TDS_YLEV_VALUE[3], TDS_YLEV_P192);
        vRegWriteFldAlign(TDSHP_2B, TDS_YLEV_VALUE[4], TDS_YLEV_P256);
        vRegWriteFldAlign(TDSHP_2B, TDS_YLEV_VALUE[5], TDS_YLEV_P320);
        vRegWriteFldAlign(TDSHP_2B, TDS_YLEV_VALUE[6], TDS_YLEV_P384);
        vRegWriteFldAlign(TDSHP_2B, TDS_YLEV_VALUE[7], TDS_YLEV_P448);
        vRegWriteFldAlign(TDSHP_2C, TDS_YLEV_VALUE[8], TDS_YLEV_P512);
        vRegWriteFldAlign(TDSHP_2C, TDS_YLEV_VALUE[9], TDS_YLEV_P576);
        vRegWriteFldAlign(TDSHP_2C, TDS_YLEV_VALUE[10], TDS_YLEV_P640);
        vRegWriteFldAlign(TDSHP_2C, TDS_YLEV_VALUE[11], TDS_YLEV_P704);
        vRegWriteFldAlign(TDSHP_2D, TDS_YLEV_VALUE[12], TDS_YLEV_P768);
        vRegWriteFldAlign(TDSHP_2D, TDS_YLEV_VALUE[13], TDS_YLEV_P832);
        vRegWriteFldAlign(TDSHP_2D, TDS_YLEV_VALUE[14], TDS_YLEV_P896);
        vRegWriteFldAlign(TDSHP_2D, TDS_YLEV_VALUE[15], TDS_YLEV_P960);
        vRegWriteFldAlign(TDSHP_2E, TDS_YLEV_VALUE[16], TDS_YLEV_P1024);
}
...
//Begin of Work around for sharpness
#define TDS_GLOBAL_GAIN_UNIT 0x80
void vDrvGlobalGainbyYLEV(void)
{
    UINT8 bIndex;
    
    static UINT8 u1PreGlobalGain = 0x80;
    
    UINT8 u1GlobalGain = 0x80;

    if(u4DrvTDTV3DModeQuery() != E_TDTV_UI_3D_MODE_OFF)
    {
        u1GlobalGain = IO32ReadFldAlign(GLOBAL_SHARPNESS_GAIN, SHARPNESS_GAIN_3D);
    }
    else
    {
        u1GlobalGain = IO32ReadFldAlign(GLOBAL_SHARPNESS_GAIN, SHARPNESS_GAIN);
    }    

    if((u1PreGlobalGain != u1GlobalGain))
    {
        u1PreGlobalGain = u1GlobalGain;
        //vDrvPostSharpParam();
        for (bIndex = 0; bIndex < 17; bIndex ++)
        {
            TDS_YLEV_VALUE[bIndex]=MIN((u1GlobalGain*TDS_YLEV[bIndex])/TDS_GLOBAL_GAIN_UNIT, 0xFF);
        }
    }
}
//End of Work around for sharpness
...
void vDrvAdapYLev(void)
{
    if (fgAdapYLEV == SV_ON && IO32ReadFldAlign(TDPROC_YLEV_00, TDS_YLEV_ADAP_ENA))
    {
       ...
       vDrvGlobalGainbyYLEV();    //Work around for sharpness
