...
#define MAX_LISR_VECTOR     (31)
#define MAX_VECTOR_ID       (MAX_IRQ_VECTOR)    // Max vector ID
...
#define HISR_PRIORITY_HIGH  0                   // Priority of the high-priority HISR
...
{
    ASSERT((i4Vector >= 0) && (i4Vector <= MAX_LISR_VECTOR));
    ASSERT(_arIsrList[i4Vector].u2Vector == (UINT16)i4Vector);
    ASSERT(_arIsrList[i4Vector].pfIsr != NULL);

    if (i4Vector==VECTOR_T1)
    {
        // Invoke user ISR directly.
        _arIsrList[i4Vector].pfIsr((UINT16)i4Vector);
    }
    // this is only for DRAM controller interrupt, MID, not for ALL MISC
    else if ((i4Vector==VECTOR_MISC) && (BIM_READ32(REG_MISC_IRQEN) & 1) && (BIM_READ32(REG_MISC_IRQST) & 1))
    {
        _arIsrList[i4Vector+1].pfIsr((UINT16)(i4Vector+1));
    }
    else
    {
        DRVCUST_UserLISR(i4Vector);
...
/*----------------------------------------------------------------------------
 * Function: DriverMiscHISR
 *
 * Description:
 *      The misc HISR for registered user ISR.
 ...
 * Returns: -
 *---------------------------------------------------------------------------*/
static void DriverMiscHISR(UINT16 ui2_vector_id)
{
    UINT32 i, u4Val;
    INT32 i4Vector;

    ASSERT(ui2_vector_id == VECTOR_MISC);

    u4Val = BIM_READ32(REG_MISC_IRQST) & BIM_READ32(REG_MISC_IRQEN);
    for (i=0; i<MAX_MISC_VECTOR; i++)
    {
        if (u4Val & (1U << i))
        {
            i4Vector = ((VECTOR_MISC + i) + 1);
            if (_arIsrList[i4Vector].pfIsr != NULL)
            {
                _arIsrList[i4Vector].pfIsr((UINT16)i4Vector);
            }
            else
            {
                Printf("MISC(0x%08x) interrupt %d no routine hook\n", u4Val, i);
            }
            BIM_WRITE32(REG_MISC_IRQCLR, (1U << i));
        }
    }
}

#if defined(CC_MT5368) || defined(CC_MT5396) || defined(CC_MT5389)
static void DriverMisc2HISR(UINT16 ui2_vector_id)
{
    UINT32 i, u4Val;
    INT32 i4Vector;

    ASSERT(ui2_vector_id == VECTOR_MISC2);

    u4Val = BIM_READ32(REG_MISC2_IRQST) & BIM_READ32(REG_MISC2_IRQEN);
    for (i=0; i<MAX_MISC2_VECTOR; i++)
    {
        if (u4Val & (1U << i))
        {
            i4Vector = VECTOR_MISC2_BASE + i;
            if (i4Vector < ISR_VECTOR_SIZE)
            {
                if (_arIsrList[i4Vector].pfIsr != NULL)
                {
                    _arIsrList[i4Vector].pfIsr((UINT16)i4Vector);
                ...
                {
                    Printf("MISC2(0x%08x) interrupt %d no routine hook\n", u4Val, i);
                }
            }
            BIM_WRITE32(REG_MISC2_IRQCLR, (1U << i));
        }
    }
}
#endif
...
#if (TIMEPROFILE_LOG == 1)
    ExeTimeRecordStart(&ExeTime[pDrvIsr->u2Vector].RawTimeStart);
#endif
    // Invoke user ISR
    pDrvIsr->pfIsr(pDrvIsr->u2Vector);
#if (TIMEPROFILE_LOG == 1)
    ExeTimeRecordStop(&ExeTime[pDrvIsr->u2Vector] );
#endif
                 ...
                 x_os_drv_isr_fct   *ppf_old_isr)
{
...
#if defined(CC_MT5368) || defined(CC_MT5396) || defined(CC_MT5389)
    if (ui2_vec_id >= VECTOR_MISC2_BASE)
    {
        if (_arIsrList[VECTOR_MISC2].pfIsr == NULL)
        {
            x_os_drv_reg_isr(VECTOR_MISC2, DriverMisc2HISR, &pf_old_isr);
        }
    }
    else
#endif
    if (ui2_vec_id > MAX_LISR_VECTOR)
    {
        if (_arIsrList[VECTOR_MISC].pfIsr == NULL)
        {
            x_os_drv_reg_isr(VECTOR_MISC, DriverMiscHISR, &pf_old_isr);
        }
    }
    ...
    if (pf_isr != NULL)
    {
        if ((ui2_vec_id <= VECTOR_MISC) && (_arIsrList[ui2_vec_id].pfIsr == NULL))
        {
        ...
        else if (_arIsrList[ui2_vec_id].pfIsr == NULL) // ui2_vec_id > VECTOR_MISC
        {
            _arIsrList[ui2_vec_id].u2Vector = ui2_vec_id;
    ...
    else
    {
        ...
        else if (_arIsrList[ui2_vec_id].pfIsr != NULL)
        {
            _arIsrList[ui2_vec_id].u2Vector = MAX_VECTOR_ID + 1;
            _arIsrList[ui2_vec_id].pvStack = NULL;
            _arIsrList[ui2_vec_id].pfIsr = pf_isr;
                 ...
                 x_os_drv_isr_fct   *ppf_old_isr)
{
    ...
    if (pf_isr != NULL)
    {
        if (_arIsrList[ui2_vec_id].pfIsr == NULL)
        {
            ...
            memset(&_arIsrList[ui2_vec_id].hisr, 0, sizeof(NU_HISR));
            if (NU_Create_HISR(&_arIsrList[ui2_vec_id].hisr, (CHAR*)szName,
                DriverCommonHISR, HISR_PRIORITY_HIGH, pvStack, HISR_STACK_SIZE)
                != NU_SUCCESS)
            {
                DriverMemFree(pvStack);
                goto _Done;
