...
static INT32 _PODCI_CheckCis(void)
{
    ...
    pu1CisMap = (UINT8 *)BSP_AllocDmaMemory(MAX_CIS_MAP_SIZE);
    if ( pu1CisMap == NULL )
    {
    ...
    {
    LOG(1, "_PODCI_CheckCis allocate memory ok !\n");
    }
    ...
    {
        LOG(1, "_PODCI_CheckCis: ok2 %d\n", i4Return);
    }

    BSP_FreeDmaMemory((UINT32)pu1CisMap);
...
static BOOL _PODCI_NegotiateBufSize(UINT16* pu2Buf)
{
    ...
    {
        case POD_DATA_CHANNEL:
            LOG(1, "eAccessMode = POD_DATA_CHANNEL\n");
            u4DataRegAddr   = (UINT32)POD_DATA_REG & (UINT32)DATA_CHANNEL;
            u4CtrlStsRegAddr= (UINT32)POD_CTRL_STS_REG & (UINT32)DATA_CHANNEL;
            u4SizeRegLsAddr = (UINT32)POD_SIZE_REG_LS & (UINT32)DATA_CHANNEL;
            u4SizeRegMsAddr = (UINT32)POD_SIZE_REG_MS & (UINT32)DATA_CHANNEL;
        ...
        case POD_EXTENDED_CHANNEL:
            LOG(1, "eAccessMode = POD_EXTENDED_CHANNEL\n");
            u4DataRegAddr   = (UINT32)POD_DATA_REG | (UINT32)EXT_CHANNEL;
            u4CtrlStsRegAddr= (UINT32)POD_CTRL_STS_REG | (UINT32)EXT_CHANNEL;
            u4SizeRegLsAddr = (UINT32)POD_SIZE_REG_LS | (UINT32)EXT_CHANNEL;
            u4SizeRegMsAddr = (UINT32)POD_SIZE_REG_MS | (UINT32)EXT_CHANNEL;
        ...
        case PCMCIA_COMMON_MEM:
        default:
		    printf("_PODCI_NegotiateBufSize: POD_INVALID_ACCESS_MODE\n");
            return FALSE;
    }
    ...
    *pu2Buf = 16;
    /* Size read */
    u1CmdRegister |= SR;
    printf("_PODCI_NegotiateBufSize: u1CmdReg=0x%x\n", u1CmdRegister);
    PCMCIAHW_WRITE32(REG_DEV_ADDR, u4CtrlStsRegAddr);
    PCMCIAHW_WRITE32(REG_SDA_DATA_WRITE, (UINT32)u1CmdRegister); /* SR -> 1 */
    PCMCIAHW_WRITE32(REG_CMD, (UINT32)(PC_SDA_WRITE | ICMD_IE));
    if ( PCMCIA_GetCmdStatusDone() != TRUE )
    {
        printf("_PODCI_NegotiateBufSize:_PODCI_NegotiateBufSize err1\n");
        return FALSE;
    }
    if (PODCI_WaitForModuleStatus((UINT8)DA, WAIT_FOR_MODULE_STATUS_COUNT) != POD_OK) /* Test DA */
    {
        printf("_PODCI_NegotiateBufSize:PODCI_WaitForModuleStatus DA fail1\n");
        return FALSE;
    }
    ...
    if ( PCMCIA_GetCmdStatusDone() != TRUE )
    {
        printf("_PODCI_NegotiateBufSize err2\n");
        return FALSE;
    }
    ...
    if ( PCMCIA_GetCmdStatusDone() != TRUE )
    {
        printf("_PODCI_NegotiateBufSize err3\n");
        return FALSE;
    }
    u2Tmp = (UINT16)PCMCIAHW_READ8(REG_SDA_DATA_READ);
    ...
    if (u2DataLen > 2)
    {
        printf("_PODCI_NegotiateBufSize: u2DataLen > 2\n");
        return FALSE;
    }
    ...
    if ( u2DataLen == 1 )
    {
        ...
        if ( PCMCIA_GetCmdStatusDone() != TRUE )
        {
            printf("_PODCI_NegotiateBufSize err4\n");
            return FALSE;
        }
    ...
    else if ( u2DataLen == 2 )
    {
        ...
        if ( PCMCIA_GetCmdStatusDone() != TRUE )
        {
            printf("_PODCI_NegotiateBufSize err4.1\n");
            return FALSE;
        }
        *pu2BufSize = (((UINT16)PCMCIAHW_READ8(REG_SDA_DATA_READ)<<8) & 0xFF00);
        ...
        if ( PCMCIA_GetCmdStatusDone() != TRUE )
        {
            printf("_PODCI_NegotiateBufSize err4.2\n");
            return FALSE;
        }
        *pu2BufSize |= (UINT16)(PCMCIAHW_READ8(REG_SDA_DATA_READ) & 0xFF);
    }
    ...
    {
        printf("_PODCI_NegotiateBufSize err4.3\n");
        return FALSE;
    }
    ...
    if ( PCMCIA_GetCmdStatusDone() != TRUE )
    {
        printf("_PODCI_NegotiateBufSize err5\n");
        return FALSE;
    }

    printf("Buffer size from CAM : %d\n", *pu2BufSize);
    /* Compare the buffer size with our buffer size */
    *pu2BufSize = MIN_VALUE(*pu2BufSize, 1024);
    printf("Buffer size decided : %d\n", *pu2BufSize);
    ...
    if ( PCMCIA_GetCmdStatusDone() != TRUE )
    {
        printf("_PODCI_NegotiateBufSize err6\n");
        return FALSE;
    }
    // wait FR
    if (PODCI_WaitForModuleStatus((UINT8)FR, WAIT_FOR_MODULE_STATUS_COUNT) != POD_OK) /* Test FR */
    {
        printf("_PODCI_NegotiateBufSize wait FR error\n");
        return FALSE;
    }
    ...
    if ( PCMCIA_GetCmdStatusDone() != TRUE )
    {
        printf("_PODCI_NegotiateBufSize err6.1\n");
        return FALSE;
    }
    ...
    if ( PCMCIA_GetCmdStatusDone() != TRUE )
    {
        printf("_PODCI_NegotiateBufSize err8\n");
        return FALSE;
    }
    ...
    if ( PCMCIA_GetCmdStatusDone() != TRUE )
    {
        printf("_PODCI_NegotiateBufSize err9\n");
        return FALSE;
    }
    for (u1I = (UINT8)u2DataLen; u1I > 0; u1I--)
    {
        ...
        if ( PCMCIA_GetCmdStatusDone() != TRUE )
        {
            printf("_PODCI_NegotiateBufSize err9.1\n");
            return FALSE;
        }
    }
    ...
    if ( PCMCIA_GetCmdStatusDone() != TRUE )
    {
        printf("_PODCI_NegotiateBufSize err10\n");
        return FALSE;
    }
    ...
    if ( PCMCIA_GetCmdStatusDone() != TRUE )
    {
        printf("_PODCI_NegotiateBufSize err10.1\n");
        return FALSE;
    }
    ...
    *pu2Buf = *pu2BufSize;
    ...
    if (PODCI_WaitForModuleStatus((UINT8)FR, WAIT_FOR_MODULE_STATUS_COUNT) != POD_OK) /* Test FR */
    {
        printf("_PODCI_NegotiateBufSize wait FR error1\n");
        return FALSE;
    }

    printf("_PODCI_NegotiateBufSize end\n");
    return TRUE;
}
                              ...
                              UINT16   u2TimeOut)
{
    ...
    while (u2Count < u2TimeOut)
    {
        ...
        if ( PCMCIA_GetCmdStatusDone() != TRUE )
        {
            LOG(1, "PODCI_WaitForModuleStatus err1\n");
            return POD_CMD_ERROR;
        }
    ...
    {
        printf("PODCI_WaitForModuleStatus timeout, status:0x%x, statusbit:0x%x\n",u4Status,  u1StatusBit);
        return POD_TIME_OUT;
    }
    else
    {
        LOG(5, "PODCI_WaitForModuleStatus 0x%x ok: u4Status=0x%x\n", u1StatusBit, u4Status);
    }
...
BOOL PODCI_ResetChannel(void)
{
    ...
    if ( PCMCIA_GetCmdStatusDone() != TRUE )
    {
        printf("PODCI_ResetChannel err1\n");
        return FALSE;
    }
    ...
    if ( PCMCIA_GetCmdStatusDone() != TRUE )
    {
        printf("PODCI_ResetChannel err2\n");
        return FALSE;
    }
    x_thread_delay(5); /* At least 40us */
    ...
    {
        printf("ResetChannel failed!  no FR!!\n");
        return FALSE;
    }
...
INT32 PODCI_WriteCard(POD_IO_STATE_T eIoState, const UINT8* pu1Data, UINT16 u2DataLen)
{
    ...
    u2TempSize = u2DataLen;
    while (u2TempSize > u2NgoBufSize)
    {
        switch (eIoState)
        {
            case POD_SDA:
                i4Return = _PCMCIAHW_WriteCard(pu1Data, u2NgoBufSize, FALSE);
                break;
        ...
        u2TempSize -= u2NgoBufSize;
    }
    switch (eIoState)
    {
        case POD_SDA:
            i4Return = _PCMCIAHW_WriteCard(pu1Data, u2TempSize, FALSE);
            break;
...
void PODCI_AllocDmaBuf(void)
{
    if ( pu1DataBuf == NULL )
    {
        pu1DataBuf = (UINT8*)(VIRTUAL(BSP_AllocAlignedDmaMemory(DMA_BUF_SIZE,32)));
