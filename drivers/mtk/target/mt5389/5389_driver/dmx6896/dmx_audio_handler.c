...
static UINT8 _u1DmxAudActivePidx = 0xFF;
...
static BOOL _DmxAudActHandler(DMX_AUDIO_PES_T* prPes)
{
    ...
    if(!prPes->fgEOS)
    {
        ...
        if((VIRTUAL(prPes->u4PesRp)+u4SrcSize)>VIRTUAL(prSrcPid->u4BufEnd))
		{
		  HalFlushInvalidateDCacheMultipleLine(VIRTUAL(prPes->u4PesRp),VIRTUAL(prSrcPid->u4BufEnd)-VIRTUAL(prPes->u4PesRp));
		  HalFlushInvalidateDCacheMultipleLine(VIRTUAL(prSrcPid->u4BufStart),u4SrcSize-(VIRTUAL(prSrcPid->u4BufEnd)-VIRTUAL(prPes->u4PesRp)));
		}
		else
		{
	    ...
	    if(_arDmxAudQRec[prPes->u1PidIndex].rQRec.u2Size > (_arDmxAudQRec[prPes->u1PidIndex].rQRec.u2Capacity - 2))
	    {
	        ...
	        if(!_DMX_UpdateReadPointer(prPes->u1PidIndex, VIRTUAL(rElement.u4Wp), 0))
	        {
	            return FALSE;
	        }
	    }
        ...
        HalFlushInvalidateDCacheMultipleLine(prDstPid->u4BufStart,
            (prDstPid->u4BufEnd + 1) - prDstPid->u4BufStart);
    ...
    {
        LOG(3, "DMX audio handler send audio fail\n");
        return FALSE;
    }
...
static VOID _DmxAudThread(VOID* pvArg)
{
    ...
    while (1)
    {
        ...
        {
            _DMX_UnlockApi();
            LOG(3, "%s, DMX audio handler send audio fail\n", __FUNCTION__);
            continue;
        }
...
BOOL _DMX_AudHandler_SetActivePidxEx(UINT8 u1Idx, UINT8* pu1Buf, UINT32 u4Size)
{
    ...
    if((_arDmxAudQRec[u1Idx].fgEnable) && (_arDmxAudQRec[u1Idx].rQRec.u2Size > 0))
    {
        ...
        while(u4SrcSize > (prDstPid->u4BufLen - u4DstSize) && (_arDmxAudQRec[u1Idx].rQRec.u2Size > 0))
        {
	        ...
	        if(!_DMX_UpdateReadPointer(u1Idx, VIRTUAL(rElement.u4Wp), 0))
	        {
	            return FALSE;
	        }
