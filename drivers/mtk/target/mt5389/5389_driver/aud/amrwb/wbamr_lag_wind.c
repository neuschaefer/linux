...
/*************************************************************************
 *  Description : WBAMR_Lag_window on WBAMR_Autocorrelations.                         
 *                 r[i] *= lag_wind[i]                                     
 *                 r[i] and lag_wind[i] are in special double precision.
 *************************************************************************/
 
WBAMR_CODE_SECTION void WBAMR_Lag_window ( 
  fract16 r_h[],
  fract16 r_l[]
 ) 

{
  ...
  for ( i = 1; i <= M11; i++ )
  {
    x = WBAMR_Mpy_32 ( r_h[i], r_l[i], lag_h[i - 1], lag_l[i - 1] );
  
    WBAMR_L_Extract ( x, &r_h[i], &r_l[i] );
  }
