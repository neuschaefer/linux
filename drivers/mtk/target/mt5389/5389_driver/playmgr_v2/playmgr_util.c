...
BOOL _PLAYMGR_InitMediaInfo(VOID)
{
    ...
    if (x_minfo_ogg_init()!= MFMTRECGR_OK)
    {
        return FALSE;
    }
  /*  if (x_midxbuld_ogg_init()!= MFMTRECGR_OK)
    {
        return FALSE;
    }*/
#endif
	...
	if(x_midxbuld_flv_init()!=MIDXBULDR_OK)
	{
       return FALSE;
	}
#endif
...
BOOL _PLAYMGR_RecgFile(PLAYMGR_INFO_T* prMediaInfo, CHAR *szFileName)
{
   ...
   // FeederSetRing((FeederSourceType)prMediaInfo->rMediaFeeder.h_feeder, FALSE);
   // FeederSetOffset((FeederSourceType)prMediaInfo->rMediaFeeder.h_feeder, (UINT64)0, PLAYMGR_SEEK_BGN, 0);
...
VOID _PLAYMGR_InitSubtitle(UINT8 u1SrcId)
{
    ...
    prPlaymgrInst=_PLAYMGR_GetInst(u1SrcId);
    if(!prPlaymgrInst)
    {
        LOG(3, "_PLAYMGR_ChkSubtitle(%d), prPlaymgrInst null\n", u1SrcId);
        return;
    }
    prAviInfo = &prPlaymgrInst->FmtInfo.rAviInfo;
    i4Height = prAviInfo->rStmAttribute[0].u.t_avi_video_stm_attr.i4_height;
  ...
  if (PLAYMGR_GetInfo(u1SrcId, &rPlaymgrStatus))
  {
    ...
    {
      LOG(1, "Unsupported subtitle format!!!\n");
    }
  }
}
...
VOID _PLAYMGR_InitSubtitle(UINT8 u1SrcId)
{
  ...
  prPlaymgrInst->rSpuStatus.eState = SPU_NONE;
  prPlaymgrInst->rSpuStatus.fgOutBufEmpty = TRUE;
  prPlaymgrInst->rSpuStatus.u4SpuRp = 0;
  prPlaymgrInst->rSpuStatus.u4SpuWp = 0;

  if (i4Height <= 576)
  {
    prPlaymgrInst->rSpuStatus.u4WindowWidth = 640;
    prPlaymgrInst->rSpuStatus.u4WindowHeight = 480;
  }
  else if (i4Height <= 720)
  {
    prPlaymgrInst->rSpuStatus.u4WindowWidth = 960;
    prPlaymgrInst->rSpuStatus.u4WindowHeight = 720;
  }
  else
  {
    prPlaymgrInst->rSpuStatus.u4WindowWidth = 1440;
    prPlaymgrInst->rSpuStatus.u4WindowHeight = 1080;
  }
  ...
  if (PLAYMGR_GetInfo(u1SrcId, &rPlaymgrStatus))
  {
    if (rPlaymgrStatus.aeCapFormat[0] == PLAYMGR_CAPTION_AVI_BITMAP)
    {
      prPlaymgrInst->rSpuStatus.fgXsubPlus = FALSE;
    }
    else if (rPlaymgrStatus.aeCapFormat[0] == PLAYMGR_CAPTION_AVI_DXSA)
    {
      prPlaymgrInst->rSpuStatus.fgXsubPlus = TRUE;
    }
    else
    {
      LOG(1, "Unsupported subtitle format!!!\n");
...
VOID vSpuDec(UINT8 u1SrcId, BOOL fgRealDec, UCHAR *pucSrc, UCHAR *pucDst, PLAYMGR_SUBTITLE_INFO_T *prSubtitleInfo)
{
    UINT32 u4Width;
    UINT32 u4Height;
    UINT32 u4Left;
  ...
  prSubtitleInfo->u4WindowWidth = prPlaymgrInst->rSpuStatus.u4WindowWidth;
  prSubtitleInfo->u4WindowHeight = prPlaymgrInst->rSpuStatus.u4WindowHeight;
  ...
  u4Col2 = (UINT32)LOAD_RGB(47);
  u4Col3 = (UINT32)LOAD_RGB(50);
  if (prPlaymgrInst->rSpuStatus.fgXsubPlus)
  {
    u1Alpha0 = SPU_LOAD_BYTE(53);
    u1Alpha1 = SPU_LOAD_BYTE(54);
    u1Alpha2 = SPU_LOAD_BYTE(55);
  ...
  if (fgRealDec)
  {
    ...
    vSpuSWDec(pucSrc + (prPlaymgrInst->rSpuStatus.fgXsubPlus ? 57 : 53), pucDst, u4Width, u4Height, u4Width, 0xFFFFFFFF);
    vSpuSWDec(pucSrc + (prPlaymgrInst->rSpuStatus.fgXsubPlus ? 57 : 53) + u4BotOfst, pucDst + u4Width, u4Width, u4Height, u4Width, 0xFFFFFFFF);
  }
...
BOOL _PLAYMGR_DrawSubtitle(UINT8 u1SrcId, PLAYMGR_STATUS_CMD_T *prStatusCmd,
    PLAYMGR_INFO_T* prMediaInfo)
{
    ...
    if (prMediaInfo->rPlayerStatus.eMediaType == PLAYMGR_MEDIA_TYPE_AVI)
    {
        ...
        SWDMX_ReadBuffer(u1SrcId, eSWDMX_STRM_TYPE_SUBTITLE, &rSwdmxReadInfo);

        if (prPlaymgrInst->rSpuStatus.eState == SPU_NONE)
        {
          prSubtitleInfo = (PLAYMGR_SUBTITLE_INFO_T *)prPlaymgrInst->pucSubOutBuf;
          fgRealDec = TRUE;
        }
        else
        {
          prSubtitleInfo = &prPlaymgrInst->rTmpSubtInfo;
          fgRealDec = FALSE;
        }
        ...
        vSpuDec(u1SrcId,
                ...
                prPlaymgrInst->pucSubOutBuf + 64,   // first 64 bytes are reserved for SPU parameter structure
                prSubtitleInfo);

        if (fgRealDec && prSubtitleInfo->u4StartPts <= PLAYMGR_GetPos(u1SrcId))
        {
          LOG(5, "SPU W x H = %u x %u, X : Y = %d : %d, S.PTS = %d, E.PTS = %d drawn at once\n", prSubtitleInfo->u4Width, prSubtitleInfo->u4Height, prSubtitleInfo->u4XPos, prSubtitleInfo->u4YPos, prSubtitleInfo->u4StartPts, prSubtitleInfo->u4EndPts);
          ...
          FeederNotifyHandle(prMediaInfo->eFeederSrc, FEEDER_NOTIFY_DRAWSUBTITLE, u4Param);
          //_rSpuStatus.eState = SPU_WAIT_DRAW_ACK;  // ToDo: ACK mechanism to water proof
          prPlaymgrInst->rSpuStatus.eState = SPU_NONE;  // this assumes the user space drawing code is fast enough
          prPlaymgrInst->rSpuStatus.fgOutBufEmpty = TRUE;

          // Unlock subtitle buffer
          SWDMX_ReleaseBuffer(u1SrcId, eSWDMX_STRM_TYPE_SUBTITLE, &rSwdmxReadInfo);
        }
        else
        {
          LOG(5, "SPU W x H = %d x %d, X : Y = %d : %d, S.PTS = %d, E.PTS = %d pending\n", prSubtitleInfo->u4Width, prSubtitleInfo->u4Height, prSubtitleInfo->u4XPos, prSubtitleInfo->u4YPos, prSubtitleInfo->u4StartPts, prSubtitleInfo->u4EndPts);
          if (fgRealDec)
          {
            prPlaymgrInst->rSpuStatus.eState = SPU_DEC_OK;
            prPlaymgrInst->rSpuStatus.fgOutBufEmpty = FALSE;
          }
          else
          {
            prPlaymgrInst->rSpuStatus.eState = SPU_WAIT_DEC;
          }
          prPlaymgrInst->rSpuStatus.pucSpuBuf = rSwdmxReadInfo.pucDest;
          prPlaymgrInst->rSpuStatus.u4FrameAddr[prPlaymgrInst->rSpuStatus.u4SpuWp] = rSwdmxReadInfo.u4FrameAddr;
          prPlaymgrInst->rSpuStatus.u4CopyLen[prPlaymgrInst->rSpuStatus.u4SpuWp] = rSwdmxReadInfo.u4CopyLen;
          prPlaymgrInst->rSpuStatus.u1SerialNumber[prPlaymgrInst->rSpuStatus.u4SpuWp] = rSwdmxReadInfo.u1SerialNumber;
          prPlaymgrInst->rSpuStatus.u4StartPts[prPlaymgrInst->rSpuStatus.u4SpuWp] = prSubtitleInfo->u4StartPts;
          prPlaymgrInst->rSpuStatus.u4EndPts[prPlaymgrInst->rSpuStatus.u4SpuWp++] = prSubtitleInfo->u4EndPts;
          prPlaymgrInst->rSpuStatus.u4SpuWp %= 5;
        }
...
BOOL _PLAYMGR_ChkSubtitle(UINT8 u1SrcId, BOOL fgDrawAck)
{
    PLAYMGR_INFO_T *prPlaymgrInst;
    UINT32 u4Param;
    PLAYMGR_SUBTITLE_INFO_T *prSubtitleInfo;
    ...
    if (prPlaymgrInst->rPlayerStatus.eMediaType == PLAYMGR_MEDIA_TYPE_AVI)
    {
        if (prPlaymgrInst->rSpuStatus.eState == SPU_WAIT_DRAW_ACK && fgDrawAck)
        {
          prPlaymgrInst->rSpuStatus.fgOutBufEmpty = TRUE;

          // Unlock subtitle buffer
          rSwdmxReadInfo.u4FrameAddr = prPlaymgrInst->rSpuStatus.u4FrameAddr[prPlaymgrInst->rSpuStatus.u4SpuRp];
          rSwdmxReadInfo.u4CopyLen = prPlaymgrInst->rSpuStatus.u4CopyLen[prPlaymgrInst->rSpuStatus.u4SpuRp];
          rSwdmxReadInfo.u1SerialNumber = (UINT8)prPlaymgrInst->rSpuStatus.u1SerialNumber[prPlaymgrInst->rSpuStatus.u4SpuRp++];
          prPlaymgrInst->rSpuStatus.u4SpuRp %= 5;
          SWDMX_ReleaseBuffer(u1SrcId, eSWDMX_STRM_TYPE_SUBTITLE, &rSwdmxReadInfo);

          if (prPlaymgrInst->rSpuStatus.u4SpuRp == prPlaymgrInst->rSpuStatus.u4SpuWp)
            prPlaymgrInst->rSpuStatus.eState = SPU_NONE;
          else
            prPlaymgrInst->rSpuStatus.eState = SPU_WAIT_DEC;
        ...
        if (prPlaymgrInst->rSpuStatus.eState != SPU_DEC_OK && prPlaymgrInst->rSpuStatus.eState != SPU_WAIT_DEC)
          return FALSE;

        if (prPlaymgrInst->rSpuStatus.u4StartPts[prPlaymgrInst->rSpuStatus.u4SpuRp] > PLAYMGR_GetPos(u1SrcId))
          return FALSE;

        if (prPlaymgrInst->rSpuStatus.eState == SPU_DEC_OK)
        {
          fgRealDec = FALSE;
        }
        if (prPlaymgrInst->rSpuStatus.eState == SPU_WAIT_DEC && prPlaymgrInst->rSpuStatus.fgOutBufEmpty)
        {
          fgRealDec = TRUE;
          prPlaymgrInst->rSpuStatus.eState = SPU_DEC_OK;
          prPlaymgrInst->rSpuStatus.fgOutBufEmpty = FALSE;
        ...
        // Get subtitle buffer for decoding.
        rSwdmxReadInfo.u4FrameAddr = prPlaymgrInst->rSpuStatus.u4FrameAddr[prPlaymgrInst->rSpuStatus.u4SpuRp];
        rSwdmxReadInfo.pucDest = prPlaymgrInst->rSpuStatus.pucSpuBuf;
        rSwdmxReadInfo.u4CopyLen = prPlaymgrInst->rSpuStatus.u4CopyLen[prPlaymgrInst->rSpuStatus.u4SpuRp];
        rSwdmxReadInfo.u1SerialNumber = prPlaymgrInst->rSpuStatus.u1SerialNumber[prPlaymgrInst->rSpuStatus.u4SpuRp];
        SWDMX_ReadBuffer(u1SrcId, eSWDMX_STRM_TYPE_SUBTITLE, &rSwdmxReadInfo);

        // decode subtitle
        vSpuDec(u1SrcId,
                fgRealDec,
                prPlaymgrInst->rSpuStatus.pucSpuBuf,
                prPlaymgrInst->pucSubOutBuf + 64,   // first 64 bytes are reserved for SPU parameter structure
                prSubtitleInfo);

        LOG(5, "SPU W x H = %d x %d, X : Y = %d : %d, S.PTS = %d, E.PTS = %d  ==> drawn!!!\n", prSubtitleInfo->u4Width, prSubtitleInfo->u4Height, prSubtitleInfo->u4XPos, prSubtitleInfo->u4YPos, prSubtitleInfo->u4StartPts, prSubtitleInfo->u4EndPts);

        if (prPlaymgrInst->rSpuStatus.fgOutBufEmpty)
        {
            LOG(1, "!!! fatal error !!!\n");
        }
        ...
        //call back function
        //Printf("notify info : %d\n",u4Param);
        // FixMe: should be prMediaInfo->eFeederSrc
        FeederNotifyHandle(FEEDER_PROGRAM_SOURCE, FEEDER_NOTIFY_DRAWSUBTITLE, u4Param);
        prPlaymgrInst->rSpuStatus.eState = SPU_WAIT_DRAW_ACK;
