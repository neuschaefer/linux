...
UINT32 u4RstPinNum;
    ...
    if (DRVCUST_OptQuery(eDemodResetGpio, &u4RstPinNum) == 0)
    {
        ...
        mcSHOW_USER_MSG(("Reset ISDBT Demod GPIO=0x%x, 0->1, %dms\r\n",u4RstPinNum, u2DelayTime));
    }
}
...
STATIC void ISDBT_TunerDtdCommand(PTD_SPECIFIC_CTX ptTDSpecificCtx,INT32 i4Argc, const CHAR** aszArgv)
{
    ...
    if (i4Argc < 1)
    {
        ...
        mcSHOW_USER_MSG(("\t d [DevAddr] :    Register Page Dump Result\n")); 
        mcSHOW_USER_MSG(("\t r  [RegAddr] [Num]:  read  Demod register\n"));
        mcSHOW_USER_MSG(("\t w [RegAddr] [Value]: write Demod register\n"));
        mcSHOW_USER_MSG(("\t t  [Top]: Set/Get Tuner Top, (Top range: 0~7)\n"));   
        mcSHOW_USER_MSG(("\t p:          Tuner PLL status\n"));           
    }	
    ...
    case 'f':
        ...
        {		
            ...
            {
                mcSHOW_HW_MSG(("Failed on Config tuner (%d KHz)\n", u4Freq));
            }
            else
            {
                mcSHOW_USER_MSG(("Config tuner(%d KHz) OK!\n", u4Freq));
            }
        }
        break;

    case 'd':        
        mcSHOW_USER_MSG(("  Register Page Dump Result:\n"));
        mcSHOW_USER_MSG(("     00 01 02 03 04 05 06 07 08 09 0A 0B 0C 0D 0E 0F\n"));
        if(i4Argc >1)
        {
            u1SubAddr=(UINT8)StrToInt(aszArgv[1]);
            mcSHOW_USER_MSG(("Addr: 0x%02x \n", u1SubAddr));
            for (ii=0; ii<16; ii++)
            {
    ...
    case 't':
     ...
     case 'p':
        ...
        if(u2OPRet)
        {
            mcSHOW_HW_MSG(("ITuner_OP (eOperation = %d) Fail! Return Value = %d\n", itGetStatus, u2OPRet));
        }
        else
        {
            mcSHOW_USER_MSG(("Tuner PLL Status = %d (0:Unlock, 1:Lock)\n", bTunerSts));
        }
        break;
                                ...
                                VOID *pzAttribute, SIZE_T* pzAttributeLen)
{
    ...
    case TUNER_GET_SPECIAL_TYPE_EWS_STATUS:
     ...
     case TUNER_GET_TYPE_EWS_STATUS:
	 	{
			EWS_INFO  t_ews = {0};
			if(!ISDBT_GetEWSInfo(&t_ews))
        	{
            	t_ews.val=prevEwsVal;			
       	 	}
			*((UINT8 *)pzAttribute) = t_ews.val;
...
INT32 ISDBT_TunerOpen(PTD_CTX ptTDCtx, TUNER_CONFIG_T * ptTunerCfgInfo, PTD_SPECIFIC_CTX* pptTDSpecificCtx)
{
    ...
    *pptTDSpecificCtx = pPdCtx;
    ...
    if (psDemodCtx == NULL)
    {
        mcSHOW_HW_MSG(("SBTVD_DEMOD_CTX_T malloc fail\n"));
        return (DRVAPI_TUNER_ERROR);
    }
