                                ...
                                VOID *pzAttribute, SIZE_T* pzAttributeLen)
{
    ...
    default:
        mcSHOW_USER_MSG(("unknown: get Attribute\r\n"));
        break;
    }
    mcSEMA_UNLOCK_MTFE(pPdCtx->hHalLock);
    return DRVAPI_TUNER_OK;
...
INT32 ISDBT_TunerOpen(PTD_CTX ptTDCtx, TUNER_CONFIG_T * ptTunerCfgInfo, PTD_SPECIFIC_CTX* pptTDSpecificCtx)
{
    ...
    *pptTDSpecificCtx = pPdCtx;
    ...
    if (psDemodCtx == NULL)
    {
        mcSHOW_USER_MSG(("DEMOD_CTX_T malloc fail\n"));
        return (DRVAPI_TUNER_ERROR);
    }
    ...
    {
        mcSHOW_USER_MSG(("%s: DVB x_sema_create(pPdCtx->hHalLock) Fail!\n",__FUNCTION__));
        return (DRVAPI_TUNER_ERROR);
    }
...
VOID *GetDigiTunerCtx(VOID)
{
    if (!psDemodCtx)
    {
        mcSHOW_USER_MSG(("[ISDBT ERR] ISDBT_TunerOpen hasn't been executed!!!\n"));
    }
