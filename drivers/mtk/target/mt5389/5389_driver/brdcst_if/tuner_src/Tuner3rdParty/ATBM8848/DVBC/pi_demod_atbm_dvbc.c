...
BOOL ATBM_DVBC_Write(pDVBC_Demod_Ctx pDemodCtx,UINT16 u2RegAddr,UINT32 u4Data)
{
 ...
 mcSHOW_DBG_MSG(("Prepare to write DVBC register 0x%04X-0x%08X\n",u2RegAddr,u4Data));
...
BOOL ATBM_DVBC_Read(pDVBC_Demod_Ctx pDemodCtx,UINT16 u2RegAddr,UINT32 * u4Data)
{
  ...
  *u4Data|=(u1Data<<24);
  ...
  mcSHOW_DBG_MSG((" Read DVBC register 0x%04X-0x%08X\n",u2RegAddr,*u4Data));
...
BOOL ATBM_DVBC_Init(pDVBC_Demod_Ctx pDemodCtx)
{
  ...
  fgRet&=ATBM_DVBC_Write(pDemodCtx,0x0100,0x00);//IQ mode
  ...
  fgRet&=ATBM_DVBC_Write(pDemodCtx,0x0104,0x0001);//External gain control   
  fgRet&=ATBM_DVBC_Write(pDemodCtx,0x020C,0x0010);
  
  fgRet&=ATBM_DVBC_Write(pDemodCtx,0x0004,0x00);//DVBC Module start processing
  
  fgRet&=ATBMWrite(pDemodCtx->u1I2cAddr,0x0004,0x01);//1 bit: 1:Software complete the register setting 0:register setting  is not completed
 ...
 * u4Ber=u4PreBer;
 ...
 fgRet&=ATBM_DVBC_Read(pDemodCtx,0x0430,&u4CorrectedBerBits);//16bit width
 ...
 * u4Per=0;
 ...
 fgRet&=ATBM_DVBC_Read(pDemodCtx,0x0424,&u4TotalPackets);//16bits width
 ...
 * u4Per=u4PrePer;
 ...
 fgRet&=ATBM_DVBC_Read(pDemodCtx,0x0434,&u4UnCorrectedPackets);//16bit width
