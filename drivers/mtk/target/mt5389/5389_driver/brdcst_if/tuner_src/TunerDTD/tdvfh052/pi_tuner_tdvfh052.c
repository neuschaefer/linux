 ...
 * $Author: dtvbm11 $
 * $Date: 2012/04/27 $
 * $RCSfile: pi_tuner_tdvfh052.c,v $
 * $Revision: #1 $
 *
 *---------------------------------------------------------------------------*/

/** @file pi_tuner_tdvfh052.c
 *  Tuner control for LG TDVFH052.
 */
...
//-----------------------------------------------------------------------------
#define LG_TDVFH052_BASE          ((UINT16) 0xC2)
#define LG_TDVFH052_IF_FREQUENCY  ((UINT16) 44)   /* MHz */
#define LG_TDVFH052_LO_STEP       ((UINT16) 625)  /* 100Hz */
#define LG_TDVFH052_STR           "LG_TDVFH052"
...
//----------------------------------------------------------------------------- 
/** TDVFH052_TunerInit
 *  Tuner initialzation for TDVFH052.
 *  @param  psTunerCtx  Pointer of RF tuner driver context.
 *  @retval void
 */
//-----------------------------------------------------------------------------
VOID TDVFH052_TunerInit(RF_TUNER_CTX_T *psTunerCtx, UINT8 I2cAddress)
{
    ...
    psTunerCtx->u1TunerType = cLG_TDVFH052;
    psTunerCtx->u2IF_Freq = LG_TDVFH052_IF_FREQUENCY;
    psTunerCtx->u2LO_Step = LG_TDVFH052_LO_STEP;
...
#endif
    mcSHOW_DBG_MSG(("Init " LG_TDVFH052_STR "\n"));
}


//----------------------------------------------------------------------------- 
/** TDVFH052_TunerSetFreq
 *  Set Tuner PLL for TDVFH052 to tune RF frequency.
 *  @param  psTunerCtx  Pointer of RF tuner driver context.
 ...
 */
//-----------------------------------------------------------------------------
INT16 TDVFH052_TunerSetFreq(RF_TUNER_CTX_T *psTunerCtx, UINT32 Freq, UINT8 u1Mod, UINT16 u2Step)
{
UINT32  Lo;
UINT8   Nband, CB, AB;
UINT16  Ndivider;
...
UINT16  u2LO_Freq = 0;
    ...
    mcSHOW_DBG_MSG(("LG TDVFH052_SetFreq\n"));
    ...
    Freq = Freq / 1000;
    u2LO_Freq = LG_TDVFH052_IF_FREQUENCY*1000;
	
    if (fgAnalogSignal)
    	u2LO_Freq += 1750;
	
 /* Assign Control word for specific tuner */
   if (Freq < 53 || Freq > 865)         // For HRC issue & Chan135
    {
    ...
    TunerS[2] = (UCHAR) CB;   //62.5k 
    TunerS[3] = (UCHAR) Nband;//XTB=1
    TunerS[4] = (UCHAR) (CB | 0x18) ;
    TunerS[5] = (UCHAR) AB;//103dbuV
...
/* Write to tuner device */
    ...
    else
    {
        mcSHOW_DBG_MSG(("LO_%02X: (%d) %02X-%02X-%02X-%02X-%02X-%02X (%dKHZ)\n",
                       psTunerCtx->I2cAddress, TunerDataCnt, TunerS[0], TunerS[1], TunerS[2], TunerS[3], TunerS[4], TunerS[5],Ndivider* LG_TDVFH052_LO_STEP/10 - u2LO_Freq));
    }
...
//----------------------------------------------------------------------------- 
/**  TDVFH052_TunerGetStatus
 *  Set Tuner PLL staus for TDVFH052
 *  @param  psTunerCtx  Pointer of RF tuner driver context.
 *  @retval TRUE        PLL lock
 *  @retval FALSE       PLL unlock
 */
//-----------------------------------------------------------------------------
BOOL TDVFH052_TunerGetStatus(RF_TUNER_CTX_T *psTunerCtx)
{
UINT8 TunerS[4];
 ...
 */
//-----------------------------------------------------------------------------
CHAR *TDVFH052_TunerGetVer(RF_TUNER_CTX_T *psTunerCtx)
{
    return (LG_TDVFH052_STR);
}
/***********************************************************************/
