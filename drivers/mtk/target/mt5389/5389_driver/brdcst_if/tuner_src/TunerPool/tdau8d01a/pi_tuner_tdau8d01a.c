 ...
 * $Author: dtvbm11 $
 * $Date: 2012/04/27 $
 * $RCSfile: pi_tuner_TDAU8D01A.c,v $
 * $Revision: #1 $
 *
 *---------------------------------------------------------------------------*/

/** @file pi_tuner_TDAU8D01A.c
 *  Tuner control for SS TDAU8D01A
 */
...
#include "eq_script_tdau8d01a.h"
#include "pi_demod_atd.h"
//-----------------------------------------------------------------------------
// Constant definitions
//-----------------------------------------------------------------------------
#define ALPS_TDAU8D01A_BASE            ((UINT16) 0xC0)
#define ALPS_TDAU8D01A_ANA_LO_STEP      ((UINT16) 6250)  /* 10Hz */
#define ALPS_TDAU8D01A_LO_STEP         ((UINT16) 6250)  /* 10Hz */
#define ALPS_TDAU8D01A_STR             "ALPS_TDAU8D01A 2010-01-01\n 2011-04-09\n v1.0"

#define ALPS_TDAU8D01A_IF_FREQUENCY    ((UINT16) 44000)   /* KHz */
#define ALPS_TDAU8D01A_IF_FREQUENCY_ANA    ((UINT16) 45750)   /* KHz */

#define CTRL_NUM 11



STATIC UINT8* pTDAU8D01AEqNormal[MOD_ANA_TYPE_END - MOD_ANA_TYPE_BEGIN] = {NULL};
STATIC UINT8* pTDAU8D01AEqWeak[MOD_ANA_TYPE_END - MOD_ANA_TYPE_BEGIN] = {NULL};
...
//
/** TDAU8D01A_TunerInit
 *  Tuner initialzation forTDAU8D01A.
 *  @param  pTCtx       Pointer of pi tuner driver context.
 *  @retval void
 */
//-----------------------------------------------------------------------------
VOID TDAU8D01A_TunerInit(
    ITUNER_CTX_T *pTCtx
    )
{
    ...
    psTunerCtx->I2cAddress = ALPS_TDAU8D01A_BASE;
    psTunerCtx->u4RF_Freq = 0;
    psTunerCtx->fgRFAGC = FALSE;
    /* Specific part: Only for this tuner model */
    psTunerCtx->u2IF_Freq = ALPS_TDAU8D01A_IF_FREQUENCY;
    psTunerCtx->u2IF_Freq_A=ALPS_TDAU8D01A_IF_FREQUENCY_ANA;
    ...
    psTunerSpecificCtx->m_aucPara[TUNER_PARA_TARGET_LEVEL_LSB] = 0x00;   
     ...
     psTunerSpecificCtx->u2LO_Step=ALPS_TDAU8D01A_ANA_LO_STEP;
    ...
    // init EQ script
	 // init EQ script
    pTDAU8D01AEqNormal[MOD_NTSC_M - MOD_ANA_TYPE_BEGIN]  = EQLGTDAU8D01AMN;
    pTDAU8D01AEqWeak[MOD_NTSC_M - MOD_ANA_TYPE_BEGIN]  = EQLGTDAU8D01AMNWeak;
	
    mcSHOW_USER_MSG(("Init " ALPS_TDAU8D01A_STR "\n"));
}


//----------------------------------------------------------------------------- 
/** TDAU8D01A_TunerSetFreq
 *  Set Tuner PLL forTDAU8D01A/TDAU8D01A to tune RF frequency.
 *  @param  pTCtx       Pointer of pi tuner driver context.
...
UINT8  u1Mod=param->Modulation;
  ...
  mcSHOW_DBG_MSG((ALPS_TDAU8D01A_STR"_TunerSetFreq %dKHz\n",param->Freq));
  if (u1Mod>= MOD_ANA_TYPE_BEGIN )
    ...
    if(!fgAnalogSignal){//digital  mode use digital LO step
      Ndivider = (UINT16) ((Lo * 100) / ALPS_TDAU8D01A_LO_STEP);
    }
    else{
      Ndivider = (UINT16) ((Lo * 100+ALPS_TDAU8D01A_ANA_LO_STEP/2) /ALPS_TDAU8D01A_ANA_LO_STEP);
    }
/* Assign Control word for specific tuner */
 ...
 /* Write to tuner device */
    ...
    {
        if(fgAnalogSignal){
            psTunerCtx->u4RF_Freq=(Ndivider*ALPS_TDAU8D01A_ANA_LO_STEP)/100-psTunerCtx->u2IF_Freq-1750;
        }
        else{
            psTunerCtx->u4RF_Freq=(Ndivider*ALPS_TDAU8D01A_LO_STEP)/100-psTunerCtx->u2IF_Freq;
        }
...
//----------------------------------------------------------------------------- 
/**  TDAU8D01A_TunerGetStatus
 *  Set Tuner PLL staus forTDAU8D01A/TDAU8D01A
 *  @param  pTCtx       Pointer of pi tuner driver context.
 *  @retval TRUE        PLL lock
 *  @retval FALSE       PLL unlock
 */
//-----------------------------------------------------------------------------
STATIC BOOL TDAU8D01A_TunerGetStatus(ITUNER_CTX_T *pTCtx)
{
    ITUNER_CTX_T *psTunerCtx = pTCtx;
 ...
 */
//-----------------------------------------------------------------------------
STATIC CHAR *TDAU8D01A_TunerGetVer(VOID)
{
    return (ALPS_TDAU8D01A_STR);
}
/***********************************************************************/
//----------------------------------------------------------------------------- 
 ...
 */
//-----------------------------------------------------------------------------
STATIC INT16 TDAU8D01A_SignalLevel_Handler(ITUNER_CTX_T * pTCtx,CMD_SIGLVL_PARAM_T * rCmd){
     INT16 ret=ITUNER_OK;
 ...
 */
//-----------------------------------------------------------------------------
INT16 TDAU8D01A_TunerOP(ITUNER_CTX_T * pTCtx, ITUNEROP_T eOperation, UINT8 SetVal,  VOID * pInOutVal){
         
         switch(eOperation){
              case itGetStatus:
                {
                    BOOL * Retval=(BOOL *)pInOutVal;
                    *Retval=TDAU8D01A_TunerGetStatus(pTCtx);
                }
                break;
              case itGetVer:
                 {
                    CHAR ** RetStr=(CHAR **)pInOutVal;
                    *RetStr=TDAU8D01A_TunerGetVer();
                 }
                 break;
              case itGetSignalLevel:
                 {
                   CMD_SIGLVL_PARAM_T * SigLvlParam=(CMD_SIGLVL_PARAM_T *)pInOutVal;
                   return TDAU8D01A_SignalLevel_Handler(pTCtx,SigLvlParam);
                 }
              case itGetEqScriptNormal: 
                  *(UINT8**)pInOutVal = pTDAU8D01AEqNormal[SetVal];
                  break;
    
              case itGetEqScriptWeak:
                  *(UINT8**)pInOutVal = pTDAU8D01AEqWeak[SetVal];
                  break;
