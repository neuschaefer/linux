...
#define ALPS_TDQU8_IF_FREQUENCY    ((UINT16) 44000)   /* KHz */
#define ALPS_TDQU8_LO_STEP         ((UINT16) 6250)  /* 10Hz */
#define ALPS_TDQU8_ANA_LO_STEP ((UINT16) 6250)  /* 10Hz */
#define ALPS_TDQU8_STR             "ALPS_TDQU8 2010-01-01\n 2011-04-09\n v1.0"
//-----------------------------------------------------------------------------
 ...
 */
//-----------------------------------------------------------------------------
INT16 TDQU8_TunerSetFreq( ITUNER_CTX_T *pTCtx,PARAM_SETFREQ_T* param)
{
ITUNER_CTX_T *psTunerCtx = pTCtx; 
UINT32  Lo;
UINT8   Nband;
...
UINT32 freq;
    ...
    mcSHOW_DBG_MSG(("ALPS TDQU8_TunerSetFreq %dKHz\n",param->Freq));
    ...
    if(!fgAnalogSignal){//digital  mode use digital LO step
      Ndivider = (UINT16) ((Lo * 100) / ALPS_TDQU8_LO_STEP);
    }
    else{
     Ndivider = (UINT16) ((Lo * 100) /ALPS_TDQU8_ANA_LO_STEP);
    };
...
/* LO settings */
    ...
    TunerS[2] = 0x9B;
    TunerS[3] = Nband;
    TunerS[4] = fgAnalogSignal?0xC6:0xF6;
...
/* Write to tuner device */
    ...
    {
        if(fgAnalogSignal){
            psTunerCtx->u4RF_Freq=(Ndivider*ALPS_TDQU8_ANA_LO_STEP)/100-psTunerCtx->u2IF_Freq-1750;
        }
        else{
            psTunerCtx->u4RF_Freq=(Ndivider*ALPS_TDQU8_LO_STEP)/100-psTunerCtx->u2IF_Freq;
        }
 ...
 */
//-----------------------------------------------------------------------------
STATIC BOOL TDQU8_TunerGetStatus(ITUNER_CTX_T *pTCtx)
{
UINT8 TunerS[4];
ITUNER_CTX_T *psTunerCtx = pTCtx;
 ...
 */
//-----------------------------------------------------------------------------
STATIC CHAR *TDQU8_TunerGetVer(VOID)
{
    return (ALPS_TDQU8_STR);
}
/***********************************************************************/
//-----------------------------------------------------------------------------
STATIC INT16 TDQU8_SignalLevel_Handler(ITUNER_CTX_T * pTCtx,CMD_SIGLVL_PARAM_T * rCmd){
     INT16 ret=ITUNER_OK;
 ...
 */
//-----------------------------------------------------------------------------
INT16 TDQU8_TunerOP(ITUNER_CTX_T * pTCtx, ITUNEROP_T eOperation, UINT8 SetVal,  VOID * pInOutVal){
         switch(eOperation){
              case itGetStatus:
                {
                    BOOL * Retval=(BOOL *)pInOutVal;
                    *Retval=TDQU8_TunerGetStatus(pTCtx);
                }
                break;
              case itGetVer:
                 {
                    CHAR ** RetStr=(CHAR **)pInOutVal;
                    *RetStr=TDQU8_TunerGetVer();
                 }
                 break;
              case itGetSignalLevel:
                 {
                   CMD_SIGLVL_PARAM_T * SigLvlParam=(CMD_SIGLVL_PARAM_T *)pInOutVal;
                   return TDQU8_SignalLevel_Handler(pTCtx,SigLvlParam);
                 }
              default:
