...
)
{
    ...
    err = ddTDA182I2GetInstance(tUnit, &pObj);
	 if (err != TM_OK)
	 	{
           mcSHOW_DBG_MSG(("ddTDA182I2GetInstance failed.\n"));	
	    }
	 else
	 	{
           mcSHOW_DBG_MSG(("ddTDA182I2GetInstance success.\n"));
    ...
    if(err == TM_OK)
    {
        ...
        err = ddTDA182I2Read(pObj, 0x06, 15);
		 if (err != TM_OK)
	 	{
          mcSHOW_DBG_MSG(("tmddTDA182I2SetPowerState:ddTDA182I2Read failed.\n"));	
	    }
		 else
		 {

           mcSHOW_DBG_MSG(("tmddTDA182I2SetPowerState:ddTDA182I2Read success.\n"));
		 }
        ...
        if(err == TM_OK)
        {
            ...
            case tmddTDA182I2_PowerStandby:
                ...
                err = tmddTDA182I2SetDigital_Clock_Mode(tUnit, 0);
				 if (err != TM_OK)
	 	          {
                     mcSHOW_USER_MSG(("tmddTDA182I2SetDigital_Clock_Mode(16 Mhz xtal clock) failed.\n"));	
	              }
                ...
                if(err == TM_OK && pObj->I2CMap.uBx02.bF.Major_rev == 1 && pObj->I2CMap.uBx02.bF.Minor_rev == 0)
                {
					...
					if (err != TM_OK)
	 	          {
                     mcSHOW_USER_MSG(("tmddTDA182I2SetAGCK_Mode(RSSI disable) failed.\n"));	
	              }
...
/*============================================================================*/
/* FUNCTION:    tmddTDA182I2WriteTuner_Register                              */
/*                                                                            */
/* DESCRIPTION: Write  the Tuner register value                       */
/*                                                                            */
/* RETURN:      TM_OK if no error                                             */
/*                                                                            */
/* NOTES:                                                                     */
...
/*============================================================================*/
tmErrorCode_t
tmddTDA182I2WriteTuner_Register
(
    tmUnitSelect_t  tUnit,  /* I: Unit number */
    UInt8           uSubAddress,    /* I: sub address */
    UInt8           uValue  /* I: Item value */
)
{
    ...
    if (err != TM_OK)
		  {
             mcSHOW_DBG_MSG(( "ddTDA182I2GetInstance(0x%08X) failed.\n",tUnit));
    ...
    if(err == TM_OK)
    {
         err = POBJ_SRVFUNC_SIO.Write (pObj->tUnitW, 1, &uSubAddress, 1, &uValue);
		 if (err != TM_OK)
		  {
             mcSHOW_DBG_MSG(( "tmddTDA182I2WriteTuner_Register(0x%08X) failed..\n",uSubAddress));
	...
	/*============================================================================*/
	/* FUNCTION:	tmddTDA182I2ReadTuner_Register							  */
	/*																			  */
	/* DESCRIPTION: Read  the tuner register value						  */
	/*																			  */
	/* RETURN:		TM_OK if no error											  */
	/*																			  */
	/* NOTES:																	  */
	...
	/*============================================================================*/
	tmErrorCode_t
		tmddTDA182I2ReadTuner_Register
	...
	)
	{
		...
		{
        /* Read data from the Tuner */
         err = POBJ_SRVFUNC_SIO.Read(pObj->tUnitW, 1, &uSubAddress, 1, puValue);
         if (err != TM_OK)
         	{
                mcSHOW_DBG_MSG(( "tmddTDA182I2ReadTuner_Register(0x%08X)failed..\n", uSubAddress));
            }
         if(err == TM_OK)
          {
            mcSHOW_DBG_MSG(( "tmddTDA182I2ReadTuner_Register(0x%08X)success!,its value is(0x%08X)..\n", uSubAddress,*puValue));
		   switch(pObj->versionForced)
           {
             default:
             case tmddTDA182I2_Version_Not_Forced:
