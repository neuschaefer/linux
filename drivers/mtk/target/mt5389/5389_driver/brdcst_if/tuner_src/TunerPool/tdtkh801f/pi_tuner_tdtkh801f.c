 ...
 * $Author: dtvbm11 $
 * $Date: 2012/04/27 $
 * $RCSfile: pi_tuner_TDTKH801F.c,v $
 * $Revision: #1 $
 *
 *---------------------------------------------------------------------------*/

/** @file pi_tuner_TDTKH801F.c
 *  Tuner control for SS TDTKH801F
 */
...
#include "eq_script_tdtkh801f.h"


//-----------------------------------------------------------------------------
// Constant definitions
//-----------------------------------------------------------------------------
#define LG_TDTKH801F_BASE            ((UINT16) 0xC2)
#define LG_TDTKH801F_IF_FREQUENCY_ANA    ((UINT16) 45750)   /* KHz */
#define LG_TDTKH801F_IF_FREQUENCY    ((UINT16) 44000)   /* KHz */
#define LG_TDTKH801F_LO_STEP         ((UINT16) 6250)  /* 10Hz */
#define LG_TDTKH801F_ANA_LO_STEP         ((UINT16) 6250)  /* 10Hz */
#define LG_TDTKH801F_STR             "LG_TDTKH801F 2010-01-01\n 2011-04-09\n v1.0"

//PLL lock check parameters
#define LG_TDTKH801F_PLL_POLL_INTERVAL      10 //ms
#define LG_TDTKH801F_PLL_POLL_TIMETOUT      100 //ms
#define LG_TDTKH801F_PLL_POLL_CNT           LG_TDTKH801F_PLL_POLL_TIMETOUT/LG_TDTKH801F_PLL_POLL_INTERVAL //counter
...
STATIC UINT8* pTDTKH801FEqNormal[MOD_ANA_TYPE_END - MOD_ANA_TYPE_BEGIN] = {NULL};
STATIC UINT8* pTDTKH801FEqWeak[MOD_ANA_TYPE_END - MOD_ANA_TYPE_BEGIN] = {NULL};

STATIC UINT8 u1AnaTop=5;
//
/** TDTKH801F_TunerInit
 *  Tuner initialzation forTDTKH801F.
 *  @param  pTCtx       Pointer of pi tuner driver context.
 *  @retval void
 */
//-----------------------------------------------------------------------------
VOID TDTKH801F_TunerInit(
    ITUNER_CTX_T *pTCtx
    )
{
    ...
    psTunerCtx->I2cAddress = LG_TDTKH801F_BASE;
    psTunerCtx->u4RF_Freq = 0;
    psTunerCtx->fgRFAGC = FALSE;
    /* Specific part: Only for this tuner model */
    psTunerCtx->u2IF_Freq = LG_TDTKH801F_IF_FREQUENCY;
    psTunerCtx->u2IF_Freq_A = LG_TDTKH801F_IF_FREQUENCY_ANA;
    ...
    // init EQ script
    pTDTKH801FEqNormal[MOD_NTSC_M - MOD_ANA_TYPE_BEGIN]  = EQLGTDTKH801FMN;
    pTDTKH801FEqWeak[MOD_NTSC_M - MOD_ANA_TYPE_BEGIN]  = EQLGTDTKH801FMNWeak;

    //pTDTKH801FEqNormal[MOD_NTSC_M_54M_44M]  = EQLGTDTKH801FMN;
    //pTDTKH801FEqNormal[MOD_NTSC_M_54M_36M]  = EQLGTDTKH801FMN;
   psTunerSpecificCtx->u2LO_Step=LG_TDTKH801F_ANA_LO_STEP;
   
    mcSHOW_USER_MSG(("Init " LG_TDTKH801F_STR "\n"));
}

//----------------------------------------------------------------------------- 
/**  TDTKH801F_TunerGetStatus
 *  Set Tuner PLL staus forTDTKH801F/TDTKH801F
 *  @param  pTCtx       Pointer of pi tuner driver context.
 *  @retval TRUE        PLL lock
 *  @retval FALSE       PLL unlock
 */
//-----------------------------------------------------------------------------
STATIC BOOL TDTKH801F_TunerGetStatus(ITUNER_CTX_T *pTCtx)
{
    ITUNER_CTX_T *psTunerCtx = pTCtx;
...
//----------------------------------------------------------------------------- 
/** TDTKH801F_TunerSetFreq
 *  Set Tuner PLL forTDTKH801F/TDTKH801F to tune RF frequency.
 *  @param  pTCtx       Pointer of pi tuner driver context.
...
u1Mod=param->Modulation;
   ...
   mcSHOW_DBG_MSG(("LG TDTKH801F_TunerSetFreq %dKHz\n",param->Freq));
  if ( u1Mod>= MOD_ANA_TYPE_BEGIN )
    ...
    if(!fgAnalogSignal){//digital  mode use digital LO step
      Ndivider = (UINT16) ((Lo * 100) / LG_TDTKH801F_LO_STEP);
    }
    else{
     Ndivider = (UINT16) ((Lo * 100) /LG_TDTKH801F_ANA_LO_STEP);
    }
...
/* LO settings */
 ...
 /* Write to tuner device */
	...
	for(ii=0;ii<LG_TDTKH801F_PLL_POLL_CNT;ii++)
	{
	  mcDELAY_MS(LG_TDTKH801F_PLL_POLL_INTERVAL);
	  if(TDTKH801F_TunerGetStatus(psTunerCtx))break;
	}
	
	if(ii<LG_TDTKH801F_PLL_POLL_CNT)
	{
	...
	else
	{
	 mcSHOW_HW_MSG(("PLL unlock !(%d)",ii));
        ...
        if(fgAnalogSignal){
            psTunerCtx->u4RF_Freq=(Ndivider*LG_TDTKH801F_ANA_LO_STEP)/100-psTunerCtx->u2IF_Freq_A;
        }
        else{
            psTunerCtx->u4RF_Freq=(Ndivider*LG_TDTKH801F_LO_STEP)/100-psTunerCtx->u2IF_Freq;
        }
 ...
 */
//-----------------------------------------------------------------------------
STATIC CHAR *TDTKH801F_TunerGetVer(VOID)
{
    return (LG_TDTKH801F_STR);
}
/***********************************************************************/
//----------------------------------------------------------------------------- 
 ...
 */
//-----------------------------------------------------------------------------
STATIC INT16 TDTKH801F_SignalLevel_Handler(ITUNER_CTX_T * pTCtx,CMD_SIGLVL_PARAM_T * rCmd){
     INT16 ret=ITUNER_OK;
 ...
 */
//-----------------------------------------------------------------------------
INT16 TDTKH801F_TunerOP(ITUNER_CTX_T * pTCtx, ITUNEROP_T eOperation, UINT8 SetVal,  VOID * pInOutVal){
         
    switch(eOperation){
    case itGetStatus:
        {
        BOOL * Retval=(BOOL *)pInOutVal;
        *Retval=TDTKH801F_TunerGetStatus(pTCtx);
        }
        break;
    case itGetVer:
        {
        CHAR ** RetStr=(CHAR **)pInOutVal;
        *RetStr=TDTKH801F_TunerGetVer();
        }
        break;
    case itGetSignalLevel:
        {
        CMD_SIGLVL_PARAM_T * SigLvlParam=(CMD_SIGLVL_PARAM_T *)pInOutVal;
        return TDTKH801F_SignalLevel_Handler(pTCtx,SigLvlParam);
        }

    case itGetEqScriptNormal: 
        *(UINT8**)pInOutVal = pTDTKH801FEqNormal[SetVal];
        break;

    case itGetEqScriptWeak: 
        *(UINT8**)pInOutVal = pTDTKH801FEqWeak[SetVal];
        break;
