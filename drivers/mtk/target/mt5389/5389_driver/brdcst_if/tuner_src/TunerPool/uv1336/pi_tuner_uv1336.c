 ...
 * $Author: dtvbm11 $
 * $Date: 2012/04/27 $
 * $RCSfile: pi_tuner_uv1336.c,v $
 * $Revision: #1 $
 *
 *---------------------------------------------------------------------------*/

/** @file pi_tuner_uv1336.c
 *  Tuner control for NXP UV1336.
 */
...
//-----------------------------------------------------------------------------
#define NXP_UV1336_BASE          ((UINT16) 0xC0)
#define NXP_UV1336_IF_FREQUENCY  ((UINT16) 44)   /* MHz */
#define NXP_UV1336_LO_STEP       ((UINT16) 625)  /* 100Hz */
#define NXP_UV1336_STR           "NXP_UV1336 2010-01-01\n 2011-04-09\n v1.0"
...
//----------------------------------------------------------------------------- 
/** UV1336_TunerInit
 *  Tuner initialzation for UV1336.
 *  @param  pTCtx       Pointer of pi tuner driver context.
 *  @retval void
 */
//-----------------------------------------------------------------------------
void UV1336_TunerInit(
    ITUNER_CTX_T *pTCtx,
    UINT8 i2cAddr
    )
{
    RF_TUNER_CTX_T *psTunerCtx = &pTCtx->dtdTunerCtx;
    ...
    psTunerCtx->I2cAddress = NXP_UV1336_BASE;
    psTunerCtx->u1IF_I2cAddr = i2cAddr;
    psTunerCtx->u4RF_Freq = 0;

    /* Specific part: Only for this tuner model */
//    psTunerCtx->u1TunerType = MT5112_TUNER_NXP_UV1336;
    psTunerCtx->u1TunerType = 16;
    psTunerCtx->u2IF_Freq = NXP_UV1336_IF_FREQUENCY;
    psTunerCtx->u2LO_Step = NXP_UV1336_LO_STEP;
    ...
    mcSHOW_DBG_MSG(("Init " NXP_UV1336_STR "\n"));
}


//----------------------------------------------------------------------------- 
/** UV1336_TunerSetFreq
 *  Set Tuner PLL for UV1336 to tune RF frequency.
 *  @param  pTCtx       Pointer of pi tuner driver context.
    ...
    )
{
    ...
    BOOL  fgBigWantThanCur;

    mcSHOW_DBG_MSG(("NXP UV1336_SetFreq\n"));
	
//+++++++++++++++++++++++++
    if (freq>= psTunerCtx->u4RF_Freq)
	fgBigWantThanCur = TRUE;
   else
   	fgBigWantThanCur = FALSE;
//--------------------------------
 ...
 /* Assign Control word for specific tuner */
   if (freq < 52 || freq > 865)         //for ROW spec
    {
   ...
   if(fgBigWantThanCur)
    ...
    {
        TunerS[0] = (UINT8) CB;               // Must be set first for init
        TunerS[1] = (UINT8) Nband;
...
/* Write to tuner device */
    if (ICtrlBus_I2cTunerWrite(psTunerCtx->I2cAddress, TunerS, TunerDataCnt))
    {
        mcSHOW_HW_MSG(("I2cAddress = %02X \n",psTunerCtx->I2cAddress));
    ...
    )
{
    ...
    if (ICtrlBus_I2cTunerRead(psTunerCtx->I2cAddress, TunerS, 1))
    {
 ...
 */
//-----------------------------------------------------------------------------
char *UV1336_TunerGetVer(
    ITUNER_CTX_T *pTCtx
    )
{
    return (NXP_UV1336_STR);
}
/***********************************************************************/
