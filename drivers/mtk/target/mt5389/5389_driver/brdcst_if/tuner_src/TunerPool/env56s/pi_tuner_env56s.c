 ...
 * $Author: dtvbm11 $
 * $Date: 2012/04/27 $
 * $RCSfile: pi_tuner_ENV56S.c,v $
 * $Revision: #1 $
 *
 *---------------------------------------------------------------------------*/

/** @file pi_tuner_ENV56S.c
 *  Tuner control for SS ENV56S
 */
...
#include "eq_script_env56s_6595.h"
#else
#include "eq_script_env56s_5388.h"
#endif

//-----------------------------------------------------------------------------
// Constant definitions
//-----------------------------------------------------------------------------
#define PANASONIC_ENV56S_BASE            ((UINT16) 0xC2)
#define PANASONIC_ENV56S_IF_FREQUENCY    ((UINT16) 44000)   /* KHz */
#define PANASONIC_ENV56S_IF_FREQUENCY_ANA    ((UINT16) 45750)   /* KHz */
#define PANASONIC_ENV56S_LO_STEP         ((UINT16) 6250)  /* 10Hz */
#define PANASONIC_ENV56S_ANA_LO_STEP         ((UINT16) 6250)  /* 10Hz */
#define PANASONIC_ENV56S_STR             "PANASONIC_ENV56S 2010-01-01\n 2011-04-06\n v1.0"
//PLL lock check parameters
#define PANASONIC_ENV56S_PLL_POLL_INTERVAL      10 //ms
#define PANASONIC_ENV56S_PLL_POLL_TIMETOUT      100 //ms Max time  according to tuner spec
#define PANASONIC_ENV56S_PLL_POLL_CNT           PANASONIC_ENV56S_PLL_POLL_TIMETOUT/PANASONIC_ENV56S_PLL_POLL_INTERVAL //counter
//-----------------------------------------------------------------------------
...
STATIC INT16 IFCTRL_VHFL[] =  {1141,  625,  183 , -222 , -594,  -949,  -1341, -1392, -90};//From -90dBm
STATIC INT16 IFCTRL_VHFH[] =  {1675,  1065,  545,   105 , -286,  -656,  -1028, -1396, -1400, -90};//From -90dBm
STATIC INT16 IFCTRL_UHF [] =  {1795, 1177, 641, 189, -216, -595, -959, -1348, -1407, -90};//From -90dBm


STATIC UINT8* pENV56SEqNormal[MOD_ANA_TYPE_END - MOD_ANA_TYPE_BEGIN] = {NULL};

//
/** ENV56S_TunerInit
 *  Tuner initialzation forENV56S.
 *  @param  pTCtx       Pointer of pi tuner driver context.
 *  @retval void
 */
//-----------------------------------------------------------------------------
VOID ENV56S_TunerInit(
    ITUNER_CTX_T *pTCtx
    )
{
    ...
    psTunerCtx->I2cAddress = PANASONIC_ENV56S_BASE;
    psTunerCtx->u4RF_Freq = 0;
    psTunerCtx->fgRFAGC = FALSE;
    /* Specific part: Only for this tuner model */
    psTunerCtx->u2IF_Freq = PANASONIC_ENV56S_IF_FREQUENCY;
    psTunerCtx->u2IF_Freq_A =PANASONIC_ENV56S_IF_FREQUENCY_ANA;
    ...
    psTunerSpecificCtx->m_aucPara[TUNER_PARA_TARGET_LEVEL_LSB] = 0x00;  
     psTunerSpecificCtx->u2LO_Step=PANASONIC_ENV56S_ANA_LO_STEP;
    // AGC para
    ...
    // init EQ script
   pENV56SEqNormal[MOD_NTSC_M - MOD_ANA_TYPE_BEGIN]  = EQENV56SMN;
    //pENV56SEqWeak[MOD_NTSC_M - MOD_ANA_TYPE_BEGIN]  = EQENV56SMNWeak;
    //pENV56SEqNormal[MOD_NTSC_M_54M_44M]  = EQPANASONICENV56SMN;
    //pENV56SEqNormal[MOD_NTSC_M_54M_36M]  = EQPANASONICENV56SMN;

   
    mcSHOW_USER_MSG(("Init " PANASONIC_ENV56S_STR "\n"));
}

//----------------------------------------------------------------------------- 
/**  ENV56S_TunerGetStatus
 *  Set Tuner PLL staus forENV56S/ENV56S
 *  @param  pTCtx       Pointer of pi tuner driver context.
 *  @retval TRUE        PLL lock
 *  @retval FALSE       PLL unlock
 */
//-----------------------------------------------------------------------------
STATIC BOOL ENV56S_TunerGetStatus(ITUNER_CTX_T *pTCtx)
{
    ITUNER_CTX_T *psTunerCtx = pTCtx;
...
//----------------------------------------------------------------------------- 
/** ENV56S_TunerSetFreq
 *  Set Tuner PLL forENV56S/ENV56S to tune RF frequency.
 *  @param  pTCtx       Pointer of pi tuner driver context.
...
u1Mod=param->Modulation;
   ...
   mcSHOW_DBG_MSG(("PANASONIC ENV56S_TunerSetFreq %dKHz\n",param->Freq));
  if ( u1Mod>= MOD_ANA_TYPE_BEGIN )
    ...
    if(!fgAnalogSignal){//digital  mode use digital LO step
      Ndivider = (UINT16) ((Lo * 100) / PANASONIC_ENV56S_LO_STEP);
    }
    else{
     Ndivider = (UINT16) ((Lo * 100) /PANASONIC_ENV56S_ANA_LO_STEP);
    }
...
/* LO settings */
 ...
 /* Write to tuner device */
	...
	for(ii=0;ii<PANASONIC_ENV56S_PLL_POLL_CNT;ii++)
	{
	 mcDELAY_MS(PANASONIC_ENV56S_PLL_POLL_INTERVAL);
	 if(ENV56S_TunerGetStatus(psTunerCtx))break;
	}
	
	if(ii<PANASONIC_ENV56S_PLL_POLL_CNT)
	{
	...
	else
	{
	 mcSHOW_HW_MSG(("PLL unlock !(%d)",ii));
        ...
        if(fgAnalogSignal){
            psTunerCtx->u4RF_Freq=(Ndivider*PANASONIC_ENV56S_ANA_LO_STEP)/100-psTunerCtx->u2IF_Freq-1750;
        }
        else{
            psTunerCtx->u4RF_Freq=(Ndivider*PANASONIC_ENV56S_LO_STEP)/100-psTunerCtx->u2IF_Freq;
        }
 ...
 */
//-----------------------------------------------------------------------------
STATIC CHAR *ENV56S_TunerGetVer(VOID)
{
    return (PANASONIC_ENV56S_STR);
}
/***********************************************************************/
//----------------------------------------------------------------------------- 
 ...
 */
//-----------------------------------------------------------------------------
STATIC INT16 ENV56S_SignalLevel_Handler(ITUNER_CTX_T * pTCtx,CMD_SIGLVL_PARAM_T * rCmd){
     INT16 ret=ITUNER_OK;
     ...
     switch(rCmd->Id){
            ...
            {
             *Retval = 634 * (i2CtrlWord-625) / 2017 + 1481;
            }
	        else if (Freq <=465000 ) // VHF-High band
            {
            *Retval = 782 * (i2CtrlWord -1065) /2465 + 1625;
            }
  	        else // UHF band
            {
            *Retval= 819* (i2CtrlWord - 1177) / 2584 + 1661;
            }
            }
            ...
            case IFAGCVOL2CTRL://Convert IfAGC Voltage to Ctrl Word
            {
               ...
               if(Freq<=165000)
               	{
               		*Retval=2017*(i2Vol-1481)/634+625;
               	}
               	else if(Freq<=465000)
               	{
               	  *Retval=2465*(i2Vol-1625)/782+1065;	
               	}
               	else
               	{
               	 *Retval=2584*(i2Vol-1661)/819+1177;	
               	}
            }
 ...
 */
//-----------------------------------------------------------------------------
INT16 ENV56S_TunerOP(ITUNER_CTX_T * pTCtx, ITUNEROP_T eOperation, UINT8 SetVal,  VOID * pInOutVal){
         
    switch(eOperation){
    case itGetStatus:
        {
        BOOL * Retval=(BOOL *)pInOutVal;
        *Retval=ENV56S_TunerGetStatus(pTCtx);
        }
        break;
    case itGetVer:
        {
        CHAR ** RetStr=(CHAR **)pInOutVal;
        *RetStr=ENV56S_TunerGetVer();
        }
        break;
    case itGetSignalLevel:
        {
        CMD_SIGLVL_PARAM_T * SigLvlParam=(CMD_SIGLVL_PARAM_T *)pInOutVal;
        return ENV56S_SignalLevel_Handler(pTCtx,SigLvlParam);
        }

    case itGetEqScriptNormal: 
        *(UINT8**)pInOutVal = pENV56SEqNormal[SetVal];
        break;
