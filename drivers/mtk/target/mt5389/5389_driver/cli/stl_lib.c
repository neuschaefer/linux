...
//20100524, Daniel, for minus decimal input support
INT32 i4StrToInt(const CHAR* pszStr)
{
    UINT32 u4Len;
    ...
    if (u4Len > 2)
    {
        if ((pszStr[0] == '0') && (pszStr[1] == 'x'))
        {
            return (INT32)StrToHex(&pszStr[2], u4Len - 2);
        }
    }

    if (pszStr[0] == '-')
    {
        return -((INT32)StrToDec(&pszStr[1], u4Len-1));
    }
    else
    {
        return (INT32)StrToDec(pszStr, u4Len);    
    }
}
