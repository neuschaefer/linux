...
static BOOL b_boot_rec_once_1st_frame = FALSE;
static BOOL b_boot_rec_once_1st_i = FALSE;

static MPV_ES_INFO_T _arMpvEsInfo[MPV_MAX_ES];
static UCHAR* _apucMatrix[MPV_MAX_ES];   // hold quantization table ptr
static MPV_CC_T _arMpvCc[MPV_MAX_ES];
static MPV_EXT_INFO_T _rExtInfo;
static MPV_INFO_T _rMpvInfo;

static VDEC_PES_INFO_T _arMpvPesInfo[MPV_MAX_ES]; // this is for cli
...
//    HANDLE_T hThread;
    ...
    if (!_fgMpvInitiated)
    {
        ...
        {
            prMpvEsInfo = _arMpvEsInfo;
            prMpvEsInfo += (UINT32)ucEsId;
            x_memset((void *)prMpvEsInfo, 0, sizeof(MPV_ES_INFO_T));	//init state = VDEC_ST_STOP
            _apucMatrix[ucEsId] = (UCHAR*)x_mem_alloc(MPV_MATRIX_RAW_SIZE*sizeof(UCHAR));
        }
...
/*
        ...
        {
            CHAR szBuf[16];
            x_snprintf(szBuf, sizeof(szBuf), "MPV%d", ucEsId);
...
void MPV_Play(UCHAR ucEsId)
{
    ...
    _arMpvPesInfo[ucEsId].ucEsId = ucEsId;
    _arMpvPesInfo[ucEsId].ucMpvId = VDEC_MAX_VLD;

    VERIFY(VDEC_SendEs(&_arMpvPesInfo[ucEsId]) == 1);
    */
}
...
void MPV_PlayCc(UCHAR ucEsId)
{
    ...
    {
        x_snprintf(szBuf, sizeof(szBuf), "CC-Q%d\n", ucEsId);
...
UCHAR MPV_CallBackEsm(const MPV_PES_INFO_T *prPesInfo)
{
    ...
    if(!prPesInfo)
    {
        LOG(3, "prPesInfo is null\n");
        return 1;
    }
    rVdecPesInfo.fgSeqHdr = prPesInfo->fgSeqHdr;
    rVdecPesInfo.fgGop = prPesInfo->fgGop;
    rVdecPesInfo.fgDtsValid = prPesInfo->fgDtsValid;
    rVdecPesInfo.ucMpvId = prPesInfo->ucMpvId;
    rVdecPesInfo.ucEsId = prPesInfo->ucEsId;
    rVdecPesInfo.ucPicType = prPesInfo->ucPicType;
    rVdecPesInfo.u4FifoStart = prPesInfo->u4FifoStart;
    rVdecPesInfo.u4FifoEnd = prPesInfo->u4FifoEnd;
    rVdecPesInfo.u4VldReadPtr = prPesInfo->u4VldReadPtr;
    rVdecPesInfo.u4PTS = prPesInfo->u4PTS;
    rVdecPesInfo.u4DTS = prPesInfo->u4DTS;
...
UINT32 MPV_GetDecMode(UCHAR ucEsId)
{
    ...
    u4DecMode = (UINT32)prVdecEsInfoKeep->eCurState;
...
void MPV_SetDecMode(UCHAR ucEsId, UINT32 u4DecMode)
{
    ...
    prVdecEsInfoKeep->eCurState = (ENUM_VDEC_STATE_T)u4DecMode;
...
INT32 MPV_AllocIFrameBuf(UCHAR ucEsId, MPV_DEC_I_FRAME_BUFF_T* prIFrameBuff)
{
    ...
    if (prEsInfo->pucIFrameBuf == NULL)
    {
        ...
        #ifdef __KERNEL__
        {
    	    ...
    	    {
        	    prEsInfo->pucIFrameBuf = (UCHAR*)VIRTUAL(PAGE_ALIGN(prFbmPool->u4Addr));
    	    }
        	else
    	    {
...
INT32 MPV_SetIFrameBuf(UCHAR ucEsId, const MPV_DEC_I_FRAME_BUFF_T* prIFrameBuff)
{
    ...
    prVdecEsInfoKeep = _VDEC_GetEsInfoKeep(ucEsId);
    _arMpvPesInfo[ucEsId].fgSeqHdr = TRUE;
    _arMpvPesInfo[ucEsId].fgNonBuffer = TRUE;
    _arMpvPesInfo[ucEsId].ucEsId = ucEsId;
    _arMpvPesInfo[ucEsId].ucMpvId = VLD0;
    _arMpvPesInfo[ucEsId].u4FifoStart = PHYSICAL((UINT32)(prIFrameBuff->pucIFrameBufAddr));
    _arMpvPesInfo[ucEsId].u4FifoEnd = PHYSICAL(MPV_ALIGN_MASK(((UINT32)(prIFrameBuff->pucIFrameBufAddr) +
                              (prIFrameBuff->u4Size - 1)), MPV_ESFIFO_ALIGN));
    #if defined(CC_MT5365) || defined(CC_MT5395) || defined(VDEC_IS_POST_MT5368)
    HalFlushInvalidateDCacheMultipleLine(VIRTUAL(_arMpvPesInfo[ucEsId].u4FifoStart), (_arMpvPesInfo[ucEsId].u4FifoEnd - _arMpvPesInfo[ucEsId].u4FifoStart));
    #else
    HalFlushInvalidateDCache();
    #endif
    if(prVdecEsInfoKeep->eCurFMT == VDEC_FMT_H264)
    {
        if((*(UCHAR*)(VIRTUAL((UINT32)prIFrameBuff->pucIFrameBufAddr) + 2)) == 1)  // 3 byte start code
        {
            _arMpvPesInfo[ucEsId].ucPicType = (*(UCHAR*)VIRTUAL((UINT32)prIFrameBuff->pucIFrameBufAddr + 3));
            _arMpvPesInfo[ucEsId].u4VldReadPtr = PHYSICAL((UINT32)(prIFrameBuff->pucIFrameBufAddr) + 4);
        }
        else  // 4 byte start code
        {
            _arMpvPesInfo[ucEsId].ucPicType = (*(UCHAR*)VIRTUAL((UINT32)prIFrameBuff->pucIFrameBufAddr + 4));
            _arMpvPesInfo[ucEsId].u4VldReadPtr = PHYSICAL((UINT32)(prIFrameBuff->pucIFrameBufAddr) + 5);
        }
    }
    else
    {
        _arMpvPesInfo[ucEsId].ucPicType= I_TYPE;
        _arMpvPesInfo[ucEsId].u4VldReadPtr = PHYSICAL((UINT32)(prIFrameBuff->pucIFrameBufAddr));
    }
    prVdecEsInfo = _VDEC_GetEsInfo(ucEsId);
    prVdecEsInfo->fgIFrameDec = FALSE;

    if (VDEC_SendEs(&_arMpvPesInfo[ucEsId]) != 1)
    {
        return (MPV_DRV_SET_FAILED);
    }
...
void MPV_FlushEsmQ(UCHAR ucEsId, BOOL fgAbortDecode)
{
    ...
    if((fgAbortDecode) && (prVdecEsInfoKeep->eCurFMT == VDEC_FMT_MPV))
    {
...
void MPV_UpdateVldWptr(UCHAR ucEsId, UINT32 u4VldWptr)
{
    ...
    //new, multi MMP only
    //_MPV_VldSetWptr(prVdecEsInfo->u4VldId, prEsInfo->u4VldWptr);
...
void MPV_SetRR(UCHAR ucEsId, UINT32 u4Ratio)
{
    ...
    VERIFY(VDEC_SendEs(&_arMpvPesInfo[ucEsId]));
...
void MPV_QueryStatus(void)
{
    ...
    for (ucEsId = 0; ucEsId < MPV_MAX_ES; ucEsId++)
    {
        ...
        if (prVdecEsInfoKeep->eCurState != VDEC_ST_IDLE)
    	 {
            LOG(1, "DecMode(0x%x),RR(%d, %d),Mode(FullB:%d,HalfB:%d,TRACK:%d),Dbk(%d),DbkEng(%d), DBKThrsd(%d), PartialPes(%d), (%d,%d)\n",
                prEsInfo->u4DecMode, IS_DECFLAG_SET(prVdecEsInfo, DEC_FLAG_REDUCE_REF), prEsInfo->u4RRatio,
                IS_DECFLAG_SET(prVdecEsInfo, DEC_FLAG_RACING_FULLB),
                IS_DECFLAG_SET(prVdecEsInfo, DEC_FLAG_RACING_HALFB),
                IS_DECFLAG_SET(prVdecEsInfo, DEC_FLAG_TRACKING_MODE),
...
//    LOG(1, "MPV_IsLock (%d) (%d)\n", prEsInfo->u4LastGetLockRxPacket, prEsInfo->u4LastGetLockDecOkPacket);
        ...
        (prMpvCounter->u4RetrieveNs == prEsInfo->u4LastGetLockRxPacket))
    {
        ...
        x_memset(&rCurrentTime, 0, sizeof(HAL_TIME_T));
...
void _MPV_ClearEsInfo(UCHAR ucEsId)
{
	...
	//if(prVdecInfo->eCurFMT == VDEC_FMT_MPV)
	{
        ...
        prMpvEsInfo->pucMatrix =  _apucMatrix[ucEsId];
       
	    prMpvEsInfo->pu4IntraMatrix = (UINT32*)(((MPV_READ_ADDR_ALIGN - 1)
	        + (UINT32) prMpvEsInfo->pucMatrix) & (~(MPV_READ_ADDR_ALIGN - 1)));
...
BOOL _MPV_EsFlwCtrl(UCHAR ucEsId, UCHAR* pucMpvId, INT8 i1EsBufStatus, UCHAR ucSkipMode)
{
    ...
    if (ucPicType == B_TYPE)
    {
        if (prVdecEsInfo->ucFbgId != FBM_FBG_ID_UNKNOWN)
        {
            ...
            {
                if (FBM_ChkFrameBufferFlag(prVdecEsInfo->ucFbgId , FBM_FLAG_RACING_FULL_B))
                {
                    _MPV_SkipPic(*pucMpvId, ucEsId);
                    *pucMpvId = VDEC_MAX_VLD;
                    return TRUE;
                }
                else if (FBM_ChkFrameBufferFlag(prVdecEsInfo->ucFbgId , FBM_FLAG_TRACKING_MODE_ENABLE))
                {
                    _MPV_SkipPic(*pucMpvId, ucEsId);
                    *pucMpvId = VDEC_MAX_VLD;
                    return TRUE;
                }
                else if (FBM_ChkFrameBufferFlag(prVdecEsInfo->ucFbgId, FBM_FLAG_RACING_HALF_B))
                {
                    ASSERT(0);
                }
            }
                ...
                if(!prVdecEsInfo->fgMMPlayback)
                {
                    ...
                    if (prVdecEsInfo->u2Count > prVdecEsInfo->u2UdfNs)
                    {
                        LOG(3, "Q S VLD EsFlwCtrl(%d) Es(%d) G(%d) Q(%d) RP(0x%08x)\n",
                            *pucMpvId, ucEsId, prVdecEsInfo->ucFbgId, prVdecEsInfo->u2Count,
                            prVdecEsInfo->u4VldReadPtr);
                            ...
                            if (IS_DECFLAG_SET(prVdecEsInfo, DEC_FLAG_DBK) && (!prVdecEsInfo->fgRefNoDbk))
                            {
                                if (prVdecEsInfo->ucDbkRefId == FBM_FB_ID_UNKNOWN) //change normal mode to DBK mode
                                {
                                        ...
                                        (fgNotDisplay))
                                    {
										...
										LOG(1,"I/P Frame Not To Display\n");
                                    }
                                    else
                                    {
                                        ...
                                        _MPV_CheckVenc(ucEsId, ucBRefFbId);

                                        FBM_SetFrameBufferStatus(prVdecEsInfo->ucFbgId,
                                            ucBRefFbId, FBM_FB_STATUS_DISPLAYQ);
										LOG(1,"I/P Frame To Q, PTS = %x\n",u4PTS);
                                ...
                                else
                                {
                                    ...
                                    {
                                        #ifdef CC_REALD_3D_SUPPORT
                                        _VDEC_Check2D3DSwitch(ucEsId, prVdecEsInfo->ucDbkRefId);
                                        #endif

                                        _VDEC_JobBeforePutDispQ(ucEsId, prVdecEsInfo->ucDbkRefId);

                                        _MPV_CheckVenc(ucEsId, prVdecEsInfo->ucDbkRefId);
               ...
               (!prVdecEsInfo->fgMMPlayback))))
        {
            ...
            if (!prVdecEsInfo->ucSequenceHdrFound)
            {
                if (!prPesInfo->fgSeqHdr)
                {
                    ...
                    if(prMpvCounter->u4PsrDropNs == 0)
                    {
                        if (!b_boot_rec_once_1st_frame)
                        {
                            x_os_drv_set_timestamp("REC_1st_Frame");
                            b_boot_rec_once_1st_frame = TRUE;
                        }
                        #ifdef TIME_MEASUREMENT
                        TMS_DIFF_EX(TMS_FLAG_CHG_CHL, TMS_CHL_CHE_TIME, "REC_1st_Frame");
                        #endif
                    }
                ...
                {
                    {
                        ...
                        {
                            static CHAR szBuf[25];
                            x_snprintf(szBuf, sizeof(szBuf), "REC_1st_I(%3d)", prMpvCounter->u4PsrDropNs);
                            if (!b_boot_rec_once_1st_i)
                            {
                                x_os_drv_set_timestamp(szBuf);
                                b_boot_rec_once_1st_i = TRUE;
                            }
                            #ifdef TIME_MEASUREMENT
     ...
     HANDLE_T hMsgQueue, const void* pvPesInfo)
{
    ...
    prVdecEsInfoKeep = _VDEC_GetEsInfoKeep(ucEsmQId);
    ...
    while (u4Finish == 0)
    {
                ...
                &hMsgQueue, 1, X_MSGQ_OPTION_NOWAIT) == OSR_OK)
        {
            ...
            // when vdec is stop and Q is full,
            // we stop the search of B frame.
            // If we don't do this, we just looping in not sending the current
            // frame.
            if((prVdecEsInfoKeep->eCurState == VDEC_ST_STOP) &&
               (prPesInQ[u4Idx].ucMpvId < VDEC_MAX_ES))
            {
                LOG(3, "Q %d (QF)\n", prPesInfo->ucEsId);
                ucValid = 1;
                u4Finish = 1;
...
UINT32 _MPV_CalcPts(UCHAR ucEsId, UCHAR ucFbId)
{
    ...
    if(prFbmPicHdr->u4PTS == 0)
    {
        if(prVdecEsInfo->u4PTSLast != 0)
        {
            ...
            prVdecEsInfo->u4PTSLastNoneZero = prVdecEsInfo->u4PTSLast;
        ...
        {
            prFbmPicHdr->u4PTS = prVdecEsInfo->u4PTSLastNoneZero;
...
#ifndef LOCK_TIMEOUT
static HANDLE_T _hIframeTimer = 0;
static HANDLE_T _hIframeMsgQ = 0;
#else
...
#ifndef LOCK_TIMEOUT
static void _IFrameFlushMsgQ(VOID)
{
    UINT16 u2Cnt;
    UINT16 u2MsgQIdx;
    UINT32 u4DecFin;
    ...
    */
    ...
    while (x_msg_q_receive(&u2MsgQIdx, &u4DecFin, &zMsgSize,
        &_hIframeMsgQ, 1, X_MSGQ_OPTION_NOWAIT) == OSR_OK)
    {
    }

    VERIFY(x_msg_q_num_msgs(_hIframeMsgQ, &u2Cnt) == OSR_OK);
    ASSERT(u2Cnt== 0);
}

static void _IFrameTimerTimeout(HANDLE_T  pt_tm_handle, VOID *pv_tag)
{
    //UCHAR ucMpvId;
    UINT32 u4DecFin;
    ...
    */
    ...
    u4DecFin = LOGOVDEC_TIMEOUT;
    VERIFY(x_msg_q_send(_hIframeMsgQ, (void *)&u4DecFin, sizeof(UINT32), 255) == OSR_OK);
...
#ifndef LOCK_TIMEOUT
        u4DecFin = LOGOVDEC_FINISH;
        VERIFY(x_msg_q_send(_hIframeMsgQ, (void *)&u4DecFin, sizeof(UINT32), 1) == OSR_OK);
#else
...
UINT32 MPV_OsdDisplay(UINT32 u4OsdPlane, UINT32 u4Scaler, UINT32 u4ColorMode, UINT32 u4BmpAddr, UINT32 u4Width, UINT32 u4Height)
{
    ...
    // flush the data, temp solotion, OSD will modify it later.
    #ifdef VDEC_IS_POST_MT5368
    u4Addr = VIRTUAL(OSD_PLA_GetFirstRegionAddress(u4OsdPlane));
    if(u4Addr)
    {        
        HalFlushDCacheMultipleLine(u4Addr, 32);
    }
    #endif
    
    // if display is not 0
    if(DRVCUST_OptGet(eDrvLogoVdecTimeDisplay))
    {
        INT32 j;

        // Start Delay
        j = DRVCUST_OptGet(eDrvLogoVdecTimeStartDelay);
        if(j > 5000)
        {
            // start delay time too long
            LOG(0, "FadeIn(%d) > 5000 -> 5000\n", j);
            j = 5000;
        }
        x_thread_delay(j);

        // Fade In
        j = DRVCUST_OptGet(eDrvLogoVdecTimeFadeIn);
        if(j > 5000)
        {
            // fade in time too long
            LOG(0, "FadeIn(%d) > 5000 -> 5000\n", j);
            j = 5000;
        }
        else if(j < 300)
        {
            // fade in time too short
            LOG(0, "FadeIn(%d) < 300 -> 300\n", j);
            j = 300;
        }
        for(i=0;i<16;i++)
        {
            ...
            if(u4Addr)
            {        
                HalFlushDCacheMultipleLine(u4Addr + 4, 4);
            }
            #endif
            }

        // Display
        j = DRVCUST_OptGet(eDrvLogoVdecTimeDisplay);
        if(j > 5000)
        {
            // fade out time too long
            LOG(0, "Display(%d) > 5000 -> 5000\n", j);
            j = 5000;
        }
        else if(j < 300)
        {
            // fade out time too short
            LOG(0, "Display(%d) < 300 -> 300\n", j);
            j = 300;
        }
        x_thread_delay(j);

        // Fade Out
        j = DRVCUST_OptGet(eDrvLogoVdecTimeFadeOut);
        if(j > 5000)
        {
            // fade out time too long
            LOG(0, "FadeOut(%d) > 5000 -> 5000\n", j);
            j = 5000;
        }
        else if(j < 300)
        {
            // fade out time too short
            LOG(0, "FadeOut(%d) < 300 -> 300\n", j);
            j = 300;
        }
        for(i=16;i>0;i--)
        {
...
BOOL MPV_ShowLogo(VOID)
{
    ...
    if(_hIframeMsgQ == 0)
    {
        x_snprintf(szBuf, sizeof(szBuf), "IFrame-Q%d\n", ucEsId);
        VERIFY(x_msg_q_create(&_hIframeMsgQ, szBuf, sizeof(UINT32), 2) == OSR_OK);
    }
    if(_hIframeTimer == 0)
    {
        VERIFY(x_timer_create(&_hIframeTimer) == OSR_OK);
    }
    #else
    if(_hIframeSema == 0)
    {
    ...
    // for the ic after 68, add HalFlushInvalidateDCacheMultipleLine after data copy.
    #ifdef VDEC_IS_POST_MT5368
    HalFlushInvalidateDCacheMultipleLine((UINT32)(rMpvBuf.pucIFrameBufAddr), rMpvBuf.u4Size);
    #endif
    ...
    #ifndef LOCK_TIMEOUT
    _IFrameFlushMsgQ();//ucEsId);
    #else
    i4Ret = x_sema_lock(_hIframeSema, X_SEMA_OPTION_NOWAIT);
    ...
    #ifndef LOCK_TIMEOUT
    zMsgSize = sizeof(INT32);
    VERIFY(x_timer_start(_hIframeTimer, LOGOVDEC_IFRAME_TIMEOUT, X_TIMER_FLAG_ONCE,
            _IFrameTimerTimeout, (void*)(UINT32)ucEsId) == OSR_OK);
    VERIFY(x_msg_q_receive(&u2MsgQIdx, &u4DecFin, &zMsgSize, &_hIframeMsgQ, 1,
        X_MSGQ_OPTION_WAIT) == OSR_OK);
    VERIFY(x_timer_stop(_hIframeTimer) == OSR_OK);
    #else
    i4Ret = x_sema_lock_timeout(_hIframeSema, LOGOVDEC_IFRAME_TIMEOUT);
    #endif

    #ifndef LOCK_TIMEOUT
    if((u4DecFin == LOGOVDEC_TIMEOUT) || (u4DecFin == LOGOVDEC_ERROR))
    #else
    if(i4Ret != OSR_OK)
    #endif
    {
        LOG(1, " - Wait Vdec decoding timeout/error\n");
        fgRet = FALSE;
        goto DECODE_ERROR;
    }
...
#ifdef IS_SUPPORT_3D_PHOTO
    u4DataAddr = prFbmPool->u4Addr + DECODER_YC_OUTPUT_SIZE; /*canvas buffer begin from decoder output*/
#else
    u4DataAddr = prFbmPool->u4Addr;
#endif
    ...
    #endif
    {
        LOG(1, " - Wait Vdec thumbnail timeout/error\n");
        fgRet = FALSE;
    }
    if(_u4IFrameResult != LOGOVDEC_FINISH)
    {
