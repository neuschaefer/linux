...
static INT32 _GfxSwRectFill(const MI_DIF_FIELD_T *prsGfxReg)
{
    ...
    u4DstPitch = prsGfxReg->fg_OSD_WIDTH << ALIGNMENT;    // 128bit aligned
    //u4DstPitch = prsGfxReg->fg_OSD_WIDTH << 6;    // 128*4bit aligned
...
static INT32 _GfxSwGradFill(const MI_DIF_FIELD_T *prsGfxReg)
{
    ...
    rData.u4DstPitch  = prsGfxReg->fg_OSD_WIDTH << ALIGNMENT;    // 128bit aligned
...
static INT32 _GfxSwBitBlt(const MI_DIF_FIELD_T *prsGfxReg)
{
    ...
    u4FlipEn   = prsGfxReg->fg_DSTPITCH_DEC;
    u4MirrorEn = prsGfxReg->fg_MIRROR;
    u4Color    = prsGfxReg->fg_RECT_COLOR;    
    ...
    if (u4FlipEn || u4MirrorEn)
    {
        return GFX_SwFlipMirrorBlt(pu1SrcBase, pu1DstBase, 
            u4SrcPitch, u4DstPitch, u4SrcCM, u4DstCM, u4Width, u4Height, 
            u4ColorMin, u4ColorMax, u4FlipEn, u4MirrorEn);
    }
    else
    {
...
static INT32 _GfxSwYCrCb2Rgb(const MI_DIF_FIELD_T *prsGfxReg)
{
    ...
    rData.u4VFull       = prsGfxReg->fg_VFULL;
...
static INT32 _GfxSwAlphaCompositionLoop(const MI_DIF_FIELD_T *prsGfxReg)
{
    ...
    rData.u4SrcPitch = prsGfxReg->fg_SRC_PITCH << ALIGNMENT;    // 128bit aligned
    rData.pu1DstBase = (UINT8 *)prsGfxReg->fg_DST_BSAD;
    rData.u4DstPitch = prsGfxReg->fg_OSD_WIDTH << ALIGNMENT;    // 128bit aligned
...
static INT32 _GfxSwRopBitBlt(const MI_DIF_FIELD_T *prsGfxReg)
{
    ...
    rData.u4RectColor = prsGfxReg->fg_XOR_COLOR;
...
static INT32 _GfxSwIndexToDirectBitBlt(const MI_DIF_FIELD_T *prsGfxReg)
{
    ...
    rData.u4SrcPitchEn =  prsGfxReg->fg_IDX2DIR_SRC_PITCH_EN;
#endif
    // virtual address
    rData.pu1SrcBase = VIRTUAL(rData.pu1SrcBase);
    // virtual address
...
//-------------------------------------------------------------------------
/** GFX_SwSetColorCompRecede 
 *  recede frame buffer point for 1 pixel
 */
//-------------------------------------------------------------------------
/*lint -save -e703 -e704 -e826 -e961 */
void GFX_SwSetColorCompRecede(UINT8 **ppu1DestFb, UINT32 u4CM, 
...
//-------------------------------------------------------------------------
/*lint -save -e613 -e740 */
INT32 GFX_SwFlipMirrorBlt(UINT8 *pu1SrcBase, UINT8 *pu1DstBase, 
    ...
    UINT32 u4FlipEn, UINT32 u4MirrorEn)
{
    ...
    for (y = 0; y < u4Height; y++)
    {
        for (x = 0; x < u4Width; x++)
        {
            GFX_SwGetColorComponent(&pu1Read, u4SrcCM, (INT32*)au4SrcColor);

            if (u4MirrorEn == (UINT32)TRUE)
            {
                GFX_SwSetColorCompRecede(&pu1Write, u4DstCM, (INT32*)au4SrcColor);
            }
            else
            {
                GFX_SwSetColorComponent(&pu1Write, u4DstCM, (INT32*)au4SrcColor);
            }

        } // ~for

        if (u4FlipEn == (UINT32)TRUE)
        {
            pu1DstLine -= u4DstPitch;
            pu1Write    = pu1DstLine;
        }
    ...
    case 2:     // Pass 2
        for (y = 0; y < u4Height; y++)
        {
            for (x = 0; x < u4Width; x++)
            {
                ...
                case E_AC_SRC_ATOP: // 9
                    u4Fs = (UINT32)(i4Ad);
                    u4Fd = (UINT32)(256 - i4As);
                    break;

                case E_AC_DST_ATOP: // 10
                    u4Fs = (UINT32)(256 - i4Ad);
                    u4Fd = (UINT32)(i4As);
                    break;
...
INT32 GFX_SwYCbCrLinearScan(const GFX_YCBCR2RGB_DATA_T *prsData)
{
    ...
    if ((UINT32)E_VSTD_BT601 == prsData->u4VideoStd)
    {
        ...
        {
            pfnEquation = GFX_SwYCbCr601VFullEqu;
        }
    }
    ...
    {
            pfnEquation = GFX_SwYCbCr709VFullEqu;
        }
    }
    else
    {
...
//-------------------------------------------------------------------------
/** GFX_SwYCbCr709VFullEqu
 *  Y(601)CbCr -> ARGB video system equation
 *
 */
//-------------------------------------------------------------------------
/*lint -save -e704 -e834 -e961 -e613 */
INT32 GFX_SwYCbCr709VFullEqu(const GFX_YCBCR_T *prsSrc, 
    INT32 *pi4ColorComponent)
{
    INT32 i4Temp = 0;
    INT32 i4Y, i4Cb, i4Cr;
...
//-------------------------------------------------------------------------
/** GFX_SwYCbCr601VFullEqu
 *  Y(601)CbCr -> ARGB video system equation
 *
 */
//-------------------------------------------------------------------------
/*lint -save -e704 -e834 -e961 -e613 */
INT32 GFX_SwYCbCr601VFullEqu(const GFX_YCBCR_T *prsSrc, 
    INT32 *pi4ColorComponent)
{
    INT32 i4Temp = 0;
    INT32 i4Y, i4Cb, i4Cr;
    ...
    // Integer Version (like HW)
    i4Temp = (128 * i4Y) + (179 * (i4Cr - 128));
    i4Temp = i4Temp >> 7;       // i4Temp / 128
    pi4ColorComponent[1] = (i4Temp > 255) ? 255 : 
                           (i4Temp <   0) ?   0 : i4Temp;

    i4Temp = (128 * i4Y) - (44 * (i4Cb - 128)) - (91 * (i4Cr - 128));
    i4Temp = i4Temp >> 7;       // i4Temp / 128
    pi4ColorComponent[2] = (i4Temp > 255) ? 255 : 
                           (i4Temp <   0) ?   0 : i4Temp;

    i4Temp = (128 * i4Y) + (227 * (i4Cb - 128));
    i4Temp = i4Temp >> 7;       // i4temp / 128
    pi4ColorComponent[3] = (i4Temp > 255) ? 255 : 
                           (i4Temp <   0) ?   0 : i4Temp;
...
/*lint -save -e613 -e740 */
#if defined(CC_MT5368)||defined(CC_MT5396)|| defined(CC_MT5389)||defined(CC_MT5398)||defined(CC_MT5880)
void GFX_SwRop_SrcMuti(const GFX_ROP_DATA_T *prsData)
{
    UINT8 *pu1SrcBase, *pu1DstBase;
    UINT32 u4SrcPitch, u4DstPitch;
    UINT32 u4SrcCM, u4DstCM;
    ...
    u4B = (prsData->u4RectColor) & 0xff;
    u4G = (prsData->u4RectColor>>8) & 0xff;
    u4R = (prsData->u4RectColor>>16) & 0xff;
	
	#if defined(CC_MT5398)||defined(CC_MT5880)
	u4A = (prsData->u4RectColor>>24) & 0xff;
	#endif	 
    ...
    for (y = 0; y < u4Height; y++)
    {
        for (x = 0; x < u4Width; x++)
        {
           ...
           // for (i = 0; i < 4; i++)
            {
                au4DstColor[3] = ((au4SrcColor[3]*(u4B+1))>>8)&0xff;
		  		au4DstColor[2] = ((au4SrcColor[2]*(u4G+1))>>8)&0xff;
		  		au4DstColor[1] = ((au4SrcColor[1]*(u4R+1))>>8)&0xff;
				#if defined(CC_MT5398)||defined(CC_MT5880)
	 				au4DstColor[0] = ((au4SrcColor[0]*(u4A+1))>>8)&0xff;
				#else	 
		  			au4DstColor[0] = au4SrcColor[0];
				#endif	
            }
...
void GFX_SwRop_MAX(const GFX_ROP_DATA_T *prsData)
{
    UINT8 *pu1SrcBase, *pu1DstBase;
    UINT32 u4SrcPitch, u4DstPitch;
    UINT32 u4SrcCM, u4DstCM;
    ...
    for (y = 0; y < u4Height; y++)
    {
        for (x = 0; x < u4Width; x++)
        {
           ...
           for (i = 0; i < 4; i++)
            {
               if(au4DstColor[i] < au4SrcColor[i])
               {
...
void GFX_SwRop_MIN(const GFX_ROP_DATA_T *prsData)
{
    UINT8 *pu1SrcBase, *pu1DstBase;
    UINT32 u4SrcPitch, u4DstPitch;
    UINT32 u4SrcCM, u4DstCM;
...
void GFX_SwRop_NotDst(const GFX_ROP_DATA_T *prsData)
{
    ...
    UINT32 x, y, i;
    UINT8 *pu1SrcLine, *pu1DstLine;
    UINT8 *pu1Write, *pu1Dst;
    UINT32  au4DstColor[4];
    //UINT32 au4SrcColor[4];
...
INT32 GFX_SwRopBitBlt(const GFX_ROP_DATA_T *prsData)
{
    ...
    switch (prsData->u4RopOpCode)
    {
  		...
  		#if defined(CC_MT5389)||defined(CC_MT5398)||defined(CC_MT5880)
		case E_ROP_MAX_VALUE :
		 	Printf("test SW E_ROP_MAX \n ");
		 	GFX_SwRop_MAX(prsData);
		 	break;
	 	case E_ROP_MIN_VALUE :
		 	Printf("test SW E_ROP_MIN \n ");
		 	GFX_SwRop_MIN(prsData);
		 	break;
		#endif
		#if defined(CC_MT5368)||defined(CC_MT5396)|| defined(CC_MT5389)||defined(CC_MT5398)||defined(CC_MT5880)
    	case E_ROP_MULTI:
            GFX_SwRop_SrcMuti(prsData);
            break;
    #endif
        // rop #04
        case E_ROP_NOT_SRC:
...
INT32 GFX_SwIndexToDirectBitBlt(const GFX_IDX2DIR_DATA_T *prsData)
{
    ...
    pu1SrcBase = prsData->pu1SrcBase;
#if defined(CC_MT5368)||defined(CC_MT5396)|| defined(CC_MT5389)||defined(CC_MT5398)||defined(CC_MT5880)
    u4SrcPitchEnable = prsData->u4SrcPitchEn;
    u4SrcPitch = prsData->u4SrcPitch;
#else
    ...
    if (u4Remainder > 0)
    {
		...
		{
		 	u4Remainder = (u4SrcPitch % u4PixPerByte);
        		u4PixGroup  = (u4SrcPitch );
		}
#else
    u4Remainder = (u4Width % u4PixPerByte);
    u4PixGroup  = (u4Width / u4PixPerByte);
