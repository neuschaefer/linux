...
#ifdef GFX_CMD_INFO
//-------------------------------------------------------------------------
/** _VGFX_CmdQueIsFlushAll (Cmdque Mode)
 *  push back one register value into cmdque
 *  @param u4Reg indicates which register
 *  @param u4Val indicates the value of the register
 */
//-------------------------------------------------------------------------
void VGFX_CmdQueIsFlushAll(GFX_CMD_FLUSH_INFO_T *pt_info)
{
    ...
    while(i4QueSize)
    {
        ...
        {
            case 0xf0004010:
                pt_info->u4_op = ((u4Val&0x1f0)>>4);
                LOG(2, "GFX OP[0x%x]\n",pt_info->u4_op);
                if(u4Val&0x800)
                {
                    pt_info->u4_fire_cnt++;
                    LOG(2, "GFX Fire\n");
                    if( pt_info->u4_fire_cnt > 0x1 )
                    {
                        return;
                    }
                }
                break;
            case 0xf0004014:
                pt_info->u4_rect_color      = u4Val;
                LOG(2, "rect color 0x%x\n",pt_info->u4_rect_color);
                break;
            case 0xf0004018:
                pt_info->u4_src_addr        = ((u4Val&0x3fffffff));
                LOG(2, "src_addr 0x%x\n",pt_info->u4_src_addr);
                break;
            case 0xf000401c:
                pt_info->u4_dst_addr        = ((u4Val&0x3fffffff));
                LOG(2, "dst_addr 0x%x\n",pt_info->u4_dst_addr);
                break;
            case 0xf0004028:
                pt_info->u4_src_width   = (u4Val&0x7ff);
                LOG(2, "src_width 0x%x\n",pt_info->u4_src_width);
                pt_info->u4_src_height  = (((u4Val>>16)&0x7ff) +1);
                LOG(2, "src_height 0x%x\n",pt_info->u4_src_height);
                break;
            case 0xf000402c:
                pt_info->u4_src_pitch   = (((u4Val>>16)&0xffff)<<4);
                pt_info->u4_dst_pitch   = ((u4Val&0xffff)<<4);
                LOG(2, "pitch [0x%x,0x%x]\n",pt_info->u4_src_pitch,pt_info->u4_dst_pitch);
                break;
            case 0xf000408c:
                pt_info->u4_dst_width = (u4Val&0x7ff);
                LOG(2, "u4_dst_width 0x%x\n",pt_info->u4_dst_width);
                pt_info->u4_dst_height = (((u4Val>>16)&0x7ff) +1);
                LOG(2, "u4_dst_height 0x%x\n",pt_info->u4_dst_height);
                break;
            case 0xf00040d0:
                pt_info->u4_third_addr = ((u4Val&0x3fffffff));
                LOG(2, "u4_third_addr 0x%x\n",pt_info->u4_third_addr);
                break;
            case 0xf00040d4:
                pt_info->u4_pal_addr = ((u4Val&0x3fffffff));
                LOG(2, "u4_pal_addr 0x%x\n",pt_info->u4_pal_addr);
                break;
            case 0xf00040d8:
                pt_info->u4_third_pitch = ((u4Val&0xffff)<<4);
                LOG(2, "u4_third_pitch 0x%x\n",pt_info->u4_third_pitch);
                break;
            default:
                break;
        }
...
void VGFX_RiscPushBack(UINT32 u4Reg, UINT32 u4Val)
{
    UINT32 u4RegOffset;
    ...
    // check engine idle (for debug use)
        GFX_DBG_CHK_ENG_IDLE();
         // write a cmd to the hw register
        VGFX_WRITE32(u4RegOffset, u4Val);
        //Printf( "(%x, %x)\n", u4RegOffset, u4Val);    
        // unlock cmdque resource
        GFX_UnlockCmdque();
