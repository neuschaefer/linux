...
INT32 SFLASH_Write(UINT32 u4Offset, UINT32 u4MemPtr, UINT32 u4ByteCount)
{
...
#if 0
	if(u4WriteAddr < _rSFInfo.arFlashInfo[0].u4ChipSize - u4ByteCount)
	{
	    if(SFLASH_WriteProtect(0, FALSE) != 0)
        {
            LOG(3, "Disable Flash write protect fail!\n");
            return -1;
        }
	}
	else if(u4WriteAddr > _rSFInfo.arFlashInfo[0].u4ChipSize )
	{
	    if(SFLASH_WriteProtect(1, FALSE) != 0)
        {
            LOG(3, "Disable Flash write protect fail!\n");
            return -1;
        }
    ...
    {   
        ...
        for(j=0; j<_rSFInfo.arFlashInfo[i].u4SecCount; j++)
        {
            ...
            if((u4SectStart<=u4WriteAddr)&&(u4WriteAddr<u4SectEnd))
            {
				...
				if((u4WriteAddr < u4ChipOffset) && 
				   (u4WriteAddr + u4Len > u4ChipOffset))
				{
                    u4Len = u4ChipOffset - u4WriteAddr;
				}
	...
	if(u4WriteAddr < _rSFInfo.arFlashInfo[0].u4ChipSize - u4ByteCount)
	{
	    if(SFLASH_WriteProtect(0, TRUE) != 0)
        {
            LOG(3, "Enable Flash write protect fail!\n");
            return -1;
        }
	...
	{
	    if(SFLASH_WriteProtect(1, TRUE) != 0)
        {
            LOG(3, "Enable Flash write protect fail!\n");
            return -1;
        }
