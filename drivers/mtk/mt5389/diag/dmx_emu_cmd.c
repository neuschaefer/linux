...
#if 0
static void _ShowCpuDscrTestUsage(const CHAR* szCmd, UINT32 u4Max, UINT32 u4Min)
{
    Printf("Usage: %s mode[cpm|des|aes] [loop(1, 0=infinite)]"
        " [max_size=0x%x] [min_size=0x%x] [encrypt(0|1)]\n",
        szCmd, u4Max, u4Min);
}

static INT32 _CmdCpuDscrTest(INT32 i4Argc, const CHAR** aszArgv)
{
    UINT32 u4MaxLoop = 1, u4MaxSize = 0x1000, u4MinSize = 0x100;
    UINT32 u4Loop, u4Size;
    BOOL fgRet, fgEncrypt = FALSE;
    DMX_CPU_DESC_MODE eMode;

    if (i4Argc == 1)
    {
        _ShowCpuDscrTestUsage(aszArgv[0], u4MaxSize, u4MinSize);
        return -1;
    }

    if (x_strcmp(aszArgv[1], "cpm") == 0)
    {
        eMode = DMX_CPU_DESC_CPM_C2D;
    }
    else if (x_strcmp(aszArgv[1], "des") == 0)
    {
        eMode = DMX_CPU_DESC_DES_ECB;
    }
    else if (x_strcmp(aszArgv[1], "aes") == 0)
    {
        eMode = DMX_CPU_DESC_AES_ECB_128;
    }
    else
    {
        _ShowCpuDscrTestUsage(aszArgv[0], u4MaxSize, u4MinSize);
    ...
    Printf("\nStart CPU descrambler test\n");
    Printf("Round: %u, max size: 0x%x, min size: 0x%x\n\n",
        u4MaxLoop, u4MaxSize, u4MinSize);
    ...
    while (1)
    {
        ...
        u4Size = DMX_Align(random(u4MaxSize - u4MinSize) + u4MinSize - 1, 1); //TODO: change from 8 to 1

        Printf("Round %u - size: %u\n", u4Loop, u4Size);

        if (!DMX_EMU_CpuDscr(u4Size, eMode, fgEncrypt))
        {
            break;
        }
...
static HANDLE_T _hDmxCDSThread = NULL_HANDLE;

static VOID _DmxPSIThreadx(VOID* pvArg)
{
    UINT32 i, u4Loop = 40;

    for(i=0; i<u4Loop; i++)
    {
        if(!_DMX_EMU_PSI_Test(_u1TsIndex, TRUE, DMX_FILTER_MODE_GENERIC))
        {
            Printf("Fail\n");
            return -1;
        }
        Printf("PSI %d time OK\n", i);
        x_thread_delay(1);
    }
...
static VOID _DmxCDSThread(VOID* pvArg)
{
    UINT32 i, u4Loop = 1000;

    for(i=0; i<u4Loop; i++)
    {
        if (!DMX_EMU_CpuDscr(0x1000, DMX_CPU_DESC_AES_ECB_128, 0))
        {
            break;
        }
        
        Printf("CDS %d time OK\n", i);
...
static INT32 _CmdCpuDscrInterTest(INT32 i4Argc, const CHAR** aszArgv)
{
    ...
    if(u4mode == 0) 
    {
        ...
        // We need to execute PSI and CDS test one time, then use thread
        // Why?
        if(!_DMX_EMU_PSI_Test(_u1TsIndex, TRUE, DMX_FILTER_MODE_GENERIC))
        {
            Printf("Fail\n");
        ...
        // Create PSI thread
        if (x_thread_create(&_hDmxPSIThread, "xxx1",
            DMX_PSI_THREAD_STACK_SIZE, DMX_PSI_THREAD_PRIORITY, _DmxPSIThreadx, 0,
            NULL) != OSR_OK)
        {
            LOG(1, "Fail to create PSI thread!\n");
            return FALSE;
        }

        // Create CPU descrambler thread
        if (x_thread_create(&_hDmxCDSThread, "xxx2",
            DMX_PSI_THREAD_STACK_SIZE, DMX_PSI_THREAD_PRIORITY, _DmxCDSThread, 0,
            NULL) != OSR_OK)
        {
            LOG(1, "Fail to create CDS thread!\n");
            return FALSE;
        }
    }
    else 
    {
        ...
        for(i=0; i<u4Loop; i++)
        {        
            CLI_Parser("demu.dr0 aes 1");
            x_thread_delay(10);
            CLI_Parser("demu.cds aes 1");
            x_thread_delay(10);
        }
    }
...
static void _ShowMsgDigestTestUsage(const CHAR* szCmd, UINT32 u4Max, UINT32 u4Min)
{
    Printf("Usage: %s mode[sha1|sha256|sha224|md5] [loop(1, 0=infinite)]"
        " [max_size=0x%x] [min_size=0x%x]\n",
        szCmd, u4Max, u4Min);
}
static INT32 _CmdMsgDigestTest(INT32 i4Argc, const CHAR** aszArgv)
{
    UINT32 u4MaxLoop = 1, u4MaxSize = 0x1000, u4MinSize = 0x100, u4Endien = 0;
    UINT32 u4Loop, u4Size;
    BOOL fgRet;
    DMX_CPU_DESC_MODE eMode;
    ...
    if (i4Argc == 1)
    {
        _ShowMsgDigestTestUsage(aszArgv[0], u4MaxSize, u4MinSize);
        return -1;
    }

    if (x_strcmp(aszArgv[1], "sha1") == 0)
    {
        eMode = DMX_CPU_DESC_SHA_1;
    }
    else if (x_strcmp(aszArgv[1], "sha256") == 0)
    {
        eMode = DMX_CPU_DESC_SHA_256;
    }
    else if (x_strcmp(aszArgv[1], "sha224") == 0)
    {
        eMode = DMX_CPU_DESC_SHA_224;
    }
    else if (x_strcmp(aszArgv[1], "md5") == 0)
    {
        eMode = DMX_CPU_DESC_MD5;
    }    
    else
    {
        _ShowCpuDscrTestUsage(aszArgv[0], u4MaxSize, u4MinSize);
    ...
    while (1)
    {
        ...
        if (!DMX_EMU_MsgDigest_Test(u4Size, eMode))
        {
            break;
        }
...
static INT32 _CmdMsgDigestMultiPassTest(INT32 i4Argc, const CHAR** aszArgv)
{
    UINT32 u4MaxLoop = 1, u4MaxSize = 0x1000, u4MinSize = 0x100, u4Endien = 0;
    UINT32 u4Loop, u4Size;
    BOOL fgRet;
    ...
    while (1)
    {
        ...
        if (!DMX_EMU_MsgDigest_Multipass_Test(u4Size, eMode))
        {
            break;
        }
...
#ifdef TEST_MODE
static INT32 _CmdSHA1SelfTest(INT32 i4Argc, const CHAR** aszArgv)
{
    BOOL fgRet;
    fgRet = DMX_EMU_SHA1_SelfTest();
    if (!fgRet)
    {
        Printf("Fail!\n");
        return -1;
...
static INT32 _CmdSHA224SelfTest(INT32 i4Argc, const CHAR** aszArgv)
{
    BOOL fgRet;
    fgRet = DMX_EMU_SHA224_SelfTest();
    if (!fgRet)
    {
        Printf("Fail!\n");
        return -1;
...
static INT32 _CmdSHA256SelfTest(INT32 i4Argc, const CHAR** aszArgv)
{
    BOOL fgRet;
    fgRet = DMX_EMU_SHA256_SelfTest();
    if (!fgRet)
    {
        Printf("Fail!\n");
        return -1;
...
static INT32 _CmdMD5SelfTest(INT32 i4Argc, const CHAR** aszArgv)
{
    BOOL fgRet;
    fgRet = DMX_EMU_MD5_SelfTest();
    if (!fgRet)
    {
        Printf("Fail!\n");
        return -1;
...
static INT32 _CmdParserAesTest(INT32 i4Argc, const CHAR** aszArgv)
{
    UINT32 u4MaxLoop = 1, u4MaxSize = 0x1000, u4MinSize = 0x100;
    UINT32 u4Loop, u4Size;
    BOOL fgRet, fgEncrypt = FALSE;
    DMX_CPU_DESC_MODE eMode = DMX_CPU_DESC_AES_ECB_128;

    if (i4Argc == 1)
    {
        Printf("Usage: %s [loop(1, 0=infinite)] [encrypt(0|1)]"
        " [max_size=0x%x] [min_size=0x%x]\n",
        aszArgv[0], u4MaxSize, u4MinSize);
    ...
    while (1)
    {
        ...
        if (!DMX_EMU_Gcpu_Aes(u4Size, eMode, fgEncrypt))
        {
            break;
        }
...
EXTERN BOOL _DMX_EMU_FVR_PES_Test(void);
static INT32 _CmdFVRPesTest(INT32 i4Argc, const CHAR** aszArgv)
{
    BOOL fgRet;
    
    fgRet = _DMX_EMU_FVR_PES_Test();
    if(fgRet)
    {
        Printf("OK\n");
    }
...
EXTERN BOOL _DMX_EMU_FVR_TimestampOnOffTest(BOOL fgTimeStampOn);
static INT32 _CmdFVRTimestampTest(INT32 i4Argc, const CHAR** aszArgv)
{
    ...
    {
        Printf("----- Test %d -----\n", i+1);
        if(_DMX_EMU_FVR_TimestampOnOffTest(fgTimestampOn))
        {
            Printf("OK\n");
        }
        else
...
EXTERN BOOL _DMX_EMU_FVR_TimestampPauseTest(void);
static INT32 _CmdFVRTimestampPauseTest(INT32 i4Argc, const CHAR** aszArgv)
{
    ...
    {
        Printf("----- Test %d -----\n", i+1);
        if(_DMX_EMU_FVR_TimestampPauseTest())
        {
            Printf("OK\n");
        }
        else
...
BOOL _DMX_EMU_FVR_TimestampErrTest(void);
static INT32 _CmdFVRTimestampErrTest(INT32 i4Argc, const CHAR** aszArgv)
{
    ...
    {
        Printf(" -----Test %d -----\n", i+1);
        if(_DMX_EMU_FVR_TimestampErrTest())
        {
            Printf("OK\n");
        }
        else
...
EXTERN BOOL _DMX_EMU_FVR_TimestampResetTest(void);
static INT32 _CmdFVRTimestampResetTest(INT32 i4Argc, const CHAR** aszArgv)
{
    ...
    {
        Printf("----- Test %d -----\n", i+1);
        if(_DMX_EMU_FVR_TimestampResetTest())
        {
            Printf("OK\n");
        }
        else
...
#endif    
#endif 
#if 0   //#ifdef CC_CPU_DESCRAMBLER
    {
        "cpudscr", "cds", _CmdCpuDscrTest, NULL, "", CLI_SUPERVISOR
    },

    {
        "cpudscrinter", "cdsi", _CmdCpuDscrInterTest, NULL, "", CLI_SUPERVISOR
    },
   
    {
        "mdtest", "mdt", _CmdMsgDigestTest, NULL, "", CLI_SUPERVISOR
    },        
    {
        "mdmultipasstest", "mdmt", _CmdMsgDigestMultiPassTest, NULL, "", CLI_SUPERVISOR
    },   
#ifdef TEST_MODE    
    {
        "sha1selftest", "sha1", _CmdSHA1SelfTest, NULL, "", CLI_SUPERVISOR
    },        
    {
        "sha256selftest", "sha256", _CmdSHA256SelfTest, NULL, "", CLI_SUPERVISOR
    },
    {
        "sha224selftest", "sha224", _CmdSHA224SelfTest, NULL, "", CLI_SUPERVISOR
    },    
    {
        "md5selftest", "md", _CmdMD5SelfTest, NULL, "", CLI_SUPERVISOR
    },
#endif    
    {
        "aesparser", "aesp", _CmdParserAesTest, NULL, "", CLI_SUPERVISOR
    },    
#endif
#ifdef TEST_MODE
    {
