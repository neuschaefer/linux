...
CLI_EXEC_T arVdoCmdTbl[] = {
    ...
    {"VideoModeChg",    "modchg",_NptvVideoModChg,    NULL, "Video Mode Change Done",      CLI_GUEST},
    {"VideoSWRegRead",  "swr",   _NptvVideoSWRegRead, NULL, "Read Software Register",      CLI_GUEST},
    {"VideoSWRegWrite", "sww",   _NptvVideoSWRegWrite,NULL, "Write Software Register",     CLI_GUEST},
    #endif
...
{
    UINT32 u4VdpId;
    UINT8  bUnMute, bSaveEEp=0 ;

    if (i4Argc < 3)
    {
        Printf("Arg: VdpId OnOff EEpSave\n");
        Printf("<VdpId>\n");
        Printf("0: Main\n");
        Printf("1: Sub\n");
        Printf("<OnOff>\n");
    ...
    if(bSaveEEp)
    {
        ...
        fgApiEepromWriteByte(EEP_NPTV_BOOT_DEBUG_ENABLE, ((bUnMute == 0) ? 0 :SV_BOOT_DEBUG_MAGIC));
        ...
        bBootCmd &= ~(1<<7) ; //Clear bit[7]
        bBootCmd |= ((bUnMute& 0x1) << 7) ;
...
static INT32 _NptvTimeMeasureEn(INT32 i4Argc, const CHAR **szArgv)
{
    ...
    if(i4Argc==2)
    {
       _bSysMeasueTimeEn = StrToInt(szArgv[1]) ;
