...
#endif
{
    ...
    case CB_GET_CALLBACK:
        ...
        while (prCbEv == NULL)
        {
            ...
            {
                _CB_PutThdIntoWQ();
                sig_return = signal_pending(current);
                if (sig_return)
                {
                    return -EINTR;
                }
        ...
        ASSERT((sizeof(CB_GET_CALLBACK_T) + prCbEv->rGetCb.i4TagSize) <
               CB_MAX_STRUCT_SIZE);
...
static int cb_release(struct inode *inode, struct file *filep)
{
...
struct file_operations cb_fops = {
#ifndef USE_UNLOCK_IOCTL
    .ioctl		= cb_ioctl,
#else
