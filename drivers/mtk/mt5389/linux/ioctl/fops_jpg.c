...
static int jpg_mmap(struct file *, struct vm_area_struct *);
...
static int jpg_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	start = (unsigned long)_JpgOnMMAPAddr(); //info->fix.smem_start;
	//info->fix.smem_len;
	len = PAGE_ALIGN(start & ~PAGE_MASK) + _JpgOnMMAPSize();
	start &= PAGE_MASK;
...
struct file_operations jpg_fops = {
	.owner   = THIS_MODULE,
	.mmap  = jpg_mmap,
};
