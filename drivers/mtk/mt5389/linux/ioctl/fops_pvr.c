...
static int pvr_mmap(struct file *, struct vm_area_struct *);
...
static int pvr_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	start = (unsigned long)_PvrOnMMAPAddr(); //info->fix.smem_start;
	//info->fix.smem_len;
	len = PAGE_ALIGN(start & ~PAGE_MASK) + _PvrOnMMAPSize();
	start &= PAGE_MASK;
...
struct file_operations pvr_fops = {
	.owner   = THIS_MODULE,
	.mmap  = pvr_mmap,
};
