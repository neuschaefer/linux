...
#include "mutil/minfo/rm_info/rm_minfo.h"
#include "mutil/minfo/rm_info/x_minfo_rm.h"
...
#ifdef CLI_LVL_ALL
static INT32 minfo_cli_rm_fopen(INT32 i4_argc, const CHAR** pps_argv);
static INT32 minfo_cli_rm_mopen(INT32 i4_argc, const CHAR** pps_argv);
static INT32 minfo_cli_rm_close(INT32 i4_argc, const CHAR** pps_argv);
static INT32 minfo_cli_rm_getnum(INT32 i4_argc, const CHAR** pps_argv);
static INT32 minfo_cli_rm_getinfo(INT32 i4_argc, const CHAR** pps_argv);
static INT32 minfo_cli_rm_fstress(INT32 i4_argc, const CHAR** pps_argv);
static INT32 minfo_cli_rm_mstress(INT32 i4_argc, const CHAR** pps_argv);

/* mfmtrecg command table */
static CLI_EXEC_T at_rm_minfo_cmd_tbl[] =
{
    {"fopen",   NULL,  minfo_cli_rm_fopen,     NULL,    "minfo_fopen",     CLI_SUPERVISOR},
    {"mopen",   NULL,  minfo_cli_rm_mopen,     NULL,    "minfo_mopen",     CLI_SUPERVISOR},
    {"close",   NULL,  minfo_cli_rm_close,     NULL,    "minfo_close",     CLI_SUPERVISOR},
    {"getnum",  NULL,  minfo_cli_rm_getnum,    NULL,    "minfo_getnum",    CLI_SUPERVISOR},
    {"getinfo", NULL,  minfo_cli_rm_getinfo,   NULL,    "minfo_getinfo",   CLI_SUPERVISOR},
    {"fstress", NULL,  minfo_cli_rm_fstress,   NULL,    "minfo_fstress",   CLI_SUPERVISOR},
    {"mstress", NULL,  minfo_cli_rm_mstress,   NULL,    "minfo_mstress",   CLI_SUPERVISOR},
    END_OF_CLI_CMD_TBL
};
 ...
 * Returns: CLIR_OK                 Routine successful.
 ----------------------------------------------------------------------------*/
#ifdef CLI_LVL_ALL
static INT32 minfo_cli_rm_fopen(INT32        i4_argc,
                                 const CHAR** pps_argv)
{
    ...
    if ((i4_argc != 3) || (pps_argv == NULL))
    {
        ...
        x_dbg_stmt("usage:fopen filename formatflag(0/1)\r\n");
        return CLIR_INV_ARG;
    }
    /* check file path */
    ...
    x_dbg_stmt("minfo open file(%s) successfully!\r\n", ps_file_path);
    return CLIR_OK;
 ...
 * Returns: CLIR_OK                 Routine successful.
 ----------------------------------------------------------------------------*/
#ifdef CLI_LVL_ALL
static INT32 minfo_cli_rm_mopen(INT32        i4_argc,
                                 const CHAR** pps_argv)
{
#ifndef __NO_FM__
    INT32           i4_ret;
    ...
    {
        DBG_ERROR(("Invalid argument!\r\n"));
        x_dbg_stmt("usage:mopen filename formatflag(0/1) mem_length \r\n");
        return CLIR_INV_ARG;
    }
    /* check file path */
    ps_file_path = (CHAR*)pps_argv[1];
    ...
    else
    {
        ...
        if(gpui1_buf == NULL)
        {
            x_dbg_stmt("mopen alloc memory fail \r\n");
            x_fm_close(gh_file);   
            gh_file = NULL_HANDLE;
            return CLIR_CMD_EXEC_ERROR;
        }
    ...
    x_dbg_stmt("minfo mopen successfully!\r\n");
    return CLIR_OK;
#else
    return CLIR_OK;
#endif
 ...
 * Returns: CLIR_OK                 Routine successful.
 ----------------------------------------------------------------------------*/
#ifdef CLI_LVL_ALL
static INT32 minfo_cli_rm_close(INT32        i4_argc,
                                 const CHAR** pps_argv)
{
    INT32 i4_ret;

    /* check and free mopen resource */
    x_dbg_stmt("minfo_cli_rm_close \r\n");
    if(gh_file != NULL_HANDLE)
    {
        x_dbg_stmt("close file handle for mopen \r\n");
        x_fm_close(gh_file);   
        gh_file = NULL_HANDLE;
    }
    if(gpui1_buf != NULL)
    {
        x_dbg_stmt("free memory for mopen  \r\n");
        _mm_util_mem_free(gpui1_buf);
        gpui1_buf = NULL;
    }
    ...
    if (MINFOR_OK != i4_ret)
    {
        DBG_ERROR(("rm closed error=%d\r\n", i4_ret));
        return CLIR_CMD_EXEC_ERROR;
    }

    x_dbg_stmt("rm closed successfully!\r\n");
    return CLIR_OK;
}
#endif
/*-----------------------------------------------------------------------------
 * Name: minfo_cli_rm_getnum
 *
 * Description: 
 *
 * Inputs:  i4_argc         Contains the argument count.
 ...
 * Returns: CLIR_OK                 Routine successful.
 ----------------------------------------------------------------------------*/
#ifdef CLI_LVL_ALL
static INT32 minfo_cli_rm_getnum(INT32        i4_argc,
                                  const CHAR** pps_argv)
{
    ...
    {
        DBG_ERROR(("Invalid argument! \r\n"));
        x_dbg_stmt("usage:getnum A/V/G/I \r\n");
        x_dbg_stmt("     A:Audio; V:Video; G:Generic; I:Idx builder \r\n");
        return CLIR_INV_ARG;
    }
    ...
    if (i4_ret != MINFOR_OK)
    {
        x_dbg_stmt("rm minfo getnum FAIL!! \r\n");
    }
    else
    {
        switch (e_info_type)
        ...
        case MINFO_INFO_TYPE_VIDEO_ATTR:
            x_dbg_stmt("get video num = %u \r\n", ui2_num_recs);
            break;
        case MINFO_INFO_TYPE_AUDIO_ATTR:
            x_dbg_stmt("get audio num = %u \r\n", ui2_num_recs);
            break;
        case MINFO_INFO_TYPE_GENERIC:
            x_dbg_stmt("get generic num = %u \r\n", ui2_num_recs);
            break;
        case MINFO_INFO_TYPE_IDX_INFO:
            x_dbg_stmt("get idx_builder num = %u \r\n", ui2_num_recs);
            break;
        default:
            x_dbg_stmt("get num unknown ");
            break;
        }
        x_dbg_stmt("rm minfo getnum OK \r\n");
    }
    return CLIR_OK;
}
#endif
 ...
 * Returns: CLIR_OK                 Routine successful.
 ----------------------------------------------------------------------------*/
#ifdef CLI_LVL_ALL
static INT32 minfo_cli_rm_getinfo(INT32        i4_argc,
                                   const CHAR** pps_argv)
{
    MINFO_INFO_TYPE_T  e_info_type;
    INT32              i4_ret;
    ...
    {
        DBG_ERROR(("Invalid argument! \r\n"));
        x_dbg_stmt("usage:getinfo A/V/G/I index \r\n");
        x_dbg_stmt("      A:Audio; V:Video; G:Generic; I:Idx builder \r\n");
        return CLIR_INV_ARG;
    }
    ...
    if (i4_ret != MINFOR_OK)
    {
        x_dbg_stmt("rm minfo getinfo FAIL!! \r\n");
    }
    else
    {
        switch (e_info_type)
        ...
        case MINFO_INFO_TYPE_GENERIC:
            ...
            x_dbg_stmt("size =              0x%.16x\r\n", 
                      t_minfo_rec.u.t_general_info.ui8_media_sz);
            x_dbg_stmt("stream numbers =    0x%.16x\r\n", 
        ...
        case MINFO_INFO_TYPE_SP_ATTR:
            break;
        default:
            x_dbg_stmt("get info unknown type ");
            break;
        }
        x_dbg_stmt("rm minfo get info OK \r\n");
    }
    return CLIR_OK;
}
#endif
...
static INT32 minfo_cli_rm_fstress(INT32 i4_argc, const CHAR** pps_argv)
{
    ...
    {
        DBG_ERROR(("Invalid argument!\r\n"));
        x_dbg_stmt("usage: fstress filename  \r\n");
        return CLIR_INV_ARG;
    }
    /* check file path */
    ps_file_path = (CHAR*)pps_argv[1];
    ...
    while (i4_loop_count--)
    {
        ...
        if (i4_ret != MINFOR_OK)
        {
            x_dbg_stmt("rm minfo get number FAIL!! \r\n");
            x_minfo_close(gh_minfo);
            x_dbg_stmt("rm closed file! \r\n");
            continue;
        }
        x_dbg_stmt("get video num = %u \r\n", ui2_num_recs);
        ...
        {
            x_dbg_stmt("get video info steam id = %u \r\n", 
                      t_minfo_rec.u.t_stm_attribute.t_stm_id);
            x_dbg_stmt("get video info Width = %u \r\n", 
                      t_minfo_rec.u.t_stm_attribute.u.t_rm_video_stm_attr.ui2_pad_width);
            x_dbg_stmt("get video info Width = %u \r\n", 
                      t_minfo_rec.u.t_stm_attribute.u.t_rm_video_stm_attr.ui2_pad_height);
        }
        ...
        i4_ret = x_minfo_get_info(gh_minfo,
                                  ...
                                  (VOID*)&t_minfo_rec,
                                  &t_size);
        x_dbg_stmt("get audio info steam id = %u \r\n", 
                  t_minfo_rec.u.t_stm_attribute.t_stm_id);
        x_dbg_stmt("get audio info sample rate = %u \r\n", 
                  t_minfo_rec.u.t_stm_attribute.u.t_rm_audio_stm_attr.ui4_sample_rate);
        x_dbg_stmt("get audio info sample size = %u \r\n", 
                  t_minfo_rec.u.t_stm_attribute.u.t_rm_audio_stm_attr.ui2_sample_sz);
        /* generic info */
        i4_ret = x_minfo_get_info(gh_minfo,
                                  MINFO_INFO_TYPE_GENERIC,
                                  ...
                                  (VOID*)&t_minfo_rec,
                                  &t_size);
        x_dbg_stmt("get general info duration = %llu \r\n", 
                  t_minfo_rec.u.t_general_info.ui8_pb_duration);
        /* index builder info */
        i4_ret = x_minfo_get_info(gh_minfo,
                                  MINFO_INFO_TYPE_IDX_INFO,
                                  ...
                                  (VOID*)&t_minfo_rec,
                                  &t_size);
        x_dbg_stmt("get idx_builder index offset = 0x%llx \r\n", 
                   t_minfo_rec.u.t_idx_info.ui8_idx_offset);
        x_dbg_stmt("get idx_builder index size = 0x%llx \r\n", 
                   t_minfo_rec.u.t_idx_info.ui8_idx_sz);
        x_dbg_stmt("get idx_builder data offset = 0x%llx \r\n", 
                   t_minfo_rec.u.t_idx_info.ui8_data_offset);
        x_dbg_stmt("get idx_builder data size = 0x%llx \r\n", 
                   t_minfo_rec.u.t_idx_info.ui8_data_sz);
...
{    
    ...
    {
        DBG_ERROR(("Invalid argument!\r\n"));
        x_dbg_stmt("usage: mstress filename mem_length(0/1) \r\n");
        return CLIR_INV_ARG;
    }
    /* check file path */
    ps_file_path = (CHAR*)pps_argv[1];
 ...
 * Returns: 
 ----------------------------------------------------------------------------*/
INT32 rm_minfo_cli_init(CLI_EXEC_T** ppt_command_array)
{
#ifdef CLI_LVL_ALL

    *ppt_command_array = at_rm_minfo_cmd_tbl;
#endif
    return 0;
}
