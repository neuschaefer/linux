 ...
 *---------------------------------------------------------------------------*/
/*-----------------------------------------------------------------------------
 * $RCSfile: sbtl_mkv_srt_parser.c,v $
 * $Revision: #1 $
 * $Date: 2012/04/27 $
 * $Author: dtvbm11 $
 * $CCRevision: $
 ...
 * Returns: -
 ----------------------------------------------------------------------------*/
INT32 mkv_srt_seek_time_stamp(SBTL_PAGE_INST_MNGR_T*    pt_mngr,
                              PTS_T                     t_pts_current,
                              PTS_T*                    pt_pts_start,
                              PTS_T*                    pt_pts_end,
                              BOOL*                     pb_found)
 ...
 * Returns: -
 ----------------------------------------------------------------------------*/
INT32 mkv_srt_load_string(SBTL_PAGE_INST_MNGR_T*    pt_mngr)
{
    BOOL                                b_found;
    ...
    if (MMSBTLR_OK != i4_ret)
    {
        ASSERT(0, ("mkv_srt_load_string data retrival fail\n"));
        return MMSBTLR_NOT_FOUND;
    }
    ...
    if (MMSBTLR_OK != i4_ret)
    {
        x_dbg_stmt("DEMUX_SET_TYPE_UNLOCK_BUFFER fail=%d\n", i4_ret);
        //ASSERT(0, ("DEMUX_SET_TYPE_UNLOCK_BUFFER fail\n"));
    ...
    while (1)
    {
        if ((ui1_pos >= (UINT8)ui4_data_2) || (ui1_idx >= (UINT8)ui4_data_2))
        {
            break;
        }

        ui1_tmp = pt_private->aui1_text_buf[ui1_pos++];

        if (ui1_tmp == au1MatchCode[ui1_idx2])
        {
            ui1_idx2++;
            if(ui1_tmp == 0x0A)
            {
                if (ui1_idx >= (UINT8)ui4_data_2)
                {
                    break;
                }
 ...
 * Returns: -
 ----------------------------------------------------------------------------*/
UINT32 mkv_srt_timer_period(SBTL_PAGE_INST_MNGR_T*      pt_mngr)
{
 ...
 * Returns: -
 ----------------------------------------------------------------------------*/
INT32 mkv_srt_init(SBTL_PAGE_INST_MNGR_T*   pt_mngr)
{
    return MMSBTLR_OK;
}
 ...
 * Returns: -
 ----------------------------------------------------------------------------*/
INT32 mkv_srt_deinit(SBTL_PAGE_INST_MNGR_T* pt_mngr)
{
    return MMSBTLR_OK;
}
 ...
 * Returns: -
 ----------------------------------------------------------------------------*/
VOID mkv_srt_start(SBTL_PAGE_INST_MNGR_T*   pt_mngr)
{
}
 ...
 * Returns: -
 ----------------------------------------------------------------------------*/
VOID mkv_srt_stop(SBTL_PAGE_INST_MNGR_T*    pt_mngr)
{
}
