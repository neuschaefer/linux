 ...
 *---------------------------------------------------------------------------*/
/*-----------------------------------------------------------------------------
 * $RCSfile: sbtl_powerdivx_parser.c,v $
 * $Revision: #1 $
 * $Date: 2012/04/27 $
 * $Author: dtvbm11 $
 * $CCRevision: $
 ...
 * Returns: -
 ----------------------------------------------------------------------------*/
INT32 psb_seek_time_stamp(SBTL_PAGE_INST_MNGR_T*    pt_mngr,
                          PTS_T                     t_pts_current,
                          PTS_T*                    pt_pts_start,
                          PTS_T*                    pt_pts_end,
                          BOOL*                     pb_found)
    ...
    *pb_found = FALSE;
    while(1)
    {
        ...
        if (pt_private->b_UTF16)
        {
            ...
            if(('{' == aui2_buf[0]) && (':' == aui2_buf[2]) && 
               (':' == aui2_buf[5]) && ('}' == aui2_buf[8]) && 
               ('{' == aui2_buf[9]) && (':' == aui2_buf[11]) &&
               (':' == aui2_buf[14]) && ('}' == aui2_buf[17]))
            {
                /*   {0:01:40}{0:01:43}    */
                ui4_hour = sbtl_wasci2decimal(&aui2_buf[1], 1);
                ui4_min = sbtl_wasci2decimal(&aui2_buf[3], 2);
                ui4_sec = sbtl_wasci2decimal(&aui2_buf[6], 2);

                *pt_pts_start = (UINT64)(PTS_FREQ*(UINT64)ui4_sec)
                              + (UINT64)(PTS_FREQ*(UINT64)ui4_min*60)
                              + (UINT64)(PTS_FREQ*(UINT64)ui4_hour*3600);

                ui4_hour = sbtl_wasci2decimal(&aui2_buf[10], 1);
                ui4_min = sbtl_wasci2decimal(&aui2_buf[12], 2);
                ui4_sec = sbtl_wasci2decimal(&aui2_buf[15], 2);

                *pt_pts_end = (UINT64)(PTS_FREQ*(UINT64)ui4_sec)
                            + (UINT64)(PTS_FREQ*(UINT64)ui4_min*60)
                            + (UINT64)(PTS_FREQ*(UINT64)ui4_hour*3600);
        ...
        {
        i4_ret = sbtl_feeder_inpnbyte(pt_mngr->pt_engine, 18, aui1_buf, NULL, MM_SBTL_FEEDER_SUBTITLE);
        if (i4_ret != MMSBTLR_OK)
        {
            *pb_found = FALSE;
            return i4_ret;
        ...
        if(('{' == aui1_buf[0]) && (':' == aui1_buf[2]) && 
           (':' == aui1_buf[5]) && ('}' == aui1_buf[8]) && 
           ('{' == aui1_buf[9]) && (':' == aui1_buf[11]) &&
           (':' == aui1_buf[14]) && ('}' == aui1_buf[17]))
        {
                /*   {0:01:40}{0:01:43}   */
            ui4_hour = sbtl_asci2decimal(&aui1_buf[1], 1);
            ...
            *pt_pts_start = (UINT64)(PTS_FREQ*(UINT64)ui4_sec)
            ...
            ui4_hour = sbtl_asci2decimal(&aui1_buf[10], 1);
            ui4_min = sbtl_asci2decimal(&aui1_buf[12], 2);
            ui4_sec = sbtl_asci2decimal(&aui1_buf[15], 2);
 ...
 * Returns: -
 ----------------------------------------------------------------------------*/
INT32 psb_load_string(SBTL_PAGE_INST_MNGR_T*    pt_mngr)
{
    ...
    pt_private = (SBTL_TEXT_PAGE_MNGR_PRIVATE_T*)pt_mngr->pv_private;
    if (pt_feeder == NULL || pt_private == NULL)
    {
 ...
 * Returns: -
 ----------------------------------------------------------------------------*/
UINT32 psb_timer_period(SBTL_PAGE_INST_MNGR_T*  pt_mngr)
{
 ...
 * Returns: -
 ----------------------------------------------------------------------------*/
INT32 psb_init(SBTL_PAGE_INST_MNGR_T*   pt_mngr)
{
    return MMSBTLR_OK;
}
 ...
 * Returns: -
 ----------------------------------------------------------------------------*/
INT32 psb_deinit(SBTL_PAGE_INST_MNGR_T*     pt_mngr)
{
    return MMSBTLR_OK;
}
 ...
 * Returns: -
 ----------------------------------------------------------------------------*/
VOID psb_start(SBTL_PAGE_INST_MNGR_T*   pt_mngr)
{
}
 ...
 * Returns: -
 ----------------------------------------------------------------------------*/
VOID psb_stop(SBTL_PAGE_INST_MNGR_T*    pt_mngr)
{
}
 ...
 * Returns: -
 ----------------------------------------------------------------------------*/
/*
{0:01:40}{0:01:43}Yeah. The Drama Club is worried|that you haven't been coming.
*/
BOOL psb_parser_verify(SBTL_ENGINE_T*   pt_engine)
{
    INT32   i4_ret;
    UINT16  ui1_idx;
    UINT8   aui1_buf[24];
