...
#include "mutil/mfmtrecg/mkv_mfmtrecg/mkv_mfmtrecg.h"
...
#ifdef CLI_LVL_ALL
static INT32 mfmtrecg_cli_mkv_fconfirm_fct(INT32 i4_argc, const CHAR** pps_argv);     
static INT32 mfmtrecg_cli_mkv_mconfirm_fct(INT32 i4_argc, const CHAR** pps_argv);     

/* mfmtrecg command table */
static CLI_EXEC_T at_mkv_mfmtrecg_cmd_tbl[] =
{
    {"mkv fconfirm" ,       "mkvf",     mfmtrecg_cli_mkv_fconfirm_fct,  NULL,       "mkv format file check",                   CLI_SUPERVISOR},
    {"mkv mconfirm" ,       "mkvm",     mfmtrecg_cli_mkv_mconfirm_fct,  NULL,       "mkv format mem check",                    CLI_SUPERVISOR},
    END_OF_CLI_CMD_TBL
};
...
/*-----------------------------------------------------------------------------
 * Name: mfmtrecg_cli_mkv_fconfirm_fct
 *
 * Description: This API returns the media format for file handle.
 *
 * Inputs: -command format : mkvf  /mnt/usb/Mass-000/file_path
 *
 * Outputs: -
 *
 * Returns:
...
static INT32 mfmtrecg_cli_mkv_fconfirm_fct(INT32 i4_argc, const CHAR** pps_argv)
{
    ...
    do
    {
        ...
        i4_ret = mkv_handler_file_confirm_fct(h_file, &t_format);
        ...
        x_dbg_stmt("Media Format is MKV!\r\n");
    } while(0);
...
static INT32 mfmtrecg_cli_mkv_mconfirm_fct(INT32 i4_argc, const CHAR** pps_argv)
{
    ...
    {
        x_dbg_stmt("Invalid argument! \r\n");
        DBG_INFO(("Usage: mkvm filepath mem_length(0/1) \r\n"));
        return CLIR_INV_ARG;
    }
    ...
    if (0 == x_strncmp(pps_argv[2], "0", (SIZE_T)1))
    {
        ...
        i4_ret = mkv_handler_mem_confirm_fct((VOID*)&t_feeder, 
                                             (SIZE_T)0, 
                                             &t_format);
    }
    else
    {
        ...
        i4_ret = mkv_handler_mem_confirm_fct((VOID *) pui1_buf,
                                             ui4_len,
                                             &t_format);
        x_mem_free(pui1_buf);
    }
 ...
 * Returns: 
 ----------------------------------------------------------------------------*/
INT32 mkv_mfmtrecg_cli_init(CLI_EXEC_T **ppt_command_array)
{
#ifdef CLI_LVL_ALL

    *ppt_command_array = at_mkv_mfmtrecg_cmd_tbl;
