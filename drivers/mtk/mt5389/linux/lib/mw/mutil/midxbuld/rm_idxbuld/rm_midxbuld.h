...
#ifndef _RM_MIDXBULD_H_
#define _RM_MIDXBULD_H_
...
#define RM_MIDXBULD_RANGE_MAX         (5)
#define RM_MIDXBULD_STREAM_NUM_MAX    0x100
#define RM_MIDXBULD_RANGE_ID_MASK     ((UINT32)0xff00)
#define RM_MIDXBULD_TABLE_ID_MASK     ((UINT32)0xff)

/* This structure holds the obj of RM idxbuild info. */
typedef struct _RM_IDXBULD_ELMT_TBL_OBJ_T
{
    MIDXBULD_ELMT_TBL_OBJ_T             t_elmt_tbl;
    UINT32                              ui4_tbl_size;/* total number of entry in table */
    UINT32                              ui4_tbl_id;

    struct _RM_IDXBULD_ELMT_TBL_OBJ_T* pt_next;
} RM_IDXBULD_ELMT_TBL_OBJ_T;

typedef struct _RM_IDXBULD_RANGE_OBJ_T
{
    ...
    UINT8                       ui1_tbl_total;
    RM_IDXBULD_ELMT_TBL_OBJ_T* pt_tbl_obj;
} RM_IDXBULD_RANGE_OBJ_T;

typedef struct _RM_IDXBULD_OBJ_T
{
    RM_IDXBULD_RANGE_OBJ_T*     pt_curr_range_obj;
    RM_IDXBULD_RANGE_OBJ_T*     pt_key_range_obj;
    RM_IDXBULD_RANGE_OBJ_T*     apt_range_obj[RM_MIDXBULD_RANGE_MAX];
    UINT8                       ui1_range_count;

    /*for current range, get table object by stream id*/
    RM_IDXBULD_ELMT_TBL_OBJ_T*  apt_tbl_obj_map[RM_MIDXBULD_STREAM_NUM_MAX];
...
/*-----------------------------------------------------------------------------
 ...
 ----------------------------------------------------------------------------*/
extern INT32  _rm_midxbuld_key_packet_table_alloc( 
                                        MIDXBULD_KEY_INFO_T* pt_key_info, 
                                        MIDXBULD_OBJ_T*      pt_midxbuld_obj );

extern INT32 _rm_midxbuld_get_idx_elmt_tbl(
                           MIDXBULD_OBJ_T*           pt_midxbuld_obj,
                           MIDXBULD_ELMT_TBL_INFO_T* pt_elmt_tbl);

extern INT32 _rm_midxbuld_elmt_tbl_alloc(
                             MIDXBULD_OBJ_T*                 pt_midxbuld_obj, 
                             MIDXBULD_ELMT_TBL_ALLOC_INFO_T* pt_elmt_tbl_alloc);
extern INT32 _rm_midxbuld_elmt_tbl_free(MIDXBULD_OBJ_T* pt_midxbuld_obj,
                                               HANDLE_T  h_elmt_tbl);



/* This function open a handler for a file */
extern INT32 rm_midxbuld_fopen (
                              CHAR*                ps_path_name,
                              MIDXBULD_KEY_INFO_T* pt_key_info,
                              VOID*                pv_obj);

/* This function open a handler for a memory buffer */
extern INT32 rm_midxbuld_mopen (
                                 VOID*                pv_mem_data,
                                 SIZE_T               z_mem_leng,
                                 MIDXBULD_KEY_INFO_T* pt_key_info,
                                 VOID*                pv_obj);
/* This function start indexing */
extern INT32 rm_midxbuld_start_indexing (VOID* pv_obj, UINT32* pui4_range_id);

/* This API retrieves the information of media idx builder handle.
It will return MINFO_NOT_READY, before parse media complete. */
extern INT32 rm_midxbuld_get_info ( VOID*                    pv_obj,   
                                     MIDXBULD_GET_INFO_TYPE_T e_info_type,  
                                     VOID*                    pv_buf,     
                                     SIZE_T*                  pz_buf_leng); 

/* This function close a handler */
extern INT32 rm_midxbuld_close (VOID* pv_obj); 

/* This API retrieves the information of media idx builder handle. 
It will return MINFO_NOT_READY, before parse media complete. */
extern INT32 rm_midxbuld_set_info (
                                      VOID*                    pv_obj,   
