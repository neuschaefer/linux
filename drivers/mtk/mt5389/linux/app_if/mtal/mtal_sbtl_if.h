 ...
 * $Author: dtvbm11 $
 * $Date: 2012/04/27 $
 * $RCSfile: mtal_sbtl_if.h,v $
 * $Revision: #1 $
 *
 *---------------------------------------------------------------------------*/

/** @file mtal_sbtl_if.h
 *  MTAL external subtitle interface header file.
 */
...
//#include "../../../target/mt5391/5391_driver/fm/u_fm.h"
#define FMR_OK     0
#define FMR_EOF    (-1)

#define EXT_SBTL_STR_BUF_SIZE  512  // this must exactly match that in mtmplayer_lib.c

#define MST_MASK_ENABLE  0x00000001
#define MST_MASK_TRK_NO  0x00000002
#define MST_MASK_SYNC    0x00000004


typedef enum  // copied from u_mm_sbtl_engine.h, tmp solution for fixing build error
{
    MM_SBTL_TYPE_DIVX,
    MM_SBTL_TYPE_VOBSUB,
    MM_SBTL_TYPE_TEXT,
...
typedef struct
{
    ...
    BOOL           fgActive;
    BOOL           fgToChgTrack;
    MM_SBTL_TYPE_T eExtSbtlType;
    UINT32         u4ExtSbtlSize;
    UINT16         u2TrackNo;
    UINT16         u2TrackNs;
    INT8           i1Sync;
} EXT_SBTL_SETTING_T;
