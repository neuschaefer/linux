 ...
 * $Author: dtvbm11 $
 * $Date: 2012/04/27 $
 * $RCSfile: io_papi_papionly_key.c,v $
 * $Revision: #1 $
 *
 *---------------------------------------------------------------------------*/
...
// Configurations
//-----------------------------------------------------------------------------
#define IRRX_POLLING_TIMEOUT 1000

//-----------------------------------------------------------------------------
// Constant definitions
//-----------------------------------------------------------------------------
/// Common error code
#define PAPI_PAPIONLY_KEY_FAIL                     (-0x800)
...
EXTERN INT32 _io_papi_RecordActualWakeupKey(void);
...
static INT32 _ioctl_Init(unsigned long u4Arg)
{
    ...
    int id;
    id = TIME_RegisterEntry("IR_Key_Init");
    ...
    i4Ret = IRRX_InitMtkIr(IRRX_RC5_TYPE);
...
#ifndef IRRX_POLLING_TIMEOUT
    i4Ret = IRRX_WaitMtkIr(&u4Key, &u4Raw);
#else
    i4Ret = IRRX_WaitMtkIrTimeout(&u4Key, &rRaw, IRRX_POLLING_TIMEOUT);
#endif

    rArg.au4Arg[0] = *(UINT32*)rRaw.au1Data;
    rArg.au4Arg[1] = (UINT32)i4Ret;
    
    COPY_TO_USER((void*)u4Arg, &rArg, sizeof(IOCTL_2ARG_T));

    // Timeout does not mean I/O fail
...
static INT32 _ioctl_Kill(unsigned long u4Arg)
{
    INT32 i4Ret;

    i4Ret = IRRX_SendMtkIr(0xffff0000);
...
int papi_ioctl_papionly_key(struct inode *inode, struct file *file,
