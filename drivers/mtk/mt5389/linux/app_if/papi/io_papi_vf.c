...
extern int papi_ioctl_vf_vio(struct inode *inode, struct file *file,
    unsigned int cmd, unsigned long arg);

extern int papi_ioctl_vf_vim(struct inode *inode, struct file *file,
    unsigned int cmd, unsigned long arg);

extern int papi_ioctl_vf_vmt(struct inode *inode, struct file *file,
    unsigned int cmd, unsigned long arg);
...
int papi_ioctl_vf(struct inode *inode, struct file *file,
    unsigned int cmd, unsigned long arg)
{
    int i4Ret = 0;
    ...
    case PAPI_IO_VF_VIM_ISDIGITALNATURALMOTIONALLOWED:
        i4Ret = papi_ioctl_vf_vim(inode, file, cmd, arg);
        break;
    ...
    case PAPI_IO_VF_VIO_HUESUPPORTED:
        i4Ret = papi_ioctl_vf_vio(inode, file, cmd, arg);
        break;
    ...
    case PAPI_IO_VF_VMT_GETAUTOMUTE:
        i4Ret = papi_ioctl_vf_vmt(inode, file, cmd, arg);
        break;

    default:
        printk("papi_ioctl_vf: unknown IO command %u\n", cmd);
        i4Ret = -EFAULT;
    }
