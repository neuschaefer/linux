...
INT32 _ioctl_papi_ioctl_fe_stcdec_GetTime(UINT32 u4Arg)
{
    IOCTL_2ARG_T rArg;
    PTS_T* pu8Pts = (PTS_T*)&rArg;
    
    //*pu8Pts = MPV_GetStc(0);

    // Copy arguments from user space
    COPY_TO_USER(u4Arg, &rArg, sizeof (IOCTL_2ARG_T));
...
int papi_ioctl_fe_stcdec(struct inode *inode, struct file *file,
    unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd)
    {
    case PAPI_IO_FE_STCDEC_GETTIME:
        i4Ret = _ioctl_papi_ioctl_fe_stcdec_GetTime(arg);
        break;

    default:
        printk("papi_ioctl_fe_stcdec: unknown IO command %u\n", cmd);
        i4Ret = -EFAULT;
    }
