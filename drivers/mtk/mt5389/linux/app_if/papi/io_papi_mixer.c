...
//-----------------------------------------------------------------------------
extern int papi_ioctl_mixer_vmx(struct inode *inode, struct file *file,
    unsigned int cmd, unsigned long arg);

extern int papi_ioctl_mixer_vmxvl(struct inode *inode, struct file *file,
    unsigned int cmd, unsigned long arg);
...
int papi_ioctl_mixer(struct inode *inode, struct file *file,
    unsigned int cmd, unsigned long arg)
{
    int i4Ret = 0;
    ...
    case PAPI_IO_MIXER_VMX_AUTOBLANKUNBLANK:
        i4Ret = papi_ioctl_mixer_vmx(inode, file, cmd, arg);
        break; 
    ...
    case PAPI_IO_MIXER_VMXVL_GETPANORAMAMIDSCALE:
        i4Ret = papi_ioctl_mixer_vmxvl(inode, file, cmd, arg);
        break; 

    default:
        printk("papi_ioctl_mixer: unknown IO command %u\n", cmd);
        i4Ret = -EFAULT;
    }
