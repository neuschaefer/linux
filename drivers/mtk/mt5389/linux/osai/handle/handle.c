                            ...
                            const CHAR**  pps_argv)
{
    ...
    {
        x_dbg_stmt ("Total: %5d  Free: %5d  Used: %5d  Peak used: %5d\n",
                    ui2_total,
                    ui2_free,
                    (ui2_total - ui2_free),
                    ui2_peak_used);
    ...
    {
        x_dbg_stmt ("Total: %5d  Free: %5d  Used: -----  Peak used: %5d\n",
                    ui2_total,
                    ui2_free,
                    ui2_peak_used);
    }
...
{
    SIZE_T z_len = sizeof (H_CTRL_T) * (2048 + 5);
    VOID *pv_mem = kmalloc(z_len, GFP_KERNEL);

    if (handle_init(2048, &pv_mem, &z_len) != HR_OK)
    {
        ASSERT(0);
    }

    //printk(KERN_DEBUG "Handle initialized!\n");
}


EXPORT_SYMBOL(handle_delink);
EXPORT_SYMBOL(handle_free);
EXPORT_SYMBOL(handle_link_init);
EXPORT_SYMBOL(handle_alloc);
EXPORT_SYMBOL(handle_link);
EXPORT_SYMBOL(handle_get_obj);
EXPORT_SYMBOL(x_handle_get_type);
