...
UINT32 CustomShippingRegionOptionQueryFunc(VOID)
{
    UNUSED(CustomShippingRegionOptionQueryFunc);

    UINT64 u8Offset;
    static BOOL bInit = TRUE;
    static UINT8 u1Index = DRV_CUSTOM_SHIPPING_REGION_OPTION_GA;
    
    if (bInit)
    {
        u8Offset = (UINT64)CUSTOM_SHIPPING_REGION_OPTION_ADDR;
        
        if (EEPROM_Read(u8Offset, (UINT32)(void *)&u1Index, 1) == 0)
        {
            if (u1Index > DRV_CUSTOM_SHIPPING_REGION_OPTION_CHINA)
            {
                 u1Index = DRV_CUSTOM_SHIPPING_REGION_OPTION_GA;
            }
        }
...
void GPIO_CustomReadStrapPin(void)
{
    ...
    _u4gModelType |= (CustomShippingRegionOptionQueryFunc() == DRV_CUSTOM_SHIPPING_REGION_OPTION_CHINA) ?
        DRV_CUSTOM_MODEL_SHIPPING_REGION_CHINA : DRV_CUSTOM_MODEL_SHIPPING_REGION_GA;
