...
// Configurations
//---------------------------------------------------------------------------
#if (CONFIG_CHIP_VER_CURR == CONFIG_CHIP_VER_MT5365) || (CONFIG_CHIP_VER_CURR == CONFIG_CHIP_VER_MT8555) || (CONFIG_CHIP_VER_CURR == CONFIG_CHIP_VER_MT5395)
...
static UINT32 _u4SdClockSel[] = {FCI_SD_CLOCK_SELECTION_STRING};
static UINT32 _u4SdHighClockIdx = SD_DEFAULT_HIGH_CLOCK_INDEX;
static UINT32 _u4SdNormClockIdx = SD_DEFAULT_NORM_CLOCK_INDEX;

UINT32 _u4SdHighClock = 0;        // 50 Mhz
UINT32 _u4SdNormalClock = 0;        // 27/25 Mhz
...
static INT32 _Sdc_IsSDCard(VOID)
{
    ...
    // initial SD 2.0 card.
    if ((_u4SdDetectCardType == FCI_SD20_CARD) &&
    ...
    // Judge SDHC card by CCS bit of ACMD41 response
    if (_u4SdDetectCardType == FCI_SD20_CARD)
    {
        if (u4Val & SDC_CCS)
        {
            _u4SDCardType = FCI_SDHC_CARD;
            LOG(5, "SD 2.0 card - SD HC card\n");
        }
        else
        {
            _u4SDCardType = FCI_SD_CARD;
            LOG(5, "SD 2.0 card - SD SC card\n");
        }
    }
    else
    {
        _u4SDCardType = FCI_SD_CARD;
        LOG(5, "SD 1.x card - SD SC card\n");
    }
        ...
        if(pbBuf[13] & 0x02)      // Check bit 401 : bit 1 = 1 means HIGH speed function is supported
        {
              if(pbBuf[16] == 0x01)  // Check bit 379 ~ 376 : 1 means the function can be switched
              {
                  ...
                  if(pbBuf[16] == bCheckByte)  // check if the function switch ok
                  {
                      ...
                      if (prSDC != NULL)
                      {
                          prSDC->fgHighSpeedMode = bHighSpeed;
                      }
...
static INT32 _Sdc_GetAddrCID(UINT32 u4CardType, UINT16 u2RCA, UINT32 *pu4CID)
{
    ...
    if ((u4CardType == FCI_SD_CARD) || (u4CardType == FCI_SDHC_CARD))
    {
...
static INT32 _Sdc_ReadSingleBlk(UINT32 u4Addr, UINT8 *pu1Buf)
{
    ...
    if (!((prSDC->fgPIOMode) || (prSDC->fgFifoFullModeTest)))
    {
    ...
    if (prSDC->fgFifoFullModeTest)
    {
        while(!(FCI_READ32(RW_FCI_INTR_STAT_REG) & INTR_FCI_RDFIFO_FULL))
        {
            x_thread_delay(1);
        }
        LOG(1, "%s(%d) Read FIFO full !!\n", __FILE__, __LINE__);
    }
    ...
    if ((prSDC->fgPIOMode) || (prSDC->fgFifoFullModeTest))
    {
        ...
        for (index = 0; index < (UINT32)(prSDC->u2BlkSz[SDC_COMMOM]);)
        {
...
static INT32 _Sdc_ReadMultiBlk(UINT32 u4Addr, UINT8 *pu1Buf, UINT32 u4BlkNs)
{
    ...
    {    
        ...
        for (index = 0; index < (UINT32)(prSDC->u2BlkSz[SDC_COMMOM] *u4BlkNs);)
        {
          if (!fgFCIHW_ReadBufEmpty())
          {
              *((UINT32 *)((UINT32)pu1Buf + index)) = FCI_READ32(RW_RD_DATA_REG);
...
INT32 _SDC_CheckStatus(UINT32 u4CmdIdx)
{
    ...
    do
    {
        ...
        if (i4Count > 5)
        {
            LOG(1, "SDCHW_CmdIdx failed. u4Val:0x%08x, u4CmdIdx:0x%08x\n", u4Val, u4CmdIdx);
            return E_STATUS;
        }
...
VOID _SDC_ChangeClock(INT32 i4Clock)
{
    ...
    {
        // about 50 Khz
        FCIHW_ChangeSDCClock((INT32)_u4SdHighClock);
    }
    else if(i4Clock == SDC_NORM_CLOCK)
    {
        // about 25 Khz
        FCIHW_ChangeSDCClock((INT32)_u4SdNormalClock);
    }
    else if(i4Clock == SDC_LOW_CLOCK)
    {
        // 421.875 Khz for identify device
        FCIHW_ChangeSDCClock((INT32)_u4SdLowClock);
    }
}
...
//-------------------------------------------------------------------------
/** SD_FifoFullTest
 *  @param  fgFifoFullTest   TRUE : Enable test, FALSE : Disable test
 *  @retval   0 : successfully.
 */
//-------------------------------------------------------------------------

INT32 SD_FifoFullTest(BOOL fgFifoFullTest, BOOL fgHighSpeedTestBit)
{
    SDC_T *prSDC;

    prSDC = FCI_GetSDCSlot();

    prSDC->fgFifoFullModeTest = fgFifoFullTest;
    prSDC->fgHighSpeedTestBit = fgHighSpeedTestBit;

    LOG(1, "SD Read Fifo Full test %s, Test bit = %d\n", prSDC->fgFifoFullModeTest ? "Enabled" : "Disabled", prSDC->fgHighSpeedTestBit);
...
//-------------------------------------------------------------------------
/** SD_CmdTimeOutTest
 *  @param  fgCmdTimeOutTest   TRUE : Enable test, FALSE : Disable test
 *  @retval   0 : successfully.
 */
//-------------------------------------------------------------------------

INT32 SD_CmdTimeOutTest(BOOL fgCmdTimeOutTest)
{
    SDC_T *prSDC;

    prSDC = FCI_GetSDCSlot();

    prSDC->fgCmdTimeOutTest = fgCmdTimeOutTest;

    LOG(1, "SD Cmd Time out test %s\n", prSDC->fgCmdTimeOutTest ? "Enabled" : "Disabled");
...
//-------------------------------------------------------------------------
/** SD_DataTimeOutTest
 *  @param  fgDataTimeOutTest   TRUE : Enable test, FALSE : Disable test
 *  @retval   0 : successfully.
 */
//-------------------------------------------------------------------------

INT32 SD_DataTimeOutTest(BOOL fgDataTimeOutTest)
{
    SDC_T *prSDC;

    prSDC = FCI_GetSDCSlot();

    prSDC->fgDataTimeOutTest = fgDataTimeOutTest;

    LOG(1, "SD Data Time out test %s\n", prSDC->fgDataTimeOutTest ? "Enabled" : "Disabled");
...
INT32 SDC_HiSpeedModeSet(BOOL fgHiSpeedModeSet)
{
    SDC_T *prSDC;

    prSDC = FCI_GetSDCSlot();
    
    prSDC->fgHighSpeedMode = fgHiSpeedModeSet;

    LOG(1, "FCI host set High Speed Mode = %s\n", prSDC->fgHighSpeedMode ? "TRUE" : "FALSE");
        
    return 0;
}

INT32 SDC_SelectCard(BOOL fgSelect, UINT32 u2NewRCA)
{
    SDC_T *prSDC;
    INT32 i4Ret;
    ...
    {
        i4Ret = _SDC_SelectCard(u2NewRCA);
    }
...
INT32 SDC_Init(SDC_T *prSDC)
{
    ...
    while (i4Retry++ < 3)
    {
        if (((_u4SdDetectCardType == FCI_SD20_CARD) || (_u4SdDetectCardType == FCI_SD_CARD)) &&
            (_Sdc_IsSDCard() == S_OK))
        {
            // Detect SD card in slot.
            prSDC->u4State = SLOT_STATE_INIT;

            prSDC->u4CardType = _u4SDCardType;
    ...
    if ((prSDC->u4CardType != FCI_SD_CARD) && (prSDC->u4CardType != FCI_SDHC_CARD) && (prSDC->u4CardType != FCI_MMC_CARD))
    {
    ...
    LOG(6, "CSD : 0x%08X, 0x%08X, 0x%08X, 0x%08X\n", prSDC->au4CSD[0], prSDC->au4CSD[1], prSDC->au4CSD[2], prSDC->au4CSD[3]);
    ...
    // New definition of CSD for CSD structure = 1 & SDHC card
    if ((_u4SDCardType == FCI_SDHC_CARD) && (SD_CSD_STRUCTURE(prSDC->au4CSD) != 0))
    {
        /*
        ...
        */
        ...
        LOG(6, "CSD Structure : %d, block size = 0x%08X.\n", SD_CSD_STRUCTURE(prSDC->au4CSD), u4BlkLen);
        prSDC->u4CardSize = u4BlkNr * 1024;
        LOG(6, "card size/block = 0x%08X.\n", prSDC->u4CardSize);
    }
    else
    {
    ...
    // SDHC with CSD 2.0 & MMC card with size larger then 2GB => Block address mode
    if (((_u4SDCardType == FCI_SDHC_CARD) && (SD_CSD_STRUCTURE(prSDC->au4CSD) != 0)))
    // ||(_u4SDCardType == FCI_MMC_CARD && ((prSDC->u4CardSize) >> 21) > 2))
    {
        prSDC->fgBlockAddressing = TRUE;
        LOG(5, "Block Addressing Mode\n");
    }
    else    // Byte address mode
    {
        prSDC->fgBlockAddressing = FALSE;
        LOG(5, "Byte Addressing Mode\n");
    }
    ...
    if ((prSDC->u4CardType == FCI_SD_CARD) || (prSDC->u4CardType == FCI_SDHC_CARD))
    {
#ifdef CC_HIGH_SPEED_MODE_ENABLE
        ...
        if((_u4SDCardType == FCI_SDHC_CARD) && _fgHighSpeedEnable)
        {
            ...
            else
            {
                ...
                // SD High speed mode setting - for BD, TV
                u4Val = FCI_READ32(RW_FCI_CTRL_REG);
                u4Val |= CTRL_SD_HS_ADJ;
                u4Val &= ~(CTRL_SD_RECEDGE);
                FCI_WRITE32(RW_FCI_CTRL_REG, u4Val);
                
                // SD High speed mode setting - for TV
                FCI_WRITE32(RW_FCI_INTR_CTRL_REG, FCI_READ32(RW_FCI_INTR_CTRL_REG) | INTR_FCI_HIGH_SPEED_MODE);            
            }
        }
#endif  // #ifdef CC_HIGH_SPEED_MODE_ENABLE
    ...
    LOG(7, "SDCInit OK Card:%d (SDHC:%d SD:%d MMC:%d) (%s)\n", prSDC->u4CardType, FCI_SDHC_CARD, FCI_SD_CARD, FCI_MMC_CARD,
...
INT32 SDC_SetSDBusWidth(BOOL fg4BitEnable)
{
    ...
    if ((!_fgFCI4BitBusEnable) && fg4BitEnable)
    {
    ...
    else if (u4BlkNs)
    {
        if (prSDC->fgSingleBlockAccess)
        {
            for(i = 0; i < u4BlkNs; i++)
            {
                ...
                if(i4Ret != S_OK)
                {
                    LOG(1, "%s(%d) %s failed return %d at block:%d / total:%d\n", __FILE__, __LINE__, __FUNCTION__, i4Ret, i+1, u4BlkNs);
                    break;
                }
            }
 ...
 */
//-------------------------------------------------------------------------
INT32 SDC_ErrorHandling(INT32  i4Retry)
{
    SDC_T *prSDC;
    UINT32 u4ClockArraySize = sizeof(_u4SdClockSel) / sizeof(UINT32);    
    ...
    // Set clock lower
    _u4SdHighClockIdx++;
    if (_u4SdHighClockIdx >= u4ClockArraySize)
    {
        _u4SdHighClockIdx = u4ClockArraySize;
    }

    _u4SdNormClockIdx++;
    if (_u4SdNormClockIdx >= u4ClockArraySize)
    {
        _u4SdNormClockIdx = u4ClockArraySize;
    }
    
    _u4SdHighClock = _u4SdClockSel[_u4SdHighClockIdx];
    _u4SdNormalClock = _u4SdClockSel[_u4SdNormClockIdx];        

    FCIHW_ResetSlot(SDC_SLOT);
    
    LOG(1, "%s(%d) Retry = %d, High Clock = %d Mhz, Norm Clock = %d Mhz\n", __FUNCTION__, __LINE__, i4Retry, _u4SdHighClock, _u4SdNormalClock);
    
    return SDC_Init(prSDC);
}
 ...
 */
//-------------------------------------------------------------------------
VOID SDC_ResetClockSetting(VOID)
{
    // Restore original high / normal clock setting
    _u4SdHighClockIdx  = SD_DEFAULT_HIGH_CLOCK_INDEX;
    _u4SdNormClockIdx = SD_DEFAULT_NORM_CLOCK_INDEX;
...
#endif // #ifndef CC_CARD_DETECT_PIN
