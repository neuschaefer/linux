...
static UINT32 u4DecCycle = 0, u4MaxDecCycle = 0, u4PicWlst = 0, u4PicHlst = 0;
#ifdef CC_STRESS_TEST_CRC_CHK
static UINT32 u4DramCrcMC_R = 0, u4DramCrcPP_W = 0;
#endif
static BOOL fgstart = FALSE;
...
static void _vSetupPRefPicList(const VDEC_REF_PIC_LIST_T* prRefPicList, VDEC_FBUF_INFO_T* prFBufInfo,
...
static void _vSetupBRefPicList(const VDEC_REF_PIC_LIST_T* prRefPicList, VDEC_FBUF_INFO_T* prFBufInfo,
...
static void _AVLDIsr(UINT16 u2Vector)
{
    ...
    if (u2Vector == AVLD_VECTOR)
    {
        if (AVC_VLD_READ32(RO_AVLD_COMPLETE))
        {
            ...
            VLD_TOP_WRITE32(RW_VLD_TOP_MDEC_TIMEOUT_INT_SWITCH,
                            VLD_TOP_READ32(RW_VLD_TOP_MDEC_TIMEOUT_INT_SWITCH) & MDEC_INT_SWITCH_OFF);
#else
        ...
        else if (AVC_VLD_READ32(RO_AVLD_ERR_MESSAGE) & AVLD_ERR_MESSAGE_MSK)
        {
            _bTimeOutFlag = TRUE;
#ifdef IS_POST_MT5368
            VLD_TOP_WRITE32(RW_AVLD_TIMEOUT_CLR, 1);
            VLD_TOP_WRITE32(RW_AVLD_TIMEOUT_CLR, 0);
#else
        ...
        //VDEC_MISC_WRITE32(RW_VDEC_MISC_INTERRUPT_CLR, 0x1011);
        //VDEC_MISC_WRITE32(RW_VDEC_MISC_INTERRUPT_CLR, 0x1001);
...
static void _vPartitionDPB(VDEC_AVC_DecPrm *prDecPrm)
{
    UINT32 i, j, u4FbCnt = 0, u4DPBExpCnt = 0;
            ...
            IS_SEAMLESS_MODE)
    {
        ...
        if (IS_SEAMLESS_MODE && (prDecPrm->prSPS->u4NumRefFrames > 4))
        {
            LOG(2, "Warning!! RefNum %d in seamless mode\n");
        }
    }
    else
#endif
    ...
    if (u4DPBExpCnt > prDecPrm->au4MaxFBufNum[prDecPrm->u1CurViewIdx])
    {
        u4FbCnt = (UINT32)(FBM_GetFrameBufferNs(prDecPrm->aucFbgId[prDecPrm->u1CurViewIdx]) - 2);  // 2 lock frame
        if (u4FbCnt >= u4DPBExpCnt)
        {
            prDecPrm->au4MaxFBufNum[prDecPrm->u1CurViewIdx] = u4DPBExpCnt;
...
static void _vAllocateFBuf(VDEC_AVC_DecPrm *prDecPrm)
{
...
#endif
#if 0  //move to _vCheckAllocFB
    // Check if DPB full
    i4MinPOC = 0x7fffffff;
    for (i = 0; i < prDecPrm->u4MaxFBufNum; i++)
    {
        ucFbStatus = FBM_GetFrameBufferStatus(prDecPrm->ucFbgId, prDecPrm->arFBufInfo[u1ViewIdx][i].ucFbId);
    ...
    if (prDecPrm->arFBufInfo[u1ViewIdx][u4MinPOCFBufIdx].u1FBufStatus != NO_PIC)
    {
        LOG(2, "///// ClrBuf in AllocFBuf, Idx %d , Status %d(>0)\n",
            u4MinPOCFBufIdx,
            (UINT32)FBM_GetFrameBufferStatus(prDecPrm->ucFbgId, prDecPrm->arFBufInfo[u1ViewIdx][u4MinPOCFBufIdx].ucFbId));
        //find an empty fb entry, put non-ref buf to empty
        FBM_SetFrameBufferStatus(prDecPrm->ucFbgId, prDecPrm->arFBufInfo[u1ViewIdx][u4MinPOCFBufIdx].ucFbId,
                                 FBM_FB_STATUS_EMPTY);
    ...
    VDP_FrcRacingLog('0' + prDecPrm->prCurrFBufInfo->ucFbId);

    prVdecEsInfo = _VDEC_GetEsInfo(prDecPrm->ucEsId);

    if (prVdecEsInfo->u2Count > 9)
    {
        VDP_FrcRacingLog('0' + 9);
    }
    else
    {
        VDP_FrcRacingLog('0' + prVdecEsInfo->u2Count);
    }
#endif
    ...
    //picture type
    if (prDecPrm->prSliceHdr && prFbmPicHdr)
    {
            ...
            (prFbmPicHdr->fgProgressiveFrm != prFbmSeqHdr->fgProgressiveSeq))
    {
		...
		if((prFbmSeqHdr->fgProgressiveSeq == FALSE)&&(prFbmPicHdr->fgProgressiveFrm)&&((prDecPrm->u4PicW > 1280) || (prDecPrm->u4PicH > 720)))
		{
...
static void _vPrepareFBufInfo(VDEC_AVC_DecPrm *prDecPrm)
{
    ...
    LOG(9, "_vPrepareFBufInfo, W:%d, H:%d, SPS-W:%d, SPS-H:%d, fgFrmOnly:%d\n", 
      prDecPrm->u4PicW, prDecPrm->u4PicH, prDecPrm->prSPS->u4PicWidthInMbsMinus1, prDecPrm->prSPS->u4PicHeightInMapUnitsMinus1, prDecPrm->prSPS->fgFrameMbsOnlyFlag);
            ...
            (!prDecPrm->bPreAllocFBG))
    {
...
#endif
        {
            LOG(9, "_vPrepareFBufInfo, bPreAllocFBG true\n");
            ...
            prDecPrm->aucFbgId[u1ViewIdx] = FBM_CreateGroup(ucFbgType, FBM_VDEC_H264, prDecPrm->u4PicW, prDecPrm->u4PicH);
            prVdecEsInfo->aucFbgId[u1ViewIdx] = prDecPrm->ucFbgId;
            ...
            if (!prDecPrm->fgIsMVC)
            {
                FBM_SetFrameBufferFlag(prDecPrm->aucFbgId[u1ViewIdx], FBM_FLAG_TRACKING_MODE_ENABLE);
            }
#endif
...
#if 0 // 2D model: handled by sysbuild APP according to crop info, 3D model: display 3D
            if (((prVdecEsInfo->e3DType == VDEC_3D_SBS_LF) || (prVdecEsInfo->e3DType == VDEC_3D_SBS_RF) ||
                     ...
                     ((prDecPrm->prSPS->u4FrameCropLeftOffset > 0) || (prDecPrm->prSPS->u4FrameCropRightOffset > 0) ||
                      (prDecPrm->prSPS->u4FrameCropTopOffset > 0) || (prDecPrm->prSPS->u4FrameCropBottomOffset > 8)))     // 1088 to 1080 should be allowed in 3D mode
            {
                if (((prVdecEsInfo->e3DType == VDEC_3D_SBS_RF) && (prDecPrm->prSPS->u4FrameCropRightOffset > 0)) ||
                ...
                else if (((prVdecEsInfo->e3DType == VDEC_3D_TB_RF) && (prDecPrm->prSPS->u4FrameCropBottomOffset > 8)) ||
	    ...
	    if(prVdecEsInfo->fgEnableCrop)
            ...
            {
                LOG(9, "_vPrepareFBufInfo, ucAspRatInf:%d, u4AspectRatioIdc:%d\n",
            ...
            if (prDecPrm->prSPS->fgVuiParametersPresentFlag)
            {
                ...
                //Frame Crop
            if(prDecPrm->prSPS->fgFrameCroppingFlag && prVdecEsInfo->fgEnableCrop)
                {
    ...
    if (prFbmSeqHdr != NULL)
    {
        if (prDecPrm->prSPS->rVUI.u4AspectRatioIdc == 0xFF)  //extended_SAR
        {
            ...
            {
                LOG(9, "_vPrepareFBufInfo, LastWH[%d, %d], CurWH[%d, %d]\n",
                    prDecPrm->rLastInfo[u1ViewIdx].u4SARWidth,
                    prDecPrm->rLastInfo[u1ViewIdx].u4SARHeight,
                    prDecPrm->prSPS->rVUI.u4SarWidth,
                    prDecPrm->prSPS->rVUI.u4SarHeight);
				if(prDecPrm->prSPS->rVUI.u4SarHeight > prDecPrm->prSPS->rVUI.u4SarWidth)
				{
					prDecPrm->prSPS->rVUI.u4SarWidth = prDecPrm->prSPS->rVUI.u4SarHeight = 1;// ignore the Sar WH info.
				}
        ...
        {
            LOG(9, "_vPrepareFBufInfo, LastAspRatInf:%d, CurAspRatInf:%d\n",
            ...
            LOG(9, "_vPrepareFBufInfo, u4SAR:%d, W:%d, H:%d, u4DAR:%d\n",
                u4SAR,
                prDecPrm->u4PicW,
                prDecPrm->u4PicH,
                u4DAR);
        ...
        if (fgAfdChg)
        {
            ...
            LOG(9, "_vPrepareFBufInfo, u4DAR:%d, ucAspRatInf:%d, fgIsMMPlay:%d, eContainerType:%d\n",
                u4DAR,
                prFbmSeqHdr->ucAspRatInf,
                prDecPrm->fgIsMMPlay,
                prVdecEsInfo->eContainerType);

            //add default value for MM playback
            if ((prDecPrm->fgIsMMPlay) 
                #if 1 // fix 458190 @2012/11/16
                && (SWDMX_FMT_MPEG2_TS_TIME_SHIFT != prVdecEsInfo->eContainerType)
                && (SWDMX_FMT_MPEG2_TS_ENCRYPT_192 != prVdecEsInfo->eContainerType)
                #endif
               )
            {
                prFbmSeqHdr->fgPixelAspectRatio = TRUE;

                LOG(9, "_vPrepareFBufInfo, fgVuiParametersPresentFlag:%d, fgAspectRatioInfoPresentFlag:%d, prVdecEsInfo:0x%x\n",
                    prDecPrm->prSPS->fgVuiParametersPresentFlag,
                    prDecPrm->prSPS->rVUI.fgAspectRatioInfoPresentFlag,
                    prVdecEsInfo);
                        ...
                        ((prDecPrm->prSPS->fgVuiParametersPresentFlag) && (!prDecPrm->prSPS->rVUI.fgAspectRatioInfoPresentFlag)))
                {
                    ...
                    {
                        LOG(9, "_vPrepareFBufInfo, e_src_asp:%d, e_asp_usr_type:%d\n",
                            prVdecEsInfo->rMMSeqInfo.e_src_asp, prVdecEsInfo->rMMSeqInfo.e_asp_usr_type);                    
                    ...
                    LOG(9, "_vPrepareFBufInfo, ucAspRatInf:%d\n",
                        prFbmSeqHdr->ucAspRatInf);
                }
                else
                {
                    prFbmSeqHdr->ucAspRatInf = MPEG_ASPECT_RATIO_TRANSMIT;
                    LOG(9, "_vPrepareFBufInfo, ucAspRatInf:MPEG_ASPECT_RATIO_TRANSMIT\n");
                }
            }
            else
            {
            ...
            {
                u4EstimateWidth = (u4DAR * 1080) / 100;
                u4EstimateHeigh = (1920 * 100) / u4DAR;
                if (u4EstimateWidth <= 1920)
                {
                    prFbmSeqHdr->u4ResizeWidth = u4EstimateWidth;
                    prFbmSeqHdr->u4ResizeHeight = 1080;
                }
                else
                {
                    prFbmSeqHdr->u4ResizeWidth = 1920;
                    prFbmSeqHdr->u4ResizeHeight = u4EstimateHeigh;
                }
            }
            LOG(9, "AFD %d, AspRatio Chg %d, [%d:%d], IsSAR %d\n",
	...
	if (prVdecEsInfo->fgEnableCrop && 
	  ...
	  if((u4PicWlst != prDecPrm->u4PicW)||(u4PicHlst != prDecPrm->u4PicH))
	   {
		   LOG(0,"resolution change %d,%d,%d,%d\n",u4PicWlst,u4PicHlst,prDecPrm->u4PicW,prDecPrm->u4PicH);
		   u4PicWlst = prDecPrm->u4PicW;
		   u4PicHlst = prDecPrm->u4PicH;
		   if(prVdecEsInfo->fgEnableCrop && prDecPrm->prSPS->fgFrameCroppingFlag)
		   {
			 LOG(0,"fgFrameCroppingFlag %d,u4FrameCropLeftOffset %d,u4FrameCropTopOffset %d,u4CropWidth %d,u4CropHeight %d",
				 prDecPrm->prSPS->fgFrameCroppingFlag,prDecPrm->prSPS->u4FrameCropLeftOffset,prDecPrm->prSPS->u4FrameCropTopOffset
				 ,((prDecPrm->u4PicW - prFbmSeqHdr->u4CropX) -
													 prDecPrm->prSPS->u4FrameCropRightOffset),(prDecPrm->u4PicH - prFbmSeqHdr->u4CropY -
					  ((prDecPrm->prSPS->fgFrameMbsOnlyFlag) ?
					   prDecPrm->prSPS->u4FrameCropBottomOffset :
					   (prDecPrm->prSPS->u4FrameCropBottomOffset << 1))));
		   }
    ...
    if (((u4CurrFrameNum != prDecPrm->rLastInfo[u1ViewIdx].u4LastFrameNum)
            && (u4CurrFrameNum != u4UnusedShortTermFrameNum))
#ifdef ENABLE_MULTIMEDIA
            && ((!prDecPrm->fgIsMMPlay) ||
                ((prVdecEsInfo->eContainerType >= SWDMX_FMT_MPEG2_TS) && (prVdecEsInfo->eContainerType < SWDMX_FMT_VC1_ES) && (prVdecEsInfo->u4MMSpeed < STC_SPEED_TYPE_FORWARD_2X)))
            && (!prDecPrm->fgIsMVC))
#else
...
#endif
    {
        if (prDecPrm->rLastInfo[u1ViewIdx].u4LastFrameNum < (UINT32)0xffffffff)
        {
            ...
            (u4CurrFrameNum > u4UnusedShortTermFrameNum) ?
            (u4CurrFrameNum - u4UnusedShortTermFrameNum) :
                ((prDecPrm->prSPS->u4MaxFrameNum - u4UnusedShortTermFrameNum) + u4CurrFrameNum),u4CurrFrameNum,u4UnusedShortTermFrameNum);
...
static void _vSetPRefPicList(VDEC_AVC_DecPrm* prDecPrm)
{
...
#if defined(CC_MVC_SUPPORT) && !defined(CC_VDEC_MVC_DISABLE)
    u4Temp = 0;
    while (AVC_VLD_READ32(RW_AVLD_RESET_PIC_NUM) != 0)
    {
        u4Temp++;
        if (u4Temp > 100)
        {
    ...
    u4Temp = 0;
    _vSetupPRefPicList(prDecPrm->arRefPicList, (VDEC_FBUF_INFO_T *)prDecPrm->arFBufInfo, prDecPrm->u1PicStruct, &u4Temp, 0, 1);
    _vSetupPRefPicList(prDecPrm->arRefPicList, (VDEC_FBUF_INFO_T *)prDecPrm->arFBufInfo, prDecPrm->u1PicStruct, &u4Temp, 4, 5);
}
                               ...
                               UINT32 u4TFldListIdx, UINT32 u4BFldListIdx, BOOL *pfgDiff)
{
    UINT32 i;
    UINT32 u4TotalFBuf;
    BOOL fgIsTFldInterview, fgIsBFldInterview;
...
static void _vSetBRefPicList(VDEC_AVC_DecPrm* prDecPrm)
{
    ...
    u4Temp = 0;
    fgDiff = FALSE;
    _vSetupBRefPicList(prDecPrm->arRefPicList, (VDEC_FBUF_INFO_T *)prDecPrm->arFBufInfo, prDecPrm->u1PicStruct,
...
// Return      : None
// *********************************************************************
void _VDEC_ECBufSetting(const VDEC_AVC_DecPrm* prDecPrm, UINT32 u4RefCnt, UINT32 u4RefEntryCnt)
{
...
#if defined(CC_MVC_SUPPORT) && !defined(CC_VDEC_MVC_DISABLE)
void _VDEC_MVCRegSetting(const VDEC_AVC_DecPrm* prDecPrm)
{
    ...
    {
        AVC_VLD_WRITE32(RW_AVLD_EMU_PREV,
                        AVC_VLD_READ32(RW_AVLD_EMU_PREV) | MVC_SWITCH | REORDER_MVC_SWITCH |
    ...
    AVC_VLD_WRITE32(RW_AVLD_ANCHOR_L0_NUM, u4RefNum);
    ...
    for (u4Idx = 0; u4Idx < 16; u4Idx++)
    {
        if (u4Idx < u4RefNum)
        {
            AVC_VLD_WRITE32(RW_AVLD_ANCHOR_L0 + (u4Idx << 2), au4RefList[u4Idx]);
        }    
    }
    //L1
    u4RefNum = prDecPrm->fgAnchorPic ?
    ...
    AVC_VLD_WRITE32(RW_AVLD_ANCHOR_L1_NUM, u4RefNum);
    ...
    for (u4Idx = 0; u4Idx < 16; u4Idx++)
    {
        if (u4Idx < u4RefNum)
        {
            AVC_VLD_WRITE32(RW_AVLD_ANCHOR_L1 + (u4Idx << 2), au4RefList[u4Idx]);
        }    
    }

    //MVC Misc
    AVC_VLD_WRITE32(RW_AVLD_ANCHOR_MISC,
...
#ifdef IS_POST_MT5368
    LOG(0, "VLD_TOP 50(20) timeout threshold 0x%x\n", VLD_TOP_READ32(RW_AVLD_TIMEOUT_THD));
    LOG(0, "VLD_TOP 58(22) mc busy threshold 0x%x\n\n", VLD_TOP_READ32(RW_AVLD_MC_BUSY_THD));
    LOG(0, "VLD_TOP 80(32) decode cycle 0x%x\n", VLD_TOP_READ32(RO_AVLD_DEC_CYCLE));
#else
...
void _VDEC_InitVDecHW(BOOL bIsInit)
{
    ...
    _VDEC_RegIsr(AVLD_VECTOR, _AVLDIsr);
#else
...
void _VDEC_SetBsInfo(UINT8 u1Num, UINT32 u4RDPtrAddr, UINT32 u4WPtrAddr,
                     UINT32 u4BsFifoStart, UINT32 u4BsFifoEnd, UINT32 u4PredSA, UINT8 u1ECCMode, UINT32 u4ECType, BOOL fgIsMM)
{
...
#endif
    {
        ...
        while (!_VDEC_ResetVLD1(PHYSICAL(u4RDPtrAddr), PHYSICAL(u4WPtrAddr),
                                PHYSICAL(u4BsFifoStart), PHYSICAL(u4BsFifoEnd), PHYSICAL(u4PredSA), u1ECCMode, u4ECType, fgIsMM))
        {
            LOG(2, "SwRst %d\n", u4Cnt++);
...
#ifdef VDP_RACE_LOG
                VDP_FrcRacingLog('Q');
                VDP_FrcRacingLog('0' + prDecPrm->arFBufInfo[u1ViewIdx][u4MinPOCFBufIdx].ucFbId);
#endif
                ...
                else
                {
                    ...
                    else
                    {
...
#ifdef CC_ENABLE_TRACKING_MODE
                        if (prFbmPicHdr && (!prDecPrm->fgIsMVC))
                        {
                            prFbmPicHdr->fgTrackingMode = TRUE;
                            if (FbCnt == 0)
                            {
                                prDecPrm->ucTrackingFbId = prDecPrm->arFBufInfo[u1ViewIdx][u4MinPOCFBufIdx].ucFbId;
                            }
                        }
#endif
                        FBM_SetFrameBufferStatus(prDecPrm->aucFbgId[u1ViewIdx], prDecPrm->arFBufInfo[u1ViewIdx][u4MinPOCFBufIdx].ucFbId,
                        ...
                        if (prMpvCc->fgPlayCc)
                        {
                            ...
                            prCcInfo->u4BufRptr = _VDEC_SaveUserData(prDecPrm->arFBufInfo[u1ViewIdx][u4MinPOCFBufIdx].au1UsrDataFrm,
                                 ...
                                 (prExtInfo->fgPesUserData[prDecPrm->ucEsId] && (prDecPrm->arFBufInfo[u1ViewIdx][u4MinPOCFBufIdx].u4UserDataPESWptr > 0))))
                        {
                            ...
                            {
                                rUsrData.u4Rptr = _VDEC_SaveUserData(prDecPrm->arFBufInfo[u1ViewIdx][u4MinPOCFBufIdx].au1UsrDataFrm,
                                                                     prDecPrm->arFBufInfo[u1ViewIdx][u4MinPOCFBufIdx].u4UserDataFrmWptr);
                            ...
                            {
                                rUsrData.u4Rptr = _VDEC_SaveUserData(prDecPrm->arFBufInfo[u1ViewIdx][u4MinPOCFBufIdx].au1UsrDataPES,
...
void _VDEC_SetPicInfo(VDEC_AVC_DecPrm* prDecPrm)
{
    ...
    _VDEC_SetSHDRAVLD1(u4SHDRInfo, prDecPrm->prSliceHdr->u4FirstMbInSlice);

#ifdef IS_POST_MT5368
    VLD_TOP_WRITE32(RW_AVLD_PIC_SIZE, (prDecPrm->prSPS->u4PicHeightInMapUnitsMinus1 << 16) | prDecPrm->prSPS->u4PicWidthInMbsMinus1);
#else
    AVC_VLD_WRITE32(RW_AVLD_PIC_SIZE, (prDecPrm->prSPS->u4PicHeightInMapUnitsMinus1 << 8) | prDecPrm->prSPS->u4PicWidthInMbsMinus1);
#endif
}
...
// **************************************************************************
UINT8 _VDEC_WaitDecDone(UINT8 u1PicStruct, UINT32 u4W, UINT32 u4H, BOOL bWaitDecLock, UCHAR ucEsId)
{
    UINT8 u1Ret = 0;
    ...
    if (bWaitDecLock)
    {
        ...
        if (x_sema_lock_timeout(_hVdecFinSema, 500) != OSR_OK)
        {
...
#ifdef CC_ENABLE_TRACKING_MODE
    _VDEC_AVCYC(&rDispInfo);
    LOG(6, "CRC Y 0x%x, C 0x%x\n",
        rDispInfo.u4Ycrc[0] + rDispInfo.u4Ycrc[1] + rDispInfo.u4Ycrc[2] + rDispInfo.u4Ycrc[3],
        rDispInfo.u4Ccrc[0] + rDispInfo.u4Ccrc[1] + rDispInfo.u4Ccrc[2] + rDispInfo.u4Ccrc[3]);
#endif
...
// **************************************************************************
#ifdef CC_ENABLE_TRACKING_MODE
void _VDEC_DecProc(VDEC_AVC_DecPrm* prDecPrm, INT32* pi4RetValue)
#else
...
#endif
{
    ...
    {
#ifdef IS_POST_MT5368
        MC_WRITE32(RW_MC_BLK_MODE_W, ((prVdecEsInfo->u2LineSize + 15) >> 4));
#else
    ...
    {
#ifdef IS_POST_MT5368
        MC_WRITE32(RW_MC_BLK_MODE_W, ((prDecPrm->u4PicW + 15) >> 4));
#else
    ...
    if (prDecPrm->prSPS && prDecPrm->prSliceHdr)
    {
        ...
        {
            VLD_TOP_WRITE32(RW_VLD_TOP_TOTAL_MBS_IN_PIC,
...
#endif
    {
        AVC_MV_WRITE32(RW_AMV_MV_BUF_MODE, AVC_MV_READ32(RW_AMV_MV_BUF_MODE) | SIZE_REDUCE);
    }
#endif

#ifdef CC_ENABLE_TRACKING_MODE
    //apply to frame or first field -> 2nd field (if TB order the same as display)
    if (((prDecPrm->ucTrackingFbId != 0xFF) || (prDecPrm->ucTrackingFbIdPending != 0xFF))
            && (prDecPrm->prCurrFBufInfo->u1FBufStatus == FRAME))
    {
        if (prDecPrm->ucTrackingFbId == prDecPrm->prCurrFBufInfo->ucFbId)
        {
            //set tracking mode, vdec
#if 1
            if (!prDecPrm->prSliceHdr->fgFieldPicFlag)
            {
                VLD_TOP_WRITE32(RW_VLD_TOP_TRACKING_MODE,
                                VLD_TOP_READ32(RW_VLD_TOP_TRACKING_MODE) | ENABLE_TRACKING |
                                BUF_MBL_OFFSET);
            }
            else
            {
                VLD_TOP_WRITE32(RW_VLD_TOP_TRACKING_MODE,
                                VLD_TOP_READ32(RW_VLD_TOP_TRACKING_MODE) | ENABLE_TRACKING | FIELD_PICTURE |
                                BUF_MBL_OFFSET);
            }
#endif
            LOG(3, "Enable tracking mode\n");
        }
        else
        {
            if (prDecPrm->ucTrackingFbIdPending == 0xFF)
            {
                LOG(3, "Enable tracking mode fail, CurFbId %d mismatch SaveId %d\n",
                    prDecPrm->prCurrFBufInfo->ucFbId, prDecPrm->ucTrackingFbId);
            }
        }
        if (prDecPrm->ucTrackingFbIdPending != 0xFF) //field mode, set at first field
        {
            prDecPrm->ucTrackingFbId = prDecPrm->ucTrackingFbIdPending;
            prDecPrm->ucTrackingFbIdPending = 0xFF;
        }
        else
        {
            prDecPrm->ucTrackingFbId = 0xFF;
        }
    }
        ...
        do
        {
                ...
                == FBM_FB_STATUS_READY)
            {
                ...
                else
                {
                    #ifdef VDP_RACE_LOG
                    VDP_FrcRacingLog('Q');
                    VDP_FrcRacingLog('0' + prDecPrm->arFBufInfo[u4DispIdx].ucFbId);
                    #endif
...
void _VDEC_OutputDispPic(VDEC_AVC_DecPrm* prDecPrm, INT32* pi4RetValue)
{
    ...
    //For MVC field encoded, LT(get fb), RT(get fb), LB(set L ready), RB(set R ready), FBM status violation.
    //Change to, LT(get fb, set ready), RT(get fb, set ready), LB, RB
    /*if(prDecPrm->fgIsMVC)
    {
        if((FBM_GetFrameBufferStatus(prDecPrm->ucFbgId, prDecPrm->prCurrFBufInfo->ucFbId)
                    == FBM_FB_STATUS_DECODE) &&
            (prDecPrm->prCurrFBufInfo->u1FBufStatus != FRAME))
        {
            FBM_SetFrameBufferStatus(prDecPrm->ucFbgId, prDecPrm->prCurrFBufInfo->ucFbId, FBM_FB_STATUS_FIELD_READY);
        }
    }*/
    if (prDecPrm->prCurrFBufInfo->u1FBufStatus == FRAME)
    {
        ...
        else
        {
                    ...
                    == FBM_FB_STATUS_DECODE)
            {
                ...
                if (prDecPrm->fgIsMMPlay
                  ...
                  (prDecPrm->prSPS->u4PicOrderCntType == 2)*/)
                        )
#endif
                   )
                {
                    ...
                    if ((u4TmpResult != 0) && (prVdecEsInfo->eContainerType > SWDMX_FMT_MPEG2_TS) && (prVdecEsInfo->eContainerType <= SWDMX_FMT_H264_VIDEO_ES))
                    {
                        prDecPrm->prCurrFBufInfo->fgIsNotDisp = TRUE;
                    }
                    //VERIFY(x_sema_unlock(_hVdecFinSema)== OSR_OK);    //the purpose?
#endif
#endif
       ...
       )
    {
        ...
        do
        {
                    ...
                    == FBM_FB_STATUS_READY)
            {
                ...
                if (prFbmPicHdr && (!prDecPrm->fgIsMVC))
                {
                    prFbmPicHdr->fgTrackingMode = FALSE;
                    if (prDecPrm->arFBufInfo[u1ViewIdx][u4DispIdx].ucFbId == prDecPrm->arFBufInfo[u1ViewIdx][u4MinPOCFBufIdx].ucFbId)
                    {
                        prDecPrm->u4TrackingAcc++;

                        if ((prDecPrm->ucTrackingFbId == 0xFF) && (!prDecPrm->prSliceHdr->fgFieldPicFlag))
                        {
                            prDecPrm->ucTrackingFbId = prDecPrm->arFBufInfo[u1ViewIdx][u4DispIdx].ucFbId;
                        }
                        else if ((prDecPrm->ucTrackingFbIdPending == 0xFF) && prDecPrm->prSliceHdr->fgFieldPicFlag)
                        {
                            prDecPrm->ucTrackingFbIdPending = prDecPrm->arFBufInfo[u1ViewIdx][u4DispIdx].ucFbId;
                        }
                        else
                        {
                            LOG(2, "Enable tracking fail, previous frame not finish!\n");
                        }
                        //mark tracking flag in FBM
                        //after b2r get from dispQ, will put to empty right away and enable tracking.
                        //vdec should decode faster than b2r after put to dispQ,
                        //vdec need to wait dec done to start next pic(which has tracking enabled).
                        prFbmPicHdr->fgTrackingMode = TRUE;
                        LOG(3, "Tracking FbId %d set dispQ\n", prDecPrm->arFBufInfo[u1ViewIdx][u4DispIdx].ucFbId);
                    }
                    else
                    {
                        LOG(3, "Normal FbId %d set dispQ\n", prDecPrm->arFBufInfo[u1ViewIdx][u4DispIdx].ucFbId);
                        if (prDecPrm->u4TrackingAcc > prDecPrm->u4TrackingAccMax)
                        {
                            prDecPrm->u4TrackingAccMax = prDecPrm->u4TrackingAcc;
                        }
                        prDecPrm->u4TrackingAcc = 0;
                    }
                }
                else
#endif
                ...
                if (prDecPrm->i4LastPOC[u1ViewIdx] > i4DispPOC)   //temp before FBM modification ready
                {
                ...
                else
                {
#ifdef VDP_RACE_LOG
                    VDP_FrcRacingLog('Q');
                    VDP_FrcRacingLog('0' + prDecPrm->arFBufInfo[u1ViewIdx][u4DispIdx].ucFbId);
#endif
                    //notify audio start to play
                    if (!prDecPrm->fgStartPlay)
                    {
                    ...
                    else
                    {
                        ...
                        FBM_SetFrameBufferStatus(prDecPrm->aucFbgId[u1ViewIdx], prDecPrm->arFBufInfo[u1ViewIdx][u4DispIdx].ucFbId,
                                                 FBM_FB_STATUS_DISPLAYQ);
                        prVdecEsInfo->u4DisplayQPicCnt++;
                        if (fgstart && (ucCount > 0) && prDecPrm->fgIsMVC)
                        {
                            LOG(1, "PQS T(0x%x), (-), POC %d, V %d, FbId %d\n",
                                prFbmPicHdr->u4PTS, i4DispPOC, prDecPrm->u1CurViewIdx,
                                prDecPrm->arFBufInfo[u1ViewIdx][u4DispIdx].ucFbId);
                            ucCount --;
                        }
                        ...
                        if (prDecPrm->ucEsId < MAX_ES_NUM)
                        {
                            ...
                            if (prMpvCc->fgPlayCc)
                            {
                                ...
                                prCcInfo->u8Pts = (UINT64)prFbmPicHdr->u4PTS;
                                prCcInfo->u4BufRptr = _VDEC_SaveUserData(prDecPrm->arFBufInfo[u1ViewIdx][u4DispIdx].au1UsrDataFrm,
                                     ...
                                     (prExtInfo->fgPesUserData[prDecPrm->ucEsId] && (prDecPrm->arFBufInfo[u1ViewIdx][u4DispIdx].u4UserDataPESWptr > 0))))
                            {
                                ...
                                {
                                    rUsrData.u4Rptr = _VDEC_SaveUserData(prDecPrm->arFBufInfo[u1ViewIdx][u4DispIdx].au1UsrDataFrm,
                                                                         prDecPrm->arFBufInfo[u1ViewIdx][u4DispIdx].u4UserDataFrmWptr);
                                ...
                                {
                                    rUsrData.u4Rptr = _VDEC_SaveUserData(prDecPrm->arFBufInfo[u1ViewIdx][u4DispIdx].au1UsrDataPES,
...
void _VDEC_SetWptr(UCHAR ucEsId, UINT32 u4Wptr)
{
    VDEC_ES_INFO_KEEP_T* prVdecEsInfoKeep = _VDEC_GetEsInfoKeep(ucEsId);

    if (ucEsId == prVdecEsInfoKeep->rPesInfo.ucEsId)
    {
        _VDEC_SetVldWPtr(u4Wptr, prVdecEsInfoKeep->rPesInfo.u4FifoStart, prVdecEsInfoKeep->rPesInfo.u4FifoEnd);
    }
    //else, do not set wptr directly.
}
...
BOOL _VDEC_SkipFrm(VDEC_AVC_DecPrm* prDecPrm)
{
    ...
    if (((prDecPrm->prCurrFBufInfo == NULL) ||
            ...
            (prVdecEsInfoKeep && (prVdecEsInfoKeep->eCurState == VDEC_ST_PLAY_I_FRAME)) ||
...
void _VDEC_FrmFin(VDEC_AVC_DecPrm* prDecPrm)
{
...
#ifdef CC_ENABLE_TRACKING_MODE
    LOG(1, "Dec FbId %d, IsFld %d, %d ms\n", prDecPrm->prCurrFBufInfo->ucFbId,
        prDecPrm->prSliceHdr->fgFieldPicFlag, rTimeHwDt.u4Micros / 1000);
#endif
             ...
             (prDecPrm->prCurrFBufInfo->u1FBufStatus != NO_PIC)))
    {
        ...
        {
            u4PList0 = MC_READ32(RW_MC_P_LIST0) & 0xFFFFFFFF;
...
UINT32 _VDEC_SaveUserData(const UINT8* pu1UserData, UINT32 u4Size)
{
    ...
    prExtInfo = (MPV_EXT_INFO_T*)_MPV_GetExtInfo();
    prMpvCc = (MPV_CC_T*)_MPV_GetMpvCc(ES0);
