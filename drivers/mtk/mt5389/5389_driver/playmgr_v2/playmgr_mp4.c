...
static BOOL _PLAYMGR_SetMP4Info(PLAYMGR_STATUS_T* prMediaStatus, PLAYMGR_INFO_T* prMediaInfo)
{
			...
			(prMp4Info->u2AudIdx < 17))
	{
		...
		for(i = prMp4Info->u2AudIdx; 
		((i < prMediaInfo->u2AudStmNum + prMp4Info->u2AudIdx) && 
		((i - prMediaInfo->u2VidStmNum)<PLAYMGR_MAX_AUD_TRACK)) &&
		(i < 16) && (i >= prMediaInfo->u2VidStmNum); 
		i++)
	   {
			switch (prMp4Info->rStmAttribute[i].u.t_mp4i_audio_stm_attr.e_enc)
			{
				case MINFO_INFO_AUD_ENC_MPEG_1: 
					if (prMp4Info->rStmAttribute[i].u.t_mp4i_audio_stm_attr.e_layer == 3)
					{
				...
				case MINFO_INFO_AUD_ENC_ADPCM:
					...
					break;					  
				default:
					LOG(3, "Unknown Audio %d\n", (prMp4Info->rStmAttribute[i].u.t_mp4i_audio_stm_attr.e_enc));
					prMediaStatus->aeAudFormat[i - prMediaInfo->u2VidStmNum] = 
    ...
    {
        if (((i - prMediaInfo->u2VidStmNum) < 3) && (i >= prMediaInfo->u2VidStmNum))
        {
...
BOOL _PLAYMGR_Mp4Start(VOID* pvMediaInfo)
{
    ...
    if (!_PLAYMGR_ReceiveNotify(prMediaInfo->u1SrcId, (VOID*)&u4Msg))
    {
        return FALSE;
    }
    ...
    if ((prMediaInfo->u2AudStmNum > 0) &&
        (prMp4Info->u2AudIdx < 3))
    {
		(prFirstDescrib == NULL) ? (prFirstDescrib = prADescrib) : 
											   (prFirstDescrib->pt_next = prADescrib);
		prADescrib->u_info.t_es.e_es_type = MIDXBULD_ELEM_STRM_TYPE_AUDIO;
		prADescrib->u_info.t_es.ui4_strm_id = 
        	prSrcInfo->t_es_map[prMp4Info->u2AudIdx].ui4_strm_id;
        prADescrib->u_info.t_es.ui4_scale = 
        ...
        prADescrib->u_info.t_es.ui4_avg_bytes_per_sec = 
        	prSrcInfo->t_es_map[prMp4Info->u2AudIdx].ui4_avg_bytes_per_sec;
        prADescrib->u_info.t_es.b_is_cur_strm_codec_spt = 
    ...
    while(prFirstDescrib != NULL)
    {
        ...
        else
        {
            zSize = sizeof(MIDXBULD_ELMT_TBL_INFO_T);
             ...
             {
                  x_memset(&prMp4Info->rVidIDXTBL[0], 0, sizeof(MM_RANGE_ELMT_IDXTBL_T));
             ...
             {
                  x_memset(&prMp4Info->rAudIDXTBL[0], 0, sizeof(MM_RANGE_ELMT_IDXTBL_T));
...
#if 0
    return SWDMX_Play(prMediaInfo->u1SrcId);
#endif
    return TRUE;
}
...
EXTERN BOOL _PLAYMGR_Mp4Close(VOID* pvMediaInfo)
{
    ...
    if (prMediaInfo->FmtInfo.rMp4Info.hMediaInfoHandle != (UINT32)NULL)
    {
...
BOOL _PLAYMGR_OpenMp4MediaInfo(PLAYMGR_STATUS_T* prMediaStatus, VOID* pvMediaInfo)
{
    ...
    prSrcInfo->t_mp4_extra_info.pt_mp4_cmov_info = prMp4Info->rPlayerIdxInfo.t_mp4_extra_info.pt_mp4_cmov_info;
    prSrcInfo->t_mp4_extra_info.pt_piff_inet_info = prMp4Info->rPlayerIdxInfo.t_mp4_extra_info.pt_piff_inet_info;
...
BOOL _PLAYMGR_Mp4UpdateRange(VOID* pvMediaInfo, UINT32 u4Param1)
{
    ...
    if (prMediaInfo->u2VidStmNum > 0)
    {
        ...
        prVDescrib->u_info.t_es.e_es_type = MIDXBULD_ELEM_STRM_TYPE_VIDEO;
        prVDescrib->u_info.t_es.ui4_strm_id = 
        	prSrcInfo->t_es_map[0].ui4_strm_id;
        prVDescrib->u_info.t_es.ui4_scale = 
        	prSrcInfo->t_es_map[0].ui4_scale;
        prVDescrib->u_info.t_es.ui4_rate = 
        	prSrcInfo->t_es_map[0].ui4_rate;
        prVDescrib->u_info.t_es.ui4_sample_sz = 
        	prSrcInfo->t_es_map[0].ui4_sample_sz ;
        prVDescrib->u_info.t_es.ui4_avg_bytes_per_sec = 
        	prSrcInfo->t_es_map[0].ui4_avg_bytes_per_sec;
        ...
        (prMp4Info->u2AudIdx < MIDXBULD_ELMT_TBL_MAX))
    {
       ...
       (prFirstDescrib == NULL) ? (prFirstDescrib = prADescrib) : 
                                               (prFirstDescrib->pt_next = prADescrib);                                        
        prADescrib->u_info.t_es.e_es_type = MIDXBULD_ELEM_STRM_TYPE_AUDIO;
        prADescrib->u_info.t_es.ui4_strm_id = prSrcInfo->t_es_map[prMp4Info->u2AudIdx].ui4_strm_id;
        prADescrib->u_info.t_es.ui4_scale = prSrcInfo->t_es_map[prMp4Info->u2AudIdx].ui4_scale;
        prADescrib->u_info.t_es.ui4_rate = prSrcInfo->t_es_map[prMp4Info->u2AudIdx].ui4_rate;
        prADescrib->u_info.t_es.ui4_sample_sz = 0;
        prADescrib->u_info.t_es.ui4_avg_bytes_per_sec = 0;
		if(prMp4Info->u2AudIdx < 3)
        {
             prADescrib->u_info.t_es.b_is_cur_strm_codec_spt = 
                  prMp4Info->rStmAttribute[prMp4Info->u2AudIdx].b_is_codec_support;
    ...
    if ((prMediaInfo->u2VidStmNum > 0) &&
        (prVidElmtStrm->ui4_number_of_entry > 0) &&
        (prVidElmtStrm->u.pt_pb_idx_tbl_entry != NULL))
    {
        UINT32 i = (prVidElmtStrm->ui4_number_of_entry >= 5) ? (i = 5) : (i = prVidElmtStrm->ui4_number_of_entry);
        UINT32 u4VidPts = prVidElmtStrm->u.pt_pb_idx_tbl_entry[prVidElmtStrm->ui4_number_of_entry-i].ui4_pts;
    ...
    else if (prAudElmtStrm->u.pt_pb_idx_tbl_entry != NULL)
    {
        prMediaInfo->u4CurRangePTS = 
            prAudElmtStrm->u.pt_pb_idx_tbl_entry[prAudElmtStrm->ui4_number_of_entry-1].ui4_pts + 1;
    }
    ...
    if (i4RangeIdx < MAX_RANGE_NUM)
    {
        ...
        SWDMX_AddRange(prMediaInfo->u1SrcId,
            &prMediaInfo->rRangeInfo[i4RangeIdx], 
