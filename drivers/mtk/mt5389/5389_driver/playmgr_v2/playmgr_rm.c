...
static BOOL _PLAYMGR_SetRMInfo(PLAYMGR_STATUS_T* prMediaStatus, PLAYMGR_INFO_T* prMediaInfo)
{
    ...
    if ((prMediaInfo->u2VidStmNum > 0) && (prRmInfo->u2VidIdx < 17))
    {
...
#if 1 //sync from function _mm_hdlr_set_strm_codec_spec_info
        {
            ...
            //ui4_specific_size = (pt_rm_vid_spec_info->ui4_spo_extra_flags & 0x70000) >> 16;
            //ui4_specific_size = (ui4_specific_size + 1) * 8; 
...
#if 0 // defined(CC_RV_EMULATION) && defined(RV_TRACKING)
    LOG(1, "Force turn off audio\n");
...
#else
            LOG(3, "Cook is only support on 5363 now\n");
            prMediaStatus->aeAudFormat[0] = PLAYMGR_ADEC_UNKNOWN;
//                prMediaStatus->u2AudioTracks = 0;
    ...
    {
        if ((i >= prMediaInfo->u2VidStmNum) && ((i - prMediaInfo->u2VidStmNum) < PLAYMGR_MAX_AUD_TRACK))
        {
...
//    MM_RANGE_ELMT_IDXTBL_T* prAudKeyIdxTbl = &prRmInfo->rAudKeyIdxTBL[0];
    ...
    if (prRmInfo->hMediaIdxHandle == 0)
    {
        ...
        i4Ret = x_midxbuld_mopen((VOID *)&prMediaInfo->rMediaFeeder, 0, _PLAYMGR_MIdx_Notify_Rm, prMediaInfo,
...
BOOL _PLAYMGR_RmResume(VOID* pvMediaInfo)
{
    return TRUE;
}
...
BOOL _PLAYMGR_OpenRmMediaInfo(PLAYMGR_STATUS_T* prMediaStatus, VOID* pvMediaInfo)
{
    ...
    {
    case PLAYMGR_SRC_TYPE_NETWORK_XUNLEI:
        prMediaInfo->rRangeInfo[0].e_mm_src_type = MM_SRC_TYPE_NETWORK_XUNLEI;
        break;
