...
void vCreateLoopTxBuff(void)
{
        ...
        _prLoopTxPktBuff = (PKT_BUFF_T *)x_alloc_dma_mem(sizeof(PKT_BUFF_T)*NUM_RX_DESC);
   #else
   ...
   #if (CONFIG_DRV_LINUX_DATA_CONSISTENCY)
    _pucLoopTxBuf = (UINT8 *)x_alloc_dma_mem(PKT_BUFFER_SIZE*NUM_RX_DESC);
   #else
...
INT32 _i4EtherInit(void)
{
    ...
    u4Data = dReadMAC(RW_EXT_CFG);
    u4Data &= ~HRST;
    vWriteMAC(RW_EXT_CFG, u4Data);
    u4Data |= HRST;
    //vWriteMAC(RW_EXT_CFG, u4Data);
#endif /* __linux__ */
...
INT32 _i4EtherSendFrame(INT32 i4Argc, const CHAR ** szArgv)
{
    ...
    x_memset(&_rEtherTimeSum, 0x00, sizeof(HAL_TIME_T));
     ...
     _pucTxBuf = (UINT8 *)x_alloc_aligned_dma_mem(PKT_BUFFER_SIZE, 128);//kenny test 
   #else
     _pucTxBuf = (UINT8 *)x_alloc_dma_mem(PKT_BUFFER_SIZE);
   #endif 
    ...
    {
       #if (CONFIG_DRV_LINUX_DATA_CONSISTENCY)
        prPktBuff = (PKT_BUFF_T *)x_alloc_dma_mem(sizeof(PKT_BUFF_T));
       #else
...
INT32 _i4EtherMulticastList(INT32 i4Argc, const CHAR ** szArgv)
{
    ...
    #if (CONFIG_DRV_LINUX_DATA_CONSISTENCY)
    prMcList = x_alloc_dma_mem(u4ListSize);
    #else
