...
static BOOL bLBOXIsInputScalerDispmode(void)
{
    ...
    if ((u1Dispmode != VDP_SCPOS_DISPMODE_AUTO) && (u1Dispmode != VDP_SCPOS_DISPMODE_MANUAL)
		&&(u1Dispmode != VDP_SCPOS_LINESYNC_DISPMODE))
    {
        return 0;
    }
    else
...
{            
    ...
    if(u1Dispmode)//if dispmode, don't cut video
    {
        LOG(0,"WARNING: Letter box cut is disable under scaler line sync mode. \n");
    }
    else
    {
...
{    
    ...
    while (1)
    {
        while ((_u4LBoxDetectionEnable != 0)||u1WindowLBoxEnable)
        {
            ...
            if (bLBOXIsInputValid() && (_u4LBoxDetectionFixed == 0))
            {            
                UINT32 u4YIsSmall;
