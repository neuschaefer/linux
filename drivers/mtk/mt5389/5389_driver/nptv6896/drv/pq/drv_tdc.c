...
RRegDwrdTbl const CODE REGTBL_COMB_NTSC_AV[] = {
    ...
    {COMB_2D_01, 0x003D4444, 0xFFFFFFFF},// {COMB_2D_01, 0x00394444, 0xFFFFFFFF},
    {COMB_2D_02, 0x44000077, 0xFF070077},
...
RRegDwrdTbl const CODE REGTBL_COMB_PAL_AV[] = {
    ...
    {COMB_2D_0D, 0x10208003, 0xFFFFFEFF},
    {COMB_2D_0E, 0x3E6C408A, 0xFFFFFFFF},
    ...
    {COMB_2D_23, 0x0820080C, 0x3F3F3F3F},
    {COMB_2D_24, 0x84081D00, 0xFF3F3F00},
    {COMB_2D_25, 0x03201401, 0x7FFFFF7F}, //for vestel(cross color issue){COMB_2D_25, 0x03101401, 0x7FFFFF7F},
...
RRegDwrdTbl const CODE REGTBL_COMB_PAL_M_AV[] = {
    ...
    {COMB_2D_00, 0x00000000, 0x00000010},    
    {COMB_2D_01, 0x003D4444, 0xFFFFFFFF},//{COMB_2D_01, 0x00374444, 0xFFFFFFFF},
...
static void vTdcDynCost(void)
{
    ...
    if((RegReadFldAlign(COMB_3D_14, CB_3D_BAND11_SUM) < 2) &&    //Vestel,fix 'ultra black' delay issue, 120716 pingan
        (RegReadFldAlign(COMB_3D_14, CB_3D_BAND10_SUM) < 11) &&
        (RegReadFldAlign(COMB_3D_12, CB_3D_BAND0_SUM) < 20))
    {
        vRegWriteFldAlign(COMB_3D_0E, 0x30, CB_3D_FW_3D_COST);     
    }
}
...
void vTdcMovingChromaMultiburst(void)
{
	UINT8 bColorEdge,bBand8,bBand9,bBand10;
	BOOL fgPatchEn = SV_FALSE;
	
	bColorEdge = RegReadFldAlign(COMB_3D_15, CB_3D_CHROMA_EDGE_SUM);
	bBand8 = RegReadFldAlign(COMB_3D_14, CB_3D_BAND8_SUM);
	bBand9 = RegReadFldAlign(COMB_3D_14, CB_3D_BAND9_SUM);
	bBand10 = RegReadFldAlign(COMB_3D_14, CB_3D_BAND10_SUM);

	if (_rTvd3dStatus.eSourceType==SV_ST_AV)
	{
		if((bColorEdge > 0xF0 ) && (bBand8 > 0xF0 )&& (bBand9 < 0xA )&& (bBand10 < 0xA ))
		{
			fgPatchEn = SV_TRUE;
		}
	}
	...
	if (_rTvd3dStatus.eSourceType==SV_ST_TV)
	{
		...
		{
			if((bColorEdge > 0xA0 ) && (bBand8 > 0x60 )&& (bBand8 < 0xA0 )&& (bBand9 < 0x18 )&& (bBand10 < 0x20 ))
			{
				fgPatchEn = SV_TRUE;
			}
		}
		if(_rTvd3dStatus.bTvdMode == SV_CS_NTSC358)
		{
			if((bColorEdge > 0xE0 ) && (bBand8 > 0x80 )&& (bBand8 < 0xC0 )&& (bBand9 < 0x18 )&& (bBand10 < 0x18 ))
			{
				fgPatchEn = SV_TRUE;
			}
		}
 		...
 		if(fgPatchEn)
 		{
 			vRegWriteFldAlign(COMB_3D_0D, 0, CB_3D_PURE_V_TH);
			vRegWriteFldAlign(COMB_3D_0C, 0, CB_3D_V_C_T_D);
			vRegWriteFldAlign(COMB_2D_02, SV_ON, CB_2D_ESC_H_EN);
			vRegWriteFldAlign(COMB_2D_02, SV_ON, CB_2D_ESC_V_EN);
			if(_rTvd3dStatus.bTvdMode == SV_CS_PAL)
			{
				vRegWriteFldAlign(COMB_2D_00, SV_ON, CB_2D_PAL_PARITY_INVERSE_NEW_2D);
			}
			else
			{
				vRegWriteFldAlign(COMB_2D_00, SV_OFF, CB_2D_PAL_PARITY_INVERSE_NEW_2D);
			}
			
 		}	
		else
		{
			vRegWriteFldAlign(COMB_3D_0D, 8, CB_3D_PURE_V_TH);
			vRegWriteFldAlign(COMB_3D_0C, 0xA, CB_3D_V_C_T_D);
			vRegWriteFldAlign(COMB_2D_02, SV_OFF, CB_2D_ESC_H_EN);
			vRegWriteFldAlign(COMB_2D_02, SV_OFF, CB_2D_ESC_V_EN);
			vRegWriteFldAlign(COMB_2D_02, 0, CB_2D_ESC_FORCE_MODE);
		}
}
...
{
    static BOOL fgflag = SV_FALSE;  //special for magenta-green rotation pattern(vestel)
    static UINT8 u1tbth_k2b = 0;     //special for magenta-green rotation pattern(vestel)
    static UINT8 u1hfv_vyd_gain =0;  //special for magenta-green rotation pattern(vestel)
    ...
    //begin ---special for magenta-green rotation pattern(vestel)
    if((RegReadFldAlign(COMB_3D_14,CB_3D_BAND10_SUM) > 0xF5) && 
        (RegReadFldAlign(COMB_3D_14,CB_3D_BAND11_SUM) > 0xB5) &&
        (RegReadFldAlign(COMB_3D_14,CB_3D_BAND11_SUM) < 0xC5) &&
        (RegReadFldAlign(COMB_3D_12,CB_3D_BAND3_SUM) < 0x22))
    {
        if(fgflag == SV_FALSE)
        {
            u1tbth_k2b = RegReadFldAlign(COMB_2D_1A, TBTH_K2B);
            u1hfv_vyd_gain = RegReadFldAlign(COMB_2D_27, REG_HFY_VYD_GAIN);
            
            vRegWriteFldAlign(COMB_2D_1A, 0x01, TBTH_K2B);
            vRegWriteFldAlign(COMB_2D_27, 0x0A, REG_HFY_VYD_GAIN);
            fgflag = SV_TRUE;
        }
    }
    else if(fgflag == SV_TRUE)
    {
        fgflag = SV_FALSE;
        vRegWriteFldAlign(COMB_2D_1A, u1tbth_k2b, TBTH_K2B);
        vRegWriteFldAlign(COMB_2D_27, u1hfv_vyd_gain, REG_HFY_VYD_GAIN);
    }
    //end ---
...
void vDrvTdcParam(void) 
{
    vRegWriteFldAlign(COMB_3D_01, wReadQualityTable(QUALITY_COMB_CB_3D_W_BASE_COST_3D), CB_3D_W_BASE_COST_3D);
    vRegWriteFldAlign(COMB_3D_01, wReadQualityTable(QUALITY_COMB_CB_3D_W_CHECKER_BOARD_COST), CB_3D_W_CHECKER_BOARD_COST);
    vRegWriteFldAlign(COMB_3D_01, wReadQualityTable(QUALITY_COMB_CB_3D_W_CROSS_COLOR_COST), CB_3D_W_CROSS_COLOR_COST);
    vRegWriteFldAlign(COMB_3D_01, wReadQualityTable(QUALITY_COMB_CB_3D_W_INTERNAL_COST_PD), CB_3D_W_INTERNAL_COST_PD);
    vRegWriteFldAlign(COMB_3D_01, wReadQualityTable(QUALITY_COMB_CB_3D_W_INTERNAL_COST_Y_PD), CB_3D_W_INTERNAL_COST_Y_PD);
    vRegWriteFldAlign(COMB_3D_02, wReadQualityTable(QUALITY_COMB_CB_3D_W_ALIASING_COST), CB_3D_W_ALIASING_COST);
    vRegWriteFldAlign(COMB_3D_02, wReadQualityTable(QUALITY_COMB_CB_3D_W_YC_MOTION_COST), CB_3D_W_YC_MOTION_COST);
    vRegWriteFldAlign(COMB_3D_0B, wReadQualityTable(QUALITY_COMB_CB_3D_W_CHROMA_COST_2D), CB_3D_W_CHROMA_COST_2D);
    vRegWriteFldAlign(COMB_3D_0B, wReadQualityTable(QUALITY_COMB_CB_3D_W_INTERNAL_COST_2D), CB_3D_W_INTERNAL_COST_2D);
    vRegWriteFldAlign(COMB_3D_0B, wReadQualityTable(QUALITY_COMB_CB_3D_W_MOTION_COST_2D), CB_3D_W_MOTION_COST_2D);
    vRegWriteFldAlign(COMB_3D_0B, wReadQualityTable(QUALITY_COMB_CB_3D_W_OFFSET_2D), CB_3D_W_OFFSET_2D);
    vRegWriteFldAlign(COMB_3D_0E, wReadQualityTable(QUALITY_COMB_CB_3D_FW_3D_COST), CB_3D_FW_3D_COST);
    vRegWriteFldAlign(COMB_3D_0E, wReadQualityTable(QUALITY_COMB_CB_3D_FW_2D_COST), CB_3D_FW_2D_COST);
}
