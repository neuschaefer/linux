...
#define AVMUX_CH0_MSK (0x354A0000)
#define AVMUX_CH1_MSK (0x4A350000)
 ...
 * @return void
 */
PRIVATE code UINT8 bAvMuxMskshift[8][2]=
{
   #if 0
	{0,0},	/*none*/
	{20,4},	/*CVBS0*/  
...
#if defined(CC_SOURCE_AUTO_DETECT)
extern source_detect_t src_det_tbl[];
#elif defined(CC_SRCDETECT_CVBS_SUPPORT)
source_detect_t src_det_tbl[]= {
    {SV_VS_ATV1,FALSE}, 
    {SV_VS_ATD1,FALSE},
    {SV_VS_CVBS1,SUPPORT_AV1_INPUT},
    {SV_VS_CVBS2,FALSE},    
    {SV_VS_CVBS3,SUPPORT_AV3_INPUT},    
    {SV_VS_CVBS4,FALSE},    
    {SV_VS_S1,FALSE},   
    {SV_VS_S2, FALSE},  
    {SV_VS_S3,FALSE},   
...
#endif
#ifdef CC_SRCDETECT_CVBS_SUPPORT
UINT32 u32DVDModeSig =0;
#endif
...
UINT8 u1DrvDetectSourceVGA(void)
{
    ...
    {
        if (fgDrvVGAStableCnt(SourceStableCnt+1))
        {
            vDrvRstVGANoSigCnt();
            return SRC_DET_UNCHG; //stable;
        }
        if (fgDrvVGAStableCnt(SourceStableCnt))
        {
            vDrvRstVGANoSigCnt();
            LOG(3, "**********======VGA source detect OK = %d \n",
    ...
    {
        if (fgDrvVGANoSigCnt(DetectNoSigCnt+1))
        {
            vDrvRstVGAStableCnt();
            return SRC_DET_UNCHG; //nosignal
        }
        if (fgDrvVGANoSigCnt(DetectNoSigCnt))
        {
            vDrvRstVGAStableCnt();
            u1RetSrcSts = SRC_DET_NOSIGNAL;
            LOG(3, "**********======VGA source detect no signal \n");
...
UINT8 u1DrvDetectSourceYPBPR(UINT8 SOY0_1_G)
{
    ...
    u1SOGV_Stable = RegReadFldAlign(STA_SYNC0_02, AS_SOGV_STABLE);
    ...
    if ((u1CSYNC_ACT == TRUE) && (u1SOGV_Stable == TRUE))
    {
        ...
        {
            LOG(3, "*******======YPBPR %d source detect OK = %d \n",
    ...
    else
    {
        ...
        {
            u1RetSrcSts = SRC_DET_NOSIGNAL;
            LOG(3, "*******======YPBPR %d source detect no signal\n", SOY0_1_G);
            if(1==SOY0_1_G)
            {
...
void vDrvDetectSourceState(void)
{
    ...
    for(u1DetSrcIdxCnt=0;u1DetSrcIdxCnt<4;u1DetSrcIdxCnt++)
    {
        ...
        switch(VSS_MAJOR(u1DetSource))
        {
            ...
            case VSS_CVBS:
            case VSS_SV:
                if((VSS_MAJOR(_bSrcMainNew) == VSS_SV||VSS_MAJOR(_bSrcSubNew) == VSS_SV) || (VSS_MAJOR(_bSrcMainNew) == VSS_SCART||VSS_MAJOR(_bSrcSubNew) == VSS_SCART))
                {
                ...
                if( au1WaitSyncStableCnt[u1DetSrcIdxCnt] == 0)
                {
                    ...
                    if(VSS_MAJOR(_bSrcMainNew) == VSS_ATD||VSS_MAJOR(_bSrcSubNew) == VSS_ATD)
                    {
                        vDrvAvMux(dAvMuxMask(AVMUX_CH0(av_mux), 0) |
					...
					{
						if (IO32ReadFldAlign(REG_CVBS_CFG0, RG_BTM_EN) != 1)
					 	{
							vIO32WriteFldAlign(REG_CVBS_CFG0, 1, RG_BTM_EN);
					 	}
					}

                }

                if( au1WaitSyncStableCnt[u1DetSrcIdxCnt] < 10)
                {
            ...
            case VSS_VGACOMP:
                ...
                if(au1WaitSyncStableCnt[u1DetSrcIdxCnt] < 20)
                {
                    au1WaitSyncStableCnt[u1DetSrcIdxCnt]++;
                }
                else
        ...
        {
#ifdef CC_SRCDETECT_CVBS_SUPPORT
           if ( au1DetSrcIdx[u1DetSrcIdxCnt]-1 == SRC_DET_CVBS3){
               switch(u1SrcSts){
     			   case SRC_DET_STABLE:
     			   	  u32DVDModeSig =1;
     				  break;
     			   case SRC_DET_NOSIGNAL:
     			   	  u32DVDModeSig =0;
     			   	  break;
     			   default:
     			   	  break;
...
#endif
#endif
			if (u1SrcSts != SRC_DET_UNCHG)
				printf("NotifyAutoDetSrc(cursrc=%d,srcidx=%d, sts=%d)\n", 
				    _bSrcMainNew,au1DetSrcIdx[u1DetSrcIdxCnt]-1, u1SrcSts);
			
            LOG(3, "NotifyAutoDetSrc(srcidx=%d, sts=%d)\n", au1DetSrcIdx[u1DetSrcIdxCnt]-1, u1SrcSts);
        }
    } // for u1DetSrcIdxCnt
}
