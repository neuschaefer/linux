...
{
     VERIFY(pu4Val != NULL);
    *pu4Val = (_PDWNC_ReadWakeupStatus() & (0xFF << 20));
...
#endif
{
    ...
    vIO32WriteFldAlign(PDWNC_SRVCFG1, 0x300, FLD_SRVCH_EN);
            ...
            (prSingleServoKeyList[i].u4Key != DKBIT_NONE))
    {
		...
		//Get the range
            ...
            if((prSingleServoKeyList[i].u4Servo) < 6)
            	...
            	vIO32WriteFldAlign(PDWNC_ADOUT6 + ((prSingleServoKeyList[i].u4Servo - 6) * 4), u4Val, FLD_ADOUT0);
...
#if 1//defined(CC_MT5368) ||defined(CC_MT5396) 
...
UINT32 PDWNC_SRVAD_Read(UINT32 u4Channel)
{
	...
	//u4OldSrvRate0 = IO32ReadFld(PDWNC_SRVCFG0, FLD_SMP_RATE0);//need check
...
#else
        if((i4Gpio >= (ADC2GPIO_CH_ID_MIN+SERVO(0))) && (i4Gpio <= (ADC2GPIO_CH_ID_MAX+SERVO(0))))
        {
            return 1;
        }
         else
