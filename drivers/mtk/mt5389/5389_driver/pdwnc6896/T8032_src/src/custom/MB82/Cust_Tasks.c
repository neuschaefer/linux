...
#if RC56_IR_DRIVER
#define MRC56_TOGGLE_BIT_MASK(x) ((x) & 0xfffffff0)
...
*/  
    ...
    SYS7_B1_KEY[0] = 0x9a;//0x95;
    SYS7_B1_KEY[1] = 0x9a;//0x95;
    SYS7_B1_KEY[2] = 0x9a;//0x95;
    SYS7_B1_KEY[3] = 0x9a;//0x95;
    SYS7_B1_KEY[4] = 0x9a;//0x95;
    SYS7_B1_KEY[5] = 0x9a;//0x95;
    SYS7_B1_KEY[6] = 0x9a;//0x95;
    SYS7_B1_KEY[7] = 0x9a;//0x95;
    SYS7_B1_KEY[8] = 0x9a;//0x95;
    SYS7_B1_KEY[9] = 0x9a;//0x95;
    SYS7_B1_KEY[10] = 0x5a;//0x95;
    SYS7_B1_KEY[11] = 0x5a;//0xFF;
    SYS7_B1_KEY[12] = 0x9a;//0x95;
    SYS7_B1_KEY[13] = 0;// 0xFF;
    SYS7_B1_KEY[14] = 0x5a;//0x55;
    SYS7_B1_KEY[15] = 0x5a;//0x55;
    SYS7_B1_KEY[16] = 0;//0x55;
    SYS7_B1_KEY[17] = 0x5a;//0x6A;
...
//Since we have the BYTE1 information of P_COMP RC6 yellow/red/green, RC6 is support MULIT RC now            
    ...
    //we have no special funciton for rc5 address code check in 8032, so we just choose a sample way like this. (special for mb80 only)
    u1_mb80_address = 0xa0;

//system code7 always valid...        
    rc5_b2b3 = RC5_B2B3_KEY[key_map_idx];
    u4tmp = (((UINT32)rc5_b2b3) << 16) | (((UINT32)SYS7_B1_KEY[key_map_idx]) << 8) | u1_mb80_address;
    if(u4Code == u4tmp)
        return 1;
        
//Since we have not the BYTE1 information of P_COMP RC5 yellow/red/green, RC5 is not support MULIT RC now        
    u4tmp = (((UINT32)rc5_b2b3) << 16) | (((UINT32)RC5_BLUE_B1_KEY[key_map_idx]) << 8) | u1_mb80_address;
    if(u4Code == u4tmp)
        return 1;
...
{
	if(Check_IR_Key(u4Tmp, IR_POWER_IDX))
	{
        //LogS("Power");
        return PDWNC_WAKE_UP_REASON_RC_POWER;
	}
...
void vTask1_InitCust_P_IrInit()
{
    ...
    //power off on sequence here.
    vSetTimer(POWER_OFF_ON_SECQ_ID, 15);
...
//    UINT8 temp;
    vIRSpecialKeyHook(irData);//check ok key here.
 //    temp = MGET_ARM_POWER_STATE();
//     LogSD("temp", temp);
//     LogSD("irData", irData);
 
 //If system is on power off sequence, we should not do anything.
 if (bGetTimer(POWER_OFF_ON_SECQ_ID) > 0)
 {
     return;
 }
...
#if 0
	//vRegWrite4B(PDWNC_T8032_INTEN, 0x00004a00);
	//u4Data = u4RegRead4B(PDWNC_T8032_INTEN);
		while (!(RegReadFldAlign(PDWNC_STAB, FLD_WR_ALLOW))){}; 
	  u4Data = u4RegRead4B(PDWNC_T8032_INTEN);
	  ...
	  _LogS("Read Addr ");
		while (!(RegReadFldAlign(PDWNC_STAB, FLD_WR_ALLOW))){};  vIO32Write1B(PDWNC_DATAW, '0');
		while (!(RegReadFldAlign(PDWNC_STAB, FLD_WR_ALLOW))){};  vIO32Write1B(PDWNC_DATAW, 'x');	
	  _Print8bitsHex((UINT8)(PDWNC_T8032_INTEN >> 8));
	  _Print8bitsHex((UINT8)(PDWNC_T8032_INTEN & 0x00ff));
		while (!(RegReadFldAlign(PDWNC_STAB, FLD_WR_ALLOW))){};  vIO32Write1B(PDWNC_DATAW, 0x0D);
		while (!(RegReadFldAlign(PDWNC_STAB, FLD_WR_ALLOW))){};  vIO32Write1B(PDWNC_DATAW, 0x0A);	 
	  _LogS("==> ");
...
void SetCtrlLedEnable(void)
{
    ...
    {
        SetSERVOEN((UINT8)(gErrCodeCtrl.fgErrCodeLedMsk));
        SetSERVOPADEN((UINT8)(gErrCodeCtrl.fgErrCodeLedMsk));
    }
    else
    {
        SetGPIOEN(gErrCodeCtrl.fgErrCodeLedMsk);
    }
}
...
//if(1)
    {
        if(gErrCodeCtrl.fgErrCodeLedType == 1)
        {
            if(gErrCodeCtrl.fgErrCodeLedPolarity)
                SetSERVOOUT((UINT8)(gErrCodeCtrl.fgErrCodeLedMsk));
            else
                ClrSERVOOUT((UINT8)(gErrCodeCtrl.fgErrCodeLedMsk)); 
        }
        else
        {
            if(gErrCodeCtrl.fgErrCodeLedPolarity)
                SetGPIOOUT((UINT8)(gErrCodeCtrl.fgErrCodeLedMsk));
            else
                ClrGPIOOUT((UINT8)(gErrCodeCtrl.fgErrCodeLedMsk)); 
        }
    }
}
...
{
    memcpy(&(gErrCodeCtrl), 0, sizeof(Error_Code_Ctrl));
    gErrCodeCtrl.fgS_compErrLedStateMachine = CUST_S_ERROR_CODE_STATE_1;    
}
...
void vTask4_error_led_ctrl_state_machine(void)
{
    ...
    */            
    ...
    //		vRegWrite1B(REG_DATAW,0xA3); //debug
    //		vRegWrite1B(REG_DATAW,fgS_compErrLedStateMachine); //debug
    //		vRegWrite1B(REG_DATAW,fgErrCodeBlinkingTimeCnt); //debug		
    //		vRegWrite1B(REG_DATAW,0xA3); //debug		
            #endif                 
...
#if	CC_PDWNCONOFF_PATCH
Re_Wakeup_Ctrl gReWakeupCtrl;
void vTask9_InitRewakeupFunc(void)
{
	gReWakeupCtrl.fgActive = 0;
	gReWakeupCtrl.fgEnable = 0;
	gReWakeupCtrl.fgReWakeupTimeout = 0;
}

void vTask9_RewakeupFunc(void)
{
	if(gReWakeupCtrl.fgActive == 0)
	{
		return;
	}
	//LogSB("ReWakeup init", gReWakeupCtrl.fgEnable);

	if((gReWakeupCtrl.fgEnable == 0) && MPollingReWakeupCmd())
	{
		gReWakeupCtrl.fgEnable = 1;
		gReWakeupCtrl.fgReWakeupTimeout = MReadReWakeupTimeout();
//		LogSB("ReWak Req!", gReWakeupCtrl.fgReWakeupTimeout);
		vSetTimer(RE_WAKEUP_TIMER_ID, gReWakeupCtrl.fgReWakeupTimeout);
	}
	else if((gReWakeupCtrl.fgEnable == 1) && fgIsTimerUp(RE_WAKEUP_TIMER_ID))
	{
//		LogS("ReWak timeout!");
//		vKrDelay10ms(2);
		gReWakeupCtrl.fgEnable = 0;
...
#if 1 //task8
LED_SETTING LED[LED_MAX_NUM];
void vTask8_Init(void)
{
    UINT8 i = 0;
    for(i=0; i<LED_MAX_NUM; i++)
    {
        LED[i].bEnableLEDBlink = 0;
        LED[i].bLEDStatus = 0;
        LED[i].bStatusChanged = 0;
        LED[i].bOnOffStatus = 0xff;
    }
    //For power on default. we should turn on LED.
//    LED_ON(0);
//    LED_OFF(1);
}

void vTask8LEDTurnOn(UINT8 i)
{
//    i = 0;
    LED_ON(i);
}

void vTask8LEDTurnOff(UINT8 i)
{
 //    i = 0;
    LED_OFF(i);
}

UINT8 bIsLEDBlinkOn(UINT8 i)
{
    return LED[i].bEnableLEDBlink;
}

void vTask8LEDBlinkOn(UINT8 i)
{
   if(LED[i].bEnableLEDBlink == 1)
   {
       return;
   }
   LED[i].bEnableLEDBlink = 1;
   LED[i].bStatusChanged = 0;
   
   vSetTimer(LED_TIMER_ID_1+(i), LED[i].bLEDBlinkTimer);
}

void vTask8LEDBlinkOff(UINT8 i)
{
    LED[i].bEnableLEDBlink = 0;
#if 0
    //east.yan add new.
    if (0 == LED[i].bOnOffStatus)
    {
        LED_OFF(i);
    }
    else if (1 == LED[i].bOnOffStatus)
    {
        LED_ON(i);
    }
    else
...
void vTask8PowerOnLEDStatus(UINT8 i)//speed up blink
{
    LED[i].bLEDStatus = 0;
    LED_OFF(i);
    LED[i].bEnableLEDBlink = 1;
...
void vTask8SetLEDBlinkTimer(UINT8 i, UINT8 bTimer)
{
    LED[i].bLEDBlinkTimer = bTimer;
}

void vTask8LEDChangeLEDStatus(UINT8 i)
{
    if(0 == LED[i].bLEDStatus)
    {
        LED[i].bLEDStatus = 1;
        LED_ON(i);
    }
    else if (1 == LED[i].bLEDStatus)
    {
        LED[i].bLEDStatus = 0;
        LED_OFF(i);
    }
...
/*
this function should be put into mainloop/some thread
*/
void vTask8LEDBlink(UINT8 i)
{
    if(0 == LED[i].bStatusChanged)
    {
        LED[i].bStatusChanged = 1;
        vSetTimer(LED_TIMER_ID_1+(i), LED[i].bLEDBlinkTimer);
    }

     if(fgIsTimerUp(LED_TIMER_ID_1+(i)))
     {
        LED[i].bStatusChanged = 0;
        vTask8LEDChangeLEDStatus(i);
     }
}

/*
blink priority is max; then on/on.
*/
void vTask8LEDCtrl(void)
{
    UINT8 i = 0;

    for(i=0; i<LED_MAX_NUM; i++)
    {
        if(1 == bIsLEDBlinkOn(i))
        {
            vTask8LEDBlink(i);
        }
        ...
        {
            if(LED[i].bOnOffStatus == 1)
            {
                LED_ON(i);
            }
            else if(LED[i].bOnOffStatus == 0)
            {
                LED_OFF(i);
            }
        }
...
UINT8 bOkKeyHook;
UINT8 bEnableSpecialKeyHook;
void vTask10IRSpecialKeyHookInit(void)
{
    bOkKeyHook = 0;
    bEnableSpecialKeyHook = 1;
#if 0
    vSetTimer(SPECIAL_IRKEY_HOOK_ID, 10);
#endif
}

void vIRSpecialKeyHook(IR_DATA_TYPE irData)
{
    IR_DATA_TYPE tmpIrData = irData;
#if 0
    if(fgIsTimerUp(SPECIAL_IRKEY_HOOK_ID))
    {
        bOkKeyHook = 0;
    }
#endif
    if(!fgIsSpecialKeyHookEnabled())
    {
        return;
    }

//    LogSD("irData", tmpIrData);
    
    if(Check_IR_Key(MRC56_TOGGLE_BIT_MASK(tmpIrData), IR_OK_IDX)) 
    {
        bOkKeyHook = 1;
        vSetTimer(SPECIAL_IRKEY_HOOK_ID, 10);
    }
...
#if 1 //DVD wake up task.
UINT8 bDVDWakeUpCnt;
void vTask11DVDWakeUpCheckInit(void)
{
    bDVDWakeUpCnt = 0;
    return;
}

void vTask11DVDWakeUpCheck(void)
{
    UINT8 bDVDWakeUpStatus = 1;
//      return ;//disable at this stage.
    if(MGET_ARM_POWER_STATE() != ARM_POWER_OFF)
    {
        bDVDWakeUpCnt = 0;
        return;
    ...
    bDVDWakeUpStatus = GPIO_GetIn(200);//OPCTRL2
    if(bDVDWakeUpStatus == 0)
    {
        bDVDWakeUpCnt ++;
    }
    else
 ...
 //   LogSB("vTask11DVDWakeUpCheck",bDVDWakeUpCnt);

    if(bDVDWakeUpCnt >= 10)//fine tune me.
    {
        bDVDWakeUpCnt = 0;
        vWakeUpSystem(PDWNC_WAKE_UP_REASON_DVD, 0);//this is DVD wakeup.
    }
}
#endif
#if 1 //scart wakeup task.
#define ScartPin8ServoADCNum 0
#define PIN8_ACTIVE_MIN 16
#define PIN8_ACITVE_MAX 250
#define PIN8_CNT_MAX 30
#define PIN8_CNT_INT 20
#define PIN8_CNT_MIN 10
#define PIN8_SAMPLE_TIMER 3
UINT8 bPin8StableCnt;
UINT8 bPin8BeforeStatus;
UINT8 bEnableScartWakeup;
void vTask12ScartWakeUpCheckInit(void)
{
    bPin8StableCnt = PIN8_CNT_INT;
    bEnableScartWakeup = 0;
    vSetTimer(SCART_WAKEUP_ID, PIN8_SAMPLE_TIMER);
}

void vTask12ScartWakeUpConfig(UINT8 bEnablePin8WakeUp, UINT8 bStbPin8InitStatus)
{
    bEnableScartWakeup = bEnablePin8WakeUp;
    bPin8BeforeStatus = bStbPin8InitStatus;
    bPin8StableCnt = PIN8_CNT_INT;
    vSetTimer(SCART_WAKEUP_ID, PIN8_SAMPLE_TIMER);
    vConfigSrvad(ScartPin8ServoADCNum);
}

void vTask12ScartWakeupCheck(void)
{
    UINT8 bPin8Val = 0;

    //LogS("a");
    if(bEnableScartWakeup == 0)
    {
        //LogS("b");
        return;
    }
    ...
    if(fgIsTimerUp(SCART_WAKEUP_ID))
    {
        //LogS("d");
        vSetTimer(SCART_WAKEUP_ID, PIN8_SAMPLE_TIMER);
        bPin8Val = bReadSrvad(ScartPin8ServoADCNum);
        if((bPin8Val >= PIN8_ACTIVE_MIN) && (bPin8Val <= PIN8_ACITVE_MAX))
        {
            bPin8StableCnt++;
        }
        else
        ...
        //LogSB("e", bPin8StableCnt);
        if(bPin8StableCnt >= PIN8_CNT_MAX)
        {
            bPin8StableCnt = PIN8_CNT_MAX;
        }
        else if(bPin8StableCnt <= PIN8_CNT_MIN)
        {
            bPin8StableCnt = PIN8_CNT_MIN;
        }

        switch(bPin8StableCnt)
        {
            case PIN8_CNT_MAX:
                //LogSB("f", bPin8BeforeStatus);
                if(bPin8BeforeStatus == 0)
                {
                    bPin8BeforeStatus = 1;
                    //should wakeup.
                    vWakeUpSystem(PDWNC_WAKE_UP_REASON_SCART, 0);//this is scart wakeup.
                }
                break;
            case PIN8_CNT_MIN:
                //LogSB("g", bPin8BeforeStatus);
                if(bPin8BeforeStatus == 1)
                {
                    bPin8BeforeStatus = 0;
                }
                break;
...
#if 1//task13
ServADC_WAKEUP_GROUP servoadc_wakeup_group[WAKEUP_GROUP_MAX];
UINT8 bIsServoADCWakeUpEnabled;
#define SW_SAMPLE_TIMER 1
#define SW_SAMPLE_TIMES 3
void vTask13_Init(void)
{
    UINT8 i = 0;
    for(i = 0; i< WAKEUP_GROUP_MAX; i++)
    {
        servoadc_wakeup_group[i].bEnable = 0;
        servoadc_wakeup_group[i].bServVal = 0;
        servoadc_wakeup_group[i].bCnt = 0;
    }
    vTask13_EnableServoADCWakeUp();
    vSetTimer(SERVO_ADC_ID, SW_SAMPLE_TIMER);
//    vSetTimer(SERVO_ADC_ID, SW_SAMPLE_TIMER);
}

void vTask13_Config(UINT8 bEnable, UINT8 bServoADCNum, UINT8 bGroup, UINT8 bTop, UINT8 bBottom)
{
    if(bGroup > WAKEUP_GROUP_MAX)
    {
        return;
    }
    servoadc_wakeup_group[bGroup].bEnable = bEnable;
    servoadc_wakeup_group[bGroup].bServoADCNum = bServoADCNum;
    servoadc_wakeup_group[bGroup].bGroup = bGroup;
    servoadc_wakeup_group[bGroup].bTop = bTop;
    servoadc_wakeup_group[bGroup].bBottom = bBottom;
    servoadc_wakeup_group[bGroup].bServVal = 0;
    servoadc_wakeup_group[bGroup].bCnt = 0;
    vConfigSrvad(bServoADCNum);
    vSetTimer(SERVO_ADC_ID, SW_SAMPLE_TIMER);
    //vTask9_EnableServoADCWakeUp();
 //   LogS("task13 config");
}

void vTask13ServoADCWakeUpCtrl(void)
{
    UINT8 i = 0;
    UINT8 bWakeUpReason = 0;
    UINT8 bTest = 0;
  //  UINT8 u1Val;
//    UINT32 u4Pinmux = 0;
//  UINT32 u4Enable = 0;
 //   return;
//    u4Pinmux = u4RegRead4B(IO_VIRT+IO_PDWNC_BASE+0x0B4);
//    u4Enable = u4RegRead4B(IO_VIRT+IO_PDWNC_BASE+0x304);
    ...
    if(fgIsTimerUp(SERVO_ADC_ID))
    {
        vSetTimer(SERVO_ADC_ID, SW_SAMPLE_TIMER);
        for(i = 0; i< WAKEUP_GROUP_MAX; i++)
        {
#if 0
            LogSB("", servoadc_wakeup_group[i].bEnable);
            LogSB("", servoadc_wakeup_group[i].bServoADCNum);
            LogSB("", servoadc_wakeup_group[i].bGroup);
            LogSB("", servoadc_wakeup_group[i].bTop);
            LogSB("", servoadc_wakeup_group[i].bBottom);
            LogSB("", servoadc_wakeup_group[i].bServVal);
            LogSB("", servoadc_wakeup_group[i].bCnt);
 #endif
            if(0 == servoadc_wakeup_group[i].bEnable)
            {
                continue;
            }
            servoadc_wakeup_group[i].bServVal = bReadSrvad(servoadc_wakeup_group[i].bServoADCNum);
  //          LogSB("bServVal", servoadc_wakeup_group[i].bServVal);
  //          LogSD("u4Pinmux", u4Pinmux);
  //          LogSD("u4Enable", u4Enable);
   #if 0
            LogSB("i : \n", i);
            LogSB("bEnable", servoadc_wakeup_group[i].bEnable);
            LogSB("bServoADCNum", servoadc_wakeup_group[i].bServoADCNum);
            LogSB("bGroup", servoadc_wakeup_group[i].bGroup);
            LogSB("bTop", servoadc_wakeup_group[i].bTop);
            LogSB("bBottom", servoadc_wakeup_group[i].bBottom);
            LogSB("bServVal", servoadc_wakeup_group[i].bServVal);
            LogSB("bCnt", servoadc_wakeup_group[i].bCnt);
 #endif         
            
            
            if(( servoadc_wakeup_group[i].bServVal >= servoadc_wakeup_group[i].bBottom)&&( servoadc_wakeup_group[i].bServVal <= servoadc_wakeup_group[i].bTop))
            {
                servoadc_wakeup_group[i].bCnt ++;
            }
            else
            {
                servoadc_wakeup_group[i].bCnt = 0;
...
//             LogSB("bCnt: %d\n", servoadc_wakeup_group[i].bCnt);
            if(servoadc_wakeup_group[i].bCnt >= SW_SAMPLE_TIMES)
            {
                switch(servoadc_wakeup_group[i].bGroup)
                {
                    case 0:
                        bWakeUpReason = PDWNC_WAKE_UP_REASON_FP_PRG_UP;
                        break;
                    case 1:
                        bWakeUpReason = PDWNC_WAKE_UP_REASON_FP_PRG_DOWN;
                        break;
                    case 2:
                        bWakeUpReason = PDWNC_WAKE_UP_REASON_FP_POWER;
                        break;
                    default:
                        break;
                }
                vWakeUpSystem(bWakeUpReason, 0);
                vTask13_DisableServoADCWakeUp();
            }
        }
    }
}
