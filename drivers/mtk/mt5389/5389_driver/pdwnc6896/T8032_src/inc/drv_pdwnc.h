...
#define NON_GPIO_INTERRUPT_RTC_INT_FLAG 0x10
...
#define MOIRI_DIS_INT() vIO32Write1B(PDWNC_T8032_INTEN + 3, u1IO32Read1B(PDWNC_T8032_INTEN + 3) & 0xfb)
#define MOIRI_EN_INT() vIO32Write1B(PDWNC_T8032_INTEN + 3, u1IO32Read1B(PDWNC_T8032_INTEN + 3) | 0x04)
#define MOIRI_GPIO_BOTH_EDGE_SETUP() vIO32Write1B(PDWNC_EXINT2ED + 3, u1IO32Read1B(PDWNC_EXINT2ED + 3) | 0x04);\
						vIO32Write1B(PDWNC_EXINTLEV + 3, u1IO32Read1B(PDWNC_EXINTLEV + 3) & 0xfb);\
						vIO32Write1B(PDWNC_EXINTPOL + 3, u1IO32Read1B(PDWNC_EXINTPOL + 3) & 0xfb);
#elif defined(CC_MT5389)
#define MOIRI_DIS_INT() vIO32Write1B(PDWNC_T8032_INTEN, u1IO32Read1B(PDWNC_T8032_INTEN) & 0xdf)
#define MOIRI_EN_INT() vIO32Write1B(PDWNC_T8032_INTEN, u1IO32Read1B(PDWNC_T8032_INTEN) | 0x20)
#define MOIRI_GPIO_BOTH_EDGE_SETUP() vIO32Write1B(PDWNC_EXINT2ED, u1IO32Read1B(PDWNC_EXINT2ED) | 0x20);\
						vIO32Write1B(PDWNC_EXINTLEV, u1IO32Read1B(PDWNC_EXINTLEV) & 0xdf);\
						vIO32Write1B(PDWNC_EXINTPOL, u1IO32Read1B(PDWNC_EXINTPOL) & 0xdf);
#else//5395/5365
...
#if defined(CC_MT5389)||defined(CC_MT5396)||defined(CC_MT5368)	//MT5389_TODO
    #define SetSERVOEN(x)
    #define SetSerGPIO_AS_OUTPUT(x) SetSERVOEN(x)
    #define ClrSERVOEN(x)
    #define SetSerGPIO_AS_INPUT(x) ClrSERVOEN(x)
    #define ReadSERVOEN(x) 
    #define SetSERVOOUT(x) 
    #define ClrSERVOOUT(x) 
    #define ReadSERVOOUT(x) 
    #define ReadSERVOIN(x) 	
//servoadc 4~5 pinmux pd_mux [28]~[31]	, ex: adin4 as gpio ==>pd_mux[29:28] = 01
	#define SetSERVOPADEN(x) 
...
//servoadc 2~5 pinmux pd_mux [28]~[31]	, ex: adin2 as gpio ==>pd_mux[28] = 1, value of (pd_mux +3 ) | (3b'100 << 2)
...
#define CORE_POWER_SRV (8)//(6)	//it is change to 8 from 5368/5396/5389
#define MUpdateSrvad(x) vRegWrite1B(PDWNC_ADOUT0 + ((x) << 2), 0);//only support ad ch 0~5
#define MReadSrvad(x) (((u4RegRead4B(PDWNC_ADOUT0 + ((x) << 2)))>>2)&0xff);//only support ad ch 0~5
...
#define SwitchUartPD_PORT0() vRegWrite1B(PDWNC_PINMUX0+2,((u1RegRead1B(PDWNC_PINMUX0+2) & 0xfc)|0x01))  //UART0 pad as T8032 log //bit 16~17
    ...
    #endif
#elif SWITCH_UART_PORT_5389
#define SwitchUartPD_PORT0() vRegWrite1B(PDWNC_PINMUX0+1,((u1RegRead1B(PDWNC_PINMUX0+1) & 0xfc)|0x01))  //UART0 pad as T8032 log //bit 8~9
    #if T8032_OCCUPIED_UART0_AT_NORMAL_POWER
        #define SwitchBack_PORT0()
    #else
        #define SwitchBack_PORT0() vRegWrite1B(PDWNC_PINMUX0+1, u1RegRead1B(PDWNC_PINMUX0+1) & 0xfc) //UART0 pad as ARM log 
    #endif
#elif SWITCH_UART_PORT_5396
#define SwitchUartPD_PORT0() vRegWrite1B(PDWNC_PINMUX0+2,((u1RegRead1B(PDWNC_PINMUX0+2) & 0xfc)|0x01))  //UART0 pad as T8032 log //bit 16~17
    #if T8032_OCCUPIED_UART0_AT_NORMAL_POWER
        #define SwitchBack_PORT0()
    #else
