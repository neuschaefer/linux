...
INT32 OSD_RGN_Init(void)
{
    ...
    for (i4Index = 0; i4Index < OSD_MAX_NUM_RGN; i4Index++)
    {
		...
		_rAllRgnNode[i4Index].u4Original_X = 0;
		_rAllRgnNode[i4Index].u4Original_Y = 0;
...
void OSD_RGN_Set_Bmp_Addr_Wait(UINT32 u4Plane,UINT32 u4Value)
{
    ...
    while(fgEnable)
    {
        u4BmpAddrTmp = _OSD_PLA_Get_Hw_RealBmpAddr(u4Plane,&u4RealBmpAddr);
        if(u4BmpAddrTmp != u4RealBmpAddr)
        {
           break;
        }  
        
        if((u4Value&0xffffff) == (u4RealBmpAddr&0xffffff))
        {
            break;
        }
    }
...
INT32 OSD_RGN_TransformCood(UINT32 u4Region, UINT32 *retPosX, UINT32 *retPosY)
{
     ...
     if(retPosX)
     {
        ...
        if(u4Mirror > 0)
        {
            ...
            OSD_RGN_Get(u4Region,OSD_RGN_OUT_W, &u4Tmp);
     ...
     if(retPosY)
     {
        ...
        if(u4Flip > 0)
        {
            ...
            OSD_RGN_Get(u4Region,OSD_RGN_OUT_H, &u4Tmp);
...
INT32 OSD_RGN_Set(UINT32 u4Region, INT32 i4Cmd, UINT32 u4Value)
{
    ...
    case OSD_RGN_POS_X:
        ...
        if ((u4Value + u4Tmp) > (2048 - 1))
        {
            u4Tmp = (2048 - 1) - u4Value;
    ...
    case OSD_RGN_OUT_W:
        OSD_RGN_Get(u4Region, OSD_RGN_POS_X, &u4Tmp);
        if ((u4Value + u4Tmp) > (2048 - 1))
        {
            u4Value = (2048 - 1) - u4Tmp;
        }
    ...
    case OSD_RGN_OUT_H:
        OSD_RGN_Get(u4Region, OSD_RGN_POS_Y, &u4Tmp);
        if ((u4Value + u4Tmp) > (2048 - 1))
        {
            u4Value = (2048 - 1) - u4Tmp;
        }
    ...
    case OSD_RGN_BMP_ADDR:
        ...
        u4Plane = OSD_PLA_GetRgnPlane(u4Region);
        OSD_RGN_Set_Bmp_Addr_Wait(u4Plane,u4Value);
        OSD_UnLockFlipTo();
        return i4Ret;
    case OSD_RGN_BMP_PITCH:
                     ...
                     UINT32 u4DispW, UINT32 u4DispH)
{
	...
	if((u4DispX + u4DispW) > (2048 - 1))
	{
		u4DispW = (2048 - 1) -  u4DispX;
//		printf("11111111113333333333333333\n");
	}
	if((u4DispY + u4DispH) > (2048 - 1))
	{
		u4DispH = (2048 - 1) -  u4DispY;
	//	printf("2222222222444444444444444443\n");
	}
...
#ifdef ANDROID
    i4Ret = _OSD_RGN_SetColorKeyEnable(u4Region, 1);
    i4Ret = _OSD_RGN_SetColorKey(u4Region, 0xfff74dff);
#else
...
INT32 OSD_RGN_AttachTail(UINT32 u4Region, UINT32 u4RgnList)
{
    ...
    if ((prRgnList->i4Head == (INT32)NULL) && (prRgnList->i4Count == (INT32)0))
    {
...
INT32 OSD_RGN_SetDisplayWidth(UINT32 u4Region, UINT32 u4Width)
{
	...
	u4Output = u4Width;
	OSD_RGN_Get(u4Region,OSD_RGN_POS_X, &u4Tmp);
	if((u4Output + u4Tmp) > (2048 - 1))
	{
		u4Output = (2048 - 1) -	u4Tmp;
	}
...
INT32 OSD_RGN_Dump(UINT32 u4Region)
{
    ...
    Printf("REGION[%ld]\n", u4Region);
    Printf("\tCM=%ld, SrcSize=(%ld, %ld), DstSize(%ld, %ld)\n",
           u4ColorMode, u4BmpW, u4BmpH, u4DispW, u4DispH);
    Printf("\tPos=(%ld, %ld), Clip=(%ld, %ld), Step=(0x%lx, 0x%lx)\n",
           u4PosX, u4PosY, u4ClipH, u4ClipV, u4StepH, u4StepV);
    Printf("\tAlpha=%ld, BlendMode=%ld, Key=0x%lx, KeyEn=%ld\n",
           u4Alpha, u4MixSel, u4ColorKey, u4ColorKeyEn);
    Printf("\tBmpAddr=0x%lx, PalAddr=0x%lx, PalLen=%ld, PalLoad=%ld\n",
           u4BmpAddr, u4PalAddr, u4PalLen, u4PalLoad);
    Printf("\tVFlip=%ld, HMirror=%ld, LongFifo=%ld, Pitch=%ld\n",
...
INT32 OSD_RGN_SetScrollInfo(UINT32 u4Rgn, UINT32 u4BaseAddress, UINT32 u4Pitch, UINT32 u4Step)
{
	...
	if( (u4BaseAddress == (UINT32)NULL)||(u4Pitch == (UINT32)NULL)||(u4Step == (UINT32)NULL)) 
	{
		return (INT32)OSD_RET_INV_ARG;
	}
...
INT32 OSD_RGN_SC_Clip(UINT32 u4_plane, void* pv_tag)
{
    UINT32  u4_first_rgn                = 0x0;
    OSD_RGN_SC_CLIP_T* pt_reg_sc_clip = (OSD_RGN_SC_CLIP_T*)pv_tag;

    pt_reg_sc_clip->ui4_id = u4_plane;

    u4_first_rgn = OSD_PLA_GetFirstRegion(pt_reg_sc_clip->ui4_id);


    if(pt_reg_sc_clip->ui4_full_fg)
    {
        pt_reg_sc_clip->i4_dst_x = 0x0;
        pt_reg_sc_clip->i4_dst_y = 0x0;
        pt_reg_sc_clip->i4_dst_w = PANEL_GetPanelWidth();
        pt_reg_sc_clip->i4_dst_h = PANEL_GetPanelHeight();
    }

    OSD_RGN_Set(u4_first_rgn,OSD_RGN_CLIP_H,pt_reg_sc_clip->i4_clip_x);
    OSD_RGN_Set(u4_first_rgn,OSD_RGN_CLIP_V,pt_reg_sc_clip->i4_clip_y);
    OSD_RGN_Set(u4_first_rgn,OSD_RGN_BMP_W,pt_reg_sc_clip->i4_clip_w);
    OSD_RGN_Set(u4_first_rgn,OSD_RGN_BMP_H,pt_reg_sc_clip->i4_clip_h);
    OSD_RGN_Set(u4_first_rgn,OSD_RGN_OUT_W,pt_reg_sc_clip->i4_clip_w);
    OSD_RGN_Set(u4_first_rgn,OSD_RGN_OUT_H,pt_reg_sc_clip->i4_clip_h);

    OSD_BASE_SetOsdPosition(pt_reg_sc_clip->ui4_id,
                            pt_reg_sc_clip->i4_dst_x,
                            pt_reg_sc_clip->i4_dst_y);

    OSD_SC_Scale(OSD_SC_GetScaler(u4_plane),
                pt_reg_sc_clip->ui4_enable,
                pt_reg_sc_clip->i4_clip_w,
                pt_reg_sc_clip->i4_clip_h,
                pt_reg_sc_clip->i4_dst_w,
                pt_reg_sc_clip->i4_dst_h);
