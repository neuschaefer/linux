...
INT32 OSD_PLA_FlipTo(UINT32 u4Plane, UINT32 u4List)
{
    INT32 i4FirstRegion;
    UINT32 u4FirstRegionAddr;
    volatile UINT32 u4RealHeadAddr;
    ...
    while(fgEnable)
    {
        ...
        if(u4FirstRegionAddr == u4RealHeadAddr)
        {
            break;
        }
    }
...
//#ifdef CC_DEBUG
    ...
    Printf("OSD PLANE[%ld]\n", u4Plane + 1);
    Printf("\tEnable=0x%08x, FirstRegion=0x%08x\n",
 ...
 */
//-----------------------------------------------------------------------------
INT32 OSD_PLA_WaitStable(UINT32 u4Plane)
{
    UINT32 u4Enable;
    ...
    _OSD_PLA_GetEnable(u4Plane, &u4Enable);

    if(u4Enable && _arOsdPlaneInfo[u4Plane].u4FirstRegionAddr)
    {         
        while(1)
        {
            u4Region = _arOsdPlaneInfo[u4Plane].i4FirstRegionId;
            _OSD_RGN_GetDataHWAddr(u4Region, &u4BmpAddrTmp);
			u4BmpAddrTmp = u4BmpAddrTmp & 0xffffff;
            _OSD_PLA_Get_Hw_RealBmpAddr(u4Plane,&u4RealBmpAddr);
			u4RealBmpAddr = u4RealBmpAddr & 0xffffff;
			...
			{
				Printf("hardware address 0x%x,	software address 0x%x \n", u4RealBmpAddr, u4BmpAddrTmp);
			    OSD_WaitVsync();
			}
        }    
    }
...
BOOL OSD_PLA_OutputValidCnt(UINT32 u4Plane, UINT32 *retu4Value)
{
    UINT32 u4Scaler, u4BaseAddr, u4Enable;

	_OSD_PLA_GetEnable(u4Plane, &u4Enable);

	if(u4Enable == 0 && retu4Value == NULL)
	{
	     return FALSE;
	}

	if(OSD_PLANE_2 == u4Plane)
	{
	    u4Scaler =  OSD_SCALER_2;
		u4BaseAddr = OSD_SC2_REG;
	}
	else if(OSD_PLANE_3 == u4Plane)
	{
	    u4Scaler =  OSD_SCALER_3;
		u4BaseAddr = OSD_SC3_REG;
	}
	else
	{
	     return FALSE;
	...
	*retu4Value = IO_READ32(u4BaseAddr, 0xF8);

	return TRUE;
	
}

void OSD_PLA_EnableFlipMirror(UINT32 u4Plane, BOOL flip, BOOL mirror)
{
     INT32 u4Region;
     Printf("~~~~~~ %s %d u4Plane = %d, flip= %d, mirror = %d\n",__FUNCTION__,__LINE__,u4Plane, flip,mirror);

     u4Region = OSD_PLA_GetFirstRegion(u4Plane);

     OSD_RGN_Set(u4Region, OSD_RGN_V_FLIP, flip);

     OSD_RGN_Set(u4Region, OSD_RGN_H_MIRROR, mirror);
...
INT32 OSD_PLA_Set3DRegion(UINT32 u4Plane, UINT32 u4LeftRgn, UINT32 u4RightRgn)
{
    ...
    VERIFY((INT32)OSD_RET_OK ==
           _OSD_PLA_SetHeaderAddr(u4Plane, u4u4LeftRgnAddr));

    VERIFY((INT32)OSD_RET_OK ==
           _OSD_RGN_GetAddress((UINT32)u4RightRgn, &u4u4RightRgnAddr));

    VERIFY((INT32)OSD_RET_OK ==
           _OSD_PLA_SetRightHeaderAddr(u4Plane, u4u4RightRgnAddr));
