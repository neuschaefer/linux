        ...
        if ( fgCamPwrManualControl == FALSE )
        {
            if ( u4CamPwrStatus != TRUE )
            {
                ...
                else
                {
                    ...
                    {
                        u1Val = 7;
                        LOG(0, "mt5175 power on\n");
                        u1Val = 0x40;
                        SPI_DemodSingleWrite(&u1Val, 0x900);
                        u1Val = 0x3;
                        SPI_DemodSingleWrite(&u1Val, 0x902);
                    }
                }
...
static void PCMCIA_SetTsRouting_5135_2bit(BOOL fgTsGoThroughCam, TS_FMT_T *prTsFmt)
{
    ...
    if (prTsFmt->SerOrPar == SP_SERIAL)
    {
        ...
        SPI_DemodSingleWrite(&u1RegValue, u4BaseReg);
        
        // Set 0xa81 = 0x3
        u1RegValue = 0x3;
        SPI_DemodSingleWrite(&u1RegValue, u4BaseReg+1);

        // Set 0xa82 = 0
        u1RegValue = 0x40;
        SPI_DemodSingleWrite(&u1RegValue, u4BaseReg+2);
        LOG(1, "Set CI Ts In: tsif.\n");

        // Set 0xa83 = 0x80
        u1RegValue = 0x80;
        SPI_DemodSingleWrite(&u1RegValue, u4BaseReg+3);
...
// 0xa80 = 0            
            ...
            SPI_DemodSingleRead(&u1RegValue, u4BaseReg+2);
            if (prTsFmt->fgExternalDemod == BOOL_TRUE)
            {    
                if (prTsFmt->ClockPolarity == POL_LOW_ACTIVE)
                {
...
static void _ExternalIc_Reset(void)
{
    ...
    else if (DRVCUST_InitQuery(eMT8295ResetGpio, &u4GpioNum) == 0)
    {
        GPIO_SetOut(u4GpioNum, 0);
        HAL_Delay_us(20000); //delay 20ms
        GPIO_SetOut(u4GpioNum, 1);
