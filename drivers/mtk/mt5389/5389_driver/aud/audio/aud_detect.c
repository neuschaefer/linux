...
extern UINT32 GetAudioHPLimiterThreshold(void);
...
static void _AudHeadphonePlugInMonThread(void* pvArg)
{
    ...
    while (1)
    {
        ...
        {
            //Headphone plug in
            AUD_DspSetLimiterThreshold(GetAudioHPLimiterThreshold());
            ADAC_HpDetectSpkEnable(FALSE);
            _MT5365_DacChlSel(INTERNAL_DAC_ID_4TH,AUD_CHL_DOWNMIX);
            if (fLineOutDacUsage)
            {
        ...
        {
            //Headphone plug out
            _MT5365_DacChlSel(INTERNAL_DAC_ID_4TH,AUD_CHL_L_R);
            AUD_DspSetLimiterThreshold(GetAudioLimiterThreshold());
            if (pfAudDecNfy != NULL)
            {
...
void AUD_DetectInit(void)
{
    ...
    else
    {
        ...
        {
            u4HpGpioNum = DRVCUST_OptGet(eAudioHeadphonePlugInGpio);
            //VERIFY(GPIO_INTEN((INT32)u4HpGpioNum, GPIO_TYPE_INTR_BOTH) >= 0); // mark for mt5396.compile.fail
            VERIFY(x_sema_unlock(_hSemaHpPlugInWait) == OSR_OK);
        }

        if (_IsUseDcDetectionMonThread())
        {
            u4HpGpioNum = DRVCUST_OptGet(eAudioDcDetectGpio);
            //VERIFY(GPIO_INTEN((INT32)u4HpGpioNum, GPIO_TYPE_INTR_BOTH) >= 0);  // mark for mt5396.compile.fail
            VERIFY(x_sema_unlock(_hSemaHpPlugInWait) == OSR_OK);
        }
    }
