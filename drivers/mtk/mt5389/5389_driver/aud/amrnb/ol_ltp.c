  ...
  )
{
   ...
   {
      *T_op = Pitch_ol(Enc , vadSt, mode, wsp, PIT_MIN, L_FRAME, idx, dtx);
   }
   else
   {
      if ( mode <= MR795 )
      {
         *T_op = Pitch_ol(Enc , vadSt, mode, wsp, PIT_MIN, L_FRAME_BY2,
                          idx, dtx);

      }
      else if ( mode == MR102 )
      {
         *T_op = Pitch_ol_wgh(Enc , st, vadSt, wsp, old_lags, ol_gain_flg, idx, dtx);

      }
      else
      {
         *T_op = Pitch_ol(Enc , vadSt, mode, wsp, PIT_MIN_MR122, L_FRAME_BY2, idx, dtx);
