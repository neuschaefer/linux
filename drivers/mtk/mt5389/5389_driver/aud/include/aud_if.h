...
typedef enum
{
    ...
    AUD_FMT_WMA10LOSSLESS,  //33, reserved for future development
    AUD_FMT_WMA10SPEECH, //34, reserved for future development
    AUD_FMT_AMRNB,        //35, reserved for future development
    AUD_FMT_AMRWB,        //36, reserved for future development
    AUD_FMT_DSP_AMR,        //37 amr-nb run in DSP
    AUD_FMT_DSP_AWB,        //38 amr-wb run in DSP
...
typedef struct
{
    ...
    CHAR au1AudName[48];
    //Input Source Volume
    INT16 aai2AudInputSrcVol[4 /*AUD_DEC_NUM*/][AUD_STREAM_FROM_NUM]; //04  + 8 + 2 * 4 * 10 = 92
    //Master Volume Table
    ...
    UINT32 u4AudioHPLimiterThreshold;
    //Surround
    ...
    //AVC
    INT16 ai2AudAvcTarget[AUD_STREAM_FROM_NUM];     //1050 + 2 * 10 = 1070
    INT16 ai2AudAvcGainUp[AUD_STREAM_FROM_NUM];     //1070 + 2 * 10 = 1090
    INT16 ai2AudAvcAdjRate[AUD_STREAM_FROM_NUM];    //1090 + 2 * 10 = 1110
    INT16 ai2AudAvcUiAdjRate[AUD_STREAM_FROM_NUM];  //1110 + 2 * 10 = 1130
    //Channel Volume(Extra-Gain)
    INT16 aai1AudChVolExtraGain[4 /*AUD_DEC_NUM*/][AUD_CH_NUM];    //1130 + 1 * 4 * 15 = 1190
    //Channel Delay
    UINT8 au1AudChDelay[AUD_STREAM_FROM_NUM];   //1190 + 1 * 10  => 1200
    ...
    INT8 aai1AudAVCChVolExtraGain[2][AUD_CH_NUM];    

    //silence
    UINT16 u2AudioSilenceMode;
    UINT32 u4AudioSilenceThreshold;
    UINT32 u4AudioSilenceWait;
    UINT32 u4AudioSilenceAttackGain;
    UINT32 u4AudioSilenceReleasestep;
    UINT32 u4AudioSilenceAttackStep;
    
    UINT8 u4AmpSel;
    UINT8 u2BypassLineout;

    INT16 i2DVDInputSrcGain;
...
typedef struct {
    AUD_FMT_T   eFormat;
    UINT32         u4decstate;   // AUD_DEC_STATE
    HANDLE_T     hCommandQ;
    HANDLE_T     hMainTask;     //main loop
    INT32           i4decerrcode; // error code
    UINT32         u4frmcnt;      // frame count
    UINT32         u4errfrmcnt; // error frame count
    UINT32         u4chkfrmsize;
    void            *pDecInfo;      //decoder required info
    /*******************************************
    * RISC decoder input buffer
    * ian: try to keep the same as ADSP get bits
    *******************************************/
    /* buffer address is assigned by omx*/
    void             *pInBufSTRBLK;    // Input Buffer Start Address
    void             *pInBufENDBLK;    // Input Buffer End Address
    UINT32         u4InBufSize; // Input Buffer Length = ENDBLK-STARTBLK
    /*update by omx, read only for decoder*/
    void             *pInBufPRSPNT;    // Input Buffer Write Pointer
    /*update by decoder, read only for omx*/
    void             *pInBufBUFPNT; // Input Buffer Buffer Pointer
    /*decoder read/write only*/
    void             *pInBufCURPNT; // Input Buffer Read Pointer
    /*******************************************
    * RISC decoder output callback function
    * ian: assume omx will take care of the detail !!
    * need to discuss with Daniel for general cases
    *******************************************/
    void             *pOutBufSTRBLK;   // Output Buffer Start Address
    UINT32         u4OutBufSize;        // Output Buffer Length = ENDBLK-STARTBLK
    void             *pOutBufPRSPNT;   // Output Buffer Write Pointer
    UINT8           *pDecOutPCMBuf;     // output pcm buffer pointer
    UINT32 (* pu4AudDecFillFIFO)(void *pHandle, UINT32 u4From, UINT32 u4Size);
} AUD_DEC_HANDLE_T;
 ...
 */
//-----------------------------------------------------------------------------
extern void AUD_GetStreamFrom(UINT8 u1DecId, AUD_DEC_STREAM_FROM_T *peStreamFrom);
 ...
 *  @param      fgPlayTestTone
 */
extern void AUD_PlayAudClipWithRPN(UINT8 u1DecId, UINT32 u4Channel, UINT32 ReaptNumber, BOOL fgPlayTestTone);
...
extern void AUD_DrvSetMMAudioOnly(BOOL fgMMAudioOnly);
 ...
 */
//-----------------------------------------------------------------------------
extern UINT16 AUD_DspGetChannelDelay(AUD_CH_T eChIndex);
...
extern BOOL AUD_DspGetBassMngEnable(UINT8 u1DecId);
extern BOOL AUD_DspGetChannelVolumeEnable(UINT8 u1DecId);
extern BOOL AUD_DspGetDelayEnable(UINT8 u1DecId);
extern BOOL AUD_DspGetPostProcessingEnable(UINT8 u1DecId);
 ...
 */
//-----------------------------------------------------------------------------
extern void AUD_DspBypassPostProcess(UINT8 u1DecId, UINT16 u2BypassMode);
 ...
 *   SPDIF In Pin Select
 *
 *  @param  Pin number by IC.
 *
 *  @retval  void
 */
//-----------------------------------------------------------------------------
extern void AUD_SpdifInBypass(SPDIF_IN_PIN_NUM u1PIN);
 ...
 */
//-----------------------------------------------------------------------------
extern void AUD_DspDownmixPosition(UINT8 u1DMPosition);
extern UINT8 AUD_DspGetDownmixPosition(void);
...
extern UINT16 AUD_DspGetSystemSetup(UINT8 u1DecId);
...
#else
// In Linux even not define CC_AUD_VBASS_SUPPORT, it still need this 2 SYMBOLs ??
// prepare a UNUSED FUNNCTION for it.
extern void AUD_DspSetVirBassEnable(BOOL fgEnable);
extern void AUD_DspSetVirBassCfg(UINT8 u1DecId, UINT32 u4Frequency, UINT32 u4Gain, UINT32 u4Gain2);
#endif
 ...
 */
//-----------------------------------------------------------------------------
extern void AUD_RepeatMixSndClip(UINT8 u1StreamID);
 ...
 */
//-----------------------------------------------------------------------------
extern BOOL AUD_SetMixSndClip(UINT8 u1StreamID, UINT32 u4Addr, UINT32 u4Size, UINT32 u4Gain, UINT8 u1StereoOnOff, UINT32 u4SampleRate, UINT8 u1BitDepth);
...
extern void AUD_PlayMixSndRingFifo(UINT32 u4SampleRate, UINT8 u1StereoOnOff, UINT8 u1BitDepth, UINT32 u4BufferSize); //ALSA_MIXSND_PATH
extern void AUD_StopMixSndRingFifo(void); //ALSA_MIXSND_PATH
...
#ifdef SUPPORT_FLASH_AQ
extern INT32 AUD_FlashAQReadEx(UINT8 u1AQPartition, UINT16 u2Offset, UINT8 *u1Data, UINT32 u4Length);
...
extern void AUD_FlashAQSetDefault(void);
extern void AUD_FlashAQFillData(BOOL fgDelayMode);
extern BOOL AUD_FlashAQGetDscrpt(AUD_AQ_DSCRPT_T** pAudFlashAQDscrpt);
extern void AUD_FlashAQInit(void);
...
extern void AUD_DrvGetParserDetect(BOOL *pfgDeteced);
extern void AUD_DrvGetParserFormat(AUD_FMT_T *peDecFmt);
extern AUD_FMT_T AUD_DrvPcToFormat(UINT16 u2Pc);
#endif
...
extern void AUD_PlayBeep(UINT8 u1DecId,UINT8 frequency,UINT16 duration,UINT8 percent_volume_level);
extern BOOL AUD_IsFmtSupport(AUD_FMT_T eDecType);
extern BOOL AUD_IsAACL2(void);
extern BOOL AUD_IsSupportSRS(void);
extern void AUD_SupportSRS(BOOL fgSRS);
extern UINT8 AUD_SaveSRSFlag(UINT32 u4SRSFlag);
extern UINT8 AUD_LoadSRSFlag(void);
extern UINT8 AUD_SaveMpeg4Flag(UINT32 u4Mpeg4Flag);
extern BOOL AUD_IsSupportMpeg4(void);
extern void AUD_SupportMpeg4(BOOL fgMpeg4);
extern UINT8 AUD_LoadMpeg4Flag(void);
