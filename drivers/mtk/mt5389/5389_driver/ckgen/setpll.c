...
#define AD_CLKDIG_CTS_D11 0x3A 
#define AD_VOPLL_DPIX_CK 0x40    
#define AD_VOPLL_TCON_CK 0x41    
#define AD_PPLL_DPIX_CK 0x42    
#define DBGCKSEL_TCLK 0x9
#define DBGCKSEL_TCLK_DIV 0x16
#define DBGCKSEL_OCLK 0x17 
#define DBGCKSEL_OCLK_DIV 0x18 
#define DBGCKSEL_LVDS_CLK_CTS 0x19 
...
void vDrvPIXPLLPathEnable(UINT8 u1Enable)
{
  ...
  if ((_u1TconMode!=LVDS_MODE) && (u1Enable))
  {
	  ...
	  {
          if(GetCurrentPanelIndex()== PANEL_MLVDS_SS_MB7)//TRD
          {
              vIO32WriteFldAlign(REG_VOPLL_CFG4, PPLL_REFSEL_DDDS, RG_PPLL_REFSEL);
...
//    #endif
    ...
    {      
      ...
      vIO32WriteFldAlign(CKGEN_VPCLK_CFG, 0, FLD_OCLK_DIV_SEL);
        ...
        {          
         ...
         if(GetCurrentPanelIndex()== PANEL_MLVDS_SS_MB7)//TRD
         {
             vIO32WriteFldMulti(REG_VOPLL_CFG3,P_Fld(0x01,RG_PPLL_PREDIV)|P_Fld(0x00,RG_PPLL_FBKSEL)
    ...
    else  if (LVDS_OUTPUT_PORT == DUAL_PORT) 
    {
        ...
        // To OSD and LVDSTx setting        
         vIO32WriteFldAlign(CKGEN_VPCLK_CFG, u1SsOclkDiv, FLD_OCLK_DIV_SEL);
    }
    else //if (LVDS_OUTPUT_PORT == FOUR_PORT) 
    {
#ifdef CC_MT5396
...
//        vIO32WriteFldAlign(CKGEN_DBGCFG, 0x38, FLD_CKMON_SEL);
        u4Syspll = BSP_GetPLLClock(SRC_SYSPLL_CLK); //SYSPLL = 459MHz in MT5395, 648MHz for 68/96 
...
//     vIO32WriteFldAlign(REG_VOPLL_CFG3, 0, RG_PPLL_PWD);
     ...
     HAL_Delay_us(20);
      ...
      //u2Oclk2 = GetPLLCounter(AD_CLKDIG_CTS_D11);//AD_VPLL_LVDS_CTSCLK cntseng   
      if(_u1TconMode==LVDS_MODE)//switch DPIX clock
      {
//        vIO32WriteFldAlign(CKGEN_VPCLK_CFG, FLD_OCLK_SEL_LVDS, FLD_OCLK_SEL); 	  
        u2Oclk =  GetPLLCounter(AD_VOPLL_DPIX_CK);      
	  	u2Oclk2 = GetPLLCounter(AD_VOPLL_TCON_CK);//AD_VPLL_LVDS_CTSCLK cntseng   
      }
      else
      {
...
//    vIO32WriteFldAlign(CKGEN_DBGCFG, 0x38, FLD_CKMON_SEL);
    ...
    Printf("[LVDS]AD_VOPLL_TCON_CK=%d \n",_CalGetPLLCounter(AD_VOPLL_TCON_CK));
    Printf("[LVDS]AD_VOPLL_DPIX_CK=%d \n",_CalGetPLLCounter(AD_VOPLL_DPIX_CK));
