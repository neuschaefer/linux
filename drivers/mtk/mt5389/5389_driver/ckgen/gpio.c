...
#ifdef CC_WT6720
#ifndef CC_MTK_LOADER
static HANDLE_T _hWT6720Mutex = 0 ; //= NULL;
#endif /* CC_MTK_LOADER */
#endif
#if 1
...
#define WT6720_GPIO_IN_MODE 0x0
#define WT6720_GPIO_POWER_CTRL 0x0e
#ifndef CC_MTK_LOADER
static UINT32 _UTIL_Current_Task_Pointer(void)
{
    UINT32 u4Id, u4Tag;
    UNUSED(u4Tag);
...
static INT32 _WT6720_GPIO_Output(INT32 i4GpioNum, INT32 *pfgSet)
{
        ...
        if (pfgSet == NULL)//NULL: for query gpio status, must be GPIO output , but not change pin level
        {
			...
			if(_WT6720_GPIO_Enable(i4GpioNum, NULL) == 0)//old is input state, change to be output
			{
				// Set the output mode.
				i4Val = WT6720_GPIO_OUT_MODE;
				_WT6720_GPIO_Enable(i4GpioNum, &i4Val);				
				//set the original input value as output level				
		...
		//WT6720 need change to output mode first, then set the output level
        // Set the output mode.
        i4Val = WT6720_GPIO_OUT_MODE;
        _WT6720_GPIO_Enable(i4GpioNum, &i4Val);		
