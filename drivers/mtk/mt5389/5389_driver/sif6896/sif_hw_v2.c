...
#define PINTNO_SIF1      20			// SIF1 interrupt number inside Pdwnc
#endif

#define SIF_TIMEOUT_MAX  (2000)//(2000)
...
#endif
{
    ...
    {
        IO_WRITE32MSK(PDWNC_BASE, RW_AINTEN_REG, 0, SIF_M3_INTEN);
        IO_WRITE32MSK(PDWNC_BASE, RW_AINTCLR_REG, SIF_M3_INTCLR, SIF_M3_INTCLR);
    }
#endif
...
static VOID _SIF_Control(VOID)
{
    ...
    #else
    u4Reg |= DEGLITCH_EN;
    u4Reg |= DEGLITCH_SET(0x03);//DEGLITCH_SET(0x1f);
...
static VOID SIF_V2_Stop(VOID)
{
    ...
    if (_fgSifInit)
    {
		...
		if(_SIF_ISR_IsPdwncMasterEnable())
		{
			IO_WRITE32MSK(PDWNC_BASE, RW_SM0_CTRL0_REG - IO_VAL - 0x80, 0, SM0EN);
		}
		
#if !defined(CC_MT5389)
        if(_SIF_ISR_IsPdwncMaster1Enable())
		{
			IO_WRITE32MSK(PDWNC_BASE, RW_SM0_CTRL0_REG - IO_VAL - 0x80 - 0x60, 0, SM0EN);
		}
#endif
#ifndef CC_MTK_LOADER
#ifndef  CC_DISCARD_SUPPORT_PCMCIA
