...
//-----------------------------------------------------------------------------
#include "loader_if.h"
#if defined(CC_MT5395) || defined(CC_MT5389)
...
{
	UINT32 u4HDMIHPD0GPIO, u4HDMIHPD1GPIO,u4HDMIHPD2GPIO;
...
static void LOADER_DisableHdmiDdc(void)
{
    ...
    #ifndef CC_ENABLE_HDMI_JTAG
    BSP_PinSet(PIN_HDMI_0_SDA, PINMUX_FUNCTION2);
    BSP_PinSet(PIN_HDMI_0_SCL, PINMUX_FUNCTION2);
	BSP_PinSet(PIN_HDMI_0_HPD, PINMUX_FUNCTION1);
...
static void LOADER_MtkEnvInitFunc(void *prLdrData, UINT32 fgValidFlag)
{
    ...
    #endif
#endif
	//for FPAG verification
    //vIO32Write4B(0xf00280b4, 0/*(u4IO32Read4B(0xf00280b4) & 0xfffcffff)*/);//change pinmux to uartdbg
    ...
    IO_WRITE32(AUDIO_BASE,0x01e8, (IO_READ32(AUDIO_BASE,0x01e8))& ~(0xff << 4)|(0x0 << 4));
...
//for use extender gpio control backlight issue while press reset
    #ifdef CC_MT5389
    // turn off backlight GPIO with QFP (controlled by extender)
	    #if 0
	    //if (DRVCUST_InitGet(eFlagDDRQfp))
	    //{                  
	        u4Val = (DRVCUST_PanelGet(eBackLightOnVal)==1)? 0 : 1;
	        GPIO_SetOut(DRVCUST_PanelGet(eBackLightGpio), u4Val);
	    //}
	    #else	//use define instead of panel customized query, for shrink mtkloader size
			#if defined(BACKLIGHT_GPIO) && defined(BACKLIGHT_ON_VAL)			
	        u4Val = (BACKLIGHT_ON_VAL == 1)? 0 : 1;
			GPIO_SetOut(BACKLIGHT_GPIO, u4Val);
			#else			
			Printf("Error: BACKLIGHT_GPIO or BACKLIGHT_ON_VAL undefined!!!\n");
			#endif
		#endif
    #endif
...
static void GPIO_MtkLedCtrlFunc(LED_STATUS_T eLedStatus)
{
#if !defined(CC_MTK_LOADER) || !defined(CC_LOADER_SHRINK_LED)  //loader shrink
