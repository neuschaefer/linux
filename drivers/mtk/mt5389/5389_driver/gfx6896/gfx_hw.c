...
UINT32 GFX_HwGetRegDstBase(void)
{
    ...
    (((GFX_READ32(GFX_REG_ADDR_MSB_1)&GFX_REG_DST_ADDR_MSB_MASK)>>16)
        <<GREG_SHT_G_RST));
}
...
UINT32 GFX_HwGetRegSrcBase(void)
{
    ...
    (((GFX_READ32(GFX_REG_ADDR_MSB_1)&GFX_REG_SRC_ADDR_MSB_MASK)>>12)
        <<GREG_SHT_G_RST));
}
...
void GFX_HwReset(void)
{
    ...
    // 6: 108 MHz
    // 7: backup DMPLL
    // 8: fractional divider
    // 9: DRAM clock
    // 10: 229.5 MHz
    GFX_HwSetEngClkSrc((INT32)GFX_CLK_SEL_MT5365);
#elif defined(CC_MT5395) ||defined(CC_MT5368)||defined(CC_MT5396)|| defined(CC_MT5389)||defined(CC_MT5880)
        // 1: 240 MHz
    // 2: 216 MHz
    // 3: 168.75 MHz
    // 4: 144 MHz
    ...
    GFX_HwSetEngClkSrc((INT32)GFX_CLK_SEL_MT5395);
#elif defined (CC_MT5398)
    GFX_HwSetEngClkSrc((INT32)GFX_CLK_SEL_MT5398);

#elif defined(CC_MT5387) && !defined(CC_MT5363) 
    // 6: syspll_d1p5_ck = 216MHz
    GFX_HwSetEngClkSrc((INT32)GFX_CLK_SEL_MT5387);

#elif defined (CC_MT5360)
    // do GFX module clock selection (200 MHz)
    //for mtmt5360 200MHz
    //for ECO mt5360 216MHZ
    //for MT5362B 216MHZ
    if( BSP_GetIcVersion() >= IC_VER_5360_BA ) 
    {
        GFX_HwSetEngClkSrc((INT32)GFX_CLK_SEL_MT5362B);
    }
    else if( BSP_GetIcVersion() >= IC_VER_5360_AD ) 
    {
        GFX_HwSetEngClkSrc((INT32)GFX_CLK_SEL_MT5360ECO);
    } 
    else
    {
         GFX_HwSetEngClkSrc((INT32)GFX_CLK_SEL_MT5360);
    }

#else
    // 1: syspll_d1p5_ck (216MHz)
    GFX_HwSetEngClkSrc((INT32)GFX_CLK_SEL_MT5396);
#endif
...
#else
    GFX_WRITE32(GFX_REG_G_CONFIG, 0xA0040000);
#endif
    //enable group 1
...
#else
    //Reset control register too
    u4Value = (u4Value & (~(GREG_MSK_G_RST | GREG_MSK_CQ_RST|GREG_MSK_REG_RST)));
    u4Value = ((u4Value) | (u4ResHi << GREG_SHT_G_RST) | (u4ResHi << GREG_SHT_CQ_RST)|(u4ResHi << GREG_SHT_REG_RST));
    GFX_WRITE32(GFX_REG_G_CONFIG, u4Value);

    u4Value = (u4Value & (~(GREG_MSK_G_RST | GREG_MSK_CQ_RST|GREG_MSK_REG_RST)));
    u4Value = ((u4Value) | (u4ResLo << GREG_SHT_G_RST) | (u4ResLo << GREG_SHT_CQ_RST)|(u4ResLo << GREG_SHT_REG_RST));
    GFX_WRITE32(GFX_REG_G_CONFIG, u4Value);
#endif


#if defined(CC_MT5368)||defined(CC_MT5396)|| defined(CC_MT5389)||defined(CC_MT5398)||defined(CC_MT5880)   //reset vgfx
...
#else    
    u4Value = (u4Value) | (0x3 << GREG_SHT_SDFIFO_THRS);
#endif
...
#else        
    u4Value = (u4Value) | (0x0 << GREG_SHT_POST_THRS);
#endif
