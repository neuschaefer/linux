...
//-----------------------------------------------------------------------------
#define DVBC_MON_POLL_INTRVL             500  // 10ms
#define DVBC_LOCK_TIME_OUT              6000  // 500ms
...
#define DVBC_TUNER_CONN_TIME_THREHOLD   750                          //add 700ms , TDAG4B03A AGC speed too slow
#else
    ...
    while (fgMonitorThreadRunning)
    {
    	if (fgBreakMonitor){
            ...
            if (fgPec)
            {
                ...
                if (mcHOST_CMD(psDvbcDemodCtx, aucData, ccOFST_DEMOD_INFO5_SIZE, 1))
                {
...
#else
                mcSHOW_USER_MSG(("\n************************\n"));
                mcSHOW_USER_MSG(("SNR = %02d\n",  DVBC_GetSignalSNR(psDvbcDemodCtx)/10));
                     ...
                     PTD_SPECIFIC_CTX* pptTDSpecificCtx)
{
	...
	mcSHOW_USER_MSG(("Enter DVBC open\n"));
 ...
 #if (fcTUNER_TYPE != cDVBT_TUNER_ALL) &&(fcTUNER_TYPE != cDVBT_2K10_TUNERLST)  //liuqu,20090716,add for DVBT TunerAll structure
    ITunerRegistration(cITUNER_DONT_CARE);
 #else
    tunerType = checkTunerTypeFromEeprom();
    ITunerRegistration(tunerType);
    Printf("Register DVBC Tunerall type = %d\n",tunerType);		
 #endif
                    ...
                    void* pvArg)
{
   TUNER_CAB_DIG_TUNE_INFO_T *pt_tuner_info = (TUNER_CAB_DIG_TUNE_INFO_T *) pv_conn_info;
    ...
    {	
        case MOD_QAM_16:
            mode = constellation_qam16_dvbc;
            break;
        case MOD_QAM_32:
            mode = constellation_qam32_dvbc;
            break;
        case MOD_QAM_64:
            mode = constellation_qam64_dvbc;
            break;
        case MOD_QAM_128:
            mode = constellation_qam128_dvbc;
            break;
        case MOD_QAM_256:
            mode = constellation_qam256_dvbc;
            break;
        case MOD_UNKNOWN:
            mode = constellation_AutoQam_dvbc;
            break;
        default:
            mode = constellation_qam64_dvbc;
    ...
    if ((u4ConnTime > DVBC_TUNER_CONN_TIME_THREHOLD) && (fgChannelScan))
    {
        ...
        DVBC_Connect(psDvbcDemodCtx, mode, symbolRate);
        ...
        if (fgConnSts) 
        {
	        ...
	        {
	            mcSHOW_DBG_MSG(("DVBC unlock! time-out\n"));
	        }
            else // check Demod lock time only in non-time-out case
            {
                mcSHOW_DBG_MSG(("After %d loop, DVBC lock!\n",ii+1));
                u4ConnTime = (os_get_sys_tick()-u4TickCnt) * x_os_drv_get_tick_period();
	...
	if(fgRetSts)
	{
            DVBC_TSParallelSerialSwitch(psDvbcDemodCtx,TRUE);
	}
#endif
                    ...
                    SIGNAL *_pSignal)
{
	...
	{
		case constellation_qam16_dvbc:  _pSignal->QAMSize = 16; break;//MOD_QAM_16;	 break;
		case constellation_qam32_dvbc:  _pSignal->QAMSize = 32; break;//MOD_QAM_32;	 break;
		case constellation_qam64_dvbc:  _pSignal->QAMSize = 64; break;//MOD_QAM_64;	 break;
		case constellation_qam128_dvbc: _pSignal->QAMSize = 128; break;//MOD_QAM_128;	 break;
		case constellation_qam256_dvbc: _pSignal->QAMSize = 256; break;//MOD_QAM_256;	 break;
...
S16 DVBC_TunerGetSignalLeveldBm(PTD_SPECIFIC_CTX ptTDSpecificCtx, 
                                    DRV_CONN_TYPE_T e_conn_type)
{
    mcSHOW_DBG_MSG4(("DVBC_TunerGetSignalLeveldBm pDVBCPdCtx->hHalLock\n"));                
    mcMUTEX_LOCK(pDVBCPdCtx->hHalLock);
    mcSHOW_DBG_MSG4(("escape DVBC_TunerGetSignalLeveldBm pDVBCPdCtx->hHalLock\n"));         

    mcMUTEX_UNLOCK(pDVBCPdCtx->hHalLock);
	mcSHOW_DBG_MSG4(("DVBC_TunerGetSignalLeveldBm unlock pDVBCPdCtx->hHalLock\n"));
    return 0;
}
    ...
    switch (ucCmdId)
    {
	 ...
	 case 'z':
		...
		case 'e':
         ...
         case 't': 
				if (i4Argc ==2)
			    {
                  ...
                  if((ITuner_SetFreq(ITunerGetCtx(), &SetFreqPara)) != 0)
				  	{
                       printf("Failed on Tuner_SetFreq(%d)", SetFreqPara.Freq);
                       break;
                    }
		...
		case 'r':
		   ...
		   if (DVBC_ExDemodGetReg(psDvbcDemodCtx, u1I2cAddr, u2RegAddr, &ucPar, 1))
		   {		
        ...
        case 'w':
            ...
            if (DVBC_ExDemodSetReg(psDvbcDemodCtx, u1I2cAddr, u2RegAddr, &ucPar, 1))
            {        
                        ...
                        INT32 i4Argc, const CHAR** aszArgv)
{
    ...
    case 1: // one-word cmd
        if (x_strcmp((CHAR *) aszArgv[0], "s") == 0) 
            ...
            default:
                tunerType = checkTunerTypeFromEeprom();
                Printf("tunerType decision(after check EEPROM)=0x%x\r\n",tunerType);
                break;
            }     
        }
        break;
...
/*            mcSHOW_USER_MSG(("DTD PLL CFG (0x2000d180) = 0x%x\n", mcRISC_Reg(CKGEN_BASE, 0x180)));
        	...
        	//DVBT_GetReg(psDvbcDemodCtx, 0x0D, ucData, 1);
        	//DVBT_GetReg(psDvbcDemodCtx, 0x0E, ucData+1, 1);
        	mcSHOW_USER_MSG(("uP TOGGLE= 0x%02x\n", ucData[0]));
        	mcSHOW_USER_MSG(("uP RUN_TAG= 0x%02x\n", ucData[1]));

         	DVBC_GetReg(psDvbcDemodCtx, 0x1A0, ucData, 2);
         	//DVBT_GetReg(psDvbcDemodCtx, 0x1A0, ucData, 1);
         	//DVBT_GetReg(psDvbcDemodCtx, 0x1A1, ucData+1, 1);
        	mcSHOW_USER_MSG(("IFAGC Target Level Set= 0x%02x%02x\n", ucData[1], ucData[0]));

        	DVBC_GetReg(psDvbcDemodCtx, 0x1D7, ucData, 2);
        	//DVBT_GetReg(psDvbcDemodCtx, 0x1d7, ucData, 1);
         	//DVBT_GetReg(psDvbcDemodCtx, 0x1d8, ucData+1, 1);
...
{
	//DVBC_TunerI2c(ptTDSpecificCtx,bDetach);
}
...
INT16 DVBC_TunerGetAttribute(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_GET_TYPE_T  e_get_type, VOID *pAnazAttribute, SIZE_T* pzAnaAttributeLen)
{
    ...
    case TUNER_GET_TYPE_DIG_ATTRIBUTE:
        ...
        #if 0
       	switch (DVBC_GetBW(psDvbcDemodCtx))
		{
    ...
    case TUNER_GET_TYPE_CELL_ID:
    	  *pzAnaAttributeLen = sizeof(UINT16);
    	  u2Temp = DVBC_GetCellId(psDvbcDemodCtx);
...
/***********************************************************************/
void DVBC_SetDmxExt(void)
{
	
    mcSHOW_DBG_MSG4(("DVBC_SetDmxExt pDVBCPdCtx->hHalLock\n")); 
    mcMUTEX_LOCK(pDVBCPdCtx->hHalLock);
    mcSHOW_DBG_MSG4(("escape DVBC_SetDmxExt pDVBCPdCtx->hHalLock\n")); 
        
    mcMUTEX_UNLOCK(pDVBCPdCtx->hHalLock);
    mcSHOW_DBG_MSG4(("DVBC_SetDmxExt unlock pDVBCPdCtx->hHalLock\n")); 
...
{	
	DVBC_TunerI2c(ptTDSpecificCtx,bSwitchOn);	
}
