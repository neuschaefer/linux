...
STATIC DEMOD_I2C_DATA_T InitTbl_1[]=
	  {
...
STATIC DEMOD_I2C_DATA_T InitTbl_2[]=
	  {{0x0606,0x02},
	   {0x010c,0x01},
	   {0x10f7,0xc0},
	   {0x10fb,0x00}, //PWM for ATBM8846, ATBM8846E Hardware compatible
	   {0x0af9,0x00},
...
BOOL ATBMWrite(UINT8 u1I2cAddr,UINT16 u2RegAddr, UINT8 data)
{
	...
	#endif
	mcSHOW_DBG_MSG4((" Write 0x%02X:0x%02X-0x%02X-0x%02X\n",u1I2cAddr,u1RegAddr[0],u1RegAddr[1],data));
	return TRUE;	
}
 ...
 if(ATBM_ChipID == 0x30)
  {
    switch(portSts)
 	{
	 case (TESTIO10_0 | TESTIO23_0):
	 {	fgRet&=ATBMWrite(u1I2cAddr,0x10fb,0x84);}
	 break;
	 case (TESTIO10_0 | TSTTIO23_1):
	 {   fgRet&=ATBMWrite(u1I2cAddr,0x10fb,0x94);}
	 break;
	 default:
	 break;
 	}
...
UINT8 u1Gpio7Sts,u1Gpio8Sts;
 ...
 if(fgAnalog)//ATV
 {
   ...
   if(ATBM_ChipID == 0x30)
   	{ 
   	fgSts&=SetDemodIO(u1I2cAddr,TSTTIO23_1);  //pin54
   	}
 }
 else//DTV
 {
   ...
   if(ATBM_ChipID == 0x30)
   	{ 
   	fgSts&=SetDemodIO(u1I2cAddr,TESTIO23_0);  //pin54 
   	}
 }
 if(!fgSts)
 {
...
BOOL ATBM_Standby(UINT8 u1I2cAddr)
{
 ...
 if(ATBM_ChipID == 0x20)
 	{ 
	 fgRet&=ATBMWrite(u1I2cAddr,0x0008, 0x01); 
 	}
 if(ATBM_ChipID == 0x30)
 	{
...
BOOL ATBM_WakeUp(UINT8 u1I2cAddr)
{
 ...
 fgRet&=ATBMWrite(u1I2cAddr,0x0301,u1Data);
  ...
  if(ATBM_ChipID == 0x20)
	{
	fgRet&=ATBMWrite(u1I2cAddr,0x0008, 0x00);
	}
  if(ATBM_ChipID == 0x30)
	{	 
		fgRet&= ATBMRead(u1I2cAddr,0x10f7,&u1Data);	
...
STATIC BOOL ATBM_SetIF(UINT8 u1I2cAddr)
{
  ...
  {
   u4ADC_Rate=0x100000;
   fgRet&=ATBMWrite(u1I2cAddr,0x0A03,0x00);
   fgRet&=ATBMWrite(u1I2cAddr,0x0A04,0x00);
   fgRet&=ATBMWrite(u1I2cAddr,0x0A05,0x10);
  }
  else
  {
  ...
  u1Data=(UINT8)((u4ADC_Rate>>16)&0x1F);
   ...
   if(ATBM_ChipID == 0x20)
	{
	  ...
	  fgRet &=ATBMWriteDemodData(u1I2cAddr,InitTbl_1,sizeof(InitTbl_1)/sizeof(InitTbl_1[0]));
	}
   if(ATBM_ChipID == 0x30)
	{
	 fgRet &=ATBMWriteDemodData(u1I2cAddr,InitTbl_2,sizeof(InitTbl_2)/sizeof(InitTbl_2[0])); 
	 ATBM_SwitchIfAGC(u1I2cAddr,FALSE);
	}
...
BOOL ATBMIsDemodOk(UINT8 u1I2cAddr,BOOL * fgNeedReset)
{
 ...
 {	PLL_state=ATBM_Check_PLL_Status(u1I2cAddr);
	...
	{if((u1Data&0x02)==0x02)
 	...
 	if(ii==4)
 	   {fgRet&=ATBMWrite(u1I2cAddr,0x060d, 0x60);
