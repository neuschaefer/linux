...
#include "eq_script_lgtdvwh810f_5388.h"


//-----------------------------------------------------------------------------
// Constant definitions
//-----------------------------------------------------------------------------
#define LG_TDVWH810F_BASE            ((UINT16) 0xC2)
#define LG_TDVWH810F_IF_FREQUENCY_ANA    ((UINT16) 45750)   /* KHz */
#define LG_TDVWH810F_IF_FREQUENCY    ((UINT16) 44000)   /* KHz */
#define LG_TDVWH810F_LO_STEP         ((UINT16) 6250)  /* 10Hz */
#define LG_TDVWH810F_ANA_LO_STEP         ((UINT16) 6250)  /* 10Hz */
#define LG_TDVWH810F_STR             "LG_TDVWH810F 2010-01-01\n 2011-04-09\n v1.0"

//PLL lock check parameters
#define LG_TDVWH810F_PLL_POLL_INTERVAL      10 //ms
#define LG_TDVWH810F_PLL_POLL_TIMETOUT      100 //ms
#define LG_TDVWH810F_PLL_POLL_CNT           LG_TDVWH810F_PLL_POLL_TIMETOUT/LG_TDVWH810F_PLL_POLL_INTERVAL //counter
...
STATIC UINT8* pTdvwh810fEqNormal[MOD_ANA_TYPE_END - MOD_ANA_TYPE_BEGIN] = {NULL};
STATIC UINT8* pTdvwh810fEqWeak[MOD_ANA_TYPE_END - MOD_ANA_TYPE_BEGIN] = {NULL};
//Tuner releated ATD register
STATIC ATD_TUNER_REL_REG_T arrAtdRelReg[]={
 //CR case for AMFV; Clipping effect;modify 100716
 {0x2b7,0x13},//PRA target level[11:4] upper boundary
 {0x2cb,0x01},
 {0x2d5,0x04},//IF_BIAS1
 {0x2d6,0x04},//IF_BIAS2
 {0x2d8,0xf4},//IF_SLOPE1
 {0x2d9,0xf4},//IF_SLOPE2
 {0x0,0x0}//end flag
 };
//
/** TDVWH810F_TunerInit
    ...
    )
{
    ...
    psTunerCtx->I2cAddress = LG_TDVWH810F_BASE;
    psTunerCtx->u4RF_Freq = 0;
    psTunerCtx->fgRFAGC = FALSE;
    /* Specific part: Only for this tuner model */
    psTunerCtx->u2IF_Freq = LG_TDVWH810F_IF_FREQUENCY;
    psTunerCtx->u2IF_Freq_A = LG_TDVWH810F_IF_FREQUENCY_ANA;
    ...
    // init EQ script
    pTdvwh810fEqNormal[MOD_NTSC_M - MOD_ANA_TYPE_BEGIN]  = EQLGTDVWH810FMN;
    pTdvwh810fEqWeak[MOD_NTSC_M - MOD_ANA_TYPE_BEGIN]  = EQLGTDVWH810FMNWeak;

    //pTdvwh810fEqNormal[MOD_NTSC_M_54M_44M]  = EQLGTDVWH810FMN;
    //pTdvwh810fEqNormal[MOD_NTSC_M_54M_36M]  = EQLGTDVWH810FMN;
   psTunerSpecificCtx->u2LO_Step=LG_TDVWH810F_ANA_LO_STEP;
   
    mcSHOW_USER_MSG(("Init " LG_TDVWH810F_STR "(TAR=0x3D)\n"));
}
...
u1Mod=param->Modulation;
   ...
   mcSHOW_DBG_MSG(("LG TDVWH810F_TunerSetFreq %dKHz\n",param->Freq));
  if ( u1Mod>= MOD_ANA_TYPE_BEGIN )
    ...
    if(!fgAnalogSignal){//digital  mode use digital LO step
      Ndivider = (UINT16) ((Lo * 100) / LG_TDVWH810F_LO_STEP);
    }
    else{
     Ndivider = (UINT16) ((Lo * 100) /LG_TDVWH810F_ANA_LO_STEP);
    }
...
/* Assign Control word for specific tuner */
   ...
   if ((u1Mod > MOD_ATSC_8VSB)&&(u1Mod < MOD_TYPE_END))
      CB1 = 0x9B;//qam top=103dBuV

    if ((fgAnalogSignal)&&(freq>=90)&&(freq<=96))                     // For NTSC
    	{
    		 CB1 = 0xA3;	//channel95 top=100dBuV
    	//	 CB2 = 0xC3; //T0/XLO : 1
    	   //CB1 = 0xBB;
	   //CB1 = 0xA3;
       //   CB2 = 0xC2;
...
/* LO settings */
 ...
 /* Write to tuner device */
	...
	for(ii=0;ii<LG_TDVWH810F_PLL_POLL_CNT;ii++)
	{
	  mcDELAY_MS(LG_TDVWH810F_PLL_POLL_INTERVAL);
	  if(TDVWH810F_TunerGetStatus(psTunerCtx))break;
	}
	
	if(ii<LG_TDVWH810F_PLL_POLL_CNT)
	{
	...
	else
	{
	 mcSHOW_HW_MSG(("PLL unlock !(%d)",ii));
        ...
        if(fgAnalogSignal){
            psTunerCtx->u4RF_Freq=(Ndivider*LG_TDVWH810F_ANA_LO_STEP)/100-psTunerCtx->u2IF_Freq_A;
        }
        else{
            psTunerCtx->u4RF_Freq=(Ndivider*LG_TDVWH810F_LO_STEP)/100-psTunerCtx->u2IF_Freq;
        }
 ...
 */
//-----------------------------------------------------------------------------
STATIC INT16 TDVWH810F_SignalLevel_Handler(ITUNER_CTX_T * pTCtx,CMD_SIGLVL_PARAM_T * rCmd){
     INT16 ret=ITUNER_OK;
...
INT16 TDVWH810F_TunerOP(ITUNER_CTX_T * pTCtx, ITUNEROP_T eOperation, UINT8 SetVal,  VOID * pInOutVal){
    ...
    case itGetStatus:
        ...
        *Retval=TDVWH810F_TunerGetStatus(pTCtx);
        }
        break;
    case itGetVer:
        {
    ...
    case itGetSignalLevel:
        ...
        return TDVWH810F_SignalLevel_Handler(pTCtx,SigLvlParam);
        }

    case itGetEqScriptNormal: 
        *(UINT8**)pInOutVal = pTdvwh810fEqNormal[SetVal];
        break;

    case itGetEqScriptWeak: 
        *(UINT8**)pInOutVal = pTdvwh810fEqWeak[SetVal];
        break;
