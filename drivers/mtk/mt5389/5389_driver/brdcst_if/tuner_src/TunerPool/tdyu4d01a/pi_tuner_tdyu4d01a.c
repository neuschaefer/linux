 ...
 * $Author: dtvbm11 $
 * $Date: 2012/04/27 $
 * $RCSfile: pi_tuner_TDYU4D01A.c,v $
 * $Revision: #1 $
 *
 *---------------------------------------------------------------------------*/

/** @file pi_tuner_TDYU4D01A.c
 *  Tuner control for SS TDYU4D01A
 */
...
#include "eq_script_tdyu4d01a_5365.h"


//-----------------------------------------------------------------------------
// Constant definitions
//-----------------------------------------------------------------------------
#define ALPS_TDYU4D01A_BASE            ((UINT16) 0xC2)
#define ALPS_TDYU4D01A_IF_FREQUENCY_ANA    ((UINT16) 45750)   /* KHz */
#define ALPS_TDYU4D01A_IF_FREQUENCY    ((UINT16) 44000)   /* KHz */
#define ALPS_TDYU4D01A_LO_STEP         ((UINT16) 6250)  /* 10Hz */
#define ALPS_TDYU4D01A_ANA_LO_STEP         ((UINT16) 6250)  /* 10Hz */
#define ALPS_TDYU4D01A_STR             "ALPS_TDYU4D01A 2010-01-01\n 2011-04-09\n v1.0"

//PLL lock check parameters
#define ALPS_TDYU4D01A_PLL_POLL_INTERVAL      10 //ms
#define ALPS_TDYU4D01A_PLL_POLL_TIMETOUT      100 //ms spec
#define ALPS_TDYU4D01A_PLL_POLL_CNT           ALPS_TDYU4D01A_PLL_POLL_TIMETOUT/ALPS_TDYU4D01A_PLL_POLL_INTERVAL //counter
...
STATIC UINT8* pTDYU4D01AEqNormal[MOD_ANA_TYPE_END - MOD_ANA_TYPE_BEGIN] = {NULL};
STATIC UINT8* pTDYU4D01AEqWeak[MOD_ANA_TYPE_END - MOD_ANA_TYPE_BEGIN] = {NULL};

//
/** Tdyu4d01a_TunerInit
 *  Tuner initialzation forTdyu4d01a.
 *  @param  pTCtx       Pointer of pi tuner driver context.
 *  @retval void
 */
//-----------------------------------------------------------------------------
VOID TDYU4D01A_TunerInit(
    ITUNER_CTX_T *pTCtx
    )
{
    ...
    psTunerCtx->I2cAddress = ALPS_TDYU4D01A_BASE;
    psTunerCtx->u4RF_Freq = 0;
    psTunerCtx->fgRFAGC = FALSE;
    /* Specific part: Only for this tuner model */
    psTunerCtx->u2IF_Freq = ALPS_TDYU4D01A_IF_FREQUENCY;
    psTunerCtx->u2IF_Freq_A = ALPS_TDYU4D01A_IF_FREQUENCY_ANA;
    ...
    // init EQ script
    pTDYU4D01AEqNormal[MOD_NTSC_M - MOD_ANA_TYPE_BEGIN]  = EQTDYU4D01AMN;
    pTDYU4D01AEqWeak[MOD_NTSC_M - MOD_ANA_TYPE_BEGIN]  = EQTDYU4D01AMNWeak;

   psTunerSpecificCtx->u2LO_Step=ALPS_TDYU4D01A_ANA_LO_STEP;
   
    mcSHOW_USER_MSG(("Init " ALPS_TDYU4D01A_STR "\n"));
}

//----------------------------------------------------------------------------- 
/**  TDYU4D01A_TunerGetStatus
 *  Set Tuner PLL staus forTDYU4D01A/TDYU4D01A
 *  @param  pTCtx       Pointer of pi tuner driver context.
 *  @retval TRUE        PLL lock
 *  @retval FALSE       PLL unlock
 */
//-----------------------------------------------------------------------------
STATIC BOOL TDYU4D01A_TunerGetStatus(ITUNER_CTX_T *pTCtx)
{
    ITUNER_CTX_T *psTunerCtx = pTCtx;
...
//----------------------------------------------------------------------------- 
/** TDYU4D01A_TunerSetFreq
 *  Set Tuner PLL forTDYU4D01A/TDYU4D01A to tune RF frequency.
 *  @param  pTCtx       Pointer of pi tuner driver context.
...
u1Mod=param->Modulation;
   ...
   mcSHOW_DBG_MSG(("ALPS TDYU4D01A_TunerSetFreq %dKHz\n",param->Freq));
  if ( u1Mod>= MOD_ANA_TYPE_BEGIN )
    ...
    if(!fgAnalogSignal){//digital  mode use digital LO step
      Ndivider = (UINT16) ((Lo * 100) / ALPS_TDYU4D01A_LO_STEP);
    }
    else{
     Ndivider = (UINT16) ((Lo * 100) /ALPS_TDYU4D01A_ANA_LO_STEP);
    }
...
/* LO settings */
 ...
 /* Write to tuner device */
	...
	for(ii=0;ii<ALPS_TDYU4D01A_PLL_POLL_CNT;ii++)
	{
	  mcDELAY_MS(ALPS_TDYU4D01A_PLL_POLL_INTERVAL);
	  if(TDYU4D01A_TunerGetStatus(psTunerCtx))break;
	}
	
	if(ii<ALPS_TDYU4D01A_PLL_POLL_CNT)
	{
	...
	else
	{
	 mcSHOW_HW_MSG(("PLL unlock !(%d)\n",ii));
        ...
        if(fgAnalogSignal){
            psTunerCtx->u4RF_Freq=(Ndivider*ALPS_TDYU4D01A_ANA_LO_STEP)/100-psTunerCtx->u2IF_Freq_A;
        }
        else{
            psTunerCtx->u4RF_Freq=(Ndivider*ALPS_TDYU4D01A_LO_STEP)/100-psTunerCtx->u2IF_Freq;
        }
 ...
 */
//-----------------------------------------------------------------------------
STATIC CHAR *TDYU4D01A_TunerGetVer(VOID)
{
    return (ALPS_TDYU4D01A_STR);
}
/***********************************************************************/
//----------------------------------------------------------------------------- 
 ...
 */
//-----------------------------------------------------------------------------
STATIC INT16 TDYU4D01A_SignalLevel_Handler(ITUNER_CTX_T * pTCtx,CMD_SIGLVL_PARAM_T * rCmd){
     INT16 ret=ITUNER_OK;
 ...
 */
//-----------------------------------------------------------------------------
INT16 TDYU4D01A_TunerOP(ITUNER_CTX_T * pTCtx, ITUNEROP_T eOperation, UINT8 SetVal,  VOID * pInOutVal){
         
    switch(eOperation){
    case itGetStatus:
        {
        BOOL * Retval=(BOOL *)pInOutVal;
        *Retval=TDYU4D01A_TunerGetStatus(pTCtx);
        }
        break;
    case itGetVer:
        {
        CHAR ** RetStr=(CHAR **)pInOutVal;
        *RetStr=TDYU4D01A_TunerGetVer();
        }
        break;
    case itGetSignalLevel:
        {
        CMD_SIGLVL_PARAM_T * SigLvlParam=(CMD_SIGLVL_PARAM_T *)pInOutVal;
        return TDYU4D01A_SignalLevel_Handler(pTCtx,SigLvlParam);
        }

    case itGetEqScriptNormal: 
        *(UINT8**)pInOutVal = pTDYU4D01AEqNormal[SetVal];
        break;

    case itGetEqScriptWeak: 
        *(UINT8**)pInOutVal = pTDYU4D01AEqWeak[SetVal];
        break;
