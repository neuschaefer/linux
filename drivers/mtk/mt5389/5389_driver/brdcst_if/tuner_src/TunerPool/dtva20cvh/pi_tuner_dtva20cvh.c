 ...
 * $Author: dtvbm11 $
 * $Date: 2012/04/27 $
 * $RCSfile: pi_tuner_DTVA20CVH.c,v $
 * $Revision: #1 $
 *
 *---------------------------------------------------------------------------*/

/** @file pi_tuner_DTVA20CVH.c
 *  Tuner control for SS DTVA20CVH
 */
...
#include "eq_script_dtva20cvh.h"


//-----------------------------------------------------------------------------
// Constant definitions
//-----------------------------------------------------------------------------
#define SS_DTVA20CVH_BASE            ((UINT16) 0xC0)
#define SS_DTVA20CVH_IF_FREQUENCY_ANA    ((UINT16) 45750)   /* KHz */
#define SS_DTVA20CVH_IF_FREQUENCY    ((UINT16) 44000)   /* KHz */
#define SS_DTVA20CVH_LO_STEP         ((UINT16) 6250)  /* 10Hz */
#define SS_DTVA20CVH_ANA_LO_STEP         ((UINT16) 6250)  /* 10Hz */
#define SS_DTVA20CVH_STR             "DTVA20CVH 2010-01-01\n 2011-04-06\n v1.0"

//PLL lock check parameters
#define SS_DTVA20CVH_PLL_POLL_INTERVAL      10 //ms
#define SS_DTVA20CVH_PLL_POLL_TIMETOUT      120 //ms typ by tuner spec
#define SS_DTVA20CVH_PLL_POLL_CNT           SS_DTVA20CVH_PLL_POLL_TIMETOUT/SS_DTVA20CVH_PLL_POLL_INTERVAL //counter
...
STATIC UINT8* pDTVA20CVHEqNormal[MOD_ANA_TYPE_END - MOD_ANA_TYPE_BEGIN] = {NULL};
STATIC UINT8* pDTVA20CVHEqWeak[MOD_ANA_TYPE_END - MOD_ANA_TYPE_BEGIN] = {NULL};

STATIC UINT8 u1AnaTop=3;
//
/** DTVA20CVH_TunerInit
 *  Tuner initialzation forDTVA20CVH.
 *  @param  pTCtx       Pointer of pi tuner driver context.
 *  @retval void
 */
//-----------------------------------------------------------------------------
VOID DTVA20CVH_TunerInit(
    ITUNER_CTX_T *pTCtx
    )
{
    ...
    psTunerCtx->I2cAddress = SS_DTVA20CVH_BASE;
    psTunerCtx->u4RF_Freq = 0;
    psTunerCtx->fgRFAGC = FALSE;
    /* Specific part: Only for this tuner model */
    psTunerCtx->u2IF_Freq = SS_DTVA20CVH_IF_FREQUENCY;
    psTunerCtx->u2IF_Freq_A = SS_DTVA20CVH_IF_FREQUENCY_ANA;
    ...
    // init EQ script
    pDTVA20CVHEqNormal[MOD_NTSC_M - MOD_ANA_TYPE_BEGIN]  = EQSSDTVA20CVHMN;
    pDTVA20CVHEqWeak[MOD_NTSC_M - MOD_ANA_TYPE_BEGIN]  = EQSSDTVA20CVHMNWeak;

    //pDTVA20CVHEqNormal[MOD_NTSC_M_54M_44M]  = EQSSDTVA20CVHMN;
    //pDTVA20CVHEqNormal[MOD_NTSC_M_54M_36M]  = EQSSDTVA20CVHMN;
   psTunerSpecificCtx->u2LO_Step=SS_DTVA20CVH_ANA_LO_STEP;
   
    mcSHOW_USER_MSG(("Init " SS_DTVA20CVH_STR "\n"));
}

//----------------------------------------------------------------------------- 
/**  DTVA20CVH_TunerGetStatus
 *  Set Tuner PLL staus forDTVA20CVH/DTVA20CVH
 *  @param  pTCtx       Pointer of pi tuner driver context.
 *  @retval TRUE        PLL lock
 *  @retval FALSE       PLL unlock
 */
//-----------------------------------------------------------------------------
STATIC BOOL DTVA20CVH_TunerGetStatus(ITUNER_CTX_T *pTCtx)
{
    ITUNER_CTX_T *psTunerCtx = pTCtx;
    ...
    if (mcTEST_BIT(TunerS, 6))
        return TRUE;
    return FALSE;
}

//----------------------------------------------------------------------------- 
/** DTVA20CVH_TunerSetFreq
 *  Set Tuner PLL forDTVA20CVH/DTVA20CVH to tune RF frequency.
 *  @param  pTCtx       Pointer of pi tuner driver context.
...
u1Mod=param->Modulation;
   ...
   mcSHOW_DBG_MSG(("SS DTVA20CVH_TunerSetFreq %dKHz\n",param->Freq));
  if ( u1Mod>= MOD_ANA_TYPE_BEGIN )
    ...
    if(!fgAnalogSignal){//digital  mode use digital LO step
      Ndivider = (UINT16) ((Lo * 100) / SS_DTVA20CVH_LO_STEP);
    }
    else{
     Ndivider = (UINT16) ((Lo * 100) /SS_DTVA20CVH_ANA_LO_STEP);
    }
...
/* Assign Control word for specific tuner */
   ...
   if(fgAnalogSignal)
   {
    CB1&=0xC7;
	CB1|=(u1AnaTop<<3);
	if(((freq>82) && (freq<88)) || ((freq>90) && (freq<96))) 
	{
	 CB1=0xB3;
	}
   }
...
/* LO settings */
 ...
 /* Write to tuner device */
	...
	for(ii=0;ii<SS_DTVA20CVH_PLL_POLL_CNT;ii++)
	{
	  mcDELAY_MS(SS_DTVA20CVH_PLL_POLL_INTERVAL);
	  if(DTVA20CVH_TunerGetStatus(psTunerCtx))break;
	}
	
	if(ii<SS_DTVA20CVH_PLL_POLL_CNT)
	{
	...
	else
	{
	 mcSHOW_HW_MSG(("PLL unlock !(%d)",ii));
        ...
        if(fgAnalogSignal){
            psTunerCtx->u4RF_Freq=(Ndivider*SS_DTVA20CVH_ANA_LO_STEP)/100-psTunerCtx->u2IF_Freq_A;
        }
        else{
            psTunerCtx->u4RF_Freq=(Ndivider*SS_DTVA20CVH_LO_STEP)/100-psTunerCtx->u2IF_Freq;
        }
 ...
 */
//-----------------------------------------------------------------------------
STATIC CHAR *DTVA20CVH_TunerGetVer(VOID)
{
    return (SS_DTVA20CVH_STR);
}
/***********************************************************************/
//----------------------------------------------------------------------------- 
 ...
 */
//-----------------------------------------------------------------------------
STATIC INT16 DTVA20CVH_SignalLevel_Handler(ITUNER_CTX_T * pTCtx,CMD_SIGLVL_PARAM_T * rCmd){
     INT16 ret=ITUNER_OK;
 ...
 */
//-----------------------------------------------------------------------------
INT16 DTVA20CVH_TunerOP(ITUNER_CTX_T * pTCtx, ITUNEROP_T eOperation, UINT8 SetVal,  VOID * pInOutVal){
         
    switch(eOperation){
    case itGetStatus:
        {
        BOOL * Retval=(BOOL *)pInOutVal;
        *Retval=DTVA20CVH_TunerGetStatus(pTCtx);
        }
        break;
    case itGetVer:
        {
        CHAR ** RetStr=(CHAR **)pInOutVal;
        *RetStr=DTVA20CVH_TunerGetVer();
        }
        break;
    case itGetSignalLevel:
        {
        CMD_SIGLVL_PARAM_T * SigLvlParam=(CMD_SIGLVL_PARAM_T *)pInOutVal;
        return DTVA20CVH_SignalLevel_Handler(pTCtx,SigLvlParam);
        }

    case itGetEqScriptNormal: 
        *(UINT8**)pInOutVal = pDTVA20CVHEqNormal[SetVal];
        break;

    case itGetEqScriptWeak: 
        *(UINT8**)pInOutVal = pDTVA20CVHEqWeak[SetVal];
        break;
