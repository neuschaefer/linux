 ...
 * $Author: dtvbm11 $
 * $Date: 2012/04/27 $
 * $RCSfile: pi_tuner_DVT10A.c,v $
 * $Revision: #1 $
 *
 *---------------------------------------------------------------------------*/

/** @file pi_tuner_DVT10A.c
 *  Tuner control for SS DVT10A
 */
...
#include "eq_script_xgdvt10a.h"


//-----------------------------------------------------------------------------
// Constant definitions
//-----------------------------------------------------------------------------
#define XG_DVT10A_BASE            ((UINT16) 0xC2)
#define XG_DVT10A_IF_FREQUENCY_ANA    ((UINT16) 45750)   /* KHz */
#define XG_DVT10A_IF_FREQUENCY    ((UINT16) 44000)   /* KHz */
#define XG_DVT10A_LO_STEP         ((UINT16) 6250)  /* 10Hz */
#define XG_DVT10A_ANA_LO_STEP         ((UINT16) 6250)  /* 10Hz */
#define XG_DVT10A_STR             "XG_DVT10A 2010-01-01\n 2011-04-06\n v1.0"

//PLL lock check parameters
#define XG_DVT10A_PLL_POLL_INTERVAL      10 //ms
#define XG_DVT10A_PLL_POLL_TIMETOUT      100 //ms
#define XG_DVT10A_PLL_POLL_CNT           XG_DVT10A_PLL_POLL_TIMETOUT/XG_DVT10A_PLL_POLL_INTERVAL //counter
...
STATIC UINT8* pDVT10AEqNormal[MOD_ANA_TYPE_END - MOD_ANA_TYPE_BEGIN] = {NULL};
STATIC UINT8* pDVT10AEqWeak[MOD_ANA_TYPE_END - MOD_ANA_TYPE_BEGIN] = {NULL};

//
/** DVT10A_TunerInit
 *  Tuner initialzation forDVT10A.
 *  @param  pTCtx       Pointer of pi tuner driver context.
 *  @retval void
 */
//-----------------------------------------------------------------------------
VOID DVT10A_TunerInit(
    ITUNER_CTX_T *pTCtx
    )
{
    ...
    psTunerCtx->I2cAddress = XG_DVT10A_BASE;
    psTunerCtx->u4RF_Freq = 0;
    psTunerCtx->fgRFAGC = FALSE;
    /* Specific part: Only for this tuner model */
    psTunerCtx->u2IF_Freq = XG_DVT10A_IF_FREQUENCY;
    psTunerCtx->u2IF_Freq_A = XG_DVT10A_IF_FREQUENCY_ANA;
    ...
    // init EQ script
    pDVT10AEqNormal[MOD_NTSC_M - MOD_ANA_TYPE_BEGIN]  = EQXGDVT10AMN;
    pDVT10AEqWeak[MOD_NTSC_M - MOD_ANA_TYPE_BEGIN]  = EQXGDVT10AMNWeak;

    //pDVT10AEqNormal[MOD_NTSC_M_54M_44M]  = EQXGDVT10AMN;
    //pDVT10AEqNormal[MOD_NTSC_M_54M_36M]  = EQXGDVT10AMN;
   psTunerSpecificCtx->u2LO_Step=XG_DVT10A_ANA_LO_STEP;
   
    mcSHOW_USER_MSG(("Init " XG_DVT10A_STR "\n"));
}

//----------------------------------------------------------------------------- 
/**  DVT10A_TunerGetStatus
 *  Set Tuner PLL staus forDVT10A/DVT10A
 *  @param  pTCtx       Pointer of pi tuner driver context.
 *  @retval TRUE        PLL lock
 *  @retval FALSE       PLL unlock
 */
//-----------------------------------------------------------------------------
STATIC BOOL DVT10A_TunerGetStatus(ITUNER_CTX_T *pTCtx)
{
    ITUNER_CTX_T *psTunerCtx = pTCtx;
...
//----------------------------------------------------------------------------- 
/** DVT10A_TunerSetFreq
 *  Set Tuner PLL forDVT10A/DVT10A to tune RF frequency.
 *  @param  pTCtx       Pointer of pi tuner driver context.
...
u1Mod=param->Modulation;
   ...
   mcSHOW_DBG_MSG(("XG DVT10A_TunerSetFreq %dKHz\n",param->Freq));
  if ( u1Mod>= MOD_ANA_TYPE_BEGIN )
    ...
    if(!fgAnalogSignal){//digital  mode use digital LO step
      Ndivider = (UINT16) ((Lo * 100) / XG_DVT10A_LO_STEP);
    }
    else{
     Ndivider = (UINT16) ((Lo * 100) /XG_DVT10A_ANA_LO_STEP);
    }
   // freq =freq /1000;//convert to unit MHZ

/* Assign Control word for specific tuner */
    if (freq < 53000 || freq > 865000)
    {
...
/* LO settings */
 ...
 /* Write to tuner device */
	...
	for(ii=0;ii<XG_DVT10A_PLL_POLL_CNT;ii++)
	{
	  mcDELAY_MS(XG_DVT10A_PLL_POLL_INTERVAL);
	  if(DVT10A_TunerGetStatus(psTunerCtx))break;
	}
	
	if(ii<XG_DVT10A_PLL_POLL_CNT)
	{
	...
	else
	{
	 mcSHOW_HW_MSG(("PLL unlock !(%d)",ii));
        ...
        if(fgAnalogSignal){
            psTunerCtx->u4RF_Freq=(Ndivider*XG_DVT10A_ANA_LO_STEP)/100-psTunerCtx->u2IF_Freq_A;
        }
        else{
            psTunerCtx->u4RF_Freq=(Ndivider*XG_DVT10A_LO_STEP)/100-psTunerCtx->u2IF_Freq;
        }
 ...
 */
//-----------------------------------------------------------------------------
STATIC CHAR *DVT10A_TunerGetVer(VOID)
{
    return (XG_DVT10A_STR);
}
/***********************************************************************/
//----------------------------------------------------------------------------- 
 ...
 */
//-----------------------------------------------------------------------------
STATIC INT16 DVT10A_SignalLevel_Handler(ITUNER_CTX_T * pTCtx,CMD_SIGLVL_PARAM_T * rCmd){
     INT16 ret=ITUNER_OK;
 ...
 */
//-----------------------------------------------------------------------------
INT16 DVT10A_TunerOP(ITUNER_CTX_T * pTCtx, ITUNEROP_T eOperation, UINT8 SetVal,  VOID * pInOutVal){
         
    switch(eOperation){
    case itGetStatus:
        {
        BOOL * Retval=(BOOL *)pInOutVal;
        *Retval=DVT10A_TunerGetStatus(pTCtx);
        }
        break;
    case itGetVer:
        {
        CHAR ** RetStr=(CHAR **)pInOutVal;
        *RetStr=DVT10A_TunerGetVer();
        }
        break;
    case itGetSignalLevel:
        {
        CMD_SIGLVL_PARAM_T * SigLvlParam=(CMD_SIGLVL_PARAM_T *)pInOutVal;
        return DVT10A_SignalLevel_Handler(pTCtx,SigLvlParam);
        }

    case itGetEqScriptNormal: 
        *(UINT8**)pInOutVal = pDVT10AEqNormal[SetVal];
        break;

    case itGetEqScriptWeak: 
        *(UINT8**)pInOutVal = pDVT10AEqWeak[SetVal];
        break;
