 ...
 * $Author: dtvbm11 $
 * $Date: 2012/04/27 $
 * $RCSfile: pi_tuner_va1e1bf2403.c,v $
 * $Revision: #1 $
 *
 *---------------------------------------------------------------------------*/

/** @file pi_tuner_va1e1bf2403.c
 *  Tuner control for sahrp va1e1bf2403
 */
...
#include "eq_script_va1e1bf2403.h"
#include "sif_if.h"
#include "x_gpio.h"
...
UINT32 u4GpioRfAgcNum = 22;   //MT5395, GPIO[22]
#else
UINT32 u4GpioRfAgcNum = 60;    //MT5363, GPIO[9]
#endif

//-----------------------------------------------------------------------------
// Constant definitions
//-----------------------------------------------------------------------------
#define SHARP_VA1E1BF2403_BASE                           ((UINT8) 0xC0)
#define SHARP_VA1E1BF2403_STR                             "SHARP_VA1E1BF2403 2010-01-01\n 2011-04-09\n v1.0"

#define SHARP_VA1E1BF2403_IF_FREQUENCY           ((UINT16) 44000)   /* KHz */
#define SHARP_VA1E1BF2403_IF_FREQUENCY_ANA  ((UINT16) 45750)   /* KHz */
//#define SHARP_VA1E1BF2403_LO_STEP         ((UINT16) 14286)  /* 10Hz */
#define SHARP_VA1E1BF2403_ANA_LO_STEP            ((UINT16) 5000)  /* 10Hz */
...
STATIC UINT8* pVA1E1BF2403EqNormal[MOD_ANA_TYPE_END - MOD_ANA_TYPE_BEGIN] = {NULL};
STATIC UINT8* pVA1E1BF2403EqWeak[MOD_ANA_TYPE_END - MOD_ANA_TYPE_BEGIN] = {NULL};
...
//-----------------------------------------------------------------------------
/** VA1E1BF2403_TunerInit
 *  Tuner initialzation forVA1E1BF2403.
 *  @param  pTCtx       Pointer of pi tuner driver context.
 *  @retval void
 */
//-----------------------------------------------------------------------------
VOID VA1E1BF2403_TunerInit(ITUNER_CTX_T *pTCtx)
{
    ...
    psTunerCtx->I2cAddress = SHARP_VA1E1BF2403_BASE;
    psTunerCtx->u4RF_Freq = 0;
	
    psTunerCtx->fgRFAGC = TRUE;   //external RF_AGC
	
    /* Specific part: Only for this tuner model */
    psTunerCtx->u2IF_Freq = SHARP_VA1E1BF2403_IF_FREQUENCY;
    psTunerCtx->u2IF_Freq_A=SHARP_VA1E1BF2403_IF_FREQUENCY_ANA;
  ...
  //  psTunerSpecificCtx->m_aucPara[TUNER_FUNC_EN_TARGET_LEVEL] = TRUE;
   ...
   // psTunerSpecificCtx->m_aucPara[TUNER_PARA_TARGET_LEVEL_LSB] = 0x00;  
    ...
    // init EQ script
    pVA1E1BF2403EqNormal[MOD_NTSC_M - MOD_ANA_TYPE_BEGIN]  = EQSHARPVA1E1BF2403MN;
    pVA1E1BF2403EqWeak[MOD_NTSC_M - MOD_ANA_TYPE_BEGIN]  = EQSHARPVA1E1BF2403MNWeak;
    //pVA1E1BF2403EqNormal[MOD_NTSC_M_54M_44M]  = EQSHARPVA1E1BF2403MN;
    //pVA1E1BF2403EqNormal[MOD_NTSC_M_54M_36M]  = EQSHARPVA1E1BF2403MN;
    psTunerSpecificCtx->u2LO_Step=SHARP_VA1E1BF2403_ANA_LO_STEP;
   
    mcSHOW_USER_MSG(("Init " SHARP_VA1E1BF2403_STR " External?%d\n",psTunerCtx->fgRFAGC));
}


//----------------------------------------------------------------------------- 
/** VA1E1BF2403_TunerSetFreq
 *  Set Tuner PLL forVA1E1BF2403/VA1E1BF2403 to tune RF frequency.
 *  @param  pTCtx       Pointer of pi tuner driver context.
 ...
 */
//-----------------------------------------------------------------------------
INT16 VA1E1BF2403_TunerSetFreq(
    ITUNER_CTX_T *pTCtx,
...
UINT32 freq;
INT32 i4GpioSts;  // Get the RFAGC GPIO Status 

    u1Mod=param->Modulation;
    mcSHOW_DBG_MSG((SHARP_VA1E1BF2403_STR " SetFreq %dKHz Mode=%d \n",param->Freq,u1Mod));

    if(u1Mod>= MOD_ANA_TYPE_BEGIN)
    {   // Analog Mode, GPIO status is 0
        i4GpioSts = GPIO_Output(u4GpioRfAgcNum, NULL);  
        mcSHOW_DBG_MSG3(("Analog Mode, RF_AGC Switch GPIO %d status is %d!\n", u4GpioRfAgcNum, i4GpioSts));	
        if(i4GpioSts)
        {  // if RF AGC GPIO is High, Set RF AGC GPIO to Low
            GPIO_SetOut(u4GpioRfAgcNum, 0);     
            mcSHOW_DBG_MSG3(("Set GPIO %d Low!\n", u4GpioRfAgcNum));
            mcDELAY_MS(50); 		// According to the Spec, the Delay min is 50msec	
        }
    } 
#if defined(CC_MT5365) || defined(CC_MT5395)
    else
    {   // Digital Mode, GPIO status is 1
        i4GpioSts = GPIO_Output(u4GpioRfAgcNum, NULL);    
        mcSHOW_DBG_MSG3(("Digital Mode, RF_AGC Switch GPIO %d status is %d!\n", u4GpioRfAgcNum, i4GpioSts));	
        if(!i4GpioSts)
        {  // if RF AGC GPIO is Low, Set RF AGC GPIO to High
            GPIO_SetOut(u4GpioRfAgcNum, 1);     
            mcSHOW_DBG_MSG3(("Set GPIO %d High!\n", u4GpioRfAgcNum));
            mcDELAY_MS(50); 		// According to the Spec, the Delay min is 50msec	
        }
    } 
#endif
    ...
    {
        Ndivider = (UINT16) ((Lo * 100) /SHARP_VA1E1BF2403_ANA_LO_STEP);
    }
    ...
    {
        if(!psTunerCtx->fgRFAGC)    //RF_AGC internal
        {
            CB1 = 0xA7;    //106dBuv
        }
        else
        {		
            CB1 = 0xBD;   //External RF_AGC
        }
    }
  //  else
  //  	{
  ...
  //  }
    ...
    if(fgAnalogSignal)
    {
        psTunerCtx->u4RF_Freq=(Ndivider*SHARP_VA1E1BF2403_ANA_LO_STEP)/100-psTunerCtx->u2IF_Freq-1750;
    }
    else
    {
...
//----------------------------------------------------------------------------- 
/**  VA1E1BF2403_TunerGetStatus
 *  Set Tuner PLL staus for VA1E1BF2403
 *  @param  pTCtx       Pointer of pi tuner driver context.
 *  @retval TRUE        PLL lock
 *  @retval FALSE       PLL unlock
 */
//-----------------------------------------------------------------------------
STATIC BOOL VA1E1BF2403_TunerGetStatus(ITUNER_CTX_T *pTCtx)
{
    //ITUNER_CTX_T *psTunerCtx = pTCtx;
 ...
 */
//-----------------------------------------------------------------------------
STATIC CHAR *VA1E1BF2403_TunerGetVer(VOID)
{
    return (SHARP_VA1E1BF2403_STR);
}
//------------------------------------------------------- 
/** VA1E1BF2403_TunerOP
 *  Set/Get function
 ...
 */
//-----------------------------------------------------------------------------
INT16 VA1E1BF2403_TunerOP(ITUNER_CTX_T * pTCtx, ITUNEROP_T eOperation, UINT8 SetVal,  VOID * pInOutVal)
{        
    switch(eOperation)
    {
    case itGetStatus:
        {
            ...
            *Retval=VA1E1BF2403_TunerGetStatus(pTCtx);
        }
        break;
    ...
    case itGetVer:
        {
            ...
            *RetStr=VA1E1BF2403_TunerGetVer();
        }
        break;
		
    case itGetEqScriptNormal: 
        *(UINT8**)pInOutVal = pVA1E1BF2403EqNormal[SetVal];
        break;

    case itGetEqScriptWeak: 
        *(UINT8**)pInOutVal = pVA1E1BF2403EqWeak[SetVal];
        break;
