...
/*#define RegWrite8(Addr, Value)    \ //EC_debug_ATD_dA315
{                                 \
...
//For Driver ATP Auto Test
   mcMSG_IN_MONITOR_AUTO_TEST(u2LOStep)
    ...
    else
    {
        ...
        {
            if( (psDemodCtx->u1VIFLock == 1) && (psDemodCtx->s4Cfo > (-1*s2PartiVIFLockRange)) && (psDemodCtx->s4Cfo < s2PartiVIFLockRange) )
            {
                mcSHOW_DBG_MSG(("[ATuner] CFO > %d kHz but < %d kHz!\n",s2FullVIFLockRange,s2PartiVIFLockRange));
                return TRUE;
            }
            else if( (psDemodCtx->u1VIFLock == 1) && (ucVpress == 0x01) && (psDemodCtx->s4Cfo > (-3*s2PartiVIFLockRange)) && (psDemodCtx->s4Cfo < 3*s2PartiVIFLockRange) )
            {
            ...
            else if((psDemodCtx->s4Cfo > (-1*s2FullVIFLockRange)) && (psDemodCtx->s4Cfo < s2FullVIFLockRange) && (ucVpress == 0x01))
            {
...
static UINT8 ATDNA_GetCRLock(ATD_CTX_T *psDemodCtx, BOOL fgLogMessage)
{
    ...
    else if((fgACI)&&((ucData2&0x20) == 0x20)) //for audio carrier > video carrier case
    {
        psDemodCtx->fgCRLock = TRUE;
        mcSHOW_DBG_MSG2(("VIF lock modifued by ACI case!\n"));
    }
    else if(_fgAutoSearch)
    {
        ...
        if(ucPOAGain <= 0x02)
        {
            ...
            ATD_GetReg(psDemodCtx, REG_ATD_CR_STATE0, &ucData, 1);
            ATD_GetReg(psDemodCtx, REG_ATD_CR_STATE1, &ucData2, 1);
            if(((ucData & 0x0C) != 0x00) || ((ucData2 & 0x30) != 0x00))
            {
			    psDemodCtx->fgCRLock = TRUE;
			}
            else
...
//	mcSHOW_DBG_MSG(("u2LOStep=%d,u2LOStep=%u,s2FullVIFLockRange=%d,s2PartiVIFLockRange=%d\n",u2LOStep,u2LOStep,s2FullVIFLockRange,s2PartiVIFLockRange));
    ...
    {
    	if( (psDemodCtx->s4Cfo > (-1*s2FullVIFLockRange)) && 
