			       ...
			       bool use_test_mode, u32 np_val)
{
	...
	//printk("epdc_submit_update start\n");
				...
				struct mxc_epdc_fb_data *fb_data)
{
	...
	DBG_MSG("%s():upd rect merge,a(%d,%d,%d,%d)+b(%d,%d,%d,%d)=>c(%d,%d,%d,%d)\n",
			__FUNCTION__,arect->left,arect->top,arect->width,arect->height,\
	...
	*arect = combine;
	...
	if (use_flags)
	{
		...
		a->flags = b->flags;
		if( a->waveform_mode==fb_data->wv_modes.mode_aad &&
...
static void epdc_submit_work_func(struct work_struct *work)
{
	...
	int direct_draw = 0;
//printk("=> epdc_submit_workqueue start\n");
	...
	list_for_each_entry_safe(next_update, temp_update,
		...
		next_update->update_desc->upd_data.update_mode =
			UPDATE_MODE_PARTIAL;
	 ...
	 */
	DBG_MSG("%s(%d):is_aa=%d,wb_pause=%d,flags=0x%x\n",__FUNCTION__,__LINE__,
	...
	/* Release buffer queues */
	mutex_unlock(&fb_data->queue_mutex);
//printk("=> epdc_submit_workqueue end\n");
				   ...
				   struct fb_info *info)
{
	...
	DBG_MSG("%s():AlgorithmVerion=%d,is_aa=%d\n",__FUNCTION__,algorithmVersion,upd_desc->is_aa);
	...
	if (fb_data->upd_scheme != UPDATE_SCHEME_SNAPSHOT) {
		...
		mutex_unlock(&fb_data->queue_mutex);
		//printk("=> queue_work epdc_submit_workqueue\n");
				   ...
				   struct fb_info *info)
{
	...
	//printk( "[%s -- %d] update_mode[%x], waveform_mode[%x], ( %d, %d, %d, %d ) \n" ,__func__,__LINE__, upd_data->update_mode ,upd_data->waveform_mode, upd_data->update_region.left, upd_data->update_region.top, upd_data->update_region.width, upd_data->update_region.height);		
	...
	case MXCFB_SEND_UPDATE:GALLEN_DBGLOCAL_RUNLOG(4);
       ...
       case MXCFB_SET_GAMMA:
               printk("\n==> MXCFB_SET_GAMMA\n");
               {
                       ...
                       if (!copy_from_user(&gamma_lut, argp,
                               sizeof(gamma_lut))) {
				   ...
				   struct fb_info *info)
{
	...
	//Yian: patch from freescale HK. Peter, fixed the broken screen on EPD
	//mutex_lock(&fb_data->queue_mutex);
	mutex_lock(&fb_data->pxp_mutex);
...
#ifdef FW_IN_RAM//[ gallen modify 20110609 : waveform pass from bootloader in RAM .
	#if 1
		queue_work(fb_data->epdc_firmware_workqueue,
...
static DEVICE_ATTR(update, 0644, NULL, store_update);

static struct attribute* epdc_attr[] = {
...
int __devinit mxc_epdc_fb_probe(struct platform_device *pdev)
{
	...
	if (sysfs_create_group(&pdev->dev.kobj, &attr_group))
		dev_err(&pdev->dev, "Unable to create file from attr_group\n");
...
static int mxc_epdc_fb_resume(struct platform_device *pdev)
{
	...
	GALLEN_DBGLOCAL_BEGIN();
//printk("=> mxc_epdc_fb_resume start\n");
#if 0
	if(NTXHWCFG_TST_FLAG(gptHWCFG->m_val.bPCB_Flags,4)) {
		// PCB is designed for low voltage .
