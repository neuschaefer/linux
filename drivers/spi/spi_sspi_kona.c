...
static int spi_kona_setup_dma(struct spi_kona_data *spi_kona)
{
	spi_kona->tx_dma_chan = sdma_request_channel(DMA_DEVICE_SSP0B_TX0);
	if (spi_kona->tx_dma_chan < 0) {
		pr_debug("%s: Tx sdma_request_chan failed\n", __func__);
		return -EIO;
	}
	if (sdma_set_device_handler(spi_kona->tx_dma_chan, spi_dma_callback,
				    &spi_kona->tx_dma_evt) < 0) {
		pr_debug("%s: Tx sdma_register_callback failed\n", __func__);
		goto err;
	}
	spi_kona->rx_dma_chan = sdma_request_channel(DMA_DEVICE_SSP0A_RX0);
	if (spi_kona->rx_dma_chan < 0) {
		pr_debug("%s: Tx sdma_request_chan failed\n", __func__);
		goto err;
	}
	if (sdma_set_device_handler(spi_kona->rx_dma_chan, spi_dma_callback,
				    &spi_kona->rx_dma_evt) < 0) {
		pr_debug("%s: Tx sdma_register_callback failed\n", __func__);
		goto err1;
	}
