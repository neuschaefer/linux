...
static int fsl_qspi_remove(struct platform_device *pdev)
{
	...
	/* switch pinctrl if optional gpio pinctrl available */
	pinctrl = devm_pinctrl_get(dev);
	if (!IS_ERR(pinctrl)) {
		pstate = pinctrl_lookup_state(pinctrl, "gpio");
		if (IS_ERR(pstate)) {
			dev_dbg(dev, "no optional gpio pinctrl available\n");
		} else {
			if (pinctrl_select_state(pinctrl, pstate))
				dev_err(dev, "unable to set pinctrl state\n");
		}
	}
