...
struct tegra_i2c_dev {
	...
	const struct tegra_i2c_clk_table *clk_table;
	int table_size;
	unsigned long default_bus_clk;
	...
	int num)
{
	...
	/* Slave addrs declared in the table are set to designated freq */
	if (i2c_dev->clk_table) {
		for (i = 0; i < i2c_dev->table_size; i++) {
			i2c_bus->bus_clk_rate = 
				(i2c_dev->clk_table[i].addr == msgs->addr) ? 
				i2c_dev->clk_table[i].clk_rate : i2c_dev->default_bus_clk;
		}
	}
...
static int tegra_i2c_probe(struct platform_device *pdev)
{
	...
	i2c_dev->default_bus_clk = plat->bus_clk_rate[0] ?: 100000;
	i2c_dev->msgs = NULL;
	i2c_dev->msgs_num = 0;
	...
	if (plat->clk_table){
		i2c_dev->clk_table = plat->clk_table;
		i2c_dev->table_size = plat->table_size;
	}
