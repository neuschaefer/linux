...
#define PROC_ENTRY_REG_WRITE      "regWrite"
#define PROC_ENTRY_REG_READ       "regRead"
...
static void client_speed_set(struct i2c_adapter *adapter, unsigned short addr)
{
	 ...
	 * problem we need to enable autosense with the timeout disabled
	 */
	if (pd && TIMEOUT_IS_VALID(pd))
		bsc_set_autosense((uint32_t)dev->virt_base, 1,
				  pd->autosense_timeout_enable);
	else {
...
static int
proc_reg_write_write(struct file *file, const char __user *buffer,
		unsigned long count, void *data)
{
	struct bsc_i2c_dev *dev = (struct bsc_i2c_dev *)data;
	...
	if (sscanf(kbuf, "%x %x %x", &addr, &reg, &val) != 3) {
		printk(KERN_ERR "echo <addr> <reg> <val> > %s\n", PROC_ENTRY_REG_WRITE);
		return count;
	}
	...
	rc = bsc_xfer(adapter, &msg, 1);
	if (rc < 0) {
		printk(KERN_ERR "bsc_xfer failed\n");
		return count;
	}
...
static int
proc_reg_read_write(struct file *file, const char __user *buffer,
		unsigned long count, void *data)
{
	struct bsc_i2c_dev *dev = (struct bsc_i2c_dev *)data;
	struct i2c_adapter *adapter = &dev->adapter;
	...
	if (sscanf(kbuf, "%x %x", &addr, &reg) != 2) {
		printk(KERN_ERR "echo <addr> <reg> > %s\n", PROC_ENTRY_REG_READ);
		return count;
	}
	...
	rc = bsc_xfer(adapter, msg, 2);
	if (rc < 0) {
		printk(KERN_ERR "bsc_xfer failed\n");
		return count;
	}

	printk(KERN_INFO "Read I2C reg=0x%02x val=0x%02x\n", reg, (unsigned int)rbuf);
...
static int proc_init(struct platform_device *pdev)
{
	...
	struct proc_dir_entry *proc_reg_write, *proc_reg_read;
   ...
   if (proc->parent == NULL)
	...
	proc_reg_write = create_proc_entry(PROC_ENTRY_REG_WRITE, 0644, proc->parent);
	if (proc_reg_write == NULL) {
		rc = -ENOMEM;
		goto err_del_rx_fifo;
	}
	proc_reg_write->write_proc = proc_reg_write_write;
	proc_reg_write->data = dev;

	proc_reg_read = create_proc_entry(PROC_ENTRY_REG_READ, 0644, proc->parent);
	if (proc_reg_read == NULL) {
		rc = -ENOMEM;
		goto err_del_reg_write;
	}
	proc_reg_read->write_proc = proc_reg_read_write;
	proc_reg_read->data = dev;

	return 0;

err_del_reg_write:
	remove_proc_entry(PROC_ENTRY_REG_WRITE, proc->parent);
...
static int proc_term(struct platform_device *pdev)
{
   ...
   remove_proc_entry(PROC_ENTRY_REG_READ, proc->parent);
   remove_proc_entry(PROC_ENTRY_REG_WRITE, proc->parent);
   remove_proc_entry(PROC_ENTRY_RX_FIFO, proc->parent);
   remove_proc_entry(PROC_ENTRY_TX_FIFO, proc->parent);
   remove_proc_entry(PROC_ENTRY_RESET, proc->parent);
