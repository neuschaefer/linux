	...
	struct uvc_streaming_control *ctrl)
{
	...
	for (i = 0; i < video->streaming->nformats; ++i) {
		if (video->streaming->format[i].index == ctrl->bFormatIndex) {
...
static void uvc_free_urb_buffers(struct uvc_video_device *video)
{
	...
	for (i = 0; i < UVC_URBS; ++i) {
		if (video->urb_buffer[i]) {
			usb_free_coherent(video->dev->udev, video->urb_size,
...
static int uvc_init_video(struct uvc_video_device *video, gfp_t gfp_flags)
{
	...
	if (intf->num_altsetting > 1) {
		...
		if (i >= intf->num_altsetting) {
			uvc_printk(KERN_ERR, "uvc_init_video failed with EIO, i %d >= num_alt %d, bandwidth = %u\n", i, intf->num_altsetting, bandwidth);
			return -EIO;
		}
