...
long gencmd_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	int ret = -1;
        ...
        case GENCMD_IOCTL_CMD_SEND_RESP:
            {
                ...
                /* call vc_gencmd */
                g_buf->resp_len = vc_gencmd( g_buf->resp, GENCMD_RESP_SIZE, "%s", g_buf->cmd);
