						...
						struct lsm330dlc_triple *data)
{
	...
	if (mod_debug) {
		printk("LSM330DLC GYR return(%d,%d,%d)\n", data->x, data->y, data->z);
	}
...
static int lsm330dlc_device_power_on(struct lsm330dlc_data *dev_data)
{
	...
	if (dev_data->hw_initialized) {
		...
		pr_debug("dev_data->pdata->gpio_int2 = %d\n",
...
#if 0
static void lsm330dlc_gyr_set_power_mode(struct i2c_client* client, u8 val)
{
 struct lsm330dlc_data *gyro =  i2c_get_clientdata(client);
    if(val == 0) /* power off */
        lsm330dlc_device_power_off(gyro);
    else
        lsm330dlc_device_power_on(gyro);

}

static int lsm330dlc_gyr_read_data(struct i2c_client* client, struct lsm330dlc_axis *axis)
{
	int err;
	struct lsm330dlc_triple data_out;
	...
	axis->z = data_out.z;
		...
		pr_info("%s read x=%d, y=%d, z=%d\n",
				LSM330DLC_GYR_DEV_NAME, data_out.x, data_out.y, data_out.z);
#endif
	return 0;
}
#endif
					...
					const struct i2c_device_id *devid)
{
	...
	/* create sysfs instead of device fs */
    ...
    brvsens_register(SENSOR_HANDLE_GYROSCOPE,
                LSM330DLC_GYR_DEV_NAME,
                (void*)client,
                (PFNACTIVATE)lsm330dlc_gyr_set_power_mode,
                (PFNREAD)lsm330dlc_gyr_read_data);
