...
/* The time in ms allowed to wait for videocore resume requested */
#define VC_RESUME_TIMEOUT_MS 2000
...
static inline USER_SERVICE_T *
find_service_by_handle(VCHIQ_INSTANCE_T instance, int handle)
{
	USER_SERVICE_T *user_service;

	if ((handle >= 0) && (handle < MAX_SERVICES)) {
...
static inline USER_SERVICE_T *
find_avail_service_handle(VCHIQ_INSTANCE_T instance)
{
	int handle;
	...
	VCHIQ_SERVICE_HANDLE_T handle, void *bulk_userdata)
{
	...
	*/
	...
	VCHIQ_INSTANCE_T instance = user_service->instance;
	DEBUG_INITIALISE(g_state.local)
	...
	if (header && user_service->is_vchi) {
		...
		/* A write memory barrier is needed to ensure that the store of
		** header is completed before the insertion point is updated
		*/
		wmb();
...
vchiq_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case VCHIQ_IOC_CREATE_SERVICE: {
		...
		if (mutex_lock_interruptible(&instance->state->mutex) == 0) {
	...
	case VCHIQ_IOC_CLOSE_SERVICE: {
		...
		if (user_service != NULL) {
			VCHIQ_SERVICE_T *service = user_service->service;
			int is_server = (service->public_fourcc !=
				VCHIQ_FOURCC_INVALID);

			status = vchiq_close_service(&service->base);
	...
	case VCHIQ_IOC_QUEUE_BULK_RECEIVE: {
		...
		if (args.mode == VCHIQ_BULK_MODE_BLOCKING) {
			waiter = kmalloc(sizeof(struct bulk_waiter_node),
				GFP_KERNEL);
			if (!waiter) {
				ret = -ENOMEM;
				break;
	...
	case VCHIQ_IOC_AWAIT_COMPLETION: {
		...
		while ((instance->completion_remove ==
			...
			&& !instance->closing) {
			DEBUG_TRACE(AWAIT_COMPLETION_LINE);
			if (down_interruptible(&instance->insert_event) != 0) {
				DEBUG_TRACE(AWAIT_COMPLETION_LINE);
				vchiq_log_info(vchiq_arm_log_level,
		...
		*/
		...
		if (ret == 0) {
			...
			for (ret = 0; ret < args.count; ret++) {
				...
				user_service = (USER_SERVICE_T *)completion->
					service_userdata;
				...
				if (header) {
					...
					** can be released. */
					vchiq_release_message(&user_service->
						service->base, header);
	...
	case VCHIQ_MINOR: {
		...
		for (i = 0; i < MAX_SERVICES; i++) {
			USER_SERVICE_T *user_service =
				 &instance->services[i];
			if (user_service->service != NULL) {
				/* Wake the slot handler if the msg queue is
				** full. */
				up(&user_service->remove_event);

				if ((user_service->service->srvstate !=
					VCHIQ_SRVSTATE_CLOSEWAIT) &&
					(user_service->service->srvstate !=
					VCHIQ_SRVSTATE_LISTENING))
		...
		for (i = 0; i < MAX_SERVICES; i++) {
			USER_SERVICE_T *user_service =
				 &instance->services[i];
			if (user_service->service != NULL) {
				/* Wait in this non-portable fashion because
				** interruptible calls will not block in this
				** context. */
				while ((user_service->service->srvstate !=
					VCHIQ_SRVSTATE_CLOSEWAIT) &&
					(user_service->service->srvstate !=
					VCHIQ_SRVSTATE_LISTENING))
					down(&user_service->service->
						remove_event);
...

		{
			...
			list_for_each_safe(pos, next,
				...
				kfree(waiter);
				vchiq_log_warning(vchiq_arm_log_level,
					"bulk_waiter - cleaned up %x for pid %d",
		...
		enum USE_TYPE_E use_type)
{
	...
	if (use_type != USE_TYPE_SERVICE) {
	...
	} else if (service && service->magic == VCHIQ_MAGIC) {
		...
		if (service)
			vchiq_log_error(vchiq_susp_log_level, "%s invalid "
				"service (0x%p Magic: %c%c%c%c, use_type %d)",
				__func__, service, VCHIQ_FOURCC_AS_4CHARS(
					service->magic), use_type);
		else
	...
	while (arm_state->resume_blocked) {
		...
		if (!try_wait_for_completion(&arm_state->resume_blocker)) {
			...
			write_unlock(&arm_state->susp_res_lock);
			vchiq_log_warning(vchiq_susp_log_level, "%s %s resume "
				"blocked - state %d:%d waiting...", __func__, entity, arm_state->vc_resume_state, arm_state->vc_suspend_state);
	...
	if (!try_wait_for_completion(&arm_state->vc_resume_complete)) {
		vchiq_log_warning(vchiq_susp_log_level, "%s %s wait for vc resume state %d:%d",
			__func__, entity, arm_state->vc_resume_state, arm_state->vc_suspend_state);
		long rc = 0;
		rc = wait_for_completion_killable_timeout(&arm_state->vc_resume_complete, msecs_to_jiffies(VC_RESUME_TIMEOUT_MS));
		if (rc == 0 && arm_state->vc_resume_state == VC_RESUME_RESUMED && arm_state->vc_suspend_state == VC_SUSPEND_IDLE) {
			// timeout, workaround the lockup
			complete_all(&arm_state->vc_resume_complete);
			vchiq_log_warning(vchiq_susp_log_level, "%s %s lockup detected, state %d:%d",
			__func__, entity, arm_state->vc_resume_state, arm_state->vc_suspend_state);
		} else if (rc < 0) {
...
vchiq_release_internal(VCHIQ_STATE_T *state, VCHIQ_SERVICE_T *service)
{
	...
	if (service && service->magic != VCHIQ_MAGIC) {
		vchiq_log_error(vchiq_susp_log_level,
		       "%s: Invalid service magic number", __func__);
		ret = VCHIQ_ERROR;
		goto out;
	}
