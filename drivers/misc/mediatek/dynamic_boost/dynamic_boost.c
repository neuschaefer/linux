...
static int dboost_dvfs_hotplug_thread(void *ptr)
{
	...
	while (!kthread_should_stop()) {
		...
		/* printk("dynamic boost: set freq min/max=(%d,%d)\n", max_freq, MAX_FREQUENCY); */
		if (!max_freq) {
			policy->max = MAX_FREQUENCY;
			policy->min = MIN_FREQUENCY;
		} else {
			cpufreq_verify_within_limits(policy, max_freq, MAX_FREQUENCY);
		}
		/* printk("dynamic boost: policy min/max=(%d,%d)\n", policy->min, policy->max); */
...
static int flag = -1; /* flag to filter botton event */
/******************************************************************************
 *                         Handle touch boost                                 *
 ******************************************************************************/
...
static int dynamic_boost_lcm_fb_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
	...
	case FB_BLANK_UNBLANK:
		set_dynamic_boost(200, PRIO_TWO_LITTLES);
		break;
	/* LCM OFF */
