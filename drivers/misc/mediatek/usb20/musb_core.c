...
#if (defined(CONFIG_MFD_MT6397) && defined(CONFIG_POWER_EXT))
u32 usb_chrdet_number;
#endif
bool mtk_usb_power;
...
{
	DBG(0, "[USB][ops_usbdet_int_handler]....\n");
...
#ifndef CONFIG_USB_PHYCHK_EXTCONN
#ifdef CONFIG_OF
...
static int musb_probe(struct platform_device *pdev)
{
	...
	#if (defined(CONFIG_MFD_MT6397) && defined(CONFIG_POWER_EXT))
	usb_chrdet_number = irq_of_parse_and_map(pdev->dev.of_node, 1);
	printk("musb_probe: usb_irq = %d, usb_chrdet = %d\n", usb_irq_number, usb_chrdet_number);
	#endif
	...
	ret = request_threaded_irq(usb_chrdet_number, NULL,
					   ...
					   IRQF_TRIGGER_HIGH | IRQF_ONESHOT, "pmic_usb_chrdet", dev);
	if (ret) {
