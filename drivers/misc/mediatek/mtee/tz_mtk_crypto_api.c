...
#ifdef CRYPTO_API_TEST
	{
		extern int test_secure_api_thread(void *param);	
		kthread_run(test_secure_api_thread, NULL,
								"secure api test");
	}
#endif
	return 0;
...
)
{
	...
	pctx->mutex = (u64)(unsigned long)kmalloc(sizeof(struct mutex), GFP_KERNEL);
	mutex_init((struct mutex *)(unsigned long)pctx->mutex);
...
)
{
	...
	kfree((void*)(unsigned long)pctx->mutex);
...
int mtk_crypto_aes_with_keymode(
struct mtk_crypto_ctx *pctx,	/* [in]  context of mtk crypto driver */
u8		  key_mode, 		 /* [in]  0 -> plain key, 1 -> key encrypted by hw key idx 0, 2 -> key encrypted by hw key idx 1*/
u8        blk_mode,		/* [in]  0 -> ecb, 1 -> cbc, 2 -> ctr, 3 -> GCM */
u8        op_mode,		/* [in]  0 -> enc, 1 -> dec */
const u8 *piv,			/* [in]  initial vector or initial counter value buffer, 16 bytes */
				/*	== NULL for ecb mode */
...
)
{
	...
	op_mode = ((key_mode << 4)|op_mode);
	return mtk_crypto_aes_helper(pctx, blk_mode, op_mode, piv,
				     pkey, key_len, psrc, pdes, dat_len);
}
EXPORT_SYMBOL(mtk_crypto_aes_with_keymode);
...
)
{
	...
	/* check key_idx, only support 0 */
	if (key_idx > 0xF) {
		pr_warn("key_idx must less than 16!\n");
		return -1;
	}
	return mtk_crypto_aes_helper(pctx, blk_mode, op_mode, piv,
...
)
{
	...
	if (!mutex_trylock((struct mutex *)(unsigned long)pctx->mutex))
...
)
{
	...
	mutex_unlock((struct mutex *)(unsigned long)pctx->mutex);
	return 0;
}
