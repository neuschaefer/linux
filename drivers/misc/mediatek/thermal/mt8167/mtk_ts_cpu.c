...
#else
	tscpu_printk("set_adaptive_gpu_power_limit  no gpu power limit\n");
#endif
}
...
static void set_static_gpu_power_limit(unsigned int limit)
{
	...
	mt_gpufreq_thermal_protect((final_limit != 0x7FFFFFFF) ? final_limit : 0);
#else
	tscpu_printk("set_static_gpu_power_limit  no gpu power limit\n");
#endif
}
	...
	for (i = 0; i < Num_of_OPP; i++) {
		if (cl_dev_state[i] == 1) {
			if (i != previous_step) {
				...
				if (!mtk_gpu_power) {
					tscpu_dprintk("%s GPU POWER NOT READY!!", __func__);
					/* GPU freq = 299000, power = 548 */
					power = i * 50 + 50;
					tscpu_dprintk("%s cpu_power=%d\n", __func__, power);
...
void tscpu_start_thermal_timer(void)
{
	...
	if (thz_dev != NULL && interval != 0) {
		/* 500ms */
		mod_delayed_work(system_freezable_wq, &(thz_dev->poll_queue), round_jiffies(msecs_to_jiffies(500)));
