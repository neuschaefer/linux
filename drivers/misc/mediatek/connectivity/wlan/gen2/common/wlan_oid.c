...
*******************************************************************************
*/
#if CFG_SUPPORT_FRAMES_TO_CUSTOMER
	/*Bitmap format. Indicate which types frames need to send to customer  layer*/
	UINT_32 g_toCustomerFrameTypes = 0;
#endif
...
#endif
#if CFG_SUPPORT_FRAMES_TO_CUSTOMER
	ENUM_FRAME_TYPES_TO_CUSTOMER_T frameType = FRAME_TYPES_NUMS;
	UINT_32 frameToCustomerEnable = 0;
	UINT_32 frameForceRxByHW = 0;
#endif
...
#endif
#if CFG_SUPPORT_FRAMES_TO_CUSTOMER
		if (prMcrWrInfo->u4McrOffset == 0x11111113) {
				rCmdAccessReg.u4Address = prMcrWrInfo->u4McrOffset;
				/*
				  *McrData is 32bits value from user view.
				  *bits[31:4] means frame type and this value can be choosed from ENUM_FRAME_TYPES_TO_CUSTOMER_T.
				  *bit[3:0] value 2'b0000 means not send to customer and not force HW to rx.
				  *bit[3:0] value 2'b0001 means send to customer and force HW to rx.
				  *bit[3:0] value 2'b0010 means send to customer but not force HW to rx.
				  */
				switch (prMcrWrInfo->u4McrData & FRAME_TYPES_TO_CUSTOMER_ENABLE_MASK)
				{
					case 0:
						frameToCustomerEnable = 0;
						frameForceRxByHW = 0;
						break;
					case 1:
						frameToCustomerEnable = 1;
						frameForceRxByHW = 1;
						break;
					case 2:
						frameToCustomerEnable = 1;
						...
						break;
					default:
						DBGLOG(INIT, WARN, "Not support this action\n");
						return WLAN_STATUS_NOT_SUPPORTED;						
				}
				/*
				  *AccessReg.Data is 32bits value from FW view.
				  *bits[31:4] means frame type and this value can be choosed from ENUM_FRAME_TYPES_TO_CUSTOMER_T.
				  *bit[3:0] value 2'b0000 means not force HW to rx.
				  *bit[3:0] value 2'b0001 means force HW to rx.
				  */				
				switch (prMcrWrInfo->u4McrData >> FRAME_TYPES_TO_CUSTOMER_OFFSET)
				{
					case FRAME_TYPES_BEACON:
						rCmdAccessReg.u4Address = 0x11111130;
						rCmdAccessReg.u4Data = ((MAC_FRAME_BEACON << FRAME_TYPES_TO_CUSTOMER_OFFSET) | frameForceRxByHW);
						break;
	
					case FRAME_TYPES_PROBE_REQ:
						rCmdAccessReg.u4Address = 0x11111113;
						rCmdAccessReg.u4Data = frameForceRxByHW;
						break;

					case FRAME_TYPES_PROBE_RSP:
						rCmdAccessReg.u4Address = 0x11111130;
						rCmdAccessReg.u4Data = ((MAC_FRAME_PROBE_REQ << FRAME_TYPES_TO_CUSTOMER_OFFSET) | frameForceRxByHW);
						break;

	
					default:
						DBGLOG(INIT, WARN, "Not support this type frame\n");
						return WLAN_STATUS_NOT_SUPPORTED;
				}
	
				if (frameToCustomerEnable)
					g_toCustomerFrameTypes |= BIT((prMcrWrInfo->u4McrData >> FRAME_TYPES_TO_CUSTOMER_OFFSET));
				else
					g_toCustomerFrameTypes &= ~BIT((prMcrWrInfo->u4McrData >> FRAME_TYPES_TO_CUSTOMER_OFFSET));
...
WLAN_STATUS wlanoidQueryDbgCntr(IN P_ADAPTER_T prAdapter,
		 OUT PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	CMD_SW_DBG_CTRL_T rCmdSwCtrl;
	WLAN_STATUS rStatus;
	...
	*pu4QueryInfoLen = sizeof(CMD_SW_DBG_CTRL_T);
	...
	rStatus = wlanSendSetQueryCmd(prAdapter,	/* prAdapter */
				      ...
				      TRUE,	/* fgNeedResp */
				      TRUE,	/* fgIsOid */
				      nicCmdEventDebugQuery,	/* pfCmdDoneHandler */
				      nicOidCmdTimeoutCommon,	/* pfCmdTimeoutHandler */
