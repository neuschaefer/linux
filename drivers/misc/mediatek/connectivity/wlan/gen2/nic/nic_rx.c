...
VOID nicRxProcessMonitorPacket(IN P_ADAPTER_T prAdapter, IN OUT P_SW_RFB_T prSwRfb)
{
	...
	P_WLAN_MAC_HEADER_T mac_header;
	UINT_8 valid_pkt_flag = 1;
	...
	mac_header = prSwRfb->pvHeader;
	/* if pkt is 802.11 data pkt */
	if (((mac_header->u2FrameCtrl & MASK_FC_TYPE) >> 2) != 0x2)
		valid_pkt_flag = 0;
	...
	u4RadiotapLen = 8; /* only include basic radiotap header */
	rMonitorRadiotap.u2ItLen = cpu_to_le16(u4RadiotapLen);
	rMonitorRadiotap.u4ItPresent = 0;
	...
	skb_trim(prSkb, 0);
	skb_put(prSkb, prSwRfb->u2PacketLen + u4RadiotapLen - 9);
...
#if CFG_ENABLE_BT_OVER_WIFI
		{
			UINT_8 aucTmpDisconnected[sizeof(AMPC_EVENT) + sizeof(BOW_LINK_DISCONNECTED)];
			UINT_8 aucTmpConnected[sizeof(AMPC_EVENT) + sizeof(BOW_LINK_CONNECTED)];
			...
			if (prEventBtOverWifi->ucLinkStatus == 0) {
				...
				prBowEvent = (P_AMPC_EVENT) aucTmpConnected;
				...
				/* construct event header */
				prBowEvent = (P_AMPC_EVENT) aucTmpDisconnected;
