...
P_SW_RFB_T qmHandleRxPackets(IN P_ADAPTER_T prAdapter, IN P_SW_RFB_T prSwRfbListHead)
{
	...
	do {
		...
		if (qmIsDuplicatedIcmpReply(prCurrSwRfb)) {
...
static UINT_16 su2LastIcmpIpId;
static UINT_32 su2LastIcmpSsn;
static UINT_16 su2LastIcmpId;
BOOLEAN qmIsDuplicatedIcmpReply(IN P_SW_RFB_T prSwRfb)
{
	PUINT_8 pucPkt = prSwRfb->pvHeader;
	PUINT_8 pucEthBody;
	UINT_16 u2IpId = 0;
	...
	if (pucPkt == NULL || kalGetPktEtherType(pucPkt) != ENUM_PKT_ICMP)
	return FALSE;

	pucEthBody = &pucPkt[ETH_HLEN];
	u2IpId = (pucEthBody[4] << 8) | pucEthBody[5];
	u4Ssn =  (UINT_32) HIF_RX_HDR_GET_SN(prSwRfb->prHifRxHdr);
	u2IcmpId = (pucEthBody[24] << 8) | pucEthBody[25];

	if (u2IpId == su2LastIcmpIpId && u4Ssn == su2LastIcmpSsn && u2IcmpId == su2LastIcmpId) {
		DBGLOG(QM, INFO, "Duplicated ICMP reply(IPID: %d, SSN: %d, IcmpId:%d), ignore it.\n",
			u2IpId, u4Ssn, u2IcmpId);
		return TRUE;
	}

	su2LastIcmpIpId = u2IpId;
	su2LastIcmpSsn = u4Ssn;
	su2LastIcmpId = u2IcmpId;
	return FALSE;
}
