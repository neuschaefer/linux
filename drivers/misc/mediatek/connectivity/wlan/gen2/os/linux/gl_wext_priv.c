...
static int
getLastRxRate(IN struct net_device *prNetDev, OUT UINT32 *pLastRxRate)
{
	PUINT_32 pu4IntBuf;
	UINT_32 u4BufLen = 0;
	int status = 0;
...
static int getAndPrintStats (IN struct net_device *prNetDev,
	IN struct iw_request_info *prIwReqInfo,
	IN union iwreq_data *prIwReqData,
	IN OUT char *pcExtra)
{
	UINT_8 aucBuffer[512] = {0};
	UINT_8 aucBuffer2[512] = {0};
	UINT_8 *p_buffer;
	UINT32 u4BufLen;
	...
	if (kalIoctl(prGlueInfo,
		wlanoidQueryDbgCntr,
		(PVOID)aucBuffer, sizeof(UINT_8) *512, TRUE,TRUE, TRUE, FALSE, &u4BufLen)
		== WLAN_STATUS_SUCCESS) {
		...
		if (pSwDbgCtrl) {
			if (pSwDbgCtrl->u4Data == SWCR_DBG_TYPE_ALL) {
				...
				n = sprintf(&p_buffer[pos], "Tx fail to Rcv ACK after retry = %d\n",
				...
				n = sprintf(&p_buffer[pos], "Rx drop due to out of resource = %d\n",
	...
	if (wlanQueryInformation(prGlueInfo->prAdapter, wlanoidQueryBssid,
		...
		if (wlanQueryInformation(prGlueInfo->prAdapter, wlanoidQuerySsid,
			&ssid, sizeof(ssid), &u4BufLen) == WLAN_STATUS_SUCCESS) {
			n = sprintf(&p_buffer[pos], "\n[STA] connected SSID = %s\n", ssid.aucSsid);
			pos += n;
		}

		n = sprintf(&p_buffer[pos], "[STA] connected AP MAC Address = ");
		pos += n;
		...
		n = sprintf(&p_buffer[pos], "channel = %d\n", prBssInfo->ucPrimaryChannel);
		pos += n;
...
/*----------------------------------------------------------------------------*/
/*!
* \brief Private ioctl get string handler.
*
* \param[in] pDev Net device requested.
* \param[out] pIwReq Pointer to iwreq structure.
* \param[in] prIwReqData The ioctl req structure, use the field of sub-command.
* \param[out] pcExtra The buffer with put the return string
*
* \retval 0 For success.
* \retval -EOPNOTSUPP If cmd is not supported.
* \retval -EFAULT For fail.
	...
	)
{
	...
	u4SubCmdFlags = (UINT_32) prIwReqData->data.flags;
	...
	if (copy_from_user(InBuf, prIwReqData->data.pointer,
				prIwReqData->data.length))
		return -EFAULT;

	DBGLOG(INIT, INFO, "priv_get_string (%s)(%d)\n",
			InBuf, (INT_32) prIwReqData->data.length);

	switch (u4SubCmdFlags) {
	case PRIV_CMD_STAT:
		Status =	getAndPrintStats(prNetDev, prIwReqInfo, prIwReqData, pcExtra);
		break;
	default:
		break;
	}
