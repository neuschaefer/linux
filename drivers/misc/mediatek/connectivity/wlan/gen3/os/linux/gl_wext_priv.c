	...
	case PRIV_CMD_GET_CH_LIST:
		{
			...
			if (copy_to_user(prIwReqData->data.pointer, ch, NumOfChannel * sizeof(INT_32)))
				...
				COMPAT_TOUSER(prIwReqInfo, prIwReqData);
				return status;
			}
		}
	default:
...
#define CMD_SETWOWENABLE		"SETWOWENABLE"
...
#if CFG_SUPPORT_WOWLAN
int priv_driver_set_wow_enable(IN struct net_device *prNetDev, IN char *pcCommand, IN int i4TotalLen)
{
	P_GLUE_INFO_T prGlueInfo = NULL;
	INT_32 i4Argc = 0;
	PCHAR apcArgv[WLAN_CFG_ARGV_MAX];
	...
	if (i4Argc >= 2) {
		...
		DBGLOG(REQ, INFO, "%s: Set wow enable mode [%u]\n", __func__, fgEnable);

		if (prGlueInfo->fgIsWowEnable == fgEnable) {
			DBGLOG(REQ, INFO, "%s: Already enabled, SKIP!\n", __func__);
			return 0;
		}

		prGlueInfo->fgIsWowEnable = fgEnable;
		kalWlanNotifyFwWowEnable(prGlueInfo, fgEnable);
...
int priv_driver_set_nvram(IN struct net_device *prNetDev, IN char *pcCommand, IN int i4TotalLen)
{
	...
	if (i4Argc >= 3) {
		u4Ret = kalkStrtou32(apcArgv[1], 0, &u4Parse);
		offset = (UINT_32)u4Parse;

		u4Ret = kalkStrtou32(apcArgv[2], 0, &u4Parse);
		u2Data = (UINT_16)u4Parse;

		DBGLOG(REQ, INFO, "priv_driver_set_nvram offset=0x%x, data=0x%04x\n", offset, u2Data);
	}
	else{
...
int priv_driver_get_nvram(IN struct net_device *prNetDev, IN char *pcCommand, IN int i4TotalLen)
{
	...
	if (i4Argc >= 2) {
		...
		DBGLOG(REQ, INFO, "priv_driver_get_nvram offset=0x%x\n", offset);
	}
	else{
		DBGLOG(REQ, INFO, "priv_driver_get_nvram cmd error\n");
		DBGLOG(REQ, INFO, "--priv_driver_get_nvram--\n");
		i4BytesWritten = snprintf(pcCommand, i4TotalLen, "Exec result Fail");
		return -1;
	}
...
#if CFG_SUPPORT_WOWLAN
		else if (strncasecmp(pcCommand, CMD_SETWOWENABLE, strlen(CMD_SETWOWENABLE)) == 0) {
...
void convert_compat_fromuser(IN struct iw_request_info *prIwReqInfo, IN OUT union iwreq_data *prIwReqData)
{
	if (prIwReqInfo->flags == IW_REQUEST_FLAG_COMPAT) {
		...
		DBGLOG(REQ, INFO,
			"%s: convert 32 bit userspace ptr to 64 bit kernel compat\n", __func__);
...
void convert_compat_touser(IN struct iw_request_info *prIwReqInfo, IN OUT union iwreq_data *prIwReqData)
{
	if (prIwReqInfo->flags == IW_REQUEST_FLAG_COMPAT) {
		struct compat_iw_point *iwp_compat;
		struct iw_point *iwp = &prIwReqData->data;
		...
		DBGLOG(REQ, INFO,
			"%s: convert to 64 bit kernel ptr to 32 bit userspace compat\n", __func__);
		iwp_compat = (struct compat_iw_point *) &prIwReqData->data;
