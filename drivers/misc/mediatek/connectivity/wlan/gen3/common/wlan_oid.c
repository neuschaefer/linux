		    ...
		    PUINT_8 pucInfoBuffer, OUT PVOID pvSetQueryBuffer, IN UINT_32 u4SetQueryBufferLen)
{
	...
	if ((prAdapter->prGlueInfo->fgIsInSuspendMode) && (fgNeedResp)) {
		DBGLOG(OID, WARN, "suspend mode[%u], drop command CID[0x%02x]\n", prAdapter->prGlueInfo->fgIsInSuspendMode, ucCID);
		return WLAN_STATUS_FAILURE;
	}
...
WLAN_STATUS
wlanoidNotifyWowEnable(IN P_ADAPTER_T prAdapter,
		       IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	CMD_WOW_ENABLE_SETTING_T rWowCmd;

	if (!prAdapter || !pvSetBuffer)
		return WLAN_STATUS_INVALID_DATA;

	rWowCmd.ucBssIndex = 0;
	rWowCmd.fIsEnableWow= *(PBOOLEAN)pvSetBuffer;
	return wlanSendSetQueryCmd(prAdapter,
				   CMD_ID_SET_WOW_ENABLE,
				   TRUE,
				   FALSE,
				   TRUE,
				   nicCmdEventSetCommon,
				   nicOidCmdTimeoutCommon,
				   sizeof(CMD_WOW_ENABLE_SETTING_T),
				   (PUINT_8)&rWowCmd,
				   NULL,
				   0);
}
