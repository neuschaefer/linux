...
ssize_t BT_write_fwlog(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
	...
	memset(p_buf, 0, count);
	if (copy_from_user(p_buf, buf, count)) {
		retval = -EFAULT;
		goto OUT;
	}

	if (0 == memcmp(p_buf, "dbglvl=", strlen("dbglvl="))) {
		if (p_buf[strlen("dbglvl=")] >= '0' && p_buf[strlen("dbglvl=")] <= '9') {
			int lvl = p_buf[strlen("dbglvl=")] - '0';
			gDbgLevel = lvl < BT_LOG_DBG ? lvl : BT_LOG_DBG;
		}
		retval = 0;
		goto OUT;
	}
	...
	for (i = 0, j = 0; i < count; i++) {
		...
		if ((p_buf[i] >= '0' && p_buf[i] <= '9')
			...
			if (ret == 0)
				o_buf_fwlog[j++] = (UINT8)res;
			else
				BT_ERR_FUNC("%s: Convert %s failed(%d)", __func__, temp_str, ret);
		}
	}
...
ssize_t BT_read_fwlog(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
	...
	INT32 retval = 0;
	char str[FWLOG_I_LEN_FWLOG * 3 + 32];
	char *ptr = str;
	UINT8 len, i;
	...
	if (read_flag == 0) {
		if (read_flag_fwlog == true) {
			...
			ptr += sprintf(ptr, "FW read event:");
			for (i = 0; i < len; i++)
				ptr += sprintf(ptr, " %02X", i_buf_fwlog[i]);

			ptr += sprintf(ptr, "\n");

			retval = (count - 1) < strlen(str) ? (count - 1) : strlen(str);
			if (retval > 0) {
