...
static ssize_t tonly_pwm_led(struct device *dev, struct device_attribute *attr, const char *buf,
                               size_t count)
{
    int on_off, pwm_no, period, duty;
   // int i=0;

   // printk("[xu pwm]:user input length:%d\n", (int)count);
   // for(i=0; i<count; i++)
   //     printk("[xu pwm]:user input byte:%d\n", (int)buf[i]);

    /*ret = sscanf(buf, "%d %d %d %d %d", &cmd, &period1, &period2, &duty1, &duty2);
    if (!ret)
        pr_err("tonly_pwm_led parameter nothing\n");*/
    ...
    on_off = (buf[0]&0xF0)>>4;
    pwm_no = buf[0]&0x0F;
    period = (buf[1]<<8) + buf[2];
    duty = (buf[3]<<8) + buf[4];

    //on_off = (cmd&0xF0)>>4;
    //pwm_no = cmd&0x0F;
    //period1 = (period1<<8) + period2;
    //duty1 = (duty1<<8) + duty2;
    
     if(strncmp(buf,"ON_W", 4) == 0){
                //printk("<zqb> 2017-4-24 \n");
		on_off = 1;
		pwm_no = 2;
		period = 10;
		duty = 10;
        }
    if(strncmp(buf,"OFFW", 4) == 0){
                //printk("<zqb> 2017-4-24 \n");
                on_off = 1;
                pwm_no = 2;
                period = 10;
    ...
    if (on_off == 1)
    {
        ...
        conf.PWM_MODE_OLD_REGS.GDURATION = 0;
        conf.PWM_MODE_OLD_REGS.WAVE_NUM = 0;
        conf.PWM_MODE_OLD_REGS.DATA_WIDTH = period;//9=300us
        conf.PWM_MODE_OLD_REGS.THRESH = duty;//5;
        pwm_set_spec_config(&conf);
    }
    else if(on_off == 2)
    {
...
static DEVICE_ATTR(tonly_pwm, 0644, pwm_debug_show, tonly_pwm_led);
...
*/
    ...
    ret = device_create_file(&pdev->dev, &dev_attr_tonly_pwm);
    if (ret)
        PWMINFO("error creating sysfs files: tonly_pwm\n");
