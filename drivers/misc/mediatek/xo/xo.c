...
static unsigned int nvramread_error_cnt;
...
static void xocap_work_func(struct work_struct *work)
{
	...
	if( ret < 0) { /* retry after 5s when nvram_read fail */
		pr_err("[xo] nvram read ret is %d!\n", ret);
		if(nvramread_error_cnt < 3) {
			nvramread_error_cnt += 1;
			pr_err("[xo] xocap_work retry time: %d!\n", nvramread_error_cnt);
			bsi_clock_enable(false);
