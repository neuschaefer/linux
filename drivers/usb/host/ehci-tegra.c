...
)
{
	...
	if (typeReq == ClearPortFeature && wValue == USB_PORT_FEAT_ENABLE) {
		...
		if (tegra->port_resuming) {
			...
			if (delay > 0) {
				ehci_warn(ehci, "GetPortStatus called too soon, waiting %dms...\n", delay);
				mdelay(jiffies_to_msecs(delay));
			}
			/* Ensure the port has left suspended state and PORT_RESUME has cleared.
			 * The K signal will be maintained thanks to the OBS bus.
			 * 25 ms timeout should be more than enough as it covers the whole K
			 * signaling interval. */
			if (handshake(ehci, status_reg, (PORT_SUSPEND | PORT_RESUME), 0, 25000)) {
				ehci_err(ehci, "%s: timeout waiting for SUSPEND to clear, status: 0x%x\n", __func__, temp);
			}
			tegra->port_resuming = 0;
			tegra_usb_phy_postresume(tegra->phy, false);
			/* RUN bit is re-enabled from here */
			if (tegra->phy->usb_phy_type == TEGRA_USB_PHY_TYPE_UTMIP) {
		...
		/* All resume-related bits are cleared now, we can proceed with regular ehci_hub_control */
