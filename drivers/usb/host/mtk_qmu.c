...
static int QMU_set_gpd(MGC_LinuxCd *pThis, uint8_t *pDataBuf, uint32_t data_len, uint8_t EP_Num, uint8_t isIOC, uint8_t IsRx)
{
    ...
    if (EP_Num < 1)
    {
        printk("ERROR: EP_Num < 1.\n");        
        return -ENOMEM;
    }

    if (((EP_Num - 1) >= RXQ_NUM) ||((EP_Num - 1) >= TXQ_NUM))
    {
        printk("ERROR: (EP_Num - 1) overflow.\n");        
        return -ENOMEM;
    }
...
{   
    ...
    for (i=0; i<RXQ_NUM; i++)
    {
        ...
        if (!pBuf->pLogicalBuf)
        {
            printk("pBuf->pBuffer kzalloc failed\n");
            return -ENOMEM;
        }
...
int QMU_insert_task(MGC_LinuxCd *pThis, uint8_t EP_Num, uint8_t isRx, uint8_t *buf, uint32_t length, uint8_t isIOC)
{
    ...
    //printk("EP_Num: %d, isRx: %d, buf: %x, length: %d\n", EP_Num, isRx, (uint32_t)buf, length);
...
static void QMU_rx_error(MGC_LinuxCd *pThis, uint8_t bEnd)
{
    ...
    if (MUSB_C_NUM_EPS <= bEnd)
    {
        printk("ERROR: bEnd overflow.\n");        
        return;
    }
...
int QMU_irq(MGC_LinuxCd *pThis, uint32_t qisar)
{
    ...
    if (wQmuVal & DQMU_M_RXQ_ERR)
    {
        ...
        printk("Rx Queue error in QMU mode![0x%x]\n", wRetVal);
        ...
        for (i = 0; i < RXQ_NUM; i++)
        {
            ...
            {
                printk("Rx %d GPD checksum error!\n", EP_Num);
                result = 1;
                break;
            }
            ...
            {
                printk("Rx %d recieve length error!\n", EP_Num);

                MGC_SelectEnd(base, EP_Num);
                printk("RxCsr=0x%04X, RxCount=0x%04X, RxMaxp=0x%04X.\n", 
            ...
            {
                printk("Rx %d recieve an zlp packet!\n", EP_Num);
                result = 1;
                break;
            }            
        }
    ...
    if (wQmuVal & DQMU_M_TXQ_ERR)
    {
        ...
        printk("Tx Queue error in QMU mode![0x%x]\n", wRetVal);
        ...
        for (i= 0; i < TXQ_NUM; i++)
        {
            ...
            {
                printk("Tx %d GPD checksum error!\n", EP_Num);
                result = 1;
                break;
            }
            if (wRetVal & DQMU_M_TX_LEN_ERR(EP_Num))
            {
                printk("Tx %d buffer length error!\n", EP_Num);
                result = 1;
                break;
            }
        }
    ...
    for (i = 0; i < RXQ_NUM; i++)
    {
        ...
        if (wQmuVal & DQMU_M_RX_DONE(EP_Num))
        {
            ...
            while (Buf_GetReadPhyAddr(pBuf) != u4Val)
            {
                if (Buf_IsEmpty(pBuf))
                {
                    printk("ERROR: rx_gpd_container empty!\n");
...
wait_rx_ioc:
                    ...
                    {
                        //printk("QMU_IRQ: Rx HWO is on !\n");
                        //printk("PhyAddr=0x%08X, EndAddr=0x%08X.\n", Buf_GetReadPhyAddr(pBuf), u4Val);
                        goto wait_rx_ioc;
                    }
                    ...
                    if (usb_pipeisoc(pUrb->pipe))
                    {
                        ...
                        if (d->actual_length == 0)
                        {
                            ...
                            printk("[USB] QMU_IRQ: Rx expect %d, actual = 0.\n", d->length);
                        }
                        */
                        d->status = 0; /* Fixed me !! */
                        ...
                        {
                            printk("[USB] QMU_IRQ: address compare fail, 0x%08X, 0x%08X.\n", 
                    ...
                    else
                    {
                        ...
                        //printk("QMU_IRQ: RX, actual_length = %d.\n", pUrb->actual_length);                                                
    ...
    for (i = 0; i < TXQ_NUM; i++)
    {
        ...
        if (wQmuVal & DQMU_M_TX_DONE(EP_Num))
        {
            ...
            while (Buf_GetReadPhyAddr(pBuf) != u4Val)
            {
                if (Buf_IsEmpty(pBuf))
                {
                    printk("ERROR: tx_gpd_container empty!\n");
                    return -ENOMEM;
                }
                else
                {
wait_tx_ioc:   
                    ...
                    else
                    {
                        ...
                        //printk("QMU_IRQ: TX, actual_length = %d.\n", pUrb->actual_length);                        
