		...
		union xhci_trb *event)
{
	...
	if ((temp & PORT_PLC) && (temp & PORT_PLS_MASK) == XDEV_RESUME) {
		...
		xhci_writel(xhci, temp1, addr);
		goto cleanup;
	} else if ((temp & PORT_PLS_MASK) == XDEV_RXDETECT) {
		if (temp & PORT_RESET) {
			report_status = 0;
			temp1 = xhci_port_state_to_neutral(temp);
			temp1 |= PORT_CSC;
		...
		if (td->tmpbuf) {
			if (usb_pipein(td->urb->pipe)) {
				...
				memcpy(td->urb->transfer_buffer + frame->offset,
					td->tmpbuf, frame->length - TRB_LEN(event->transfer_len));
			} else {
				dma_unmap_single(&td->urb->dev->dev, td->tmpbuf_dma,
	...
	struct urb *urb, int slot_id)
{
	...
	if (USB_REQ_CLEAR_FEATURE == setup->bRequest &&
		USB_ENDPOINT_HALT == le16_to_cpu(setup->wValue) &&
		EP_STATE_HALTED != (ep_ctx->ep_info & EP_STATE_MASK))
		return -EINVAL;
