...
static void ohci_hcd_resume_handler(struct work_struct *work);
static DECLARE_DELAYED_WORK(ohci_hcd_resume_work, ohci_hcd_resume_handler);
...
struct device *ohci_hcd_dev = NULL;
...
static void ohci_hcd_resume_handler(struct work_struct *work)
{
        struct device *dev = ohci_hcd_dev;
	struct sw_hci_hcd *sw_ohci = NULL;
	struct usb_hcd *hcd = NULL;
...
{
       ohci_hcd_dev = dev;
       schedule_delayed_work(&ohci_hcd_resume_work, HZ / 2);
       return 0;
}
