...
static void ehci_hcd_resume_handler(struct work_struct *work);
static DECLARE_DELAYED_WORK(ehci_hcd_resume_work, ehci_hcd_resume_handler);
...
struct device *ehci_hcd_dev = NULL;
...
static void ehci_hcd_resume_handler(struct work_struct *work)
{
	struct device *dev = ehci_hcd_dev;
	struct sw_hci_hcd *sw_ehci = NULL;
	struct usb_hcd *hcd = NULL;
	struct ehci_hcd *ehci = NULL;
...
{
	ehci_hcd_dev = dev;
	schedule_delayed_work(&ehci_hcd_resume_work, HZ / 4);
	return 0;
}
