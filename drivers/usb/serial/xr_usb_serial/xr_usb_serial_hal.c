...
void xr_usb_serial_special(struct usb_serial_port *port, unsigned short DeviceProductID);
...
/*This is special function which is created to perform a special feature for customer.
It sets DTR low when the device when called
*/
void xr_usb_serial_special(struct usb_serial_port *port, unsigned short DeviceProductID)
{
	struct xr_usb_serial_port_private *portdata = usb_get_serial_port_data(port);
	portdata->DeviceProduct=DeviceProductID;
	if((portdata->DeviceProduct == 0x1410)||
		   (portdata->DeviceProduct == 0x1412)||
		   (portdata->DeviceProduct == 0x1414))
	{
	...
	xr_usb_serial_set_reg(port, portdata->reg_map.uart_gpio_mode_addr, 0); 
	xr_usb_serial_set_reg(port, portdata->reg_map.uart_gpio_dir_addr, 0x00);  
}
