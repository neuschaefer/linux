...
#ifndef        __LINUX_USB_AESS_COMPOSITE_H
#define        __LINUX_USB_AESS_COMPOSITE_H
...
struct _usb_composite_dev;
	

/**
 * struct _usb_function - describes one function of a composite device
 ...
 * @function_index: index into _usb_composite_dev.function_data[]
...
struct _usb_function {
	...
	int    (*bind)(struct _usb_composite_dev *);
	void   (*unbind)(struct _usb_composite_dev *);
	int    (*setup)(struct _usb_composite_dev *,
				const struct usb_ctrlrequest *);
	void   (*disconnect)(struct _usb_composite_dev *);
	void   (*suspend)(struct _usb_composite_dev *);
	void   (*resume)(struct _usb_composite_dev *);
...
struct _usb_composite_dev {
	...
	u8                   cdev_index;
	u8                   set_config_count;
	struct _usb_function  *current_bind;
	struct _usb_function  *function[MAX_NUM_FUNCTIONS];
...
struct usb_composite_devices {
	u8                       next_cdev_index;
	u8                       num_cdevs;
	struct _usb_composite_dev *cdevs;
};

int _usb_composite_register(struct _usb_function *);
int _usb_composite_unregister(struct _usb_function *);
...
#endif /* __LINUX_USB_AESS_COMPOSITE_H */
