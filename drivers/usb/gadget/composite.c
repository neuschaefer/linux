...
#define DRIVER_VERSION  "20080519"
...
static struct usb_composite_dev* get_cdev(struct usb_function *f)
{
...
static int config_buf(struct usb_composite_dev *cdev, void *buf, u8 type)
{
...
/*-------------------------------------------------------------------------*/
static int composite_set_config(struct usb_composite_dev *cdev, unsigned number)
{
	struct usb_function     *f;
	int                     result = 0;
	int			i;
	...
	printk(KERN_DEBUG DRIVER_NAME " %s: %s speed config #%d\n",
...
static struct usb_function* get_function_by_interface(
					struct usb_composite_dev *cdev,
					u16	w_index)
{
	struct usb_function		*f;
			...
			const struct usb_ctrlrequest *ctrl)
{
	...
	case USB_REQ_SET_CONFIGURATION:
		...
		spin_unlock(&cdev->lock);
		value = 0;   // for status stage response
		respond = 1;
		break;
...
int usb_composite_register(struct usb_function *f)
{
	struct usb_composite_dev *cdev;
	int status;
...
EXPORT_SYMBOL(usb_composite_register);

int usb_composite_unregister(struct usb_function *f)
{	
	struct usb_composite_dev *cdev;
...
EXPORT_SYMBOL(usb_composite_unregister);
...
static int __init composite_init(void)
{
	...
	the_cdevs.cdevs = kzalloc(mod_data.num_controllers *
				sizeof(struct usb_composite_dev), GFP_KERNEL);
	if (!the_cdevs.cdevs)
		return -ENOMEM;
