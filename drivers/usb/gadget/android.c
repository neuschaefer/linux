...
#ifdef CONFIG_USB_MTK_UAC1
#include "f_uac1.c"
#endif

#ifdef CONFIG_USB_MTK_UAC2
#include "f_uac2.c"
#endif
...
static int uac1_function_init(struct android_usb_function *f,
			struct usb_composite_dev *cdev)
{
	return 0;
}

static int uac1_function_bind_config(struct android_usb_function *f,
		struct usb_configuration *c)
{
	pr_notice("%s bind", __func__);
	c->cdev->gadget->is_uac1 = 1;

	return audio_bind_config(c);
}

static void uac1_function_unbind_config(struct android_usb_function *f,
						struct usb_configuration *c)
{
	pr_notice("%s unbind", __func__);
	c->cdev->gadget->is_uac1 = 0;
...
static struct android_usb_function uac1_function = {
	...
	.unbind_config	= uac1_function_unbind_config,
};
#endif
...
static void uac2_function_unbind_config(struct android_usb_function *f,
						struct usb_configuration *c)
{
	pr_notice("%s unbind", __func__);
...
static struct android_usb_function uac2_function = {
	...
	.unbind_config	= uac2_function_unbind_config,
};
#endif
