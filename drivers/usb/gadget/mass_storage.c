...
#define DRIVER_DATE		"20120912"

#if defined (MS_DEVICE_1)
  #define DRIVER_NAME		"g_mass_storage1"
  #define THREAD_NAME		"MSD1"
  #define DRIVER_VERSION	DRIVER_DATE"-1"
#elif defined (MS_DEVICE_2)
  #define DRIVER_NAME		"g_mass_storage2"
  #define THREAD_NAME		"MSD2"
  #define DRIVER_VERSION	DRIVER_DATE"-2"
#elif defined (MS_DEVICE_3)
  #define DRIVER_NAME		"g_mass_storage3"
  #define THREAD_NAME		"MSD3"
  #define DRIVER_VERSION	DRIVER_DATE"-3"
#elif defined (MS_DEVICE_4)
  #define DRIVER_NAME		"g_mass_storage4"
  #define THREAD_NAME		"MSD4"
  #define DRIVER_VERSION	DRIVER_DATE"-4"
#else
  #define DRIVER_NAME		"g_mass_storage"
  #define THREAD_NAME		"MSD"
  #define DRIVER_VERSION	DRIVER_DATE
#endif

#define CONTROLLER_NAME         "PCD"
#define VENDOR_NAME             "Linux"
#define PRODUCT_NAME            "Virtual Media"
#define PRODUCT_VERSION			"ffff"

#define DRIVER_VENDOR_ID        0x0624	// Avocent
#if defined (MS_DEVICE_1)
  #define DRIVER_PRODUCT_ID       0x0251
#elif defined (MS_DEVICE_2)
  #define DRIVER_PRODUCT_ID       0x0252
#elif defined (MS_DEVICE_3)
  #define DRIVER_PRODUCT_ID       0x0253
#elif defined (MS_DEVICE_4)
  #define DRIVER_PRODUCT_ID       0x0254
#else
  #define DRIVER_PRODUCT_ID       0x0250
#endif
...
module_param_named(productVersion, mod_data.productVersion, charp, S_IRUGO);
MODULE_PARM_DESC(productVersion, "Product version for device for inquiry");
...
MODULE_PARM_DESC(idProduct, "idProduct field of Device Descriptor");
...
module_param_named(dbg, mod_data.dbg, uint, S_IRUGO);
MODULE_PARM_DESC(dbg, "debug level");


/*-------------------------------------------------------------------------*/
#define	DEBUG_PRINT_ENABLED	0x0001
#define	DEBUG_SETUP_ENABLED	0x0002
#define	DEBUG_WRITE_ENABLED	0x0004
#define	DEBUG_READ_ENABLED	0x0008
#define	DEBUG_IOCTL_ENABLED	0x0010
...
static void DEBUG_PRINT(char *fmt, ...)
{
	...
	if (mod_data.dbg & DEBUG_PRINT_ENABLED) {
		va_start(args, fmt);
...
static void DEBUG_SETUP(char *fmt, ...)
{
	va_list args;
	char buf[256];

	if (mod_data.dbg & DEBUG_SETUP_ENABLED) {
		va_start(args, fmt);
		vsnprintf(buf, 256, fmt, args);
		va_end(args);
		printk("%s", buf);
...
static void DEBUG_WRITE(char *fmt, ...)
{
	va_list args;
	char buf[256];

	if (mod_data.dbg & DEBUG_WRITE_ENABLED) {
		va_start(args, fmt);
		vsnprintf(buf, 256, fmt, args);
		va_end(args);
		printk("%s", buf);
...
static void DEBUG_READ(char *fmt, ...)
{
	va_list args;
	char buf[256];

	if (mod_data.dbg & DEBUG_READ_ENABLED) {
		va_start(args, fmt);
		vsnprintf(buf, 256, fmt, args);
		va_end(args);
		printk("%s", buf);
...
static void DEBUG_IOCTL(char *fmt, ...)
{
	va_list args;
	char buf[256];

	if (mod_data.dbg & DEBUG_IOCTL_ENABLED) {
		va_start(args, fmt);
		vsnprintf(buf, 256, fmt, args);
		va_end(args);
		printk("%s", buf);
...
/*-------------------------------------------------------------------------*/
// IOCTL types
#define IOCTL_MSD_MAGIC				'i'

#define IOCTL_MSD_OPEN              _IOW(IOCTL_MSD_MAGIC, 190, struct msd_open_data)
#define IOCTL_MSD_EJECT             _IOW(IOCTL_MSD_MAGIC, 191, unsigned int)
#define IOCTL_MSD_STATUS            _IOWR(IOCTL_MSD_MAGIC, 192, struct msd_status_data)
#define IOCTL_MSD_RESET             _IOW(IOCTL_MSD_MAGIC, 193, unsigned int)
#define IOCTL_MSD_SET_LUN_TYPES     _IOWR(IOCTL_MSD_MAGIC, 194, struct msd_type_data)
#define IOCTL_MSD_GET_LUN_TYPES     _IOWR(IOCTL_MSD_MAGIC, 195, struct msd_type_data)
#define IOCTL_MSD_SET_INQUIRY_DATA  _IOWR(IOCTL_MSD_MAGIC, 196, struct msd_inquiry_data)
#define IOCTL_MSD_WRITE_COMPLETE    _IO(IOCTL_MSD_MAGIC, 197)
#define IOCTL_MSD_READ_COMPLETE     _IO(IOCTL_MSD_MAGIC, 198)
#define IOCTL_MSD_WRITE_ERROR       _IO(IOCTL_MSD_MAGIC, 199)
#define IOCTL_MSD_READ_ERROR        _IO(IOCTL_MSD_MAGIC, 200)

#define IOCTL_MSD_MAXNR				200	
...
struct msd_dev {
	...
	struct _usb_composite_dev *c_dev;	
	bool		bAttached;
};
...
#ifdef CONFIG_USB_COMPOSITE
static struct _usb_function	msd_driver;
#else
static struct usb_gadget_driver	msd_driver;
#endif
...
static struct usb_config_descriptor msd_config = {
	.bLength                = sizeof msd_config,
                ...
                u8 *buf, u8 type, unsigned index)
{
	...
	len = usb_gadget_config_buf(&msd_config, buf, EP0_BUFSIZE, (const struct usb_descriptor_header **)function);
...
#if defined (CONFIG_USB_SH7757_ENABLE_DMA_TRANSFER)
...
#ifdef CONFIG_USB_COMPOSITE
static void msd_disconnect(struct _usb_composite_dev *cdev)
#else
static void msd_disconnect(struct usb_gadget *gadget)
#endif
{
#ifdef CONFIG_USB_COMPOSITE
	struct msd_dev *msd = get_composite_data(cdev, function_index);
#else
	struct msd_dev *msd = get_gadget_data(gadget);
#endif
...
static int ep0_queue(struct msd_dev *msd)
{
	...
	if (rc != 0 && rc != -ESHUTDOWN) {
		...
		DEBUG_PRINT(KERN_DEBUG "%s: %()s error in submission: %s --> %d\n",
				DRIVER_NAME, __FUNCTION__, msd->ep0->name, rc);
	}
	return rc;
}
...
static void ep0_complete(struct usb_ep *ep, struct usb_request *req)
{
	if (req->status || req->actual != req->length)
		DEBUG_PRINT(KERN_DEBUG "%s: %s() %d, %u/%u\n", DRIVER_NAME, __FUNCTION__,
...
static void bulk_in_complete(struct usb_ep *ep, struct usb_request *req)
{
	...
	if (req->status || req->actual != req->length) {
		DEBUG_PRINT(KERN_DEBUG "%s: %s() %d, %u/%u, req=0x%08X, LUN[%d][%d]\n", DRIVER_NAME, __FUNCTION__,
...
static void bulk_out_complete(struct usb_ep *ep, struct usb_request *req)
{
	...
	if (req->status || req->actual != bh->bulk_out_intended_length) {
		DEBUG_PRINT(KERN_DEBUG "%s: %s() %d, %u/%u, req=0x%08X, LUN[%d][%d]\n", DRIVER_NAME, __FUNCTION__,
				...
				bh->bulk_out_intended_length, (u32)req,
				interface->index, interface->lun);
	}
	if (req->status == -ECONNRESET)		// Request was cancelled
		usb_ep_fifo_flush(ep);
		...
		const struct usb_ctrlrequest *ctrl)
{
	...
	case USB_BULK_RESET_REQUEST:
		...
		for (i = 0; i < msd->num_interfaces; i++) {
			if (w_index == intf_desc[i].bInterfaceNumber) {
				...
				DEBUG_SETUP(KERN_DEBUG "%s: %s() USB_BULK_RESET_REQUEST: interface[%d] w_index=%d\n",
				DRIVER_NAME, __FUNCTION__, msd->interface[i].index, w_index);
	...
	case USB_BULK_GET_MAX_LUN_REQUEST:
		...
		DEBUG_SETUP(KERN_DEBUG "%s: %s() get max LUN: %d for interface[%d], w_index=%d\n",
				DRIVER_NAME, __FUNCTION__, msd->interface[i].nluns - 1, i, w_index);
		break;
	}

	if (value == -EOPNOTSUPP)
		printk(KERN_ERR "%s: %s() unknown class-specific control req "
		...
		const struct usb_ctrlrequest *ctrl)
{
...
#endif /* CONFIG_USB_GADGET_DUALSPEED */
		case USB_DT_CONFIG:
			value = populate_config_buf (msd->gadget, req->buf,
					ctrl->wValue >> 8,
					ctrl->wValue & 0xff);
			if (value >= 0)
	...
	case USB_REQ_GET_CONFIGURATION:
		if (ctrl->bRequestType != (USB_DIR_IN | USB_TYPE_STANDARD |
				USB_RECIP_DEVICE))
			break;
		DEBUG_SETUP(KERN_DEBUG "%s: %s() get configuration\n", DRIVER_NAME, __FUNCTION__);
	...
	case USB_REQ_SET_INTERFACE:
		...
		for (i = 0; i < msd->num_interfaces; i++) {
			if (msd->config && w_index ==
						intf_desc[i].bInterfaceNumber && w_value == 0) {
				raise_exception(&msd->interface[i],
						MSD_STATE_INTERFACE_CHANGE);
				value = DELAYED_STATUS;
			}
	...
	case USB_REQ_GET_INTERFACE:
		...
		if (!msd->config)
			break;
		if (w_index < 0 || w_index >= msd->num_interfaces) {
			value = -EDOM;
			break;
		}

		DEBUG_SETUP(KERN_DEBUG "%s: %s() get interface\n", DRIVER_NAME, __FUNCTION__);
		value = -EDOM;
		for (i = 0; i < msd->num_interfaces; i++) {
			if (w_index == intf_desc[i].bInterfaceNumber) {
				*(u8 *) req->buf = intf_desc[i].bAlternateSetting;
				value = 1;
				break;
			}	
		}
	...
	default:
		printk(KERN_ERR "%s: %s() unknown control req %02x.%02x v%04x"
				" i%04x l%u\n", DRIVER_NAME, __FUNCTION__,
			ctrl->bRequestType, ctrl->bRequest,
...
#ifdef CONFIG_USB_COMPOSITE
static int msd_setup(struct _usb_composite_dev *cdev,
		     const struct usb_ctrlrequest *ctrl)
#else
static int msd_setup(struct usb_gadget *gadget,
		     const struct usb_ctrlrequest *ctrl)
#endif
{
#ifdef CONFIG_USB_COMPOSITE
		...
		enum msd_buffer_state *state)
{
	...
	*state = BUF_STATE_BUSY;
	...
	DEBUG_PRINT(KERN_DEBUG "%s: %s() calling usb_ep_queue ep=%s, len=%u, req=0x%08X LUN[%d][%d]\n", DRIVER_NAME, __FUNCTION__,
				ep->name, req->length, (u32)req, 
				interface->index, interface->lun);
	...
	if (rc != 0) {
		...
		*state = BUF_STATE_EMPTY;
		...
		if (rc != -ESHUTDOWN && !(rc == -EOPNOTSUPP &&
						req->length == 0))
			DEBUG_PRINT(KERN_DEBUG "%s: %s() error in submission: %s --> %d\n",
				DRIVER_NAME, __FUNCTION__, ep->name, rc);
	}
}
...
static void read_timeout(unsigned long _interface)
{
	...
	DEBUG_READ(KERN_DEBUG "%s: %s() READ10 timed out LUN[%d][%d], stalling %s\n",
			DRIVER_NAME, __FUNCTION__, interface->index, interface->lun,
	...
	if (rc != 0) {
		printk(KERN_ERR "%s: %s() Could not halt %s\n",
			DRIVER_NAME, __FUNCTION__, interface->bulk_in->name);		
	}
...
static int do_read(struct interface_dev *interface)
{
	...
	interface->status.num_blocks      = interface->data_size_from_cmnd / curlun->block_size;
	interface->status.lun             = interface->lun;
	curlun->curlba                    = lba;
	curlun->amount_left_to_read       = interface->data_size_from_cmnd;
	...
	DEBUG_READ(KERN_DEBUG "\n%s: %s() Start!!! drive[%d][%d], start_block=0x%08X,"
		" num_blocks=%d, pending=%d\n",
		 DRIVER_NAME, __FUNCTION__, interface->index,
		 interface->status.lun, interface->status.start_block,
		 interface->status.num_blocks, curlun->read_pending);
		 
	mod_timer(&interface->read_timer, jiffies + msecs_to_jiffies(10000));
	...
	DEBUG_READ(KERN_DEBUG "%s: %s() Done!!! drive[%d][%d], pending=%d\n\n",
			DRIVER_NAME, __FUNCTION__, interface->index, interface->lun,
			curlun->read_pending);
...
static int do_write(struct interface_dev *interface)
{
	...
	if (curlun->ro) {
		DEBUG_WRITE(KERN_INFO "%s: %s() Write Protect Error LUN[%d][%d]\n",
				DRIVER_NAME, __FUNCTION__, interface->index, interface->lun);
	...
	curlun->curlba                     = lba;
	curlun->amount_left_to_write       = interface->data_size_from_cmnd;
	curlun->write_pending              = 1;
	interface->read_aborted            = 0;
	
	DEBUG_WRITE(KERN_DEBUG "\n%s: %s() Start!!! lba = %d, blocks = %d, drive[%d][%d]\n",
				DRIVER_NAME, __FUNCTION__, interface->status.start_block,
	...
	while (curlun->write_pending == 1) {
		//DEBUG_WRITE(KERN_DEBUG "%s: %s() sleeping main thread, drive[%d][%d]\n",
		//		DRIVER_NAME, __FUNCTION__, interface->id, interface->lun);
		if ((rc = sleep_thread(interface)) != 0)
			return rc;
	}
	
	DEBUG_WRITE(KERN_DEBUG "%s: %s() done!!! drive[%d][%d]\n\n",
			DRIVER_NAME, __FUNCTION__, interface->index, interface->lun);
...
static int do_inquiry(struct interface_dev *interface, struct msd_buffhd *bh)
{
	...
	DEBUG_PRINT(KERN_DEBUG "%s: %s() for LUN[%d][%d]\n", DRIVER_NAME, __FUNCTION__, interface->index, interface->lun);

	if (!curlun) {		// Unsupported LUNs are okay
		DEBUG_PRINT(KERN_DEBUG "%s: %s() Unsupported LUN[%d][%d]\n",
			DRIVER_NAME, __FUNCTION__, interface->index, interface->lun);
	...
	if (curlun->type == USB_DRIVE_TYPE_CD) {
		...
		buf[4] = 0x1F;		// Additional length
					// No special options
		if (strncmp(mod_data.productVersion, "ffff", 4)) {
			// The productVersion argument was set.
			// Use productVersion otherwise use release
			sprintf(buf + 8, "%-8s%-16s%-4s", curlun->vendor,
						curlun->product, mod_data.productVersion);
		} else {
	...
	case 0: // No change
		...
		case 2:	// Eject
			if (curlun->prevent_medium_removal) {
				printk(KERN_ERR "%s: %s() Eject attempt"
						" prevented\n", DRIVER_NAME, __FUNCTION__);
...
{	
...
#ifdef MSD_DEBUG	
	{
		...
		DEBUG_PRINT(KERN_DEBUG "%s: %s() Toc Data: length = %d\n", DRIVER_NAME, __FUNCTION__,
							toc_data.length);
		DEBUG_PRINT(KERN_DEBUG "msf = %d, format9 = %d format2 = %d\n",
					msf, format9, format2);
		
		for (i = 0; i < (2 + toc_data.length); ++i)
		{
			DEBUG_PRINT(KERN_DEBUG "%02X ", pTocData[i]);
			if (((i - 3) % 8) == 0)
...
static int halt_bulk_in_endpoint(struct interface_dev *interface)
{
	...
	DEBUG_PRINT(KERN_DEBUG "%s: %s() \n", DRIVER_NAME, __FUNCTION__);

	rc = msd_set_halt(interface, interface->bulk_in);
	while (rc != 0) {
		if (rc != -EAGAIN) {
			printk(KERN_WARNING "%s: %s() usb_ep_set_halt -> %d\n",
						 DRIVER_NAME, __FUNCTION__, rc);
			rc = 0;
			break;
		}
...
static int finish_reply(struct interface_dev *interface)
{
	...
	case DATA_DIR_UNKNOWN:
		if (mod_data.can_stall) {
			DEBUG_PRINT(KERN_DEBUG "%s: %s() stalling endpoints\n",
					DRIVER_NAME, __FUNCTION__);
...
static int send_status(struct interface_dev *interface)
{
	...
	if (interface->phase_error) {
		DEBUG_PRINT(KERN_DEBUG "%s: %s() sending phase-error status\n",
						DRIVER_NAME, __FUNCTION__);
		status = USB_STATUS_PHASE_ERROR;
		sd = SS_INVALID_COMMAND;
	} else if (sd != SS_NO_SENSE) {
		DEBUG_PRINT(KERN_DEBUG "%s: %s() sending command-failure status\n",
						DRIVER_NAME, __FUNCTION__);
		status = USB_STATUS_FAIL;
		DEBUG_PRINT(KERN_DEBUG "%s: %s() sense data: SK 0x%02X, ASC 0x%02X,"
		...
		int needs_medium, const char *name)
{
	...
	DEBUG_PRINT(KERN_DEBUG "%s: %s() SCSI command: %s;  Dc=%d, D%c=%u;  Hc=%d%s, LUN[%d][%d]\n",
			DRIVER_NAME, __FUNCTION__, name, cmnd_size, 
	...
	if (interface->lun != lun) 
		DEBUG_PRINT(KERN_DEBUG "%s: %s() using LUN %d from CBW, "
				"not LUN %d from CDB\n", DRIVER_NAME, __FUNCTION__,
				interface->lun, lun);

	/* Check the LUN */
	if (interface->lun < interface->nluns) {
		...
		if (curlun->type == USB_DRIVE_TYPE_NONE ||
			...
			if (interface->cmnd[0] != SC_INQUIRY &&
				...
				DEBUG_PRINT(KERN_DEBUG "%s: %s() LUN[%d][%d]"
...
static int do_scsi_command(struct interface_dev *interface)
{
	...
	struct lun		*curlun = &interface->luns[interface->lun];
	...
	case SC_READ_6:
		i = interface->cmnd[4];
		interface->data_size_from_cmnd = (i == 0 ? 256 : i) * curlun->block_size;
		if ((reply = check_command(interface, 6, DATA_DIR_TO_HOST,
				(7<<1) | (1<<4), 1,
				"READ(6)")) == 0)
	...
	case SC_READ_10:
		interface->data_size_from_cmnd = get_be16(&interface->cmnd[7]) * curlun->block_size;
		if ((reply = check_command(interface, 10, DATA_DIR_TO_HOST,
				(1<<1) | (0xf<<2) | (3<<7), 1,
				"READ(10)")) == 0)
			reply = do_read(interface);			
	...
	case SC_READ_12:
		interface->data_size_from_cmnd = get_be32(&interface->cmnd[6]) * curlun->block_size;
...
static int received_cbw(struct interface_dev *interface, struct msd_buffhd *bh)
{
	...
	if (req->actual != USB_BULK_CB_WRAP_LEN ||
			cbw->Signature != __constant_cpu_to_le32(
				USB_BULK_CB_SIG)) {
		DEBUG_PRINT(KERN_INFO "%s: %s() invalid CBW: len %d,"
	...
	if (cbw->Lun >= MAX_LUNS || cbw->Flags & ~USB_BULK_IN_FLAG ||
		...
		DEBUG_PRINT(KERN_DEBUG "%s: %s() non-meaningful CBW: lun = %u,"
				" flags = 0x%x, cmdlen %u\n", DRIVER_NAME, __FUNCTION__,
		...
		if (mod_data.can_stall) {
			DEBUG_PRINT(KERN_INFO "%s: %s() stalling"
						" endpoints\n", DRIVER_NAME, __FUNCTION__);
			msd_set_halt(interface, interface->bulk_out);
			halt_bulk_in_endpoint(interface);
		}
		...
		const struct usb_endpoint_descriptor *d)
{
	...
	if (rc)
		printk(KERN_ERR "%s: %s() can't enable %s, result %d\n",
				DRIVER_NAME, __FUNCTION__, ep->name, rc);
	return rc;
}
		...
		struct usb_request **preq)
{
	*preq = usb_ep_alloc_request(ep, GFP_ATOMIC);
	if (*preq)
		return 0;
	printk(KERN_ERR "%s: %s() can't allocate request for %s\n",
				DRIVER_NAME, __FUNCTION__, ep->name);
	return -ENOMEM;
}
...
static int do_set_interface(struct interface_dev *interface, int altsetting)
{
	...
	if (interface->running)
		DEBUG_SETUP(KERN_DEBUG "%s: %s() reset_interface\n", DRIVER_NAME, __FUNCTION__);
reset:
	/* Disable the endpoints */
	if (interface->bulk_in_enabled) {
		usb_ep_disable(interface->bulk_in);
		interface->bulk_in_enabled = 0;
		DEBUG_SETUP(KERN_DEBUG "%s: %s() bulk IN ep for interface[%d] disabled\n",
					DRIVER_NAME, __FUNCTION__, interface->index);
	}
	if (interface->bulk_out_enabled) {
		usb_ep_disable(interface->bulk_out);
		interface->bulk_out_enabled = 0;
		DEBUG_SETUP(KERN_DEBUG "%s: %s() bulk OUT ep for interface[%d] disabled\n",
					DRIVER_NAME, __FUNCTION__, interface->index);
	}
	...
	DEBUG_SETUP(KERN_DEBUG "%s: %s() set interface %d\n", DRIVER_NAME, __FUNCTION__, altsetting);
	...
	if (intf_info[interface->index].enabled) {
		...
		DEBUG_SETUP(KERN_DEBUG "%s: %s() bulk IN ep for interface[%d] enabled\n",
					DRIVER_NAME, __FUNCTION__, interface->index);
		...
		DEBUG_SETUP(KERN_DEBUG "%s: %s() bulk OUT ep for interface[%d] enabled\n",
					DRIVER_NAME, __FUNCTION__, interface->index);
...
static int do_set_config(struct interface_dev *interface, u8 new_config)
{
	...
	if (new_config != 0) {
		...
		if ((rc = do_set_interface(interface, 0)) != 0) {
			...
			DEBUG_SETUP(KERN_DEBUG DRIVER_NAME ": %s speed config #%d"
					" for interface[%d]\n",
...
static void handle_exception(struct interface_dev *interface)
{
	...
	for (;;) {
		...
		if (sig != SIGUSR1) {
			if (interface->state < MSD_STATE_EXIT)
				DEBUG_PRINT(KERN_DEBUG "%s: %s() Main thread"
					" for interface[%d]"
					" exiting on signal\n", DRIVER_NAME, __FUNCTION__,
							interface->index);
	...
	case MSD_STATE_RESET:
		 ...
		 * requires this.) */
		DEBUG_SETUP(KERN_DEBUG "%s: %s() Handling MSD_STATE_RESET for"
				" interface[%d]\n",
				DRIVER_NAME, __FUNCTION__, interface->index);
	...
	case MSD_STATE_CONFIG_CHANGE:
		DEBUG_SETUP(KERN_DEBUG "%s: %s() Handling MSD_STATE_CONFIG_CHANGE for"
				" interface[%d]\n",
				DRIVER_NAME, __FUNCTION__, interface->index);
		...
		if (rc != 0) {			// STALL on errors
			...
			if (interface->index == the_msd->num_interfaces - 1) {
				DEBUG_SETUP(KERN_DEBUG "%s: %s() MSD_STATE_CONFIG_CHANGE Sending status stage for" 
					" interface[%d]\n",
					DRIVER_NAME, __FUNCTION__, interface->index);
#ifdef CONFIG_USB_COMPOSITE
				// Call composite driver to let it determine whether or not it can 
				// send te status stage yet.
				usb_composite_set_config_status_stage(the_msd->c_dev);
#else
				ep0_queue(the_msd);
#endif
			}
	...
	case MSD_STATE_DISCONNECT:
		DEBUG_SETUP(KERN_DEBUG "%s: %s() Handling MSD_STATE_DISCONNECT for"
				" interface[%d]\n",
				DRIVER_NAME, __FUNCTION__, interface->index);
...
static int msd_main_thread(void *interface_)
{
	...
	DEBUG_PRINT(KERN_DEBUG "%s: %s() Starting thread for interface[%d]\n", 
				DRIVER_NAME, __FUNCTION__, interface->index);
...
#if 0
#ifdef CONFIG_USB_COMPOSITE						
		_usb_composite_unregister(&msd_driver);
#else
		usb_gadget_disconnect(the_msd->gadget);
		msd->bAttached = false;
		usb_gadget_unregister_driver(&msd_driver);
#endif
#endif
	}	
...
ssize_t msdUsbRead(struct file *filp, char *buf, size_t count, loff_t *f_pos)
{
	...
	if (minor_num >= the_msd->num_interfaces) {
		printk(KERN_ERR "%s: %s() invalid minor_num (%d)\n",
				DRIVER_NAME, __FUNCTION__, minor_num);
		return (-EINVAL);
	}
	...
	while ((bytes_left > 0) && (interface->read_aborted == 0)) {	
		...
		if (interface->read_aborted) {
			printk(KERN_ERR "%s: %s() LUN[%d][%d] read aborted for"
					" device reset\n", DRIVER_NAME, __FUNCTION__,
					interface->index, interface->lun);				
			return (-1);
		}
		...
		/* copy to user */
		if (copy_to_user(&buf[num_bytes], bh->buf,
							bh->outreq->actual)) {
			return (-EFAULT);
		}
	...
	DEBUG_WRITE(KERN_DEBUG "%s: %s() received %d bytes, %d left\n", DRIVER_NAME, __FUNCTION__,
                    ...
                    loff_t *f_pos)
{
	...
	DEBUG_READ(KERN_DEBUG "%s: %s() %d bytes on LUN[%d][%d]\n", DRIVER_NAME, __FUNCTION__, 
					count, interface->index, interface->lun);
	
	if (curlun->amount_left_to_read <= 0) {
		printk(KERN_ERR "%s: %s() LUN[%d][%d] Unexpected write, amount_left"
					" = %d\n", DRIVER_NAME, __FUNCTION__,
					interface->index, interface->lun,
	...
	while(bytes_left > 0) {
		...
		if (interface->write_aborted) {
			printk(KERN_ERR "%s: %s() LUN[%d][%d] write aborted for device"
					" reset.\n", DRIVER_NAME, __FUNCTION__,
					interface->index, interface->lun);				
			return (-1);
		}
		...
		/* copy from user */
		if(copy_from_user(bh->buf, buf_ptr, byte_count)) {
			return (-EFAULT);
		}
		...
		DEBUG_READ(KERN_DEBUG "%s: %s() LUN[%d][%d] Sending %d bytes\n", DRIVER_NAME, __FUNCTION__,
						interface->index, interface->lun,
						bh->inreq->length);
		...
		DEBUG_READ(KERN_DEBUG "%s: %s() LUN[%d][%d] %d bytes left\n", DRIVER_NAME, __FUNCTION__,
						interface->index, interface->lun,
						bytes_left);
	}
...
unsigned int msdUsbPoll(struct file *filp, struct poll_table_struct *wait)
{
	...
	DEBUG_PRINT(KERN_DEBUG "%s: %s() called for minor_num %d\n", DRIVER_NAME, __FUNCTION__,
								minor_num);
	...
	DEBUG_PRINT(KERN_DEBUG "%s: %s() returned 0x%08X\n", DRIVER_NAME, __FUNCTION__, mask);
	return (mask);	
}
 
#ifndef CONFIG_USB_COMPOSITE
static bool isInterfaceEnabled(void);
#endif
                ...
                unsigned long arg)
{
	...
	if (_IOC_TYPE(cmd) != IOCTL_MSD_MAGIC) {
		printk(KERN_ERR "%s: %s() incorrect ioctl type\n",
				DRIVER_NAME, __FUNCTION__);
		return (-ENOTTY);
	}

	if (_IOC_NR(cmd) > IOCTL_MSD_MAXNR) {
		printk(KERN_ERR "%s: %s() invalid ioctl\n",
				DRIVER_NAME, __FUNCTION__);
		return (-ENOTTY);
	}

	if (_IOC_DIR(cmd) & _IOC_READ)
		err = !access_ok(VERFY_WRITE, (void __user *)arg, _IOC_SIZE(cmd));
	else if (_IOC_DIR(cmd) & _IOC_WRITE)
		err = !access_ok(VERFY_READ, (void __user *)arg, _IOC_SIZE(cmd));
	if (err) {
		printk(KERN_ERR "%s: %s() Can't access arg\n",
				DRIVER_NAME, __FUNCTION__);
		return (-EFAULT);
	}
	...
	case IOCTL_MSD_OPEN:
		if (arg != 0) {
			struct msd_open_data open_data[1];

			if (__copy_from_user((void*) open_data, (void*)arg,
								sizeof(struct msd_open_data))) {
				return (-EFAULT);
			}

			lun = open_data->lun;			
			interface = &msd->interface[minor_num];
			DEBUG_IOCTL(KERN_DEBUG "%s: %s() - IOCTL_MSD_OPEN "
						"LUN[%d][%d]\n",
					DRIVER_NAME, __FUNCTION__, interface->index, lun);
	...
	case IOCTL_MSD_EJECT:
	{
		lun = arg;
		interface = &msd->interface[minor_num];
		DEBUG_IOCTL(KERN_DEBUG "%s: %s() - IOCTL_MSD_EJECT "
						"LUN[%d][%d]\n",
					DRIVER_NAME, __FUNCTION__, interface->index, lun);
	...
	{
		struct msd_status_data status_data[1];

		interface     = &msd->interface[minor_num];
		DEBUG_IOCTL(KERN_DEBUG "%s: %s() - IOCTL_MSD_STATUS interface[%d]\n",
					DRIVER_NAME, __FUNCTION__, interface->index);
		...
		if (__copy_to_user((void*)arg, (void*)status_data, sizeof(struct msd_status_data))) {
			return (-EFAULT);
		}

		DEBUG_IOCTL(KERN_DEBUG "%s: %s() - IOCTL_MSD_STATUS %d occured for LUN[%d][%d]\n",
					DRIVER_NAME, __FUNCTION__,
					status_data->flags, interface->index,
					interface->status.lun);
		break;
	}
	case IOCTL_MSD_RESET:
		DEBUG_IOCTL(KERN_DEBUG "%s: %s() - IOCTL_MSD_RESET\n", DRIVER_NAME, __FUNCTION__);

		build_function_tables();
#ifdef CONFIG_USB_COMPOSITE
		DEBUG_IOCTL(KERN_DEBUG "%s: %s() - IOCTL_MSD_RESET calling usb_gadget_disconnect()\n", DRIVER_NAME, __FUNCTION__);
		usb_gadget_disconnect(msd->gadget);
		msleep(700);

		if (usb_composite_reconfigure_interface_ids(the_msd->c_dev)
									 > 0) {
			DEBUG_IOCTL(KERN_DEBUG "%s: %s() - IOCTL_MSD_RESET calling usb_gadget_connect()\n", DRIVER_NAME, __FUNCTION__);
...
//		}
	...
	{
		struct msd_type_data type_data[1];
		u8 i,j;

		if (__copy_from_user((void*)type_data, (void*)arg,
							sizeof(struct msd_type_data))) {
			return (-EFAULT);
		}
		...
		DEBUG_IOCTL(KERN_DEBUG "%s: %s() IOCTL_MSD_SET_LUN_TYPES for"
			" interface[%d]\n", DRIVER_NAME, __FUNCTION__, interface->index);
		for (i = 1; i <= MAX_LUNS; i++) {
			DEBUG_IOCTL(KERN_DEBUG "[0x%02X,%d] ", type_data->types[i-1],
						type_data->preventRemoval[i-1]);
			if (i%8 == 0)
				DEBUG_IOCTL(KERN_DEBUG "\n");
		}
		DEBUG_IOCTL(KERN_DEBUG "\n");
		...
		DEBUG_IOCTL(KERN_DEBUG "%s: %s() %d LUNs for interface[%d]\n",
			DRIVER_NAME, __FUNCTION__,interface->nlunsReset, interface->index);
		for (i = 1; i <= MAX_LUNS; i++) {
			DEBUG_IOCTL(KERN_DEBUG "[0x%02X,%d] ", interface->luns[i-1].type,
	...
	case IOCTL_MSD_GET_LUN_TYPES:
	{
		...
		DEBUG_IOCTL(KERN_DEBUG "%s: %s() - IOCTL_MSD_GET_LUN_TYPES interface[%d]\n",
					DRIVER_NAME, __FUNCTION__, interface->index);
		...
		if (__copy_to_user((void*)arg, (void*)type_data, sizeof(struct msd_type_data))) {
			return (-EFAULT);
		}
	...
	{
		struct msd_inquiry_data inquiry_data[1];

		if(__copy_from_user((void*)inquiry_data, (void*)arg,
				sizeof(struct msd_inquiry_data))) {
			return (-EFAULT);
		}

		interface = &msd->interface[minor_num];
		
		DEBUG_IOCTL(KERN_DEBUG "%s: %s() - IOCTL_MSD_SET_INQUIRY_DATA"
				" interface[%d][%d] %s\n",
				DRIVER_NAME, __FUNCTION__, interface->index,
				inquiry_data->lun, inquiry_data->productId);

		if (inquiry_data->lun < MAX_LUNS) {
			memcpy(interface->luns[inquiry_data->lun].product,
	...
	case IOCTL_MSD_WRITE_COMPLETE:
		interface = &msd->interface[minor_num];
		curlun = interface->curlun;
		DEBUG_WRITE(KERN_DEBUG "%s: %s() IOCTL_MSD_WRITE_COMPLETE LUN[%d][%d]\n",
			DRIVER_NAME, __FUNCTION__, interface->index, interface->lun);
		curlun->write_pending = 0;
		if (interface->read_aborted == 0) {
			...
			DEBUG_WRITE(KERN_ERR "%s: %s() WRITE_COMPLETE LUN[%d][%d]"
	...
	case IOCTL_MSD_READ_COMPLETE:
		...
		del_timer(&interface->read_timer);
		if (curlun->amount_left_to_read == 0) {
			DEBUG_READ(KERN_DEBUG "%s: %s() READ_COMPLETE LUN[%d][%d]\n",
				DRIVER_NAME, __FUNCTION__, interface->index, interface->lun);
			curlun->read_pending = 0;
			if (interface->write_aborted == 0) {
				wakeup_thread(interface);
			} else {
				DEBUG_READ(KERN_ERR "%s: %s() READ_COMPLETE LUN[%d][%d]"
					" aborted for device reset.\n",
					DRIVER_NAME, __FUNCTION__, interface->index,
					interface->lun);
			}
	...
	case IOCTL_MSD_WRITE_ERROR:
		interface = &msd->interface[minor_num];
		DEBUG_WRITE(KERN_DEBUG "%s: %s() - IOCTL_MSD_WRITE_ERROR LUN[%d][%d]\n",
					DRIVER_NAME, __FUNCTION__,
					interface->index, interface->lun);		
		curlun = interface->curlun;
	...
	case IOCTL_MSD_READ_ERROR:
		interface = &msd->interface[minor_num];
		DEBUG_READ(KERN_DEBUG "%s: %s() - IOCTL_MSD_READ_ERROR LUN[%d][%d]\n",
				DRIVER_NAME, __FUNCTION__, interface->index, interface->lun);
		curlun = interface->curlun;
		curlun->sense_data = SS_UNRECOVERED_READ_ERROR;
		curlun->sense_data_info = curlun->curlba;
		...
		break;
	default:
		printk(KERN_ERR "%s: %s() Bad Command %d\n", DRIVER_NAME, __FUNCTION__, cmd);
		return (-ENOTTY);
	}
	return (0);
}
...
int msdUsbOpen(struct inode *inode, struct file *filp)
{
	...
	if (minor_num >= the_msd->num_interfaces) {
		printk(KERN_ERR "%s: %s() called for invalid minor_num %d\n",
					DRIVER_NAME, __FUNCTION__, minor_num);
		return (-EINVAL);
	}
	
	DEBUG_PRINT(KERN_DEBUG "%s: %s() for interface[%d]\n", DRIVER_NAME, __FUNCTION__, minor_num);
...
/*-------------------------------------------------------------------------*/
#ifdef CONFIG_USB_COMPOSITE
static void msd_unbind(struct _usb_composite_dev *cdev)
#else
static void msd_unbind(struct usb_gadget *gadget)
#endif
{
#ifdef CONFIG_USB_COMPOSITE
	struct msd_dev *msd = get_composite_data(cdev, function_index);
...
static int __init check_parameters(struct msd_dev *msd)
{
	...
	if (mod_data.release == 0xffff) {	// Parameter wasn't set
		...
		if (gcnum >= 0)
			...
			printk(KERN_WARNING "%s: %s() controller '%s' not recognized\n",
					DRIVER_NAME, __FUNCTION__, msd->gadget->name);
...
static int create_luns(struct usb_gadget *gadget,
...
static int msd_start_thread(struct interface_dev *interface)
{
	...
	snprintf(threadName, sizeof(threadName), "%s-%d", THREAD_NAME, interface->index);
	
	interface->thread_task = kthread_create(msd_main_thread, interface, "%s", threadName);
...
#ifdef CONFIG_USB_COMPOSITE
static int __init msd_bind(struct _usb_composite_dev *cdev)
#else
static int __init msd_bind(struct usb_gadget *gadget)
#endif
{
	struct msd_dev	        *msd = the_msd;
	struct interface_dev	*interface;
	...
	if (function_index < 0) {
		printk(KERN_ERR "%s: %s() failed getting function index\n", 
						DRIVER_NAME, __FUNCTION__);
		rc = -EINVAL;
		goto out;		
	}
	...
	for (i = 0; i < msd->num_interfaces; i++) {
		...
		rc = create_luns(gadget, interface, mod_data.num_luns);
		if (rc != 0)
			goto out;
		...
		if (interface_id >= 0)
			...
			printk(KERN_ERR "%s: %s() invalid interface number %d\n",
					DRIVER_NAME, __FUNCTION__, interface_id);
			rc = -EINVAL;
			goto out;
		}
#endif
	...
	if (set_string_ids(cdev) < 0) {
		printk(KERN_ERR "%s: %s() failed setting string IDs\n", DRIVER_NAME, __FUNCTION__);
		rc = -EINVAL;
		goto out;		
	}
#endif
	...
	rc = alloc_chrdev_region(&msd->chrdev, 0, MAX_INTERFACES, DRIVER_NAME); 
    	if (rc < 0) {
    		printk(KERN_ERR "%s: %s() Failed to obtain major number\n",
    						DRIVER_NAME, __FUNCTION__);
		goto out;
	}
	
	printk(KERN_DEBUG "%s: %s() Using Major Number %d\n",
				DRIVER_NAME, __FUNCTION__, MAJOR(msd->chrdev));
	...
	if (rc < 0) {
		printk(KERN_ERR "%s: %s() Failed to register char device\n",
					DRIVER_NAME, __FUNCTION__); 
		goto out;
	}
...
autoconf_fail:
	printk(KERN_ERR "%s: %s() unable to autoconfigure all endpoints\n",
					DRIVER_NAME, __FUNCTION__); 
	rc = -ENOTSUPP;
...
/*-------------------------------------------------------------------------*/
#ifdef CONFIG_USB_COMPOSITE
static void msd_suspend(struct _usb_composite_dev *cdev)
#else
static void msd_suspend(struct usb_gadget *gadget)
#endif
{
#ifdef CONFIG_USB_COMPOSITE
	struct msd_dev *msd = get_composite_data(cdev, function_index);
...
#ifdef CONFIG_USB_COMPOSITE
static void msd_resume(struct _usb_composite_dev *cdev)
#else
static void msd_resume(struct usb_gadget *gadget)
#endif
{
#ifdef CONFIG_USB_COMPOSITE
	struct msd_dev *msd = get_composite_data(cdev, function_index);
...
static int __init msd_init(void)
{
	...
	printk(KERN_DEBUG "%s: %s() Desc: %s\n", DRIVER_NAME, __FUNCTION__, DRIVER_DESC);
	printk(KERN_DEBUG "%s: %s() Name: %s\n", DRIVER_NAME, __FUNCTION__, DRIVER_NAME);
	printk(KERN_DEBUG "%s: %s() Version: %s\n", DRIVER_NAME, __FUNCTION__, DRIVER_VERSION);
...
#ifdef CONFIG_USB_COMPOSITE
	rc = _usb_composite_register(&msd_driver);
#else
  #ifndef CONFIG_USB_GADGET_DUAL_CONTROLLERS
	rc = usb_gadget_register_driver(&msd_driver);
  #endif
#endif 
	if (rc == 0)
		msd->is_registered = 1;
