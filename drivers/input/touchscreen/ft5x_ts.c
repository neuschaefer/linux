...
user_gpio_set_t tp_info;
__hdle tp_hd;

user_gpio_set_t tp_type_info;
__hdle tp_type_hd;
...
extern int ft5x02_Init_IC_Param_qtw(struct i2c_client *client);
...
static int  TP_Power_Para()
{	
	 int err = -1;    
     int device_used = -1;		
	//printk("%s %s %d \n",__FILE__,__func__,__LINE__);
	if(SCRIPT_PARSER_OK != (err = script_parser_fetch("TP_para", "TP_used", &device_used, 1)))
	{	                
		pr_err("%s: hxm@@@ script_parser_fetch err.ret = %d. \n", __func__, err);
		return device_used;	
	}	
	if(1 == device_used){
		pr_err("%s: hxm TP_used. \n",  __func__);	
	}	
	else{
		pr_err("%s: hxm TP_unused. \n",  __func__); 
		return device_used ;	
	}	

    if(OSAL_Script_FetchParser_Data("TP_para", "GPIO_TP_PIN", (int *)&tp_info,sizeof(user_gpio_set_t)/sizeof(int)) < 0)
    {
        __wrn("TP_para GPIO_TP_PIN fail\n");
        return -1;
    }
	tp_hd = OSAL_GPIO_Request(&tp_info,1);
	tp_info.mul_sel = 1;
	tp_info.pull = 1;
	tp_info.drv_level = 1;
	tp_info.data = 0;
	gpio_set_one_pin_status(tp_hd,&tp_info,"GPIO_TP_PIN",1);

    if(OSAL_Script_FetchParser_Data("TP_para", "GPIO_TP_TYPE", (int *)&tp_type_info,sizeof(user_gpio_set_t)/sizeof(int)) < 0)
    {
        __wrn("TP_para GPIO_TP_TYPE fail\n");
        return -1;
    }
	tp_type_hd = OSAL_GPIO_Request(&tp_type_info,1);
	tp_type_info.mul_sel = 0;
	tp_type_info.pull = 1;
	tp_type_info.drv_level = 1;
	tp_type_info.data = 0;
	gpio_set_one_pin_status(tp_type_hd,&tp_type_info,"GPIO_TP_TYPE",1);

	tp_type_value = gpio_read_one_pin_value(tp_type_hd,"GPIO_TP_TYPE");
	printk("Jim_dream gpio_value %d\n",tp_type_value);
...
*/
	...
	err = request_irq(SW_INT_IRQNO_PIO, ft5x_ts_interrupt, IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING| IRQF_SHARED, "ft5x_ts", ft5x_ts);
