...
static int ug_tty_init(void)
{
	...
	/* Alloc a tty_driver with two "lines" */
	driver = tty_alloc_driver(2, 0);
	...
	for (i = 0; i < 2; i++) {
		struct ug_adapter *adapter = &ug_adapters[i];
		tty_port_init(&adapter->tty_port);
		tty_port_link_device(&adapter->tty_port, driver, i);
	}
