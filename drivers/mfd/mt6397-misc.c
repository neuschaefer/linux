...
#if 0
	ret = regmap_read(rtc_misc->regmap, 0x0402,
			  &data);
	if (ret) {
		dev_err(misc->dev, "Failed to read chip id: %d\n", ret);
		return ret;
	}

	data &= 0xF7FF;
	data |= 0x1;
	ret = regmap_write(rtc_misc->regmap, 0x0402, data);
	if (ret) {
		dev_err(misc->dev, "Failed to read chip id: %d\n", ret);
		return ret;
	}
...
//	dev_err(misc->dev, "rtc_gpio_enable_32k \n");
