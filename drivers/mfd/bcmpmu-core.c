...
static DEVICE_ATTR(value, 0644, bcmpmu_show_value, NULL);
				...
				unsigned long arg)
{
	...
	case BCM_PMU_IOCTL_READ_REG:
		if (copy_from_user
			...
			if (ret != 0) {
				...
				printk(KERN_DEBUG "reg=0x%X, val=0x%X, map=0x%X\n",
	...
	case BCM_PMU_IOCTL_BULK_READ_REG:
		if (copy_from_user
		    ((void *)&reg, argp,
		     sizeof(struct bcmpmu_rw_data_ltp)) == 0) {
			printk(KERN_DEBUG "regMap=0x%X, addr=0x%X, len=0x%X\n",
				...
				if (ret != 0) {
					...
					return 0;
						...
						for (i = 0; i < reg.len; i++) {
							reg.val[i] =
								value[i] & 0xff;
							printk(KERN_DEBUG "reg=0x%X, value=0x%X\n",
								reg.addr+i,
								reg.val[i]);
					}
				}
	...
	case BCM_PMU_IOCTL_ADC_READ_REG:
		if (copy_from_user
			...
			printk(KERN_DEBUG "adcCH=0x%X, TM=0x%X\n", adcreq.sig,
			       adcreq.tm);
			...
			printk(KERN_DEBUG "adcADC CH=0x%X, TM=0x%X, CNV=%d\n",
