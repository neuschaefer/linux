...
static struct bcmpmu_accy *g_paccy; //sehyoung_temp
...
{
	struct bcmpmu_accy *paccy =  g_paccy;
...
void send_chrgr_insert_event(enum bcmpmu_event_t event, void *para) // sehyoung_temp
{
	...
	printk("send_chrgr_insert_event: charge current limit =0x%X. calling blocking_notifier_call_chain\n",
	...
	if(event == BCMPMU_CHRGR_EVENT_CHGR_DETECTION){
		if( *((int *)para) == PMU_CHRGR_TYPE_SDP) // CABLE_TYPE_USB
		{
			paccy->bcmpmu->usb_accy_data.chrgr_type = PMU_CHRGR_TYPE_SDP;
			paccy->det_state = USB_CONNECTED;
		}
		else if( *((int *)para) == PMU_CHRGR_TYPE_DCP) //CABLE_TYPE_AC
		{
			last_chgr_str = chrgr_str ="charger";
			paccy->bcmpmu->usb_accy_data.chrgr_type = PMU_CHRGR_TYPE_DCP;
			paccy->bcmpmu->usb_accy_data.usb_type =PMU_USB_TYPE_NONE;
		}
		else if( *((int *)para) ==PMU_CHRGR_TYPE_NONE) // CABLE_TYPE_NONE
		{
			paccy->bcmpmu->usb_accy_data.chrgr_type = PMU_CHRGR_TYPE_NONE;
	...
	if (event == BCMPMU_CHRGR_EVENT_CHGR_DETECTION) {
		...
		pr_accy(FLOW, "3 %s, charge current limit =0x%X\n",
