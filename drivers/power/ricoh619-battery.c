...
#define BATTERY_TEMP_0_VOL	1824	// 0 degree 269.96K, 2.5V * 270K / (270K+100K)
#define BATTERY_TEMP_60_VOL	577		// 60 degree 30.546K, 2.5V * 30K / (30K+100K)
#define BATTERY_TEMP_10_VOL	1601	// 10 degree 178.29K, 2.5V * 178K / (178K+100K)
#define BATTERY_TEMP_45_VOL	827		// 45 degree 49.441K, 2.5V * 50K / (50K+100K)
...
static int calc_capacity_in_period(struct ricoh61x_battery_info *info,
				 int *cc_cap, long *cc_cap_mas, bool *is_charging, int cc_rst)
{
	printk(KERN_INFO "PMU: xiaobo calc_capacity_in_period\n");
	int err;
...
#ifdef	_RICOH619_DEBUG_
	printk(KERN_INFO"PMU:xiaobo ricoh61x_displayed_work STATUS= %d: IBAT= %d: VSYS= %d: VBAT= %d: DSOC= %d: RSOC= %d: cc_delta=%d: rrf= %d\n",
...
//		info->soca->displayed_soc, info->soca->soc);
	...
	|| (info->soca->last_displayed_soc/100 != (info->soca->displayed_soc+50)/100))
			power_supply_changed(&info->battery); // write battery to "/sys/class/power_supply/mc13892_bat/capacity"
...
static int ricoh61x_init_charger(struct ricoh61x_battery_info *info)
{
	...
	val = BATTERY_TEMP_45_VOL * 255 / 2500;
	...
	val = BATTERY_TEMP_10_VOL * 255 / 2500;
...
//	mutex_lock(&info->lock);
	...
	{	// set 1000mA if DCP(10) or CDP(01) .
		ricoh61x_write(info->dev->parent, REGISET1_REG, 0x0D);
		ricoh61x_write(info->dev->parent, REGISET2_REG, 0x2D);
		ricoh61x_write(info->dev->parent, CHGISET_REG, val|0x09);
	}
	else 
	{
		ricoh61x_write(info->dev->parent, REGISET1_REG, 0x06);
		ricoh61x_write(info->dev->parent, REGISET2_REG, 0xE6);
...
static void battery_temp_irq_work(struct work_struct *work)
{
	...
	if (bat_temp_vol < BATTERY_TEMP_60_VOL) {
		printk (KERN_INFO "PMU:%s Battery temp > 60 degrees !! \n", __func__);
		/* Set VTHM threshold high voltage value = (voltage(V)*255)/(2.5) */
		val = BATTERY_TEMP_60_VOL * 255 / 2500;
		ricoh61x_write(info->dev->parent, RICOH61x_ADC_VTHM_THH, val);
		/* Enable VTHM threshold high interrupt */
		ricoh61x_set_bits(info->dev->parent, RICOH61x_INT_EN_ADC2, 0x20);
	}
	...
	else if (bat_temp_vol < BATTERY_TEMP_45_VOL) {
		printk (KERN_INFO "PMU:%s 40 degrees < Battery temp < 60 degrees\n", __func__);
		/* Set VTHM threshold low voltage value = (voltage(V)*255)/(2.5) */
		val = BATTERY_TEMP_60_VOL * 255 / 2500;
		ricoh61x_write(info->dev->parent, RICOH61x_ADC_VTHM_THL, val);
		/* Enable VBAT threshold Low interrupt */
	...
	else if (bat_temp_vol > BATTERY_TEMP_10_VOL){
		printk (KERN_INFO "PMU:%s 0 degrees < Battery temp < 10 degrees\n", __func__);
		/* Set VTHM threshold low voltage value = (voltage(V)*255)/(2.5) */
		val = BATTERY_TEMP_10_VOL * 255 / 2500;
		ricoh61x_write(info->dev->parent, RICOH61x_ADC_VTHM_THL, val);
		/* Enable VBAT threshold Low interrupt */
		...
		printk (KERN_INFO "PMU:%s 10 degrees < Battery temp < 45 degrees\n", __func__);
		/* Set VTHM threshold low voltage value = (voltage(V)*255)/(2.5) */
		val = BATTERY_TEMP_45_VOL * 255 / 2500;
		ricoh61x_write(info->dev->parent, RICOH61x_ADC_VTHM_THL, val);
		/* Enable VBAT threshold Low interrupt */
...
static int calc_capacity(struct ricoh61x_battery_info *info)
{
	printk(KERN_INFO "PMU: xiaobo calc_capacity\n");
	uint8_t capacity;
	long	capacity_l;
	int temp;
	int ret = 0;
...
static int calc_capacity_2(struct ricoh61x_battery_info *info)
{
	printk(KERN_INFO "PMU: xiaobo calc_capacity_2\n");
	uint8_t val;
	long capacity;
	int re_cap, fa_cap;
	int temp;
	...
	if (info->soca->rsoc_ready_flag  != 0) {
		...
		fa_cap = get_check_fuel_gauge_reg(info, FA_CAP_H_REG, FA_CAP_L_REG,
							0x7fff);
		printk(KERN_INFO "PMU: xiaobo calc_capacity_2 re_cap= %d: fa_cap= %d:\n", re_cap, fa_cap);
		if (fa_cap != 0) {
...
static __devinit int ricoh61x_battery_probe(struct platform_device *pdev)
{
	...
	printk(KERN_INFO "PMU: %s : version is %s\n", __func__,RICOH61x_BATTERY_VERSION);
...
//		suspend_period_time *= 1;
	...
	printk("PMU : %s : 	RICOH61x_INT_IR_ADCL 0x%02X\n",	 __func__, val);
	if (val & 0x02) {
			...
			if((info->soca->cc_delta - info->soca->suspend_cc) <= 0){
				...
				if ((30 > suspend_period_time) || ((soc_current - soc_voltage) < 1000)) {
					...
					if((info->soca->status == RICOH61x_SOCA_LOW_VOL) && (is_low_current == false)){
						...
						printk( "PMU: %s : temp_cc_delta_cap_mas(%d), extra value(%d) ==========\n",
