...
static struct volt_cal_fact low_cal_fact_map[] = {
	{20, 0, 0, 0},
	{19, 0, 0, 0},
	{18, 0, 0, 0},
	{17, 0, 0, 0},
...
struct volt_cutoff_fact {
	int volt;
	int fact;
	int state;
};

static struct volt_cutoff_fact cutoff_cal_map[] = {
	{3380, 2, 0},
	{3340, 1, 0},
	{3300, 0, 0},
};
...
static ssize_t fg_low_batt_cal_show(struct device *dev,
				  struct device_attribute *attr,
				  char *buf)
{
	struct bcmpmu *bcmpmu = dev->platform_data;
	...
	int i;
	ssize_t count;
	count = sprintf(buf, "Fuel Low Batt Cal Data\n");
	count += sprintf(buf + count, "low_cal_fact=%d\n",
		pem->low_cal_fact);
	count += sprintf(buf + count, "cap_delta_now=%d\n",
		pem->cap_delta_now);
	count += sprintf(buf + count, "low_cal_index=%d\n",
		pem->low_cal_index);

	for (i = 0; i < ARRAY_SIZE(low_cal_fact_map); i++)
		count += sprintf(buf+count,
			"volt=%d, fact_up=%d, fact_dn=%d, state=%d\n",
				low_cal_fact_map[i].volt,
				low_cal_fact_map[i].fact_up,
				low_cal_fact_map[i].fact_dn,
				low_cal_fact_map[i].state);
	return count;
}
...
static ssize_t cutoff_table_show(struct device *dev,
				  struct device_attribute *attr,
				  char *buf)
{
	int i;
	ssize_t count;
	count = sprintf(buf, "Cutoff Voltage mapping table\n");
	
	for (i = 0; i <ARRAY_SIZE(cutoff_cal_map); i++)
		count += sprintf(buf+count, "volt=%d, cap=%d, st=%d\n",
				cutoff_cal_map[i].volt,
				cutoff_cal_map[i].fact,
				cutoff_cal_map[i].state);
	return count;
}
...
static DEVICE_ATTR(fg_low_batt_cal, 0644, fg_low_batt_cal_show, NULL);
static DEVICE_ATTR(fg_bat_cap, 0644, NULL, fg_bat_cap_set);
static DEVICE_ATTR(cutoff_table, 0644, cutoff_table_show, NULL);
...
static struct attribute *bcmpmu_em_attrs[] = {
	...
	&dev_attr_fg_low_batt_cal.attr,
	&dev_attr_fg_bat_cap.attr,
	&dev_attr_cutoff_table.attr,
...
static int is_lowbat_cal(struct bcmpmu_em *pem, int volt)
{
	int i;
	int ret = 0;
	return 0;
	for (i = 0; i < ARRAY_SIZE(low_cal_fact_map); i++) {
		if ((low_cal_fact_map[i].state == 0) &&
			(volt <= low_cal_fact_map[i].volt)) {
			pem->low_cal_index = i;
			ret = 1;
			pr_em(FLOW, "%s, volt=%d, ret=%d\n", __func__, volt, ret);
			break;
		}
	}
	return ret;
...
static void set_low_cal_state(struct bcmpmu_em *pem)
{
	return;
	low_cal_fact_map[pem->low_cal_index].state = 1;
}

static void clear_low_cal_state_all(struct bcmpmu_em *pem)
{
	int i;
	for (i = 0; i <ARRAY_SIZE(low_cal_fact_map); i++)
		low_cal_fact_map[i].state = 0;
	for (i = 0; i <ARRAY_SIZE(cutoff_cal_map); i++)
...
static int get_low_bat_cal_factor(struct bcmpmu_em *pem)
{
	int delta_factor = 0;
	int delta;
	int i;
	...
	vcap = em_batt_get_capacity(pem,
				pem->avg_oc_volt, 0);
	pem->oc_cap = vcap;
	pr_em(FLOW, "%s, cap=%d, oc_cap=%d, avg_ocv=%d, avg_curr=%d\n",
		__func__, pem->batt_capacity, pem->oc_cap,
		pem->avg_oc_volt, pem->avg_curr);

	if (is_charger_present(pem))
		return 0;
	delta = vcap - pem->batt_capacity;
	if (delta == 0)
		return 0;
	if (pem->oc_cap > 10)
		return 0;
	if (pem->avg_oc_volt > 3550)
		return 0;

	for (i = 0; i < ARRAY_SIZE(low_cal_fact_map); i++) {
		if (vcap == low_cal_fact_map[i].volt) {
			if (delta < 0)
				delta_factor = low_cal_fact_map[i].fact_dn;
			else if (delta > 0)
				delta_factor = low_cal_fact_map[i].fact_up;
			break;
		}
	}
	pem->low_cal_factor = delta_factor;
	return delta_factor;
}
...
static int pre_cutoff_check(struct bcmpmu_em *pem)
{
	...
	if (pem->batt_volt > LOW_BATT_VOLT) {
		pem->cutoff_chk_cnt = 0;
		return capacity;
	}
	...
	if (pem->cutoff_chk_cnt > pem->cutoff_count_max) {
		pem->cutoff_chk_cnt = 0;
		capacity = cutoff_cal_map[i].fact;
...
static int update_batt_capacity(struct bcmpmu_em *pem, int *cap)
{
	...
	if (first_time == 0) {
		...
		pem->avg_curr = (pem->avg_curr * (AVG_LENGTH - 1))/AVG_LENGTH +
		...
		pem->avg_oc_volt = (pem->avg_oc_volt *
			(AVG_LENGTH - 1))/AVG_LENGTH + volt / AVG_LENGTH;
	}

	if (pem->support_fg == 0) {
		capacity = em_batt_get_capacity(pem, volt, 0);
	} else {
...
*/
		...
		if (calibration == 0) {
			...
			if (pem->capacity_cutoff >= 0) {
				pem->cutoff_delta = capacity - pem->capacity_cutoff;
				if (pem->cutoff_delta > 0) {
					pem->cutoff_update_step = 3 + (pem->capacity_cutoff * 6)/ pem->cutoff_delta;
					if (pem->cutoff_update_step > 12)
					pem->cutoff_update_step = 12;
				}
				pem->cap_delta_now = 0;
			}
			if ((pem->capacity_cutoff >= 0) &&
				...
				if (pem->cutoff_update_cnt > pem->cutoff_update_step) {
					capacity--;
					pem->cutoff_delta--;
			...
			pr_em(FLOW, "%s, cutoff check, step=%d, delta=%d, cnt=%d\n",
				__func__, pem->cutoff_update_step,
				pem->cutoff_delta, pem->cutoff_update_cnt);
		}
		pr_em(FLOW, "%s, calm=%d, lst_calm=%d, retry=%d, tcm=%d\n",
			__func__, pem->cal_mode,
			pem->last_cal_mode, pem->retry_cnt, pem->fg_comp_mode);
	}
...
*/
				pem->cap_delta_now = capacity -
					pem->batt_capacity;
				set_low_cal_state(pem);
				pr_em(FLOW, "%s, Low Volt cal, delta=%d.\n",
					__func__, pem->cap_delta_now);
	...
	if ((!is_charger_present(pem)) &&
		...
		if (capacity >= pem->batt_capacity)
			...
			if (pem->cap_delta_now > 0) {
				capacity = pem->batt_capacity;
				pem->cap_delta_now--;
			} else if (pem->cap_delta_now < 0) {
				if (pem->batt_capacity > 2)
					capacity = pem->batt_capacity - 2;
				else
			...
			pem->fg_capacity  = pem->fg_capacity_full * capacity;
			pem->fg_capacity = div_s64(pem->fg_capacity, 100);
			pr_em(FLOW, "%s, tuning, cap_n=%d, cap_o=%d, dlt=%d\n",
		...
		unsigned long event, void *para)
{
	...
	case BCMPMU_CHRGR_EVENT_CHRG_CURR_LMT:
		pem->chrgr_curr =
		  ((*(int *)para) < PMU_CHRGING_CURR_MIN) ? 0 : *(int *)para;
		pem->force_update = 1;
