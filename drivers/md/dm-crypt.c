...
static int crypt_decode_key(u8 *key, char *hex, unsigned int size)
{
	...
	{
		int ret = 0;
		char* t_key = kzalloc(size,GFP_KERNEL);

		ret = get_encrypted_key(hex, t_key, size);
		if ( ret == KEY_FOUND_SUCCESS) {
			get_derived_key(t_key, size, key);
			kfree(t_key);
			return 0;
		}
		if ( ret == MANDATORY_KEY_NOT_FOUND) {
			kfree(t_key);
			return -EINVAL;
		}
	}
