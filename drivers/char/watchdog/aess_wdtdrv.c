/*
 * $RCSfile: aess_wdtdrv.c,v $
 * $Revision: 1.2 $
 * $Date: 2008/09/19 08:55:40 $
 * $Author: sonata $
 *
...
int aess_wdt_keepalive(struct file *file)
{
//	struct vwdt_private *lp = file->private_data;
	struct vwdt_private *lp = NULL;
	int wdt_base = 0 ;
	...
	//test_and_set_bit(WDT_RESET_BIT0, (void *)wdt_base);
	set_bit(WDT_RESET_BIT0, (void *)wdt_base);
	wdt_count++;
...
static int __init aess_wdt_init(void)
{
	...
	for(i=0 ; i<VWDT_MAX_DEVS ; i++) {
		...
		switch (i) {
			case 0:
				lp->base_addr = (int)WDT_REG_CONTROL_ADDR;
				lp->cstat = WDT_CLOSE_NOT_ALLOW;
