...
#if defined(CONFIG_MACH_CAPRI_SS)
			 ...
			 size_t count, loff_t *f_pos)
{
	...
	snprintf(status_buf, sizeof( status_buf)-1, "%d\n", (ch->state == HDMI_CONNECTED ? 1 : 0));
			  ...
			  int *eof, void *data)
{
	...
	sprintf(buf, "%s: %lu\n",
		((ch->state ==
		  2) ? HDMI_WIFIHDMI_STR : ((ch->state ==
					     1) ? HDMI_CONNECTED_STR :
					    HDMI_UNPLUGGED_STR)),
		ch->changed_at);
	*eof = 1;
	return strlen(buf);
...
static int __devinit hdmi_pltfm_probe(struct platform_device *pdev)
{
	...
	if (ch->hw_cfg.reg_hdmi_blk[0]) {
		...
		if (IS_ERR_OR_NULL(hdmi_blk_regulator)) {
			...
			printk(KERN_INFO
			       "hdmi-detection: enabling regulator %s",
			       ch->hw_cfg.reg_hdmi_blk);
