   ...
   if ( !err )
   {
      ...
      for ( i = 0, id = 0; i < hwinfo.interfaces && id < HALAUDIO_MAX_INTERFACES; i++ )
      {
         ...
         do
         {
            ...
            if ( !err )
            {
               len += sprintf( buf+len, " %s: id=%i codecs=%i frame_period=%i enabled=%i sync=%i sync_err=%i\n",
                     ifinfo.name, id, ifinfo.codecs, ifinfo.frame_period_us, ifinfo.enabled, ifinfo.sync, ifinfo.sync_err );
            }
            id++;
         }
         while ( err );
