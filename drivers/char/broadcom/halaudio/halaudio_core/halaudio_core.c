...
struct halaudio_if_node
{
   ...
   unsigned int               sync_err;         /* Synchronization error count */
...
static DEFINE_SPINLOCK( gInterfLock );
...
)
{
   ...
   info->sync_err          = interfp->sync_err;
...
static void halaudio_frame_elapsed( void *data )
{
   ...
   spin_lock_irqsave( &gInterfLock, flags );
   ...
   ||   interfp->enabled == HALAUDIO_INTERFACE_DISABLED )
   {
       ...
       */
      spin_unlock_irqrestore( &gInterfLock, flags );
      return;
   }
   ...
   if ( interfp->rollcall > 1 )
   {
       ...
       */
      HALAUDIO_LOG( "Looks like %s is triggering too fast", interfp->name );
      interfp->sync_err++;
   }
