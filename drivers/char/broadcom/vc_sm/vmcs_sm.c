...
static struct SM_RESOURCE_T *vmcs_sm_acquire_resource(
	struct SM_PRIV_DATA_T *private, unsigned int res_guid)
{
...
** The resource won't be deleted while there is a reference on it.
*/
static struct SM_RESOURCE_T *vmcs_sm_acquire_global_resource(
...
static void vmcs_sm_release_resource(struct SM_RESOURCE_T *resource, int force)
{
	 ...
	 */
	list_for_each_entry(res_tmp, &private->resource_list,
			    global_resource_list) {
		      ...
		      struct vmcs_sm_ioctl_alloc *ioparam)
{
	...
	alloc.type =
	    ((ioparam->cached == VMCS_SM_CACHE_VC)
	     || (ioparam->cached ==
...
{
	struct SM_RESOURCE_T *resource = NULL, *shared_resource;
	int ret = 0;
	...
	case VMCS_SM_CMD_MAPPED_USR_ADDRESS:
		{
			 ...
			 */
			ioparam.addr =
			    vmcs_sm_usr_address_from_pid_and_usr_handle(ioparam.
									pid,
									ioparam.
									handle);
