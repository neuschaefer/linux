...
static int vc_vchi_sm_videocore_io(void *arg)
{
	...
	int timeout = CONFIG_DEFAULT_HUNG_TASK_TIMEOUT ?
		((CONFIG_DEFAULT_HUNG_TASK_TIMEOUT/2 > 0) ?
		CONFIG_DEFAULT_HUNG_TASK_TIMEOUT/2 : 1) : 0;
#else
	/* CONFIG_DEFAULT_HUNG_TASK_TIMEOUT is defined only when kernel debug
         * is enabled. So taking 120 as CONFIG_DEFAULT_HUNG_TASK_TIMEOUT in 
         * non-debug mode
         */
	int timeout = 60;
#endif
	LOG_INFO("%s: explicit timeout %u jiffies",
		__func__, timeout * HZ);
	...
	while (1) {
		...
		svc_use = 0;
		if (!down_timeout(&instance->io_sema,
				timeout ?
				HZ * timeout : MAX_SCHEDULE_TIMEOUT)) {
		...
		do {
			...
			/* Send the command */
			status = vchi_msg_queue(instance->vchi_handle[0],
						cmd->msg, cmd->length,
						VCHI_FLAGS_BLOCK_UNTIL_QUEUED,
						NULL);
			if (status) {
				LOG_ERR("%s: failed to queue message (%d)",
					__func__, status);
			}
		...
		while (!vchi_msg_peek(instance->vchi_handle[0], (void **)&reply,
			...
			mutex_lock(&instance->lock);
			list_for_each_entry(cmd, &instance->rsp_list, head) {
			...
			if (&cmd->head == &instance->rsp_list) {
				...
				LOG_ERR
				    ("%s: reply too big (%u) %u," \
					  " throw away...",
				     __func__, reply_len, reply->trans_id);
			} else {
