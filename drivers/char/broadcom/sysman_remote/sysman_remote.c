...
static VCHIQ_STATUS_T change_regulator_state(
	struct remote_sysman_state *state, const char *name, uint32_t enable)
{
...
{
	VCHIQ_STATUS_T status = VCHIQ_SUCCESS;
	struct remote_sysman_state *state = service->userdata;
	...
	case VCHIQ_MESSAGE_AVAILABLE:
		...
		if (state && state->p_dev &&
			...
			status = process_message(vchiq_header, service, state);
		}
...
static void vc_sysman_remote_connected_init(void)
{
	...
	ret = vchiq_open_service(
			remote_sm_stt->initialise_instance,
...
static int vc_sysman_proc_read(
	char *buf,
	char **start,
	off_t offset,
	int count,
...
static int vc_sysman_proc_write(
	struct file *file,
...
late_initcall(vc_sysman_remote_init);
