...
#define DEFINE_SYSCTL_ICP(id)   { \
   ...
   { \
      .procname   = "updateIntervalMsec", \
      .data       = &obj[id].state.updateIntervalMsec, \
      .maxlen     = sizeof( int ), \
      .mode       = 0644, \
      .proc_handler  = &proc_dointvec \
   }, \
           ...
           void __user *buffer, size_t *lenp, loff_t *ppos )
{
   ...
   if ( write )
   {
      ...
      statep->alphaFactor = (1<<5);   /* 1/8 in Q8 number */
   }
   else
   {
...
int hostIcpThread(void *data)
{
   ...
   while( 1 )
   {
      ...
      {
		memmove(&statsp->countHistory[1], &statsp->countHistory[0],
      ...
      else if (datap->total < (0xFFFFFFFF / 1000))
      {
      ...
      /* y(i) = avgCount, x(i) = currCount */
      /* First order filter: y(i) = alpha*x(i) + (1 - alpha)y(i-1) */
      /* Adjusting for Q8 alpha factor: y(i) = (alpha*x(i) + 256*y(i-1) - alpha*y(i-1)) / 256 */
      statsp->avgCount = statsp->currCount*statep->alphaFactor 
                       + (statsp->avgCount * (1 << 8)) 
                       - statsp->avgCount*statep->alphaFactor;
      statsp->avgCount >>= 8;
