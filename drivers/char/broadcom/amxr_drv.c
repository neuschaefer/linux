...
union amxr_ioctl_params
{
   ...
   struct amxr_ioctl_get_portlist          getportlist;
};
...
)
{
   if ( gApi.getPortList )
   {
      return gApi.getPortList( hdl, portlist, maxlen );
   }
   return -EPERM;
}
EXPORT_SYMBOL( amxrGetPortList );
...
)
{
   ...
   switch ( cmdnr )
   {
      ...
      break;
      case AMXR_CMD_GET_PORTLIST:
      {
      	void *memp;
      	memp = vmalloc( parm.getportlist.maxlen);
      	if ( memp == NULL )
      	{
      		return -ENOMEM;
      	}
      	rc = amxrGetPortList(client_hdl, memp, parm.getportlist.maxlen);
      	if (rc == 0)
      	{
      		rc = copy_to_user( parm.getportlist.portlist, memp, parm.getportlist.maxlen);
      	}
      }
      break;
      default:
...
)
{
   ...
   {
      return gApi.setCsxIoPoints( pid, point, fncp, data );
   }
   return -EPERM;
}
EXPORT_SYMBOL( amxrSetCsxIoPoints );
...
EXPORT_SYMBOL( amxrSetCsxFrameSync );
