	...
	if (mm_vc_mem_size == 0) {
		...
		if ((wakeup_register & ~1) == 0) {
			...
			if (vc_mem_access_mem
			    (0, &vc_dbg,
			     mm_vc_mem_base + VC_DEBUG_HEADER_OFFSET,
			     sizeof(vc_dbg)) == 0) {
...
static int __init vc_mem_init(void)
{
	...
	if (vc_dt_get_load_and_size(&base, &size) == 0) {
		mm_vc_mem_phys_addr = (unsigned long)(base & 0xC0000000);
		mm_vc_mem_base      = (unsigned int)base;
		mm_vc_mem_size      = size;
		printk(KERN_INFO "vc-mem: phys = 0x%08lx, base = 0x%08x, size = 0x%08x\n",
				 mm_vc_mem_phys_addr, base, size);
	} else {
		/* fallback to default VC EMI, Capri <= A1 */
		mm_vc_mem_phys_addr = 0x40000000;
		mm_vc_mem_base = (unsigned int)mm_vc_mem_phys_addr;
		printk(KERN_WARNING "vc-mem: legacy-base = 0x%08x, update your dt-blob!\n",
				 mm_vc_mem_base);
	}
