...
static uint32_t           g_wrt_right_ch_set;
                         ...
                         sizeof ( g_hal_device ) ) == 0 )
         {
            ...
            */
            if ( g_hal_wrt_device.name[0] != 0 )
            {
               ...
               */
               if ( g_wrt_right_ch_set )
               {
                  amxrDisconnectByName ( g_amxr_client,
                                         AAA_PORT_NAME,
                                         "halaudio.audioh4" );
                  g_wrt_right_ch_set = 0;
               }
            }
#endif /* defined(AAA_RUNS_AT_44_1_KHz) */
            ...
            */
            ...
            else
            {
               ...
               /* Special case (on Island) if a single audio channel is used and
               ** the device is handsfree, setup the right speaker (handsfree)
               ** channel to go through "audioh4".
               */
               g_wrt_right_ch_set =
                  ( strcmp( g_hal_device.name, "handsfree-spkr" ) == 0 );
...
/***************************************************************************/
/**
*  'WFD' (WiFi Display) loopback method to be invoked from third party
*  to manage a connection between output and input ports.
*
*  @return
*     >= 0           Success
*     -ve            Error code
*/
int aaa_wfd_loopback( int loopback )
{
   int rc = -EINVAL;

   /* TODO: make the rest of the AAA driver aware of the loopback state such that
    *       connection are not changed on us.
    */

   if( loopback )
   {
      rc = amxrDisconnectByName( g_amxr_client,
                                 AAA_PORT_NAME,
                                 g_hal_wrt_device.info.mport_name );
      if( rc )
         printk( KERN_INFO "%s: amxr disconnect failed \'%s\' and \'%s\'\n",
                 __FUNCTION__, AAA_PORT_NAME, g_hal_wrt_device.info.mport_name );
      ...
      if( rc )
         printk( KERN_INFO "%s: amxr disconnect failed \'%s\' and \'%s\'\n",
                 __FUNCTION__, g_hal_rd_device.info.mport_name, AAA_PORT_NAME );

      rc = amxrSetPortDstChannels( g_aaa_port,
                                   2 * AAA_PORT_DST_CHAN,
                                   2 * AAA_PORT_DST_CHAN * AAA_PORT_DST_FRAME );
      if( rc )
         printk( KERN_ERR "%s: amxr setting stereo dst failed", __FUNCTION__ );

      rc = amxrConnectByName( g_amxr_client,
                              AAA_PORT_NAME,
                              AAA_PORT_NAME,
                              AMXR_CREATE_DESC( AMXR_CONNECT_EXTRACT_AND_DEPOSIT, 2, 0, 2, 0 ) );
      if( rc )
         printk( KERN_ERR "%s: amxr connect loopback (left) failed \'%s\' and \'%s\'\n",
                 __FUNCTION__, AAA_PORT_NAME, AAA_PORT_NAME );

      rc = amxrConnectByName( g_amxr_client,
                              AAA_PORT_NAME,
                              AAA_PORT_NAME,
                              AMXR_CREATE_DESC( AMXR_CONNECT_EXTRACT_AND_DEPOSIT, 2, 1, 2, 1 ) );
      if( rc )
         printk( KERN_ERR "%s: amxr connect loopback (right) failed \'%s\' and \'%s\'\n",
                 __FUNCTION__, AAA_PORT_NAME, AAA_PORT_NAME );
   }
   else
   {
      ...
      rc = amxrConnectByName( g_amxr_client,
                              ...
                              AMXR_CONNECT_STEREO2STEREO );
      if( rc )
         printk( KERN_ERR "%s: amxr connect restore failed \'%s\' and \'%s\'\n",
                 __FUNCTION__, AAA_PORT_NAME, g_hal_wrt_device.info.mport_name );
   }

   return rc;
}
EXPORT_SYMBOL_GPL(aaa_wfd_loopback);
