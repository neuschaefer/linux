...
struct amxr_port_node
{
   ...
   /* CSX data */
   struct amxr_csx_data csx_data[AMXR_NUM_CSX_PORT_POINTS]; /* Array of CSX data structures */
...
/**
*  csx framesync callback
*/
struct amxr_csx_framesync
{
...
static spinlock_t                 gCnxLock;
static spinlock_t                 gPortLock;
...
static struct amxr_csx_framesync gCsxFrameSync;
...
#if 0
   /* Comment out this check for the time being to allow connection with same channel number
    * involving resampling to be accepted.
    *
    * Note that if the 'FIXME' below was implemented, commenting out this check would not be
    * necessary.
    */
   else
   {
...
*     -ve         On general failure
*/
int amxrCoreGetPortList(
   AMXR_HDL             hdl,        /*<< (i) Mixer client handle */
   AMXR_PORTS          *portlistp,   /*<< (o) Ptr to store port list */
   int                  maxlen      /*<< (i) Max length in bytes */
)
{
	struct amxr_port_node  *portlp;
	int err = 0;
	int mem_required, ports;
	AMXR_PORT_EXTINFO *portinfop;

	mutex_lock( &gPorts.mutex );

	mem_required = sizeof(AMXR_PORTS) + sizeof(AMXR_PORT_EXTINFO) * gPorts.total;
	ports = gPorts.total;
	...
	portlistp->ports=ports;
	if ( portlistp->ports )
	{
		portinfop=&portlistp->list[0];
		llist_foreach_item( portlp, &gPorts.list, lnode )
		{
			portinfop->dst_cnxs = portlp->dst_cnxs;
			portinfop->src_cnxs = portlp->src_cnxs;
			portinfop->port = getId(portlp);
			memcpy(&portinfop->info,&portlp->info,sizeof(AMXR_PORT_INFO));
			portinfop++;
		}
	}
...
)
{
   ...
   spin_lock_irqsave( &gPortLock, state );
   llist_add_tail( &portp->lnode, &gPorts.list );
   gPorts.total++;
   spin_unlock_irqrestore( &gPortLock, state );
   ...
   if ( changed && portp->src_cnxs )
   {
      ...
      llist_foreach_item_safe( cnxlp, tmpcnxlp, &gCnxs.list, lnode )
      {
         if ( cnxlp->srcportp == portp )
         {
            ...
            if ( supported )
            {
               spin_lock_irqsave( &gCnxLock, state );
               cnxlp->cnx_type   = cnx_type;
               cnxlp->src_chans  = src_chans;
               spin_unlock_irqrestore( &gCnxLock, state );
...
)
{
   ...
   spin_lock_irqsave( &gPortLock, port_state );
   llist_foreach_item( portp, &gPorts.list, lnode )
   {
      if ( portp->src_cnxs && AMXR_SYNC_FREQ( portp->info.src_hz ))
      {
         portp->srcp = portp->cb.getsrc( portp->info.src_bytes, portp->privdata );
         /* csx per-port pre-mix tap */
         if ( portp->csx_data[AMXR_CSX_POINT_SYNCPORT_SRC_PREMIX].csx_ops.csxCallback )
			{
				portp->csx_data[AMXR_CSX_POINT_SYNCPORT_SRC_PREMIX].csx_ops.csxCallback(
						(void*)(portp->srcp), portp->info.src_bytes,
						portp->csx_data[AMXR_CSX_POINT_SYNCPORT_SRC_PREMIX].priv );
			}
      }
      if ( portp->dst_cnxs && AMXR_SYNC_FREQ( portp->info.dst_hz ))
      {
         ...
         /* csx per-port pre-mix tap */
         if ( portp->csx_data[AMXR_CSX_POINT_SYNCPORT_DST_PREMIX].csx_ops.csxCallback )
			{
				portp->csx_data[AMXR_CSX_POINT_SYNCPORT_DST_PREMIX].csx_ops.csxCallback(
						(void*)(portp->dstp), portp->info.dst_bytes,
						portp->csx_data[AMXR_CSX_POINT_SYNCPORT_DST_PREMIX].priv );
			}
   ...
   spin_lock_irqsave( &gCnxLock, cnx_state );
   /* Service connections */
   llist_foreach_item( cnxlp, &gCnxs.list, lnode )
   {
   ...
   spin_unlock_irqrestore( &gCnxLock, cnx_state );

   /* Issue done callbacks */
   llist_foreach_item( portp, &gPorts.list, lnode )
   {
      /* csx per-port post-mix tap */
      if (portp->src_cnxs && AMXR_SYNC_FREQ( portp->info.src_hz )
				&& portp->csx_data[AMXR_CSX_POINT_SYNCPORT_SRC_POSTMIX].csx_ops.csxCallback)
		{
			portp->csx_data[AMXR_CSX_POINT_SYNCPORT_SRC_POSTMIX].csx_ops.csxCallback(
					(void*)(portp->srcp), portp->info.src_bytes,
					portp->csx_data[AMXR_CSX_POINT_SYNCPORT_SRC_POSTMIX].priv );
		}

      if (portp->dst_cnxs && AMXR_SYNC_FREQ( portp->info.dst_hz )
				&& portp->csx_data[AMXR_CSX_POINT_SYNCPORT_DST_POSTMIX].csx_ops.csxCallback)
		{
			portp->csx_data[AMXR_CSX_POINT_SYNCPORT_DST_POSTMIX].csx_ops.csxCallback(
					(void*)(portp->dstp), portp->info.dst_bytes,
					portp->csx_data[AMXR_CSX_POINT_SYNCPORT_DST_POSTMIX].priv );
		}
   ...
   spin_unlock_irqrestore( &gPortLock, port_state );
}
...
)
{
   ...
   /* csx per-port pre-mix tap */
   if ( portp->csx_data[AMXR_CSX_POINT_UNSYNCPORT_SRC_PREMIX].csx_ops.csxCallback )
	{
		portp->csx_data[AMXR_CSX_POINT_UNSYNCPORT_SRC_PREMIX].csx_ops.csxCallback(
				(void*)portp->srcp, portp->info.src_bytes,
				portp->csx_data[AMXR_CSX_POINT_UNSYNCPORT_SRC_PREMIX].priv );
	}
   ...
   /* csx per-port post-mix tap */
   if ( portp->csx_data[AMXR_CSX_POINT_UNSYNCPORT_DST_POSTMIX].csx_ops.csxCallback )
	{
		portp->csx_data[AMXR_CSX_POINT_UNSYNCPORT_DST_POSTMIX].csx_ops.csxCallback(
				(void*)portp->dstp, portp->info.dst_bytes,
				portp->csx_data[AMXR_CSX_POINT_UNSYNCPORT_DST_POSTMIX].priv );
	}
...
static int __init amxr_init( void )
{
   ...
   spin_lock_init( &gCnxLock );
   spin_lock_init( &gPortLock );
   ...
   apifuncs.getPortList          = amxrCoreGetPortList;
   ...
   apifuncs.setCsxIoPoints       = amxrCoreSetCsxIoPoints;
   apifuncs.setCsxFrameSync      = amxrCoreSetCsxFrameSync;
...
*/
int amxrCoreSetCsxIoPoints(
		AMXR_PORT_ID pid, 					/**< (i) Port to install CSX callbacks */
		AMXR_CSX_PORT_POINT_ID point,    /**< (i) Point ID for CSX callbacks */
...
)
{
   unsigned long      flags;
	int err = -EPERM;
	struct amxr_port_node* portp = getPort( pid );
	...
	if ( point >= AMXR_NUM_CSX_PORT_POINTS ){
		return -EINVAL;
	}
	mutex_lock( &gPorts.mutex );

	/* Enable CSX I/O point */
	local_irq_save( flags );
	memcpy(&portp->csx_data[point].csx_ops, fncp,
			sizeof(portp->csx_data[point].csx_ops));
	portp->csx_data[point].priv = data;
	err = 0;
