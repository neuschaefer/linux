...
struct ppxy_src_buf *alloc_src_buf( struct ppxy_node *nodep, int bytes )
{
	...
	for ( i = 0; i < NUM_SRC_BUFS; i++, srcbufp++ )
	{
		if ( !atomic_read( &srcbufp->alloc ))
		{
			...
			srcbufp->offset = 0;
			newsrcbufp = srcbufp;
			PPXY_TRACE( "alloc buff %i bytes=%i", srcbufp->idx, bytes );
			break;
		}
	}
...
static int realloc_data_buf( struct ppxy_buf *bp, int new_bytes )
{
	if ( bp->frame_bytes < new_bytes )
	{
		...
		new_bufp = kzalloc( size, GFP_KERNEL );
		if ( new_bufp == NULL )
		{
			return -ENOMEM;
		}
...
)
{
	...
	nodep = kzalloc( (sizeof(*nodep)+PAGE_SIZE-1) & PAGE_MASK, GFP_KERNEL );
	if ( nodep == NULL )
	{
		return -ENOMEM;
	}
	...
	switch ( cmdnr )
	{
		case AMXR_PORT_CMD_CREATE_PORT:
		{
			...
			if ( rc == 0 )
			{
				...
				rc = copy_to_user( parmp->portidp, &portid,
						sizeof(portid) );
			}
		}
		break;
...
{
	int rc;
	struct ppxy_node *nodep = getNodep( filp->private_data );
		...
		const char __user *buffer, size_t count, loff_t *ppos )
{
	...
	if ( srcbufp == NULL )
	{
		PPXY_TRACE( "out of mem count=%i", count );
		return -ENOMEM;
	}
		...
		int count, int *eof, void *data )
{
	...
	list_for_each_entry( nodep, &gPortList.list, lnode )
	{
		...
		{
			if ( atomic_read( &nodep->srcbufs[i].alloc ))
			{
				alloc++;
			}
		}
		len += sprintf( buf+len, "src(idx=%i bytes=%i buf_alloc=%i) ",
				nodep->src.bufidx,
				nodep->src.frame_bytes, alloc );
		len += sprintf( buf+len, "dst(idx=%i bytes=%i) ",
				nodep->dst.bufidx, nodep->dst.frame_bytes );
		len += sprintf( buf+len, "msgs(readi=%i writei=%i num=%i errs=%i)\n\n",
