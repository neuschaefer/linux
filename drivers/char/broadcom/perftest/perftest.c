...
#define KTHREAD_MODEL                   1       // Use kthread_xxx() functions. kernel_thread() is low_level one, function exporting would be removed later.
...
typedef struct
{
    PERFCMD_OBJ perfcmd;
#if (KTHREAD_MODEL == 1)
    struct task_struct *perfcmdThread_task;
#else
...
//    volatile int nop_pid;               // Alamy (fix): We need this variable to be voltile or compiler optimized the loop
    struct task_struct *noploop_task;
#if (ENABLE_SRAM_TEST == 1)
    struct task_struct *sramtest_task;
#endif
}
PERFTEST_OBJ;
...
static int proc_do_perftest_intvec_memcpy(ctl_table *table, int write, struct file *filp,
           void __user *buffer, size_t *lenp, loff_t *ppos );
static int proc_do_perftest_intvec_memcpy_armlib(ctl_table *table, int write, struct file *filp,
           void __user *buffer, size_t *lenp, loff_t *ppos );
...
static int proc_do_perftest_intvec_noploop(ctl_table *table, int write, struct file *filp,
           void __user *buffer, size_t *lenp, loff_t *ppos );
static int proc_do_perftest_intvec_sramtest(ctl_table *table, int write, struct file *filp,
           void __user *buffer, size_t *lenp, loff_t *ppos );
static int proc_do_perftest_intvec_cpu(ctl_table *table, int write, struct file *filp,
           void __user *buffer, size_t *lenp, loff_t *ppos );
static int proc_do_perftest_intvec_quiet(ctl_table *table, int write, struct file *filp,
           void __user *buffer, size_t *lenp, loff_t *ppos );
static int proc_do_perftest_intvec_verify(ctl_table *table, int write, struct file *filp,
           void __user *buffer, size_t *lenp, loff_t *ppos );
#endif
...
static int sdma_test_set( void )
{
    ...
    if (( rc = sdma_set_device_handler( DMA_DEVICE_MEM_TO_MEM, sdmaHandler, NULL )) != 0 )
    {
...
//   tmrStart = chal_gptimer_get_timer_current_tick(timer_handle, &lsw, &msw);
    ...
    {
        /* Start DMA operation */
        sdma_transfer_mem_to_mem(dmaHandle, src.physPtr, dst.physPtr, alloc_size );
        ...
        if (perfflag.verify)
        {
            ...
            {
                printk( KERN_ERR ": ========== SDMA memcpy test failed [%03d] ==========\n\n", i );
                goto cleanup;           // Alamy: Don't want memory-leakage
            }
        }
    }
...

{
    ...
    if (( rc = dma_transfer_mem_to_mem( gDmaHandle, src->physPtr, dst->physPtr, src->numBytes )) != 0 )
    {
...
//    uint32_t lsw, msw;
    ...
    if (( rc = dma_set_device_handler( DMA_DEVICE_MEM_TO_MEM_32, dmaHandler, NULL )) != 0 )
    {
        printk( KERN_ERR "dma_set_device_handler failed: %d\n", rc );
        goto cleanup;                   // Alamy: Don't want memory-leakage
    }
...
//    tmrStart = chal_gptimer_get_timer_current_tick(timer_handle, &lsw, &msw);
    ...
    for (i=0; i<iterations; i++)
    {
        ...
        if (perfflag.verify)
        {
            if (( rc = dma_check_mem( &dst )) != 0 )
            {
                printk( KERN_ERR ": ========== DMA memcpy test failed [%03d] ==========\n\n", i );
                goto cleanup;           // Alamy: Don't want memory-leakage
            }
        }
    }
...
static void memcpy_armlib_perf_test(size_t n, uint32_t iterations, int verbose, char *banner)
{
    uint32_t iter;
    char modbanner[80];
    char *bufp;
...
//    uint32_t lsw, msw;
    ...
    if (!perfflag.quiet)
    {
        printk("%s: vmalloc, arm library memcpy, size=%u iterations=%u\n", __func__, n, iterations);
    }
    bufp = vmalloc(2*n);
    if (!bufp)
    {
...
//    tmrStart = chal_gptimer_get_timer_current_tick(timer_handle, &lsw, &msw);
    ...
    if (perfflag.verify)
    {
        if (memcmp(da, sa, n))
        {
            printk( KERN_ERR ": ========== armlib memcpy test failed [%03d] ==========\n\n", iter );
            return;
        }
    }
//    tmrEnd = chal_gptimer_get_timer_current_tick(timer_handle, &lsw, &msw);
...
*/
/****************************************************************************/
static void memcpy_perf_test(size_t n, uint32_t iterations, int verbose, char *banner)
{
    uint32_t iter;
    int i = 0;
    char modbanner[80];
...
static int proc_do_perftest_intvec_sdma_memcpy(ctl_table *table, int write,
           void __user *buffer, size_t *lenp, loff_t *ppos )
#else
static int proc_do_perftest_intvec_sdma_memcpy(ctl_table *table, int write, struct file *filp,
           void __user *buffer, size_t *lenp, loff_t *ppos )
#endif
{
    int rc = 0;
...
static int proc_do_perftest_intvec_dma_memcpy(ctl_table *table, int write,
           void __user *buffer, size_t *lenp, loff_t *ppos )
#else
static int proc_do_perftest_intvec_dma_memcpy(ctl_table *table, int write, struct file *filp,
           void __user *buffer, size_t *lenp, loff_t *ppos )
#endif
{
    int rc = 0;
...
/****************************************************************************/
/**
*  @brief   low priority noploop_kthread to avoid CPU sleeping and stress i-cache
*
*  @return
*/
/****************************************************************************/
static int noploop_kthread(void *data)
{
    int cpu_enum = 0;
    int rc;
    ...
    // Find out which CPU this thread belongs to
    for (cpu_enum = 0; cpu_enum < num_online_cpus(); cpu_enum++)
    {
        if (data == &(perftest[cpu_enum]))
        {
            break;
        }
    }
    ...
    {
        printk(KERN_ERR "noploop_kthread: bad cpu number %d\n", cpu_enum);
        return -1;
    }
    ...
    */
    ...
    if (rc)
    {
        printk(KERN_ERR "noploop_kthread sched_setscheduler failed, rc=%d\n", rc);
        return rc;
    }

    printk("noploop_kthread (PID %d) starting on CPU %d, 'echo 0 > noploop' to stop...\n"
        , task_pid_nr(perftest[cpu_enum].noploop_task)
        , cpu_enum
    );
    ...
    if (cpu_enum != cpuid)
    {
        printk(KERN_ERR "noploop test running on wrong CPU. CPU %d instead of CPU %d\n", cpuid, cpu_enum);
    }
    else
    {
        printk(KERN_INFO "noploop test exiting on CPU %d\n", cpu_enum);
    }
...
#endif
{
    ...
    if ( write )
    {
         ...
         *      perftest[perfflag.cpu_nr] is safe because perfflag.cpu_nr is guaranteed in the safe range.
         *
        ...
        {
            if (perftest[cpu_enum].noploop_task == NULL)
            {
                /*
                 * The @data field is a pointer
                 * 1) We can not pass an address of variable in this function, when the created thread
                 *    is created and scheduled to run later, this function is already finished,
                 *    and the variable (in the stack) is invalid.
                 * 2) We can not pass an point to variable which the data may be changed later.
                 *    There is a latency when the thread is created and scheduled, and the thread
                 *    may fetch wrong data if it is already changed.
                 *    (exp: &perftest.cpu_nr)
                 *
                 * Pass the perftest[cpu_enum] address so that the thread could detect which cpu
                 * it belongs to.
                 */
                perftest[cpu_enum].noploop_task = kthread_create_on_node(noploop_kthread \
                        , &(perftest[cpu_enum]) \
                        , cpu_to_node(cpu_enum) \
                        , "noploop/%d", cpu_enum);

                if (IS_ERR(perftest[cpu_enum].noploop_task))
                {
                    printk(KERN_WARNING "noploop: Error creating thread for CPU %d\n", cpu_enum);
                }
                else
                {
                    kthread_bind( perftest[cpu_enum].noploop_task, cpu_enum );
                    wake_up_process( perftest[cpu_enum].noploop_task );
                    printk(KERN_INFO "noploop_kthread created on CPU %d\n", cpu_enum);
                }
            }
            else
            {
                printk(KERN_ERR "noploop test (PID %d) already running on CPU %d\n"
                    , task_pid_nr( perftest[cpu_enum].noploop_task )
                    , cpu_enum);
            }
        }
        ...
        {   // Stop the thread
            if ( perftest[cpu_enum].noploop_task )
            {
                printk(KERN_INFO "Stopping noploop_kthread (PID %d) on CPU %d ...\n"
                    , task_pid_nr(perftest[cpu_enum].noploop_task)
                    , cpu_enum);
                kthread_stop( perftest[cpu_enum].noploop_task );
                perftest[cpu_enum].noploop_task = NULL;
            }
            else
            {
                printk(KERN_ERR "no noploop test running on CPU %d\n", cpu_enum);
            }
        }
    }
...
static int sramtest_kthread(void *data)
{
    int cpu_enum;
    int rc;
    struct sched_param param = { .sched_priority = 0 };
    ...
    if (cpu_enum >= num_online_cpus())
    {
        printk(KERN_ERR "sramtest_kthread: bad cpu number %d\n", cpu_enum);
        return -1;
    }
    ...
    printk(KERN_INFO "sramtest_kthread (PID %d) starting on CPU %d, 'echo 0 > sramtest' to stop...\n"
        , task_pid_nr( perftest[cpu_enum].sramtest_task )
        , cpu_enum
    );
    ...
    if (cpu_enum != cpuid)
    {
        printk(KERN_ERR "sramtest running on wrong CPU. CPU %d instead of CPU %d\n", cpuid, cpu_enum);
    }
    else
    {
...
#endif
{
    ...
    if ( write )
    {
        ...
        {
            if (perftest[cpu_enum].sramtest_task == NULL)
            {
                perftest[cpu_enum].sramtest_task = kthread_create_on_node(sramtest_kthread \
                        , &(perftest[cpu_enum]) \
                        , cpu_to_node(cpu_enum) \
                        , "sramtest/%d", cpu_enum);

                if (IS_ERR(perftest[cpu_enum].sramtest_task))
                {
                    printk(KERN_WARNING "sramtest: Error creating thread for CPU %d\n", cpu_enum);
                }
                else
                {
                    kthread_bind( perftest[cpu_enum].sramtest_task, cpu_enum );
                    wake_up_process( perftest[cpu_enum].sramtest_task );
                    printk(KERN_INFO "sramtest_kthread created on CPU %d\n", cpu_enum);
                }
            }
            else
            {
                printk(KERN_ERR "sram test (PID %d) already running on CPU %d\n"
                    , task_pid_nr( perftest[cpu_enum].sramtest_task )
                    , cpu_enum);
            }
        }
        ...
        {   // Stop the thread
            if ( perftest[cpu_enum].sramtest_task )
            {
                printk(KERN_INFO "Stopping sramtest_kthread (PID %d) on CPU %d ...\n"
                    , task_pid_nr( perftest[cpu_enum].sramtest_task )
                    , cpu_enum);
                kthread_stop( perftest[cpu_enum].sramtest_task );
                perftest[cpu_enum].sramtest_task = NULL;
            }
            else
            {
                printk(KERN_ERR "no sram test running on CPU %d\n", cpu_enum);
            }
        }
    }
...
*
***************************************************************************/
static void do_perfcmd(int cpu)
{
    PERFCMD cmd = perftest[cpu].perfcmd.cmd;
...
//            memcpy_perf_test(perfflag.memcpysize, perftest[perfflag.cpu_nr].memcpyIter, perfflag.verbose, "mempcy test ddr ");
            memcpy_perf_test(perfflag.memcpysize, perftest[cpu].memcpyIter, perfflag.verbose, "mempcy test ddr ");
            break;
#endif  // ENABLE_ARM_PERF_MONITOR_TEST

         case CMD_MEMCPY_ARMLIB_TEST :
//            memcpy_armlib_perf_test(perfflag.memcpysize, perftest[perfflag.cpu_nr].memcpyIter, perfflag.verbose, "mempcy test ddr ");
            memcpy_armlib_perf_test(perfflag.memcpysize, perftest[cpu].memcpyIter, perfflag.verbose, "mempcy test ddr ");
            break;

        case CMD_SDMA_MEMCPY_TEST :
//            sdma_memcpy_perf_test(perfflag.memcpysize, perftest[perfflag.cpu_nr].memcpyIter);
            sdma_memcpy_perf_test(perfflag.memcpysize, perftest[cpu].memcpyIter);
            break;
...
//            dma_memcpy_perf_test(perfflag.memcpysize, perftest[perfflag.cpu_nr].memcpyIter);
            dma_memcpy_perf_test(perfflag.memcpysize, perftest[cpu].memcpyIter);
            break;
#endif  // ENABLE_DMA_MEMCPY_PERF_TEST
        ...
        case CMD_NOP_TEST :
//            nop_perf_test(perftest[perfflag.cpu_nr].nopIter, perfflag.verbose, "64k nop test");
            nop_perf_test(perftest[cpu].nopIter, perfflag.verbose, "64k nop test");
            break;
#endif  // ENABLE_ARM_PERF_MONITOR_TEST
...
#endif
#if (KTHREAD_MODEL != 1)
        case CMD_MODULE_EXIT :
            printk(KERN_INFO "Set thread id (%d/%ld) to -1 to terminate thread loop\n"
                , cpu
                , perftest[cpu].perfcmdThreadPid);
            perftest[cpu].perfcmdThreadPid = -1;
#endif
            break;
...
*
***************************************************************************/
static int perfcmd_kthread(void *data)
{
    int cpu = *((int *)data);
    int intr_event = -ERESTARTSYS;
    ...
    printk(KERN_DEBUG "perfcmd_kthread running on cpu %d\n", cpu);
    while ( 1 )
    {
        intr_event = wait_event_interruptible(perftest[cpu].perfcmdWakeQ, perftest[cpu].perfcmd.perfcmdWake);
        perftest[cpu].perfcmd.perfcmdWake = 0;

        if (intr_event != 0)
        {   // We may receive (-ERESTARTSYS)
            printk(KERN_INFO "perfcmd_kthread on cpu %d got signal interrupt\n", cpu);
            continue;
        }

        if (kthread_should_stop())
        {   // Thread is stopped by calling kthread_stop(), and having the condition (perfcmdWake) to be true before it.
            break;
        }

        // Here we got interrupt event triggered by someone calling send_perfcmd()
        if (!perfflag.quiet)
        {
            printk(KERN_INFO "perfcmd_kthread on cpu %d got command %d\n"
                , cpu
                , perftest[cpu].perfcmd.cmd);
        }
        do_perfcmd(cpu);
    }
...
static int perfcmd_thread(void *data)
{
    ...
    while (perftest[cpu].perfcmdThreadPid != -1)
    {
        intr_event = wait_event_interruptible(perftest[cpu].perfcmdWakeQ, perftest[cpu].perfcmd.perfcmdWake);
        if (intr_event == 0)
        {
        ...
        else if (intr_event == -ERESTARTSYS)
        {
            printk(KERN_INFO "perfcmd_thread on cpu %d got signal interrupt\n", cpu);
        }
    }
...
/****************************************************************************/
/**
*  @brief   Initialize by setting up the sysctl and proc/sys/perftest entries, allocating
...
static int __init perftest_init(void)
{
    ...
    for (i=0; i<num_online_cpus(); i++)
    {
        ...
        perftest[i].noploop_task = NULL;
#if (ENABLE_SRAM_TEST == 1)
        perftest[i].sramtest_task = NULL;
#endif
        init_waitqueue_head(&perftest[i].perfcmdWakeQ);

#if (KTHREAD_MODEL == 1)
        //   It makes more sense to use kthread_create_on_node() for
        // we are going to bind the thread to specific cpu.
        //   And we have to call wake_up_process() ourself.
        // * process name length (TASK_COMM_LEN) is 16, shows only 15 characters.
        perftest[i].perfcmdThread_task = kthread_create_on_node(perfcmd_kthread \
                        , &perftest[i].cpu_enum \
                        , cpu_to_node(perftest[i].cpu_enum) \
                        , "perftest/%d", perftest[i].cpu_enum);

        if (IS_ERR(perftest[i].perfcmdThread_task))
        {
            printk(KERN_WARNING "perftest: Error creating thread for cpu %d\n", i);
        }
        else
        {
            kthread_bind( perftest[i].perfcmdThread_task, i );
            wake_up_process(perftest[i].perfcmdThread_task);
#if (ENABLE_ARM_PERF_MONITOR_TEST == 1)
            send_perfcmd(i, CMD_EVENT);
#endif  // ENABLE_ARM_PERF_MONITOR_TEST
        }

#else   // KTHREAD_MODEL != 1
        ...
        perftest[i].perfcmdThreadPid = kernel_thread(perfcmd_thread, &perftest[i].cpu_enum, 0);
...
static void __exit perftest_exit(void)
{
    ...
    // Exit main threads - perfcmd_thread()
    for (i = 0; i < num_online_cpus(); i++)
    {
#if (KTHREAD_MODEL == 1)
        if (perftest[i].perfcmdThread_task)
        {
            perftest[i].perfcmd.perfcmdWake = 1;        // Cause the thread to leave the wait_event_interruptible() loop
            kthread_stop(perftest[i].perfcmdThread_task);
            perftest[i].perfcmdThread_task = NULL;
        }
#else
        if (perftest[i].perfcmdThreadPid >= 0)
        {
//            send_perfcmd(i, CMD_MODULE_EXIT);
            KILL_PROC(perftest[i].perfcmdThreadPid, SIGTERM);
            wait_for_completion(&perftest[i].perfcmdExited);
        }
#endif
    }
