...
UInt16 SYSPARM_GetLogFormat(void)
{
	...
	log_format_addr
		= SYSPARM_GePAtByIndex("log_format",
				sizeof(*log_format_ptr), 1);
	if (!log_format_addr) {
	...
	*APSYSPARM_GetIndMultimediaAudioParmAccessPtr(void)
{
	...
	if (!ind_mmaudio_parm_table) {
		...
		if (!ind_mmaudio_parm_table)
			pr_err("[sysparm]: ind_mmaudio_parm_addr ioremap failed\n");
	}
...
UInt16 SYSPARM_GetActualLowVoltReading(void)
{
	...
	default_batt_ActualLowVoltReading_addr
		= SYSPARM_GePAtByIndex("batt_ActualLowVoltReading",
			sizeof(*default_batt_ActualLowVoltReading_ptr), 1);
