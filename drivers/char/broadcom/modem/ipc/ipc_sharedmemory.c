...
#define RAISE_INTERRUPT	\
	do {	\
		...
		SmLocalControl.SmControl->Interrupts	\
		[IPC_CPU_ID_INDEX(IPC_SM_CURRENT_CPU)].InterruptRaised++; \
	} while (0)
...
void IPC_Initialise(void *Sm_BaseAddress,
	...
	if (SmControl->Initialised
			[IPC_CPU_ID_INDEX(IPC_OTHER_CPU[CPUID])] == 0) {
...
#elif defined(FUSE_APPS_PROCESSOR) && !defined(UNDER_CE) \
		&& !defined(UNDER_LINUX)
