...
#define PCM_AADMA_MODE                             1  /* Set to 0 to use SDMA */

#if PCM_AADMA_MODE

#define PCM_DMA_Device_t         AADMA_Device_t
#define PCM_DMA_Status_t         AADMA_Status_t

#else

#define PCM_DMA_Device_t         DMA_Device_t
#define PCM_DMA_Status_t         DMA_Status_t
...
#define PCM_SSP4_PHYS_BASE_ADDR_START              ( SSP4_BASE_ADDR )
#define PCM_SSP5_PHYS_BASE_ADDR_START              ( SSP5_BASE_ADDR )
#define PCM_SSP6_PHYS_BASE_ADDR_START              ( SSP6_BASE_ADDR )
...
#define SSPI_PROTOCOL_FMT_SEL			  SSPI_PROT_I2S_MODE2 //SSPI_PROT_I2S_MODE1
...
#define PCM_MAX_SAMPLING_RATE             48000    /* in Hz - do not change */

/* PCM channel default configuration */
#define PCM_DEFAULT_FRAME_PERIOD          5000     /* In usec - do not change */
#define PCM_DEFAULT_INTERCHS              2        /* interleaved data: 1 for mono, 2 for stereo, etc */
...
#define PCM_DEFAULT_SAMP_FREQ             48000     /* In Hz  */

#define I2S_BCLK_CLK_RATE                 (PCM_DEFAULT_SAMP_FREQ * (WORD_LENGTH * 2) * PCM_DEFAULT_INTERCHS * 2)  
                                          /* Ex. 48K * 32 * 2 * 2 */ 

#define CALCFRAMESZ(freq,period,sampwidth)	 ((freq/1000) * (period/1000) * (sampwidth) * (PCM_DEFAULT_INTERCHS)) /* in bytes */
...
#if PCM_AADMA_MODE
   SSASW_Handle_t       ssasw_igr_handle; /* SSASW driver ingress handle */
   SSASW_Handle_t       ssasw_egr_handle; /* SSASW driver egress handle */

   SSASW_Device_t       switch_dev_igr;   /* Ingress CAPH Switch trigger condition setting */
   SSASW_Device_t       switch_dev_egr;   /* Egress CAPH Switch trigger condition setting */
#endif
...
static void pcmDmaIngressHandler( PCM_DMA_Device_t dev, PCM_DMA_Status_t *status, void *data );
static void pcmDmaEgressHandler( PCM_DMA_Device_t dev, PCM_DMA_Status_t *status, void *data );
...
)
{
   ...
   if( err )
   {
      printk(KERN_ERR "PCM: Failed to initialize AADMA rc=%i\n", err );
      return err;
   }
   ...
   if( err )
   {
      printk(KERN_ERR "PCM: Failed to initialize SSASW rc=%i\n", err );
      return err;
   }
...
static int pcmPrepare( void )
{
   ...
   HALAUDIO_PCM_PLATFORM_INFO *info;
#if PCM_AADMA_MODE
   uint32_t egr_cfifo_addr;
   uint32_t igr_cfifo_addr;
   ...
   for ( i = info->channel_select; i < (info->channel_select + info->channels); i++, ch++ )
   {
...
#if PCM_AADMA_MODE
      aadma_set_device_handler( ch->dma_egr.device, pcmDmaEgressHandler, ch );
      aadma_set_device_handler( ch->dma_igr.device, pcmDmaIngressHandler, ch );
#else
...
#if PCM_AADMA_MODE
      ch->dma_egr.handle = aadma_request_channel( ch->dma_egr.device );
#else
...
#if PCM_AADMA_MODE
      egr_cfifo_addr = aadma_get_cfifo_address( ch->dma_egr.handle );
      if ( !egr_cfifo_addr )
      {
         printk( KERN_ERR "%s: Invalid egress CFIFO address\n", __FUNCTION__ );
         goto cleanup_dma_channels;
      }

      src_addr = egr_cfifo_addr;
      dst_addr = ch->dma_egr.fifo_addr;

      ch->ssasw_egr_handle = ssasw_request_channel( ch->switch_dev_egr, (uint16_t)src_addr, (uint16_t)dst_addr );
      if ( ch->ssasw_egr_handle == SSASW_INVALID_HANDLE )
      {
         printk( KERN_ERR "%s Allocation of egress SSASW channel failed\n", __FUNCTION__ );
         goto cleanup_dma_channels;
      }
#endif
...
#if PCM_AADMA_MODE
      rc = aadma_alloc_ring_descriptors( ch->dma_egr.handle,
            ch->buf_egr[0].phys, ch->buf_egr[1].phys, ch->frame_size );

      if ( rc != 0 )
      {
         printk( KERN_ERR "%s: [ch=%u] Error %d reported when allocating ring descriptors\n",
               __FUNCTION__, ch->ch, rc );
         goto cleanup_dma_channels;
      }
#else
...
#if PCM_AADMA_MODE
      rc = aadma_start_transfer( ch->dma_egr.handle);
      ssasw_enable( ch->ssasw_egr_handle, 1 );
      aadma_enable( ch->dma_egr.handle );
#else
...
#if PCM_AADMA_MODE
      ch->dma_igr.handle = aadma_request_channel( ch->dma_igr.device );
#else
...
#if PCM_AADMA_MODE
      igr_cfifo_addr = aadma_get_cfifo_address( ch->dma_igr.handle );
      if ( !igr_cfifo_addr )
      {
         printk( KERN_ERR "%s: Invalid ingress CFIFO address\n", __FUNCTION__ );
         goto cleanup_dma_channels;
      }

      src_addr = ch->dma_igr.fifo_addr;
      dst_addr = igr_cfifo_addr;

      ch->ssasw_igr_handle = ssasw_request_channel( ch->switch_dev_igr, (uint16_t)src_addr, (uint16_t)dst_addr );
      if ( ch->ssasw_igr_handle == SSASW_INVALID_HANDLE )
      {
         printk( KERN_ERR "%s Allocation of ingress SSASW channel failed\n", __FUNCTION__ );
         goto cleanup_dma_channels;
      }
#endif
      ...
      /* Allocate ingress double buffer DMA descriptors */
#if PCM_AADMA_MODE
      rc = aadma_alloc_ring_descriptors( ch->dma_igr.handle,
            ch->buf_igr[0].phys, ch->buf_igr[1].phys, ch->frame_size );
       ...
       */
#if PCM_AADMA_MODE
      rc = aadma_start_transfer( ch->dma_igr.handle);
      ssasw_enable( ch->ssasw_igr_handle, 1 );
      aadma_enable( ch->dma_igr.handle );
#else
...
static int pcmDisableNoCheck( void )
{
   ...
   rc = chal_sspi_enable( gPcmHwCore.handle, 0);
   ...
   for ( i = info->channel_select; i < (info->channel_select + info->channels); i++, ch++ )
   {
...
#if PCM_AADMA_MODE
      rc = aadma_stop_transfer( ch->dma_egr.handle );
      ssasw_enable( ch->ssasw_egr_handle, 0 );
      ssasw_remove_all_dst( ch->ssasw_egr_handle );
      ssasw_mixer_enable( ch->ssasw_egr_handle, 0 );
#else
...
#if PCM_AADMA_MODE
      rc = aadma_stop_transfer( ch->dma_igr.handle );
      ssasw_enable( ch->ssasw_igr_handle, 0 );
      ssasw_remove_all_dst( ch->ssasw_igr_handle );
      ssasw_mixer_enable( ch->ssasw_igr_handle, 0 );
#else
...
#if PCM_AADMA_MODE
      aadma_set_device_handler( ch->dma_egr.device, NULL, NULL );
      aadma_set_device_handler( ch->dma_igr.device, NULL, NULL );
#else
      ...
      {
#if PCM_AADMA_MODE
         aadma_free_channel( ch->dma_egr.handle );
         ssasw_free_channel( ch->ssasw_egr_handle );
         ch->dma_egr.handle = AADMA_INVALID_HANDLE;
         ch->ssasw_egr_handle = SSASW_INVALID_HANDLE;
#else
      ...
      {
#if PCM_AADMA_MODE
         aadma_free_channel( ch->dma_igr.handle );
         ssasw_free_channel( ch->ssasw_igr_handle );
         ch->dma_igr.handle = AADMA_INVALID_HANDLE;
         ch->ssasw_igr_handle = SSASW_INVALID_HANDLE;
#else
...
static int pcm_sspi_hw_config( struct pcm_sspi_hw_core_t *pCore, unsigned int samp_freq )
{
    ...
    if( chal_sspi_set_idle_state( pCore->handle, SSPI_PROTOCOL_FMT_SEL ) )
    {
        return -EINVAL;
    }
    ...
    clk_set_rate(gSspiClk, I2S_BCLK_CLK_RATE);
    clk_enable(gSspiClk);

    chal_sspi_set_clk_divider(pCore->handle, SSPI_CLK_DIVIDER0, 0);
    chal_sspi_set_clk_divider(pCore->handle, SSPI_CLK_REF_DIVIDER, 0);
    chal_sspi_set_caph_clk(pCore->handle, SSPI_CAPH_CLK_TRIG_48kHz, WORD_LENGTH, CHAN_NUM);
		
    /* Set to full FIFO usage (no division of FIFO) */
#if PCM_AADMA_MODE
    //chal_sspi_set_fifo_size(pCore->handle, SSPI_FIFO_ID_RX0, SSPI_FIFO_SIZE_HALF);
...
#if PCM_AADMA_MODE
    //chal_sspi_set_fifo_size(pCore->handle, SSPI_FIFO_ID_TX0, SSPI_FIFO_SIZE_HALF);
...
#if PCM_AADMA_MODE
    chal_sspi_set_fifo_pio_threshhold(pCore->handle, SSPI_FIFO_ID_TX0, 3, 3);
    chal_sspi_enable_fifo_pio_start_stop_intr(pCore->handle, SSPI_FIFO_ID_TX0, 1, 1);

    chal_sspi_set_fifo_pio_threshhold(pCore->handle, SSPI_FIFO_ID_RX0, 3, 3);
    chal_sspi_enable_fifo_pio_start_stop_intr(pCore->handle, SSPI_FIFO_ID_RX0, 1, 1);
#endif
    ...
    if( (rc = chal_sspi_set_task(pCore->handle, 0, SSPI_PROTOCOL_FMT_SEL, &gTask_conf)) )
    {
       return rc;
    }

    gSeq_conf.tx_enable = 1;
    gSeq_conf.rx_enable = 0;
    gSeq_conf.cs_activate = 0;
    gSeq_conf.cs_deactivate = 1;
    gSeq_conf.pattern_mode = 0;
    gSeq_conf.rep_cnt = 0;
    gSeq_conf.opcode = SSPI_SEQ_OPCODE_NEXT_PC;
    ...
    gSeq_conf.next_pc = 0;
    if ((rc= chal_sspi_set_sequence(pCore->handle, 0, SSPI_PROTOCOL_FMT_SEL, &gSeq_conf)))
    	return rc;
    ...
    gSeq_conf.tx_sidetone_on = 0;
    gSeq_conf.next_pc = 0;
    if ((rc = chal_sspi_set_sequence(pCore->handle, 1, SSPI_PROTOCOL_FMT_SEL, &gSeq_conf)))
	    return rc;

    gSeq_conf.tx_enable = 0;
    gSeq_conf.rx_enable = 0;
    gSeq_conf.cs_activate = 0;
    gSeq_conf.cs_deactivate = 0;
    gSeq_conf.pattern_mode = 0;
    ...
    gSeq_conf.tx_sidetone_on = 0;
    gSeq_conf.next_pc = 0;
    if ((rc = chal_sspi_set_sequence(pCore->handle, 2, SSPI_PROTOCOL_FMT_SEL, &gSeq_conf)))
	    return rc;

    if( (rc = chal_sspi_set_frame(pCore->handle, &frmMask, SSPI_PROTOCOL_FMT_SEL, 16, 16)) )
    {
       return rc;
    }
...
static void pcmDmaIngressHandler(
   PCM_DMA_Device_t   dev,       /**< (i) Device that triggered callback */
   PCM_DMA_Status_t  *status,    /**< (i) DMA status information */
...
)
{
...
#if PCM_AADMA_MODE
   if ( status->dma_status & CAPH_READY_HIGH )
   {
   ...
   else if ( status->dma_status & CAPH_READY_LOW )
   {
      /* DMA finished on low buffer, so we set the active index to be 0 */
      atomic_set(&ch->active_idx_igr, 0);
   }
   else
   {
   ...
   if ( aadma_continue_transfer( ch->dma_igr.handle, CAPH_READY_HIGHLOW ) )
   {
      ch->errs.dma_igr++;
   }
...
)
{
   ...
   ch = data;
   PCMLOG( "dev=%u isr=%u", dev, atomic_read(&ch->isrcount_egr) );
   ...
   {
      /* DMA finished on low buffer, so we set the active index to be 0 */
      atomic_set(&ch->active_idx_egr, 0);
   }
   else
   {
      /* Somehow none of the buffers are ready */
   ...
   /* Continue DMA ring operation */
   if ( aadma_continue_transfer( ch->dma_egr.handle, CAPH_READY_HIGHLOW ) )
   {
      ch->errs.dma_egr++;
   }
...
static int8_t				 igbuf[PCM_MAX_DMA_BUFFER_SIZE_BYTES];
static int8_t				 egbuf[PCM_MAX_DMA_BUFFER_SIZE_BYTES];
...
static int pcm_platform_init( HALAUDIO_PCM_PLATFORM_INFO *info )
{
   ...
   if ( info->bt_gpio.rst_b >= 0 )
   {
      ...
      if ( err )
      {
         printk( KERN_ERR "%s: failed to request BT_RST_B gpio pin rc=%d\n", __FUNCTION__, err );
         err = -EBUSY;
      ...
      if( info->bt_gpio.vreg_ctl >= 0 )
      {
         ...
         if ( err )
         {
            printk( KERN_ERR "%s: failed to request BT_VREG_CTL gpio pin rc=%d\n", __FUNCTION__, err );
            err = -EBUSY;
            goto cleanup_and_exit;
         }
      ...
      if( info->bt_gpio.wake >= 0 )
      {
         ...
         if ( err )
         {
            printk( KERN_ERR "%s: failed to request BT_WAKE gpio pin rc=%d\n", __FUNCTION__, err );
            err = -EBUSY;
            goto cleanup_and_exit;
         }
...
static int pcm_probe( struct platform_device *pdev )
{
   ...
   if( info->core_id_select == SSPI_CORE_ID_0 )
   {
      ...
      gSspiClk = clk_get( &pdev->dev, SSP0_AUDIO_PERI_CLK_NAME_STR );
   }
   else if( info->core_id_select == SSPI_CORE_ID_2 )
   {
      ...
      gSspiClk = clk_get( &pdev->dev, SSP2_AUDIO_PERI_CLK_NAME_STR );
   }
   else if( info->core_id_select == SSPI_CORE_ID_3 )
   {
      ...
      gSspiClk = clk_get( &pdev->dev, SSP3_AUDIO_PERI_CLK_NAME_STR );
   }
   else if( info->core_id_select == SSPI_CORE_ID_4 )
   {
      gPcmIrqId = BCM_INT_ID_SSP4;
      gPcmPhysBaseAddr = PCM_SSP4_PHYS_BASE_ADDR_START;

      gSspiClk = clk_get( &pdev->dev, SSP4_AUDIO_PERI_CLK_NAME_STR );
   }
   else if( info->core_id_select == SSPI_CORE_ID_5 )
   {
      gPcmIrqId = BCM_INT_ID_SSP5_ERR;
      gPcmPhysBaseAddr = PCM_SSP5_PHYS_BASE_ADDR_START;

      gSspiClk = clk_get( &pdev->dev, SSP5_AUDIO_PERI_CLK_NAME_STR );
   }
   else if( info->core_id_select == SSPI_CORE_ID_6 )
   {
      gPcmIrqId = BCM_INT_ID_SSP6_ERR;
      gPcmPhysBaseAddr = PCM_SSP6_PHYS_BASE_ADDR_START;

      gSspiClk = clk_get( &pdev->dev, SSP6_AUDIO_PERI_CLK_NAME_STR );
   }
   else
   {
   ...
   err = clk_set_rate( gSspiClk, I2S_BCLK_CLK_RATE );
   ...
   for ( i = info->channel_select; i < (info->channel_select + info->channels); i++, ch++ )
   {
      ...
      {
#if PCM_AADMA_MODE
         goto halaudio_pcm_dma_config_fail;
#else
      ...
      {
#if PCM_AADMA_MODE
         ch->dma_igr.device = AADMA_DEVICE_PCM_SSP3_TO_CFIFO_TO_MEM;
         ch->dma_egr.device = AADMA_DEVICE_PCM_MEM_TO_CFIFO_TO_SSP3;
         ch->switch_dev_igr = SSASW_DEVICE_SSASW_SSP3_PCM_IGR;
         ch->switch_dev_egr = SSASW_DEVICE_SSASW_SSP3_PCM_EGR;
#else
      ...
      {
#if PCM_AADMA_MODE
         ch->dma_igr.device = AADMA_DEVICE_PCM_SSP4_TO_CFIFO_TO_MEM;
         ch->dma_egr.device = AADMA_DEVICE_PCM_MEM_TO_CFIFO_TO_SSP4;
         ch->switch_dev_igr = SSASW_DEVICE_SSASW_SSP4_PCM_IGR;
         ch->switch_dev_egr = SSASW_DEVICE_SSASW_SSP4_PCM_EGR;
#else
         ch->dma_igr.device = DMA_DEVICE_SSP4A_RX0;
         ch->dma_egr.device = DMA_DEVICE_SSP4B_TX0;
#endif
      }
      else if( info->core_id_select == SSPI_CORE_ID_5 )
      {
#if PCM_AADMA_MODE
         ch->dma_igr.device = AADMA_DEVICE_PCM_SSP5_TO_CFIFO_TO_MEM;
         ch->dma_egr.device = AADMA_DEVICE_PCM_MEM_TO_CFIFO_TO_SSP5;
         ch->switch_dev_igr = SSASW_DEVICE_SSASW_SSP5_PCM_IGR;
         ch->switch_dev_egr = SSASW_DEVICE_SSASW_SSP5_PCM_EGR;
#else
         ch->dma_igr.device = DMA_DEVICE_SSP5A_RX0;
         ch->dma_egr.device = DMA_DEVICE_SSP5B_TX0;
#endif
      }
      else if( info->core_id_select == SSPI_CORE_ID_6 )
      {
#if PCM_AADMA_MODE
         ch->dma_igr.device = AADMA_DEVICE_PCM_SSP6_TO_CFIFO_TO_MEM;
         ch->dma_egr.device = AADMA_DEVICE_PCM_MEM_TO_CFIFO_TO_SSP6;
         ch->switch_dev_igr = SSASW_DEVICE_SSASW_SSP6_PCM_IGR;
         ch->switch_dev_egr = SSASW_DEVICE_SSASW_SSP6_PCM_EGR;
#else
         ch->dma_igr.device = DMA_DEVICE_SSP6A_RX0;
         ch->dma_egr.device = DMA_DEVICE_SSP6B_TX0;
#endif
      }
      else
      {
