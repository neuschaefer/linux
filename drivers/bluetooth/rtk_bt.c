...
int rtk_parse_config_file(unsigned char* config_buf, int* filelen, char bt_addr[6])
{
    ...
    for (i=0; i<config_len;) {
	    ...
	    config_has_bdaddr = 1;
	    snprintf(tbuff, sizeof(tbuff), "%2.2X:%2.2X:%2.2X:%2.2X:%2.2X:%2.2X",
			entry->entry_data[5],
			entry->entry_data[4],
			entry->entry_data[3],
			entry->entry_data[2],
    ...
    //    RTKBT_DBG("There is no bdaddr in config file.");
    //    *filelen += 9;
...
int load_firmware(dev_data* dev_entry, uint8_t** buff)
{
	...
	mm_segment_t oldfs;
	char bdbuf[FACTORY_BT_BDADDR_STORAGE_LEN+1];
	int32_t i = 0;
	char *str;
#endif
	...
	if (ret_val < 0)
		...
		config_len = fw->size;
#ifndef USE_CONTROLLER_BDADDR
		memset(bdbuf, 0, FACTORY_BT_BDADDR_STORAGE_LEN+1);
		...
		if (IS_ERR(bdaddr_file)){
			...
			bdaddr_file->f_op->read(bdaddr_file, bdbuf,
					FACTORY_BT_BDADDR_STORAGE_LEN,
					&bdaddr_file->f_pos);

			str = bdbuf;
			for (i = 5; i >= 0; i--, str += 3)
				vnd_local_bd_addr[5 - i] =
					(uint8_t)simple_strtoul(str, NULL, 16);
