...
static void btusb_isoc_tx_complete(struct urb *urb)
{
	...
	if (skb && hdev) {
		...
		RTKBT_INFO("%s: skb 0x%p hdev 0x%p", __func__, skb, hdev);
...
static int btusb_open(struct hci_dev *hdev)
{
	...
	RTKBT_DBG("%s start power.usage_count(0x%x)", __func__,
...
#endif
	RTKBT_DBG("%s end power.usage_count(0x%x)", __FUNCTION__,
		  atomic_read(&(data->intf->dev.power.usage_count)));
...
failed:
	...
	usb_autopm_put_interface(data->intf);
	RTKBT_ERR("%s failed power.usage_count(0x%x)", __FUNCTION__,
		  atomic_read(&(data->intf->dev.power.usage_count)));
	return err;
}
...
static void btusb_waker(struct work_struct *work)
{
	...
	err = usb_autopm_get_interface(data->intf);
	RTKBT_DBG("%s start power.usage_count(0x%x)", __FUNCTION__,
		  atomic_read(&(data->intf->dev.power.usage_count)));
	if (err < 0)
		return;
...
int load_firmware(dev_data * dev_entry, uint8_t ** buff)
{
	...
	RTKBT_INFO("fw name is  %s", fw_name);
	...
	if (lmp_version == ROM_LMP_8723a) {
		...
		RTKBT_INFO("This is not 8723a, use new patch style!");
