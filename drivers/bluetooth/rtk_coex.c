...
static void set_afh_host_ch_cls(void)
{
	...
	RTKBT_DBG("%s: BR/EDR set host channel classification.", __func__);

	rtk_vendor_cmd_to_fw(opcode, 10, afh_map);
}

static void le_set_host_ch_cls(void)
{
	/* 7.8.19 LE Set Host Channel Classification Command */
	u16 opcode = 0x2014;
	u8 ch_map[5] = {
		0xff, 0xff, 0xfd, 0xff, 0x1f
	};

	RTKBT_DBG("%s: BLE set host channel classification.", __func__);

	rtk_vendor_cmd_to_fw(opcode, 5, ch_map);
}
