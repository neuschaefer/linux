...
static struct pwm_pin_sleep_cfg pwm_pin_sleep_info[] = {

	[0] = {
	       .chan = 2,
	       .config = PIN_CFG(GPIO06, GPIO_006, 0, ON, OFF, 0, 0, 8MA),
	       },
};
...
static int kona_pwmc_suspend(struct platform_device *pdev, pm_message_t state)
{
	int chan, i;
	struct pin_config config;
	int size = sizeof(pwm_pin_sleep_info) / sizeof(struct pin_config);

	/* Save current config, change config to GPIO 
	   Input pulldown, if needed*/
	for (chan = 0; chan < KONA_PWM_CHANNEL_CNT; chan++) {
		for (i = 0; i < size; i++) {
			if (chan == pwm_pin_sleep_info[i].chan) {
				config.name = pwm_pin_sleep_info[i].config.name;
				pinmux_get_pin_config(&config);
				pinmux_set_pin_config(&pwm_pin_sleep_info[i].
						      config);
				pwm_pin_sleep_info[i].config = config;
				break;
			}
		}
	}
