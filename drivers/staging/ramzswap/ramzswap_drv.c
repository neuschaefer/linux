...
#ifdef CONFIG_TOI
static int ramzswap_toi_flag_disk(struct ramzswap *ramzswap)
{
	int index, t = 0;

	for (index = 0; index < ramzswap->disksize >> PAGE_SHIFT; index++) {
		struct page *page = ramzswap->table[index].page;
...
int ramzswap_toi_flag_disks(void)
{
	int i, t = 0;
	struct ramzswap *ramzswap;

	for (i = 0; i < num_devices; i++) {
		ramzswap = &devices[i];

		t += ramzswap_toi_flag_disk(ramzswap);
	}
...
{
	toi_flag_ramzswap_disks = ramzswap_toi_flag_disks;
}

void clear_toi_hook(void)
{
	toi_flag_ramzswap_disks = NULL;
}
#else
#define set_toi_hook() { }
