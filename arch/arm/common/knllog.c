	...
	*line = '\0';
	...
	if (knllog.deltatime) {
		if (knllog.msec) {
			...
			if (timer_get_tick_rate() >= 1000000) {
				int deltausecs =
			    		deltaticks / (timer_get_tick_rate() / 1000000);
				snprintf(field, sizeof(field), "%6d.%03d: ",
				 	deltausecs / 1000u, deltausecs % 1000u);
			} else { /* timer does not have microsecond resolution */
				int deltamsecs = deltaticks / ( timer_get_tick_rate() / 1000 );
				snprintf(field, sizeof(field), "%6d: ", deltamsecs);
			}
		} else {
