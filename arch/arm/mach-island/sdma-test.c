...
static void *alloc_mem( dma_mem_t *mem, size_t numBytes )
{
    ...
    pr_warn( "dma_alloc_writecombine of %d bytes returned virtPtr: 0x%08lx physPstr: %08x\n",
...
static int dma_test_check_mem( dma_mem_t *mem, uint16_t start_count )
{
    ...
    for ( i = 0; i < numWords; i++ )
    {
        if ( *memInt != counter )
        {
            pr_warn( "Destination: base=%p, i=%d, found 0x%4.4x, expecting 0x%4.4x\n",
...
static int memcpy_tests( long iterations )
{
   ...
   int run_one_test( int test_number )
   {
      ...
      for ( chan = 0; chan < SDMA_NUM_CHANNELS; chan++ )
      {
         ...
         {
            pr_err( ": ========== DMA memcpy test%d channel %d failed [%03d] ==========\n\n", test_number, chan, i );
            return rc;
         }
...
int dma_test_thread( void *data )
{
   ...
   for ( devIdx = 0; devIdx < DMA_NUM_DEVICE_ENTRIES; devIdx++ )
   {
      ...
      if ( test == 1 )
      {
         ...
         {
            pr_warn( "Call to sdma_request_channel failed: %d\n", gTestHandle[ devIdx ] );
            continue;
         }
      ...
      else
      {
         ...
         {
            pr_warn( "About to relase channel for device %d, handle 0x%04x\n", devIdx, gTestHandle[ devIdx ] );
            if (( rc = sdma_free_channel( gTestHandle[ devIdx ])) < 0 )
            {
               pr_warn( "Call to sdma_free_channel failed: %d\n", rc );
            }
            msleep( 33 );
         }
      }
...
static int sdma_register_test_file( void )
{
   ...
   if ( gDmaDir == NULL )
   {
       pr_err( "Unable to create /proc/sdma/test\n" );
   }
   else
   {
