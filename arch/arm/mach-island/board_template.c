...
//#include <bcm915500_i2c_ts_settings.h>
#endif
...
/* keypad interface */
#define KEYPAD_CORE_REG_SIZE    (0xD0)
...
static struct resource board_keypad_device_resource[] = {
    [0] = {
        ...
        .end   = KEYPAD_BASE_ADDR + KEYPAD_CORE_REG_SIZE -1,
        .flags = IORESOURCE_MEM,
    },
    [1] = {
...
/* 
 * For island, it used to have hard-coded gpios, and since we're not going to be supporting
 * island any more, I left them as hard-coded. If it is desired to have per-platform
 * custom configurations, then HW_CFG_CAMERA should be moved into hw_cfg/camera_settings.h
 */

#if defined(CONFIG_BCM_HDMI_DET) || defined(CONFIG_BCM_HDMI_DET_MODULE)
/*
 * Leave GPIO alone so it can be claimed by the hot-plug detection driver
 */
#define	HW_CFG_HDMI_GPIO
#else
#define	HW_CFG_HDMI_GPIO \
  { 62, 1, 0, "hdmi_hot_plug"}
#endif
...
static void __init add_i2c_device(void)
{
   ...
   printk("PPTEST calling i2c_register_board_info() for bcm915500\n");
