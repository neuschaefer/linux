...
#include <asm-arm/arch-avalanche/puma5/puma5_cppi_prv.h>
...
static struct map_desc puma5_io_desc[] = {
...
static void __init puma5_map_io(void)
{
    ...
    puma5_io_desc[ no_linux_mem_desc_idx ].pfn    = __phys_to_pfn(no_linux_mem_start);
    puma5_io_desc[ no_linux_mem_desc_idx ].length = no_linux_mem_size;
#endif
...
unsigned long long notrace avalanche_clock_vp(void)
{
    ...
    ns = (unsigned long long )(curr_hi) << 32 | (unsigned long long )(curr_lo);
    /* The base clock is 100MHz, so normalize to nSec */
    /* ns = cnt*tick_ns = cnt*(10^9/freq_hz) = cnt*(10^9 / 100MHz) = cnt * 10 */
...
unsigned int avalanche_no_OperSys_memory_phys_to_virt( unsigned int pAddr )
{
    ...
    if ( (pAddr <  __pfn_to_phys(puma5_io_desc[ no_linux_mem_desc_idx ].pfn)) ||
         (pAddr > (__pfn_to_phys(puma5_io_desc[ no_linux_mem_desc_idx ].pfn) +
                                 puma5_io_desc[ no_linux_mem_desc_idx ].length )))
    {
        printk("%s:%d\n", __FUNCTION__,__LINE__);
        return (unsigned int)-1;
    }
    ...
    return ( pAddr
             - __pfn_to_phys(puma5_io_desc[ no_linux_mem_desc_idx ].pfn)
             +               puma5_io_desc[ no_linux_mem_desc_idx ].virtual );
}
...
unsigned int avalanche_no_OperSys_memory_virt_to_phys( unsigned int vAddr )
{
    ...
    if ( (vAddr <  puma5_io_desc[ no_linux_mem_desc_idx ].virtual) ||
         (vAddr > (puma5_io_desc[ no_linux_mem_desc_idx ].virtual) +
                   puma5_io_desc[ no_linux_mem_desc_idx ].length ))
    {
        printk("%s:%d\n", __FUNCTION__,__LINE__);
        return (unsigned int)-1;
...
static void __init puma5_fixup(struct tag *tag,
...
#else
    .atag_offset = (unsigned long)&puma5_command_line_tag,
#endif
    .map_io     =   puma5_map_io,
    .init_irq   =   arch_init_irq,
    .init_early =   puma_avalanche_setup,
    .init_time  =   puma5_timer_init,
    .fixup      =   puma5_fixup,
MACHINE_END
