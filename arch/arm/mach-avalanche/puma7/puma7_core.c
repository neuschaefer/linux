...
#define VDSP_PRE_ALLOCATED_START_ADDR    0x00C00000
#endif
...
static unsigned int no_linux_mem_last       = 0;
static unsigned int no_linux_mem_start      = 0;
...
static struct map_desc puma7_io_desc[]  = {
#if CONFIG_ARM_AVALANCHE_TOP_MEM_RESERVED
...
static void __init puma7_map_io(void)
{
#if CONFIG_ARM_AVALANCHE_TOP_MEM_RESERVED
    no_linux_mem_desc_idx = 0;
    puma7_io_desc[ no_linux_mem_desc_idx ].pfn    = __phys_to_pfn(no_linux_mem_start);
    puma7_io_desc[ no_linux_mem_desc_idx ].length = no_linux_mem_size;
#endif
    iotable_init(puma7_io_desc, ARRAY_SIZE(puma7_io_desc));
 ...
 */
static void __init puma7_timer_init(void)
{
   int timer_clk;
   int scale;
   ...
   printk("Puma7 Timer0 & Timer2 initialized\n");
}
#else
/* For VeryPrecise clock */
...
static void late_time_init_f(void)
{
    ...
    /* Restart the tick timer so it is in sync with the FRC (may loose 0..1 tick, but so early in the kernel startup we do not care */
    PAL_sysResetCtrl(AVALANCHE_TIMER0_RESET, IN_RESET);
    PAL_sysResetCtrl(AVALANCHE_TIMER0_RESET, OUT_OF_RESET);
    SC_timer_clk = PAL_sysClkcGetFreq(PAL_SYS_CLKC_TIMER0);
    PAL_sysTimer16Ctrl(AVALANCHE_TIMER0_BASE, TIMER16_CTRL_STOP);
    /* Make err <=0, i.e. actual clock is not slower than required */
    sc_timer_err_nsec = -1;
    ...
    printk("FRC hi %d lo %d\n", FREE_RUNNING_COUNTER_H_GET(), FREE_RUNNING_COUNTER_L_GET());
...
static void __init puma7_timer_init(void)
{
    ...
    PAL_sysTimer16SetParams(AVALANCHE_TIMER0_BASE, timer_clk,
            TIMER16_CNTRL_AUTOLOAD, (int)((1.0/(float)(HZ)) * 1000000.0), NULL);
    PAL_sysTimer16Ctrl(AVALANCHE_TIMER0_BASE, TIMER16_CTRL_START);
    ...
    printk("Precise sched_clock: load %d, nsec/count %d", PAL_sysTimer16GetLoad(AVALANCHE_TIMER0_BASE), sc_count_nsec);

    setup_irq(AVALANCHE_TIMER_0_INT, &puma_timer0_irq);

    printk("Puma7 Timer0 initialized\n");
}
#endif
...
unsigned int avalanche_no_OperSys_memory_phys_to_virt( unsigned int pAddr )
{
    ...
    if ( (pAddr <  __pfn_to_phys(puma7_io_desc[ no_linux_mem_desc_idx ].pfn)) ||
         (pAddr > (__pfn_to_phys(puma7_io_desc[ no_linux_mem_desc_idx ].pfn) +
                                 puma7_io_desc[ no_linux_mem_desc_idx ].length )))
    {
        printk("%s:%d\n", __FUNCTION__,__LINE__);
        return (unsigned int)-1;
    }
    ...
    return ( pAddr
             - __pfn_to_phys(puma7_io_desc[ no_linux_mem_desc_idx ].pfn)
             +               puma7_io_desc[ no_linux_mem_desc_idx ].virtual );
}
...
unsigned int avalanche_no_OperSys_memory_virt_to_phys( unsigned int vAddr )
{
    ...
    if ( (vAddr <  puma7_io_desc[ no_linux_mem_desc_idx ].virtual) ||
         (vAddr > (puma7_io_desc[ no_linux_mem_desc_idx ].virtual) +
                   puma7_io_desc[ no_linux_mem_desc_idx ].length ))
    {
        printk("%s:%d\n", __FUNCTION__,__LINE__);
        return (unsigned int)-1;
...
static void __init puma7_fixup(struct tag *tag,
...
#if CONFIG_ARM_AVALANCHE_TOP_MEM_RESERVED
            /* Currently we have only 1 memory tag, so we pre allocte VDSP memory with hard coded address and size */
            /* We should add another memory tag which will describe the VDSP memory */
            no_linux_mem_start = VDSP_PRE_ALLOCATED_START_ADDR;
            no_linux_mem_last  = no_linux_mem_start + no_linux_mem_size;
#endif
            break;
        }
    }
