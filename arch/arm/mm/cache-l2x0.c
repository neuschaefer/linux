 ...
 * Ported back from kernel 4.0 and add following changes:
 *  1. call read_cpuid_part_number instead of read_cpuid_part
 *  2. add macros to support l2cache big-endian mode.
 * 
 */
#include <linux/cpu.h>
#include <linux/err.h>
...
/* Puma7 L2C310 is working in BE and this driver assume all l2caches are LE */
#if CONFIG_CACHE_L2X0_BIG_ENDIAN
...
#undef readl_relaxed
#endif
#define writel_relaxed(v,c)     __raw_writel((__force u32) cpu_to_be32(v),c)
#define readl_relaxed(c)        ({ u32 __r = be32_to_cpu((__force __le32) __raw_readl(c)); __r; })
#endif
