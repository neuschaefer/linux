...
#define VA_VIC_BASE  IO_ADDRESS(SPEARBASIC_VIC1_BASE) 
...
/****************************************************************************/
    unsigned long 
spearbasic_timer_read ( int nr )
{
    unsigned long usec;
    /* Timer 0 is used by spearbasic _gettimeoffset */
    switch ( nr ) 
    {
        case 1:
        ...
        default:
            printk ( KERN_ERR "spearbasic_timer_read(%d): unknown timer\n", nr);
            BUG();
    }
    return 0;
}

EXPORT_SYMBOL ( spearbasic_timer_read );


/****************************************************************************/
/* NAME:  spearbasic_timer_interrupt                                        */
/*--------------------------------------------------------------------------*/
/* DESCRIPTION:                                                             */
...
/****************************************************************************/
    static irqreturn_t
spearbasic_timer_interrupt ( int irq, void *dev_id/*, struct pt_regs *regs*/ )
{
...
static struct irqaction spearbasic_timer_irq = 
{
    .name           = "Spearbasic Timer Tick",
    .flags          = SA_INTERRUPT,
    .handler        = spearbasic_timer_interrupt
};


/****************************************************************************/
/* NAME:          __init spearbasic_time_init                               */
/*--------------------------------------------------------------------------*/
/* DESCRIPTION:                                                             */
...
/****************************************************************************/
    void 
__init spearbasic_time_init ( void )
{
    ...
    TMR_Init ( TMR_DEVICE_ID_0 , ( t_logical_address )VA_SPEARBASIC_START_APB_GPT0 ); 
    
    TMR_SetBaseAddress ( TMR_DEVICE_ID_0 , ( t_logical_address ) VA_SPEARBASIC_START_APB_GPT0 ); 
    ...
    setup_irq ( IRQ_GPT0, &spearbasic_timer_irq ); 
...
struct sys_timer spearbasic_timer = 
{
    .init           = spearbasic_time_init,
    .offset         = spearbasic_gettimeoffset,
};
