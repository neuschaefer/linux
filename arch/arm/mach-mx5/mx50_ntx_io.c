...
//	        	gpio_inactivate_audio_ports(); // �|�y��IO REQUEST ERROR !?
	...
	//				printk ("[%s-%d] Micro P MSP430 status 0x%04X ....\n", __func__, __LINE__,tmp);
	    ...
	    case CM_GET_SLEEPSCREEN_STATE:
	        ...
	        break;
#endif
        case CM_CRITICALLOW_OCCUR:
		//printk("[%s %d] msp430_read (0x60) = %x\n",__func__, __LINE__,msp430_read (0x60));
            i = (msp430_read (0x60) & 0x03) ? 1:0;
            //printk("[%s %d] CM_CRITICALLOW_OCCUR i = %d\n",__func__, __LINE__,i);
            copy_to_user((void __user *)arg, &i, sizeof(int));
            break;
        case CM_SET_LAST_SHUTDOWN_CRITICALLOW_OCCUR:
			copy_from_user(&i, (void __user *)arg, sizeof(int));
			//printk("[%s %d] CM_SET_LAST_SHUTDOWN_CRITICALLOW_OCCUR i = %d\n",__func__, __LINE__,i);
			g_last_shutdown_due_to_critical_low = i;
...
static struct workqueue_struct *gFrontLightWorkQuene;
struct delayed_work gFrontLightDelayWork;
int g_duty = 0;

static bool power_key_down = false;
...
void mxc_set_fl_duty (int duty)
{
	...
	if (duty) {
		...
		{
			gpio_direction_output(FL_R_EN,0);
			msp430_write (0xA7, FL_table0[2*(duty-1)]&0xFF00);
			msp430_write (0xA6, FL_table0[2*(duty-1)]<<8);
			printk("PWMCNT : 0x%04x\n", FL_table0[2*(duty-1)]);
		}else if(duty>50 && duty<=100){
			gpio_direction_output(FL_R_EN,1);
			msp430_write (0xA7, FL_table0[duty-1]&0xFF00);
			msp430_write (0xA6, FL_table0[duty-1]<<8);
			printk("PWMCNT : 0x%04x\n", FL_table0[duty-1]);
		}else{
			printk("Wrong number! level range from 0 to 100\n");
		}
#else
		if (30==check_hardware_name() || 45==check_hardware_name()) {	// E606E2 and E606H2 use the same front light table
			int freq = 8000000/FL_table1[duty-1].freq;
			...
			msp430_write (0xA7, FL_table1[duty-1].duty&0xFF00);	// Set PWM duty
			msp430_write (0xA6, FL_table1[duty-1].duty<<8);
			gpio_direction_output (FL_R_EN, FL_table1[duty-1].fl_r_en);
		}
		else {
			gpio_direction_output(FL_R_EN,1);
			msp430_write (0xA7, FL_table0[duty-1]&0xFF00);
...
// Terry add 20130613 : Bugfix: when device try to go to sleep mode, keep pressing power key will not let frontlight disable
static void frontlight_delayfunc () {
    mxc_set_fl_duty (g_duty);
}

/*
static bool g_FL_on=true;

static void ISR_FL_key_func(void) {
	//2012.11.19 Added by Antony Won to honour Deutsche Telekom's request
	//change: Do not turn on front light while device is in suspend mode
...
#else
	gpio_set_value (GPIO_LED_ON,((on)?0:1));
#endif
}
...
extern int g_InSuspendProcess = 0; // Terry add : flag set to be true while kernel in suspend process
static struct wake_lock g_PowerKeyWakeLock;
static struct wake_lock acin_lock;
static struct timer_list power_key_timer;
...
static void power_key_chk(unsigned long v)
{
	...
	if ( g_InSuspendProcess == 0) {
	    if (pwr_key) {

			if (0 == g_power_key_debounce) {
             //Terry add 20130613 : Bugfix: need trigger twice to let system wake up if device keep pressing power key in sleep mode process
				wake_lock_timeout(&g_PowerKeyWakeLock,15*HZ);
				mxc_kpp_report_power(1);
				printk("[Luke]: mxc_kpp_report_power(1) \n");
				power_key_down = true;
				// Terry add 20130307 : Trigger userprocess
				if (fasync_queue)
			...
			printk("[Luke]: mxc_kpp_report_power(0) \n");
			power_key_down = false;
			LED(0);
			islongpress = false;
			wake_unlock(&g_PowerKeyWakeLock);
		}
	}
...
//Terry add 20140502
struct workqueue_struct *acin_wakelock_wq;
struct delayed_work acin_wakelock_work;
static void wakelock_release_thread(struct work_struct *work) {
	printk("[%s %d] delay acin release wakelock !!\n",__func__, __LINE__);
	wake_unlock(&acin_lock);
}
...
static void acin_pg_chk(unsigned long v)
{
	...
	//			mxc_misc_report_usb(0);
		//ntx_charger_online_event_callback ();// Terry add 20130122 : we check charger status while msp interrupt trigger
		}
}

//#define NTX_BAT_CHG_SW_SIM
#ifdef NTX_BAT_CHG_SW_SIM
static unsigned long ntx_charge_96p_sec;
#define CHARGE_FULL_TIME	10*60
#include <linux/time.h>
#endif

int ntx_charge_status (void)
{
	if ((10 == check_hardware_name() || 5 == check_hardware_name() || 30 ==check_hardware_name() || 45==gptHWCFG->m_val.bPCB) && !g_pwr_key ) {//E606A2, E606E2 or E60672 E606H2
#ifdef NTX_BAT_CHG_SW_SIM
		if (gpio_get_value (GPIO_ACIN_PG)) {
			ntx_charge_96p_sec = 0;
        	return 0;
        }
        else if (ntx_charge_96p_sec) {
			unsigned long now=get_seconds();
        	if (CHARGE_FULL_TIME <= ((long)now-(long)ntx_charge_96p_sec))
        		return 2;
        }
       	return 3;
#else
	if (gIsMSP430IntTriggered) {
		//force read msp430 once to avoid bug
		//when device critical low , charge state is set to "charging". that is not correct.
		printk("gIsMSP430IntTriggered  msp430_read (0x60) = %x\n",msp430_read (0x60));
	}
    return ((gpio_get_value (GPIO_ACIN_PG)?0:1) | ((msp430_read (0x60)&0x08)?0:2));
#endif
}
    else
...
extern int gTerryBatteryDebug = 0; //Terry add 20130418 : add for debug battery behavior
int gSaveLargeBatteryPercentageDiffFlag = 0;
int gLargeBatteryPercentageDiff = 0;
int gTrustCount = 0;
int gUSBPlugOutTick = 0;
int gUSBPlugInTick = 0;
int gUSBPlugInBatteryPercentageStableTimeTick = 2000; // 20 sec
int gUSBPlugOutBatteryPercentageStableTimeTick = 6000; // 60 sec
int ntx_get_battery_vol (void)
{
	if ( gTerryBatteryDebug )
	    printk ("[%s-%d]\n", __func__, __LINE__);
...
//			gLastBatValue = 0;
    ...
    if ( gTerryBatteryDebug )
	    printk("[%s %d] (gIsMSP430IntTriggered, gLastBatValue, gUSB_Change_Tick, (jiffies - gLastBatTick)) = (%d, %d, %d, %d)\n",__func__,__LINE__,gIsMSP430IntTriggered,gLastBatValue, gUSB_Change_Tick, (jiffies - gLastBatTick));

    if (gIsMSP430IntTriggered || !gLastBatValue || ((0 == gUSB_Change_Tick) && (200 < (jiffies - gLastBatTick))) && !g_pwr_key ) {
		
		gLastBatTick = jiffies+200;
		battValue = msp430_battery ();
		if( gTerryBatteryDebug )
            printk ("[%s-%d] battValue = %d\n", __func__, __LINE__,battValue);
		...
		if (battValue) {
			...
			if (gpio_get_value (GPIO_ACIN_PG)) {// not charging
				...
				if (0x02 & temp)
				{
					printk ("[%s-%d] =================> Micro P MSP430 Battery_Low(3.4V) <===================\n", __func__, __LINE__);
					gLastBatValue = 0;
					gBatteryPercentage = 0;
					return gBatteryPercentage;
				}
				if (0x01 & temp) {
					...
					gLastBatValue = battValue;
				else if ( g_last_shutdown_due_to_critical_low ) {
					if ( gTerryBatteryDebug )
					    printk("-=-=-=-=-=-=  use real batt value: due to last shutdown critical low !!\n");
					g_last_shutdown_due_to_critical_low = 0;
					gLastBatValue = battValue;
				} else {
			...
			gIsMSP430IntTriggered = 0; // Terry add 20130418 : reset to default
		}
		else {
			gLastBatTick = jiffies;
			printk ("[%s-%d] MSP430 read failed\n", __func__, __LINE__);
...
#ifdef NTX_BAT_CHG_SW_SIM
	if (10 == check_hardware_name() || 5 == check_hardware_name() || 30 ==check_hardware_name()) { //E606A2, E606E2 or E60672
		if (1010 <= battValue) {
			unsigned long now=get_seconds();
			if (ntx_charge_status() && !ntx_charge_96p_sec) {
				ntx_charge_96p_sec = now;
				//printk ("[%s-%d] charge start count.\n",__func__,__LINE__, ntx_charge_96p_sec);
			}
			battValue = 1023;	// report 100%
		}
		else {
			ntx_charge_96p_sec = 0;
		}
...
#ifdef BATTERY_PERCENTAGE_SMOOTH

	int current_battery_percentage = 100-result;

    //initial
	if ( gBatteryPercentage == 0 ) {
	    gBatteryPercentage = current_battery_percentage;
	}

    if ( gTerryBatteryDebug )
	    printk ("[%s-%d] gBatteryPercentage current_battery_percentage = %d %d\n", __func__, __LINE__, gBatteryPercentage,current_battery_percentage);

	if ( gpio_get_value (GPIO_ACIN_PG) ) {
		gLargeBatteryPercentageDiff = 0;

		if( (jiffies - gUSBPlugInTick) > gUSBPlugOutBatteryPercentageStableTimeTick ) {
			if ( gTerryBatteryDebug )
				printk ("[%s-%d] usb out after %d sec, use real battery percentage\n", __func__, __LINE__, gUSBPlugOutBatteryPercentageStableTimeTick/100);
            gBatteryPercentage = current_battery_percentage;

		}
		gUSBPlugOutTick = jiffies;


	} else {
		if ( (jiffies - gUSBPlugOutTick) > gUSBPlugInBatteryPercentageStableTimeTick ) {
			if ( gTerryBatteryDebug )
				printk ("[%s-%d] usb in after %d sec, use real battery percentage\n", __func__, __LINE__, gUSBPlugInBatteryPercentageStableTimeTick/100);
			if ( current_battery_percentage > gBatteryPercentage) {
				if (gLargeBatteryPercentageDiff > 0) {
					current_battery_percentage -= gLargeBatteryPercentageDiff;
				}

				if( (current_battery_percentage - gBatteryPercentage) > 3 && gLargeBatteryPercentageDiff == 0) {
					gLargeBatteryPercentageDiff = current_battery_percentage - gBatteryPercentage;
		...
		gUSBPlugInTick = jiffies;

	}

#else
    gBatteryPercentage = 100-result;
#endif

   if (gTerryBatteryDebug)
    	printk ("[%s-%d] gBatteryPercentage = %d\n", __func__, __LINE__, gBatteryPercentage);
	//printk ("[%s-%d] %d,bat=%d\n", __func__, __LINE__, (100-result),battValue);

	return gBatteryPercentage;
}

int g_USBWakeUp = 0; // Terry add 20130205 : just in case
...
int gLowBatteryISRCounter = 0;
int gLastUSBStat = -1;
static struct workqueue_struct *gWorkQueue;
struct delayed_work gMsp430CheckIntTypeWork;
static struct wake_lock msp430_wakeup;
#if 0
static irqreturn_t msp_int(int irq, void *dev_id)
{
	//usbStat ==1 not plugged in, 0=plugged in
	int usbStat = gpio_get_value (GPIO_ACIN_PG);
	//printk ("[%s-%d] MSP430 interrupt triggered !!!\n",__func__,__LINE__);
	printk ("[%s-%d] MSP430 interrupt triggered usb=%d gLowBatteryISRCounter=%d battPer=%d g_wakeup_by_alarm=%d gSleep_Mode_Suspend=%d\n",__func__, __LINE__, usbStat, gLowBatteryISRCounter, gBatteryPercentage, g_wakeup_by_alarm, gSleep_Mode_Suspend);
	gIsMSP430IntTriggered = 1;
	printk ("[%s-%d] assign gIsMSP430IntTriggered = %d\n",__func__,__LINE__,gIsMSP430IntTriggered);
	ntx_charger_online_event_callback ();	// update charge / battery status
	if (g_wakeup_by_alarm != 1 && gSleep_Mode_Suspend && (gLastUSBStat == usbStat) || gBatteryPercentage <= 8)
		gLowBatteryISRCounter++;
	if( gLowBatteryISRCounter == 1 || (gLowBatteryISRCounter %20 == 0) && gLowBatteryISRCounter != 0 && usbStat==1)
	{
		printk ("[%s-%d], LowBatteryISRCounter=%d, global battPercent=%d, sleep=%d\n", __func__, __LINE__, gLowBatteryISRCounter, gBatteryPercentage, gSleep_Mode_Suspend);
		mxc_kpp_report_power(1);
		mxc_kpp_report_power(0);
	}

	gLastUSBStat = usbStat;
	return 0;
}
#else
static irqreturn_t msp_int(int irq, void *dev_id)
{
   ...
   //Terry add 20130506 : do not invoke ISR while booting
	if (gFinishKernelBoot) {
		gIsMSP430IntTriggered = 1;
	    wake_lock_timeout(&msp430_wakeup,300*HZ); // Terry add 20130418 : let system does not go to hiberation mode until queue work finish
		ntx_charger_online_event_callback ();
		queue_delayed_work(gWorkQueue, &gMsp430CheckIntTypeWork, HZ/2);
	}
...
//Terry add 20130418 : check msp430 interrupt type
static void check_msp430_interrupt_type () {
	if (gTerryBatteryDebug)
        printk("[%s -- %d]\n",__func__,__LINE__);
	...
	printk ("[%s-%d] Battery status register=0x%X\n", __func__, __LINE__, temp);
    if (0x8000 & temp) {
		printk ("[%s-%d] =================> Micro P MSP430 alarm triggered <===================\n", __func__, __LINE__);
		g_wakeup_by_alarm = 1;
	}
	if (0x02 & temp) {
		gLowBatteryISRCounter++;
		printk ("[%s-%d] =================> Micro P MSP430 Battery_Low(3.4V) ISRCounter=%d\n", __func__, __LINE__, gLowBatteryISRCounter);
	}
	if (0x01 & temp) {
	    printk ("[%s-%d] =================> Micro P MSP430 Critical_Battery_Low(3.2V)<===================\n", __func__, __LINE__);
		gLowBatteryISRCounter++;
	}

	if ( gSleep_Mode_Suspend && (gLowBatteryISRCounter == 1 || (gLowBatteryISRCounter %20 == 0) && gLowBatteryISRCounter != 0 && g_wakeup_by_alarm !=1) ) {
	    mxc_kpp_report_power(1);
		mxc_kpp_report_power(0);
	}
...
static int gpio_initials(void)
{
	...
	// Turn on internal SD/eMMC power (models with SPD_EN switch)  		
	if(34==gptHWCFG->m_val.bPCB || 35==gptHWCFG->m_val.bPCB || 41==gptHWCFG->m_val.bPCB) {	// E606FXA , E606FXB , E606G0
		mxc_iomux_v3_setup_pad(MX50_PAD_ECSPI1_MOSI__GPIO_4_13);
		gpio_request((3*32+13), "GPIO_4_13");
		gpio_direction_output(SPD_EN,1);
	}
 
 if(45==gptHWCFG->m_val.bPCB) { // E606H0
  mxc_iomux_v3_setup_pad(MX50_PAD_EIM_LBA__GPIO_1_26);
  gpio_request(GPIO_ISD_PWR, "GPIO_ISD_PWR");
  gpio_direction_output(GPIO_ISD_PWR, 0);
 }



	if(28==gptHWCFG->m_val.bPCB || 34==gptHWCFG->m_val.bPCB || 35==gptHWCFG->m_val.bPCB || 41==gptHWCFG->m_val.bPCB) {
		// E606CX, E606FXA, E606FXB , E606G2
...
#if 1 // Terry add 20130128 : we dont need acin ISR because we use msp_int to change charger state
// Terry add 20140422 : for fast react when usb in/out
      // Terry add 20130205 : Bugfix : AC charger can not wake up Android system
	{
		...
		if (!gpio_get_value (GPIO_ACIN_PG)) {
			printk("[%s %d] AC WAKELOCK ACQUIRE\n",__func__, __LINE__);
			wake_lock(&acin_lock);
		}
	...
	if( !(gptHWCFG->m_val.bPCB == 30 || gptHWCFG->m_val.bPCB == 45) ) {  // !E606E2,!E606H2
...
//	if ((1 == check_hardware_name()) || (10 == check_hardware_name()) || (14 == check_hardware_name())) 	// E60612 , E606A2 ,E606B2 , ir touch 
	...
	{    //IR touch
		...
		if (41==gptHWCFG->m_val.bPCB) {
			...
			msleep(20);
			gpio_direction_input(TOUCH_RST_5_12);
		} 
		else {
			// Touch reset
	...
	switch (gptHWCFG->m_val.bPCB) {
		...
		{
			// E60610D 
			printk("set ESD drive strength Medium\n");
		...
		case 28: //E606C0
		{
			printk("set ESD driver strength Low /ISD Medium\n");
			// External SD .
		...
		case 41: //E606G0
		{
			printk("Set ISD/eMMC drive strength Medium, ESD Low\n");
			mxc_iomux_v3_setup_pad(MX50_PAD_SD3_CMD__SD3_CMD_DSM);
			mxc_iomux_v3_setup_pad(MX50_PAD_SD3_CLK__SD3_CLK_DSM);
			mxc_iomux_v3_setup_pad(MX50_PAD_SD3_D0__SD3_D0_DSM);
			mxc_iomux_v3_setup_pad(MX50_PAD_SD3_D1__SD3_D1_DSM);
	...
	if( 0 != gptHWCFG->m_val.bFrontLight ){
		if( 41==gptHWCFG->m_val.bPCB ) {
                        mxc_iomux_v3_setup_pad(MX50_PAD_ECSPI1_MISO__GPIO_4_14_100KUP);
                } else {
...
//	enable_irq_wake(irq);
  ...
  if(NTXHWCFG_TST_FLAG(gptHWCFG->m_val.bPCB_Flags,0)){
		...
		}else if(gptHWCFG->m_val.bPCB == 28 || gptHWCFG->m_val.bPCB == 41) {  // E606C2, E606G0			
			mxc_iomux_v3_setup_pad(MX50_PAD_KEY_COL0__GPIO_4_0);
			gpio_request(GPIO_KEY_COL_0, "GPIO_KEY_COL_0");
			gpio_direction_output(GPIO_KEY_COL_0,0);
	...
	gLastUSBStat = gpio_get_value (GPIO_ACIN_PG);
...
*/
	...
	if (gSleep_Mode_Suspend && (41==gptHWCFG->m_val.bPCB && 3!=gptHWCFG->m_val.bUIConfig)) {
		mdelay (20);
		disable_irq(gpio_to_irq(TOUCH_INT));
	...
	if (gSleep_Mode_Suspend&&(35==gptHWCFG->m_val.bPCB || 34==gptHWCFG->m_val.bPCB || 41==gptHWCFG->m_val.bPCB || 45==gptHWCFG->m_val.bPCB))
	{// 
		// E606F0/E606F0B/E606G0/E606H2 
		//
		// for better power consumption H/W design .

		
	
		if(45!=gptHWCFG->m_val.bPCB) {
			tps65185_ONOFF(0);
			gpio_direction_output(SPD_EN,0);
		}

		if(34==gptHWCFG->m_val.bPCB||45==gptHWCFG->m_val.bPCB)
		{
			...
			{  //eMMC
				...
				mxc_iomux_v3_setup_multiple_pads(mx50_sd3_hi4bits_disable_pads, gdw_mx50_sd3_hi4bits_disable_pads);
				for(i=0;i<ARRAY_SIZE(mx50_sd3_hi4bits_gpioA);i++) {
					gpio_direction_input(mx50_sd3_hi4bits_gpioA[i]);
				}
			}
		}
	}
	...
	if (45==gptHWCFG->m_val.bPCB)
		gpio_direction_output(GPIO_ISD_PWR, gSleep_Mode_Suspend?1:0);
...
void ntx_gpio_resume (void)
{
	...
	if (45==gptHWCFG->m_val.bPCB)
		gpio_direction_output(GPIO_ISD_PWR, gSleep_Mode_Suspend?0:1);
	...
	if (gSleep_Mode_Suspend&&(35==gptHWCFG->m_val.bPCB || 34==gptHWCFG->m_val.bPCB || 41==gptHWCFG->m_val.bPCB||45==gptHWCFG->m_val.bPCB) ) 
	{
		...
		if(34==gptHWCFG->m_val.bPCB||45==gptHWCFG->m_val.bPCB) {
			...
			mxc_iomux_v3_setup_multiple_pads(mx50_sd3_enable_pads, gdw_mx50_sd3_enable_pads);
			if(2==gptHWCFG->m_val.bIFlash) {  //eMMC
				mxc_iomux_v3_setup_multiple_pads(mx50_sd3_hi4bits_enable_pads, gdw_mx50_sd3_hi4bits_enable_pads);
			}
		}
	...
	if (gSleep_Mode_Suspend && (41==gptHWCFG->m_val.bPCB && 3!=gptHWCFG->m_val.bUIConfig)) {
		...
		mdelay (50);
		gpio_direction_input(TOUCH_INT);
		enable_irq(gpio_to_irq(TOUCH_INT));
	}
...
//	gpio_direction_output(GPIO_AUDIO_PWR, 0);
	...
	wifi_sdio_enable (1);
	gpio_direction_output(GPIO_WIFI_3V3,0);
	gpio_direction_output(GPIO_WIFI_RST,1);
#endif
}
...
static int __init initDriver(void)
{
	...
	wake_lock_init(&msp430_wakeup, WAKE_LOCK_SUSPEND, "msp430_wakeups");
	wake_lock_init(&g_PowerKeyWakeLock, WAKE_LOCK_SUSPEND, "powerkey_wakelock");
	wake_lock_init(&acin_lock, WAKE_LOCK_SUSPEND, "acin_wakelock");

	//Terry add	20140502
	INIT_DELAYED_WORK(&acin_wakelock_work, wakelock_release_thread);
	acin_wakelock_wq = create_singlethread_workqueue("mxc_acin_wakelock");
	if (!acin_wakelock_wq) {
		return -ESRCH;
	}
	...
	//printk("======%s() PCB flags=0x%x.=====\n",__FUNCTION__,gptHWCFG->m_val.bPCB_Flags);
	if(NTXHWCFG_TST_FLAG(gptHWCFG->m_val.bPCB_Flags,0)){

		if(gptHWCFG->m_val.bPCB == 30 || gptHWCFG->m_val.bPCB == 45) {   //E606E2, E606H2
#ifdef CONFIG_ANDROID // [
			//ntx_gpio_buttons[gi_ntx_gpio_buttons_total].code=KEY_HOME; //gallen orig
          ntx_gpio_buttons[gi_ntx_gpio_buttons_total].code=KEY_F2;
#else //][! CONFIG_ANDROID
			ntx_gpio_buttons[gi_ntx_gpio_buttons_total].code=KEY_FL;
#endif //]  CONFIG_ANDRIOD

			ntx_gpio_buttons[gi_ntx_gpio_buttons_total].gpio=GPIO_KEY_FL;
...
#ifdef CONFIG_ANDROID
			//ntx_gpio_buttons[gi_ntx_gpio_buttons_total].code=KEY_F1;  // gallen orig 
			ntx_gpio_buttons[gi_ntx_gpio_buttons_total].code=KEY_HOME;  
#else //][!CONFIG_ANDROID
			ntx_gpio_buttons[gi_ntx_gpio_buttons_total].code=KEY_F3;  // KEY_HOME 61
#endif //] CONFIG_ANDROID
			//ntx_gpio_buttons[gi_ntx_gpio_buttons_total].gpio=GPIO_KEY_ROW_0; //gallen orig
          ntx_gpio_buttons[gi_ntx_gpio_buttons_total].gpio=GPIO_KEY_HOME; 
			ntx_gpio_buttons[gi_ntx_gpio_buttons_total].active_low=1;
			ntx_gpio_buttons[gi_ntx_gpio_buttons_total].type=EV_KEY;
			ntx_gpio_buttons[gi_ntx_gpio_buttons_total].wakeup=1;
			ntx_gpio_buttons[gi_ntx_gpio_buttons_total].debounce_interval=30;
		...
		if(gptHWCFG->m_val.bPCB == 28 || gptHWCFG->m_val.bPCB == 34 || gptHWCFG->m_val.bPCB == 35 || gptHWCFG->m_val.bPCB == 41) {   //E606C2, E606F2 , E606G0	
			ntx_gpio_buttons[gi_ntx_gpio_buttons_total].code=KEY_FL; 
		...
		//printk("======%s() gpio keys total=%d.=====\n",__FUNCTION__,gi_ntx_gpio_buttons_total);
	...
	gWorkQueue = create_singlethread_workqueue("mxc_msp430");
	if ( gWorkQueue ) {
		INIT_DELAYED_WORK(&gMsp430CheckIntTypeWork, check_msp430_interrupt_type);
	} else {
	    printk(" gWorkQueue fail initial !!\n");
	}

	gFrontLightWorkQuene = create_singlethread_workqueue("mxc_frontlight");
	if ( gFrontLightWorkQuene ) {
		INIT_DELAYED_WORK(&gFrontLightDelayWork, frontlight_delayfunc);
	} else {
	    printk(" gFrontLightWorkQuene fail initial !!\n");
	}
