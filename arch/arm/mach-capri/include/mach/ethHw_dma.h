...
#ifdef HW_IO_VIRT_TO_PHYS
#define ETH_IO_VIRT_TO_PHYS HW_IO_VIRT_TO_PHYS
#else
#define ETH_IO_VIRT_TO_PHYS(va) (va - (uint32_t)KONA_ESW_VA + (uint32_t)ESW_CONTRL_BASE_ADDR)
#endif
...
#define ETHHW_DMA_MTP_FIFO_ADDR_CFG    ETH_IO_VIRT_TO_PHYS( (uint32_t)&ethHw_regMtpFifoConfig )
#define ETHHW_DMA_MTP_FIFO_ADDR_DATA   ETH_IO_VIRT_TO_PHYS( (uint32_t)&ethHw_regMtpFifoData )
...
#define ETHHW_DMA_MTP_DSTAT            ETH_IO_VIRT_TO_PHYS( (uint32_t)&ethHw_regMtpDmaStatus )

#define ETHHW_DMA_PTM_FIFO_ADDR        ETH_IO_VIRT_TO_PHYS( (uint32_t)&ethHw_regPtmFifoConfig )
...
#define ETHHW_DMA_PTM_CFG_HI           (dmacHw_REG_CFG_HI_UPDATE_SRC_STAT | \
                                        dmacHw_REG_CFG_HI_AHB_HPROT_1 | \
                                        dmacHw_REG_CFG_HI_FIFO_ENOUGH)
#define ETHHW_DMA_PTM_SSTAT            ETH_IO_VIRT_TO_PHYS( (uint32_t)&ethHw_regPtmDmaStatus )

#define ETHHW_DMA_BUF_LEN(descp)       (((descp)->stat1 >> 13) & 0x0000ffff)
#define ETHHW_DMA_CTL_INT_EN           dmacHw_REG_CTL_INT_EN

/* NOTE: Hardware bug work-around for PTM transfers
*     The done bit in ctlHi may be updated before the status in stat1, so
*     stat1 must be cleared when the descriptor is created and stat1 will
*     be non-zero when PTM DMA transfer is completed
*/
#define ETHHW_DMA_PTM_TRANSFER_DONE(descp)   ((descp)->stat1 && ((descp)->ctlHi & dmacHw_REG_CTL_DONE))
#define ETHHW_DMA_MTP_TRANSFER_DONE(descp)   ((descp)->ctlHi & dmacHw_REG_CTL_DONE)
...
static inline int ethHw_dmaRxConfig(int burst, int block, int offset)
{
	reg32_write( &ethHw_regPtmConfig,
			...
			ETHHW_REG_PTM_CONFIG_OFFSET( offset ) |
			ETHHW_REG_PTM_CONFIG_INITIAL_OFS_ENABLE );
