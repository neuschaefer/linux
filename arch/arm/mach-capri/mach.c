...
static int l2_disable_scu_standby = 0;
...
static int __init l2_enable_idle_delay_setup(char *str)
{
	l2_disable_idle_delay = 0;
	return 1;
}
__setup("l2_enable_idle_delay", l2_enable_idle_delay_setup);

static int __init l2_disable_scu_standby_setup(char *str)
{
	l2_disable_scu_standby = 1;
	return 1;
}
__setup("l2_disable_scu_standby", l2_disable_scu_standby_setup);

extern void enable_l2_service_call(void);
...
static int __init mach_l2x0_init(void)
{
	...
	if ( l2_disable_idle_delay )
	{
		...
		void __iomem *arm_sys_idle_delay = (void __iomem *)(KONA_PROC_CLK_VA + KPROC_CLK_MGR_REG_ARM_SYS_IDLE_DLY_OFFSET);
		void __iomem *kproc_wr_access    = (void __iomem *)(KONA_PROC_CLK_VA + KPROC_CLK_MGR_REG_WR_ACCESS_OFFSET);

		writel( (0xA5A5 << KPROC_CLK_MGR_REG_WR_ACCESS_PASSWORD_SHIFT)
				  | KPROC_CLK_MGR_REG_WR_ACCESS_CLKMGR_ACC_MASK, kproc_wr_access );
	...
	if ( l2_disable_linefill )
	{
		...
		void __iomem *l2_debug_ctrl = (void __iomem *)(KONA_L2C_VA + L2X0_DEBUG_CTRL);
	...
	if ( l2_disable_scu_standby )
	{
		...
		void __iomem	*scu_control_reg = (void __iomem *)(KONA_SCU_VA + SCU_CONTROL_OFFSET);

		writel( readl( scu_control_reg ) & ~SCU_CONTROL_SCU_STANDBY_EN_MASK, scu_control_reg );
	...
	/* Clear exlusive bit */
	aux_mask &= ~( 1 << L2X0_AUX_CTRL_EXCLUSIVE_SHIFT );
	...
	secure_api_call(SSAPI_ENABLE_L2_CACHE, 0, 0, 0);
#endif
	if (*(uint32_t*)KONA_SRAM_VA == 0xBADDCAFE)
		enable_l2_service_call();
...
#ifdef CONFIG_KONA_ATAG_DT
	printk(KERN_INFO "pinmux_gpio_mask: 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x\n",
		dt_pinmux_gpio_mask[0], dt_pinmux_gpio_mask[1], dt_pinmux_gpio_mask[2],
		dt_pinmux_gpio_mask[3], dt_pinmux_gpio_mask[4], dt_pinmux_gpio_mask[5]);
#endif
