...
int capri_wifi_status_register(void (*callback) (int card_present, void *dev_id),
			      void *dev_id);

EXPORT_SYMBOL(capri_wifi_status_register);

int omap4_capri_get_type(void);
...
{
	pr_debug("%s: %d\n", __func__, val);
	printk(KERN_ERR " %s INSIDE capri_wifi_set_carddetect\n", __func__);
	capri_wifi_cd = val;
	if (wifi_status_cb) {
...
static struct regulator_consumer_supply capri_vmmc5_supply = {
	.supply = "vmmc",
	.dev_name = "omap_hsmmc.4",
};

static struct regulator_init_data capri_vmmc5 = {
	.constraints = {
			.valid_ops_mask = REGULATOR_CHANGE_STATUS,
			},
	.num_consumer_supplies = 1,
	.consumer_supplies = &capri_vmmc5_supply,
};

static struct fixed_voltage_config capri_vwlan = {
	.supply_name = "vwl1271",
...
static struct platform_device omap_vwlan_device = {
	...
	.id = 1,
	.dev = {
		.platform_data = &capri_vwlan,
		},
};
#endif

static int capri_wifi_power(int on)
{
	printk(KERN_ERR " %s INSIDE capri_wifi_power\n", __func__);
#if 0
	if (!clk32kaudio_reg) {
	...
	if (clk32kaudio_reg && on && !capri_wifi_power_state)
