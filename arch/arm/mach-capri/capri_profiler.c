...
DEFINE_CCU_PROFILER(kproc) = {
	...
	.clk_dev_id = KPROC_CCU_CLK_NAME_STR,
	.ctrl_offset = KPROC_CLK_MGR_REG_KPROC_CCU_PROF_CTL_OFFSET,
	.policy_sel_offset = KPROC_CLK_MGR_REG_KPROC_CCU_PROF_SEL_OFFSET,
	.auto_gate_sel0_offset = KPROC_CLK_MGR_REG_KPROC_CCU_PROF_SEL_OFFSET,
	.clk_req_sel0_offset = KPROC_CLK_MGR_REG_KPROC_CCU_PROF_SEL_OFFSET,
	.counter_offset = KPROC_CLK_MGR_REG_KPROC_CCU_PROF_CNT_OFFSET,
	.cntr_start_mask =
...
static int __init capri_profiler_register(void)
{
	...
	for (idx = 0; idx < ARRAY_SIZE(capri_ccu_profiler_tbl); idx++)
		ccu_profiler_register(capri_ccu_profiler_tbl[idx]);
	/**
	 * Register all PI ON profilers
	 */
	for (idx = 0; idx < ARRAY_SIZE(capri_pi_profiler_tbl); idx++)
		pi_profiler_register(capri_pi_profiler_tbl[idx]);
	return 0;
}
