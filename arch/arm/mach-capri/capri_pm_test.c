...
static int set_pm_gpio_event(void *data, u64 val)
{
	...
	pwr_mgr_register_event_handler(GPIO_102_A_EVENT, gpioCallback, test_string);
	return 0;
}
...
int __init capri_pm_test_init(void)
{
	...
	if (!debugfs_create_file("set_gpio_event", S_IWUSR|S_IRUSR, dent_pm_test_dir, NULL, &pm_set_gpio_event))
		return -ENOMEM;
