...
static const DMA_INSTR_ENCODE_t dmacInstEncode[ DMAC_INST_MAX + 1 ] = {
   ...
   DECLARE_INST(DMAWFPP,   0x31,   2), /* Handles both burst and single transfer */
...
static int chal_dma_prepare_peripheral_controlled_transfer
(
   CHAL_CHANNEL_HANDLE handle
);
...
)
{
   ...
   switch (inst) 
   {
      ...
      case DMAC_INST_DMAWFPP:
         ...
         (*ucode)[1] = (dmacInstEncode[DMAC_INST_DMAWFPP].inst_enc>>8) | ( para1<< 3);
...
)
{
   ...
   if ( ( pChanInfo->descType == CHAL_DMA_DESC_RING ) || ( pChanInfo->descType == CHAL_DMA_DESC_PCTL ) )
   {
      if((src_inc) && (dst_inc))
      {
        /* memory to memory */
...
)
{
   ...
   if ( pChanInfo->curDescCount == 1)
   {
      ...
      /* mov CCR */
      chal_dma_encode_instruction ( &inst, DMAC_INST_DMAMOV, 1, chanDesc[0].control.control );
      /* mov SRC */
      chal_dma_encode_instruction ( &inst, DMAC_INST_DMAMOV, 0, chanDesc[0].src );
      /* mov DST */
      chal_dma_encode_instruction ( &inst, DMAC_INST_DMAMOV, 2, chanDesc[0].dst );
	 ...
	 else if ( dst_inc )
	 {
	    ...
	    chal_dma_encode_instruction ( &inst, DMAC_INST_DMAWFPP, pChanInfo->config.ChanCtrlBitField.srcBurstLen, peripheralId );
	    /* DMALDP */
	    chal_dma_encode_instruction ( &inst, DMAC_INST_DMALDP, pChanInfo->config.ChanCtrlBitField.srcBurstLen, peripheralId );
	 }
	 else
	 ...
	 {
	    /* DMAWFPP (wait for single or burst request)*/
	    chal_dma_encode_instruction ( &inst, DMAC_INST_DMAWFPP, pChanInfo->config.ChanCtrlBitField.dstBurstLen, peripheralId );
            /* DMASTP */
	    chal_dma_encode_instruction ( &inst, DMAC_INST_DMASTP, pChanInfo->config.ChanCtrlBitField.dstBurstLen, peripheralId );
	 }
	 else
