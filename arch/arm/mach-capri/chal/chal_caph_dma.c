...
#define CHAL_CAPH_DMA_HI_CH_REG_SIZE    ((CPH_AADMAC_CH2_AADMAC_CR_3_OFFSET - CPH_AADMAC_CH1_AADMAC_CR_3_OFFSET)/sizeof(uint32_t))
...
CHAL_HANDLE chal_caph_dma_init(uint32_t baseAddress)
{
    ...
    for(ch=0; ch < CHAL_CAPH_DMA_MAX_CHANNELS; ch++)
    {
        ...
        reg_val = BRCM_READ_REG_IDX( baseAddress,  CPH_AADMAC_CH1_AADMAC_CR_2, (ch*CHAL_CAPH_DMA_CH_REG_SIZE));
        reg_val &= ~CPH_AADMAC_CH1_AADMAC_CR_2_CH1_AADMAC_WRAP_MASK;
	reg_val |= (0x100 << CPH_AADMAC_CH1_AADMAC_CR_2_CH1_AADMAC_WRAP_SHIFT);
        BRCM_WRITE_REG_IDX(baseAddress,  CPH_AADMAC_CH1_AADMAC_CR_2, (ch*CHAL_CAPH_DMA_CH_REG_SIZE), reg_val);
    }
...
*
****************************************************************************/
void chal_caph_dma_set_hibuffer(CHAL_HANDLE handle, CAPH_DMA_CHANNEL_e  channel, uint32_t address)
{
    uint32_t     base = ((chal_caph_dma_cb_t*)handle)->base;
    uint8_t      index;
    uint32_t     cr;
    ...
    for(index = 0; index < CHAL_CAPH_DMA_MAX_CHANNELS; index++)
    {
        if((1UL << index)&channel)
        {
            ...
            BRCM_WRITE_REG_IDX( base,  CPH_AADMAC_CH1_AADMAC_CR_3, (index*CHAL_CAPH_DMA_HI_CH_REG_SIZE), address);
