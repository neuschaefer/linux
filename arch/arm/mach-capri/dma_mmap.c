...
static void CacheMaintenance(const void *virtAddr,
			     dma_addr_t physAddr,
			     size_t numBytes,
			     int direction)
{
...
static inline void SyncCpuToDev(const void *virtAddr,
				dma_addr_t physAddr,
				size_t numBytes,
				int direction)
{
...
static inline void SyncDevToCpu(const void *virtAddr,
				dma_addr_t physAddr,
				size_t numBytes,
				int direction)
{
 ...
 */
int dma_mmap_start(DMA_MMAP_CFG_T *memMap,
 ...
 * @return     0 on success, error code otherwise
 */
int dma_mmap_add_region(DMA_MMAP_CFG_T *memMap,
			void *mem,
			size_t numBytes)
{
 ...
 */
int dma_mmap_unmap(DMA_MMAP_CFG_T *memMap,
		   DMA_MMAP_DIRTIED_T dirtied)
{
	int rc = 0;
	int regionIdx;
	int segmentIdx;
...
void dma_mmap_memcpy(DMA_MMAP_CFG_T *memMap, void *mem)
{
	...
	for (regionIdx = 0; regionIdx < memMap->numRegionsUsed; regionIdx++) {
		...
		if (region->memType == DMA_MMAP_TYPE_USER) {
			...
			for (pageIdx = 1; pageIdx < pagelist->numLockedPages;
			     pageIdx++) {
				size_t bytesThisPage =
				    (bytesRemaining >
				     PAGE_SIZE ? PAGE_SIZE : bytesRemaining);
		...
		int *eof, void *data)
{
	...
	len +=
	    sprintf(buffer + len, "Debug print is %s\n",
