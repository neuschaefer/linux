...
struct mm_ctrl capri_mm_ctrl = {
	.mm_ctrl_reg = KONA_CHIPREG_VA + CHIPREG_PERIPH_MISC_REG2_OFFSET,
	.mm_crystal_clk_is_idle_mask = CHIPREG_PERIPH_MISC_REG2_MM_CRYSTAL_CLK_IS_IDLE_MASK,
};
...
int __init capri_pwr_mgr_init()
{
	...
	{
	    int sts, clkVal, regVal, reg_val;
	    /*select PMU_BSC clock to 26Mhz*/
	    reg_val = CLK_WR_ACCESS_PASSWORD << CLK_WR_PASSWORD_SHIFT;
	    writel(reg_val | CLK_WR_ACCESS_EN, KONA_AON_CLK_VA + KHUBAON_CLK_MGR_REG_WR_ACCESS_OFFSET);
	    /*set rate to 26Mhz*/
	    clkVal = readl(KONA_AON_CLK_VA + KHUBAON_CLK_MGR_REG_PMU_BSC_DIV_OFFSET);
	    clkVal &= ~KHUBAON_CLK_MGR_REG_PMU_BSC_DIV_PMU_BSC_DIV_MASK;
	    writel(clkVal, KONA_AON_CLK_VA + KHUBAON_CLK_MGR_REG_PMU_BSC_DIV_OFFSET);
	    /* Trigger the new clock frequency */
	    regVal = readl(KONA_AON_CLK_VA + KHUBAON_CLK_MGR_REG_PERIPH_SEG_TRG_OFFSET);
	    regVal &= ~KHUBAON_CLK_MGR_REG_PERIPH_SEG_TRG_PRIV_ACCESS_MODE_MASK;
	    regVal |= KHUBAON_CLK_MGR_REG_PERIPH_SEG_TRG_PMU_BSC_TRIGGER_MASK;
	    writel(regVal, (KONA_AON_CLK_VA + KHUBAON_CLK_MGR_REG_PERIPH_SEG_TRG_OFFSET));
	    while ((readl(KONA_AON_CLK_VA + KHUBAON_CLK_MGR_REG_PERIPH_SEG_TRG_OFFSET) &
			KHUBAON_CLK_MGR_REG_PERIPH_SEG_TRG_PMU_BSC_TRIGGER_MASK))
		;

	    writel(reg_val, KONA_AON_CLK_VA + KHUBAON_CLK_MGR_REG_WR_ACCESS_OFFSET);
	    /* Configure PMU_BSC i2c timing to HS mode*/
	    pwmgr_pmu_bsc_init(PMU_BSC_BASE_ADDR, PMU_BSC_BASE_ADDR + 0x100 - 1, BCM_INT_ID_PM_I2C, &sts);
	    if (sts)
 ...
 * team is to set the POWER_OK_MASK bit to 0 */
	...
	for (i = 0; i < ARRAY_SIZE(event_table); i++) {
		...
		pwr_mgr_event_set_pi_policy(event_id, PI_MGR_PI_ID_ESUB_SUB, &cfg);
#endif
		...
		pwr_mgr_event_set_pi_policy(event_id, PI_MGR_PI_ID_MM_SUB, &cfg);

		cfg.policy = event_table[i].policy_mm_sub2;
		pwr_mgr_event_set_pi_policy(event_id, PI_MGR_PI_ID_MM_SUB2, &cfg);
