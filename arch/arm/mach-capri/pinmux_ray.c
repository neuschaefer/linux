...
const struct pin_group pin_config_list[] = {
   ...
   PIN_GROUP_CFG(PG_ETMTRACE,   etm_trace_config,        ARRAY_LEN(etm_trace_config) ),
   PIN_GROUP_CFG(PG_SDIO2,      sdio2_config,            ARRAY_LEN(sdio2_config) ),
   PIN_GROUP_CFG(PG_SIM1,       sim1_config,             ARRAY_LEN(sim1_config) ),
   PIN_GROUP_CFG(PG_KEYPAD_2X3, keypad_2x3_config,       ARRAY_LEN(keypad_2x3_config) ),
   ...
   PIN_GROUP_CFG(PG_DIGIMIC,    digital_mic_config,      ARRAY_LEN(digital_mic_config) ),
   PIN_GROUP_CFG(PG_PMU1,       pmu1_config,             ARRAY_LEN(pmu1_config) ),
   PIN_GROUP_CFG(PG_USB_HOST1,  usb_host1_config,        ARRAY_LEN(usb_host1_config) ),
   ...
   PIN_GROUP_CFG(PG_CAM_FLASH,  cam_flash_config,        ARRAY_LEN(cam_flash_config) ),
   PIN_GROUP_CFG(PG_NFC,        nfc_config,              ARRAY_LEN(nfc_config) ),
   PIN_GROUP_CFG(PG_USB_HSIC,   usb_hsic_config,         ARRAY_LEN(usb_hsic_config) ),
   PIN_GROUP_CFG(PG_STATUS_LED, status_led_config,       ARRAY_LEN(status_led_config) ),
   ...
   PIN_GROUP_CFG(PG_USB_HOST2,  usb_host2_config,        ARRAY_LEN(usb_host2_config) ),
   PIN_GROUP_CFG(PG_CAM_PWM,    cam_pwm_config,          ARRAY_LEN(cam_pwm_config) ),
   PIN_GROUP_CFG(PG_BATT_CHRG,  battery_charging_config, ARRAY_LEN(battery_charging_config) ),

   /* Tertiary Pin Groups */
   PIN_GROUP_CFG(PG_DEBUG_BUS,  debug_bus_config,        ARRAY_LEN(debug_bus_config) ),
   PIN_GROUP_CFG(PG_KEYPAD_8X8, keypad_8x8_config,       ARRAY_LEN(keypad_8x8_config) ),
...
int pinmux_block_init(int group_id)
{
    ...
    if (index != -1)
    {
        struct pin_config *block_pin_config = pin_config_list[index].listp;
