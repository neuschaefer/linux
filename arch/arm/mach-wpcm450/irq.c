...
//	if(irqno == IRQ_TIMER0)
 //  		__raw_writel(0x01, WPCM450_TISR); /* clear TIF0 */
    ...
    /* write any value to indicate the end of interrupt service */
...
static struct irq_chip wpcm_irq_edge_chip = {
    .name      = "WPCM_IRQ_EDGE",
...
static struct irq_chip wpcm_irq_chip = {
    .name      = "WPCM_IRQ",
	.ack	   = wpcm_irq_ack,
	.mask	   = wpcm_irq_mask,
	.unmask	   = wpcm_irq_unmask,
...
static struct irq_chip wpcm_irq_maskack_chip = {
    .name      = "WPCM_IRQ_MASKACK",    
	.ack	   = wpcm_irq_maskack,
	.mask	   = wpcm_irq_mask,
	.unmask	   = wpcm_irq_unmask,
//	.set_wake	   = NULL
...
void __init wpcm450_init_irq(void)
{
	...
	for (irqno = IRQ_WDT; irqno <= IRQ_ADC; irqno++) {
		...
		default:
			...
			set_irq_chip(irqno, &wpcm_irq_maskack_chip);
