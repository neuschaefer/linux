...
#define pr_info
#endif
/* MJF: this really should come from mach-capri/<some austin header> */
#if defined(CONFIG_MACH_CAPRI_ROKU_AUSTIN)
#define CAPRI_ROKU_AUSTIN_LED 12
#endif
...
#if defined(CONFIG_MACH_CAPRI_ROKU_AUSTIN)
	/* MJF: ROKU apps manipulate the led by setting/clearing the
	 * gpio.  Austin is wired backwards so that 0 = on. 1 could be
	 * off, but that cause current draw due to the pull down.  So
	 * for Austin, we switch the gpio's mode. No apps were changed.
	 */
	if (gpio == CAPRI_ROKU_AUSTIN_LED) {
		if (value)	/* turn it on. set it to an input */
			kona_gpio_direction_input(chip, gpio);
		else	/* turn it off. set it as an output of 1 */
			kona_gpio_direction_output(chip, gpio, 1);
	} else {
#endif
...
#if defined(CONFIG_MACH_CAPRI_ROKU_AUSTIN)
	/* MJF: ROKU apps manipulate the led by setting/clearing the
	 * gpio.  Austin is wired 'funny' so that if the pin is an output,
	 * than it means 'on', but if the pin is an input, it means 'off'
	 */
	if (gpio == CAPRI_ROKU_AUSTIN_LED)
	{
		if (GPIO_GPCTR0_IOTR_CMD_INPUT == val)
