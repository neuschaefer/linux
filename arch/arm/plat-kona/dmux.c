...
static const struct dmac_mux_map kona_dmux_map[] = {
	...
	{.name = DMA_CHAN_CIR_TX,.id = KONA_DMACH_CIR_TX,.mux =
	 KONA_DMUX_CIR_TX},
	{.name = DMA_CHAN_CIR_RX,.id = KONA_DMACH_CIR_RX,.mux =
	 KONA_DMUX_CIR_RX},
...
__init int dmux_init(void)
{
	...
	/* The bootrom allocates 2 channels in normal bootup mode for authentication.
	 * They are not freed to allow later stages to use it's API. Now in the
	 * kernel we are long past that point of needing it and can release these
	 * channels. But we want to rig it so that the allocation is 0-7 instead of
	 * 2-7,0,1 to eliminate any differences between SWDEV mode and normal mode.
	 */
	{
		int i;
		volatile u32 *dmuxp = (volatile u32 *)(dmux.base_addr);
		u32 val;
		/* flush out all channels */
		while (1)
		{
			val = *dmuxp;
			if (val == 0xf)
		...
		/* reinit available channels in 0-7 alloc order */
		for (i=0; i<8; i++)
		{
			*dmuxp = i;
		}
