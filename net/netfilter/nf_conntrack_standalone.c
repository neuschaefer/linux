...
#if 1
static int clear_all_nf(struct nf_conn *i, void *data)
{
	return 1;
}

static int reset_nf_conntrack_net_ops(struct file *f, char *buf, size_t cnt, loff_t *pos)
{
    struct net *net = &init_net;

    printk("###%s;%d;###\n", __FUNCTION__, __LINE__);
    local_bh_disable();
    nf_ct_iterate_cleanup(net, clear_all_nf, NULL, 0, 0);
    local_bh_enable();
    return 0;
}

static struct file_operations nf_clear_fops = {
    .owner   = THIS_MODULE,
    .read    = reset_nf_conntrack_net_ops,
};


struct proc_dir_entry *proc_clearconntrack;               
void add_nf_clear_allconntrack(struct net *net)
{
    proc_clearconntrack = proc_create( "nf_clear_allconntrack", S_IRUGO, net->proc_net, &nf_clear_fops);
}

void remove_nf_clear_allconntrack(struct net *net)
{
    remove_proc_entry("nf_clear_allconntrack",net->proc_net);
}
#endif
...
#if 1
    remove_nf_clear_allconntrack(net);
#endif   
	return ret;
}
