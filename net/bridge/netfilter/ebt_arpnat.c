...
__STATIC int do_pppoed_in(struct sk_buff **pskb, const struct net_device *in, int *target, int vlan)
{
	...
	while (tag && tag->tag_type != PTT_EOL && (uint8_t*)tag < (uint8_t*)((*pskb)->nh.raw) + sizeof(*pppoe) + ntohs(pppoe->length))
	{
		if (tag->tag_type == PTT_RELAY_SID)
		{
			//update relay_sid for mac
			if (pppoe->code == PADS_CODE)
			{
				spin_lock_bh(&arpnat_lock);
				update_pppoe_nat(&arpnat_table, tag->tag_data, pppoe->sid);
				spin_unlock_bh(&arpnat_lock);
			}
			memcpy(eth_dmac, tag->tag_data, ETH_ALEN);
			return 0;
		}
		tag = (struct pppoe_tag*)(tag->tag_data + tag->tag_len);
	}
	if (tag && ! (eth_dmac[0] & 1))
	{
		if ((*pskb)->len <= 1484)
			skb_put(*pskb, (2 + 2 + 12));
		tag = (struct pppoe_tag*)(((uint8_t*)(pppoe + 1)) + ntohs(pppoe->length));
		//add relay_sid to the packet
		tag->tag_type = PTT_RELAY_SID;
		tag->tag_len = 12;
		memset(tag->tag_data, 0, tag->tag_len);
		memcpy(tag->tag_data, eth_dmac, ETH_ALEN);
		pppoe->length += (2 + 2 + tag->tag_len);
	}
	return 1;
}
...
__STATIC int do_pppoe_out(struct sk_buff **pskb, const struct net_device *out, int *target, int vlan)
{
	...
	while (tag && tag->tag_type != PTT_EOL && (uint8_t*)tag < (uint8_t*)((*pskb)->nh.raw) + sizeof(*pppoe) + ntohs(pppoe->length) )
	{
		if (tag->tag_type == PTT_RELAY_SID)
		{
			...
			{
				spin_lock_bh(&arpnat_lock);
				update_pppoe_nat(&arpnat_table, eth_hdr(*pskb)->h_source, pppoe->sid);
				spin_unlock_bh(&arpnat_lock);
			}
			tag = NULL;
			break;
	...
	if (tag && ! (eth_hdr(*pskb)->h_source[0] & 1))
	{
		if (sizeof(*pppoe) + ntohs(pppoe->length) <= 1484)
		{
			if ((*pskb)->len <= 1484)
				skb_put(*pskb, (2 + 2 + 12));
			tag = (struct pppoe_tag*)(((uint8_t*)(pppoe + 1)) + ntohs(pppoe->length));
