...
/* Initialices/destroys the structure we maintain.
   Returns -1 on error */
/* Huawei 2007-8-14 BT modify begin 
...
/* Huawei 2007-8-14 BT modify end */ 
...
struct mac_addr {
  ...
  /*start modifying for vdf qos by Huawei 2008.01.16*/
  int mask;                   // mask 
 /*end modifying for vdf qos by Huawei 2008.01.16*/
};
...
struct mac_head {
  ...
  /* Huawei 2007-8-14 BT add begin */
  char usefw;
  /* Huawei 2007-8-14 BT add end */
...
static int mac_classify(struct mac_head *head, struct sk_buff *skb)
{
  ...
  if (head->usefw) {
	...
	/* Start of modified  for qos wrr function 2012-1-12 */
    /*start add :to keep the kernel and cms of qos consistent*/
    //unsigned int uMark = (skb->mark & 0xff);
    /*end add :to keep the kernel and cms of qos consistent*/
	unsigned int uMark = (skb->mark & 0xfff);
    uMark = uMark >> 8;
    /* End of modified  for qos wrr function 2012-1-12 */
#else
...
static struct sk_buff *wrr_dequeue(struct Qdisc *sch)
{
    ...
    /*start of  ADD for : WRR ShapingRate is invalid */
	/* No packet - so machine should be removed from heap */
    if (!b->que->q.qlen)
	    heap_remove(&q->h, band);
    /*end of  ADD for : WRR ShapingRate is invalid */
...
static int wrr_tune_std(struct Qdisc *sch, struct nlattr *opt)
{
	...
	if (qopt->change_class) {
  	/* Huawei 2007-8-21 BT modify begin */
  	int idx;
	if (q->filter.usefw) {
		idx = qopt->addr[0] - 1;
  	...
  	/* Huawei 2007-8-21 BT modify end */
		weight_setvalue
